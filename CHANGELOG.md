...
다이제스트 일괄 제거 · 자리 삭제 · pathspec 삭제 · 자리 목록 축소)이 각각 해당 단언을 RED 로 만든다. 추출기와
판정의 **분기 11개**(리소스 · 컨테이너의 0개 / 중복 · 빈 값 / 비문자열 값 · `latest` · distroless …)도 각 분기를
뒤집는 뮤턴트가 그 분기의 경계 테스트 하나를 RED 로 만든다.
