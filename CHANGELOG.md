...
4. **cross-session staleness 가드**: 비동기 응답(`getStatus`·명령)이 도착하기 전 "새 대화"/"대화 종료" 로 세션이 교체되면 그 응답을 폐기한다 — 옛 세션의 지연 응답이 살아있는 새 대화를 오종료시키거나(410) 유령 표면을 그리지 않는다. `seedWaitingFromStatus` 는 3-state(`"ended"`/`"stale"`/`"continue"`) 반환으로 호출부가 후속 `openStream`/`scheduleRefresh` 진행 여부를 판정하도록 계약을 명시화했다.
