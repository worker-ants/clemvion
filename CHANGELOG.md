...
>    여기다. 판정은 **main 대비**다 — PR 안에서 생겼다 사라진 것은 변화가 아니다.
...
- **frontend 테스트가 어떤 게이트에서도 타입체크되지 않았다**(`#1263`). `frontend-checks` 에 타입체크 ratchet 을 세웠다
  (baseline 51건 · 14파일, 판정 규칙은 backend 와 한 파일 공유). 그 사각이 숨기던 것: `jest-axe.d.ts` 의
...
놓쳤다. 이제 엔티티의 `| null` 필드 전수(135개, 관계 포함)를 술어로 두고 `.spec.ts` 를 훑는다(잔존 0). 같은 PR 의
