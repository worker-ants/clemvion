...
> **항목을 낸다**
>
> 1. **제품 동작이 바뀐다** — 사용자 · 운영자가 관측하는 동작: 기능 추가, 동작을 고치는 수정, 측정되는 성능 변화,
>    API 응답 · OpenAPI 로 광고하는 계약 · 에러 코드 · 감사 기록의 변화. 커밋 타입이 `refactor` 여도 동작이 바뀌면
>    여기다. 판정과 수치는 **main 대비 · 머지 시점**이다 — PR 안에서 생겼다 사라진 것은 변화가 아니고, PR 본문 앞쪽의
>    수치는 뒤 라운드에서 정정됐을 수 있다.
> 2. **배포에 닿는다** — DB 마이그레이션, Dockerfile · compose · k8s, 환경 변수, 런타임 · 툴체인 핀(`packageManager` 등),
>    프로덕션 의존성의 메이저 상향.
> 3. **개발 흐름이 바뀐다** — 가드(특정 결함 클래스를 **전역으로** 막는 검사: CI 잡 · 저장소 스캔 · 정적 검사 · 설정
>    대조 · 훅)를 세우거나 조이거나 **느슨하게** 하는 것, 그리고 하네스가 개발자에게 관측되게 남기던 결함을 고치는 것.
>    «무엇이 뚫려 있었고 무엇이 바뀌는가» 를 적는다.
>
> **항목을 내지 않는다** — 동작이 그대로인 리팩터 · 한 기능의 동작을 고정하는 테스트 추가(가드가 아닌 커버리지) ·
> 의존성 patch · minor 범프(dependabot 포함 — PR 자체가 기록이다) · 개발 도구만 바꾸는 범프(그 도구가 가드면 3) ·
> 문서 · spec · plan · 리뷰 산출물만의 변경.
>
...
가리키는 외래 키가 사용자 삭제를 막는다. 이제 `creator`(작성자 `id` · `name` · `email`)는 required 로, `changeSummary` 는 항상 실리고 값이 null 일 수 있는 필드로
광고된다. 응답 자체는 그대로다 — 생성 클라이언트의 타입이 실제에 맞게 좁아진다.
...
검사가 없었다(바로 아래 항목의 3곳이 그렇게 생겼다). 새 가드 `request-body-advertised` 가 모든 라우트에서 `@Body()` 자리의 설계 타입이
클래스가 아닌데 `@ApiBody` 가 없으면 실패한다(`@ApiExcludeEndpoint()` · `@ApiExcludeController()` 제외, 베이스라인 0).

- 판정은 reflection — 전역 검증 파이프가 건너뛰는 설계 타입 목록을 파이프가 export 하는 그 상수로 본다. 인터페이스 · 타입 별칭은
  런타임에 `Object` 라 소스(AST)로는 클래스와 구별되지 않는다.
- 파라미터를 클래스로 받게 강제하지 않는다 — 클래스로 바꾸면 파이프가 진입해 에러 코드 · 여분 키 처리가 바뀐다. 문서 전용 DTO 를
  `@ApiBody` 로 광고하면 통과다. 규칙: `spec/conventions/swagger.md` §5-4.
...
헬퍼로 맞췄다. 응답 자체(상태 · 본문 · 코드)는 그대로이고 **문서의 설명 문장**만 바뀐다. 실리는 코드도 그대로다.

- `POST /auth/workspaces/:id/switch` — «대상 워크스페이스의 멤버가 아님» → 공용 비멤버 문장.
- `POST /executions/:id/re-run` · `GET /executions/:id/chain` — «… · editor 이상 권한 필요 — RolesGuard / … — 서비스» 형식 → 공용
  역할 문장 뒤에 «… (RERUN_PERMISSION_DENIED — 서비스 판정)».
- 통합 8 · 워크스페이스 4 · 테스트 데이터셋 2 — «A 또는 B, 또는 C» → «A 또는 B 또는 C». 서비스 문장을 잇는 이음을 헬퍼
  `forbiddenWithService` 한 곳이 정한다.

근거: `spec/conventions/swagger.md` §5-4 «문장은 공용 헬퍼로 만들고, 서비스가 내는 403 은 그 뒤에 덧붙인다».
...
중). 응답 자체(상태 · 본문 · 코드)는 그대로이고 **문서의 설명 문장**만 바뀐다 — OpenAPI 로 클라이언트를 만드는 쪽이 어떤 코드가 오는지
알 수 있게 된다.

- 워크스페이스를 받는 라우트는 «워크스페이스 멤버가 아님(`NOT_A_MEMBER`)» 을 싣는다.
- 역할을 요구하는 라우트는 비멤버 코드와 역할 미달 코드를 함께 싣는다 — 예: «워크스페이스 멤버가 아님(`NOT_A_MEMBER`) 또는 Editor 이상
  권한 필요(`EDITOR_REQUIRED`)». 비멤버는 요구 역할과 무관하게 `NOT_A_MEMBER` 라서 종전 «editor 이상 권한 필요» 만 적힌 설명은 비멤버
  코드를 빠뜨렸다. Viewer 요구 라우트는 멤버십과 같아 비멤버 문장뿐이다(종전 «viewer 이상 권한 필요»).
- 표기가 제각각이던 문장(«Admin 미만 권한» · «관리자 권한 필요» · «Editor 미만 권한» 등)을 같은 형식으로 맞췄다.
- 테스트 데이터셋 수정 · 삭제는 서비스 거부 «데이터셋 소유자가 아님(`FORBIDDEN`)» 을 덧붙인다.

근거: `spec/conventions/swagger.md` §5-4 · Rationale «403 설명의 거부 코드».

## Unreleased — 저장소 가드: 403 설명이 가드가 낼 수 있는 거부 코드를 담는다

§5-4 는 새 엔드포인트 체크리스트라 기존 라우트가 따라가지 않았고, 설명이 상수 보간이라 소스만 봐서는 최종 문장을 알 수 없었다.
`src/repo-guards/__tests__/forbidden-response-codes.spec.ts` 가 `src/modules` 컨트롤러를 불러와(reflection) 라우트마다 가드가 낼 수
있는 코드(`@Public` · `@Roles` · 워크스페이스 소비 — `RolesGuard` 와 같은 규칙)를 계산하고, 데코레이터가 평가된 403 설명에 그 코드가
전부 있는지 본다(베이스라인 0, 동결 목록 없음).

- 요구 중 가장 낮은 역할을 고르는 식은 가드와 같은 함수(`lowestRequiredRole`)를 쓰고, 나머지 분기는 대조군에 **실제 `RolesGuard`** 를
  돌리는 캐너리로 모델과 대조한다.
- **못 보는 것**: 설명에 남은 코드(역할을 내린 뒤의 옛 역할 코드)와 서비스가 내는 403. 역할을 바꾸면 설명도 손으로 맞춘다.
- 설명 문장은 공용 헬퍼(`common/swagger` 의 `FORBIDDEN_NOT_A_MEMBER` · `forbiddenForRole`)로 만든다.
...
- e2e 가 대상 라우트에 `[200, 201]` 로 받던 22곳 · 이양 성공 `201` · 저장 성공을 `201` 로 기대하던 4곳을 `200` 으로 조였고, SSE
  라우트의 상태를 보는 e2e 를 더했다.
...
- **`inputOverride` 가 열린 map 으로 광고된다**(`#1206`). 종전 축약형 `type: Object` 는 `type: object` 로 해석되지만
  `additionalProperties` 가 붙지 않아, 생성기가 «선언된 프로퍼티가 없는 닫힌 모델» 로 읽고 빈 인터페이스를 만들었다.
- **`rotate-bot-token` 이 404 와 래핑된 200 응답 DTO(`ChatChannelRotateBotTokenDto`)를 광고한다**(`#1326`). 종전엔
  이 엔드포인트에 OpenAPI 데코레이터가 없었다. 컨트롤러 반환 타입도 서비스 추론형에서 그 DTO 로 바꿔, swagger 선언과
  실제 응답이 조용히 갈리지 않게 했다.

## Unreleased — WS 토큰 만료 타이머가 셧다운을 최대 900초 붙잡던 것 (#1270 CHANGELOG 누락 backfill)

만료 통지 · 연결 끊기 타이머가 event loop 를 붙잡아, 프로세스 종료가 최대 토큰 수명(900초)만큼 늦어질 수 있었다. 두
타이머에 `.unref()` 를 걸었다. **캐비엇**: 그 대가로 그레이스풀 드레인 중엔 만료 **사전 통지**가 발화하기 전에 프로세스가
끝날 수 있다 — 소켓도 함께 사라지므로 실질 영향은 작다고 보지만 관측되면 되돌린다
(`plan/in-progress/ws-token-expired-socket-lifetime-impl.md` 가 추적한다).

## Unreleased — 엔티티 선언이 실제 DB 와 같은지 보는 가드 — 인덱스 · 제약, 그리고 컬럼 층 (#1354 · #1358 CHANGELOG 누락 backfill)

`synchronize: false` 라 엔티티 선언은 스키마를 만들지 않는다 — «이 인덱스 · 제약 · 컬럼이 있다» 고 읽는 사람에게 말할
뿐이다. 그 말이 거짓인 자리가 열일곱이었다.

- **인덱스 · 제약 층**(`#1354`) — e2e 가드 `entity-schema-declarations.e2e-spec.ts` 신설: 선언이 있으면 DB 에도 그대로
  있다(한 방향). 선언 104개를 V001~V132 를 적용한 DB 와 대조해 여덟 곳을 고쳤다 — 없는 인덱스 선언, SQL 로는 컬럼
  이름으로 읽히던 `@Check`, 실제와 다른 FK `onDelete` 등.
- **컬럼 층**(`#1358`) — 같은 가드에 TypeORM 스키마 비교기(`createSchemaBuilder().log()`, DDL 을 실행하지 않고 기록만)를
  얹어 **양방향**으로 본다. 아홉 곳을 고쳤다 — `type: 'uuid'` 누락 다섯, enum 타입 이름 둘, 기본값 둘. `default` 를
  선언한 두 컬럼은 이제 insert 뒤 엔티티에 채워진다(값은 DB 기본값 그대로).

## Unreleased — 가드가 안 보던 자리 넷 — lint 글롭 · frontend 테스트 타입체크 · 도구 태그 잔재 · 인용문 안 체크박스 (#1261 · #1262 · #1263 CHANGELOG 누락 backfill)

- **lint 가 최상위 파일 18개를 건너뛰었다**(`#1261`). 패키지 6/8 의 스크립트가 `eslint src/**/*.ts` 를 따옴표 없이 써서
  `sh` 가 `src/*/*.ts` 로 줄였다 — `parser.ts` · `evaluator.ts` 같은 핵심이 그 자리다. 고치자 에러 하나가 드러났다
  (`parser.ts` 의 `case` 스코프).
- **frontend 테스트가 어떤 게이트에서도 타입체크되지 않았다**(`#1263`). `frontend-checks` 에 타입체크 ratchet 을 세웠다
  (머지 시점 baseline 52건 · 15파일, 판정 규칙은 backend 와 한 파일 공유). 그 사각이 숨기던 것: `jest-axe.d.ts` 의
  `declare module "vitest"` 가 augmentation 이 아니라 shadowing 이라 `toHaveNoViolations` 의 타입 보장이 처음부터 죽어 있었다.
- **도구 태그 잔재 가드**(`#1262`) — `plan/` · `spec/` 에 짝 없는 `</content>` 류 태그가 다시 들어오면 파일:줄로 잡는다.
- **plan 체크박스 정규식이 인용문 안 `[ ]` 를 보게 했다**(`#1262`). 못 보면 최상위가 다 닫힌 문서에서 인용문 안 잔여가
  숨고, Stop nudge 가 «전부 완료» 로 `complete/` 이동을 권하는 경로였다. 그 PR 시점에 판정이 뒤집히는 문서는 0건이었다 —
  숨는 경로를 막는 예방이다.

## Unreleased — 넓혀진 엔티티 필드를 겨눈 낡은 `.spec.ts` 캐스트를 가드가 잡는다 (#1275 CHANGELOG 누락 backfill)

필드가 `| null` 로 넓혀지면 테스트 fixture 의 `<필드>: null as unknown as` 캐스트는 불필요해지는데, 기존 캐스트 가드는
`.spec.ts` 를 의도적으로 보지 않아(부분 객체 캐스트는 정당하다) 구조적으로 못 잡았다 — 배치마다 손으로 찾았고 한 번은
놓쳤다. 이제 **어느 엔티티에서도 non-null 이 아닌** `| null` 필드 이름(관계 포함, 2026-09-04 실측 115개)을 술어로 두고
`.spec.ts` 를 훑는다(잔존 0). 이름이 같아도 다른 엔티티에서 non-null 인 필드는 뺀다 — 그쪽 fixture 의 캐스트는 정당해서,
지우라는 처방이 `tsc` 를 깨뜨린다(그 PR 의 리뷰가 잡은 오탐 20건). 같은 PR 의
repo-guard walker 사본 다섯 통합은 수집 집합이 같음을 실측한 동작 불변 리팩터다.

## Unreleased — 리뷰 산출물 파일에 반환용 프로토콜 헤더가 새던 것 (#1245 CHANGELOG 누락 backfill)

sub-agent 반환 계약이 결과를 파일에 쓰라고만 적고, `STATUS=` 헤더와 구분자가 **파일용인지 반환 메시지용인지** 말하지
않았다. 그래서 반환할 것을 그대로 파일에도 써 리뷰 · 검토 산출물 첫 줄이 프로토콜 헤더가 됐다(1행이 `STATUS=` 인 파일
536개). 계약에서 두 sink 를 갈랐다 — 파일은 마크다운 본문만, 헤더 · 구분자는 반환 메시지에만. 기존 536개는 리뷰 이력을
사후에 바꾸는 일이라 고치지 않았다.

## Unreleased — 인덱스 마이그레이션 21개(V110~V130)가 이 파일에 없던 것 (#1285 · #1349~#1352 CHANGELOG 누락 backfill)

다섯 PR 이 삭제 연쇄와 목록 조회에 쓸 인덱스를 더했는데 모두 이 파일 없이 머지됐다 — 위 기준 2(배포에 닿는다)에
정확히 해당하는데 운영자가 알아야 할 마이그레이션이 한 줄도 없었다. 수치는 각 PR 이 잰 것이다.

| 마이그레이션 | PR | 무엇 | 실측 |
| --- | --- | --- | --- |
| V110 | `#1285` | `idx_schedule_next_run (next_run_at, is_active) WHERE is_active` 를 `idx_schedule_workspace_next_run (workspace_id, next_run_at)` 으로 **교체**(옛 인덱스를 지운다) | 스케줄 목록 5.99 → 0.30 ms |
| V111 | `#1349` | `trigger (workflow_id)` | 워크플로 삭제가 트리거 테이블을 세 번 전부 훑던 것 |
| V112~V116 | `#1350` | 삭제 연쇄의 FK 인덱스 다섯 | 워크플로 삭제 2,225 → 7 ms · 캔버스 노드 삭제 207 → 0.8 ms |
| V117~V120 | `#1351` | 그래프 RAG 삭제 연쇄의 FK 인덱스 넷 | KB 하나 삭제 129,941 → 48 ms |
| V121~V130 | `#1352` | 쓸 인덱스가 없던 FK 서른하나를 처분하며 인덱스 열 | 캔버스 노드 삭제 29 → 0.15 ms |

**배포 시**: 21개 모두 `CREATE INDEX CONCURRENTLY` 를 트랜잭션 밖에서 돈다(동봉 `.conf` 의
`executeInTransaction=false`) — 테이블 쓰기를 막지 않지만 큰 테이블에서는 시간이 걸린다. 중간에 실패하면 invalid
인덱스가 남을 수 있어 각 파일이 같은 이름의 `DROP INDEX CONCURRENTLY IF EXISTS` 를 먼저 둔다 — 재실행이 잔재를 치운다.
...
`cp` 로 원복, `plan/in-progress/` 에 고정 이름 파일, `review/consistency/` 에 세션, 저장소 루트에 임시 디렉터리.
리뷰어 여럿이 같은 워크트리에서 하네스를 동시에 돌리면 한쪽의 원복이 다른 쪽 프로브를 백업해 되살렸다. 실측:
pytest 4개를 동시에 돌린 6라운드 중 **5라운드**에서 spec 에 프로브 줄이 남았고 24개 프로세스 중 22개가 실패했다.
그 사이 누가 커밋하면 무관한 spec 편집이 PR 에 섞인다.

네 테스트 모두 임시 디렉터리에서 돈다. 파일을 바꿔야 하는 프로브는 새 헬퍼 `_harness.make_temp_repo_copy` 가
실제 하위 트리를 임시 git 저장소에 커밋해 준 곳에서 하고, 오케스트레이터에는 그 루트를 cwd 나 인자로 준다 — 두
오케스트레이터 모두 원래 그렇게 루트를 받아 새 주입점이 필요 없었다. 사본은 `origin/main` 이 HEAD 라 변경 집합이
프로브 하나뿐이어서, 순위 단언이 «tier 0 안» 에서 «1위» 로 오히려 좁아졌다. 사본이 `origin/main` 을 HEAD 에 두는
계약은 프로브가 미커밋 변경만 써서 아무 테스트도 보지 않았다(그 줄을 지운 뮤턴트 생존) — 별도 테스트가 고정한다.
`.claude/tests/README.md` 규약에 «이 체크아웃에 쓰지 않는다» 를 적었다.

같은 재현이 고친 뒤 잔여 **0/6** 라운드 · 실패 **0/24** 다. 전수는 하위 프로세스까지 물린 감사 훅과 mtime 비교
두 방식으로 셌고, 감사 훅으로 전체 하네스 동안 이 체크아웃에 쓴 행이 **97 → 0** 이다.
...
하네스 테스트 `test_minio_image_parity.py` 가 세 파일을 YAML 파서로 읽는다. 파일에서 자리를 못 찾으면 그 자리를
**이름으로 대며** 실패하고(서비스 이름이 바뀐 경우), 그 위에서 네 가지를 고정한다: 선언된 자리 목록이 **여섯에서
줄지 않는다** · 여섯 값이 **같다** · 각각 **태그 + 다이제스트**로 고정이고 태그가 `latest` 가 아니다 ·
**`-distroless` 가 아니다**(compose 헬스체크가 이미지 안 `curl` 을 쓴다). 뮤턴트 다섯(k8s 한 곳만 옛 이미지 ·
다이제스트 일괄 제거 · 자리 삭제 · pathspec 삭제 · 자리 목록 축소)이 각각 해당 단언을 RED 로 만든다.

구조 검사(매핑 경로 · 시퀀스 · «정확히 하나» · 이미지 값)는 **각각 헬퍼 하나에만** 있고 추출기는 배선만 한다 —
리뷰가 네 라운드 연속 같은 형태를 자리 하나씩 안쪽에서 찾았는데, 원인이 검사를 자리마다 복제한 것이었다. 헬퍼 ·
배선 · 판정(`latest` · distroless · 뒤따르는 개행)을 뒤집는 뮤턴트 **16개가 전부** RED 로 잡힌다.
...
코드는 `[204, 404]` · 감사 1건). 워크스페이스 경로도 **실 DB 로 고정했다** —
`workspace-delete-concurrency.e2e-spec.ts` 에서 404 단락 분기를 지운 뮤턴트가 기대 404 자리에 **403 을**
돌려주는 것을 재현했다(리뷰가 읽기로 지적한 값이 그대로 나왔다). 단위 뮤테이션도 함께 돈다 — (1) 404 단락
분기를 지우면 새 단위 테스트가 RED, (2) `NotFoundException` 재던짐 가드를 지우면 `Logger.error` 미호출
단언이 RED(거짓 「트리거가 발화하지 않을 수 있다」 로그가 실제로 찍힘). **단위만으로는 부족했다**: mock 은
`parentPresence: 'absent'` 를 직접 주입해 «이긴 쪽이 멤버 행까지 지운다» 는, 403 을 만든 바로 그 현실을
재현하지 못한다. 워크플로 쪽 로그 억제 가드도 같은 방식으로 확인 — 가드를 지워도 기존 단언 9개는 전부 GREEN 이었고
...
비어, 그 경로를 아는 누구든 자기 워크스페이스에 다시 등록해 옛 URL 로 오는 요청을 받을 수 있었다. 경로를 아는 사람은 넓다 —
웹챗은 `endpointPath` 가 외부 사이트 스니펫에 박히는 공개 UUID 라 그 사이트를 연 누구나 알고, 인스턴스를 지워도 사이트에 남은
위젯은 옛 경로로 계속 요청한다.

**고친 것** (spec 데이터 모델 §2.8.1 · Rationale «지운 · 바꾼 웹훅 경로의 영구 예약»):
- 트리거가 경로를 **처음** 가질 때(생성 · 경로 변경) 그 경로를 트리거의 워크스페이스 소유로 예약하고, 예약은 지우지 않는다(V133
  `webhook_endpoint_reservation`). 다른 워크스페이스는 지운 · 바꾼 경로를 영원히 쓸 수 없고, **같은 워크스페이스는 다시 쓸 수
  있다**(지운 트리거를 같은 URL 로 다시 만들거나, 바꿨던 경로로 되돌리기). 워크스페이스를 지우면 예약은 주인 없이 남아 누구도 못
  쓴다.
- 거부는 지금 쓰는 트리거와 겹칠 때와 **같은** 409 `RESOURCE_CONFLICT`(`details.code=TRIGGER_ENDPOINT_PATH_CONFLICT`) — 경로가
  한때 쓰였는지를 응답으로 알려 주지 않는다. 메시지는 «쓸 수 없는 경로» 로 바꿨다(«이미 다른 트리거가 쓰고 있다» 는 예약만
  남은 경로에는 거짓이다).
- 강제는 `trigger` 의 DB 트리거가 한다 — 서비스가 아닌 쓰기(수동 SQL)도 막는다. 수신(`/api/hooks/:endpointPath`)은 그대로다.

**배포 뒤 보일 수 있는 것**: V133 이 그때 살아 있는 트리거의 경로만 예약한다 — 이미 지워진 경로는 기록이 없어 보호되지 않는다.
같은 URL 을 다른 워크스페이스로 옮기려던 사용자는 409 를 받는다. 마이그레이션은 커밋까지 `trigger` 에 쓰기 잠금을 잡는다(백필 한
문장 동안 — 트리거 생성 · 수정 요청이 그만큼 기다린다).
...
`(workspace_id, endpoint_path)` — 워크스페이스 단위뿐이었다. `endpoint_path` 는 클라이언트가 만들어 보내고 나중에
바꿀 수도 있어서, 경로를 **알고 있는** 사람(그 워크스페이스의 뷰어 · 전 멤버 · URL 을 받은 외부 서비스)이 자기
워크스페이스에 같은 경로로 트리거를 만들 수 있었다. 수신 조회 셋(수신 · 공개 가드 · 웹챗 embed-config)은 워크스페이스
없이 · 정렬 없이 한 행을 고르므로, 웹훅이 복사한 쪽으로 갈 수 있었다 — 옛 스키마에서 재현했다(워크스페이스 id 가 더
작은 쪽이 나중에 복사하자 조회가 복사한 쪽을 골랐다). spec 은 «UUID 의 고엔트로피가 squatting 을 막는다» 는 전제만
적었는데, 고엔트로피는 **추측**을 막을 뿐 **복사**는 막지 못한다.

**고친 것** (spec 데이터 모델 §3 · Rationale «Webhook `endpoint_path` 전역 유일», 12-webhook WH-SC-01):
- `(endpoint_path)` 전역 UNIQUE 가 워크스페이스 단위 UNIQUE 를 교체한다(V132). 다른 워크스페이스의 트리거와 겹치는
  경로로 생성 · 수정하면 409 `RESOURCE_CONFLICT`(`details.code=TRIGGER_ENDPOINT_PATH_CONFLICT`) — wire 형태는 그대로이고,
  메시지는 더 이상 «같은 워크스페이스» 를 말하지 않는다.
- 기존 중복은 V131 이 정리한다 — 경로가 같은 묶음마다 가장 먼저 만든 트리거만 경로를 유지하고 나머지는 새 UUID 를
  받는다(복사는 원본보다 나중에만 생길 수 있다). 정상 경로로는 워크스페이스 간 중복이 생기지 않아 중복이 있다면 복사
  등록의 흔적이다. NOTICE 에는 트리거 id · 워크스페이스 id · 채팅 채널 여부만 남는다(경로는 비밀 키라 남기지 않는다).
- 수신 조회도 이 인덱스로 한 번에 찾는다(웹훅 트리거 5만에서 0.200 → 0.025 ms — 부수 효과).

**배포 때 할 것**: V131 NOTICE 에 `chat_channel=t` 가 있으면 그 트리거의 채팅 채널 provider 등록은 옛 경로에 남아 있다 —
SQL 은 provider API 를 부를 수 없다. 소유 워크스페이스에 알려 채널 설정을 **다시 저장**하게 한다(`setupChannel` 이 새
경로로 재등록). 일반 웹훅이 새 경로를 받았으면 그 URL 을 외부 서비스에 다시 등록해야 한다 — 역시 NOTICE 대상에게 알린다.

**남는 창**: 전역 UNIQUE 는 **동시에 존재하는** 중복만 막는다. 주인이 트리거를 지우면 그 경로는 비고, 경로를 아는 누구든
다시 등록할 수 있다(묘비 부재 — 트래커 등재, 위 «지우거나 바꾼 웹훅 경로를…» 항목이 닫았다). V131 과 V132 사이에 복사가 끼어들면 V132 가 실패하지만 옛 인덱스는 유효하게
남는다 — 복구 절차는 V132 헤더에 있다.

## Unreleased — 워크플로·워크스페이스를 지워도 트리거의 자원이 남았다

트리거 행을 지우는 경로는 넷(트리거 · 스케줄 · 워크플로 · 워크스페이스 삭제)인데, 트리거가 쓰던 자원을
정리하는 곳은 **트리거 삭제 하나뿐**이었다. 워크플로·워크스페이스 삭제는 FK CASCADE 로 트리거를 지우면서
BullMQ job scheduler(발화마다 `Schedule not found` warn)·chat channel provider 의 콜백 등록·`secret_store`
의 암호화된 bot token·서명 비밀을 **전부 남겼고**, 스케줄 삭제는 비밀을 남겼다.

**고친 것** (spec 트리거 목록 §3·§4.3·§4.4):
- 네 경로가 모두 정리한다. 외부 자원은 행 삭제 **전**·트랜잭션 **밖**(teardown 이 비밀을 읽는다),
  비밀은 행 삭제 **커밋 뒤**. 트리거 삭제도 비밀 순서를 뒤집었다 — 전에는 락 대기 5초를 넘기면 «비밀까지
  지워졌는데 행은 남은» 트리거가 남았다.
- 삭제와 겹친 쓰기(PATCH 정규화 · bot token 회전 · 승격 cron · chat channel setup)가 락 안 재기록에서 행
  부재를 보면, 락 밖에서 쓴 비밀과 provider 등록을 **스스로 되돌린다**. 삭제 쪽 순서와 짝을 이뤄 비밀 쪽
  인터리빙이 닫힌다(실제 Postgres 에서 «정리 → 늦은 쓰기 → 0행 → 보상» 순서를 고정해 확인).
- 워크플로·워크스페이스 삭제는 부모 행을 잠근 뒤 같은 트랜잭션에서 트리거를 열거한다 — 잠금 뒤엔 새
  트리거 INSERT 가 FK 검사에서 막혀 비밀 정리 대상에서 빠지지 않는다.
- 워크스페이스 삭제는 권한 검사를 외부 해제보다 **먼저** 한다(안 그러면 403 요청이 provider 등록부터 뜯는다).
  잠금 순서도 소유권 이전과 같게(워크스페이스 → 멤버십) 맞춰 두 요청이 겹칠 때의 교착을 없앴다.

**리뷰가 잡은 것**: 여러 스케줄을 가진 부모를 지우다 k 번째 job 해제가 실패하면, 앞의 job 은 Redis 에서
사라졌는데 삭제는 취소돼 «활성인데 발화하지 않는» 스케줄이 남았다. 이제 전부 시도하고, 실패가 있으면 이미
해제한 **활성** job 을 다시 등록한 뒤 삭제를 멈춘다. 그리고 워크플로·워크스페이스 삭제는 외부 자원을 되돌릴 수
없게 해제한 **뒤** 부모 행을 잠그는데 그 잠금에 상한이 없어(Postgres `lock_timeout` 기본 0), 동시 트랜잭션과
겹치면 반쯤 삭제된 상태가 hang 으로 굳을 수 있었다 — 트리거·스케줄 삭제와 같은 5초 상한을 걸었다(멤버십 잠금·
CASCADE 되는 트리거 행 잠금에도 걸린다).

**남는 창**(spec §4.3): 외부 해제용 열거 뒤에 생긴 트리거의 외부 자원, 해제 뒤·행 삭제 전에 동시 요청이
다시 만든 provider 등록·schedule job, 커밋과 비밀 정리 사이의 프로세스 종료. 워크스페이스는 선검사와 잠금
재검사 사이의 역할 변경도 — 이 경우들은 error 로그로 드러난다.

**테스트가 스스로 틀렸던 것**: schedule job 해제를 `Queue.getJobScheduler(id)` 의 `undefined` 로 판정했는데,
bullmq 5 는 id 에 `:` 가 있으면(`schedule:<id>`) 해제 뒤에도 껍데기 객체를 돌려준다 — «해제됐다» 는 항상
실패, «남아 있다» 는 항상 참이었다. 스케줄러 목록(zset) 소속으로 바꿨다.

## Unreleased — PATCH 가 동시에 커밋된 컬럼을 옛 값으로 되돌렸다 (이론적 TOCTOU 가 아니었다)

`PATCH /api/triggers/:id` 가 이름만 바꿔도, 그 요청이 트리거를 락 안에서 **재읽은 뒤** 락 밖에서
커밋된 컬럼을 **옛 값으로 되돌렸다**. 되돌아가는 것은 `rotateNotificationSecret` 이 쓴
`notification_secret_v2`(24h grace 회전 상태), 웹훅 인입의 `last_triggered_at`, cron 이 지운
`chat_channel_token_v2`, 스케줄 편집이 동기화한 `name`·`is_active` 다.

원인은 저장 방식이다. 창 1(`TriggersService.update()`)은 재읽은 엔티티를 **통째로** `save` 했는데,
TypeORM `save` 는 저장 시점의 DB 값과 **엔티티가 다른 컬럼**을 되쓴다 — 재읽기 뒤에 커밋된 컬럼은
엔티티에 옛 값으로 남아 있으므로 «다르다» 로 잡혀 되써진다.

`#1334` 는 이 자리를 «락 밖 컬럼 갱신 3곳의 이론적 TOCTOU» 로 유예했다. 실제 Postgres 에 TypeORM
을 붙여 그 창을 결정적으로 재현하니(트랜잭션 A 재읽기 → 연결 B 컬럼 커밋 → A 저장) 두 컬럼이
그대로 `null` 로 돌아갔다.

**고친 것**: 저장 대상을 **이 요청이 바꾸는 필드만 담은 부분 객체**로 좁혔다. 동사는 `save` 그대로다
— `update` 로 바꿨다 되돌린 이력이 있다(반환 엔티티·`endpointPath` 충돌 경로). 넘기지 않은 컬럼은
비교에서 빠지므로 락 밖 커밋이 보존된다. 락을 공유하는 형제 창이 쓴 컬럼도 «재읽은 값을 다시
싣는» 방식이 아니라 «싣지 않는» 방식으로 보호돼, 타이밍과 무관해졌다.

**함께 실측으로 확정한 것**:
- 재읽기 뒤 `workflow` 삭제(FK CASCADE)가 끼어들면 저장은 **시끄럽게 실패**하고 롤백된다
  — 트리거가 되살아나지 않는다(부분 객체면 23502, 통째면 23503).
  `workspace` 삭제도 같은 `ON DELETE CASCADE` 구조(`trigger.workspace_id`)라 동일할 것으로
  보지만 **따로 재지 않았다**.
- main 에 올라간 `pg` 드라이버에서 없는 행 `update` 의 `affected` 는 `0` 이다 — `#1341` 의 0행 판정이
  실제로 동작한다.

**PR 안에서 낸 회귀를 e2e 가 잡았다**: 부분 객체 `save` 의 **반환값**을 «DB 재조회 값» 으로 오해해
응답에 통째로 덮었더니, 반환값은 넘기지 않은 nullable 컬럼을 `null` 로 채운 객체라 `endpointPath` 가
지워지고 `chatChannel` PATCH 가 전부 400 이 됐다. 단위 테스트는 mock 이라 613건 전부 GREEN 이었다.
응답은 이제 `updatedAt` 하나만 반환값에서 취한다.
...
> **갱신(2026-09-17) — 이 창은 이제 실측됐다.** 운영 코드에 훅을 넣지 않고 TypeORM 을 실제
> Postgres 에 직접 붙여 재현했고, 추정대로 **시끄럽게 실패**하며 롤백·부활이 없었다(통째
> `save` 23503). 같은 측정에서 이 창이 **락 밖 컬럼 쓰기를 되돌리는** 실결함도 드러나 고쳤다 —
> 항목 «PATCH 가 동시에 커밋된 컬럼을 옛 값으로 되돌렸다» 참조. 위 «아직 재지 않았다» 는
> 이 항목이 쓰인 시점의 서술로 남긴다.
...
`CONTAINER_MULTIPLE_EMIT` **로 실행 실패해요**"* 라고 적어 두 이름이 `error.code` 로
나가는 것처럼 읽혔다. 실제로는 **실패 메시지의 접두**이고 구조화된 코드로는 발행되지
않는다(`execution-engine.service.ts:7121·7125`). 문장을 *"실패 **메시지 앞에** 붙어요 —
전용 에러 코드는 없으니 코드가 아니라 메시지를 봐야 해요"* 로 고쳤다(KO/EN).

`guide-identifier-existence` 가드에 **발행 축**을 더해 같은 형태가 다시 들어오는 것을 막는다:

> 인용 토큰이 소스에 **메시지 접두로만** 등장하고 `3-error-handling.md` 카탈로그에도
> 없으면, `GUIDE_NON_EMITTED_VOCABULARY` 에 **어디서 접두가 붙는지 + 사유**를 적어야
> 통과한다.

**카탈로그를 요구 조건이 아니라 «탈출구» 로 쓴다.** 요구 조건으로 쓰면 오늘 거짓 RED 가
25건 난다 — 가이드가 인용하는 에러 코드 78종 중 28종이 카탈로그 미등재이고 그중 25종은
**진짜 발행되는** 통합 코드다(별건으로 트래커에 등재된 planner 결함).

> **그 탈출구는 오늘 한 번도 발화하지 않는다.** 처음엔 *"`MAX_ITERATIONS_EXCEEDED` 처럼
> 접두로만 발행되지만 spec 이 정식 코드로 인정한 것이 탈출구로 통과한다"* 고 적었는데
> **틀렸다** — 그 토큰은 소비자 Set(`execution-failure-classifier.ts`)이 정확 리터럴로
> 인용해서 **접두-전용 단계에서 이미 탈락**하고, 카탈로그 검사에 도달하지 않는다.
> 전수로 세면 «접두 전용 ∩ 카탈로그» 는 인용·비인용 통틀어 **공집합**이다.
>
> 그래도 남긴 이유는 트래커의 planner 항목(*"`CONTAINER_*` 를 §1.4 에 backfill"*)이
> 집행되는 순간 발화하기 때문이고, **0회 발화라는 사실 자체를 테스트가 단언으로 고정**한다.

**잔여 한계**: 소비자·분류기 목록이 토큰을 따옴표로 인용하면 접두-전용 판정이 풀려
통과한다. `3-error-handling.md §1.4` 가 *"소비자·분류기 쪽 어휘이지 발행 경로의 앵커가
아니다"* 라고 그 구분을 명시하지만, 기계적으로 가르려면 **앵커 없는 코드 7종**(같은 §1.4 가
정식으로 인정한다) 문제를 먼저 풀어야 한다.
...
- `guide-identifier-existence` — 가이드가 적은 **식별자**(에러 코드 + 환경변수)가
  실재해야 한다. 기준집합 = backend·packages 소스 토큰 **∪ env 선언처**. 3축
  (`<FieldTable>` 의 `name` · `code:` 값 · **백틱 전수**), 베이스라인 0.
  외부 어휘 허용목록(`GUIDE_EXTERNAL_VOCABULARY`)이 있고 테스트가 넷을 강제한다 —
  외부 시스템 이름 의무 · 상한 · 여전히 인용될 것 · 기준집합에 없을 것.
  **한계**: *존재* 검사이지 *방출* 검사가 아니다 — 에러 **메시지 접두**로만 쓰이는 토큰,
  아무도 읽지 않는 env 변수도 통과한다. 사유와 후속 축은 가드 주석·트래커에 있다.

  > **이 항목은 두 PR 에 걸쳐 두 번 바뀌었다.** 처음 판(`guide-error-code-existence`)은
  > *에러 코드 전용* 이었고 **허용목록이 없었다** — 그 두 성질이 합쳐져 이 가드를 등재시킨
  > 과거 결함(가이드가 적던 환경변수 오기)을 **못 잡았다**. 문맥 게이팅으로 오탐을 0 으로
  > 만들면서 **잡아야 할 것까지 빠졌고**, 베이스라인 0 이 그것을 가렸다. 후속 PR 이 축을
  > 백틱 전수로 넓히고 허용목록을 **4강제와 함께** 도입했다.
...
**⚠️ 배포 시 확인**: `?workflowId=…` 를 보내던 요청은 이제 400 을 받는다. 연동 이슈가
보고되면 이 항목을 먼저 보라.

저장소 안에서 확인한 것 — 서비스가 읽지 않고(`findByWorkflow` 는
...
약속하며, OpenAPI 코드젠 소비자가 없고, e2e 292건이 그대로 통과한다.
...
지켜지지 않은 것이므로, 지켜지는지 검사하는 AST 가드를 뒀다 — 앞으로 생길 **helper** 도 잡는다.

가드의 범위는 `recordAudit` 이름의 helper 선언이다. `record()` 호출 27곳을 `action` 의 형태로
갈라 보면 그 경계가 왜 거기인지 보인다:

| `action` 형태 | 개수 | 성격 |
|---|---|---|
| 리터럴 / `AUDIT_ACTIONS.X` — 같은 객체 리터럴에 `resourceType` 이 나란히 | 22 | 양쪽이 한눈에 보인다 |
| `params.action` — 파라미터 | **5** | **한쪽이 숨는다** (= 그 5개 helper) |

helper 는 `resourceType` 을 고정해 놓고 `action` 만 받으므로 **호출자가 불일치를 볼 수 없다**.
인라인 22곳은 두 값이 같은 리터럴에 붙어 있어 성격이 다르다. 가드가 helper 만 보는 것은
그래서다 — 다만 그 22곳도 타입으로 강제되지는 않으므로, 호출식까지 넓히는 것은 후속으로
등재했다(`plan/in-progress/spec-sync-auth-gaps.md`).
판정은 **값이 아니라 형태**로 한다(액션이 추가될 때마다 가드를 고치지 않아도 되고, 정작
"묶이지 않았다" 는 구조적 사실을 놓치지 않는다).

> **보강 (리뷰 5R).** 처음 가드는 "무언가에 **묶였는가**" 까지만 봤다. 그 술어는 지키려는
> 불변식보다 한 칸 좁아서, `AuditActionFor<'workflow'>` 로 선언하고 `resourceType:
> 'auth_config'` 를 기록하는 **엉뚱한 리소스에 묶인** 형태를 통과시켰다.
> `findMisboundHelpers` 로 제네릭 인자와 실제 `resourceType` 을 비교해 닫았다 — `typeof CONST`
> 와 `'literal'` 표기는 상수 해석으로 정규화한다.
>
> 유예 근거로 적었던 "`_NoCrossDomain` 캐너리가 이미 막는다" 는 **뮤테이션에 반증됐다**.
> 오귀속 뮤턴트는 `tsc` 에러 5건을 내는데, 캐너리를 함께 제거해도 **5건 그대로**였다 —
> 잡는 주체는 캐너리가 아니라 **호출부의 액션 리터럴**이다(에러 5건이 전부 호출부에 찍힌다).
> 따라서 진짜 갭은 **호출부가 아직 없는 helper**이고, 새 가드는 선언 단계에서 그것을 잡는다.
...
## Unreleased — (부수) 로그인 실패 카운터가 아바타 URL 을 되돌리고 있었다
...
분류기가 **값으로 분기**한다. 그런데 엔진이 쓰는 코드 넷은 상수도 타입도 없는 맨 문자열이었다
— `EXECUTION_QUEUE_WAIT_TIMEOUT` · `WEBCHAT_IDLE_TIMEOUT`(둘 다 `const code = '…'`) ·
`WORKER_HEARTBEAT_TIMEOUT` · `SERVER_INTERRUPTED`(×2). **오탈자를 잡는 것이 아무것도 없었다.**
한 글자 틀리면 그 오타가 DB 에 들어가고 소비처는 조용히 else 로 떨어진다. 여기에 값은 이미
enum 에 있으면서 상수를 안 거치던 `ai-turn-orchestrator` 의 `LLM_*` 4지점을 더해 **9지점**을
상수 참조로 바꿨다.

**원 티켓의 처방("`error-codes.ts` 엔진 레벨 레이어 *분리*")은 재고 나니 틀렸다.** 저장소
전체에서 `code:` 리터럴 201개가 `ErrorCode` 밖에 있지만 대다수(`ACCOUNT_LOCKED`·
`WORKSPACE_NOT_FOUND` 등)는 **API 예외 코드**로 애초에 그 enum 소관이 아니다 — docstring 이
범위를 *"node handlers' `output.error.code`"* 로 못박고 있어 "혼재" 는 과장이었다. 게다가 같은
파일이 WS ack 용 `RETRY_*` 를 품으며 *"canonical code strings 는 **one source of truth**"* 를
근거로 적어 두었으니, 파일을 나누면 그 원칙이 깨진다. 그래서 **파일은 하나, const 는 둘** —
...
**설계의 전제도 테스트로 고정했다** — *"파일은 하나, const 는 둘"* 이 성립하려면 두 const 가
겹치지 않아야 한다. 겹치면 "레이어를 타입으로 가른다" 는 주장이 무너지고 앵커 가드도 그 값을
어느 쪽 근거로 통과시킨 것인지 말할 수 없다. 키 교집합이 빈 집합인지 단언한다.
...
통째로 놓쳤고, `workflow-errors.ts` 의 **주석 속 예시**를 위반으로 오탐했다. AST 가 둘 다 해결한다.

**뮤테이션이 가드 설계를 한 번 뒤집었다.** 첫 판은 *"모르는 코드 **값**"* 을 찾도록 짰는데,
이미 enum 에 있는 값을 맨 문자열로 되돌려도 통과했다 — 막으려던 회귀를 그대로 통과시킨 것이다.
막으려는 것은 값이 아니라 **"상수를 안 거치는 사이트"** 라 판정을 **형태**로 옮겼고, 그제서야
네 축(리다이렉트 되돌림 · 수집 형태 누락 · UPPER_SNAKE 필터 · 예외 목록)이 전부 RED 가 됐다.
...
`hasRawUpdateReturning`/`countRawUpdateReturning` 전용 단위 테스트를 신설해 판정 축(**양성 7·
음성 8**)을 합성 문자열로 직접 고정했다 — 그 전에는 이 판정이 "오늘의 실제 소스가 우연히 그
형태를 담고 있는가" 에만 의존했다. 음성에는 **의도된 한계 셋**(변수 SQL `.query(sqlVar)` ·
2단계 이상 중첩 제네릭 · CTE 접두 `WITH … UPDATE … RETURNING`)이 포함된다 — 고칠 대상이
아니라 **한계를 RED 방향으로 고정**한 것이라 테스트 이름에 그렇게 적었다.

양성에는 **멀티라인 백틱 리터럴**(`UPDATE` 와 `RETURNING` 이 다른 줄)도 들어간다. 실제 지점의
지배적 형태인데 이 축만 오늘의 소스에 결합돼 있었다 — 뮤테이션으로 재니 개행 금지 시 RED 4 중
3이 실제 소스를 읽는 테스트였다. 캐너리는 그 결합을 끊는다. 직전 커밋 `#1235` 가 자매 가드에서
정확히 이 멀티라인 축으로 무너졌다.

허용목록도 파일 단위 전면 면제에서 **(파일, 사유, 그 사유가 검토한 지점 수)** 로 좁혔다.
전면 면제였을 때는 이미 허용된 파일 안에 두 번째 미가드 지점이 생겨도 통과했다. 그리고 그
판정 전체를 파일시스템과 무관한 순수 함수(`findUnguarded`)로 뽑아, **부분 커버리지**·
**허용 수 초과**·**다중 unguarded 전부 보고** 를 합성 스텁으로 고정했다 — 그 전에는 이
하드닝을 지키는 영속 테스트가 없어 검증이 매번 임시 프로브에 의존했다.

허용목록의 **선언 개수 자체가 참인지**도 별도로 조인다. `findUnguarded` 는 `rawCount >
allowedCount` 만 보므로 선언값에 대해선 **상한 검사**일 뿐이라, 오타로 부풀리면(`1` → `99`)
그 파일에 새로 생긴 미가드 지점이 조용히 통과한다. 선언값이 `discover()` 실측과 **정확히
일치**하는지 보는 테스트를 더해 반대 방향을 닫았다 — 뮤테이션으로 재니 이 테스트만 RED 이고
나머지 22개는 GREEN 이었다(기존 스위트가 못 보던 축이라는 뜻).
...
Re-run 프리필이 읽어 그대로 재제출하므로 마스킹하면 `'***'` 가 새 실행의 실제 입력이 됐다.
§R17 은 이 예외를 "영구" 가 아니라 **조건부**로 적었고, 닫는 조건이 *"프런트가 마커를 감지해
재입력을 **강제**하는 가드"* 였다. 세 소비처가 전부 갖추면서 닫는다.

> **닫힌 범위를 정확히 적는다 — UI 정상 흐름이다.** 가드는 프런트 렌더 경로에 있으므로,
> `curl` 등으로 `POST /re-run` 을 직접 호출해 `inputOverride` 에 리터럴 `'***'` 를 실으면
> 서버는 여전히 받는다(타입·필수값만 본다). 기밀성 문제는 아니고 — 이미 마스킹된 값을
> 쓰는 **반대 방향**이라 — 피해는 호출자 자신의 새 실행에 한정된다. 서버측 거부는
> 트래커 항목(`spec-sync-external-interaction-api-gaps.md`)으로 남겼고, 착수 시
> **planner 턴으로 §R17 에 범위를 명문화**하는 것까지 함께 한다.

**두 가드의 형태가 다르다.** Re-run 모달은 필드 단위라 스칼라 마커를 비우고 **그 키가
안전해질 때까지 제출을 막는다**(판정은 아래 세 조건) — 비우고 안내만 하면 오염의 값이
`'***'` → `''` 로 바뀔 뿐이다. 토글 ON
(`useOriginalInput`)은 서버가 엔티티를 직접 읽으므로 **막지 않는다**(오히려 정답인 경로).
에디터 히스토리 로드는 JSON 텍스트 전체라 비우면 *어느* 필드가 가려졌는지가 지워져서,
마커를 보여 주고 Run 만 막는다.

**object/array 안쪽 마커도 본다** — 첫 구현은 정확 일치만 봐서 `{"headers":{"apiKey":"***"}}`
가 통째로 뚫렸다(리뷰가 CRITICAL 로 잡았다). 이쪽도 값은 지우지 않고 제출만 막는다.

**차단 판정은 세 조건의 합이다** — *"사용자가 그 키를 건드렸다"* **그리고** *"현재 값에
마커가 없다"* **그리고** *"구조 필드라면 JSON 파싱에 성공했다"*. 리뷰 **세 라운드**에 걸쳐
좁혔는데, 각 조건 단독으로는 서로 다른 경로로 뚫리기 때문이다:

- **값만** 보면 — 스키마가 늦게 로드될 때 재조정이 `coerceInput("boolean","")` 을 돌려
  비워 둔 값이 `false` 가 되고 차단이 조용히 풀린다.
- **터치만** 보면 — 한 번 건드린 뒤 값을 **다시 마커로 되돌려도** 영구 해제된다.
- **앞의 둘만** 보면 — object/array 필드를 **무효 JSON** 으로 만들면 `coerceInput` 이 raw
  문자열로 폴백하고, 문자열은 정확 일치만 보므로 마커로 안 잡혀 또 풀린다.

마커 판별기는 `lib/utils/masked-markers.ts` 로 승격했다(소비처 셋). backend 는 두 관문에
...
잔여**다. 무수정 프로브로 실증했다 — `defaultValue: 'Bearer sk-live-ABC'` 가 `'***'` 로
마스킹된 채 폼에 **프리필**되고, 사용자가 손대지 않고 제출하면 그 리터럴이 실제 폼 값이 된다.

Re-run 모달 케이스와 **같은 클래스** — *읽혀서 되쓰이는 값에 마스킹을 걸면 가시성이 아니라
데이터 무결성 문제가 된다*.

**그런데 여기서는 카브아웃을 쓸 수 없다.** `Execution.inputData` 는 외부 노출이 없어 마스킹을
끄는 것이 안전했지만, `formConfig` 는 `execution.waiting_for_input` 을 타고 **SSE·notification
webhook 으로도 나간다** — 끄면 외부 누출이 열린다. 그래서 마스킹은 유지하고 **소비 쪽에서
마커를 감지**해(`isMaskedMarker`) 프리필을 건너뛰고 재입력을 안내한다. §R17 에 *"외부로도
나가면 마커 가드, 안 나가면 카브아웃"* 판단 기준을 명문화했다.

**보장의 경계는 정확 일치다 (의도)** — `postgres://***@db` 처럼 마커를 *포함*할 뿐인 부분-매치
결과는 계속 프리필된다. 포함-매치로 넓히면 `a***b` 같은 정상 기본값까지 비워져 가드가 정상
워크플로를 망가뜨린다. 캐너리 테스트로 이 경계를 못박았다.

마커 집합은 backend `sanitize-error-message.ts` 가 SoT 이고 프런트가 미러한다 — 동기화를
grep 으로 하므로 **이름을 양쪽 동일**(`MASKED_MARKERS`/`isMaskedMarker`)하게 두고 상호참조
주석을 달았다.
...
서버가 엔티티를 직접 읽어 영향 없다. 회귀 캐너리로 비대상임을 고정했다.
(**닫는 조건**인 프런트 마커 가드는 위 항목이 폼 프리필에 세웠다 — Re-run·히스토리 로드로
확장하면 이 컬럼도 닫힌다.)
