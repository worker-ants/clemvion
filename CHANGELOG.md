...
## Unreleased — 웹채팅 위젯: 마지막 `wc:boot` 적용(§106) + 종료된 대화 부활 fix

host 는 iframe 재생성 없이 `wc:boot` 을 재전송해 boot config 를 갱신할 수 있고, spec 은 **"위젯은 마지막 `wc:boot` 의 config 를 적용"** 한다고 정한다(`2-sdk §106`). 그 계약이 구현돼 있지 않았다 — 겹친 부팅에서 **`embed-config` 왕복의 resolve 순서가 승자를 정했다**(먼저 보낸 config 가 나중에 응답하면 그게 이겼다). 부팅 시도 세대를 도입해 나중 `wc:boot` 이 앞선 시도를 대체하게 했다.

1. **종료된 대화 부활 (사용자 가시 버그 수정)**: `ERROR`(명령 실패 등)는 대화를 `[ended]` 로 보내면서도 **세션을 정리하지 않는 유일한 종료 경로**였다. 그래서 SSE 가 열린 채 남고 저장 세션도 남아, host 가 `wc:boot` 을 재전송하면 그 세션이 복원돼 **실패해 끝난 대화가 이유 없이 되살아났다**(화면 복귀 + `getStatus` 재조회 + SSE 재오픈 + 토큰 갱신 예약). 이제 에러도 다른 종료와 동일하게 세션을 정리하며, 상태 리듀서에도 "종료된 대화는 복원으로 되살아나지 않는다" 최후 방어선을 뒀다.
2. **마지막 config 적용(§106)**: 겹친 부팅에서 나중 `wc:boot` 이 앞선 시도를 대체한다. 대체된 시도는 config 를 적용하지도, 차단 화면을 띄우지도, 접수된 "새 대화" 를 이행하지도 않고 물러난다 — 그 리셋 요청은 소실이 아니라 **다음 성공하는 부팅으로 이월**된다.
3. **대체된 시도가 살아있는 부팅을 죽이지 않는다**: 대체된 시도가 복원 중 "이미 종료된 세션" 을 발견해도 종료를 **확정하지 않는다**. 확정은 정당한 세션 무효화라 살아있는 마지막 부팅까지 함께 무효화해 §106 을 깨뜨렸기 때문 — 종료가 유실되진 않고 살아있는 시도가 같은 스냅샷을 보고 확정한다(주체만 바뀐다).

SoT: `spec/7-channel-web-chat/2-sdk.md §106`. 관리자 라이브 미리보기가 외형 폼 변경 시 이 재전송 경로를 쓴다.
