...
- `execution-engine.service.ts` — `assertFormSubmissionValid` 내부 분기 리팩터(scalar → file 분기). 기존 `FormValidationError` 예외 인터페이스(`field`, `message`, `code: 'VALIDATION_ERROR'`)는 변경 없음. HTTP/WS API 경로·상태코드·응답 구조 불변.
- `dynamic-form-ui.tsx` — 프론트엔드 클라이언트 가드(선택 즉시 reject). 서버 API 계약에 영향 없음.
