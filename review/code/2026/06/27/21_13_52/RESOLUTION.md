...
|---|---|---|---|
| W#1 SPEC-DRIFT | FIX | `spec/5-system/17-agent-memory.md` §3·§7(실현됨)·§7(Rationale) 4곳의 AGM-08 watermark 경로를 `_resumeState.memoryState.lastExtractionTurnSeq` 로 갱신 + in-flight 폴백 병기 (SPEC-DRIFT back-flow — 구현 I12 에 spec 정합) | `<resolution>` |
...
| W#4 Side Effect (saveMemories 시그니처 파괴) | 대응 | 의도된 변경(I3). TS 컴파일이 누락 호출부 감지(빌드 PASS). `embedCfgSource` JSDoc 추가로 계약 명시 | `<resolution>` |
| W#5 Maint/Test (buildCosineMatch 파라미터 순서) | FIX | recall/dedup 파라미터 순서 계약($1=vector…$4=임계) 직접 어설션 테스트 2건 추가 | `<resolution>` |
| W#6 Test (IE hydrateState 폴백 미검증) | FIX | 구 평면 키 `_resumeState.lastExtractionTurnSeq` 로 resume 시 watermark 폴백 통합 테스트 추가 | `<resolution>` |
...
| INFO #3 | 채택 | `updateSummaryState` "두 필드 함께 제공" 계약 JSDoc 추가 | `<resolution>` |
| INFO #5 | 채택 | IE `hydrateState` IIFE → `const extractionSeq` 분리(스타일 통일) | `<resolution>` |
| INFO #9 | 채택 | `readExtractionWatermark` seq=0 경계(유효 watermark) 테스트 추가 | `<resolution>` |
| INFO #10 | 채택 | `updateSummaryState` 빈 객체 호출 시 필드 클리어 테스트 추가 | `<resolution>` |
| INFO #11 | 채택 | `memoryState` 병합 시 타 키 보존 테스트 추가 | `<resolution>` |
| INFO #12 | 채택 | `saveMemories.embedCfgSource` JSDoc 추가 | `<resolution>` |
| INFO #15 | 채택 | 테스트 헬퍼 `wmOf` → `getWatermark` 개명 | `<resolution>` |
