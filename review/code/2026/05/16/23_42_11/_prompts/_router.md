...
- **database** (데이터베이스(Database)) — 마이그레이션·스키마·SQL 변경: backend/migrations/V052__notification_type_integration_action_required.sql, backend/migrations/V053__notification_workspace_type_resource_idx.conf, backend/migrations/V053__notification_workspace_type_resource_idx.sql
- **dependency** (의존성(Dependency)) — 패키지 매니페스트·lockfile 변경 — dependency 영향 + README/CHANGELOG 동반 갱신 점검: backend/package-lock.json, backend/package.json
- **documentation** (문서화(Documentation)) — 패키지 매니페스트·lockfile 변경 — dependency 영향 + README/CHANGELOG 동반 갱신 점검: backend/package-lock.json, backend/package.json / 문서 파일(.md/.txt/.rst/.adoc/LICENSE/CHANGELOG 등) 변경: README.md, frontend/README.md, packages/expression-engine/README.md (외 13건)
- **maintainability** (유지보수성(Maintainability)) — 소스 코드 변경 — 코드 변경 시 항상 적용: backend/src/common/dto/pagination.dto.ts, backend/src/main.ts, backend/src/modules/execution-engine/execution-engine.service.ts (외 12건)
- **requirement** (요구사항(Requirement)) — spec 본문 변경 — documentation 외에도 요구사항 일관성 검증 필요: spec/0-overview.md, spec/1-data-model.md, spec/2-navigation/4-integration.md (외 8건) / 소스 코드 변경 — 코드 변경 시 항상 적용: backend/src/common/dto/pagination.dto.ts, backend/src/main.ts, backend/src/modules/execution-engine/execution-engine.service.ts (외 12건)
- **scope** (변경 범위(Scope)) — 소스 코드 변경 — 코드 변경 시 항상 적용: backend/src/common/dto/pagination.dto.ts, backend/src/main.ts, backend/src/modules/execution-engine/execution-engine.service.ts (외 12건)
- **security** (보안(Security)) — 소스 코드 변경 — 코드 변경 시 항상 적용: backend/src/common/dto/pagination.dto.ts, backend/src/main.ts, backend/src/modules/execution-engine/execution-engine.service.ts (외 12건)
- **side_effect** (부작용(Side Effect)) — 소스 코드 변경 — 코드 변경 시 항상 적용: backend/src/common/dto/pagination.dto.ts, backend/src/main.ts, backend/src/modules/execution-engine/execution-engine.service.ts (외 12건)
- **testing** (테스트(Testing)) — 소스 코드 변경 — 코드 변경 시 항상 적용: backend/src/common/dto/pagination.dto.ts, backend/src/main.ts, backend/src/modules/execution-engine/execution-engine.service.ts (외 12건)
...
+| W-25 | `backend/src/modules/websocket/websocket.service.ts:92` | `sanitizePayloadForWs` 가 자식 mutation
