...
| W-negative-test | testing | union "닫힘" 을 고정하는 `@ts-expect-error` negative test 없음(allow 방향만) | 위젯 + SDK 에 negative 케이스 추가(ButtonsContext buttonConfig 누락·두 판별키 모두 누락). **검증 통로 정정(fresh review 12_15_40)**: SDK 는 `test`(ts-jest, cmd_unit 배선)가 타입체크 — build(tsc)는 tsconfig 가 `*.spec.ts` exclude 라 무관. 위젯은 typecheck 배선 후 유효. `cmd_lint` 에 SDK 미추가 = SDK 에 `eslint.config.js` 부재라 의도적 제외 | **fixed** |
