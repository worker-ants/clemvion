...
### 1. `main()` 의 `_push_targets` 폴백 무검증 — **내 2차 RESOLUTION 이 틀렸다**

2차에서 나는 이 경로를 "위 2건이 커버" 라고 적었다. **틀렸다.** 그 테스트들
...
경로다" 를 명시해 같은 혼동이 재발하지 않게 했다. **2차 RESOLUTION 의 과대 표기도 정정**했다.
...
1차: "테스트로 커버" 주장 → 2차가 PLAN 게이트 미검증 발견.
2차: "위 2건이 커버" 주장 → 3차가 `main()` 폴백 미검증 발견.
