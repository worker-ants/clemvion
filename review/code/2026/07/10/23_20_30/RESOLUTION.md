...
| --- | --- | --- | --- |
| W1 | Warning (documentation) — plan 체크박스 미갱신에 durable 추적 부재 | **defer + durable 등록** (아래 §보류·후속 항목 1). WARNING 이 실제로 요구한 "durable 추적" 은 본 RESOLUTION + PR 설명 + background task chip `task_e03a0b87` 3중으로 해소 | (문서만) |
...
| plan-coherence Warning | 종결 조건이 `plan/in-progress/resume-llm-usage-attribution.md:53` 의 `- [ ] PR (push + gh pr create)` (#879 시절부터 남은 stale 미체크) 를 누락 | **fix.** §보류·후속 항목 1 의 종결 조건에 추가. task chip `task_e03a0b87` 에도 반영 |
...
**durable 등록**: 본 RESOLUTION.md(저장소 영구 기록) + PR 설명 + background task chip `task_e03a0b87`.
