...
| build | **PASS** (`14:34:53`) |
| e2e | **PASS** — 51 스위트 / 295 통과 (`14:24:19`) |

e2e 를 인용한 `14:24:19` 실행은 **본 커밋과 동일한 codebase 내용**에서 돌았다. 그 뒤의
codebase 편집은 (a) 유출 복귀 뮤턴트와 (b) 그 `cp` 원복 둘뿐이고, 원복 직후
`git diff --stat` 으로 서비스 파일이 제자리로 돌아온 것을 확인했다.
