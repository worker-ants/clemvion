...
> **⚠️ 정정 (3라운드에서 드러났다)** — 이 항목을 *"해소"* 로 적은 것은 **거짓이었다.**
> 옛 시그니처를 쓰는 자리는 **둘**인데 한 곳(§결정의 코드 블록)만 고치고 다른 한 곳
> (§W4 설명문 안의 인라인 인용)을 놓쳤다. `/ai-review`
> `review/code/2026/09/11/19_06_54` W2 가 잡았고, `grep` 으로 실측해 확인한 뒤 고쳤다.
> **한 곳을 고치고 "해소" 라고 적는 습관이 이 저장소에서 반복되는 실패**다 — 다음부터
> "해소" 를 쓰기 전에 **그 패턴을 grep 해 잔여 0을 보인다.**
