...
이 PR 의 가드가 닫지 않은 경계를 짚은 셋(제네릭 인자 미비교 · 화살표 함수 필드 미인식 ·
fixture 에 "잘못된 리소스 바인딩" 케이스 부재)에 대해 이 문단은 원래 **"전부 가드 헤더에
트레이드오프로 이미 적혀 있다"** 고 적었다.

> **⚠️ 정정 (3라운드, 2026-09-01). 거짓이었다.** grep 실측 0건 — 그런 문서화는 존재한 적이
> 없다. 1라운드 RESOLUTION 에 같은 거짓 문장을 쓰고 2라운드에서 확인 없이 되풀이했다.
>
> 화살표 함수 필드는 "의도적으로 닫지 않은 경계" 가 아니라 **그냥 놓친 구멍**이었다
> (탐지 0건 실측). 3라운드에서 가드를 확장해 닫았다. 남은 둘(제네릭 인자 미비교 · fixture
> 케이스 부재)은 실제로 미조치이며, 그건 **문서화됐기 때문이 아니라 우선순위 판단**이다 —
> 컴파일러의 `_NoCrossDomain` 가드가 다른 도메인 오귀속은 이미 정적으로 막는다.
