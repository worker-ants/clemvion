...
으로 판정한 것들이다. 그중 셋은 이 PR 의 가드가 **의도적으로 닫지 않은 경계**를 정확히
짚었다(메서드 이름 기반 탐지 · 화살표 함수 필드 미인식 · `startsWith` 접두 비교) — 전부
가드 헤더에 트레이드오프로 이미 적혀 있고, 이번에 고친 "맨 union" 결함 자체는 정확히 잡는다.
