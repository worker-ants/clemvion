...
`12_24_55`(재실행) 셋을 한 항목으로 묶었다.

> **초안은 재실행 ID 를 `12_37_04` 로 적었다 — 세션이 생기기도 전에 쓴 숫자다.**
> 실제는 `12_24_55` 였고, 게이트를 돌린 직후 정정했다. 이 세션이 반복 기록한
> *"실측했다면서 프록시를 쟀다"* 의 축소판이다 — **아직 없는 것에 번호를 붙이지 마라.**
