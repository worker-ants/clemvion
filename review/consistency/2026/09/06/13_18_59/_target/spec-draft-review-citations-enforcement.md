...
- [ ] (A) Rationale 정정 — 취소선 + **DTO/컨트롤러를 가른** 강제 여부 표
- [ ] (B) `code:` 에 `dto-jsdoc-citation*.ts` 등재 (glob 폭 주의)
- [ ] (C) `spec-impl-evidence.md §2.1` 선례 인용을 **축 단위**로 좁힘
- [ ] **자매 plan 동기화** — `spec-draft-nullable-notation-followups.md` 의 「신규 검출 3축
