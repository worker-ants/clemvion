...
> **정정 (2026-09-06)**: 이제 **한 축은 강제된다.**
> `repo-guards/__tests__/dto-jsdoc-citation-guard.ts` 가 §3 의 DTO 카브아웃 —
> *"응답 DTO 의 `/** */` JSDoc 에 리뷰 인용을 쓰지 않는다"* — 을 AST 로 센다.
> 같은 위반이 **세 번** 났고 세 번 다 사람이 읽고 잡은 것이 계기다
> (`review/code/2026/09/06/12_28_02` W2).
>
> | 절 | 강제되나 |
> |---|---|
> | §2 (bare `hh_mm_ss` 금지) — `codebase/**` 전반 | **아니오** — 여전히 사람이 본다 |
...
- [ ] (A) Rationale 정정 — 취소선 + 강제/비강제 표
- [ ] (B) `code:` 에 `dto-jsdoc-citation*.ts` 등재
- [ ] (C) `spec-impl-evidence.md §2.1` 선례 인용 **실측 후** 필요 시 정정
- [ ] `--spec` 게이트 BLOCK:NO 확인 후 `spec/` 반영
