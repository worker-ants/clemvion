...
### 1.5 변경안 (A) — `codebase/backend/migrations/README.md` §5
...
| | 값 |
|---|---|
| `origin/main` `codebase/` 안 `hh_mm_ss` 인용 | **107개 파일 · 514회** |
...
- 리뷰 산출물 인용은 **유지**한다. `review/**` 는 커밋되므로(`_prompts/` 만 gitignore)
  세션 경로는 **영구 참조**다.
...
```markdown
### 5-1. `CONCURRENTLY` 로 인덱스를 **교체**할 때 — DROP-먼저
