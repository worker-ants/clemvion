...
### 반증 1 — `chat-channel.md` R-K 가 이 비대칭을 명시적으로 결정했다
...
- **미머지 브랜치 문구 정정** — `spec-draft-nullable-notation-followups.md` 의
  *"§5.4 를 시행하는 **유일한** 코드"* 를 *"**런타임으로** 시행하는 유일한 코드 (정적 축은
  `swagger-dto-contract-guard.ts`)"* 로 좁힌다. `claude/sweep-response-contract-5ba0ad`
  에 있는 문장이라 그 브랜치에서 고친다.
...
> 주장하는 것이 아니라, **그것이 이미 내려진 결정**이며 이 턴의 일이 아니라는 뜻이다.
> 저장 형태 자체를 재검토하려면 R-K 를 다시 여는 별도 결정이어야 한다.
