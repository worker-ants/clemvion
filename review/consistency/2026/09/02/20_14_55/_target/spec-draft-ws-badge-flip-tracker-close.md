...
| 7 | 〃 `:1101` status 강등 기록 | 원문 보존 + **승격 후속 주석**(2026-06-03 강등 → 2026-09-02 복귀) |
| 8 | 〃 `:1115`·`:1133` 범위 밖 | 원문 보존 + **처분 완료 포인터** |
...
| 12 | `spec-draft-ws-wontdo-maintenance-appping.md` | 체크리스트 채움 + **`complete/` 이동** (`--impl-done` INFO#2) |
| 13 | `spec-draft-ws-socket-lifetime-binds-token.md` | 구현 항목 `[x]` + **`complete/` 이동** |
| 14 | `ws-token-expired-socket-lifetime-impl.md` | PR 체크 + 이 planner 항목 `[x]`. **in-progress 유지** — 잔여(지터·e2e·배포 런북·flaky watch·이월 INFO)가 있다 |
