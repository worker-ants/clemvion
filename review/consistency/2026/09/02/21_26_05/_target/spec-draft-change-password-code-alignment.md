...
| 11b | 〃 §5 머리말 | *"코드베이스에서 완전 제거"* 에 **레이어 caveat 한 문장** — 같은 문자열이 다른 레이어(감사값·엔진 분류)에 남을 수 있고, 그때 은퇴는 **wire 발행 중단**을 뜻한다. 이 행이 그 첫 사례다 (`--spec` INFO#3) |
