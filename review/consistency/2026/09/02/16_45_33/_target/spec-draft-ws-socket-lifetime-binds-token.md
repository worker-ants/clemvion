...
(`auth.refreshed.expiresAt` 등)와 같은 표기다. 의미는 **이 소켓이 끊기는 시각**이며,
`auth.refreshed.expiresAt`(새 토큰의 만료 시각)과 이름은 같지만 가리키는 대상이 다르므로
§4.6 표에 그 뜻을 명시한다.
...
| 8 | 〃 Rationale 신설 | `R-ws-socket-lifetime-binds-token` |
