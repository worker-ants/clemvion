...
싣는다 — main 에 들어가는 순간 현재형이 참이다. 이 저장소의 «계획(Planned)/미구현» 인라인 마커(`spec-impl-evidence.md` §3)는
...
(C-9) 여섯이다. «403» 만 적은 서술(예: `3-workflow-editor/3-execution.md` «서버도 `@Roles('editor')` 로 403») 은 상태가 그대로라 참이다.
