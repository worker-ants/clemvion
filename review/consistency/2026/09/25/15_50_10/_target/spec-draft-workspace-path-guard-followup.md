...
> 이 아니라 기록 오류다 — 등재 자체를 지우고, 지운 자리의 비고에 실측을 남긴다(예: 2026-09-25 §3 의 `forbidden` — `code: 'forbidden'`
> 발행처 0건).
...
- **기각한 대안**(이 턴에서 견줬다): 변경 3 을 `repo-guards/__tests__/**` 통째 glob 으로 — 저장소의 `code:` 는 전부 개별 glob 이고
  (`swagger.md` 실측, checker 도 같은 관찰을 적었다), 통째 glob 은 무관한 가드 변경까지 `1-auth` 를 spec-linked 로 만들어 `--impl-done`
  스코프를 부풀린다.
