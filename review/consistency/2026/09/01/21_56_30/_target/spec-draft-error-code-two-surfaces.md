...
- 둘은 **같은 파일의 자매 const**(`nodes/core/error-codes.ts`)이고 키가 겹치지 않는다
  (테스트로 고정)
- 경계는 **누가 발행하는가**이며, 카탈로그의 "엔진 수준 에러" 분류와 **1:1 대응하지 않는다**
...
**재개 신호는 "세 번째 자매 const 가 생길 때"** 다 — 그때는 형태가 관례가 되므로 기준이
필요해진다. (`WsErrorCode` 가 그 세 번째인지는 재개 시점에 함께 판정한다.)

**다른 문서의 선재 drift 는 여기서 안 고친다** (3차 `--spec` `21_39_47` cross_spec W1·W2).
`1-data-model.md:474` 가 엔진 인프라 코드 6종을 소속 구분 없이 나열하고, `3-error-handling.md`
§1.4 가 "엔진 수준 에러" 10종을 단일 집합처럼 적는다(그중 named const 등재는 2종뿐). 둘 다
**이 draft 가 만든 것이 아니고**, 층 기반 병기로 바꾸면서 이 draft 가 그 서술과 충돌하는
주장을 더는 하지 않는다. 다른 spec 파일을 고치는 별도 planner 턴이라 착수 근거 plan 에
등재했다.
