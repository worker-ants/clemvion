...
**노출 금지는 여기 다시 적지 않는다** — [§1.1](./conventions/secret-store.md#11-비대상-필드도-응답-바디에는-나가지-않는다)
...
- [ ] C-2 `1-data-model.md §2.8` 저장 형태 명시 + 링크
- [ ] C-3 `2-api-convention.md §5.4` §1-3·§1-4 병기
- [ ] C-4 `code:` 등재 — **4개 문서, 6개 항목**. 등재 후 게이트에 다시 물어 0/5 → 5/5 · 9/13 → 11/13 확인
- [ ] C-4 `production-build-devdep*` 소유자 부재를 트래커에 별 항목으로 등재
- [ ] C-5 `3-error-handling.md` 3곳 `requestId` 정본 값으로
- [ ] 문서 가드(`vitest src/lib/docs/__tests__`) — 앵커·`code:` 경로 실재
- [ ] 자매 트래커 체크박스 **5건** 플립 + 신규 1건 등재 (개별 열거로 확인)
- [ ] `--impl-done` (spec 편집이 있으므로)
