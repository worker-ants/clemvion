...
- [`chat-channel-adapter.md R-CCA-5`](../../spec/conventions/chat-channel-adapter.md) — 분류 입력은
  화이트리스트(`error.code` + `details.statusCode`)만, **`error.message` 원문은 배제**.
...
이미 옳게 동작하고, 접두/`code` 를 아직 안 붙인 경로가 **조용히 502 로 빠지는 것보다** 400 을
주는 편이 사용자에게 낫다. 단 이것은 **transitional fail-safe** 이고, 세 adapter 가 모두 `code`
를 달면 제거 후보다 — 그 조건을 §1.1.2 에 적는다.
...
### (4) `2-api-convention.md §6` 에 **502 행 신설**

§5.4 가 쓰는 값인데 카탈로그에 없다. 추가한다:

> `502 | Bad Gateway | 외부 provider API 호출이 실패 — 재시도 가능. 우리 의존성(Redis 등)의
> 일시 장애인 `503` 과 구분한다: 502 는 **외부 제3자**, 503 은 **우리 인프라**다.`

**기각한 대안**: 기존 `503` 재사용. §6 의 503 은 *"upstream 의존성(Redis 등) 일시 장애"* 로
**우리 인프라**에 스코프돼 있고 코드도 `SERVER_SHUTTING_DOWN` 계열이다. 외부 provider 실패를
거기 섞으면 그 구분이 무너진다 — 운영자가 *"우리가 죽었나 남이 죽었나"* 를 상태 코드로 못 가린다.
...
- [ ] `/consistency-check --spec` **재실행** — draft 가 3파일 늘고 설계가 바뀌었다. BLOCK: NO 필요
- [ ] (1) `§5.4` 에러 표 두 행 + 원문/로그 분리 명시
- [ ] (2) `chat-channel-adapter.md` §1.1.2 신설 + §1.1 표 `setupChannel` 행에서 링크
- [ ] (4) `2-api-convention.md §6` 502 행 신설
- [ ] (5) 복제 3곳을 §5.4 링크로 축약
