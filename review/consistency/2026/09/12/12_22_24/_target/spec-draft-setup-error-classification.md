...
> **이 fallback 은 R-CCA-5 / CCH-ERR-02 의 「message 원문 배제」에 대한 의도적·한시적 예외**다
> (`--spec` 2회차 INFO 3). 예외인 이유를 §1.1.2 에 적고 **제거 조건도 함께 적는다**:
> 세 adapter 가 모두 `code` 를 달면 이 분기는 삭제 후보다. 조건을 안 적으면 한시적 예외가
> 영구 예외가 된다.

**두 `code` 는 다른 네임스페이스다** (`--spec` 2회차 INFO 2) — §1.1.2 의 `code` 는
`setupChannel` 실패 판별용이고, §3.1 의 `event.error.code` 는 EIA 실행 실패 분류용이다.
같은 파일 안에 같은 이름이 두 뜻으로 있으므로 §1.1.2 서두에 그 구분을 적는다.
제목도 §1.1.1(**멱등**의 뜻)과 대조되게 **「실패 판별」** 로 짓는다(INFO 7).
...
→ 응답은 **고정 client-safe message + code** 만, 원문은 **서버 로그**(§7.5.2 의 `serverDetail`
자리)로. §5.4 표에 그 분리를 명시한다.
...
```markdown
| 502 | Bad Gateway | **외부 제3자 API** 호출 실패 — 재시도 가능. 우리 의존성(Redis 등) 일시 장애인 `503` 과 구분한다 |
```
...
- **`chatChannelLastError` 에 외부 원문이 저장되는 문제** — (3)과 **같은 클래스지만 다른 표면**
  (응답 vs DB 컬럼)이고 별 항목으로 등재돼 있다. 이 턴에서 함께 고치지 않는 이유는 그쪽이
  **운영자용 진단**이라 제거 판단에 다른 근거가 필요하기 때문이다.
...
- [ ] `/consistency-check --spec` **3회차** — draft 가 또 2파일 늘었다(`providers/slack.md` ·
      `conventions/swagger.md`). 체커가 안 본 대상이 생겼으므로 BLOCK: NO 를 다시 받는다
- [ ] (1) `§5.4` 에러 표 두 행 + 원문/로그 분리 명시
- [ ] (2) `chat-channel-adapter.md` §1.1.2 신설 + §1.1 표 `setupChannel` 행에서 링크
- [ ] (4) `2-api-convention.md §6` 502 행 신설 (+ 노드 `statusCode` 구분 각주)
- [ ] (4-b) `conventions/swagger.md §2-4` 에 **5xx 행** 신설 — 없으면 컨트롤러가 502 를 문서화할
      데코레이터 근거가 없다
- [ ] (6) `providers/slack.md §3.1` 에 실패 응답 형태(`{ok:false, error}` + HTTP 200) 한 줄
- [ ] (5) 복제 3곳을 §5.4 링크로 축약
- [ ] (3) `R-CC-23` + `R-CCA-9` (cross-file 인용 포맷 `[CCA §R-CCA-9]` 준수)
- [ ] **반영 후 `401/403` 잔여를 grep 해 0 확인** (*"해소" 전에 잔여 0 을 보인다*)
- [ ] 트래커 **정정 3건** (`spec-draft-nullable-notation-followups.md`) —
      ...
      **기각된 접근을 되살린다**(`--spec` 2회차 W3 — 이 저장소의 반복 실패 클래스).
      ...
      미등재를 **살아있는 항목으로 등재**(INFO 6) — 이 draft 가 `complete/` 로 봉인되면 유예
      근거가 사라진다.
- [ ] 트래커 항목을 **developer → planner 완료 + developer 후속**으로 재기술.
