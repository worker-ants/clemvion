...
2. **기존 중복은 나중 것을 새 UUID 로** — 경로가 같은 묶음마다 가장 먼저 만든 트리거(`created_at`, 같으면 `id`)만 경로를 유지하고,
   ...
   **새 경로를 받은 트리거가 채팅 채널**(`config.chatChannel`)이면 provider(Telegram · Slack · Discord)에 등록된 URL 은 옛 경로 그대로다 —
   SQL 은 provider API 를 부를 수 없다. 그래서 그 트리거는 `chat_channel_health = 'degraded'` 와 «채널 설정을 다시 저장하라» 는
   `chat_channel_last_error` 로 표시하고(NOTICE 에도 `chat_channel=true`), 재등록은 **사용자의 다시 저장 → `setupChannel`** 정상 경로에
   맡긴다(아래 Rationale «채팅 채널 트리거 — R-CC-21 과의 관계», `--spec` Critical 1).
...
(CLAUDE.md 자기-반증형 소정정과 같은 «원문 보존» 관례), 그 뒤에 `2-trigger-list.md` R-2 의 «정정 (날짜)» 블록 형식으로 정정을 잇는다:
...
- **V131__trigger_endpoint_path_dedupe.sql** (트랜잭션) — `DO $$ … $$`: `row_number() OVER (PARTITION BY endpoint_path ORDER BY created_at, id)`
  가 1 보다 큰 행에 `endpoint_path = gen_random_uuid()::text, updated_at = now()`. 그 행이 채팅 채널(`config ? 'chatChannel'`)이면
  `chat_channel_health = 'degraded'`, `chat_channel_last_error = '<다시 저장 안내>'` 도 함께. 행마다 id · 워크스페이스 id · `chat_channel` 여부 NOTICE,
  끝에 건수 NOTICE.
...
- [ ] `--spec` 이 draft — 1차 `review/consistency/2026/09/18/23_39_46` **BLOCK: YES**(Critical 2 · WARNING 1) → 아래 «`--spec` 1차 처분» 반영 뒤 재검토
- [ ] S1~S7 반영
- [ ] `--impl-prep`
- [ ] V131 · V132 · 서비스 · 테스트
- [ ] lint · unit · build · e2e
...
### 채팅 채널 트리거 — R-CC-21 과의 관계 (`--spec` 1차 Critical 1)
...
재등록을 흉내 내지 않고, **재등록을 정상 경로로 돌려보낸다**: 그 트리거를 `degraded` 로 표시하고 `chat_channel_last_error` 에 «엔드포인트
경로가 다른 워크스페이스의 트리거와 겹쳐 새로 발급됐어요. 채팅 채널 설정을 다시 저장하면 새 주소가 등록돼요.» 를 남긴다. 사용자가 다시
저장하면 `setupChannel` 이 새 경로로 재등록하고 health 가 `healthy` 로 돌아온다. `degraded` · `chat_channel_last_error` 는 이미 채팅 채널
실패를 드러내는 표준 자원이다(CCH-SE-01 · CCH-NF-03).
...
V131 은 누가 받는지를 «가장 먼저 만든 쪽» 으로 정할 뿐, 받던 쪽이 둘이던 상태를 하나로 줄이는 것이 아니다.
