...
- **V111** `codebase/backend/migrations/V111__trigger_workflow_id_index.sql` + `.conf`(`executeInTransaction=false`) —
  `DROP INDEX CONCURRENTLY IF EXISTS idx_trigger_workflow_id;` → `CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_trigger_workflow_id
  ON trigger (workflow_id);` (수동 롤백 주석 포함). **V106 처럼 CREATE 만 두지 않는다** — `migrations/README.md` §5 가 V106 형태를
  «CREATE 가 중간에 실패하면 invalid 인덱스가 이름을 점유해 `IF NOT EXISTS` 재실행이 건너뛰고, **영영 유효해지지 않는다**» 로
  적는다. 앞에 invalid 잔재 정리(V110 의 0) 단계)를 둔다. 신규 추가라 지울 옛 인덱스가 없으므로 README 의 «감수하는 비대칭»
  (성공 뒤 재실행 시 정상 인덱스 재빌드)은 그 구간 seq scan 뿐이다.
  새 식별자 `idx_trigger_workflow_id`·`V111` 은 `codebase/` · `spec/` · `plan/` grep 0건(제안 단계의 `review/**` 언급은 제외 —
  그 제안을 이 draft 가 실행한다). 커밋 직전 `V111` 미점유를 다시 확인한다(병렬 세션 race).
...
- [ ] `/ai-review` — 1라운드 `review/code/2026/09/18/12_43_23` LOW · C0 · W1(V111 형태가 README §5 에 없다 → `ff7d79967` README 규약화,
...
### `--spec` 처분 (`review/consistency/2026/09/18/12_18_52` — **BLOCK: NO**, Critical·Warning 0 · INFO 6)

- **INFO 1** §3 표에 `notification_health` 부분 인덱스(V061)가 원래 없었다 → S1 과 같은 자리라 **행을 함께 넣었다**. 목적 칸은
  «V061 이 적은 목적» 으로만 적었다 — 그 인덱스를 실제로 쓰는 쿼리는 이번에 확인하지 않았다.
- **INFO 2** Rationale 의 `integration_oauth_*` 두 테이블은 `1-data-model.md` 에 엔티티가 없다 → `data-flow/5-integration §2.1` 링크 동행.
- **INFO 3** V110(DROP-먼저)과 다른 형태를 쓰는 이유가 없다 → 구현 결정을 바꿨다: V111 도 invalid 잔재 정리를 먼저 둔다(위 «구현»).
