...
**dead path 발견**: `backend/src/modules/knowledge-base/graph/kb-stats.helper.ts:42-46` 가 `emitExecutionEvent(`kb:${kbId}`, 'kb:graph_stats_updated' as never, …)` 호출. 그러나 `emitExecutionEvent` 는 channel 을 `execution:${첫인자}` 로 prefix 하므로 실제로 frontend 의 `kb:` 구독에 도달하지 못한다. type union 에도 없음 (`as never` 강제 캐스트). → **본 spec 정비에서는 권위에서 제외 (코드 측 결함은 후속 plan 분리)**.
...
| 위치 | 현재 | 변경 |
|---|---|---|
| §1 헤더 | `## 1. 개요` | `## Overview` (CLAUDE.md 권장 패턴) |
...
| §6.1 DocumentChunk 테이블 | 6필드 표 inline | `1-data-model.md §2.12.1` 로 링크 참조하고 본 spec 의 표는 핵심만 (chunk_index, content, embedding, token_count) — 중복 제거. **보수적 접근**: 표는 유지하되 상단에 "권위 정의는 `1-data-model.md §2.12.1`" 한 줄 명시. 변경 위험 최소화. |
| §6.2 DDL 예시 | IVFFlat 단일 인덱스 | 동일 DDL 유지하되 "실제 운영 인덱스는 V022/V023 으로 차원별 partial HNSW(vector / halfvec) 로 분리되어 있다. 본 DDL 은 컨셉 예시" 노트 추가. **DDL 코드블록 자체를 V022/V023 SQL 로 교체하는 건 마이그레이션 history 종속이라 보수적으로 노트만 추가.** |
| §8 WebSocket 알림 표 | embedding 6개만 (started/progress/completed/error/retry/failed) | embedding 6 + graph 6 = 12개. 채널 명명규약 그대로 (`kb:${documentId}`). graph 추출 영역 footnote 로 분리. |
| §8 헤더 끝 | (없음) | "§9.2 의 `error` 상태와 직접 대응되며, `error` 의 의미 변경(2026-05-11) 이후 일시 오류만 의미한다" 한 줄 교차 참조 추가 (Info #3 흡수) |
| §9.4 retry-failed | `scope: 'embedding' \| 'graph' \| 'all'` | 표기 유지 + footnote "프론트엔드 UI 는 `'embedding'` 과 `'graph'` 두 버튼만 노출. `'all'` 은 운영/스크립트용" 추가 (frontend 가 buttons 별 분리 UI 라는 게 §2.4.1 권위) |
...
| §4.3 임베딩 이벤트 섹션 | 채널 `embedding:{knowledgeBaseId}` + 점 표기 4개 이벤트 (`embedding.started/progress/completed/failed`) | 섹션 제목 "KB 문서 이벤트 (Server → Client)", 채널 `kb:{documentId}`, embedding 6 + graph 6 = 12개. payload 권위는 backend 구현 기준. error/retry/failed 의미 구분 명시. |

### 3. `spec/2-navigation/5-knowledge-base.md`

| 위치 | 현재 | 변경 |
|---|---|---|
| §2.4.1 line 105 | `POST /api/knowledge-bases/:id/retry-failed { scope: 'embedding'\|'graph' }` | `{ scope: 'embedding' \| 'graph' \| 'all' }` + "UI 는 '실패 문서 재시도' 버튼이 vector/graph 두 분리 영역에 있어 `scope: 'embedding'` 또는 `'graph'` 만 전송. `'all'` 은 운영/스크립트용" 한 줄 |
...
| 위치 | 현재 | 변경 |
|---|---|---|
| §2.12.1 DocumentChunk 인덱스 | `ivfflat (embedding vector_cosine_ops) — 유사도 검색 성능` | `partial HNSW (V022 vector / V023 halfvec) — 차원별 분리, 유사도 검색 성능. 자세한 마이그레이션은 backend/migrations/V022·V023 참조` |

## Rationale 섹션 정리 (spec/5-system/8-embedding-pipeline.md)

CLAUDE.md 원칙: Rationale 은 "결정의 배경·근거·폐기된 대안" 위주. 현재 작업 일지(2026-05-02) 형식으로 적혀있는 부분을 보존하되 형식을 다듬는다. **기존 정보 손실 금지** — 결정 항목·후속 적용 모두 유지.
...
- 섹션 헤더 "### 작업 메모: 지식베이스 임베딩 모델 사용자 선택 (2026-05-02 완료)" → "### 결정: 다중 차원 임베딩 + KB 단위 모델 선택 (2026-05-02)" (작업 일지 → 결정 중심)
- 본문 형식은 그대로 유지 (배경 → 사용자 결정 → 핵심 결과 → 검증 → 후속) — 정보 손실 위험 회피.

## 후속 항목 (별도 plan/dev 위임)

- **dead path 처리**: `backend/src/modules/knowledge-base/graph/kb-stats.helper.ts:42-46` 의 `kb:graph_stats_updated` emit 이 잘못된 채널(`execution:kb:...`) 로 broadcast 됨. 옵션: (1) `emitKbEvent` + 새 이벤트 type 추가, (2) 코드 제거. developer 결정 사항.
- frontend `useKbEvents` 의 type union 도 12개와 일치해야 — 이미 일치 확인 (use-kb-events.ts:66-79).

## 검토 후 단계

1. 본 draft 에 대해 `/consistency-check --spec plan/in-progress/spec-draft-embedding-pipeline-consistency.md` 의무 호출
2. Critical 0 확인 → spec 본문 4개 파일에 반영
3. plan/in-progress/spec-update-embedding-pipeline-consistency.md 갱신 (모든 항목 체크 → complete/ 이동)
4. PR 생성
