...
## Target 문서
경로: `plan/in-progress/spec-draft-ai-thread-source-mark.md`
...
# Spec Draft — AI 대화 메시지 source 마커

본 문서는 `/consistency-check --spec` 호출 대상이다. Critical 0 건 확인 후 `spec/` 본문에 반영하고 `plan/in-progress/` 에서 정리한다.

## 변경 대상 1: `spec/5-system/6-websocket-protocol.md`

### 1-A. §4.1 표 — `execution.ai_message` 행 수정
...
### 1-B. §4.4 — `conversationConfig.messages` JSON 예시 수정

`ai_conversation` waiting payload 의 `conversationConfig.messages` 예시에 `source` 필드 추가:
...
> 주의: 기존 예시는 system 메시지를 포함하고 있었으나, `buildConversationConfigFromOutput` 가 system 을 필터링하므로 emit 페이로드의 messages 에는 system 이 나타나지 않는다 (이 부분은 마커 도입과 무관한 기존 동작이지만 예시도 동시에 정합화).

### 1-C. §4.4 — `execution.ai_message` 페이로드 필드표 확장

기존 표의 `messages` 행 아래에 다음 두 행 추가:

| 필드 | 타입 | 설명 |
|---|---|---|
| `messages[].source` | `'live' \| 'injected'` | 메시지 origin 식별. 자세한 정의는 §4.4.6 |

### 1-D. §4.4 — `execution.ai_message` JSON 예시 수정
...
"messages": [
  ...
  { "role": "user", "content": "주문 ORD-12345 확인해줘",
    "source": "live" },
  { "role": "assistant", "content": "주문번호 ORD-12345 는 현재 배송 중입니다.",
    "source": "live" }
],
```

### 1-E. §4.4 신규 하위절 — §4.4.6 `messages[].source` 마커
...
| 값 | 의미 |
|---|---|
| `live` | AI Agent 핸들러가 해당 노드의 현재 실행 turn 에서 실제로 처리/생성한 메시지. `processMultiTurnMessageInner` 가 push 한 user / assistant / tool 메시지, single-turn 의 userPrompt → 최종 assistant 등이 모두 여기에 해당. |
| `injected` | `ConversationThread` 자동 주입 (`contextScope: 'thread' \| 'lastN'`) 또는 명시 `$thread` 사용으로 messages 배열 앞에 prepend 된 컨텍스트 메시지. 예: 업스트림 Form/Carousel/Template 노드의 `presentation_user` turn, 다른 AI Agent 의 `ai_user`/`ai_assistant` turn 이 §[Conversation Thread §5.1](../conventions/conversation-thread.md#51-messages-모드-매핑) 매핑으로 변환된 결과. |
...
- `source` 필드가 누락된 경우 (옛 backend 호환) `'live'` 로 간주한다.
```
...
2. (기각) backend 가 `injectedContextLength: number` 만 동봉. 단순하지만 messages 배열의 prefix 가 연속된 injection 이라는 단단한 가정 필요. 향후 inline 주입(중간 삽입)/멀티 thread 머지 등 시나리오에서 깨진다.
3. (기각) 어시스턴트 메시지에 `turnIndex` 를 직접 동봉. 의미 중복 (`llmCalls[]` 와 1:1 매칭은 시간순 누적이면 충분), 또한 user 메시지의 turn 매핑은 여전히 필요해 근본 해소 불가.

`source` 의 의미와 매핑 규약은 [Conversation Thread §5.1](../conventions/conversation-thread.md#51-messages-모드-매핑) 의 messages 모드 매핑이 단일 진실 공급원. emit 페이로드는 그 매핑의 결과를 표면화한다.
```

---

## 변경 대상 2: `spec/conventions/conversation-thread.md`

### 2-A. §5.1 messages 모드 매핑 — 표 아래 보강 문단 추가

기존 표 아래에 다음 단락 추가:

```markdown
> **WebSocket emit 결과의 `source` 마커**: 위 매핑으로 messages 배열에 prepend 된 모든 항목은 emit 시 `source: 'injected'` 를 동봉한다. 한편 AI Agent 핸들러가 `processMultiTurnMessageInner` 등에서 실제 turn 처리 결과로 push 하는 user/assistant/tool 메시지는 `source: 'live'`. 자세한 정의는 [Spec WebSocket Protocol §4.4.6](../5-system/6-websocket-protocol.md#446-messagessource-마커).
```

### 2-B. §9 CHANGELOG — 신규 항목

```markdown
| 2026-05-16 | injection 산출 메시지가 WebSocket emit 시 `source: 'injected'` 마커를 동봉하도록 정합화 (§5.1) — 디버깅 타임라인의 turn 카운팅이 backend `turnCount` 와 일치하기 위한 전제 |
```
...
- §1.1 ConversationTurnSource enum 은 그대로 유지 — 백엔드 내부 모델은 5값 분류를 계속 사용한다. emit 단계에서 2값으로 축약하는 것은 WebSocket 페이로드의 소비자(frontend) 가 필요한 최소 정보만 노출하기 위함.
- §3 영속화 표의 `output.messages` 와 §1.4 `text` 변환 규칙도 본 변경에 영향 없음 — source 마커는 transport 레벨의 add-on.
- `messages[].source` 가 `output.messages` (DB 영속) 에도 함께 들어갈지는 backend 구현 phase 의 결정 사항으로 둔다 (parseHistoryMessages 일관성 위해 권장하지만 spec 강제는 아님).
...
### Phase 1 — Spec 갱신 (project-planner)

- [ ] `spec/5-system/6-websocket-protocol.md`
  - §4.1 표의 `execution.ai_message` · `execution.waiting_for_input` 행에 `messages[].source` 언급 추가.
  - §4.4 의 `conversationConfig.messages` JSON 예시 및 페이로드 필드표에 `source` 필드 정의 (`'live' | 'injected'`).
  - §4.4 의 `execution.ai_message` 페이로드 필드표·예시에 `messages[].source` 추가.
  - `## Rationale` 에 결정 근거 inline.
- [ ] `spec/conventions/conversation-thread.md`
  - §5.1 messages 모드 매핑 표 아래에 "주입 결과 메시지는 emit 시 `source: 'injected'`, 핸들러가 push 한 실제 turn 산출 메시지는 `source: 'live'`" 명시.
  - §9 CHANGELOG 에 2026-05-16 항목 추가.
- [ ] `/consistency-check --spec` 호출. Critical 0 건 확인 후 spec write.
...
- (Phase 2) source 마커를 `output.messages` (DB 영속화) 까지 보존할 것인지. **잠정 결정**: 보존. parseHistoryMessages 도 같은 컨버터를 거치므로 일관성 확보 효과.
...
- [x] **Tailwind v4 `@theme` directive 갱신** — `--color-vine-300 ~ --color-vine-900` ramp + `--color-ink`, `--color-soil-50/100`, `--color-vine-border` 등록. **다크 토큰(`vine-dark-*`) 은 별도 Tailwind 키로 등록하지 않는다** — 
