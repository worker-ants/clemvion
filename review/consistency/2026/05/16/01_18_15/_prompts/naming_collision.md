...
## Target 문서
경로: `plan/in-progress/spec-draft-cafe24-private-followup.md`
...
```markdown
| `[Request scopes]` 버튼 | 체크된 추가 scope 와 함께 `POST /api/integrations/:id/request-scopes` 호출. 응답 분기는 아래 두 가지 — provider 분기는 backend 가 응답 shape 으로 결정하므로 frontend 는 응답 shape 만 보고 UI 를 분기한다. |

**분기 ① — 일반 OAuth provider (Google / GitHub / Cafe24 Public)**

- 응답: `{ authUrl, state }`.
- UI: 새 창으로 OAuth 팝업 열고 `toast.success("Scope request window opened" / "권한 요청 창을 열었어요")`. 팝업 닫힘 시 success 면 부모 페이지가 `credentials.scopes` 병합 결과를 refetch.
...
- UI:
  ...
  - **즉시 토스트** — `toast.info(<title>)` 한 번 호출 (alert 표시와 동시 신호).
  - **다음 mutate 시 reset** — `onMutate` 에서 alert 를 null 로 리셋해 사용자가 새 요청을 시작하면 옛 안내가 사라진다.
  - **refetch 미실행** — Cafe24 측 후속 작업 완료까지 실제 token 변화가 없으므로 본 분기에서는 부모 페이지의 `onChanged()` 콜백을 호출하지 않는다 ("테스트 실행" 콜백 성공 시 별도 경로로 refresh 된다).
```

§4.4 표 하단에 한 줄: `> 비OAuth 연동에서는 이 탭이 숨겨진다.` 는 그대로 유지.

## 변경 2 — Rationale "Cafe24 install_token mismatch 회복 흐름" cross-reference 보강 (consistency W-1)

현행 `line 1014` 의 "옛 폐기된 '100건 mall_id 스캔 + trial HMAC' 과 형태는 비슷하나 …" 문장에 다음 두 가지를 보강한다.

1. **명시적 Rationale 항목 참조** — `"install_token 을 App URL path 식별 키로 승격" Rationale 항 참조` 를 본문에 박는다 (현재 그 항목명이 직접 명시되지 않아 검색·역추적이 불편).
2. **N ≤ 2 상한의 구조적 보장 근거** — V046 partial UNIQUE `(workspace_id, mall_id) WHERE service_type='cafe24' AND mall_id IS NOT NULL` 이 같은 workspace + 같은 mall_id 의 row 를 최대 1개로 제한한다는 SQL 구조 사실을 명시. "보통 1~2건" 의 1~2 가 어디서 나오는지 본문 안에서 자명해진다 (한 workspace 의 1 + 다른 workspace 의 1 = 최대 2, 같은 workspace 안에선 1 보장).
3. **TOCTOU 부재 명시** (consistency I-5) — 회복 분기는 SELECT + HMAC verify 만 수행하는 read-only 조회로, INSERT/UPDATE 가 없어 TOCTOU 위험이 없음을 한 문장 추가.

## 변경 3 — 구 flat 경로 참조 교정 (consistency W-2)

`line 903` 의 `(참고: review/consistency/2026-05-14_18-23-55)` 를 nested ISO 형식 `review/consistency/2026/05/14/18_23_55` 로 교체. 실제 디렉토리는 이미 마이그레이션 완료 (`ls review/consistency/2026/05/14/18_23_55/` 존재 확인).
...
- [ ] consistency-check --spec 호출
- [ ] Critical 0 확인 후 spec 본문 반영
- [ ] cafe24-request-scopes-ui-b6e34d worktree 의 plan follow-up 체크박스 cross-mark (해당 worktree 가 살아있어 직접 편집 가능)
- [ ] plan complete 이동
...
- [x] expanded → **Full logo (dark variant) on `bg-[#111e14]` h-14 헤더 row 전체** (commits `ecc94a6f`, `fddc426d`). 옛 *"theme=auto size=150"* 계획에서 라이트 모드 시인성 이슈로 dark 컨테이너 + dark variant 로 정착.
- [x] collapsed → `<LogoMark theme="dark" size={32}>` (자체 dark 라운드 rect 보유). 같은 dark band 안에 자연스럽게 blend.
...
- [x] `(auth)/layout.tsx` 의 카드 컨테이너 위에 풀로고 중앙 배치. **최종 형태** (commits `df1533ab`, `fddc426d`): 그라데이션 배경 (Shadcn neutral) 위에 `rounded-2xl bg-[#111e14]` 라운드 컨테이너 + `<Logo variant="full" theme="dark" size={200} />`. 옛 *"soil-50 단색"* 계획은 테마 롤백으로 그라데이션 복원, 시인성 위해 dark pill 추가.
- [x] 로고는 non-link 이미지 (a11y smoke 회귀 잠금, commit `3af19e21`).
...
사용자가 UI에서 설정한 **리터럴 값**은 **`config` 에만** 존재하고, 해당 값을 `output` 에
