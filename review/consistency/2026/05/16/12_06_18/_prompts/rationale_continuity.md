...
**알림 정책 (§11.2)**: `integration_expired` 알림은 여전히 `expired` 전이에만 발사. `error(auth_failed)` 전이는 별도 알림 없이 UI 배지로만 통지. 향후 별도 알림 타입 필요 시 `integration_action_required` 등 신설 검토.
...
- Review skip 조건 변경 시: `prompts/sy
