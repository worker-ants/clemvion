...
## Target 문서
경로: `plan/in-progress/spec-draft-cafe24-cleanup.md`
...
같은 패턴 (KeyValueEditor + object-shaped backend contract) 이 다른 통합 노드에 도입될 때 동일 결정을 그대로 적용한다. backend 가 받는 직렬화 형식 (`Record<string, unknown>`) 은 불변이다.

> 출처: consistency-check 세션 `review/consistency/2026/05/16/09_03_04/` (INFO 1·2 — cross_spec + rationale_continuity).
```
...
```markdown
| 2026-05-16 | 본문 정리 (코드/계약 무변경) — §2 설정 UI 에 fields 편집 버퍼 분리 원칙 한 줄 추가, §9 Rationale 에 §9.9 신설 (PR #62 후속), §9.7 본문 위치 정정 (9.8 뒤에 orphan 으로 있던 본문을 9.7 헤더 바로 뒤로 이동), §5 Case 번호 연속화 (5.1·5.3·5.8 → 5.1·5.2·5.3). 출처: `review/consistency/2026/05/16/09_03_04/` INFO 1·2·3·4. |
```
...
- **코드/API/데이터모델 변경**: 없음
- **다른 spec 영향**: cross-reference 검색 후 결정 (예상: 없음)

## 작업 항목

- [x] Draft 작성
- [ ] cross-reference 외부 검색 (다른 spec 에서 §5.3 / §5.8 / §9.7 / §9.8 anchor 참조 있는지)
- [ ] `/consistency-check --spec` 호출
- [ ] Critical 0 확인 후 spec 본문 반영
- [ ] CHANGELOG 갱신
- [ ] 위임 plan + 본 draft `plan/complete/` 로 이동
- [ ] PR 등록
