...
- **의존성 취약점 audit·핀 거버넌스**: CI(`deps-security-checks.yml`)가 (1) `pnpm audit --audit-level=moderate` 로 신규 moderate+ 취약점을 PR·주간 스케줄에서 차단하고, (2) `scripts/check-pnpm-security-config.py` 로 `pnpm-workspace.yaml` 의 `overrides`(키+**값**)·`onlyBuiltDependencies`·`auditConfig.ignoreCves` 가 기대 baseline 과 일치하는지(핀 무단 삭제·**값 약화**·허용목록 변경·**무단 CVE 억제** 방지) 검증한다. 검토 후 **수용**하는 취약점은 `pnpm-workspace.yaml` 의 `auditConfig.ignoreCves` 에 **사유·영향경로·해소조건 주석과 함께** 등재한다(무단 억제 금지). 핀을 의도적으로 추가/제거할 때는 `pnpm-workspace.yaml` 과 `check-pnpm-security-config.py` 의 `EXPECTED_*` 를 **함께** 갱신한다(2-place 편집 = 리뷰 게이트). 단 위 audit 은 **pnpm 워크스페이스만** 커버한다 — 워크스페이스 밖 독립 npm 트리(하네스 툴링 `.claude/tools/mermaid-lint` 등)는 `.github/dependabot.yml` 의 `npm` ecosystem 항목이 별도로 커버한다(주간 스케줄·리액티브: 신규 CVE 를 자동 업데이트 PR 화하되, PR 시점 신규 의존성 도입을 능동 차단하진 않는다). 이 트리를 pnpm 워크스페이스로 흡수하기 전까지 두 경로가 병존한다.
