...
- `codebase/frontend/src/lib/docs/__tests__/guide-error-code-existence.test.ts` — 유저 가이드가 **에러 코드 문맥**에서 이름 붙인 UPPER_SNAKE 토큰이 backend·packages 소스에 실재하는지 검증(3축: `<FieldTable>` 의 `name` · `code:` 값 · 실패 문맥 산문). 베이스라인 0, 허용목록 없음. **한계**: *존재* 검사이지 *방출* 검사가 아니다 — 에러 **메시지 접두**로만 쓰이는 토큰도 통과한다(실측 사례: `MAKESHOP_UNRESOLVED_PATH_PARAM`). SoT: `spec/conventions/user-guide-evidence.md §2`
