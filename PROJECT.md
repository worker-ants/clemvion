...
- **테스트 프레임워크 이원화 (정책)**: backend·내부 packages = jest / frontend·channel-web-chat = vitest. 신규 패키지는 소속 스택의 기본값을 따른다 — `.claude/tools/run-test.sh` wrapper 가 양쪽을 묶어 한 단계로 운용한다. packages/* 의 vitest 이행은 jest 가 실제로 막는 ESM 의존이 등장하는 별도 트리거 전까지 보류한다. **그 트리거는 2026-09-24 backend 에서 한 번 발화했고(`@nestjs/typeorm@12` 의 `import.meta.url`), 이행이 아니라 두 줄로 풀렸다** — jest 를 `node --experimental-vm-modules` 로 띄우고 `transformIgnorePatterns` 를 기본값으로 되돌리면 jest 가 ESM 을 네이티브로 로드한다(두 방향 실측과 불변식 가드는 `codebase/backend/src/repo-guards/__tests__/esm-native-load.spec.ts` 헤더에 있다). packages/* 가 같은 벽을 만나면 **이행 판단에 앞서 이 레버를 먼저 검토할 것** — 다만 적용·검증된 것은 backend 뿐이고 packages/* 에서는 아직 재지 않았다.
- **버전 핀 정책**: (a) 기본 caret(`^`) — `pnpm-lock.yaml` 이 재현성의 단일 진실(SoT)이므로 선언 범위는 넓게 둔다. 보안 취약점 등으로 전이 의존성을 강제할 때는 **`pnpm-workspace.yaml` 의 `overrides`** 를 쓴다 (pnpm 10.23 부터 `package.json` 의 `pnpm.overrides` 는 install 시 무시되므로 — `The "pnpm" field ... is no longer read` — pnpm 10 정규 위치인 `pnpm-workspace.yaml` 사용. npm `overrides` 와 달리 workspace-global, 패키지별 충돌 시 버전-레인지 `pkg@range` 로 스코프). native 빌드 lifecycle-script 허용목록도 같은 파일의 `onlyBuiltDependencies` 로 관리한다. (b) exact·tilde 핀은 **사유 주석 필수** — package.json 인접 `"//pin"` 필드에 근거를 명시한다 (예: sanitize 경로 공급망 무결성 = `dompurify`·`marked`, 0.x semver 의 minor-breaking 위험 = `three ~0.184.0`, monorepo 전역 버전 정렬 = `react`·`react-dom`). (c) 사유 없는 핀은 caret 으로 완화한다.
- **의존성 취약점 audit·핀 거버넌스**: CI(`deps-security-checks.yml`)가 (1) `pnpm audit --audit-level=moderate` 로 신규 moderate+ 취약점을 PR·주간 스케줄에서 차단하고, (2) `scripts/check-pnpm-security-config.py` 로 `pnpm-workspace.yaml` 의 `overrides`(키+**값**)·`onlyBuiltDependencies`·`auditConfig.ignoreCves` 가 기대 baseline 과 일치하는지(핀 무단 삭제·**값 약화**·허용목록 변경·**무단 CVE 억제** 방지) 검증한다. (3) `scripts/check-override-floors.py` 로 **이미 override 로 관리 중인 패키지가 다시 취약해졌는지**(바닥 침식) 를 좁혀 검출한다 — audit 이 이미 보고하는 것의 부분집합이지만, "판단이 필요한 신규 취약점" 과 "값만 올리면 되는 낡은 핀" 은 조치 성격이 달라 한 목록에 섞이면 후자가 묻힌다(#1038 이 17건 중 4건으로 그 상태였다). 따라서 override 값을 바꿀 때는 이 게이트도 함께 통과해야 한다. 검토 후 **수용**하는 취약점은 `pnpm-workspace.yaml` 의 `auditConfig.ignoreCves` 에 **사유·영향경로·해소조건 주석과 함께** 등재한다(무단 억제 금지). 핀을 의도적으로 추가/제거할 때는 `pnpm-workspace.yaml` 과 `check-pnpm-security-config.py` 의 `EXPECTED_*` 를 **함께** 갱신한다(2-place 편집 = 리뷰 게이트). 단 위 audit 은 **pnpm 워크스페이스만** 커버한다 — 워크스페이스 밖 독립 npm 트리(하네스 툴링 `.claude/tools/mermaid-lint` 등)는 `.github/dependabot.yml` 의 `npm` ecosystem 항목이 별도로 커버한다(주간 스케줄·리액티브: 신규 CVE 를 자동 업데이트 PR 화하되, PR 시점 신규 의존성 도입을 능동 차단하진 않는다). 이 트리를 pnpm 워크스페이스로 흡수하기 전까지 두 경로가 병존한다.
- **빌드 툴체인 major 자동 bump 차단**: 툴체인 패키지의 **major** 상향은 dependabot 자동 PR 대상에서 뺀다 — 현재 `typescript` 1건(`.github/dependabot.yml` 의 `ignore` + `update-types: ["version-update:semver-major"]`). **결속**: 이 개수는 `.github/dependabot.yml` 의 해당 `ignore` 블록 항목 수와 항상 같아야 한다 — 항목을 추가/제거할 때는 이 문장의 카운트도 같은 커밋에서 함께 갱신할 것(2-place 편집). minor/patch 는 계속 받고, 별개 토글인 security update 도 patch 로 오는 한 영향 없다.
  - `typescript` 근거: `#1047` 이 `typescript 5.9.3 → 7.0.2` 를 올려 main 빌드가 전면 실패했다(Jenkins Clemvion/337 — backend `nest build` · frontend `pnpm install` 둘 다). TS7 은 Go 재작성판이라 JS compiler API 를 `typescript/unstable/*` 로 옮겼고, `require('typescript')` 가 `{version, versionMajorMinor}` 스텁만 돌려준다 — nest CLI·ts-jest·fork-ts-checker·typescript-eslint 가 전부 그 API 소비자다. 즉 TS major 는 "의존성 하나" 가 아니라 **툴체인 전체의 계약**이라 사람이 올릴 판단을 해야 한다. 계약 자체의 검사는 버전 상한 하드코딩이 아니라 `codebase/frontend/src/lib/repo-guards/__tests__/typescript-toolchain.test.ts` 가 맡는다(compiler API 실재 + 워크스페이스 major lockstep) — 상한을 박으면 정당한 상향까지 막는 마찰이 되기 때문이다.
  - `eslint-plugin-unicorn` 근거 (역사적 각주, ~~2026-08-28 부로 ignore 항목 제거~~): dependabot `#1049` 가 backend 전용 devDependency 를 `^56.0.1` → `^72.0.0`(16 major) 로 올려 `eslint.config.mjs` 의 pin 근거 주석(v57+ 는 eslint peer floor 가 backend 선언 floor `^9.18` 을 넘는다)을 깨고 unmet peer(`eslint@>=10.4`, 설치본 9.39.4) 를 만들었다 — 값만 바뀌고 주석은 갱신되지 않아 코드-문서가 어긋난 채 머지됐다. `#1058`(TS 롤백) 의 TEST WORKFLOW 로그를 사람이 읽고서야 발견되어 되돌렸다. **2026-08-28 eslint 10 상향으로 전제 소멸**(backend·packages 9개 워크스페이스 `^10.9.1`, unicorn 도 `^73` 으로 동반 상향) — `ignore` 항목은 제거했고, 재발 방지는 상시 가드로 이관됐다: `codebase/backend/src/repo-guards/__tests__/eslint-unicorn-peer.spec.ts`(`unicorn/catch-error-name` 실발화 + 설치본 peer eslint range ↔ backend 선언 eslint range 정합 실측) + CI install 의 `--strict-peer-dependencies`(unmet peer 즉시 실패). 버전별 eslint peer floor 실측 표의 SoT 는 여전히 `codebase/backend/eslint.config.mjs`(`unicorn/catch-error-name` 등록 블록 주석).
- **Node 지원 floor**: 내부 앱(backend·frontend·channel-web-chat)·내부 packages = `engines.node >=24` (운영 `node:24`·CI 와 정렬 — 실제 빌드·테스트되는 환경만 약속). 외부 배포 SDK(`@workflow/sdk`·`@workflow/web-chat`) = `>=20` (소비자 호환폭 보존). `@types/node` 도 동일 기준으로 정렬한다. README "20+" 표기는 "외부 SDK 소비 기준" 을 의미한다. engines 는 engine-strict 미사용 시 advisory 다. **backend floor 를 내릴 때의 숨은 결속**: backend 런타임은 CJS 인데 `@nestjs/typeorm@12` 는 ESM-only(`type: module`)라 Node 의 **`require(esm)`** 로 로드된다 — 그 패키지의 `engines` 는 `>=20.19.0` 이고, 현 floor(24)에서 e2e 부팅으로 확인했다(2026-09-24). engines 가 advisory 라 floor 를 낮춰도 **설치는 조용히 성공하고 부팅에서야 깨진다** — 낮출 때는 backend 부팅을 먼저 확인할 것.
...
| **신규 BullMQ 큐 추가** (`@Processor` 신설 + `MONITORED_QUEUES` 등록) | (a) `codebase/backend/src/modules/system-status/system-status.constants.ts` 의 `MONITORED_QUEUES` 등록<br>(b) `codebase/backend/test/system-status.e2e-spec.ts` 의 `EXPECTED_QUEUE_NAMES`<br>(c) `spec/5-system/16-system-status-api.md` §1 모니터링 큐 표<br>(d) `spec/data-flow/0-overview.md` §3 인프라 큐 개수 + §4 BullMQ 큐 카탈로그 | `make e2e-test` (system-status e2e) |
...
| **신규 backend zod `ui.label` / `hint` / `group` / `itemLabel` 값** | `codebase/frontend/src/lib/i18n/backend-labels.ts` 의 `LABEL_KO` / `HINT_KO` / `GROUP_KO` / `ITEM_LABEL_KO` / `OPTION_LABEL_KO` 중 적절한 매핑에 동일 PR 안에서 한국어 등록. SoT: `spec/conventions/i18n-userguide.md §Principle 3-B` | `pnpm --filter frontend test -- ui-label-parity` |
...
| **AuthConfig type enum 변경** (`api_key` / `bearer_token` / `basic_auth` / `hmac`) | (a) `spec/1-data-model.md §2.17` + §2.17.1<br>(b) `spec/2-navigation/6-config.md` Part A AUTH_TYPES<br>(c) `codebase/frontend/src/lib/i18n/dict/{ko,en}/authentication.ts` 의 type 라벨 + 폼 helper text<br>(d) `codebase/frontend/src/content/docs/06-integrations-and-config/` 의 인증 설정 안내 페이지<br>(e) DB migration (CHECK 제약) | `pnpm --filter frontend test -- i18n docs` |
...
- `codebase/frontend/src/lib/docs/__tests__/guide-identifier-existence.test.ts` — 유저 가이드가 이름 붙인 UPPER_SNAKE **식별자**(에러 코드 + 환경변수)가 실재하는지 검증. 기준집합 = backend·packages 소스 토큰 **∪ env 선언처**(`.env.example`·compose). 3축: `<FieldTable>` 의 `name` · `code:` 값 · **백틱 전수**. 베이스라인 0. **외부 어휘 허용목록**(`GUIDE_EXTERNAL_VOCABULARY`)이 있고 테스트가 4가지를 강제한다 — 외부 시스템 이름 의무 · 상한 · 여전히 인용될 것 · 기준집합에 없을 것. **발행 축**(2026-09-13 추가): *"소스에 **메시지 접두로만** 등장하고 `3-error-handling.md` 카탈로그에도 없는"* 인용은 `GUIDE_NON_EMITTED_VOCABULARY` 에 **사유와 함께 등록**해야 통과한다 — 카탈로그는 요구 조건이 아니라 **탈출구**다(요구 조건으로 쓰면 카탈로그 미등재 25종에 거짓 RED 가 난다). **잔여 한계**: 여전히 *존재* 검사에 가깝다 — 소비자·분류기 목록이 토큰을 따옴표로 인용하면 접두-전용 판정이 풀려 통과한다(`spec/5-system/3-error-handling.md §1.4` 가 그 구분을 명시). 아무도 읽지 않는 env 변수도 통과한다. SoT: `spec/conventions/error-codes.md` + `spec/5-system/3-error-handling.md §1` — **`user-guide-evidence.md §2` 표에는 아직 없다**(가족 규약은 거기지만 이 가드의 등재는 planner 트래커 대기)
...
- **timeout**: positive wait(`toBeVisible`·`waitForURL` 등)에 전역 기본(`playwright.config.ts` 의 `expect.timeout`, 현재 10s) **미만의 하드코딩 timeout 을 지정하지 않는다** — 개별 `{ timeout: N }` 이 전역보다 항상 우선하므로 sub-global 은 전역 상향의 slack 을 차단해 부하 시 flake 를 유발한다(PR #872). 명시 timeout 을 빼 전역에 위임하고, 더 필요하면 전역 초과값·명명 상수(`PAGE_READY_TIMEOUT` 등)를 쓴다. 가드: `src/__tests__/e2e-no-sub-global-timeout.test.ts`(unit, CI 차단)
...
docker compose exec backend pnpm run cleanup:queue-jobs           # dry-run — 후보만 출력
docker compose exec backend pnpm run cleanup:queue-jobs:apply     # --apply --pause-during-sweep
