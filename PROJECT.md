...
- **버전 핀 정책**: (a) 기본 caret(`^`) — `pnpm-lock.yaml` 이 재현성의 단일 진실(SoT)이므로 선언 범위는 넓게 둔다. 보안 취약점 등으로 전이 의존성을 강제할 때는 `pnpm.overrides` 를 쓴다 (npm `overrides` 와 달리 workspace-global — 패키지별 충돌 시 버전-레인지 `pkg@range` 로 스코프). **⚠️ 주의(2026-07 실측)**: pnpm 10.23 은 `package.json` 의 `pnpm.overrides`·`pnpm.onlyBuiltDependencies` 를 **더 이상 읽지 않는다**(`The "pnpm" field ... is no longer read` 경고). 현재 핀은 `pnpm-lock.yaml` 관성으로만 유지되므로, 신규 오버라이드는 pnpm 10 정규 위치인 `pnpm-workspace.yaml` 에 추가해야 실제 적용된다. 기존 `package.json` 핀의 정규 위치 이전은 미완 — [`plan/in-progress/pnpm-migration-followups.md`](plan/in-progress/pnpm-migration-followups.md) §1 부수 발견 참조. (b) exact·tilde 핀은 **사유 주석 필수** — package.json 인접 `"//pin"` 필드에 근거를 명시한다 (예: sanitize 경로 공급망 무결성 = `dompurify`·`marked`, 0.x semver 의 minor-breaking 위험 = `three ~0.184.0`, monorepo 전역 버전 정렬 = `react`·`react-dom`). (c) 사유 없는 핀은 caret 으로 완화한다.
