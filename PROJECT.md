...
- **버전 핀 정책**: (a) 기본 caret(`^`) — `pnpm-lock.yaml` 이 재현성의 단일 진실(SoT)이므로 선언 범위는 넓게 둔다. 보안 취약점 등으로 전이 의존성을 강제할 때는 루트 `package.json` 의 `pnpm.overrides` 를 쓴다 (npm `overrides` 와 달리 workspace-global — 패키지별 충돌 시 버전-레인지 `pkg@range` 로 스코프). (b) exact·tilde 핀은 **사유 주석 필수** — package.json 인접 `"//pin"` 필드에 근거를 명시한다 (예: sanitize 경로 공급망 무결성 = `dompurify`·`marked`, 0.x semver 의 minor-breaking 위험 = `three ~0.184.0`, monorepo 전역 버전 정렬 = `react`·`react-dom`). (c) 사유 없는 핀은 caret 으로 완화한다.
