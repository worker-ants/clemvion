...
- **의존성 취약점 audit·핀 거버넌스**: CI(`deps-security-checks.yml`)가 (1) `pnpm audit --audit-level=moderate` 로 신규 moderate+ 취약점을 PR·주간 스케줄에서 차단하고, (2) `scripts/check-pnpm-security-config.py` 로 `pnpm-workspace.yaml` 의 `overrides`/`onlyBuiltDependencies` 가 기대 baseline 과 일치하는지(보안 핀 무단 삭제 방지) 검증한다. 검토 후 **수용**하는 취약점은 `pnpm-workspace.yaml` 의 `auditConfig.ignoreCves` 에 **사유·영향경로·해소조건 주석과 함께** 등재한다(무단 억제 금지). 핀을 의도적으로 추가/제거할 때는 `pnpm-workspace.yaml` 과 `check-pnpm-security-config.py` 의 `EXPECTED_*` 를 **함께** 갱신한다(2-place 편집 = 리뷰 게이트).
