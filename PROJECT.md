...
- **빌드 툴체인 major 자동 bump 차단**: 툴체인 패키지의 **major** 상향은 dependabot 자동 PR 대상에서 뺀다 — 현재 `typescript`·`eslint-plugin-unicorn` 2건(`.github/dependabot.yml` 의 `ignore` + `update-types: ["version-update:semver-major"]`). **결속**: 이 개수는 `.github/dependabot.yml` 의 해당 `ignore` 블록 항목 수와 항상 같아야 한다 — 항목을 추가/제거할 때는 이 문장의 카운트도 같은 커밋에서 함께 갱신할 것(2-place 편집). minor/patch 는 계속 받고, 별개 토글인 security update 도 patch 로 오는 한 영향 없다.
  ...
  - `eslint-plugin-unicorn` 근거: dependabot `#1049` 가 backend 전용 devDependency 를 `^56.0.1` → `^72.0.0`(16 major) 로 올려 `eslint.config.mjs` 의 pin 근거 주석(v57+ 는 eslint peer floor 가 backend 선언 floor `^9.18` 을 넘는다)을 깨고 unmet peer(`eslint@>=10.4`, 설치본 9.39.4) 를 만들었다 — 값만 바뀌고 주석은 갱신되지 않아 코드-문서가 어긋난 채 머지됐다. `#1058`(TS 롤백) 의 TEST WORKFLOW 로그를 사람이 읽고서야 발견되어 되돌렸다. 버전별 eslint peer floor 실측 표의 SoT 는 `codebase/backend/eslint.config.mjs`(`unicorn/catch-error-name` 등록 블록 주석) — `.github/dependabot.yml`·plan 문서는 그 표를 참조만 하고 중복 기재하지 않는다. 계약 자체의 검사는 `codebase/backend/src/repo-guards/__tests__/eslint-unicorn-peer.spec.ts` 가 맡는다(`unicorn/catch-error-name` 실발화 + 설치된 unicorn peer eslint range ↔ backend 선언 eslint range 정합).
...
- `codebase/frontend/src/lib/docs/__tests__/plan-frontmatter.test.ts` — plan 라이프사이클 가드 **3종**: (1) top-level `plan/in-progress/*.md` 의 `worktree`(sentinel `(unstarted)` 허용)/`started`/`owner` frontmatter 강제, (2) `plan/complete/**` 가 `status` 를 선언했다면 종료 상태여야 함(`complete`/`implemented`/`applied`/`superseded`; 선언 자체가 없으면 위반 아님 — 선택 필드), (3) top-level 살아있는 plan 의 상대링크 무결성(`plan/complete/**` 는 시점 기록이라 제외). 판정 로직은 `plan-scan.ts`(수집·status)와 `spec-links.ts`(링크). SoT: `.claude/docs/plan-lifecycle.md §4`
