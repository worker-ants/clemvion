...
#   - backend/src/migrations.spec.ts (유닛테스트, 매 빌드/CI)
...
#     SQL_DIR 기본값 /flyway/sql. backend/migrations/Dockerfile 의 RUN 단계에서
...
# sed 패턴은 backend/src/migrations.spec.ts 의 SQL_NAME_RE 와 동일 규약을 따른다.
