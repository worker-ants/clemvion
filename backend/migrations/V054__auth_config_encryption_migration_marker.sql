...
--   남아있다. 운영 환경에서는 backend/scripts/encrypt-auth-config.ts 를 1회
...
BEGIN
    ...
    IF plaintext_count > 0 THEN
        RAISE NOTICE
            'V054: % auth_config row(s) still hold plaintext config. Lazy migration on next write OR run backend/scripts/encrypt-auth-config.ts to eager-migrate.',
            plaintext_count;
    END IF;
END $$;
