-- V035a: execution_node_log append-only 테이블 + 기존 execution_path 데이터 이행
--
...
--
-- 본 V035a 는 신규 테이블 생성 + 데이터 이행만 수행한다 (DROP 미포함).
-- INSERT...SELECT 가 큰 운영 DB 에서 시간이 걸려도 read traffic 을 막지
-- 않도록 executeInTransaction=false 로 단계 분리 — 트랜잭션 lock 확장
-- 회피. 컬럼 DROP 은 별도 후속 배포 (V035b) 에서 적용한다.
--
-- 컬럼 자체는 V035b 까지 유지되며 application 코드는 컬럼을 더 이상 읽지
-- 않으므로 (V035a 배포 시점에 새 코드도 함께 배포), 옛 코드와 새 코드의
