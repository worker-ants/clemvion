-- V034: node_executions (execution_id, node_id, started_at DESC) 복합 인덱스
--
-- WARN #3 (DB) 조치 — execution-engine.service.ts 의 다음 3개 호출 패턴이
-- node_executions 의 (execution_id, node_id) 로 lookup 후 startedAt DESC 로
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_node_executions_exec_node_started_desc
