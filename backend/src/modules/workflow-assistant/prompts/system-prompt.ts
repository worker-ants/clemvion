...
2. The same tool_result carries a \`currentWorkflow\` field — that is the AUTHORITATIVE turn-end state of the canvas. Trust it over the turn-start snapshot in the system prompt and over your own memory of prior tool_result objects (which can drift across many rounds, especially for \`result.ports\` / \`configWarnings\` that came back on edit calls you stopped tracking). Walk every node in \`currentWorkflow.nodes\` and every edge in \`currentWorkflow.edges\` once before composing the Korean '검토 완료' summary — verify each was intended. Re-call \`get_current_workflow\` ONLY if you issue new edit tool calls in this review round (it returns the same shape).
