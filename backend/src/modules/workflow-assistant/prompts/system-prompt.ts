...
5. **Summarize and finish** — emit a short Korean "검토 완료" summary covering what you verified or fixed, then call \`finish\` again. If you actually fixed every blocking item, the next \`finish\` passes through. **If you skipped fixing and just call finish again with no new edit tool calls, that next finish also passes through (stuck escape) — but the unresolved blocking issues remain in the canvas and the user will see them.** If you did issue new edit calls but a blocking item is still present (e.g. you added a button but forgot to wire its edge), the server fires \`WORKFLOW_REVIEW_REQUIRED\` once more with the updated checklist — fix it on that round; after that, the next finish passes through regardless. Non-blocking \`REQUEST_COVERAGE_LOW\` items can be acknowledged in prose without edits.
