...
| 항목 | 동작 |
|------|------|
| 적용 service_type | 현재 `cafe24`, `makeshop` ([Spec MakeShop 노드 §8](../4-nodes/4-integration/5-makeshop.md#8-ai-agent-노출-internal-mcp-bridge)) — 향후 first-party 통합(예: Shopify, Naver Smartstore)이 같은 패턴 사용 가능 |
| 구현 형태 | backend 모듈이 AI Agent 핸들러의 `AgentToolProvider` 인터페이스를 직접 구현 (`Cafe24McpToolProvider`, `MakeshopMcpToolProvider`). HTTP fetch 가 아니라 직접 함수 호출 |
...
>
> **Production fail-closed 강제 (refactor 04 M-7)**: 위 "절대 금지" 를 enforcement 로 일치시킨다 — `NODE_ENV=production` 에서 `MCP_ALLOW_INSECURE_URL=true` 면 부팅을 거부한다 (`main.ts` 의 `assertProductionConfig`, `OAUTH_STUB_MODE`/`LLM_STUB_MODE` 와 동형). 정당한 self-host 용도(VPC 내부 호스트 등)가 있는 `ALLOW_PRIVATE_HOST_TARGETS`(HTTP Request·Database Query·Send Email 통합 노드 SSRF 가드 **공통** 플래그, [1-http-request §4](../4-nodes/4-integration/1-http-request.md#4-실행-로직))는 정책이 달라 **throw 가 아닌 warn** 으로 분리한다 — production 에서 켜져 있으면 부팅은 하되 경고 로그를 남긴다. 즉 "절대 금지" 플래그는 throw, "정당 용도 있는" 플래그는 warn 이 분류 기준이다.

### 3.3 capabilities 캐시 _(비채택 won't-do — 측정된 병목 부재)_

> **비채택 (won't-do, 2026-07-16)**: `credentials.cached_capabilities` 캐시는 **채택하지 않는다**. 노드 설정 UI 미리보기는 매번 live `initialize` 결과(§9 연결 테스트)를 사용하는 현행 방식이 정식 경로이며, 이는 캐시보다 **항상 최신**이라는 정확성 이점도 갖는다. 본 캐시는 순수 성능 최적화(첫 렌더 지연 단축)인데 그 전제인 측정된 병목이 없고, Integration credentials JSONB 구조 변경(infra) + 캐시 무효화 문제를 새로 들여온다 (근거 §Rationale `R-wontdo-cached-capabilities`). 아래 본문은 **비채택된 참고 설계**이며, 재개 트리거는 Rationale 참조.

`Integration.last_error` 와 별개로, 서버 등록 시 1회 `initialize` 응답의 `capabilities` 객체를 `credentials.cached_capabilities` (write-only로 처리하지 않음, 메타데이터) 에 저장해 노드 설정 UI 의 즉시 미리보기에 활용할 수 있다. **저장된 capabilities 는 hint 일 뿐, 실제 실행 시점에 다시 조회한 결과를 우선한다.** ← 이 "hint 일 뿐" 성질이 곧 비채택 근거다(정확성을 위해 어차피 재조회하므로 캐시가 없애는 것은 첫 렌더 지연뿐).
...
MCP 서버가 노출하는 도구 **정의(스키마)** 는 AI Agent 의 **도구 정의 payload 예산**(직렬화 bytes 기준)에 합산된다 — [AI Agent §4.2](../4-nodes/3-ai/1-ai-agent.md#42-도구-정의-payload-예산-tool-definition-payload-budget). 이는 §5.7 의 도구 **호출 횟수** 한도(`maxToolCalls`)와 **별개 축**이다 (호출 횟수 vs 정의 크기).

대형 카탈로그(Cafe24 Admin API **485** operation — [4-cafe24 §지원 범위](../4-nodes/4-integration/4-cafe24.md) SoT, 2026-07-17 실측 · MakeShop **161** REST operation — [5-makeshop](../4-nodes/4-integration/5-makeshop.md); 실제 노출 수는 granted scope 로 필터됨)를 `enabledTools`(§5.6) allowlist 없이 전량 노출하면 도구 정의 payload 가 프롬프트를 provider transport timeout 너머로 밀어 provider 무관하게 응답 실패(hang/무한 재시도)를 유발할 수 있다. 따라서 대형 서버는 `mcpServers[].enabledTools` 로 노출 도구를 좁히는 것이 **사실상 필수**다 — 이들 카탈로그는 bytes 예산 이전에 **도구 개수만으로도** `AI_AGENT_TOOL_COUNT_MAX` 기본값(128)을 상시 초과하므로, allowlist 없이는 정상 설정으로 보이는 상태에서도 런타임 pre-flight 가 실패한다(권장 이상의 강도 — [AI Agent §4.2 대형 카탈로그 주의](../4-nodes/3-ai/1-ai-agent.md#42-도구-정의-payload-예산-tool-definition-payload-budget)). 예산 초과 판정(런타임 pre-flight 에러코드 `TOOL_DEFINITION_PAYLOAD_EXCEEDED`)·저장 시점 경고(graph warning `ai_agent:tool-payload-budget`)의 단일 진실은 [AI Agent §4.2](../4-nodes/3-ai/1-ai-agent.md#42-도구-정의-payload-예산-tool-definition-payload-budget)/[§10](../4-nodes/3-ai/1-ai-agent.md#10-에러-코드).
...
> **구현 현황 (2026-07-06 갱신)**: `mcpDiagnostics` 는 아래 예시의 **구조화 객체** (`attempted` / `serverCount` / `toolCalls` / `resourceReads` / `promptGets` / `serverSummaries[]` / `errors[]`) 로 emit 된다 (`mcp-diagnostics.ts` `finalizeMcpDiagnostics` · `ai-turn-executor.ts buildMcpDiagnosticsMeta`). 노드 실행에서 MCP 가 1회도 시도되지 않으면 `mcpDiagnostics` 키 자체를 omit 한다 (meta lean).
>
> - `serverSummaries[]` 를 push 하는 provider 는 Internal Bridge (`Cafe24McpToolProvider`·`MakeshopMcpToolProvider`) 와 외부 `service_type='mcp'` 의 `McpToolProvider` **둘 다**다 — `McpToolProvider` 가 connect+list 성공 시 `connected`(toolCount), serviceType 확정 이후 status/connect/list 실패 시 `skipped`(`skipReason='error'`) serverSummary 를 push 한다 (serverSummaries push 에 한해 Cafe24 와 대칭 — **build 단계** `errors[]` push 는 `McpToolProvider` 전용이나, **call 단계** `errors[]`(mcpErrorDelta)는 Internal Bridge 도 보고한다. serviceType 판정 전 lookup 실패는 double-push 회피 위해 미push).
> - `errors[]` 는 격리된 부분 실패를 granular code + `phase` 로 누적한다:
>   - **build 단계**(connect/`tools/list`) — `MCP_TIMEOUT`/`MCP_CONNECT_FAILED`/`MCP_LIST_FAILED`(§8.2). provider 가 `ctx.mcpDiagnosticErrors` 로 push (`McpToolProvider.openServer`, skipped serverSummary 와 병존).
>   - **call 단계**(`tools/call`/`resources/read`/`prompts/get`/`resources/list`/`prompts/list`) — 서버측 실패. provider 가 `AgentToolResult.mcpErrorDelta` 로 보고하면 핸들러가 execute choke point 에서 push 한다. 외부 MCP 는 `MCP_TIMEOUT`/`MCP_TOOL_ERROR`/`MCP_AUTH_FAILED`/`MCP_CALL_FAILED`, Internal Bridge 는 `CAFE24_*`/`MAKESHOP_*` vocabulary(§2.3). **client-side 실패**(`INVALID_TOOL_ARGUMENTS`·`MCP_UNKNOWN_TOOL`·`*_MISSING_FIELDS` 등)는 서버 실패가 아니므로 `errors[]` 에 담지 않는다(tool_result 로만).
>   - `serverCount` 는 `serverSummaries[]` 중 `status='connected'` 행 수, `attempted` 는 serverSummary·error·counter 중 하나라도 있으면 true 로 파생.
> - `toolCalls` / `resourceReads` / `promptGets` 는 노드 실행의 tool 실행 지점에서 `mcp_*` 호출을 종류별(`tools/call` / `read_resource` / `get_prompt`)로 집계한다(성공/실패 무관 시도 1회). `list_resources`/`list_prompts` discovery 메타도구는 미집계 (§8.3 의 usage 로그 제외와 정합).
>
> **잔여 없음 (2026-07-16)**: 종전 이 자리에 있던 §3.3 `credentials.cached_capabilities` 캐시는 **비채택 (won't-do)** 으로 확정됐다 (§3.3 · §Rationale `R-wontdo-cached-capabilities`). 이로써 본 문서에 미구현(Planned) 표면이 남지 않아 frontmatter 를 `status: implemented` 로 승격하고 `pending_plans` 를 제거했다 (추적하던 `spec-sync-mcp-client-gaps.md` 는 `plan/complete/` 로 종결).
>
...
**`skipReason` vocabulary**. 본 vocabulary 는 **Internal Bridge (cafe24) 경로의 buildTools 단계 skip** 을 위주로 정의된다. 외부 MCP 의 status/connect/`tools/list` 실패는 `serverSummaries[]` 의 `skipped(skipReason='error')` 로 표면화되며 (사용자용 요약), **동시에 `errors[]` 에 granular code (`MCP_TIMEOUT`/`MCP_CONNECT_FAILED`/`MCP_LIST_FAILED`, §8.2) + `phase` 로 누적**된다 (2026-07-06 — `skipReason` 은 요약, `errors[]` 는 코드 granularity 로 역할 분리, 병존).
...
| 상황 | 처리 |
|------|------|
| `initialize` 실패 / `tools/list` 실패 / connect 타임아웃 | 해당 서버 도구는 LLM 에 **노출하지 않음**. `meta.mcpDiagnostics.errors` 에 granular code(§8.2) + `phase` 로 기록. 다른 서버·KB·일반 도구는 정상 노출 |
| `tools/call` 실패 (네트워크 / 5xx / RPC error) | 해당 호출만 실패. LLM 에 `tool_result` 로 `{ "error": "<code>", "message": "..." }` 전달 → LLM 이 graceful 응답 결정. `IntegrationUsageLog`(§8.3) 및 `mcpDiagnostics.errors[]`(phase=`tools/call`, granular code)에 기록 |
...
| 코드 | 의미 |
|------|------|
| `MCP_CONNECT_FAILED` | TCP / TLS / DNS 실패, HTTPS 강제 위반, `initialize` RPC 실패 (프로토콜 버전 불일치 등 포함) — connect 단계의 모든 실패가 하나로 흡수된다. SDK 가 connect 와 initialize 를 하나의 호출로 묶어 처리하므로 두 단계를 의미적으로 분리하기 어려움. **emit 지점: §9 연결 테스트 (`McpTestConnectionService`) + 노드 실행 buildTools (`McpToolProvider.openServer`)** — buildTools 단계의 connect 실패(status precheck 포함)는 `mcpDiagnostics.errors[].code = MCP_CONNECT_FAILED` (phase=`connect`) 로 surface (§6.2) |
| `MCP_LIST_FAILED` | `tools/list` 등 list RPC 실패. **emit 지점: §9 연결 테스트 + buildTools** — buildTools 단계 `tools/list` 실패는 `mcpDiagnostics.errors[].code = MCP_LIST_FAILED` (phase=`tools/list`) 로 surface |
| `MCP_CALL_FAILED` | `tools/call` / `resources/read` / `prompts/get` / `resources/list` / `prompts/list` 실패 (노드 실행 `execute` 경로에서 emit) — `tool_result` + `mcpDiagnostics.errors[]`(phase 별) 양쪽 surface |
| `MCP_TOOL_ERROR` | `tools/call` 응답이 MCP `isError: true` 인 application-level 도구 실패 — 전송은 성공했으나 도구가 에러를 보고. `tool_result.content` 에 서버 content 동봉, `mcpDiagnostics.errors[]`(phase=`tools/call`) 에도 기록 |
| `MCP_TIMEOUT` | §4.4 타임아웃 초과. `withTimeout` 의 `TimeoutError`(및 connect 단계의 AbortController 만료)로 판정하며 **모든 단계**에서 surface — build 단계(connect/`tools/list`)와 call 단계(`tools/call`/`resources/read`/`prompts/get` 등)는 `mcpDiagnostics.errors[].code = MCP_TIMEOUT`(해당 phase), §9 연결 테스트는 응답 `code = MCP_TIMEOUT` 로 노출 |
...
**실패 응답 형식**: `preview-test` 는 실패해도 예외를 던지지 않고 **HTTP 200 OK** body 에 `{ success: false, code, message }` 를 담아 반환한다 (`code` 는 §8.2 vocabulary 중 `MCP_HTTPS_REQUIRED` / `MCP_AUTH_FAILED` / `MCP_CONNECT_FAILED` / `MCP_LIST_FAILED` / `MCP_TIMEOUT` 부분집합 — connect/`tools/list` 가 `TimeoutError` 로 만료되면 `MCP_TIMEOUT`). UI 가 이 body 를 그대로 렌더한다.
...
신규 컬럼 / 신규 엔티티 **없음**. [Integration §2.10](../1-data-model.md#210-integration) 의 `service_type` String 컬럼에 다음 값들이 본 spec 의 영역에서 사용된다 — `mcp` (외부 HTTP transport §2.1), `cafe24`·`makeshop` (Internal Bridge §2.3). 세 값 모두 String 컬럼이므로 enum 마이그레이션 불필요.
...
본 절은 문서 곳곳(§2/§3/§6/§8)에 산재한 주요 설계 결정의 배경과 기각된 대안을 모은다. 본문이 "무엇을" 정의한다면 여기서는 "왜 그렇게" 를 기록한다.

### stdio transport 미지원 (§2.2)
멀티테넌트 SaaS 백엔드에서 사용자별 subprocess 를 spawn 하는 것은 (a) 프로세스·보안 격리 비용, (b) 임의 명령 실행 권한 노출, (c) "워크스페이스 공용 자원" 모델과의 부정합 때문에 MVP 에서 배제했다. Streamable HTTP 단일 transport 로 SDK import 표면을 최소화하고, stdio 는 향후 데스크톱 bridge agent 로 우회 노출하는 별도 spec 으로 분리한다.

### Internal Bridge 신설 및 자가 회복 예외 (§2.3 / §8.4)
같은 Integration(예: Cafe24)이 워크플로 노드와 AI Agent 양쪽에서 쓰이는 케이스를 위해, 외부 MCP 서버 없이 backend in-process 모듈이 `AgentToolProvider` 를 직접 구현하는 Internal Bridge 를 도입했다 — HTTP fetch·SSRF·세션 헤더가 모두 no-op 이 되어 표면이 단순하다. §8.4 의 "401 → 즉시 status 격하" 정책은 **외부 MCP(refresh_token 없는 토큰 모델) 한정**이며, refresh_token 을 보유한 Internal Bridge provider(cafe24)의 401 은 [Cafe24 §6.1](../4-nodes/4-integration/4-cafe24.md#61-인증-실패-자동-status-전환) 의 "refresh + 1회 재시도" 자가 회복을 우선 적용한다. 근거: refresh_token 이 정상 토큰 lifecycle 을 보장하므로 외부 MCP 의 race-of-clock(만료 토큰 일시 회복) 시나리오가 없어 status 깜빡임 우려가 없다. 403(스코프 부족)은 양쪽 모두 즉시 격하.

### 진단 스키마 분리 — `skipReason` vs `errors[].code` (§6.2)
진단 표면을 두 vocabulary 로 의도적으로 분리했다: `serverSummaries[].skipReason` 은 `lower_snake_case` 운영 enum(예: `expired_install_timeout`)으로 `Integration.status_reason` 의미를 캐리하고, `errors[].code` 는 `UPPER_SNAKE_CASE` 에러 코드(§8.2)로 실패 phase 별 granularity 를 담는다. 하나로 합치지 않은 이유: 전자는 "왜 이 통합이 안 보이나"(build 스냅샷)를, 후자는 "어느 RPC 가 어떻게 실패했나"(build+call phase 별)를 답하는 서로 다른 질문이기 때문이다. 또한 provider 입력 슬롯(`ProviderBuildCtx.mcpDiagnostics`=배열)과 최종 출력(`meta.mcpDiagnostics`=객체)은 이름을 공유하되 계층·shape 가 다르다 — provider 는 요약/에러 sub-array 만 채우고 카운터 집계·객체 조립은 핸들러(executor) 책임이라, 소유권 경계를 이름이 아닌 타입으로 구분한다.

### SSRF escape hatch 의 throw-vs-warn 분류 (§3.2)
운영 환경 fail-closed 강제 시, "절대 금지" 플래그(`MCP_ALLOW_INSECURE_URL`)는 부팅 **throw**, "정당 용도 있는" 플래그(`ALLOW_PRIVATE_HOST_TARGETS` — VPC 내부 호스트 등 self-host 정당성)는 **warn** 으로 분류한다. 기준은 플래그에 정당한 프로덕션 용도가 존재하는지 여부다 — 오분류하면 정당한 self-host 배포를 막거나(과도한 throw) SSRF 표면을 열어둔다(과도한 warn).

### timeout 을 별도 `TimeoutError` 로 분류 (§4.4 / §8.2)
`withTimeout` 의 soft-deadline 과 connect 단계 AbortController 만료를 모두 `TimeoutError` 서브클래스로 통일해, 호출자가 message 문자열 매칭 없이 `instanceof` 로 timeout↔기타 실패를 robust 하게 구분한다(→ `MCP_TIMEOUT` vs `MCP_CONNECT_FAILED`/`MCP_CALL_FAILED`). 하위호환을 위해 여전히 `Error` 이며 message 포맷도 불변이다.

### R-wontdo-cached-capabilities — capabilities 캐시 비채택 (§3.3)

**결정 (2026-07-16)**: `credentials.cached_capabilities` 캐시를 **비채택(won't-do)** 한다. 2026-06-03 spec-vs-code audit 이 §3.3 을 "미구현 (Planned)" 으로 표기하고 `spec-sync-mcp-client-gaps.md` 로 추적해 왔으나, 6주간의 유예 끝에 **도입하지 않는 것으로 확정**한다.

**근거**:
- **기능 결손이 아니다** — §3.3 자신이 "저장된 capabilities 는 hint 일 뿐, 실제 실행 시점에 다시 조회한 결과를 우선한다" 고 규정했다. 즉 정확성을 위해 어차피 재조회하므로 캐시가 제거하는 것은 **설정 UI 미리보기의 첫 렌더 지연**뿐이다. 현행 live `initialize` 경로(§9)는 오히려 **항상 최신**이라는 정확성 이점을 갖는다.
- **최적화의 전제인 측정된 병목이 없다** — 2026-06-03 등재 이후 실사용 성능 불만이 제기된 바 없다. 측정 없는 선제 최적화는 본 문서가 §2.2(stdio 미지원)·§3.2(SSRF escape hatch)에서 견지해 온 "비용 대비 실익" 판단과 어긋난다.
- **비용이 국소적이지 않다** — Integration 엔티티 credentials JSONB 스키마 변경은 자격증명 저장 경로 전반에 걸치는 infra 변경이고, 여기에 캐시 무효화(서버측 도구 목록 변경 감지) 라는 새 문제를 들여온다. hint 성 최적화가 치를 값이 아니다.

**재개 트리거**: 설정 UI 미리보기의 live `initialize` 지연이 **실측으로** 문제화되면(예: 대형 카탈로그 서버에서 체감 지연) 신규 plan 으로 재제안한다. 그 시점에는 credentials JSONB 확장이 유일한 선택지가 아니므로 — 별도 캐시 테이블 + TTL, 또는 §5.6 `enabledTools` 기반 지연 로딩 등 — 대안을 함께 평가할 것. 즉 본 결정은 "캐시라는 아이디어의 영구 기각" 이 아니라 **"현재 근거로는 이 구현 형태를 채택하지 않음"** 이다.
