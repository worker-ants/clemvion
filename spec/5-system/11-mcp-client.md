...
> **구현 현황 (2026-06-14 갱신)**: backend 는 `mcpDiagnostics` surface 중 **`serverSummaries[]`** 를 emit 한다 (`mcp-diagnostics.ts` · `ai-agent.handler.ts buildMcpDiagnosticsMeta`). `serverSummaries[]` 를 push 하는 provider 는 Internal Bridge (`Cafe24McpToolProvider`·`MakeshopMcpToolProvider`) 와 외부 `service_type='mcp'` 의 `McpToolProvider` **둘 다**다 — `McpToolProvider` 가 connect+list 성공 시 `connected`(toolCount), serviceType 확정 이후 status/connect/list 실패 시 `skipped`(`skipReason='error'`) serverSummary 를 push 한다(2026-06-14, Cafe24 와 대칭. serviceType 판정 전 lookup 실패는 double-push 회피 위해 미push). 아래 예시의 나머지 필드 (`attempted` / `serverCount` / `toolCalls` / `resourceReads` / `promptGets` / `errors[]`) 는 본 spec 에 정의되어 있으나 **미구현 (Planned)** — `mcpDiagnostics` 가 아직 `serverSummaries[]` 단일 배열이라 위 필드를 담으려면 구조화 객체로의 타입 승격이 필요(전 소비/생산처 동시 변경). 코드 granularity(`MCP_TIMEOUT`/`MCP_CONNECT_FAILED`/`MCP_LIST_FAILED`)는 그 `errors[]` 필드 도입과 함께 surface 예정. 추적: `plan/in-progress/spec-sync-mcp-client-gaps.md`.
