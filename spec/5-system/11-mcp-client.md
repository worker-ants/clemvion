...
> `url` 은 **HTTPS 강제** (테스트 연결 시 `https://` 시작 검증, 미충족 시 `INTEGRATION_INCOMPLETE`). 사내 IP / localhost 차단 정책은 [Spec API §SSRF 가이드](./2-api-convention.md) 와 동일하게 적용한다.
...
`mcpServers[].includeResources: false` / `includeprompts: false` 토글로 capability 단위 옵트아웃 가능 — 기본은 모두 `true` (서버가 보고했다면 노출).
...
| 코드 | 의미 |
|------|------|
| `MCP_CONNECT_FAILED` | TCP / TLS / DNS 실패, HTTPS 강제 위반 등 connect 단계 실패 |
| `MCP_INITIALIZE_FAILED` | `initialize` RPC 실패 (프로토콜 버전 불일치 등 포함) |
| `MCP_LIST_FAILED` | `tools/list` 등 list RPC 실패 |
| `MCP_CALL_FAILED` | `tools/call` / `resources/read` / `prompts/get` 실패 |
| `MCP_TIMEOUT` | §4.4 타임아웃 초과 |
| `MCP_AUTH_FAILED` | 401/403. `Integration.status` 갱신 동반 |
...
| `error` | 실패 시 `{ code, message }` (§8.2 vocabulary) |
| `duration_ms` | RPC 호출 단위의 elapsed |

`tools/list` / `resources/list` / `prompts/list` 같은 setup RPC 는 usage 로그에 기록하지 않는다 (사용자 의도 행위가 아닌 시스템 행위).
