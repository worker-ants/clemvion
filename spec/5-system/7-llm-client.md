...
- **왜 LLMClientFactory 에 통합하지 않았나**: ModelConfig 단일 테이블로 설정은 통합됐으나, rerank 호출은 입력 `(query, docs[])`·출력 score 배열·스트리밍 부재로 chat/embedding 과 API shape 가 다르다. 따라서 `RerankClientFactory` 는 분리 유지하고, `ModelConfig.kind` 로 팩토리를 선택한다.
