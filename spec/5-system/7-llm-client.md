...
| 프로바이더 | Rerank | 인증 | 비고 |
|-----------|--------|------|------|
| Cohere | ✅ | API Key | `rerank-3.5` 등 |
| Jina | ✅ | API Key | `jina-reranker-v2-base-multilingual` |
| Voyage | ✅ | API Key | `rerank-2.5` |
| TEI (자가호스팅) | ✅ | 없음 (선택) | HuggingFace Text-Embeddings-Inference 가 서빙하는 `bge-reranker-v2-m3` / `dragonkue/bge-reranker-v2-m3-ko` |
| Local | ✅ | 없음 (선택) | OpenAI-compatible `/rerank` (vLLM 등) |

> RerankConfig 엔티티: [Spec 데이터 모델 §2.16.1](../1-data-model.md#2161-rerankconfig-planned). 검색 후처리 적용: [Spec RAG 검색 §3.3](./9-rag-search.md#33-검색-후처리--리랭킹-선택적).
...
class RerankClientFactory {
  create(options: { provider: string; apiKey?: string; defaultModel: string; baseUrl?: string }): RerankClient {
    switch (options.provider) {
      ...
      case 'jina':   return new JinaRerankClient(options.apiKey!, options.defaultModel);
      case 'voyage': return new VoyageRerankClient(options.apiKey!, options.defaultModel);
      case 'tei':    // baseUrl 필수
                     return new TeiRerankClient(options.baseUrl!, options.defaultModel, options.apiKey);
      case 'local':  // baseUrl 필수, OpenAI-compatible /rerank
                     return new LocalRerankClient(options.baseUrl!, options.defaultModel, options.apiKey);
...
| provider | rerank() 엔드포인트 |
|----------|--------------------|
| Cohere | `POST /v1/rerank` |
| Jina | `POST /rerank` |
| Voyage | `POST /v1/rerank` |
| TEI | `POST {base_url}/rerank` (HF Text-Embeddings-Inference) |
| Local | `POST {base_url}/rerank` (OpenAI-compatible) 또는 vLLM `/score` |
