...
위 표는 chat/embedding 프로바이더(ModelConfig (kind=chat))다. 리랭킹은 전용 `/rerank` 엔드포인트라 **별도 프로바이더 집합·별도 설정(ModelConfig (kind=rerank))·별도 팩토리**로 분리한다 (§3.6, §4.1).

### 2.1 리랭크 프로바이더 (1차 tei/cohere 구현 완료)

| 프로바이더 | 지원 | 인증 | 비고 |
|-----------|------|------|------|
| TEI (자가호스팅) | **1차 (구현됨)** | 없음 (선택) | HuggingFace Text-Embeddings-Inference 가 서빙하는 `bge-reranker-v2-m3` / `dragonkue/bge-reranker-v2-m3-ko`. self-host 우선 경로 |
| Cohere | **1차 (구현됨)** | API Key | `rerank-3.5` 등. 외부 API 경로 |
| Jina | Dropped (2026-06-05 결정) | API Key | `jina-reranker-v2-base-multilingual` |
| Voyage | Dropped (2026-06-05 결정) | API Key | `rerank-2.5` |
| Local | Dropped (2026-06-05 결정) | 없음 (선택) | OpenAI-compatible `/rerank` (vLLM 등) |
| builtin | Dropped (2026-06-05 결정) | — | Transformers.js(onnxruntime-node) 인프로세스 추론 — 추론 컨테이너 없이 제로 인프라 self-host. worker thread·모델 캐시 필요 |

> **지원 범위**: `tei`(자가호스팅) + `cohere`(API) 2종으로 **종결** — provider 확장(jina/voyage/local/builtin)은 2026-06-05 사용자 결정으로 drop 했다 ([근거: ## Rationale](#rationale)). 재개 시에도 동일 `/rerank` HTTP 래퍼(builtin 은 인프로세스)라 확장 비용은 낮다.

> ModelConfig (kind=rerank) 엔티티: [Spec 데이터 모델 §2.16](../1-data-model.md#216-modelconfig). 검색 후처리 적용: [Spec RAG 검색 §3.3](./9-rag-search.md#33-검색-후처리--리랭킹-선택적).
...
> **용어 구분**: ModelInfo = provider `listModels` 응답 항목(DTO). ModelConfig = 저장된 provider 설정 엔티티(DB, [데이터 모델 §2.16](../1-data-model.md#216-modelconfig)). 접두어 `Model` 공유하나 별개.
...
`ModelConfig (kind=chat)` 에서 평탄화한 옵션(`LLMClientCreateOptions`)으로부터 적절한 `LLMClient` 구현체를 생성한다. 팩토리는 엔티티 전체가 아니라 `{ provider, apiKey, defaultModel, baseUrl? }` 만 받아 클라이언트별 생성자 인자로 매핑한다.

설정은 단일 `ModelConfig` 테이블(kind 판별)이나, 팩토리는 `kind` 로 분기한다 — `kind='chat'|'embedding'` → `LLMClientFactory`, `kind='rerank'` → `RerankClientFactory`. shape 차이(rerank 의 `(query, docs[])` 입력·score 배열 출력·스트리밍 부재)는 팩토리 레이어가 흡수한다.
...
> 클래스명은 코드상 `GoogleClient` (스펙 서술의 `GoogleAIClient` 와 매핑). 팩토리 자체는 `provider` 만 검증하며, azure/local 의 `baseUrl` 누락은 `Error` 로 throw — 호출 측(§5.5 preview)이 이를 `MODEL_CONFIG_INVALID` 로 래핑한다.

### 4.1 RerankClientFactory

리랭커는 **별도 팩토리**로 분리해 `LLMClientFactory` 의 chat/embedding provider switch 를 오염시키지 않는다 (별개 provider 집합·별개 인터페이스). `ModelConfig (kind=rerank)` 에서 `{ provider, apiKey?, defaultModel, baseUrl? }` 를 받아 `RerankClient` 구현체를 생성한다.

```typescript
class RerankClientFactory {
  create(options: { provider: string; apiKey?: string; defaultModel: string; baseUrl?: string }): RerankClient {
    switch (options.provider) {
      // 1차 구현. tei 의 baseUrl / cohere 의 apiKey 누락은 팩토리가 명시적 Error throw
      case 'tei':    // baseUrl 필수 (자가호스팅 HF TEI)
                     return new TeiRerankClient(options.baseUrl!, options.defaultModel, options.apiKey);
      case 'cohere': // baseUrl 선택 — 생략 시 기본 `https://api.cohere.com` (프록시/테스트 override 용)
                     return new CohereRerankClient(options.apiKey!, options.defaultModel, options.baseUrl);
      // Dropped (2026-06-05 결정): jina / voyage / local(OpenAI-compatible /rerank) / builtin(Transformers.js 인프로세스) — tei+cohere 로 종결
...
> 미지원 provider 구성은 구성 시점 검증 실패이므로 기존 `MODEL_CONFIG_INVALID` 계열로 래핑한다 — 런타임 전용 코드를 신설하지 않는다. SSRF 가드는 자가호스팅 `tei` 만 사설망을 허용하며(rerank 에는 `local` provider 가 없다 — §2.1 Dropped), §5.5 의 `tei`/`local` 예외 규칙을 재사용한다.
...
|----------|--------------------|------|
| TEI | `POST {base_url}/rerank` (HF Text-Embeddings-Inference). model 은 서버에 고정 로드돼 body 미전송(`defaultModel` 은 식별/로깅 용도), `opts.topK` 미지원 — 호출 측이 정렬 후 slice | 1차 (구현됨) |
| Cohere | `POST {base_url}/v2/rerank` (`{ model, query, documents, top_n }` → `{ results: [{ index, relevance_score }] }`). `base_url` 생략 시 `https://api.cohere.com` | 1차 (구현됨) |
| Jina | `POST /rerank` | Dropped (2026-06-05 결정) |
| Voyage | `POST /v1/rerank` | Dropped (2026-06-05 결정) |
| Local | `POST {base_url}/rerank` (OpenAI-compatible) 또는 vLLM `/score` | Dropped (2026-06-05 결정) |
| builtin | (인프로세스 Transformers.js — onnxruntime-node, HTTP 없음) | Dropped (2026-06-05 결정) |
...
- **동작**:
  ...
  - 반환값은 저장된 설정용 `GET /api/model-configs/:id/models`와 동일한 `ModelInfo[]`. (보안 계약 동일 유지)
  - 클라이언트 인스턴스는 per-config 캐시에 들어가지 않으며 요청 범위에서만 사용된다.
  - 30초 timeout 적용. Rate limit 은 provider probe 3종(`preview-models` · `:id/test` · `:id/models`) 공통 컨트롤러 상수 `PROVIDER_PROBE_THROTTLE` 로 적용된다 — 구체 수치·정책 SoT 는 [API 규약 §7 Rate Limiting](./2-api-convention.md#7-rate-limiting).
  - **결과 수 상한**: provider 응답 모델 수는 방어적 상한 **500**(`MAX_MODEL_LIST_SIZE`, `llm/list-models-cap.ts`)으로 절단된다 — 저장 설정 `GET :id/models` 와 동일 적용. 정상 provider(수십 개)에는 닿지 않으며, 사용자 지정 `baseUrl`/local 등 병적(대량) 응답의 메모리·페이로드 보호용이다. 초과 시 앞 500개만 보존(provider 순서 유지, 재정렬 없음) + 경고 로그하며, 응답 계약(`ModelInfo[]`)은 변하지 않는다.
- **권한**: `editor` 이상.
...
- **SSRF 가드**: `baseUrl` 이 loopback(`127.0.0.0/8`, `::1`), RFC1918(`10/8`·`172.16/12`·`192.168/16`), link-local(`169.254/16`, `fe80::/10`), IPv6 ULA(`fc00::/7`), IPv4-mapped IPv6, `0.0.0.0/8` 에 해당하면 `MODEL_CONFIG_INVALID` 로 차단한다. 도메인 hostname 은 `dns.lookup` 으로 1차 해석 후 같은 규칙을 재적용 (DNS rebinding 1차 방어). **`local` 프로바이더는 예외** — self-hosted Ollama/vLLM 이 localhost·사설망에 있는 것이 정상 사용 사례. **한계**: DNS rebinding 2차(TTL 경과 후 재해석) 는 connect 시점 개입이 필요해 현재는 차단 대상 아님. 실차단이 필요하면 egress 방화벽·클라우드 네트워크 정책으로 보완한다.
...
| 프로바이더 설정 오류 | 400 | `MODEL_CONFIG_INVALID` — 팩토리 생성 실패 (예: azure/local baseUrl 누락, 알 수 없는 provider) 또는 preview SSRF 차단 |
...
- `LlmService.createClient` 는 ConfigService 의 `llm.stubMode`(env `LLM_STUB_MODE` 유래 — refactor M-6 으로 서비스 계층의 `process.env` 직접 접근을 `registerAs('llm')` namespace 로 중앙화) 가 ON 일 때 **캐시/decrypt 경로보다 앞**에서 결정적 `StubLlmClient` 를 반환한다 — stub 이 항상 우선하도록 하여 실 클라이언트가 먼저 캐시된 상태에서의 오염을 막는다. 이 경로에서 llm-config `apiKey` 는 사용되지 않는다.
- **프로덕션 차단**: 부팅 가드가 `NODE_ENV=production` + `LLM_STUB_MODE=true` 조합을 fail-closed 로 throw 한다. 가드는 `main.ts` 가 호출하는 `assertProductionConfig`(`common/config/production-guards.ts` — JWT_SECRET·ENCRYPTION_KEY·MCP_ALLOW_INSECURE_URL·OAUTH_STUB·LLM_STUB 를 응집한 단일 production fail-closed 블록, refactor 04 C-1·M-4·M-7)에 있다. 프로덕션에서는 절대 활성화되지 않는다.
...
type ChatStreamEvent =
  ...
  | { type: 'tool_call_end'; id: string; name: string; arguments: string;
      signature?: string }   // 완성된 tool call 전체를 운반. signature 는 §3.4 ToolCall.signature 와 동일 (후속 history 턴에 echo 필요)
  | { type: 'done'; usage: TokenUsage; model: string;
      finishReason: ChatResponse['finishReason'] | 'aborted' }   // 'aborted' = 클라이언트 abort (§8.4)
...
| `tool_call_end` | tool_call 한 건 완성 — 완성된 `name`/`arguments`(+`signature?`) 를 운반. 호출자가 shadow 실행/검증을 시작할 시점 |
...
- **왜 SSRF 가드·secret-store 는 재사용하는가**: 자가호스팅 `tei` 리랭커(rerank 의 유일한 사설망 provider — `local` 은 §2.1 Dropped)는 사설망 endpoint 를 받아 LLM 과 동일한 SSRF 공격 면이 있다. §5.5 의 `tei`/`local` 사설망 예외 규칙을 그대로 적용해 인프라를 중복 구현하지 않는다.
...
- **왜 testConnection·listModels 가 kind-agnostic 조회를 쓰나**: 통합 Models 관리 UI 는 chat/embedding/rerank 어떤 설정이든 테스트·모델 로드한다. 구 `LlmConfigService.findEntity`(chat 고정)를 경유하면 embedding/rerank 설정이 `MODEL_CONFIG_NOT_FOUND` 로 거부돼 연결 테스트·모델 로드가 깨졌다(회귀). 따라서 두 메서드는 `ModelConfigService.findEntity`(kind 무관)를 직접 사용한다. **forwardRef 순환 해소 (refactor-02 C-2 cluster 4)**: 종전엔 `model-config.controller.ts` 의 부속 엔드포인트(`preview-models`·`:id/test`·`:id/models`)가 `LlmService`/`LlmPreviewService` 를 주입하고 `update`/`remove` 가 `LlmService.clearClientCache` 를 직접 호출해 `LlmModule ↔ ModelConfigModule` 상호 forwardRef 순환이 있었다. 이를 ⓐ 부속 엔드포인트를 `LlmModelConfigController`(llm 모듈, `@Controller('model-configs')` 라우트 무변)로 재배치하고 ⓑ 캐시 무효화를 `ModelConfigService.onConfigInvalidated` 옵저버(LlmService 가 `onModuleInit` 에서 구독)로 역전해 단방향 `LlmModule → ModelConfigModule` 로 정리했다(양측 forwardRef 제거). `kind-agnostic 조회`(testConnection/listModels → `ModelConfigService.findEntity`)는 그대로 유지된다.
