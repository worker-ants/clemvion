...
pending_plans:
  - plan/in-progress/rag-quality-improvement.md
---
...
> **상태**: `cross_encoder` 와 `cross_encoder_llm` 두 모드 모두 구현됨 (provider 1차 `tei`/`cohere`; 마이그레이션 V081 ModelConfig (kind=rerank) 테이블 · V082 KB rerank_* 컬럼). 본 리랭킹은 `rag_mode`(vector/graph)와 **직교** — graph 모드의 centrality-weighted score blending([Graph RAG §4](./10-graph-rag.md))은 graph 내부 1차 정렬이고, 본 절의 cross-encoder reranking 은 vector/graph 어느 회수 결과든 적용되는 2차 후처리다.
...
2) cross-encoder rerank: (query, chunk.content) 쌍을 ModelConfig (kind=rerank) endpoint 로 점수화
...
- **v1 결정 (2026-06-06 갱신 — [`spec-draft-rag-reranking.md §Rationale②`](../../plan/complete/spec-draft-rag-reranking.md) "항상 grading(v1)" 결정 번복)**: `cross_encoder_llm` 은 cross-encoder 상위 점수가 평탄/모호할 때만 listwise grading 으로 **conditional escalate** 한다. escalate 진입 **정량 임계는 합리적 default** 로 시작(§Rationale)하고, P0 골든셋 기반 A/B 확정은 후속([`rag-quality-improvement.md §7.C`](../../plan/in-progress/rag-quality-improvement.md)). escalate 미발생은 cross-encoder 결과를 그대로 사용한다(기존 동작의 부분집합 → 회귀 안전). "정책 판단 KB" 는 별도 컬럼/휴리스틱 없이 `rerank_mode = cross_encoder_llm` 선택으로 표현한다.
...
- 검색 쿼리 임베딩은 **Knowledge Base의 `embedding_model_config_id` (FK → ModelConfig kind=embedding) 와 동일한 endpoint** 사용
...
| ModelConfig (kind=rerank) 미구성/미지원 provider | 해당 KB `off` 강등 (cosine 경로). `RERANK_CONFIG_INVALID`. 경고 로그 |
...
> **에러 레이어 구분**: `RERANK_CONFIG_INVALID` 은 검색 실행(rerank 호출) 레이어 전용 에러이고, `MODEL_CONFIG_INVALID` / `MODEL_CONFIG_NOT_FOUND` 은 설정 CRUD(`/api/model-configs`) 레이어 전용이다.
...
- **왜 KB 단위인가**: 검색 파이프라인 소유권을 KB 에 둔 기존 결정(`rag_mode`·`embedding_model_config_id`(FK → ModelConfig kind=embedding)·graph 파라미터 KB 단위)과 일관. KB 마다 도메인·문서 성격이 달라 리랭크 전략도 KB 단위가 자연스럽다. 노드 단위로 빼면 같은 KB 를 여러 노드가 쓸 때 설정이 분산된다.
...
- **왜 D2 conditional escalate 를 지금 도입하나**: 기존 v1 결정 "`cross_encoder_llm` 은 항상 grading"([`spec-draft-rag-reranking.md §Rationale`](../../plan/complete/spec-draft-rag-reranking.md) · 본 문서 §3.3.2 구판 v1 결정 · [`rag-quality-improvement.md §6`](../../plan/in-progress/rag-quality-improvement.md) 2026-06-04 확정)은 **LLM 콜 비용 보호용 단순화**였다. conditional escalate 진입 구조는 데이터와 무관하게 안전하다 — escalate 미발생 시 cross-encoder 결과를 그대로 쓰므로 v1 동작의 부분집합이다. 따라서 escalate **메커니즘은 지금 도입**하되, escalate 진입 **정량 임계는 합리적 default**(예: 상위 N 점수의 평탄도 — 최고점과 차이/표준편차 기반)로 시작하고 P0 골든셋 기반 A/B 확정은 후속([`rag-quality-improvement.md §7.C`](../../plan/in-progress/rag-quality-improvement.md))으로 분리한다. 회귀 위험이 낮은 이유가 이 부분집합 성질이다. *(2026-07-16: 종전 이 두 곳이 지목하던 `rag-rerank-followup.md` 는 `plan/complete/` 로 종결되며 본 항목을 `rag-quality-improvement.md §7.B/C` 로 이관했다 — 본문 참조를 실제 책임 plan 으로 정정.)*
...
- **설정 테이블 통합 이후 RerankClient 분리 유지 근거 (LLMConfig/RerankConfig → ModelConfig 통합)**: 설정 테이블은 `ModelConfig`(kind=rerank) 로 통합. 단, 검색 후처리 호출 계약은 입력/출력 shape 차이로 별도 `RerankClient`/`RerankClientFactory` 를 유지한다 ([LLM Client §4](./7-llm-client.md)).
...
- **왜 `rag-dynamic-cut` 종결 후에도 `status: partial` 을 유지했나 (2026-07-16)**: 이 spec 의 `pending_plans` 는 `rag-dynamic-cut.md` 단독이었고, 그 plan 이 `plan/complete/` 로 종결되면서 [spec-impl-evidence §3](../conventions/spec-impl-evidence.md) 가드 (c)가 `implemented` 승격을 요구하는 상태가 됐다. **그러나 승격은 거짓이다** — 본 문서에는 여전히 미구현 표면이 실재한다: ① 멀티-KB 리랭크(§3.3.2 "v1 범위 — 단일 KB 한정"), ② 모델 변경 시 자동 재임베딩 트리거(위 "왜 이번 범위를 경고 노출로 한정했나"), ③ D2 escalate 정량 임계 A/B 확정(§3.3.2 v1 결정), ④ `ef_search` clamp 정밀 튜닝(위 pgvector HNSW 항목). `rag-dynamic-cut` 은 D1 동적 컷 + D2 escalate **메커니즘**만 책임졌을 뿐 이 4건의 책임 plan 이 아니었다. 따라서 승격 대신 **`pending_plans` 를 실제 책임 plan([`rag-quality-improvement.md`](../../plan/in-progress/rag-quality-improvement.md) — RAG 품질 SoT 로드맵)으로 재배선**하고 `partial` 을 유지한다. 그 4건은 해당 plan §7.C-2 에 체크박스로 명문화됐으며, 전부 닫히는 시점에 `implemented` 승격이 강제된다. *(원칙: status 는 가드를 통과시키기 위한 형식이 아니라 구현 현실의 진술이다 — 가드가 승격을 요구한다고 해서 미구현 표면을 덮으면 이 spec 이 막으려던 "spec 약속 vs 구현 부재" 갭을 spec 자신이 만든다.)*
- **폐기한 대안**:
