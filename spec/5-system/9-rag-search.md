...
- **왜 RerankConfig 를 LLMConfig 와 분리했나**: 설정 테이블은 `ModelConfig`(kind=rerank) 로 통합. 단, 검색 후처리 호출 계약은 입력/출력 shape 차이로 별도 `RerankClient`/`RerankClientFactory` 를 유지한다 ([LLM Client §4](./7-llm-client.md)).
