...
| NF-OB-06 | 시스템 상태 가시화 — 큐 적체/실패/포화도를 집계 UI 로 노출 (개별 job 미노출). 실패는 최근 윈도우 기준 주 지표 + 누적 보관 부 지표로 병기 | 권장 | ✅ (구현 완료 — `GET /api/system-status/overview` + `/system-status` 페이지. 실패 윈도우 병기는 진행 중 [plan](../../plan/in-progress/system-status-recent-failed.md). 상세 [16-system-status-api](./16-system-status-api.md), [2-navigation/15-system-status](../2-navigation/15-system-status.md)) |
