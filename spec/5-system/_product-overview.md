...
| NF-SC-02 | 역할 기반 접근 제어 (RBAC) — Owner, Admin, Editor, Viewer | 필수 | ✅ (Workflows·Triggers·Schedules·Integrations·Model Config·KB·Auth Configs·Folders 가드 적용 · 워크스페이스 멤버 관리는 Admin/Owner 분리 · Owner 이양 지원) |
...
| NF-OB-07 | 도메인/비즈니스 커스텀 메트릭 (OTel) — 워크플로 실행·큐·LLM·노드 지연·Redis fail-open 강등·감사 적재 실패를 OTel MeterProvider(NF-OB-02) 위에 노출 | 권장 | ✅ (`OTEL_ENABLED=true` 시. `BusinessMetricsService` 가 `metrics.getMeter('clemvion.business')` 로 계측 — 아래 카탈로그) |
...
**라벨을 닫는 방법은 둘이다.** 소스에 코드 리터럴 유니온이 있으면 그 유니온으로 **타입에서** 닫는다(`clemvion.redis.fail_open` 의 `component`/`reason` — [data-flow §Rationale](../data-flow/9-observability.md)). 소스 시그니처가 이미 `string` 이라 **컴파일러가 닫힘을 증명하지 못하는** 라벨(`error_code`, `resource_type`)은 값 길이를 64자로 **클램핑**해 방어한다. 증명되지 않은 닫힘을 타입으로 주장하는 대신, 실제로 지켜지는 방어를 적는다.
...
| AGM-02 | `(workspace_id, scope_key, created_at)` scope 인덱스(evict 정렬 커버) + pgvector partial 유사도 인덱스 ([§1](./17-agent-memory.md#1-데이터-모델)) | 필수 | ✅ |
...
| AGM-06 | Forgetting — scope 당 최신 `N=1000` FIFO/LRU evict ([§4](./17-agent-memory.md#4-회수-top-k-동기--forgetting)) | 필수 | ✅ |
| AGM-07 | 워크스페이스 격리 — 모든 회수·추출·evict 가 `workspace_id` 필터 강제 (cross-workspace 누수 차단) ([§5](./17-agent-memory.md#5-격리-의무)) | 필수 | ✅ |
| AGM-08 | 증분 추출 — 멀티턴 `lastExtractionTurnSeq` watermark, `seq` 초과 turn 만 snapshot ([§3](./17-agent-memory.md#3-추출-파이프라인-턴-경계-비동기)) | 필수 | ✅ |
| AGM-09 | 의미기반 dedup/갱신 — `MEMORY_DEDUP_SIMILARITY=0.85`, 유사 기존 fact UPDATE (Mem0 식 최신화) ([§4](./17-agent-memory.md#4-회수-top-k-동기--forgetting)) | 필수 | ✅ |
| AGM-10 | TTL 만료 — `expires_at`/`memoryTtlDays`, recall 필터·evict 삭제 ([§4](./17-agent-memory.md#4-회수-top-k-동기--forgetting)) | 필수 | ✅ |
| AGM-11 | 추출 분류 — `{content, kind}` (kind ∈ fact/preference/entity), fallback `fact` ([§3](./17-agent-memory.md#3-추출-파이프라인-턴-경계-비동기)) | 필수 | ✅ |
| AGM-12 | scope별 메모리 조회 API — `GET /agent-memories/scopes`·`GET /agent-memories`, `workspace_id` 격리, embedding 제외, kind 필터 ([§6](./17-agent-memory.md#6-메모리-관리-api-조회삭제-admin-surface)) | 필수 | ✅ |
| AGM-13 | 메모리 삭제 API — `DELETE /agent-memories/:id` 단건·`DELETE /agent-memories?scopeKey=` scope 전체, editor+, hard delete ([§6](./17-agent-memory.md#6-메모리-관리-api-조회삭제-admin-surface)) | 필수 | ✅ |
