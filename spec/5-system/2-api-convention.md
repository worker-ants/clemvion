...
> **예외 — 서버가 끊은 경우:** 위 두 줄은 **전송 계층이 끊긴 경우**를 말한다. 서버가 스스로 `disconnect()` 한 경우(토큰 만료 종료)는 **자동 재연결 대상이 아니고** 복구 방식도 다르다 — 클라이언트가 명시적으로 재연결해야 하며 놓친 이벤트는 스냅샷으로 복구한다. 상세와 근거는 [WebSocket 프로토콜 §1.2·§6.1·§6.2·§9.2](./6-websocket-protocol.md) 가 소유한다.
