...
- `code` 의 상태코드별 기본값: 400=`VALIDATION_ERROR`, 401=`AUTH_REQUIRED`, 403=`FORBIDDEN`, 404=`RESOURCE_NOT_FOUND`, 409=`RESOURCE_CONFLICT`, 413=`PAYLOAD_TOO_LARGE`, 422=`INVALID_STATE`, 429=`RATE_LIMITED`, 5xx=`INTERNAL_ERROR`.
  `RolesGuard` 의 멤버십 · 역할 거부는 기본값이 아니라 전용 코드를 갖는다(`NOT_A_MEMBER` · `EDITOR_REQUIRED` · `ADMIN_REQUIRED` · `OWNER_REQUIRED` — [3-error-handling §1.2](./3-error-handling.md)). 2026-09-25 이전에는 코드를 지정하지 않아 이 기본값 `FORBIDDEN` 이었다.
...
| 502 | Bad Gateway | **외부 제3자 API**(chat provider 등) 호출이 실패 — 재시도 가능. 코드: `CHAT_CHANNEL_SETUP_FAILED`(chat channel `setupChannel` 실패) — [Chat Channel §5.4](./15-chat-channel.md#54-bot-token-rotation-api-응답-계약). **아래 `503` 과 구분한다: 502 는 외부 제3자, 503 은 우리 인프라다.** (노드 출력 payload 의 `statusCode` 와는 다른 축이다 — 그쪽은 HTTP 노드가 받은 응답 상태이고, 이 표는 **우리 API 가 클라이언트에 주는 상태**다.) |
...
| 실행 제어 | Client → Server | execution.start/stop _(비채택 won't-do — REST 대체)_ · execution.continue _(계획·미구현 — 브레이크포인트 로드맵)_ ([6-websocket-protocol §4.2](./6-websocket-protocol.md#42-실행-제어-명령-client--server)) |
...
§10.4 는 재연결을 두 줄로 요약하는데, 그 둘 다 **서버가 스스로 끊은 경우**에는 틀리다 — Socket.IO 는 서버발신 `disconnect()` 에 자동 재연결을 발화하지 않고, 복구도 마지막 이벤트 ID 재전송이 아니라 스냅샷이다.

- **한때 이 절을 고치지 않기로 했다** (`#1265`). 근거는 *"요약에 예외를 복제하면 두 곳이 갈릴 자리를 새로 만든다"* 였고, 그 논리 자체는 지금도 맞다.
- **바뀐 것은 전제다.** `#1266` 이 토큰 만료 종료를 구현하면서 그 예외가 **access token 수명(15분)마다 상시 발동**하게 됐다. 드물게 나는 예외를 요약에서 생략하는 것과, **상시 경로를 요약이 반대로 적고 있는 것**은 다르다.
- **그래서 복제가 아니라 위임이다.** 예외의 **존재**만 한 줄로 알리고 내용·근거는 `6-websocket-protocol.md` 가 소유한다. 요약이 상세를 복제하기 시작하면 둘이 갈리는 것은 시간 문제이고, 이 문서의 §10 은 애초에 "상세는 WS 문서 참조" 구조로 서 있다.
- **폐기 대안**: 재연결 규칙 전체를 이 절에 펼치는 안 → §10 의 위임 구조를 깨고, WS 문서가 이미 소유한 §6.1·§6.2·§9.2 와 이중 관리가 된다.
