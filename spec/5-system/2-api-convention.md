...
> **표의 범위**: 위 표는 글로벌 throttler tier(상위 4행)와, 그 위에 라우트별 `@Throttle` 로 덮어쓴 **주요 endpoint-specific 오버라이드**(하위 3행)를 cross-cutting SoT 로 정리한 것이다. 개별 엔드포인트의 권위 기술(권한·부수효과 등)은 각 도메인 spec 에 있으며, throttle **수치**의 단일 진실은 본 표다. 인증 요청은 전역 `UserThrottlerGuard` 가 `user:<sub>` 키로 **사용자당** 집계하고(미인증만 IP 폴백), 따라서 위 인증 보호 라우트의 제한은 사용자 기준이다.
