...
| `document:embedding_error` | `{ documentId, error: string }` | **(의미 변경, 2026-05-11)** in-flight 일시 오류 — `document:embedding_retry` 또는 `embedding_failed` 가 곧 따라온다. **영구 실패 신호로 사용하지 말 것** (이전 동작은 `embedding_failed` 로 이관됨) |
...
본 이벤트들은 [§9.2 상태 전이](#92-상태-전이) 와 직접 대응되며, `embedding_error` 의 의미 변경(2026-05-11) 이후 일시 오류만 의미한다.
...
본 파이프라인의 주요 결정 사항. 폐기된 대안과 1회성 분석 자료는 `plan/complete/archive/from-memory/` 를 참조.

### 결정: 다중 차원 임베딩 + KB 단위 모델 선택 (2026-05-02)
...
- EmbeddingService 폴링 동시성(MAX_CONCURRENT) → 세마포어 패턴 교체 — **→ V024 로 완료** (Worker concurrency=3 이 폴링을 대체)
- reEmbedAll fire-and-forget → BullMQ 같은 지속형 큐 + `reEmbedStatus` 상태 컬럼 — **→ V024 로 완료**
...
`kb:graph_stats_updated` 이벤트는 `kb-stats.helper.ts:42-46` 가 `emitExecutionEvent` 로 호출해 채널이 `execution:kb:…` 로 prefix 되어 frontend 의 `kb:` 구독에 도달하지 못하는 **dead path** 였다. type union 에도 없음(`as never` 강제 캐스트). spec 에서 일괄 제거. 코드 측 결함 처리는 `plan/in-progress/kb-graph-stats-dead-path.md` 로 dev 위임. 재도입 필요 시 본 PR 의 spec 변경을 reverse.
