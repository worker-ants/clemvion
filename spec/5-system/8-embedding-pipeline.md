...
### 5.5 임베딩 설정 해석 폴백 체인 (resolveEmbedding, PR2)

KB 임베딩(문서 적재·검색 쿼리 모두)에 사용할 `(ModelConfig, model)` 은
`ModelConfigService.resolveEmbedding` 이 아래 3단계 폴백 체인으로 해석한다. 임베딩 1급화를
기존 KB 무중단으로 전환하기 위한 점진적·하위호환 설계다 (`model-config.service.ts`).
...
2. **워크스페이스 default** — `embedding_model_config_id` 미지정
   → 워크스페이스 default `kind=embedding` ModelConfig + 그 `defaultModel`.
3. **legacy 폴백** — 1·2 모두 없음
   → `embedding_llm_config_id` 가 있으면 해당 ModelConfig, 없으면 ws default `chat` config.
...
> rag-search 의 벡터 그룹 키는 1급 경로면 `embedding_model_config_id::dim`, legacy 면
> `embedding_model::dim::config` 로 구성해 임베딩이 호환되는 KB 끼리만 묶는다.
