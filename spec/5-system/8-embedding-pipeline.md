...
일부 임베딩 모델은 **query 와 document(passage)를 다르게 인코딩**해야 검색 품질이 나온다(asymmetric retrieval). `LlmService.embed(texts, model, opts, inputType)` 의 `inputType`(`'query' | 'document'`, 생략 시 `'document'`)으로 경로를 구분한다. 매핑 SoT 는 `codebase/backend/src/modules/llm/embedding-input-type.ts`(순수함수)이며, 시그니처 정의는 [`7-llm-client.md §3.3`](./7-llm-client.md).
