...
| provider / 모델 | 적용 방식 | query | document |
|-----------------|-----------|-------|----------|
...
| OpenAI text-embedding-3 / ada, bge-m3, 미매칭 모델 | no-op (대칭) | — | — |

- **호출 경로 분류**: 적재(문서 청크) = `document`, 검색 query = `query`. `*-instruct` e5 변형은 입력 포맷이 달라 보수적으로 제외(no-op).
- **정합성**: prefix/taskType 도입 **이전에 색인된 기존 KB** 는 document 가 무접두사로 임베딩돼 있다. query 에만 접두사가 붙으면 비대칭이 깨지므로, e5/Gemini 계열을 쓰는 KB 는 본 변경 후 **재임베딩(§7.3)이 필요**하다. 신규 임베딩부터는 양쪽이 일관 적용돼 silent 품질저하가 차단된다.

---

### 5.5 임베딩 설정 해석 폴백 체인 (resolveEmbedding)

KB 임베딩(문서 적재·검색 쿼리 모두)에 사용할 `(ModelConfig, model)` 은
`ModelConfigService.resolveEmbedding` 이 아래 2단계 폴백 체인으로 해석한다 (PR4b/V093-V094 이후).
임베딩은 chat piggyback 이 아닌 자체 1급 `kind=embedding` ModelConfig 가 소유한다 (`model-config.service.ts`).

1. **1급 경로** — `knowledge_base.embedding_model_config_id != NULL`
   → 해당 `ModelConfig(kind=embedding)` 직접 조회 + **`config.defaultModel`** 사용 (모델·provider·차원을 config 가 소유).
2. **워크스페이스 default** — `embedding_model_config_id` 미지정
   → 워크스페이스 default `kind=embedding` ModelConfig + 그 `defaultModel`. 둘 다 없으면 `MODEL_CONFIG_NOT_FOUND`(404).

> 문서 적재(`embedding.service`)와 검색 쿼리(`rag-search.service`)는 **동일** resolveEmbedding 을
> 거쳐 (config, model) 이 일치하므로 query/document 임베딩 차원·endpoint 가 어긋나지 않는다.
> rag-search 의 벡터 그룹 키는 `embedding_model_config_id::dim` 으로 구성해 임베딩이 호환되는 KB 끼리만 묶는다.
> 구 legacy step-3(chat piggyback 폴백, `embedding_llm_config_id`/`embedding_model`)은 **V093 repoint + V094 DROP(PR4b)에서 제거됐다** — 기존 KB 는 V093 에서 1급 `kind=embedding` config 로 repoint 됐다 ([데이터 모델 §2.11](../1-data-model.md#211-knowledgebase)).
...
`embedding_dimension` 이 NULL 인 KB 는 `RagSearchService` 에서 검색 대상에서 제외된다 — 저장 청크가 현재 모델과 차원/공간이 일치한다는 보장이 없어 stale 비교를 막기 위함이다. 두 경우가 이 상태가 된다: (a) `reembed_status='in_progress'` (재임베딩 진행 중 — 완료 후 다시 포함), (b) `reembed_status='idle'` 이며 임베딩 모델 변경(`update()`)으로 dimension 이 NULL 로 초기화됐으나 재임베딩이 아직 실행되지 않음 (사용자가 [§7.3.2 KB 전체 재임베딩]을 실행할 때까지 지속). 제외는 **silent 가 아니라** 검색 호출부에 명시 신호로 전달된다 — 에이전트에는 `status:"not_searchable"` tool_result 봉투, 진단에는 `skipReason="kb_unsearchable"` ([RAG 검색 §2.2·§3.1·§6](./9-rag-search.md#6-에러-처리)), UI 목록 카드에는 검색불가 경고 ([Knowledge Base §2.2.1](../2-navigation/5-knowledge-base.md))로 노출한다.
...
| `document:embedding_error` | `{ documentId, error: string }` | `KbEventType` union 에 **선언돼 있으나 현재 emit 경로 없음** — 일시 오류는 `embedding_status='error'` 전환과 함께 `document:embedding_retry` 로 통지된다 (data-flow §2.5). union 멤버로 남겨 forward-compat 확보. **영구 실패 신호로 사용하지 말 것** (영구 실패는 `embedding_failed`) |
...
본 이벤트들은 [§9.2 상태 전이](#92-상태-전이) 와 직접 대응된다. 실제 emit 되는 임베딩 이벤트는 5종(started/progress/completed/retry/failed)이며, `embedding_error` 는 union 선언만 남은 미emit 멤버다(일시 오류는 `_retry` 로 신호).

### 8.2 그래프 추출 이벤트

`rag_mode = 'graph'` KB 의 문서에 대해 동일 채널(`kb:${documentId}`) 로 5개 이벤트가 추가 emit 된다 — `document:graph_started`, `_progress`, `_completed`, `_retry`, `_failed`. 임베딩과 달리 graph 에는 `_error` 이벤트가 없다 (emit 경로가 없어 #443 에서 union 제거). 의미·payload 는 [`spec/5-system/10-graph-rag.md §6`](./10-graph-rag.md) 의 정의를 따른다.
...
- **LlmService.listModels**: `{ type?: 'chat'|'embedding' }` 옵션을 서비스 레이어에서 필터링한다. 진입 컨트롤러 `LlmModelConfigController`(`GET /api/model-configs/:id/models`)는 `?type` 쿼리를 `ParseEnumPipe`(허용값 `chat`·`embedding`)로 검증해 **허용값 외 입력은 서비스 도달 전 `400 Bad Request` 로 거부**하고, 통과한 값만 서비스 레이어 필터로 넘긴다. 즉 컨트롤러 = 허용값 입력 검증(400) · 서비스 = 실제 타입 필터링의 2단 책임 분리이며, `@ApiQuery(enum)` · `@ApiBadRequestResponse` 로 OpenAPI 에도 노출한다.
- **`kind='embedding'` ModelConfig 1급화 (Unified Model Management)**: `kind='embedding'` ModelConfig 1급화로 `listModels type='embedding'` **서비스 레이어 필터**가 불필요해지고(소스가 이미 embedding kind; 단 컨트롤러 `ParseEnumPipe` 입력 검증은 유지), 차원 SoT 를 ModelConfig.dimension 으로 일원화한다. 임베딩 모델은 chat 용 LLMConfig piggyback 이 아니라 자체 1급 설정이 소유한다.
- **프론트엔드**: KB 생성/편집 폼의 임베딩 입력은 **단일 `NativeSelect`** 로, 워크스페이스의 `kind=embedding` ModelConfig 목록에서 config 하나를 직접 고른다(자유 입력 fallback 없음 — [지식 저장소 §Rationale R-1](../2-navigation/5-knowledge-base.md#r-1-임베딩-모델-선택을-select-only-로-한정) "단일 select — 모델 문자열 sub-select 없음"). 선택된 config 의 `defaultModel` 을 백엔드가 서버 사이드로 파생해 `KB.embeddingModel` 에 저장하므로(§5.5 1급 경로) 별도 모델 문자열 select 나 "모델 불러오기" 단계가 없다. 초기 구현에서는 `EmbeddingModelCombobox`(모델 문자열 조회용 컴포넌트로 `embedding-model-combobox.tsx` 에 실재)가 검토됐으나, Unified Model Management 임베딩 1급화 이후 KB 폼은 위 단일 NativeSelect 로 개선됐고 해당 컴포넌트는 KB 폼에서 직접 사용되지 않는다. KB 상세에 "지식 베이스 설정" 모달, "KB 전체 재임베딩" 버튼 + 확인 모달, embeddingDimension 메타 노출. payload 타입 `KbUpdatePayload` 명시, reEmbedAll 응답 envelope unwrap, chunkSize/chunkOverlap 사전 검증.
...
- **위치 인자 확장 vs 응답/옵션 객체**: `EmbedResponse` 객체화(Planned)와 묶지 않고 `inputType?` 위치 인자만 추가했다 — 변경 표면을 최소화하고 기존 호출부 하위호환(생략 시 `'document'`)을 유지하기 위함. 객체화는 `EmbedResponse` 도입 시 함께 검토.
- **보수적 화이트리스트**: 잘 알려진 비대칭 모델(e5 비-instruct, Gemini)만 변형하고, 미매칭/대칭 모델은 no-op. 잘못된 접두사를 붙이는 것이 안 붙이는 것보다 위험하므로 안전 기본값을 no-op 으로 둔다.
- **재임베딩 정합성**: 도입 이전 색인 데이터는 비대칭이 깨질 수 있어 e5/Gemini KB 는 재임베딩(§7.3)이 필요하다. 자동 트리거 대신 모델 변경 경고(`spec/2-navigation/5-knowledge-base.md`)와 동일한 수동 재임베딩 플로우로 안내한다(비용 통제).
...
### 결정: legacy 임베딩 폴백(step-3) 은퇴 (PR4b)

resolveEmbedding step-3 legacy(chat piggyback) 폴백은 V093 repoint + V094 DROP(PR4b)에서 제거됐다 — 기존 KB 는 V093 에서 1급 `kind=embedding` config 로 repoint 됐다. PR2 의 3-step 점진 전환은 무중단 마이그레이션을 위한 한시적 하위호환 장치였고, V093 가 `embedding_model_config_id IS NULL` KB 0건을 fail-loud 로 보증한 뒤 V094 가 legacy 컬럼(`embedding_llm_config_id`·`embedding_model`)을 비가역 DROP 해 체인을 2-step 으로 단순화했다. 저장된 임베딩 벡터는 불변이라 repoint 시 원래 provider·model·dimension 을 한 글자도 바꾸지 않고 1급 config 로 보존한다.
