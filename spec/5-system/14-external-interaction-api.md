...
| 상태 | 코드 | 조건 |
|------|------|------|
| `400 Bad Request` | `VALIDATION_ERROR` | submit_form 의 field 검증 실패. body 의 `error.details[]` (`{ field, message, code: "INVALID_FIELD" }` 배열 — [API 규약 §5.3](./2-api-convention.md#53-에러-응답)) 참조. execution 상태 유지(재제출 가능). field-level 검증(필수/type/minLength 등)은 publisher 측 `continueExecution` chokepoint 에서 노드 config 의 field 정의로 수행 (typed `FormValidationError` → EIA 400 매핑) — EIA·WS·UI 3 경로 공통. `type: 'file'` MIME/크기/개수 검증만 별도 **Planned** ([Form §6.2](../4-nodes/6-presentation/4-form.md#62-form-입력-검증-실패-재제출-가능-새-출력-생성-없음)) |
