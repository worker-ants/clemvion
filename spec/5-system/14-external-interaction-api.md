...
code:
  ...
  - codebase/backend/src/nodes/core/node-output-allowlist.ts
...
| EIA-RL-07 | **공개 위젯 idle-wait execution 회수 (backstop)** — **(구현됨 — `WebChatIdleReaperService`, BullMQ repeatable 분 단위)** `auth_config_id IS NULL`(공개 웹채팅 위젯) 트리거가 시작하고 `per_execution` 토큰으로만 접근되는 `waiting_for_input` execution 이, **발급된 모든 interaction 토큰이 영구 만료**(`execution_token.exp_at` 전부 `< now` — refresh 불가, [3-auth-session §R6](../7-channel-web-chat/3-auth-session.md))된 뒤 grace window(env `WEBCHAT_IDLE_REAP_GRACE_MS`, 기본 `3600000`ms=1h) 초과 시 `cancelled`(`cancelledBy='timeout'` + `error.code='WEBCHAT_IDLE_TIMEOUT'`)로 회수한다. **판정 = provably un-continuable**: 익명 위젯은 토큰 만료 후 refresh 가 불가(만료 JWT 는 `InteractionGuard` 가 401 선차단, §5.5)하므로 alive 세션은 만료 전 proactive refresh(EIA-AU-05)로 유지되고, abandoned 세션만 토큰이 영구 만료된다 → 입력 도착 경로가 물리 소멸. 메커니즘은 **EIA-RL-06 형 BullMQ repeatable sweep**(전역 1회, 별도 서비스/큐명) — [4-execution-engine §1.1](./4-execution-engine.md#11-execution-상태)이 이미 예약한 `waiting_for_input → cancelled` "타임아웃" 사유의 구현이며 §7.4 무기한 보존 불변식의 보호 대상(legitimate 대기)은 무변경. 인증 트리거·per_trigger·워크플로우 정의 `formConfig.timeout` 은 대상 아님(§R19) | 권장 |
...
| EIA-NF-03 | 5분 이벤트 버퍼: 재연결 시 `seq > Last-Event-Id` 인 이벤트를 손실 없이 재전송. 버퍼가 요청 범위를 못 채우면(만료·폐기) `execution.replay_unavailable` 을 emit 하고 클라이언트는 REST 재조회로 폴백 (구현됨 — §5.2) | 필수 |
...
> **전송 봉투**: 위 payload 는 전송 시 전역 `TransformInterceptor` 가 `{ "data": { ... } }` 로 래핑한다 ([Webhook §3.1](./12-webhook.md#31-webhook-수신-엔드포인트) / [API 규약 §5](./2-api-convention.md#5-응답-형식)). 즉 wire format 은 `{ "data": { "executionId", "status", "interaction", ... } }`. SSE 스트림 프레임(`data: <payload>`)은 인터셉터를 거치지 않아 봉투가 없다(§5.2) — REST 응답만 `{ data }` 로 감싸짐. 클라이언트는 REST 응답에서 `res.data` 를 언랩해 읽어야 한다.
>
...
| `cancel` | `reason?` | (전체 execution) | `execution.stop` 개념 (WS 명령은 §4.2 won't-do — REST cancel 로 처리, 외부에서는 `force` 옵션 미지원) |
...
**규약:**
- `id` 필드 = execution 내 monotonic `seq` (= WebSocket §2.2 의 `seq` 와 같은 값). 단 실행-scope seq 를 배정받지 않는 control frame(`execution.replay_unavailable`, `seq=0`)은 `id:` 라인을 **생략**한다 (아래 참조).
...
> > `shared/testing/trigger-workflow-ref.ts`**(`#1308`) 가 각각 같은 컬럼 목록의 부재를
> > 단언한다. 위 문단이 한 축만 열거하고 있어 *"직접 축엔 정적 스트립만 있다"* 로 읽힐
> > 수 있어 보강한다 — **개수가 아니라 나열이 낡는 자리다.**
>
...
- **secret 출처는 토큰 family 별로 다르다**:
  ...
  - `iext_*` (`per_execution`) — **단일 글로벌 secret** 으로 서명하는 단명 JWT. trigger 별 분리가 아니라 execution scope 로 한정되는데, payload `{ sub: executionId, aud: 'interaction', jti }` 가 단일 execution 에 묶이고 jti 가 Redis blacklist 로 revoke 되기 때문이다 (아래).
    ...
    - **dev fallback**: 두 env(`INTERACTION_JWT_SECRET`, `JWT_SECRET`) 모두 미설정이면 프로세스 시작 시 1회 생성하는 ephemeral random 키(`randomBytes(32)`)로 떨어진다 — 버전 이력에 예측 가능한 고정 secret 을 남기지 않고, 재시작마다 변경돼 dev 토큰이 무효화된다.
...
codebase/backend/src/modules/
  external-interaction/
    ...
    webchat-idle-reaper.service.ts        # BullMQ repeatable scheduler (분 단위) — 공개 위젯 idle-wait execution 회수 (EIA-RL-07, §R19)
    webchat-idle-reaper.types.ts          # 큐 이름 상수 + grace env resolver (WEBCHAT_IDLE_REAP_GRACE_MS)
...
| `replay.unavailable` _(native WS 미해당 — 버퍼 없이 `execution.snapshot`)_ | `execution.replay_unavailable` **(구현됨)** | — (재연결 응답 전용) |
...
**fail-open 의 원인은 두 축이다** — Redis **미가용**(죽었다)과 캐시 **손상**(살아 있는데 값이 오염됐다). 후자는 엔트리 형태 불일치·내부 `responseJson` 파싱 실패로 나타나며, 손상 엔트리는 버리고 신규 처리로 강등한다(+warn). 그 예외를 그대로 올리면 `500` 이 되어 fail-open 원칙과 정반대가 된다. `statusCode` 는 **타입과 값 범위를 모두** 검사한다 — `isHttpStatusCode()` 가 `Number.isInteger` 와 `100`~`599` 를 본다(`#1159`). 이 검사가 없으면 `-1`·`600` 같은 값이 통과해 `res.status(-1)` 이 전송 시점 `RangeError` 로 같은 500 을 만든다.
    ...
    **적용 표면 목록은 아래 "적용 범위는 총칭이 아니라 열거다" 항목이 정본**이다 — 여기에
    ...
    - **~~잔여 ②~~ 해소(2026-08-20) — `outputData`(2026-08-16)에 이어 `Execution.inputData`
      ...
      **축을 정확히 하면: round-trip 되는 것만 카브아웃한다.**
      ...
      - **닫는 조건은 충족됐다 (2026-08-20)**. 조건은 *"프런트가 마스킹 마커를 감지해 해당
        ...
        > **후자는 재제출 전용이 아니다** — 같은 엔드포인트가 에디터 JSON 에디터의 자유
        > 편집도 받고, 값이 히스토리에서 온 것인지 방금 타이핑한 것인지 구분할 플래그가
        > 없다. 따라서 **사용자가 Manual 파라미터 값으로 리터럴 `***` 를 의도적으로 넣는
        > 것도 거부된다.**
        >
        > **판정 기준은 "출처" 가 아니라 페이로드의 저작 주체다.** Manual 트리거 파라미터는
        > 워크플로 작성자가 정의한 값 슬롯이고, 이 제품은 **이미 그 표면에서 마커 리터럴을
        > 값으로 취급하지 않는다** — 프런트 가드(2026-08-20, `editor-toolbar.tsx`)가 출처와
        > 무관하게 마커 leaf 를 막는다. 서버는 그 규칙을 API 레벨로 옮길 뿐이다. **두 층이
        > 다른 규칙을 쓰면 한쪽만 통과하는 값이 생긴다.**
        >
        > 반면 **webhook ingestion 과 schedule 은 대상이 아니다.** 그쪽 body 는 외부 시스템이
        > 저작하는 임의 페이로드라 리터럴 `***` 가 정상 값일 수 있다(사용자가 폼에 별표를
        > 입력했을 수 있다). 그래서 거부를 `resolveTriggerParameters` **공유 함수 안에 넣지
        > 않는다** — 공유 프리미티브를 넓히면 무관한 경로가 오염된다.
        >
        > **알려진 제약**: 마스킹 마커 세 문자열은 Manual 파라미터에서 **예약어**가 된다.
        > 그 값을 정말 보내야 하면 앞뒤에 문자를 붙이면 된다(정확 일치만 보므로 `a***b` 는
        > 통과한다). 이 비용을 받아들이는 이유는 프런트가 이미 같은 비용을 치르고 있고,
        > 두 층의 규칙을 갈라 두는 비용이 더 크기 때문이다.
        ...
        >
        > **보장의 경계 — 정확 일치만 감지한다.** 값-마스킹의 **부분 치환**
        > (`scheme://user:pass@host` → `scheme://***@host`)은 문자열 전체가 마커가 아니라
        > 감지되지 않는다. 자격증명은 이미 제거된 뒤라 **노출 위험은 없지만** round-trip
        > 성질은 남는다. 포함-매치로 넓히지 않는 이유는 `a***b` 같은 정상 값까지 막아
        > 가드가 정상 워크플로를 망가뜨리기 때문이다(프런트 캐너리가 양방향으로 고정).
    ...
    - **프리필 왕복 — 마스킹된 값이 되돌아와 실제 입력이 되는 경로 (2026-08-17)**:
      ...
      - 마커 집합과 깊이 상한의 SoT 는 **공유 패키지**
        ...
        > 대해 조용히 fail-open 했다. 미러를 기계가 대조하게 만들려 했더니 **CI 경로
        > 게이팅**에 막혀(한쪽 워크플로가 반대쪽 변경 때 검사를 생략한다) 값 자체를
        > 옮겼다.
...
- **~~`nodeOutput` 일반 키 allowlist (미구현·잔여)~~ 해소 (2026-08-23) — ~~단 `getStatus` 한 출구에 한정~~
  ...
  **~~REST 와 SSE 는 같은 강도다.~~** ~~초판은 *"이 시점부터 방어 강도가 다르다"* 로 비대칭을 기록했으나
  ...
  >
  > **래퍼/도메인 값 구분의 정본은 [node-output.md Principle 0 의 `wire envelope` 각주](../conventions/node-output.md) 다** — 이 절은 그 계약을
  > 재진술하지 않고 인용한다(2026-08-24, `13_30_49` W3).
  >
...
### R-replay-unavailable. SSE 버퍼 만료 신호 구현 (결정 2026-07-08)

**채택**: SSE 재연결 replay(`Last-Event-Id`) 시 5분 버퍼가 요청 범위를 **완전히** 재전송하지 못하면(만료 또는 `MAX_BUFFER_PER_EXEC` 상한 폐기로 중간 seq 유실) 부분 replay 대신 `execution.replay_unavailable` 을 한 번 push 한다 (`sse-adapter.service.ts` `replayOrSignalUnavailable`). 클라이언트는 이 신호를 받으면 `GET /api/external/executions/:id` 로 현재 상태를 재조회한다. 종전의 "만료분 silent drop" 을 명시 신호로 대체 (EIA-IN-07/EIA-NF-03 하드닝).

- **gap 판정**: seq 는 monotonic 이라 만료·폐기는 항상 앞쪽부터 발생한다. 재생 가능한(비만료) 가장 이른 seq 가 `Last-Event-Id + 1` 이 아니면 그 사이 구간이 유실된 것 → 완전 replay 불가로 판단한다. 추가로 재생 가능 구간 **내부**에 hole(예: upstream seq 유실/중복 등 비정상 상황)이 있어도 gap 으로 판정한다 — 이 기능이 없애려는 silent drop 을 놓치지 않기 위한 전 구간 연속성 방어(`replayable.every`). `seq > Last-Event-Id` 인 이벤트가 버퍼에 하나도 없으면(클라이언트가 최신까지 수신) 누락이 아니므로 신호를 보내지 않고 live stream 으로 이어진다.
- **`seq=0` control frame + `id:` 생략 채택**: 본 이벤트는 실행-scope monotonic seq 를 배정받지 않는 재연결 응답 전용이라 `seq=0` sentinel(§5.3 `SSE_SEQ_PLACEHOLDER` 관례와 동일 도메인)을 쓰고 SSE wire `id:` 라인을 생략한다. **`id: 0` 을 싣는 대안 기각**: WHATWG EventSource 는 `id:` 를 받으면 내부 Last-Event-Id 를 갱신하므로, 이후 브라우저 자동 재연결이 `Last-Event-Id: 0` 을 보내 EIA-IN-07 의 "전체 replay(`?lastEventId=0`)" 를 의도치 않게 촉발한다. `id:` 생략은 클라이언트의 실 위치를 보존한다.
- **native WS 미도입 근거**: 내부 WS 는 별도 seq 버퍼 없이 재구독 시 `execution.snapshot`(현재 전체 상태) 1회로 복구하므로([WS §6.2](./6-websocket-protocol.md#62-놓친-이벤트-복구)) 만료 신호가 구조적으로 불필요하다. 초기 WS §6.2 Rationale 의 "양 전송에 동시 도입" 문구는 WS 가 버퍼를 갖는다는 전제였으나 그 전제가 폐기됐으므로, 본 신호는 **SSE 단독** 구현이다 (WS §6.2 Rationale 동기 정정).
- **신호 후 연결 유지**: 본 신호를 push 한 뒤에도 SSE 연결은 닫지 않고 subscriber 를 활성 set 에 유지한다 — 클라이언트는 REST `getStatus` 로 현재 상태만 병행 보정하고 이후 이벤트는 열린 스트림으로 계속 수신한다(REST 응답의 `seq` 는 항상 `0` placeholder 라 재연결 근거가 없으므로 재연결이 아니라 병행 보정이 유일하게 타당한 흐름 — §5.3).
- **known limitation (v1 single-instance)**: 버퍼는 in-memory 라 프로세스 재시작 시 소실된다. 재시작 직후 아직 새 이벤트가 쌓이기 전 `lastEventId=N` 재연결은 `seq>N` 이벤트가 버퍼에 없어 "최신 수신"으로 오판돼 신호를 못 보낼 수 있다(누락분은 다음 이벤트/스냅샷·REST 로 수렴). 지속·분산 버퍼는 §R10(분산 SSE fan-out) 후속에서 함께 다룬다.

### R19. 공개 위젯 idle-wait execution 회수 (EIA-RL-07) — 토큰-만료 신호 + EIA-RL-06 형 sweep (결정 2026-07-11)

**맥락**: 공개 웹채팅 위젯(`auth_config_id IS NULL`, per_execution)은 패널 open 마다 eager-start 로 execution 을 만든다([1-widget-app §R6](../7-channel-web-chat/1-widget-app.md)). 사용자가 미전환 이탈(탭 종료·"닫기")하면 그 execution 은 명시 종료 없이 `waiting_for_input` 로 잔존한다. 위젯의 "새 대화" best-effort `cancel`([1-widget-app §R9](../7-channel-web-chat/1-widget-app.md))은 정상 경로만 즉시 닫으므로, 유실 경로의 잔존을 서버가 회수하는 backstop 이 EIA-RL-07 이다.

**채택 (신호 = 토큰 영구 만료, 메커니즘 = EIA-RL-06 형 sweep)**:
- **판정 신호를 blanket idle-timer 가 아니라 토큰 영구 만료로 둔다.** "N일 idle → kill" 류 엔진 전역 GC 는 며칠 뒤 입력이 도착할 legitimate 대기([4-execution-engine §7.4](./4-execution-engine.md#74-분산-실행-multi-instance) 무기한 보존)를 오살할 위험이 있다. 반면 익명 위젯 토큰은 **만료 후 refresh 불가**(만료 JWT 는 `InteractionGuard` 가 `TOKEN_EXPIRED`/401 로 선차단, refresh 핸들러 §5.5 도달 불가; alive 세션은 만료 전 proactive refresh(EIA-AU-05)로 유지)라 "모든 발급 토큰 만료"(`execution_token.exp_at` 전부 `< now`)는 **익명 사용자가 그 execution 을 영원히 이어갈 수 없음의 증명**이다. 회수 대상(provably un-continuable)은 불변식이 보호하는 "곧 올 입력을 기다림" 케이스와 disjoint 하므로 불변식 보호 대상을 약화하지 않는다.
- **`cancelledBy` 닫힌 union 을 확장하지 않는다.** idle-wait 도 시간 bound 초과라 §8 큐 대기와 같은 `cancelledBy='timeout'` 을 재사용하고, 신규 `error.code='WEBCHAT_IDLE_TIMEOUT'` 으로만 세분화한다(선례: `'system'`+`RESUME_*`, `'timeout'`+`EXECUTION_QUEUE_WAIT_TIMEOUT`). `error.code` prefix 를 `CHANNEL_` 이 아닌 `WEBCHAT_` 으로 둔 것은 정확히 범위 밖으로 배제한 [Chat Channel](./15-chat-channel.md) 모듈과의 네이밍 혼동을 피하기 위함이다.
- **메커니즘은 신규 아키텍처가 아니라 EIA-RL-06 sweep 의 형제다.** ① [4-execution-engine §1.1](./4-execution-engine.md#11-execution-상태) 전이표가 `waiting_for_input → cancelled` 의 사유로 "타임아웃" 을 **이미 예약**하므로 EIA-RL-07 은 그 예약 사유의 최초 구현이다. ② EIA 계층은 이미 `terminal-revoke-reconciler`(§9.3 R15, BullMQ repeatable · `execution_token` sweep · 전역 1회)를 운영하므로, 회수 sweep 은 **동일 계층·동일 데이터소스(`execution_token.exp_at`)·동일 전역-1회 패턴의 형제**(별도 서비스/큐명으로 분리 — 혼동 방지)다. 따라서 실행 엔진의 "신규 주기 스캐너 미도입" 원칙([4-execution-engine §7.4](./4-execution-engine.md#74-분산-실행-multi-instance) Rationale — *job-backed RUNNING/pending 엔진 recovery* 스코프)과는 무관하다(park 는 BullMQ job 자체가 없어 engine recovery 대상이 아님).
- **기각 대안**: execution 별 delayed job 을 매 토큰 refresh 마다 재스케줄해 exp+grace 에 발화시키는 job-based 안 — waiting execution(잠재 수백만)마다 delayed job 추적 + 매 refresh 재스케줄 비용 + park 의 no-job 설계 반전을 유발한다. `execution_token` durable 추적을 그대로 재사용하는 단일 전역 sweep 이 더 단순·저비용이라 채택.

**회수는 soft-terminal**: hard-delete 아님 — `cancelled` 전이 + 토큰 일괄 revoke(`revokeAllForExecution`, EIA-RL-06)이며 이력·`GET /:id`(§5.3) 조회는 보존된다. 범위는 `auth_config_id IS NULL` + per_execution 한정 — 인증 트리거·per_trigger·워크플로우 정의 `formConfig.timeout` 은 대상이 아니다.
