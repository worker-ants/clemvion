    ...
    - **~~잔여 ②~~ 해소(2026-08-20) — `outputData`(2026-08-16)에 이어 `Execution.inputData`
      ...
      **축을 정확히 하면: round-trip 되는 것만 카브아웃한다.**
      ...
      - **닫는 조건은 충족됐다 (2026-08-20)**. 조건은 *"프런트가 마스킹 마커를 감지해 해당
        ...
        > **가드의 범위 — 재제출 경로 한정이다.** 서버측 거부는 *"마스킹된 읽기에서
        > 되돌아온 값"* 을 겨냥하므로 **재제출 성격의 두 호출부에만** 건다. `resolveTrigger
        > Parameters` 공유 함수 안에 넣지 않는다 — 같은 함수를 쓰는 **webhook ingestion 과
        > schedule 은 대상이 아니다**. 그쪽 body 가 리터럴 `***` 를 담는 것은 정상일 수 있고
        > (사용자가 폼에 별표를 입력했을 수 있다), 공유 프리미티브를 넓히면 무관한 경로가
        > 오염된다.
        >
        > 즉 판정 기준은 값의 성질이 아니라 **출처의 성질**이다 — *"이 값이 마스킹된 읽기에서
        > 되돌아온 것인가."* 출처를 아는 호출부가 판정 지점이다.
