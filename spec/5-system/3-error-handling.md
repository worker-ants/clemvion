...
| `RESERVED_VARIABLE_NAME` | Variable Declaration/Modification 노드의 변수 이름이 시스템 예약 `__`(double-underscore) prefix — `variables.__*` 는 엔진 시스템 네임스페이스([execution-context 원칙 5](../conventions/execution-context.md)). **저장 시점 게이트**(리터럴 이름): `saveCanvas`/`importWorkflow` 가 `details.offenders[]` 와 함께 발행 (`restoreVersion` 은 legacy-data escape 로 면제). **런타임 해석 후**(`{{ }}` 표현식 이름) 는 두 노드 `handler.execute` 가 message-prefix `RESERVED_VARIABLE_NAME:` 로 throw → 엔진이 노드 실패로 분류. pre-flight 리터럴 검사는 `INVALID_NODE_CONFIG` 로 격하 | 400 |
