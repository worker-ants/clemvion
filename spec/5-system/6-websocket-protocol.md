...
> ack data 에 `nodeId` 는 포함되지 않는다 — 대기 노드는 서버가 lookup(`execution_id + status='waiting_for_input'`)하며 클라이언트는 `executionId` 만으로 ack 를 매칭한다. 단 명령 body 에 `nodeId` 를 실은 경우(`submit_message`/`end_conversation` 는 frontend 가 대기 노드 `nodeId` 를 싣는다), 서버는 lookup 으로 찾은 대기 노드와 그 `nodeId` 를 **대조**해 불일치 시 `INVALID_EXECUTION_STATE` 로 거부한다 (F-6 / [실행 엔진 §7.5.1](./4-execution-engine.md#751-publisher-측-사전-검증--invalid_execution_state)) — nodeId 를 lookup 키로 쓰는 것이 아니라 stale/오지정 제출을 걸러내는 사후 검증이다.
...
| `VALIDATION_ERROR` | `submit_form` 의 field 검증 실패 (필수/type/minLength·maxLength/min·max(숫자 범위)/pattern(정규식)/select·radio 선택지/`type:'file'` MIME·크기·개수). publisher 측 `continueExecution` chokepoint 가 노드 config 의 field 정의로 동기 검증 (typed `FormValidationError`) — EIA REST 의 `400 VALIDATION_ERROR` + `details[]` 와 동일 의미·동일 검증 지점. execution 상태 유지(재제출 가능). **ack payload 는 평면 `{ success:false, error, errorCode:'VALIDATION_ERROR' }` 로 field-level `details[]` 를 포함하지 않는다** — 어떤 필드가 왜 실패했는지의 구조화 상세(`{field,message,code}`)는 EIA REST 경로(`400` body `error.details[]`, [EIA §5.1](./14-external-interaction-api.md#51-인터랙션-명령-제출--post-apiexternalexecutionsexecutionidinteract))에서만 제공된다. WS 클라이언트는 사람용 `error` 문자열로 폼을 재표시한다 |
...
- 대화 UI (conversation Preview 탭, conversation timeline) 는 **대화 turn 의 1차 소스**로 emit messages 가 아닌 `waiting_for_input.conversationThread.turns` snapshot (§4.4.5) 을 사용한다
  - **보조 관찰성 레인은 예외** (2026-07-17): 같은 UI 가 `nodeOutput.meta.turnDebug[].ragSources` **한정**으로 KB 검색 표시(🔎 `rag` 행 · 📚 References chip)를 함께 렌더한다 — turn 을 **대체하지 않고 추가**되는 별개 축이다. 근거·범위: [Conversation Thread §8.6](../conventions/conversation-thread.md#86-rag-source-신설--보조-관찰성-레인의-정식화) · 데이터 소스 규칙: [§9.3](../conventions/conversation-thread.md#93-데이터-소스-선택). **형제 필드 `llmCalls` 는 이 예외에 포함되지 않는다** — raw request/response payload (위 §4.4 표) 로 debug 탭 전용이다. source 별 시각 매핑은 [Spec Conversation Thread §9](../conventions/conversation-thread.md#9-미리보기-ui-렌더-규칙) 의 강제 규약을 따른다 — `injected` chip 표시는 "권장" 이 아니라 §9.2 의 3중 신호(아이콘 + 컨테이너 형식 + chip) 동시 적용이 **필수** 다. LLM debug 패널 (Request / Response / LLM Usage) 만 emit messages 의 raw payload 를 사용하며, 이때도 "Raw payload" 토글로 명시한다 (§9.3·§9.4).
...
> **방향 정정**: 구현은 server-발신 app ping 이 아니라 **client-발신 ping → server pong** 이다 (`handlePing`). 서버가 주기적으로 app `ping` 을 push 하는 경로는 **비채택 (won't-do)** 이다 — §5.1 이 확정한 Socket.IO 내장 heartbeat 가 이미 그 자리를 채우고, 그 위의 앱 레벨 ping 은 소비처도 주기도 정의되지 않은 주기적 브로드캐스트를 하나 더 만든다. 근거 §Rationale `R-wontdo-maintenance-appping`.
...
`R-wontdo-rawws-rest`(2026-07-08)가 **범위 밖으로 남겨 둔** 3항목 중 2항목을 정식 비채택(won't-do)으로 종결한다. 그 결정이 이 둘을 다루지 않은 사유는 "트리거 소스 설계가 필요하다" 였는데, 2026-08-31 착수 시도에서 **설계가 필요한 것이 아니라 대상이 없다**는 것이 실측됐다.

- **§4.6 `system.maintenance` emit — 발화 주체가 존재하지 않는다**
  - 실측(2026-09-02): `system.maintenance` 는 **`spec/` 에만 등장하고 백엔드 코드에는 0건**이다. 유지보수를 선언하는 관리자 API·설정·스케줄이 어디에도 없고 계획에도 없다.
  - payload 의 `scheduledAt` 은 **사람이 미래 시점을 선언**해야 성립한다. 그 표면을 만드는 것은 spec-impl 갭 메우기가 아니라 **신규 제품 기능**이다.
  - 유일한 기존 후보인 `onApplicationShutdown(signal)`(SIGTERM)에 배선하는 안은 **기각**한다 — SIGTERM 은 사전 예고가 없어 `scheduledAt`("예정된")이 표현하는 사건과 다르다. 거기 묶으면 payload 가 약속하는 것보다 **좁은 보장을 넓은 이름으로** 내보내게 된다.
- **§5 서버발신 app ping — 전송 계층이 이미 그 자리를 채운다**
  - §5.1 이 전송 계층 heartbeat 를 Socket.IO/Engine.IO 내장 ping/pong(`pingInterval` 25s / `pingTimeout` 20s)으로 확정했다. 그 위의 앱 레벨 서버발신 ping 은 **소비처도 주기도 정의되지 않은 주기적 브로드캐스트**를 하나 더 만든다.
  - 이 줄은 `R-wontdo-rawws-rest` 가 종결한 raw-WS 2종(서브프로토콜 인증·raw close code)과 **같은 초안 전제**에서 왔다 — 위 §전송 계층 정정이 "서버발신 30s/10s app ping" 을 그 초안 약속 목록에 함께 적는다. 그때는 범위 밖으로 뒀고, 이번에 다시 물으니 답이 같다.
- **폐기 대안**
...
- **정정 내용**: (1) `execution.submit_form` payload 를 `{ executionId, formData }` 로, `execution.click_button` 을 `{ executionId, buttonId }` 로 정정 — frontend(`use-execution-interaction-commands.ts`)는 이 두 명령에 `nodeId` 를 싣지 않는다. 대기 노드 식별은 publisher 측 server lookup (§7.5.1 경로) 이 수행하므로 클라이언트 전달이 불필요하다. **(F-6 후속 정정)** click_button 은 backend handler 가 `nodeId?` optional 을 받아 제공 시 publisher 로 forward 하도록 확장됐다(frontend 미전송이라 실질 no-op). 반면 `execution.submit_message`/`end_conversation` 은 frontend 가 대기 노드 `nodeId` 를 이미 싣고 있어(§4.2 명령 표), F-6 부터 서버가 이를 대기 노드와 대조·검증한다 — nodeId 는 여전히 lookup 키가 아니라 사후 검증 입력이다. (2) 옛 표기의 `toolCallId?` 클라이언트 필드도 제거 — `render_form` 도구 응답 매칭은 클라이언트 전달값이 아니라 서버 보관 `pendingFormToolCall` resume state 로 처리된다. (3) 공통 ack payload 표에서 `nodeId` 행 제거 — 4개 continuation 명령의 ack data 어디에도 `nodeId` 가 없다.
