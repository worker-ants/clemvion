...
- **분산은 Continuation Bus (§7.4) 가 담당** — 인스턴스 간 fan-out 은 BullMQ 영속 큐 `execution-continuation` 이 처리하므로, 이벤트 발행 추상화와 분산 동작은 직교. 옛 Redis pub/sub 채널 `execution:continuation` 은 폐기 (§Rationale "Durable Continuation (2026-05-24)").
...
| **Re-run** | 새 Execution 시작 — 현재 워크플로 정의의 raw config 를 다시 평가 | ✅ 정의됨 — 명세는 [`./13-replay-rerun.md`](./13-replay-rerun.md), 구현은 v1 PR2 머지 후 활성화 | ✅ 재트리거 — 이메일 재발송, HTTP 재호출 등. dry-run 토글로 skip 가능 ([`./13-replay-rerun.md`](./13-replay-rerun.md) §7) | ✅ `$now` / `random()` 등 새 실행 시점 재고정 |
...
사용자 입력 fan-out 은 **영속 BullMQ 큐** 를 단일 진입으로 사용한다. 이전의 Redis pub/sub (`execution:continuation` 채널) 은 폐기 — at-most-once 의미론으로 인해 (a) 어느 인스턴스에도 메모리 resolver 가 없는 순간 메시지가 silent drop 되는 race 와, (b) 컨테이너 재시작 후 재개 불가 가 발생했다 (자세한 결정 근거는 §Rationale "Durable Continuation (2026-05-24)").
...
- **Stale 대상 한정**: `status='running'` 인 row 만 stuck recovery 대상. **`status='waiting_for_input'` 은 무기한 보존** — 사용자 입력은 며칠 후 도착할 수도 있고, 노드별 `formConfig.timeout` 등 워크플로우 정의된 별도 timeout 이 적용된다. 임계값 (`STUCK_RECOVERY_STALE_MS`) 은 RUNNING 의 worker heartbeat 미응답 검출에만 사용한다 (§7.1 Worker Heartbeat 와 일관).
- **WAITING_FOR_INPUT 의 운명**: 부팅 시점에 `WAITING_FOR_INPUT` 인 Execution 은 본 함수에서 **무시**. 사용자 입력이 도착하면 §7.5 의 rehydration 경로로 자연스럽게 재개된다. 옛 동작 (WAITING_FOR_INPUT 일괄 FAIL) 의 운영 회귀(2026-05-24)는 §Rationale "Durable Continuation" 참조.
...
[client] → controller / WS gateway
  ...
  → rehydrate (slow path — 신규)
       ...
       │   _resumeState / conversationThread 로드
...
| 노드 핸들러가 `waitForX()` 를 다시 부를 수 없는 경우 (예: multi-turn 의 `_resumeState` 가 schema 변경 후 deserialize 실패) | Execution `cancelled` + `error.code='RESUME_INCOMPATIBLE_STATE'`, 동반 NodeExecution `failed` |
...
| `exec:cont:seq:<executionId>` | continuation publish 의 monotonic seq (Redis INCR per executionId) — BullMQ jobId (`${executionId}:${nodeExecutionId}:${seq}`) 의 idempotency key. executionId 는 UUID, 워크스페이스 단위가 아닌 **전역**. 8 bytes 미만. executionId 종결 후에도 잔류하나 동작에 무영향 — TTL 미설정 안전 (Phase 3 후속 정리 예정) | 미설정 (자연 expire — Phase 3 후속 정리) |

> 전역 키 `exec:recover:lock` 및 `exec:cont:seq:<executionId>` 는 §9.1 의 `{service}:{workspaceId}:{resource}` 패턴을 따르지 않는다. 부팅 단일 진입 가드라는 **워크스페이스에 종속되지 않는** 책임을 가지므로 전역 키로 둔다. (옛 `execution:continuation` Redis pub/sub 채널은 BullMQ 큐 `execution-continuation` 로 교체되어 폐기 — §9.3 / §Rationale "Durable Continuation (2026-05-24)".)
...
| 환경 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `SIGTERM_GRACE_MS` | `30000` (30초) | k8s `terminationGracePeriodSeconds` 와 일관되게 설정. 셀프호스팅 환경의 Helm Chart 권장값은 [`plan/in-progress/self-hosting-deployment.md §4 Kubernetes Helm Chart`](../../plan/in-progress/self-hosting-deployment.md) 참조 — `terminationGracePeriodSeconds = ceil(SIGTERM_GRACE_MS / 1000) + 5` (5초 = readiness drain 여유) |
...
- CONVENTIONS: `아카이브/node-specs-improvement/CONVENTIONS.md` Principle 7 / 1.1.3 / 8.2.
...
### Durable Continuation & Graceful Shutdown (2026-05-24)
...
1. (기각) **Temporal / Inngest 같은 전용 워크플로우 엔진으로 이전.** 현재 엔진은 expression resolver / multi-turn / container / chat-channel trigger 등 도메인 특화 코드를 30+ 노드 핸들러 곳곳에 분산해 두었다. 전면 이전 비용이 운영 회귀 한 건의 해결 비용을 한참 초과. 향후 본 엔진의 유지 비용이 임계 (durable timer, signal, child workflow 같은 기능 요구가 누적) 에 도달하면 재검토. 본 결정은 위 "Engine Raw Config Exposure" 결정과 직교 — raw config 노출은 핸들러 계층 결정, durable continuation 은 엔진 인프라 계층 결정.

2. (기각) **WAITING_FOR_INPUT → INTERRUPTED 신규 enum 도입 후 새 컨테이너가 resume.** Execution.status enum 확장은 DB migration / frontend status pill / 외부 API 응답 / Re-run / Execution History 필터 등 cross-spec drift 가 크다. 외부에서 본 상태는 변하지 않게 두고, 내부에서 rehydrate 하는 편이 변경 표면적이 작다.

3. (기각) **계속 Redis pub/sub 으로 두고, "메시지 도착 시 어느 인스턴스도 Map 키가 없으면 잠시 대기 후 재시도" 정책.** 대기 시간 / 재시도 횟수 튜닝이 워크로드별로 다르고, 단순한 at-most-once 의 한계를 우회만 할 뿐 근본 해결이 아님.

4. (채택) **BullMQ 영속 `execution-continuation` 큐 + §7.5 rehydration 경로.** 이미 `background-execution` 큐로 동일 패턴이 검증됨 (§3.3). idempotency key + dead-letter 까지 BullMQ 내장. 신규 인프라 도입 없음.
...
- `spec/0-overview.md §2.4 Rationale "실행 엔진"` 의 "continuation bus … 같은 Redis 재사용" 문장을 BullMQ `execution-continuation` 큐 기준으로 갱신.
- `spec/0-overview.md §6.1 구현 완료` 의 "분산 continuation bus" 를 "BullMQ 영속 큐 기반" 으로 보강.
- `spec/0-overview.md §2.6 Data Layer` Redis 항목에서 "실행 상태 Pub/Sub" 표기를 continuation 목적이 아닌 다른 용도(운영 lock, KB 채널 등) 로 한정.
- `spec/data-flow/3-execution.md` §1.1 시퀀스 다이어그램 + line 20 파일 참조 + line 165 mermaid 라벨 + line 232–237 "Continuation bus = Redis pub/sub" Rationale 절을 BullMQ 큐 기반으로 갱신 (역전 갱신).
- `spec/4-nodes/6-presentation/0-common.md §10.9` 의 internal continuation bus payload SoT 표기에서 "Redis pub/sub `execution:continuation` 채널" 을 "BullMQ `execution-continuation` 큐" 로 교체. sentinel wrap 형식은 변경 없음 — 전송 매체만 바뀜.
- `spec/1-data-model.md §2.13` Execution.error 필드 설명에 신규 code 4종 (`SERVER_INTERRUPTED` / `RESUME_FAILED` / `RESUME_CHECKPOINT_MISSING` / `RESUME_INCOMPATIBLE_STATE`) 어휘 노트 추가.
- `spec/5-system/6-websocket-protocol.md §4.2` ack payload 에 `queued: boolean` 선택 필드 추가 + `submit_form` / `click_button` / `submit_message` / `end_conversation` 의 공통 에러 코드에 `RESUME_*` 3종 추가.
...
**3. heartbeat 기반 Recovery 전환 결정 (§7.4 `recoverStuckExecutions`)**:

Phase 2 WIP 까지 `recoverStuckExecutions` 의 stale 임계값은 `started_at < now() - 30분` 절대 시간으로 판정했다. 이 방식은 (a) worker 가 정상 처리 중인 장시간 실행 (AI multi-turn, 외부 API 대기 등) 을 early-FAIL 하는 false positive 위험이 있고, (b) WAITING_FOR_INPUT 상태의 Execution 을 §7.4 가 stale 대상에 포함하면 사용자가 며칠 후 응답하는 케이스까지 일괄 종결하는 운영 회귀 (2026-05-24 발생) 의 근본 원인이 됐다.

**결정**: stale 판정 기준을 절대 시간에서 **worker heartbeat 미응답** (§7.1 — 5초 간격, 3회 연속 미응답 = 15초) 으로 교체한다.
- STUCK_RECOVERY_STALE_MS 는 RUNNING 의 heartbeat 미응답 검출에만 적용. `status='waiting_for_input'` 은 무기한 보존 — 사용자 입력이 도착하면 §7.5 rehydration 으로 자연 재개.
- false positive 임계: worker heartbeat 가 살아있으면 절대 시간과 무관하게 stuck 판정 불가 → 장시간 실행도 안전.
- WAITING_FOR_INPUT 의 운영 회귀 (2026-05-24) 는 WAITING_FOR_INPUT 을 stale 대상에서 명시 제외하여 차단.
