...
> 짝 전이는 같은 트랜잭션 안에서 Execution 행을 `SELECT … FOR UPDATE` 로 잠그고 비-terminal 을
> 확인한 뒤에만 두 save 를 수행하며, 선점이 관측되면 **어느 쪽도 쓰지 않고 `false` 를 반환**한다.
...
> (`status IN (비-terminal)`)를 거친다 — 상세는
> [node-cancellation §2.4](../conventions/node-cancellation.md).
...
| `cancelled`         | 노드가 **실패한 것이 아니라 중단된** 상태. 두 경로가 같은 상태로 귀결된다 — (1) **signal 경로**: 외부 cancellation signal (`ExecutionContext.abortSignal`) 로 외부 I/O 가 중단돼 핸들러가 throw 한 `AbortError`(`error.name === 'AbortError'`) 를 엔진이 분류. dispatch 직전 이미 abort 된 경우도 동일(핸들러 미실행). (2) **DB 관측 경로** (2026-07-26~27): 엔진이 노드 경계·turn 경계·짝 전이에서 Execution 행을 재조회해 외부 cancel 을 관측하고 `ExecutionCancelledError` 를 throw — 사용자 Stop 은 signal 을 만들지 않으므로 선형·resume 경로에선 이쪽이 유일한 관측 수단이다. 생산자: Parallel `cancel-others-on-fail` / 사용자 cancel / **엔진 DB 관측 가드(노드 경계·AI turn 경계·park 짝 전이)** / (향후) Workflow timeout. 종료 시 `execution.node.cancelled` WS 이벤트 발행. SoT: [node-cancellation §2.4·§5](../conventions/node-cancellation.md#5-aborterror-분류) |
...
- shape: `_retryState` 와 동일 부분집합(messages / turnCount / model / temperature / maxTokens / knowledgeBases / RAG / MCP / pendingFormToolCall? 등)이되 **`expiresAt`(TTL) 없음** — 대화는 장시간 idle 후에도 재개 가능(waiting Execution 은 무기한 보존) — 과 **`lastUserMessage` 없음** — 재개 시 도착한 사용자 메시지(continuation payload)를 그대로 첫 turn 으로 처리. credential / context-binding 필드는 미동봉 (~~`maskSensitiveFields` boundary~~ **allow-list 로 애초에 배제** — 그 boundary 는 2026-08-24 에 제거됐고, **이 배제는 그것과 무관**하다(`buildRetryState`/`buildResumeState` 가 옮길 키를 열거한다))이며, 재개 시 **두 재유도 채널**로 재구성된다 — **조작 필드**(`llmConfigId`/`maxTurns`/`conditions`/`presentationTools` 등)는 `node.config` 재평가로, **식별 필드**(`workflowId` ← `execution.workflowId`, 재개 대상 `nodeExecutionId` ← 대기/재시도 NodeExecution row PK 를 `buildRetryReentryState` opts 로 주입, `workspaceId` ← 실행 컨텍스트)는 **호출측 컨텍스트**(execution / 재개 대상 NodeExecution row 등 opts)에서 재유도한다.
...
> **불변식 (usage-log attribution)**: multi-turn resume/retry 턴의 provider-tool `logUsage` attribution(§10.3 호출 순서 — `IntegrationUsageLog.node_execution_id`)은 위 **식별 필드** `nodeExecutionId`·`workflowId` 가 `resumeState`(=`buildRetryReentryState` 재구성)를 통해 재유도되어야 성립한다. 두 필드는 credential-strip allow-list(`CREDENTIAL_CONTEXT_FIELDS`)에 의해 checkpoint 영속에서 제거되므로, 재구성 시 호출측 컨텍스트(`execution.workflowId` + 재개 대상 NodeExecution row PK opts)에서 재주입되지 않으면 provider-tool 의 usage-log 게이트(`ctx.nodeExecutionId && ctx.workflowId`)가 false 가 되어 재개/재시도 턴의 통합 사용 로그가 조용히 누락된다(회귀 #501). **소비 sink 는 노드 유형별로 다르다** — `ai_agent` 의 provider-tool `IntegrationUsageLog` 는 게이트드(부재 시 로그 drop)이나, 두 노드의 LLM 호출이 남기는 `llm_usage_log`(`LlmUsageLogService.record`, ungated `?? null`)는 식별 필드 부재/오기입 시 로그를 drop 하지 않고 attribution 을 **오염**시킨다 — provider-tool 이 없는 `information_extractor` 재개 경로가 후자에 해당한다(§Rationale "적용 범위"). 현재 이 불변식은 회귀 테스트로만 강제되므로 `CREDENTIAL_CONTEXT_FIELDS`/`resumeStateSchema` 리팩터 시 반드시 보존한다 (§Rationale "resume/retry 턴 usage-log attribution").
...
- `_retryState` shape: `_resumeState` 의 부분집합 + `expiresAt: ISO 8601` (TTL — 기본 60분, env `AI_RETRY_STATE_TTL_MINUTES` override). credential 제거 정책은 `_resumeState` 와 동일 (~~`maskSensitiveFields` boundary~~ **allow-list 로 애초에 배제** — 그 boundary 는 2026-08-24 에 제거됐고, **이 배제는 그것과 무관**하다(`buildRetryState`/`buildResumeState` 가 옮길 키를 열거한다)). expression resolver / autocomplete 비노출.
...
- 소스 노드의 출력에 `_selectedPort`가 있는 경우: back-edge의 `sourcePort`가 선택된 포트와 일치할 때만 활성화. `_selectedPort`가 **배열**(fan-out — parallel / text_classifier 등 복수 포트 동시 활성화, [node-output Principle 5](../conventions/node-output.md))이면 `sourcePort`가 배열에 **포함**될 때 활성화 (코드 `graph-traversal.service.ts` `isPortFiltered` 의 `.includes()` 분기)
...
`_selectedPort`는 해당 노드의 엣지 라우팅에만 사용되는 내부 메타데이터이다. 단일 포트 선택은 문자열, 복수 포트 fan-out(Principle 5)은 `string[]` 이며(§5.1 `NodeHandlerOutput.port` 와 동형), 엣지 활성화는 `sourcePort` 의 문자열 일치(단일) 또는 배열 포함(fan-out)으로 판정한다. 다운스트림 노드의 input으로 전달될 때 자동으로 제거(strip)된다. 이를 통해 pass-through 노드(Variable, Set Variable 등)를 거쳐도 이후 노드가 잘못 skip되지 않는다.
...
- **재개**: 오직 사용자 인터랙션 도착만이 `execution-continuation` job 을 만들어 다음 active 세그먼트를 시작한다(§7.4/§7.5 경로 무변경). 노드 자체의 워크플로 정의 timeout(예: `formConfig.timeout`)은 엔진 자원 가드와 별개로 유지된다. **예외적으로 provably un-continuable 한 park**(공개 웹채팅 위젯의 익명 per_execution 토큰 영구 만료)은 EIA 계층 sweep 이 `cancelled` 로 회수한다(§7.4 · [EIA-RL-07](./14-external-interaction-api.md#34-신뢰성일관성)) — 엔진 boot-recovery 가 아니라 EIA token-lifecycle sweep 이므로 park 무기한 보존 불변식과 정합.
...
- **순환 의존 처리** — 엔진의 DI 순환은 성격에 따라 **두 가지 표준 기법**으로 해결한다 (둘 다 NestJS 표준이며 회피 안티패턴이 아니다). 어느 것을 쓸지는 아래 기준으로 결정한다:

  | 기법 | 적용 기준 | 사례 |
  | --- | --- | --- |
  | **`forwardRef(() => X)`** (생성자 주입 지연) | 순환이 있으나 양측이 부트스트랩 시점에 함께 인스턴스화돼 정상 해석되는 경우. 기본 선택. | `ExecutionEngineService ↔ WebsocketService`; 후속 ④에서 노출된 `ExecutionEventEmitter → WebsocketService` (ES-module 순환 봉인) |
...
interface NodeHandlerOutput {
  ...
  /** 라우팅 디렉티브. 값은 노드 정의의 출력 포트 식별자. 복수 포트 동시 활성화(fan-out — parallel / text_classifier 등)는 `string[]` (CONVENTIONS node-output Principle 5 "port 활성화 모델"). */
  port?: string | string[];
...
조건 분기 노드(`if_else`, `switch`, `text_classifier`, `http_request`, `ai_agent` 조건 라우팅)는 반환값에 `port` 필드를 함께 설정한다:
...
| `nodeExecutionId`           | 엔진이 handler.execute 호출 직전 주입, 노드별 갱신 | Integration 핸들러가 `IntegrationUsageLog.node_execution_id` 에 기록. AI 노드 핸들러(AI Agent / Text Classifier / Information Extractor)는 `LlmCallContext` 로 `llm_usage_log.node_execution_id` 에 기록한다. 멀티턴(AI Agent / Information Extractor)의 resume 턴은 `ExecutionContext` 미주입이라 엔진 `buildRetryReentryState` 가 재구성 `state` 에 주입한 현재 turn row PK 를 쓰고, Text Classifier(단발 — resume 없음)는 첫 호출의 `context.nodeExecutionId` 만 쓴다 ([data-flow/7-llm-usage §1.3](../data-flow/7-llm-usage.md))                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
> - **(부팅 backstop 트리거 — PR3 구현, 2026-07-04)**: 서버 부팅 시 `recoverStuckExecutions()` 가 `status='running' AND started_at < now() - STUCK_RECOVERY_STALE_MS(30분)` 인 Execution 을 발견하면, **일괄 `failed` 마킹이 아니라** 각 row 를 **원자 re-claim**(`UPDATE … SET started_at=now() WHERE status='running' AND started_at < :threshold RETURNING`; affected=1 인 인스턴스만 소유) 한 뒤 **§7.5 rehydration case B 로 재구동**(rehydrate + 완료 노드 이후부터 `runNodeDispatchLoop` forward)한다. 이는 §7.2 point 3(재시작 resume)을 **일반 노드 대상으로 실제 구현**한 것이다. 분산 lock·WAITING_FOR_INPUT 제외 등 recovery 동작은 §7.4 Recovery 가 SoT. **PR4 이후에도 은퇴하지 않고 backstop 으로 유지** — stalled job 자체가 없는 케이스(전체 재시작·Redis 비영속·job 유실)를 담당. **같은 스캔이 orphan `pending` 도 회수한다(2026-07-04)** — admission 재큐 job 이 소실된 `pending`(`queued_at < now − EXECUTION_QUEUE_WAIT_TIMEOUT_MS`)은 §8 wait-timeout `cancelled` 로 마감(RUNNING 은 re-drive, PENDING 은 진행 흔적이 없어 cancel — §Rationale "orphan pending backstop").
> - **(mid-operation stalled 트리거 — PR4 구현, 2026-07-04)**: 아래 표의 BullMQ stalled-job 자동 재배달(운영 중 크래시를 다른 워커가 즉시 이어받음)을 구현했다 — `execution-run` 큐 `maxStalledCount: 1` / `stalledInterval: 30초`. 워커 크래시 = job stall → BullMQ 가 **같은 jobId 로 1회 자동 재배달** → 픽업 워커의 `runExecutionFromQueue` **RUNNING 분기**(`recordRunningSegmentStart` + `redriveStuckExecution`)가 §7.5 case B 로 재구동한다(멱등: 완료 노드 skip). 재배달을 소진하면 `onFailed → finalizeStalledExhausted` 가 `WORKER_HEARTBEAT_TIMEOUT` 로 마감. **이 마감은 단일 트랜잭션이다(2026-08-15)** — Execution 을 `FAILED` 로 쓰는 UPDATE 와 자식 RUNNING `NodeExecution` cascade UPDATE 가 `dataSource.transaction` 으로 묶인다. 종전엔 둘이 각각 autocommit 이라 **부분 커밋 시 자식이 영구 `RUNNING` 으로 잔류**할 수 있었다(유령 running). 같은 2-테이블 쓰기를 하는 자매 `cancelParkedExecution`/`markWebChatIdleTimeout` 은 이미 원자적이었고 **이 경로만 열려 있었다**. ([§Rationale "크래시/재시작 RUNNING 세그먼트 제어된 re-drive" / "PR4 — BullMQ stalled 자동 재배달"](#rationale).)
...
| 항목                     | 동작                                                                                                                                                                                                       |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 부팅 backstop 재개 (**PR3 구현**) | 부팅 시 `recoverStuckExecutions()` 가 stale RUNNING 을 원자 re-claim → §7.5 rehydration case B 재구동(일반 노드). 트리거는 부팅 1회 스캔. **PR4 이후에도 유지** — stalled job 이 없는 케이스 담당. 같은 스캔이 orphan `pending`(job 소실·대기 초과)도 §8 wait-timeout `cancelled` 로 회수(2026-07-04) |
...
- **Stale 대상 = RUNNING(re-drive) + orphan PENDING(cancel)**: `status='running'` 인 stale row 는 §7.5 case B re-drive 대상. **`status='pending'` 이면서 `queued_at` 이 큐 대기 한도(`EXECUTION_QUEUE_WAIT_TIMEOUT_MS`)를 초과한 orphan**(admission 재큐 job 소실)은 같은 스캔(`recoverOrphanPendingExecutions`, 2026-07-04)이 §8 wait-timeout `cancelled` 로 회수한다 — RUNNING 은 재구동할 진행 흔적(부분 완료 노드)이 있으나 PENDING 은 없어 cancel(§Rationale "orphan pending backstop"). **`status='waiting_for_input'` 은 무기한 보존** — 사용자 입력은 며칠 후 도착할 수도 있고, 노드별 `formConfig.timeout` 등 워크플로우 정의된 별도 timeout 이 적용된다(§1.1 전이표는 `waiting_for_input → cancelled` 사유로 "타임아웃" 을 예약해 둔다). 이 "정의된 timeout" 카테고리에는 워크플로우 정의뿐 아니라 **공개 웹채팅 위젯이 판정하는 provably un-continuable 상태**(익명 per_execution 토큰 영구 만료 → [EIA-RL-07 / §R19](./14-external-interaction-api.md#34-신뢰성일관성); Chat Channel §15 과는 별개)도 포함되며, 이 회수는 본 boot-only stuck-recovery 가 아니라 **EIA 계층의 repeatable sweep**(EIA-RL-06 형제)이 담당한다 — 무기한 보존이 보호하는 legitimate 대기(입력이 실제 도착할 execution)는 무변경. 임계값은 상태별로 다르다: RUNNING 은 `STUCK_RECOVERY_STALE_MS`(30분, `started_at` 기준), orphan PENDING 은 `EXECUTION_QUEUE_WAIT_TIMEOUT_MS`(5분, `queued_at` 기준). (주의: 현 구현은 RUNNING stale 임계를 **절대 시간**(`started_at < now() - 30분`)으로 적용하며 §7.1 의 주기적 heartbeat 검출은 미구현 — §7.1 구현 상태 banner 참조. heartbeat 기반으로의 전환은 Planned.)
...
| nodeId 불일치                           | `INVALID_EXECUTION_STATE`                 | 명령이 지정한 `nodeId` 가 실제 대기 노드의 nodeId 와 다름 (stale/오지정 제출을 현재 대기 노드로 오적용하지 않고 거부). **caller 가 nodeId 를 지정할 때만** 적용 — 현재 구현은 외부 EIA `/interact`(`InteractDto.nodeId`) 진입점 1곳이 지정한다. 그 외 진입점의 커버리지는 아래 표를 참조. 근거 [EIA §5.1](./14-external-interaction-api.md#51-인터랙션-명령-제출--post-apiexternalexecutionsexecutionidinteract) (`STATE_MISMATCH` "다른 nodeId") + `InteractDto.nodeId` 계약 |
...
- 큐 대기(`now - queued_at`, [§2.13](../1-data-model.md#213-execution)) 시간이 기본 5분(시스템 env `EXECUTION_QUEUE_WAIT_TIMEOUT_MS`, 기본 `300000`ms) 초과 → Execution.status = `cancelled` + `error.code='EXECUTION_QUEUE_WAIT_TIMEOUT'` (시스템 취소 — `cancelledBy='timeout'`, `execution.cancelled` 이벤트를 §7.5 rehydration-실패 cancel 과 동일 경로로 emit, [§3-error-handling §1.5](./3-error-handling.md#15-ws-commands-에러-코드-도메인-spec-참조) / [WS Protocol §4.1](./6-websocket-protocol.md#41-실행-이벤트-server--client)) — **PR2b 구현 완료**
  - **트리거 = admission 시점 검사(주 경로) + 부팅 backstop(orphan)**: consumer(`runExecutionFromQueue`)가 job 을 pick up 할 때 admission gate 이전에 `now - queued_at` 을 확인해 초과 시 재큐 대신 `cancelled` 로 마감한다. cap 초과로 delayed 재큐된 job 은 재시도 tick 마다 다시 pick up 되어 재검사되므로 대기 초과가 반드시 포착된다. **job 자체가 소실된 orphan `pending`(예: Redis 비영속·job 유실) 회수는 `recoverStuckExecutions`(§7.4) 부팅 backstop 이 담당한다 — 구현 완료(2026-07-04)**: 부팅/재시작 스캔이 `queued_at < now − EXECUTION_QUEUE_WAIT_TIMEOUT_MS` 인 `pending`(재-pick up 될 job 이 없는 것)을 consumer pick-up 경로와 **동일한 wait-timeout `cancelled`**(`EXECUTION_QUEUE_WAIT_TIMEOUT`·`cancelledBy='timeout'`·`releaseExecutionRouting`)로 마감한다(멱등 조건부 UPDATE — 동시 admit/cancel 과 race 무해). 한도 이내 orphan 은 다음 스캔에서 초과 시 회수(낮은 확률 엣지·boot-only 트리거 — best-effort).
...
- `context.nodeExecutionId`는 각 노드 호출 직전 새로 배정되므로 순차 실행 모델에서 안전하다. **단 multi-turn resume/retry 턴**은 노드를 새로 dispatch 하지 않고 `resumeState` 로 재진입하므로, `context.nodeExecutionId`·`workflowId` 는 fresh 배정이 아니라 `buildRetryReentryState` 가 대기/재시도 NodeExecution row(§1.3 식별 필드)에서 재유도한다 — 이 재유도가 빠지면 위 `logUsage` attribution 이 누락된다(§1.3 불변식, #501).
...
1. **새 Execution 시작 거부**. 새 Execution 을 시작하는 HTTP 진입점(`POST /api/workflows/:id/execute`, 단일 노드 `POST /api/workflows/:id/nodes/:nodeId/execute`)이 **503 Service Unavailable** 응답. response body 는 표준 API 에러 shape (`{ error: { code: 'SERVER_SHUTTING_DOWN', message: '...' } }`, [Spec API 규약](./2-api-convention.md)), `Retry-After: <ceil(SIGTERM_GRACE_MS / 1000)>` 헤더 동봉. LB drain 동안 traffic 이 다른 인스턴스로 라우팅.
   ...
   > **구현 범위**: HTTP 진입점(`POST /api/workflows/:id/execute` + 단일 노드 `POST /api/workflows/:id/nodes/:nodeId/execute`) gate 가 구현됨 — 실행 시작은 **REST 전용**이라 이 두 진입점으로 시작 gate 가 완결된다. WS `execution.start` 는 [6-websocket-protocol §4.2](./6-websocket-protocol.md#42-실행-제어-명령-client--server) 에서 **비채택(won't-do)** 으로 종결됐다(REST 대체·핸들러 부재, 근거 §Rationale `R-wontdo-rawws-rest`) — 실행 시작이 REST 전용이라 위 두 진입점 gate 로 완결되며 WS 시작 경로 확장 계획은 없다.
...
publish 전에 검사한다(위 §7.5.1 표 "표면(interactionType) 불일치" 행 — F-1 이 그 앞에 "nodeId 불일치" 행을 추가해 행 번호가 밀렸다).
...
- **선례 일반화**: 이미 `_retryState`(위 R1)가 **동일한 일** — `_resumeState` 의 credential-strip 부분집합을 평문으로 `NodeExecution.outputData` 에 영속 + 재개 시 재유도 — 을 retryable error 경로에서 검증된 채로 수행한다. `_resumeCheckpoint` 는 이 선례를 **상시(매 waiting turn) 경로로 일반화**한 것이다. credential / context-binding 필드는 동일하게 미동봉(~~`maskSensitiveFields` 와 동일 allow-list 정책~~ **`buildRetryState`/`buildResumeState` 가 옮길 키를 열거하는 allow-list**; 2026-08-24 정정 — `maskSensitiveFields` 는 allow-list 가 아니라 **키 이름 목록 마스커**였고 그 boundary 도 제거됐다. 정책 적용 경로는 §5.1)하고 재개 시 **두 채널**로 재유도한다 — 조작 필드(`llmConfigId`/`presentationTools`/`conditions`/`maxTurns` 등)는 `node.config` 재평가로, 식별 필드(`workflowId`/`nodeExecutionId`/`workspaceId`)는 호출측 컨텍스트에서 (§1.3 두 재유도 채널 + 아래 "resume/retry 턴 usage-log attribution" addendum).
...
### resume/retry 턴 usage-log attribution — 식별 필드 재유도 불변식 (#501, 2026-07)

**배경(회귀 #501)**: 위 `_resumeCheckpoint`/`_retryState` 의 credential-strip allow-list(`CREDENTIAL_CONTEXT_FIELDS`)는 `workflowId`·`nodeExecutionId` 를 **영속 대상에서 제거**한다 (context-binding 이라 checkpoint 에 담지 않음). 그런데 이 둘은 provider-tool(cafe24/makeshop/mcp)의 `logUsage` 게이트 `if (ctx.nodeExecutionId && ctx.workflowId)` 의 전제이므로, 재구성(`buildRetryReentryState`) 시 재주입되지 않으면 multi-turn **resume/retry 턴**의 통합 사용 로그가 조용히 누락됐다. 바로 위 `_resumeCheckpoint` 보존 항의 "`node.config` 에서 재유도" 단순화가 이 갭을 가렸다 — 실제로 `node.config` 재평가로 재유도되는 것은 **조작 필드**(`llmConfigId`/`maxTurns` 등)뿐이고, **식별 필드**(`workflowId` ← `execution.workflowId`, `nodeExecutionId` ← 재개 대상 NodeExecution row PK, opts 주입, `workspaceId` ← 실행 컨텍스트)는 **호출측 컨텍스트**에서 재유도돼야 한다 (§1.3 두 재유도 채널).

**결정**: `buildRetryReentryState` 가 `workflowId`(execution)·`nodeExecutionId`(opts)를 재구성 resumeState 에 재주입하고, §1.3 체크포인트 항에 **불변식**으로, §10.3 호출 순서에 재개-턴 각주로 명문화한다. 코드는 이미 정합(재구성 재유도 PR #877 — 회귀 복원, `llm_usage_log` 소비 사이트 교정 PR #879)이므로 본 addendum·§1.3·§10.3 갱신은 순수 문서 정합화다. 현재 이 불변식은 회귀 테스트(`buildRetryReentryState re-injects workflowId + nodeExecutionId (#501)`)로만 강제되므로, `CREDENTIAL_CONTEXT_FIELDS`/`resumeStateSchema` 를 리팩터할 때 이 spec 불변식을 근거로 두 식별 필드의 재주입을 반드시 보존한다.

**적용 범위 (재구성 vs. 소비)**: 본 불변식은 `buildRetryReentryState` **재구성** 계층의 보장이다 — resumeState 가 `ai_agent`·`information_extractor` 두 노드 모두에 대해 `workflowId`·`nodeExecutionId` 를 실어 나른다(#877). **소비** 계층의 sink 는 노드 유형별로 다르다: `ai_agent` 재개는 provider-tool(cafe24/makeshop/mcp)의 `IntegrationUsageLog`(게이트드 — 식별 필드 부재 시 로그 자체가 drop)에, 두 노드의 LLM 호출은 공통으로 `llm_usage_log`(`LlmUsageLogService.record`, **ungated** `?? null` 폴백)에 attribution 을 남긴다. `information_extractor` 는 provider-tool 을 보유하지 않아(유일 도구 `finalize_extraction`) 유일 sink 가 `llm_usage_log` 이며, 소비 사이트 교정 이전에는 재개 `llmContext` 가 `nodeExecutionId=state.nodeId`(노드 정의 id)·`workflowId` 누락으로 이 로그의 `node_execution_id`/`workflow_id` attribution 을 **오염**시켰다(로그 drop 이 아니라 오귀속). 이 IE 소비 교정과 `ai_agent` 메인 chat 재개의 유사 `llm_usage_log` NULL 누락 교정은 **PR #879**(`task_1543860b`)로 함께 완료됐고 회귀 테스트로 pin 됐다 — 따라서 `ai_agent`·`information_extractor` 모두 §1.3 두 재유도 채널을 정합 소비한다.

**기각된 대안 — 재개 식별 필드 hydration 전용 헬퍼**: 두 노드(`ai-turn-executor` / `information-extractor.handler`)의 재개 `llmContext` 조립부를 공용 `ResumeIdentificationFields` 타입 + `pickResumeIdentificationFields()` 헬퍼로 추출하는 안이 검토됐으나(오탈자 컴파일 타임 차단 목적) **채택하지 않는다**. 근거:
- **(1) 오탈자-안전 목적은 이미 달성됐다** — `ai_agent` 는 **PR #907(B4)**이 `narrowResumeState(state)`(ai-turn-executor 전용 메서드)로 `state.X` raw 캐스트를 `resumeStateSchema` 파생 `ResumeState` 로 좁혀 소스측 필드명을 컴파일 타임 검증하고, 소비측은 `LlmCallContext` 명시 타입 주석이 대칭 보강한다(**PR #900**, B1). `information_extractor` 는 이와 별개로 자체 손수 유지 `MultiTurnState` interface(**PR #879**)가 재개 state 필드를 타입 강제한다.
- **(2) 두 노드의 타이핑 구조가 이미 다르다** — `ai_agent` 는 `resumeStateSchema`/`ResumeState`, `information_extractor` 는 `MultiTurnState`(`resumeStateSchema` 미참조)로 각자 SoT 가 다르다. 공용 hydration 헬퍼는 이 이질 구조 위에 억지 공통 타입을 얹는 셈이라 오히려 표면이 는다.
- **(3) 세 재개-hydration 사이트의 shape 이 실제로 다르다** — `ai-turn-executor` 메인 chat(3필드), 같은 파일 provider-tool 배치(5필드 + `?? ''` fallback + `workspaceId`), IE(3필드, 조건부 `undefined`) — 단일 헬퍼로 묶으면 `?? ''` 의미나 조건부 undefined 를 파라미터로 되살려야 해 이득이 marginal 하다.
- **(4) churn 리스크** — 방금 안정화된(#877/#879/#900/#907) attribution 코드에 대한 churn 이 marginal 이득을 상회한다.

`ai_agent` 측 필드 목록의 단일 진실은 `resumeStateSchema`/`CREDENTIAL_CONTEXT_FIELDS` 가 보유하며(IE 는 `MultiTurnState`), 위 불변식이 두 식별 필드의 재주입을 spec 으로 강제한다.
...
### orphan pending backstop — recoverStuckExecutions 재사용 + PENDING cancel (2026-07-04)

§8 admission gate 의 cap 초과 delayed 재큐 job 은 auto-id(jobId 없음, §7.1)라 Redis 비영속·eviction 으로 **소실되면 재-pick up 될 job 이 없어** 그 `pending` row 가 영구 잔류한다. 큐 대기 timeout 은 consumer 가 job 을 pick up 할 때만 검사(§8)되므로 orphan 은 그 검사를 받지 못한다. 이 잔여 갭을 부팅 backstop 으로 닫는다(`recoverOrphanPendingExecutions`).

- **같은 함수·트리거 재사용(별도 스캐너·주기 tick 없음)**: `recoverStuckExecutions`(§7.4, `onApplicationBootstrap` 1회 + 전역 lock)에 pending 스캔을 더한다. §7.1/§7.4 가 반복 확정한 "heartbeat → stalled-job 일원화, 신규 주기 스캐너 미도입" 원칙 유지 — orphan pending 도 boot-only best-effort(낮은 확률 엣지). 별도 lock·env·migration 없음.
  - **원칙의 스코프 — job-backed 엔진 recovery 한정**: 이 "신규 주기 스캐너 미도입" 원칙은 **BullMQ job 이 존재하는 RUNNING/pending 의 엔진 liveness recovery** 에 스코프된다(stalled 재배달이 본령이라 주기 스캔이 불요). `waiting_for_input`(park)의 client-abandonment 회수([EIA-RL-07](./14-external-interaction-api.md#34-신뢰성일관성) — 공개 위젯 토큰 영구 만료)는 이 원칙과 **무관**하다: park 는 정의상 BullMQ job 이 없어 엔진 recovery 대상이 아니며, 그 회수는 엔진 스캐너가 아니라 **EIA 계층의 token-lifecycle repeatable sweep**(EIA-RL-06 형제, `execution_token` 만료 기반)이 담당한다. 즉 원칙의 번복/예외 신설이 아니라 원칙이 커버하지 않는 별개 계층·별개 신호의 작업이다.
- **PENDING 은 cancel, RUNNING 은 re-drive(상태별 상이)**: RUNNING stale 은 부분 완료 노드라는 **진행 흔적**이 있어 §7.5 case B 로 재구동(무손실)한다. PENDING 은 아직 첫 세그먼트도 시작하지 않아 재구동할 흔적이 없고 큐 대기 한도를 **이미 초과**했으므로, §8 이 정의한 wait-timeout `cancelled`(`EXECUTION_QUEUE_WAIT_TIMEOUT`)를 그대로 적용한다. 이는 consumer 가 pick up 했을 때의 결과와 동일 — re-enqueue→재검사→cancel 과 등가라 직접 cancel 이 단순·안전하다(오래된 실행을 RUNNING 으로 부활시키지 않는다).
- **멱등·race 안전**: 대상은 `queued_at < now − EXECUTION_QUEUE_WAIT_TIMEOUT_MS` 인 pending 만(V104 이전 `queued_at IS NULL` 은 자연 제외). 회수는 기존 `markQueueWaitTimeout`(조건부 UPDATE `WHERE status='pending'`) 재사용 — 동시에 consumer 가 admit(pending→running)하거나 cancel 해도 affected=0 no-op 로 수렴. admission gate(pending→running)와 backstop(pending→cancelled)은 상호배타 status 전이라 이중 처리 없음.
- **한도 이내 orphan**: boot-only 라 아직 timeout 전인 orphan 은 다음 부팅 스캔에서 초과 시 회수한다. 운영 중 on-demand recovery 트리거는 PR4 관측성으로 별도 검토(§7.4).
...
- **추출 서비스** (전부 `codebase/backend/src/modules/execution-engine/` 내 — 엔진 spec `code:` glob 자동 커버):
  ...
  - **엔진 잔류**: registry(`resume-turn-dispatch`)·dispatch-loop·EngineDriver 멤버 집합(아래 ISP 계층의 distinct 멤버 — C-1 후속 ④ 시점 **12개**: `updateExecutionStatus`·`contextKeyOf`·`stageDurableResumeSnapshot`·`buildRetryReentryState`·`buildResumeCheckpoint`·`isCheckpointEligibleNodeType`·`applyPortSelection`·`rehydrateContext`·`loadAndBuildGraph`·`runNodeDispatchLoop`·`findActivatedBackEdge`·`clearLlmDefaultConfigCache`. 이후 취소 가드 배선으로 **15개**가 됐다 — 아래 참조). `retryLastTurn`·`applyRetryLastTurn` 은 더 이상 엔진에 잔류하지 않는다(후속 ④).
- **엔진 내부 통신 = `EngineDriver`** (token `ENGINE_DRIVER`, `useExisting: ExecutionEngineService`): 추출 서비스가 엔진의 잔류 메서드를 호출하는 **엔진 내부 전용 계약**이다. **in-process 전제** — 분산 분리가 아니라 같은 프로세스 안의 클래스 경계 정리다. 후속 ④에서 이 단일 12-멤버 계약을 **ISP 부분 인터페이스 계층**으로 코드 분해했다(`engine-driver.interface.ts`): `CoreEngineDriver`(`updateExecutionStatus`·`contextKeyOf`) ⊂ `InteractionEngineDriver`(+`stageDurableResumeSnapshot`, Form/Button 소비); `ReentryStateDriver`(`buildRetryReentryState`, AiTurn·Retry 공유); `AiTurnEngineDriver`(= Interaction+Reentry+`buildResumeCheckpoint`·`isCheckpointEligibleNodeType`·`applyPortSelection`, 7멤버, AiTurnOrchestrator 소비); `RetryEngineDriver`(= Core+Reentry+`rehydrateContext`·`loadAndBuildGraph`·`runNodeDispatchLoop`·`findActivatedBackEdge`·`clearLlmDefaultConfigCache`, 8멤버 — 신규 5멤버 `@internal`, RetryTurnService 소비); `EngineDriver`(= AiTurn+Retry, 당시 **12 distinct**, 엔진만 implements). 각 소비자의 driver 필드를 slice 로 narrow 해 ISP 컴파일 계약을 실현한다

  > **멤버 수 갱신 (2026-07-27)**: 취소 가드 배선으로 `AiTurnEngineDriver` 에 3개가 추가돼
  > (`assertExecutionNotCancelled` · `markNodeCancelled` · `tryLockActiveExecutionAndSaveNodeExec`)
  > 현재는 **`EngineDriver` distinct 15 / `AiTurnEngineDriver` 합계 10** 이다. 세 멤버 모두
  > `AiTurnOrchestrator` 전용이라 공유 기반 `CoreEngineDriver` 가 아닌 그 표면에 둔다(ISP).
  > 정확한 표면은 항상 `engine-driver.interface.ts` 의 타입 정의가 SoT 다 — 이 문단의 수치는
  > 분해 이력의 스냅샷이며, 라운드마다 stale 해지는 것이 관측돼 "코드가 SoT" 를 명시해 둔다 (이는 **코드 레벨 ISP 분해**이며 spec SoT 는 여전히 단일 `EngineDriver` 엔진-내부 계약 하나다). `WorkflowExecutor`(spec 상 engine↔**노드** 계약) **재사용은 기각** — 엔진 내부 통신에 재사용하면 그 계약의 의미가 과적된다. bootstrap 의 엔진 자기참조(`handlerDeps.build(this)`)는 별도 `WORKFLOW_EXECUTOR` 토큰화로 해소했다(이쪽은 `WorkflowExecutor` 계약의 정확한 용처라 의미 보존 — m-3, `nodes→engine` 역의존 제거).
