...
> **구현 메모 — "첫 세그먼트 배리어"(`firstSegmentBarriers`)**: 위 "BLOCK 진입 시 job 정상 ack" 는 `execution-engine.service.ts` 의 단발 Promise 배리어로 달성한다. `runExecutionFromQueue`(worker `process()` 진입점)는 `runExecution` 을 직접 await 하지 않고 **detach** 한 뒤, `armFirstSegmentBarrier(executionId)` 가 심은 배리어를 await 한다. 배리어는 **첫 세그먼트가 park 또는 terminal 에 정착하는 순간** `settleFirstSegment` 로 resolve 된다 — park 측은 각 `waitForX`(form/button/AI 멀티턴)의 pending 등록 직전 `signalParkBarrier(context)`, terminal 측은 `runExecution` finally. 둘 중 먼저 도달한 쪽이 worker 를 깨워 job 을 ack/반환하므로, park 동안 BullMQ 슬롯이 점유되지 않는다. `settleFirstSegment` 는 멱등(`settled` 플래그)·단발(즉시 Map 삭제)이고, background subgraph(`bg:` context key)는 main flow 배리어를 건드리지 않는다(`signalParkBarrier` no-op).
>
> **현재 재개 경로와 알려진 한계**: park 후 `runExecution` 코루틴은 in-process 로 살아 있어, 같은 프로세스가 재개를 받으면 무손실 fast-path(in-memory `pendingContinuations` resolve)로 이어진다. 프로세스 재시작·다른 인스턴스 재개는 §7.5 rehydration slow-path 로 처리되며, 이때 `conversationThread`/멀티턴 AI `_resumeState` 등 in-memory 전용 상태는 durable checkpoint 한도 내에서만 복원된다(`RESUME_CHECKPOINT_MISSING`/`RESUME_INCOMPATIBLE_STATE` 종결 가능 — §7.5). 유저 입력 시점이 불확실해 park 코루틴이 누적되면 메모리 압박 위험이 있으므로, **"park 즉시 코루틴 해제 + slow-path 일원화 + resume 상태 durable 영속"** 으로의 전환이 검토 대상이다(추적: `plan/in-progress/execution-engine-residual-gaps.md`).
