...
>
> **현재 재개 경로와 알려진 한계**: park 후 `runExecution` 코루틴은 in-process 로 살아 있어, 같은 프로세스가 재개를 받으면 무손실 fast-path(in-memory `pendingContinuations` resolve)로 이어진다. 프로세스 재시작·다른 인스턴스 재개는 §7.5 rehydration slow-path 로 처리된다. `conversationThread`(요약 보관 필드 포함)는 **park 시 `Execution.conversation_thread`(V084)에 durable commit 되어 rehydration 이 무손실 복원**한다(§6.2/§7.5, [conversation-thread §4·§8.4](../conventions/conversation-thread.md#84-executionconversation_thread-컬럼-채택--durable-park-resume)). 멀티턴 AI `_resumeState` 는 여전히 `_resumeCheckpoint` 한도 내 복원이라 일부 케이스에서 `RESUME_INCOMPATIBLE_STATE`/`RESUME_CHECKPOINT_MISSING` 종결 가능(§7.5). 한편 유저 입력 시점이 불확실해 park 코루틴이 누적되면 메모리 압박 위험이 있으므로 **"park 즉시 코루틴 해제 + slow-path 일원화"** 로의 전환을 추진한다 — 그 전제인 **resume 상태 durable 영속(conversationThread)은 Phase A1 에서 구현 완료**, park 해제+slow-path 일원화는 Phase B(추적: `plan/in-progress/exec-park-durable-resume.md`).
