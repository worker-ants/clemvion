...
- 누적 active-running 시간 초과 → **`EXECUTION_TIME_LIMIT_EXCEEDED`** 에러 → Execution.status = `failed` (엔진 레벨 누적 타임아웃 전용 코드. Code 노드 스크립트 타임아웃 `EXECUTION_TIMEOUT` 과 의미가 달라 코드 분리 — [§3-error-handling §1.4](./3-error-handling.md#14-워크플로우-실행-에러)) — **PR2a 구현 완료**
