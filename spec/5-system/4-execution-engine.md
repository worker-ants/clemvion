...
신규 client-safe 코드는 신규 prefix 를 만들지 않고 중앙 `ErrorCode` enum 의 기존 `EXECUTION_*` 네임스페이스를 확장한다 (예: `EXECUTION_INTERNAL_ERROR` generic fallback, `EXECUTION_MESSAGE_TOO_LONG` 메시지 길이 초과). `INVALID_EXECUTION_STATE`(prefix 없는 시스템 레벨 코드)·worker 측 `RESUME_*`·retry `RETRY_*` 는 안정성 정책상 기존 이름을 유지한다([conventions/error-codes.md](../conventions/error-codes.md)).
