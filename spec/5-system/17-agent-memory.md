...
> **비대칭 입력(inputType)**: 회수는 `LlmService.embed(..., inputType:'query')`, 저장(§3 `saveMemories`)은 `inputType:'document'` 로 호출한다 — 같은 **모델**이되 비대칭 검색 모델(e5/Gemini, [Spec 임베딩 파이프라인 §5.4](./8-embedding-pipeline.md))에서 query/passage 를 올바르게 구분하기 위함이다. 단 KB 와 달리 `agent_memory` 에는 일괄 재임베딩 경로가 없으므로, inputType 배선 **이전에 비대칭 모델로 저장된 기존 메모리**는 무접두사 상태로 남아 회수 시 약한 비대칭이 생길 수 있다 — dedup UPDATE(AGM-09) 재저장 또는 TTL 만료로 자연 해소되며, 대칭 모델(기본 `text-embedding-3-small`)에서는 no-op 이라 영향이 없다.
