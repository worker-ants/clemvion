...
**인덱스**: `(workspace_id, scope_key)` (스코프별 회수·evict) + pgvector partial 인덱스(`embedding`, `DocumentChunk` 와 동일 차원별 partial HNSW/IVFFlat 정책) + `expires_at` partial 인덱스(`WHERE expires_at IS NOT NULL` — TTL evict 만료 스캔 가속, V079). [데이터 모델 §3](../1-data-model.md#3-인덱스-전략).
