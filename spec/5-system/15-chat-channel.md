...
| **비활성 trigger (chatChannel 경로)** | **`410 Gone`** (미구현 — Planned: `202 Accepted` + `{ executionId: 'ignored' }`) | 표준 에러 envelope (`TRIGGER_INACTIVE`) | **미구현 (Planned)**: 현재 `HooksService.handle` 의 `!trigger.isActive` 검사 ([`hooks.service.ts:92`](../../codebase/backend/src/modules/hooks/hooks.service.ts)) 가 `config.chatChannel` 분기 ([`:104`](../../codebase/backend/src/modules/hooks/hooks.service.ts)) 보다 먼저 실행되어 비활성 chatChannel 트리거도 410 Gone 을 반환한다. WH-EP-07 의 chat-channel 예외 (202 silent skip) + R-CC-12 (d) 의 비활성 트리거 인증 수행은 아직 미구현 |
...
> ⚠️ **현 구현 caveat (비활성 trigger 케이스만)**: 본 Rationale 의 `202 Accepted` 고정 + auth 401 정책은 **설계 계약 (designed contract)** 이며, *정상 update / auth 실패 / 404* 케이스에는 현재 구현이 그대로 따른다. 단 **비활성 chatChannel 트리거** 만은 미구현이다 — `HooksService.handle` 의 `!trigger.isActive` 검사 ([`hooks.service.ts:92`](../../codebase/backend/src/modules/hooks/hooks.service.ts) `throw GoneException`) 가 `config.chatChannel` 분기 ([`:104`](../../codebase/backend/src/modules/hooks/hooks.service.ts)) 보다 선행하므로, 현 구현은 비활성 chatChannel 트리거도 **410 (`TRIGGER_INACTIVE`)** 을 반환한다. 따라서 §5.5 표의 "비활성 trigger (chatChannel 경로)" 행의 `202 + { ignored: true }` 와 (d) 의 "비활성 트리거도 인증 수행" 은 아직 **미구현 약속 (Planned)** — 구현하려면 is_active 검사를 chatChannel 분기 이후로 옮겨야 한다. 12-webhook.md WH-EP-07 caveat 과 단일 진실 동기화.
