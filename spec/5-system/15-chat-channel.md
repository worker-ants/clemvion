...
| CCH-SE-02 | 어댑터가 provider update id(텔레그램 `update_id` 등)를 `ChannelUpdate.idempotencyKey` 로 파싱하고, **inbound 진입에서 동일 키 30초 안 재도착을 무시**한다 (`ChatChannelDedupService`, Redis `SET NX EX 30`, 키 `cc:dedup:<triggerId>:<idempotencyKey>`). Redis 미가용 시 fail-open(+warn) | 필수 |
