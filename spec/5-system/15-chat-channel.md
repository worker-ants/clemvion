...
| CCH-MP-06 | 비-blocking presentation 노드 (`template` body, `carousel`/`table`/`chart` 의 buttons 없음 케이스) 의 `execution.node.completed` (CCH-AD-07 listener) → 채널 메시지로 변환. v1 fallback 정책은 `CCH-MP-04` (텔레그램 [§5.4](../4-nodes/7-trigger/providers/telegram.md#54-carousel--chart--table-cch-mp-04) MarkdownV2 fallback) 와 동일 — 시각형 (carousel/table/chart) 은 `uiMapping.visualNode` enum 분기 (`text` / `photo` / `auto`) 적용, `template` 은 `output.output.rendered` 텍스트 그대로(**wire `output` 은 `NodeHandlerOutput` 래퍼 전체**라 도메인 값이 한 겹 아래다 — ~~`output.rendered`~~, [WS §4.1](./6-websocket-protocol.md) 2026-08-24 정정. 렌더러는 legacy flat fallback 도 함께 훑는다). **단 "그대로" 는 emit 계층의 값-패턴 마스킹 _이후_ 값을 뜻한다 (2026-08-16)** — `execution.node.completed` payload 는 emit 시점에 자격증명 패턴이 `***` 로 치환되므로 ([WS §4.1](./6-websocket-protocol.md) 값-패턴 마스킹 캐비엇 / [EIA §R17](./14-external-interaction-api.md)), 그 텍스트에 `Bearer …` 같은 패턴이 있었다면 채널 최종 사용자에게도 마스킹된 채로 보인다. 어댑터가 별도 가공을 하지 않는다는 뜻이지 DB 원문과 byte-동일하다는 뜻이 아니다. 같은 판단이 `CCH-MP-01`(AI `render_*` presentations)·`CCH-MP-04`(시각형)에도 동일하게 적용되며, §R17 의 `execution.ai_message` 불릿이 *"내부 WS·Chat Channel 도 마스킹됨 — 수용된 trade-off"* 로 이미 세운 선례와 같은 방향이다. 본 룰은 EIA outbound HTTP notification 표면 (5종 화이트리스트, §6.1) 을 확장하지 않음 — chat-channel-internal listener 한정. SoT: [Convention chat-channel-adapter §1.3](../conventions/chat-channel-adapter.md#13-chatchannelinternalevent-입력) (`ChatChannelInternalEvent`) + [§R-CCA-7](../conventions/chat-channel-adapter.md#r-cca-7-rendernode-시그니처-union-확장--chat-channel-internal-이벤트-수용). | 필수 |

#### 3.4 신뢰성 / 보안

| ID | 요구사항 | 우선순위 |
|----|---------|---------|
| CCH-SE-01 | 어댑터의 외부 API 호출 (sendMessage 등) 에 5초 타임아웃 + 3회 지수 백오프 재시도. 최종 실패 시 trigger 의 `chat_channel_health` 를 `degraded` 로 갱신 ([§4.2](#42-trigger-테이블-신규-컬럼)). 자동 비활성화 금지 ([WH-MG-04 / EIA-NX-07](./12-webhook.md#관리) 와 동일 정책). **`degraded` 설정 경로는 본 CCH-SE-01(외부 API 호출 실패) 외에 [CCH-NF-03](#36-비기능-요구사항)(per-chat rate-limit 초과)도 포함** — 두 경로 모두 동일 health 자원·자동 비활성화 금지 (R-CC-19) | 필수 |
| CCH-SE-02 | 어댑터가 provider update id(텔레그램 `update_id` 등)를 `ChannelUpdate.idempotencyKey` 로 파싱하고, **inbound 진입에서 동일 키 30초 안 재도착을 무시**한다 (`ChatChannelDedupService`). Redis 미가용 시 fail-open — 그 구간엔 중복 처리가 가능하다. 키·TTL·게이트 순서 상세는 [data-flow/14 §2.2](../data-flow/14-chat-channel.md#22-redis) (SoT) | 필수 |
...
어댑터의 outbound subscription 은 실행 엔진 §4.4 의 **단일 sink `WebsocketService.executionEvents$` (RxJS Subject)** 에 `ChatChannelDispatcher` 가 `onModuleInit` 에서 직접 subscribe 하는 방식이다 — NotificationDispatcher 와 **형제 listener** (downstream 아님). 같은 process 안에서는 Redis pub/sub 을 우회한다. 외부 HTTP notification 와 채널 emit 은 같은 단일 sink 에서 commit 후 fan-out 되어 둘 다 [EIA-RL-04](./14-external-interaction-api.md#34-신뢰성일관성) (TX commit 후 발송) 와 정합. 어댑터가 엔진 내부 코드를 호출하지 않음 — facade 원칙 유지. (구독 대상의 코드 SoT 는 [`chat-channel.dispatcher.ts`](../../codebase/backend/src/modules/chat-channel/chat-channel.dispatcher.ts) — R8 §v1 구조 참조.)
...
```jsonc
{
  "chatChannel": {
    ...
    "languageHints": {                          // 봇이 보내는 자체 안내 메시지 i18n (사용자 override). 미설정 키는 languageLocale 의 default 문구 사용.
      ...
      "formValidationFailed":          "입력값을 다시 확인해주세요.",                        // Form 다단계 client-side 검증 실패 재질문 (평문)
      "formNextField":                 "다음 항목을 입력해주세요.",                          // Form 다단계 다음 필드 prompt (평문)
      ...
      "surfaceMismatch":               "지금은 이 입력을 받을 수 없어요 화면에 표시된 양식이나 버튼을 사용해 주세요", // 대기 노드 표면과 명령 불일치(409 STATE_MISMATCH) 시 안내 (§4.1.1). 평문 — 발송 시 어댑터 escape
...
| `surfaceMismatch` (대기 표면 ↔ 명령 불일치 — 409 `STATE_MISMATCH`) | "지금은 이 입력을 받을 수 없어요 화면에 표시된 양식이나 버튼을 사용해 주세요" | "This input can't be accepted here right now, please use the form or buttons shown above" |
...
`surfaceMismatch` 는 인바운드 명령이 현재 대기 노드의 인터랙션 표면과 맞지 않아 continuation publisher 가 409 `STATE_MISMATCH` 로 거부했을 때 (예: `form`/`buttons` 대기 중 자유 텍스트 도착 — 고정 매핑 `text_message → submit_message`) 사용자에게 발송하는 best-effort 안내다 ([실행 엔진 §7.5.1](4-execution-engine.md#751-publisher-측-사전-검증--invalid_execution_state) 표면 매트릭스). CCH-ERR-04 의 "silently swallow 금지" 관례상, 종전에 로그만 남기던 조합에 사용자 피드백을 준다 — 구현은 `HooksService.forwardToInteractionService` 가 거부를 삼킬 때 `sendSurfaceMismatchNotice` 로 발송. lookup 경로는 `sessionExpired` 와 동일.

**control-plane 안내의 발송 시 per-provider escape** — `HooksService` 가 렌더러(`renderNode`)를 거치지 않고 `adapter.sendMessage` 로 **직접** 발송하는 control-plane 키(`help` · `groupChatRefusal` · `unsupportedMessageKind` · `executionStillRunning` · `surfaceMismatch` · `formValidationFailed` · `formNextField`)는 발송 직전 `adapter.escapeControlText(text)` 로 provider 표면에 맞게 escape 한다. 이로써 default·operator override 모두 세 provider 에서 올바르게 렌더된다:

- **telegram** → `escapeMarkdownV2` (예약문자 backslash escape — `renderNode` 경로와 동일 규칙)
- **slack** → `escapeSlackMrkdwn` (`<`, `>`, `&` 만)
- **discord** → 평문 그대로 (identity — 렌더러도 escape 안 함)

따라서 **default 문구와 operator override 는 모두 평문으로 작성**한다 — 수동 escape (`\.` 등) 불필요. 어댑터가 provider 별로 escape 하므로 telegram 에서도 마침표·하이픈 등이 올바르게 렌더되고, slack/discord 에서 telegram escape 시퀀스가 literal 로 노출되던 종전 갭이 해소된다. SoT: [`conventions/chat-channel-adapter.md`](../conventions/chat-channel-adapter.md) `escapeControlText`. (`formOpenLabel` 은 버튼 라벨이라 MarkdownV2 파싱 대상이 아니고, `sessionExpired`·CCH-ERR-* 는 `renderNode` 경로라 렌더러가 escape 한다 — 모두 이 직접 발송 escape 대상 밖.)
...
**성공 응답 (200 OK)** — [API Convention §5.1](./2-api-convention.md) 의 `{ data }` 래퍼 + `TransformInterceptor` 적용. `rotateBotToken` ([`triggers.service.ts`](../../codebase/backend/src/modules/triggers/triggers.service.ts)) 은 `rotatedAt` 외에 `triggerId` / `chatChannelHealth` (setupChannel 재호출 결과, 성공 시 `'healthy'`) / `botIdentity` (getMe 캐시 갱신 결과 — `setupChannel` 의 `configUpdates.botIdentity`, 미반환 provider 는 `null`) 3필드를 동봉한다:
```jsonc
{
  "data": {
    "rotatedAt": "<ISO8601>",
...
> **`details.field` 의 SoT 는 단일하지 않다 — 그것이 확정 설계다 (2026-09-11).**
> DTO 선언(전역 파이프)과 서비스 가드는 **서로 다른 입력**을 받는다(비어있지 않은 값 vs
> `null`/`''`). 하나를 SoT 로 고르면 **다른 입력에서 문서가 거짓**이 되므로, 갈린다는
> 사실 자체를 적는다. 근거: `trigger-dto-validation.spec.ts` 의 `[실측]` 2건.
...
**`chatChannel` 필드 존재성과 `provider` 불변성은 이 표의 주제가 아니다** — [§5.4.1.2](#5412-chatchannel-필드-존재성--provider-불변성) 가 SoT 다.
...
#### 5.4.1.2 `chatChannel` 필드 존재성 · `provider` 불변성

**위 §5.4.1 표는 bot token 축만 다룬다.** PATCH 가 거부하는 나머지 두 축은 여기 적는다 —
표에 행으로 끼우면 *"Bot token 의 신규 등록·변경"* 이라는 그 표의 주제와 어긋난다.

| 시도 | 응답 | 왜 |
|---|---|---|
| `chatChannel` 이 **없는** 트리거에 PATCH 로 **사후 부착** | 400 `VALIDATION_ERROR` (`details.field='chatChannel'`) | 최초 설정은 **생성 `POST /api/triggers` 한정**(§5.4.1 표 1행). PATCH 는 비밀을 받지 않으므로([R-CC-21](#r-cc-21-patch-는-비밀을-쓰지-않는다--차단이-필드명-층에만-걸려-있었다)) 채널을 새로 붙일 방법이 없다 |
| `provider` 를 **다른 값으로** 변경 | 400 `VALIDATION_ERROR` (`details.field='provider'`) | `botTokenRef` 는 trigger id 에서만 재유도되므로 그 ref 뒤의 평문은 **여전히 옛 provider 의 토큰**이다 — 새 adapter 가 그것으로 외부 API 를 때린다. [`2-trigger-list.md` R-12](../2-navigation/2-trigger-list.md#r-12-chat-channel-provider--inboundsigning-필드-정책) 도 *"변경하려면 트리거 삭제·재생성"* 이라 적는다 |

두 항목은 **[R-CC-21](#r-cc-21-patch-는-비밀을-쓰지-않는다--차단이-필드명-층에만-걸려-있었다) 의 필연적 귀결**이라 새 결정이 아니다 — PATCH 가 비밀을 받지 않게
되면서 자동으로 따라온 제약이고, 그것을 문서에 적지 않은 것이 갭이었다.

> **사후 부착 차단이 없었다면** `setupChannel` 이 반드시 실패하고 그 실패가 CCH-SE-01 의
> best-effort catch 에 삼켜져 **`chatChannelHealth='degraded'` 로 200** 이 됐다 — 사용자에게는
> *"저장됐다"* 로 보이고 봇은 죽어 있다. 명시적 400 은 그 조용한 실패를 막는다.

**top-level code 는 기존 `VALIDATION_ERROR` 재사용**이라
[§1 카탈로그](./3-error-handling.md) 신규 등재는 필요 없다. `details[].code` 는 두 항목 모두
**서비스 가드 갈래**라 싣지 않는다(위 §5.4.1 의 두-갈래 서술 참조).
...
| private chat → 정상 update (기존 execution forwarding) | `202 Accepted` | `{ executionId }` (활성 execution id) | InteractionService.interact 호출 (새 execution 미생성) |
| group/supergroup/channel chat | `202 Accepted` | `{ executionId: 'ignored' }` | `languageHints.groupChatRefusal` 안내 sendMessage 발송 |
| `from.is_bot === true` (또는 Slack `bot_id` / Discord `member.user.bot === true`) | `202 Accepted` | `{ executionId: 'ignored' }` | silent skip |
| `parseUpdate` 미지원 update type | `202 Accepted` | `{ executionId: 'ignored' }` | silent skip ([12-webhook §7 step 7c](./12-webhook.md#7-처리-흐름) 와 동일) |
| **분당 rate-limit 초과 (per-chat)** | **`202 Accepted`** | `{ executionId: 'ignored' }` | per-chat fixed-window 카운트가 `rateLimitPerMinute`(기본 60) 초과 시 update 처리 생략(버퍼링/재발사 없음) + `chat_channel_health=degraded` 갱신 ([CCH-NF-03](#36-비기능-요구사항) / [R-CC-19](#r-cc-19-cch-nf-03-rate-limit--replay-큐-대신-skip--degraded)). 안내 sendMessage 는 v1 범위 밖 |
| **비활성 trigger (chatChannel 경로)** | **`202 Accepted`** | `{ executionId: 'ignored' }` | `HooksService.handle` 의 `config.chatChannel` 분기가 `!trigger.isActive` 검사보다 먼저 실행되며, `handleChatChannelWebhook` 은 inbound 서명 검증(`chatChannelInboundAuthenticator.verify`)을 **무조건 수행**한 뒤에야 `!trigger.isActive` 시 202 silent skip 으로 단락한다 (WH-EP-07 chat-channel 예외 + R-CC-12 (d)). 서명 실패 시 401 |
...
| 어댑터 내부 에러 (sendMessage 실패 등) | `202 Accepted` | `{ executionId: 'ignored' }` 또는 `{ executionId }` (실패 단계에 따라) | 백그라운드 처리, `chat_channel_health='degraded'` 갱신. 본 행의 정책은 **어댑터 외부 API 호출 실패** 영역 — execution 자체의 실패 안내 정책은 [§3.5 CCH-ERR-*](#35-실행-실패-사용자-안내-cch-err-) 참조 |
...
> **구현 메모 — "ignored" body 의 실제 shape**: "새 execution 미생성" 케이스의 본문은 별도 `{ ignored: true }` 객체가 아니라, `executionId` 필드에 sentinel 문자열 `'ignored'` 를 채운 `{ executionId: 'ignored' }` 형태다 ([`hooks.service.ts`](../../codebase/backend/src/modules/hooks/hooks.service.ts) 의 `handleChatChannelWebhook` 반환 — `state?.executionId ?? 'ignored'`). 기존 execution forwarding 케이스는 활성 execution 의 실제 id 를 그대로 반환한다.
...
| [EIA-RL-* 신뢰성](./14-external-interaction-api.md#34-신뢰성일관성) | 동일 적용. 특히 EIA-RL-03 (form 검증 실패 시 waiting_for_input 유지) 가 CCH-MP-03 의 "그 필드만 재질문" 의 기반 |
...
세부:
- (a) **`202` 고정 (200 대신) 이유**: [WH-RS-01](./12-webhook.md#비기능-요구사항) (202 Accepted 가 webhook 수신 응답 표준) 와 정합. 기존 SoT 채택.
...
> **구현 정합 (비활성 trigger 케이스)**: 본 Rationale 의 `202 Accepted` 고정 + auth 401 정책은 모든 케이스(정상 update / auth 실패 / 404 / 비활성 트리거)에서 구현과 일치한다. `HooksService.handle` 의 `config.chatChannel` 분기가 `!trigger.isActive` 검사보다 선행하며, `handleChatChannelWebhook` 은 `chatChannelInboundAuthenticator.verify` 로 inbound 서명을 무조건 검증한 뒤(실패 시 401) `!trigger.isActive` 일 때 `202 + { executionId: 'ignored' }` 로 silent skip 한다. 따라서 §5.5 표의 "비활성 trigger (chatChannel 경로)" 행과 (d) 의 "비활성 트리거도 인증 수행" 이 구현돼 있다. 12-webhook.md WH-EP-07 caveat 과 단일 진실 동기화.
...
**세부**:
- (a) **`renderNode` 인터페이스 변경 최소화**: 새 함수 추가 ([Convention R-CCA-5 대안 2 기각](../conventions/chat-channel-adapter.md#r-cca-5-execution-failed-분류-helper-를-convention-에-두는-이유)) 대신 시그니처 union 확장 — `renderNode(event: EiaEvent | ChatChannelInternalEvent)`. **이 결정 자체는 함수를 신설하지 않는다** (기존 함수 시그니처만 확장). SoT: [Convention §R-CCA-7](../conventions/chat-channel-adapter.md#r-cca-7-rendernode-시그니처-union-확장--chat-channel-internal-이벤트-수용). 이후 control-plane escape 근본 fix 에서 `escapeControlText` 필수 함수가 별도로 신설됐다 — provider 별 escape 규칙이 본질적으로 다른 데 따른 R-CCA-5 최소주의의 정당한 예외 ([Convention §1](../conventions/chat-channel-adapter.md#1-adapter-interface)).
...
- (c) **`render_form` 처리**: interactive form 의 native modal/Mini App 격상은 v2. v1 에서는 사용자에게 form 의 존재 자체를 알리지 못하면 메시지 0 도착이 되므로 **v1 임시 텍스트 fallback** (fields 목록 + 답변 안내) 으로 발화한다 ([R-CC-17](#r-cc-17-render_form-v1-임시-텍스트-fallback--presentation-renderer-shape-처리)). SoT: `chat-channel-adapter.md §3` 매핑 표.
...
[CCH-NF-03](#36-비기능-요구사항) 의 채널당 분당 inbound 한도 초과분을 **버퍼링 후 재발사(replay 큐)** 하지 않고 **처리 생략(skip) + `chat_channel_health=degraded`** 로 정의한다.

- **R9 와의 관계 (직교 사안)**: [R9](#r9-cch-cv-03-running-케이스의-큐잉-vs-즉시-안내) 는 rate-limit 큐를 *기각하지 않았다* — R9 는 lifecycle 케이스(`execution running 중 사용자 메시지`)와 rate-limit 케이스(`분당 60건 초과`)를 **다른 트리거 조건**으로 분리했을 뿐이다(전자=lifecycle 정합, 후자=외부 폭주 방어). 본 R-CC-19 는 그 **rate-limit 케이스 내부**에서 "초과분을 큐에 적재→재발사할지 vs skip 할지" 를 정하는 독립 결정이다.
- **skip 채택의 독립 근거**: (a) [WH-NF-01](./12-webhook.md#비기능-요구사항) 의 200ms 응답 시한상 inbound 를 동기적으로 버퍼링·보류할 수 없다 — 텔레그램은 webhook 응답 지연 시 retry 폭주(R-CC-12 (b)). (b) replay 버퍼는 input-sequence 가정 충돌·dedup 책임·TTL/순서 정렬 등 추가 메커니즘을 요구하는데(R9 가 lifecycle 맥락에서 정리한 동일 trade-off 가 rate-limit 맥락에도 성립), v1 의 단순성 대비 이득이 적다. 따라서 초과분은 skip(202 `{ executionId: 'ignored'}`, telegram-safe 2xx)하고, 운영 가시성은 `degraded` 로 확보한다.
- **fixed-window(sliding 아님) + Redis 선택**: 기존 [`PublicWebhookQuotaService`](../../codebase/backend/src/modules/hooks/public-webhook-quota.service.ts) 의 `INCR`+`EXPIRE` pipeline 과 동일 패턴을 **별도 per-chat 키**로 재사용 — 일관성·구현 단순성. Redis 기반이라 멀티 인스턴스에서도 전역 카운트가 정확하다(인스턴스-로컬 아님). 분 버킷 경계의 burst 허용은 fixed-window 의 표준 trade-off 로 v1 수용. Redis 미가용 시 **fail-open**(rate-limit 미적용) — rate-limit 은 방어적 기능이라 부재 시 차단보다 통과가 안전(기존 throttle 동일 정책).
- **degraded 의 두 경로 정합**: `chat_channel_health=degraded` 는 [CCH-SE-01](#34-신뢰성--보안)(어댑터 외부 API 호출 실패)과 본 CCH-NF-03(rate-limit 초과) 두 경로에서 설정되며, 둘 다 "채널 동작이 정상 범위를 벗어남" 을 운영자에게 알리는 동일 자원이다. 자동 비활성화 금지(CCH-SE-01)는 양쪽 공통.
...
- **채택**: 전용 `ChatChannelDedupService`(Redis `SET NX EX 30`, 키 `cc:dedup:{triggerId}:{idempotencyKey}`). `SET NX` 단일 호출이라 원자적이고, 두 인스턴스가 같은 재전송을 동시에 받아도 하나만 통과한다.
