...
`surfaceMismatch` 는 인바운드 명령이 현재 대기 노드의 인터랙션 표면과 맞지 않아 continuation publisher 가 409 `STATE_MISMATCH` 로 거부했을 때 (예: `form`/`buttons` 대기 중 자유 텍스트 도착 — 고정 매핑 `text_message → submit_message`) 사용자에게 발송하는 best-effort 안내다 ([실행 엔진 §7.5.1](4-execution-engine.md#751-publisher-측-사전-검증--invalid_execution_state) 표면 매트릭스). CCH-ERR-04 의 "silently swallow 금지" 관례상, 종전에 로그만 남기던 조합에 사용자 피드백을 준다 — 구현은 `HooksService.forwardToInteractionService` 가 거부를 삼킬 때 `sendSurfaceMismatchNotice` 로 발송. **다른 default 문구와 달리 문장부호(마침표 등)를 쓰지 않는다**: 이 안내는 EIA event 렌더러(provider 별 escape) 를 거치지 않고 `adapter.sendMessage` 로 직접 발송되는 control-plane 메시지라, telegram MarkdownV2 특수문자(`. ! - ( ) [ ]` 등)를 포함하면 raw 전송이 거부된다. 따라서 default 는 세 provider 모두에서 안전하도록 특수문자를 배제했다 (`executionStillRunning` 등 다른 hooks 발송 안내가 telegram escape 를 인라인으로 baked-in 하던 것과 달리, cross-provider 정합을 택함). lookup 경로는 `sessionExpired` 와 동일.

**control-plane raw-send 키의 등록 시점 검증 (F-5)** — `HooksService` 가 렌더러를 거치지 않고 직접 발송하는 control-plane 키 — `help` · `groupChatRefusal` · `unsupportedMessageKind` · `executionStillRunning` · `surfaceMismatch` · `formValidationFailed` · `formNextField` — 는 telegram 이 `parse_mode: MarkdownV2` 로 보내므로, operator override 에 **unescaped MarkdownV2 특수문자**(`_ * [ ] ( ) ~ \` > # + - = | { } . !`)가 들어가면 send 가 400 으로 거부돼 안내가 조용히 유실된다. 따라서 `provider === 'telegram'` 일 때 이 키들의 override 를 **등록 시점에 검증**해, unescaped 특수문자 발견 시 `400 VALIDATION_ERROR`(message `UNSAFE_TELEGRAM_MARKDOWN:<field>:<char>`)로 거부한다 (operator 가 `\.` 처럼 이미 escape 했다면 통과). slack / discord 는 raw text 를 그대로 렌더하므로 대상이 아니며, `formOpenLabel`(버튼 라벨) · `sessionExpired`(렌더러 escape 경로) · CCH-ERR-* 6 키(별도 placeholder validator) 는 제외한다. 구현: `LanguageHintsRawSendValidator` ([`chat-channel-config.dto.ts`](../../codebase/backend/src/modules/triggers/dto/chat-channel-config.dto.ts)) — MarkdownV2 특수문자 판별은 `chat-channel/shared/markdown-v2.ts` 의 `firstUnescapedMarkdownV2Special`(backslash-toggle 의미론 정확 처리) 을 공유 SoT 로 쓴다.

이 검증은 nested DTO(`ChatChannelConfigDto`) 검증이다. `UpdateTriggerDto.chatChannel` 은 `@ValidateNested()` optional 이라, PATCH payload 에 `chatChannel` 객체가 포함되면 그 안의 `languageHints` 도 함께 재검증된다. 따라서 **다른 chatChannel 필드(예: `rateLimitPerMinute`)만 바꾸려 해도 클라이언트가 기존 `languageHints` 를 함께 전송하면** F-5 이전에 저장됐던 위반 override 가 재검출돼 PATCH 가 `400` 으로 막힐 수 있다(하위호환 주의점). `languageHints` 를 payload 에서 빼면 재검증되지 않는다. 에러 표면은 기존 `UNKNOWN_PLACEHOLDER` placeholder validator 와 동형 — `message` 에 `UNSAFE_TELEGRAM_MARKDOWN:<field>:<char>` 를 colon-encoding 하고 top-level `code` 는 `VALIDATION_ERROR`(details 는 `INVALID_FIELD`). 근본적으로는 defaults 의 telegram escape baked-in(`\\.`)이 slack/discord 에서 literal 로 노출되는 잔여 갭이 있으나(발송 경로 per-provider escape 로의 이관), 이는 별도 백로그다.
...
**세부**:
- (a) **`renderNode` 인터페이스 변경 최소화**: 새 함수 추가 ([Convention R-CCA-5 대안 2 기각](../conventions/chat-channel-adapter.md#r-cca-5-execution-failed-분류-helper-를-convention-에-두는-이유)) 대신 시그니처 union 확장 — `renderNode(event: EiaEvent | ChatChannelInternalEvent)`. 함수 개수 6 유지. SoT: [Convention §R-CCA-7](../conventions/chat-channel-adapter.md#r-cca-7-rendernode-시그니처-union-확장--chat-channel-internal-이벤트-수용).
