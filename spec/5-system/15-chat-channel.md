...
| CCH-CV-03 | 같은 conversation 의 두 번째 이후 메시지는 execution 상태별 분기 처리: (a) `waiting_for_input` → 인터랙션 명령으로 forwarding, (b) `running`/`pending` (waiting_for_input 미도달) → 채널에 `languageHints.executionStillRunning` 안내 메시지 발송 + update 무시 (대기 큐 미적재, 202 ack — 정당화는 Rationale R9), (c) `completed`/`failed`/`cancelled` 또는 conversation 없음 → 새 execution 시작. `running` 케이스의 안내 default 문구 = "워크플로우가 처리 중입니다. 잠시만 기다려 주세요." <br>**(b) 분기 미구현 (Planned)**: 현재 `HooksService.isActiveExecution` ([`hooks.service.ts:733`](../../codebase/backend/src/modules/hooks/hooks.service.ts)) 은 비-terminal 상태 (`running`/`pending`/`waiting_for_input`) 를 모두 single `active` 로 collapse 하고, active 면 (b) 분기 없이 무조건 인터랙션 명령으로 forwarding ([`hooks.service.ts:451-476`](../../codebase/backend/src/modules/hooks/hooks.service.ts)) 한다. `waiting_for_input` 미도달 (b) 분기·`executionStillRunning` 안내 발송은 아직 미구현 (R9 가 우려한 input-sequence 충돌이 현재 코드에 존재). (a)/(c) 는 구현됨. | 필수 |
...
| CCH-NF-03 | 채널당 분당 최대 60건 inbound (텔레그램 Bot API 의 group rate limit 과 정합). 초과분은 어댑터의 chat 단위 큐에 적재, 폭주 시 가장 오래된 update 부터 폐기하지 않고 `chat_channel_health=degraded` 표시. <br>**미구현 (Planned)**: `config.chatChannel.rateLimitPerMinute` 는 config type ([`chat-channel/types.ts:71`](../../codebase/backend/src/modules/chat-channel/types.ts)) 에 저장 슬롯만 있고, 실제 분당 enforcement·chat 단위 큐·폭주 시 `chat_channel_health=degraded` 갱신 로직은 아직 미구현. | 필수 |
...
**성공 응답 (200 OK)** — [API Convention §5.1](./2-api-convention.md) 의 `{ data }` 래퍼 + `TransformInterceptor` 적용. 현재 구현은 `rotatedAt` 1필드만 반환한다 ([`triggers.service.ts:921`](../../codebase/backend/src/modules/triggers/triggers.service.ts) `rotateBotToken` → `{ rotatedAt }`):
```jsonc
{
  "data": {
    "rotatedAt": "<ISO8601>"
...
`triggerId` / `chatChannelHealth` (setupChannel 재호출 결과) / `botIdentity` (getMe 캐시 갱신 결과) 3필드 동봉은 **미구현 (Planned)** — 현재 응답에 미포함.
...
| 401 | `WORKSPACE_REQUIRED` | `X-Workspace-Id` 헤더 누락 (controller, [`chat-channel.controller.ts:57`](../../codebase/backend/src/modules/chat-channel/chat-channel.controller.ts)) |
