...
| `isActive` | 수신 활성/비활성 |
| `authConfigId` | Webhook 인증 검증의 단일 진입 (FK → AuthConfig). `NULL` 이면 인증 없음(none). 인증 자료·`ip_whitelist` 는 모두 AuthConfig 가 보유 — `config` 에 inline 인증 키 없음 (V065 cleanup) |
...
### inline auth path 폐지 — AuthConfig 단일 진입 (2026-05-28)

본 spec 초기 설계는 `trigger.config.authType` 등 inline 인증 필드와 `trigger.auth_config_id` FK 를 모두 도입했으나, 실제 구현은 inline path 만 사용하고 `auth_config_id` 는 read 되지 않은 채 방치되었다. 본 변경에서 `auth_config_id` 를 webhook 인증의 단일 진입(SoT)으로 격상하고 inline path 를 완전히 제거한다. 근거:
...
6. **평문 secret JSONB 잔존** — 평문 `bearerToken`/`secret` 이 `trigger.config` 에 노출되어 백업·로그·재현 시 유출 위험이 있었다. `V065__trigger_config_strip_inline_auth.sql` cleanup migration 으로 제거.

inline read 중단과 cleanup migration 을 같은 PR 에서 처리하므로 hidden break 가 없다 — `auth_config_id IS NULL` 인 기존 트리거는 본 변경 후 "인증 없음(none)" 으로 동작한다. (consistency 사전 검토: `review/consistency/2026/05/28/13_56_08/SUMMARY.md`.)
