...
| WH-EP-06 | 요청 헤더 정보를 메타데이터로 전달 (`headers`, `method`, `query`) — 민감 헤더 값은 ingestion 시 `[REDACTED]` 마스킹([§5.3](#53-민감-헤더-마스킹-ingestion)) | 권장 |
...
| WH-SC-05 | Rate limiting — 분당 최대 요청 수 (현행 구현: 글로벌 throttler **100 req/min**, [Spec API 규약 §7](./2-api-convention.md#7-rate-limiting)). 추가로 공개 webhook(`auth_config_id IS NULL`)에는 `PublicWebhookThrottleGuard` 의 IP 단위 한도(분당 10·시간당 누적 20 기본)가 적용된다 — §6 참조. **클라이언트 IP 미식별 시 미식별 요청은 단일 공유 버킷으로 묶여 동일 한도 적용(완화 — 무제한 우회 아님)**. 인증 webhook 의 `ip_whitelist`(WH-SC-09)는 반대로 IP 미식별 시 **거부(fail-closed)** — rate-limit(가용성 보호)과 인증 게이트의 성격 차이([4-security R6](../7-channel-web-chat/4-security.md#r6-공개-webhook-ip-미식별--단일-공유-버킷-완화-한도)) | 권장 |
...
| `headers` | 요청 헤더 (소문자 키). **민감 헤더 값은 ingestion 시 `[REDACTED]` 마스킹** ([§5.3](#53-민감-헤더-마스킹-ingestion)) — 위 예시의 `content-type`/`x-event-type` 처럼 비민감 헤더는 원본 유지 |
| `query` | URL 쿼리 파라미터 |
| `method` | HTTP 메서드 |
...
- **`error.details[]`**: 필드별 사유. `field` = 파라미터 이름, `code` = `UPPER_SNAKE_CASE` field code (`MISSING_REQUIRED_FIELD` = required 누락 / `TYPE_COERCION_FAILED` = 선언 타입 coerce 불가). 헬퍼는 `INVALID_SCHEMA`(스키마 구조 위반)와 `MASKED_VALUE_RESUBMITTED`(마스킹된 값 재제출)도 매핑하나 **webhook 런타임 경로에서는 둘 다 발생하지 않는다**(따라서 위 예시에 미포함) — 전자는 저장 시점 검증 코드이고, 후자는 **Manual 실행 경로 한정**이라 webhook 수신은 대상이 아니다 — 판정 기준이 값의 출처가 아니라 **페이로드의 저작 주체**이고, webhook body 는 외부 시스템이 저작한다([EIA §R17](./14-external-interaction-api.md)). 4종 카탈로그 등재 [error-handling §1.7](./3-error-handling.md#17-webhook-수신-에러-코드-도메인-spec-참조), 명명 규약 [error-codes 규약](../conventions/error-codes.md).
- **구현**: `hooks.service` 가 `BadRequestException({ code, message, details })` 를 throw 하고 `GlobalExceptionFilter` 가 `details` 를 그대로 봉투로 전달한다. 내부 분류 문자열(`missing_required`/`coerce_failed`/`invalid_schema`/`masked_value_resubmitted`, [error-codes 규약 §4](../conventions/error-codes.md#4-내부-전용-분류-코드-정규화-후-발행) 패턴)은 `toTriggerParameterErrorDetails` 가 위 public field code (`MISSING_REQUIRED_FIELD`/`TYPE_COERCION_FAILED`/`INVALID_SCHEMA`/`MASKED_VALUE_RESUBMITTED`)로 정규화한다 (Manual 실행 경로 `INVALID_TRIGGER_PARAMETERS` · Manual re-run `INVALID_INPUT` 도 동일 헬퍼 사용 — [manual-trigger §6](../4-nodes/7-trigger/1-manual-trigger.md#6-에러-코드)).

### 5.3 민감 헤더 마스킹 (ingestion)
...
- Rate Limiting (공개 webhook 전용 추가): `PublicWebhookThrottleGuard` 가 `auth_config_id IS NULL` 트리거에 한해 IP 단위 시작 한도(기본 분당 10, config `publicWebhook.startupPerMinute`) + 시간당 누적 신규 상한(기본 20, `publicWebhook.hourlyNewMax`) 을 적용. 초과 시 `429 PUBLIC_WEBHOOK_RATE_LIMIT` / `PUBLIC_WEBHOOK_HOURLY_LIMIT` (카탈로그 [error-handling §1.7](./3-error-handling.md#17-webhook-수신-에러-코드-도메인-spec-참조)). **클라이언트 IP 를 헤더에서 식별하지 못하면(공격자가 `X-Forwarded-For`/`CF-Connecting-IP` 헤더 제거 등) 미식별 요청 전체를 단일 공유 sentinel 버킷(`UNIDENTIFIED_IP_BUCKET`, `public-webhook-quota.service.ts`)으로 묶어 동일 한도를 적용한다 — 과거 무제한 통과(`return true`) 우회를 유한 상한으로 강화한 완화 한도. `req.ip`/`req.socket.remoteAddress` 폴백은 미채택(정책·근거 [4-security §4·R6](../7-channel-web-chat/4-security.md#r6-공개-webhook-ip-미식별--단일-공유-버킷-완화-한도)).** 인증 webhook 은 이 Guard 를 무제한 통과(본문 크기는 아래 라우트 스코프 파서가 별도 게이트). Redis 미가용 시 fail-open. **Guard 의 trigger 조회 실패 시에도 fail-open(통과)하되, 이는 공개 webhook 보호를 일시 무력화하므로 `error` 레벨로 로깅해 장기 DB 장애로 인한 보호 우회 지속을 모니터링이 조기 탐지하게 한다.** (정책 수치 출처: [Spec 웹채팅 보안 §4](../7-channel-web-chat/4-security.md); config 키·에러 코드 적용 SoT 는 본 §6 + [error-handling §1.7](./3-error-handling.md#17-webhook-수신-에러-코드-도메인-spec-참조))
...
7. config.chatChannel 가 있으면 (Chat Channel 분기):
   ...
   e. 활성 execution 이 있으면 InteractionService.interact() in-process 호출 (token bypass — [EIA §3.3 EIA-AU-08](./14-external-interaction-api.md#33-인증))
      없으면 `ExecutionEngineService.execute(workflowId, input, { triggerId: trigger.id, sourceIp, responseCode: '202' })` 시작 (입력 = parseUpdate 결과 변환). `sourceIp`(`extractClientIpFromHeaders` — 헤더 기반 IP 추출 정책 [1-auth Rationale 2.3.B m-3](./1-auth.md))·`responseCode`(202) 는 §A.3 호출 이력에 영속된다 ([config §A.3](../2-navigation/6-config.md), [R-6](../2-navigation/6-config.md#rationale)). schedule/manual 트리거는 두 인자를 생략 → 컬럼 NULL (ExecuteOptions 의 triggerId variant 에서 `sourceIp?`/`responseCode?` 는 optional).
...
8. config.chatChannel 가 없으면 (기존 경로):
   ...
   b. ExecutionEngineService.execute(trigger.workflowId, { parameters, body, headers, query, method }, { triggerId: trigger.id, sourceIp, responseCode: '202' })
      ...
      - `sourceIp`(`extractClientIpFromHeaders` 결과 — 인증 IP whitelist 검증과 공용, 헤더 기반 추출 정책 [1-auth Rationale 2.3.B m-3](./1-auth.md))·`responseCode`(성공 경로의 실제 HTTP 코드 `202`)도 함께 전달되어 Execution 행의 `source_ip`/`response_code` 컬럼에 영속되고, 인증 설정 사용 내역(§A.3 호출 이력)의 소스 IP·응답 코드 컬럼을 채운다 ([config §A.3](../2-navigation/6-config.md), [R-6](../2-navigation/6-config.md#rationale); [WH-MG-05](#요구사항)). schedule/manual 트리거는 두 인자를 생략하므로 컬럼 NULL — `ExecuteOptions` 의 triggerId variant 에서 `sourceIp?`/`responseCode?` 는 optional 이라 기존 호출자 호환.
...
| 요청 민감 헤더 노출 | webhook 요청의 인증/시크릿성 헤더(`Authorization`/`Cookie`/`X-Api-Key` 등)는 ingestion 시 `[REDACTED]` 마스킹 후 `Execution.inputData` 에 저장 ([§5.3](#53-민감-헤더-마스킹-ingestion)) — 실행 상세·$trigger·output.request 어디로도 raw 유출 없음. 인증은 마스킹 전 raw 로 수행돼 무영향 |
...
| Rate Limiting | 글로벌 Throttler (100 req/min, [Spec API 규약 §7](./2-api-convention.md#7-rate-limiting)) + 공개 webhook 전용 IP 단위 한도 (분당 10·시간당 20 기본, `PublicWebhookThrottleGuard`/`PublicWebhookQuotaService`). IP 미식별 시 미식별 요청은 단일 공유 버킷으로 완화 한도 적용([4-security R6](../7-channel-web-chat/4-security.md#r6-공개-webhook-ip-미식별--단일-공유-버킷-완화-한도)) |
...
### 민감 헤더 마스킹 — ingestion(저장) 시점 채택 (2026-07-07)

민감 헤더는 §5.3 대로 **ingestion(저장) 시점**에 마스킹한다. 대안인 **display(응답) 시점 마스킹**(원본을 `Execution.inputData` 에 저장하고 execution/background-run 응답 DTO 에서만 마스킹)은 기각했다:

- **채택 근거(ingestion)**: (a) secret at-rest 자체를 제거 — DB·감사 레코드에도 raw 인증 헤더가 남지 않는다. (b) `inputData`·`output.request.headers`·`$trigger.headers` + 향후 신규 read 경로까지 **단일 소스**로 커버 — 표면별 마스킹의 whack-a-mole 을 원천 차단. (c) 인증(HMAC/IP/토큰)은 마스킹 이전 raw 로 수행되고 §A.3 호출 이력도 raw 로 남으므로 인증·감사 기능 무손실. (d) `output.request.headers` 를 raw 로 소비하는 다운스트림 노드가 없어(조사 확인) 정상 use-case 회귀 없음.
- **기각(display)**: raw secret 이 DB 에 잔존해 유출 표면(DB 접근·백업·신규 endpoint)이 남고, 모든 read 경로를 개별적으로 마스킹해야 한다. execution read 는 `verifyOwnership(workspaceId)` 만 게이트하므로 워크스페이스 전 멤버 노출 위험이 저장 자체를 없애는 편이 안전.
- `$trigger.headers` 의 `buildTriggerView` view-mask 는 제거하지 않고 idempotent defense-in-depth 로 유지한다(비-webhook 경로가 triggerData 에 raw 를 넣더라도 표현식에 새지 않도록).
