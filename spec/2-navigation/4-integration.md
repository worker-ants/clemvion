...
> 정렬: first-party 서비스(Google/GitHub/Cafe24/MakeShop) → 범용(HTTP/Database/Email/Webhook) → 도구 확장(MCP). 카드 목록은 `GET /api/integrations/services` (백엔드 service registry, §9.1) 기반으로 렌더링된다 — MakeShop 카드는 registry 의 `type: 'makeshop'` 등록으로 노출.
...
7. 그동안 설정 안내 화면(2번)은 해당 Integration row 를 **3초 간격으로 자동 폴링**한다 — Cafe24 "테스트 실행" 이 띄우는 창의 `window.opener` 는 Cafe24 탭이라 `postMessage` 복귀(§3.5)가 불가능하기 때문 (Rationale "Cafe24/MakeShop pending_install 자동 폴링" 항). `connected` 전이를 감지하면 토스트 + 목록 쿼리 invalidate 후 상세 페이지(`/integrations/<id>`)로 자동 라우팅한다. **10분 soft timeout** 경과 시 폴링을 멈추고 "아직 대기 중" 안내를 표시한다 — row 는 백엔드에 유지되며 페이지 재진입/window focus 시 폴링이 재개된다. row 가 `pending_install` 인 채 callback 실패가 기록되면 `last_error`/`status_reason` 기반 진단 단서를 같은 화면에 표시한다. 통합 목록의 연결 상태 배지로도 확인 가능.

> **mall_id** 는 base URL 의 일부이며 authorize URL 도 mall 별로 달라, OAuth begin 단계에서 사용자가 선행 입력해야 한다 (Public/Private 모두 동일).

**OAuth2 흐름 (MakeShop — ShopStore install-first)** — ShopStore 마켓의 앱 설치가 install flow 의 진입점이라 우리 서비스가 OAuth 를 시작할 수 없다 (Cafe24 Private 와 같은 install-first 모델 — [§5.9](#59-makeshop) 참조).

1. **사전 입력** (제출 전 필수 검증): `Client ID`, `Client Secret` (파트너센터 앱 등록 시 발급) — 둘 중 하나라도 비어 있으면 폼 검증 에러로 차단. `shop_uid` 는 이 단계에서 입력하지 않는다 — ShopStore 설치 redirect 의 파라미터로 도착한다 (§5.9, §9.2 `makeshop/precheck`).
2. **제출** → `POST /api/integrations/oauth/begin` 호출 (body: `{ "service": "makeshop", "mode": "new", "client_id": "...", "client_secret": "...", "scopes": [...] }`). 응답: `{ "mode": "makeshop_pending_install", "integrationId": "...", "appUrl": "https://<host>/api/3rd-party/makeshop/install/:installToken", "callbackUrl": "..." }` (§9.2). Integration 이 `status=pending_install` 상태로 즉시 생성된다 (토큰 없음, 팝업 없음).
3. **설치 안내 화면** 표시 — Cafe24 Private 절의 설정 안내 화면과 동일한 복사 UX (라벨 + 모노스페이스 URL + 복사 버튼): `App URL` / `Redirect URI` 를 파트너센터 앱 설정에 등록하도록 안내.
4. **자동 폴링** — Cafe24 Private step 7 과 동일한 폴링 UX (3초 간격 / `connected` 전이 시 토스트 + 상세 페이지 자동 라우팅 / 10분 soft timeout). callback 실패 시 `status_reason` 을 안전한 i18n 메시지로 매핑해 표시한다 (§5.9 "pending_install 안내 화면의 에러 표시").
5. 사용자가 ShopStore 에서 앱 설치 → MakeShop 이 App URL 호출 (HMAC 검증, §5.9) → OAuth authorize → callback → `pending_install → connected`.
...
| App URL 카드 (`appUrl` 보유 통합 한정) | `IntegrationDto.appUrl` (derived 필드, §9.1) 이 non-null 인 통합 — 현재 **Cafe24 Private** (`service_type='cafe24' AND credentials.app_type='private'`) 과 **MakeShop ShopStore 설치 통합** — 에만 표시. **App URL** (`${APP_URL}/api/3rd-party/<provider>/install/:installToken`) 과 **Redirect URI** (App URL 의 `/install/<token>` 을 `/callback` 으로 치환) 를 복사 버튼과 함께 노출한다. 문구는 `service_type` 별 분기 — cafe24 는 Cafe24 Developers Console "앱 URL" 갱신 안내 (App URL HMAC 검증 실패 에러 페이지가 안내하는 비교 대상이 본 카드), makeshop 은 파트너센터 앱 설정 안내. 신규 등록 흐름의 `Cafe24PrivatePending` 컴포넌트와 동일한 복사 UX 패턴 (라벨 + 모노스페이스 URL + 복사 버튼 + 1줄 안내) 재사용. 결정 근거는 Rationale "Cafe24 App URL 상세 페이지 표시" 항. |
...
│                                                            │
│ Workflow B (Inactive)                                      │
│  └─ Support Agent  [MCP]  (node id: ghi)  [Open in editor →]│
└────────────────────────────────────────────────────────────┘
```
...
- `usageKind='mcp'` 노드는 라벨 옆에 **`MCP` 배지** (보조 톤) 를 표시 — 해당 노드가 통합을 직접 노드가 아니라 AI Agent 의 MCP 도구로 사용함을 구분한다. `usageKind='direct'` 는 배지 없음.
...
- **연결 안 됨 배너 (§4.1 status)**: 통합 `status ≠ connected` (`error` / `expired` / `pending_install`) 이면 활동 목록·빈 상태 **위에** [Inline Alert](../0-overview.md#34-상태-표시-패턴) 를 노출한다 — 이 상태에서는 **새 호출이 기록되지 않으므로**(AI Agent 는 MCP bridge 가 미연결 통합의 tool 을 노출하지 않아 호출 자체가 없고, 직결 노드는 `INTEGRATION_NOT_CONNECTED` 로 즉시 실패), 단순 "활동 없음"(빈 상태)과 구분해 원인을 알린다. 톤은 §3.4 status→tone escalation 에 맞춰 `error` 는 **red**, `expired`/`pending_install` 은 **warning(amber)** — 헤더 `StatusBadge`(error=red) 신호와 일치시킨다. 배너는 상태 확인·재연결로 유도하는 **"상태 확인" 버튼(클릭 시 [개요 탭] 이동)**을 포함한다. `connected`(곧 만료 expires-soon 포함)는 여전히 기록되므로 미노출. `pending_install` 을 포함하는 이유는 §2.4 attention 필터(정상 전환 상태로 제외)와 **다른 축** — "조치 불요 여부" 가 아니라 "활동 데이터 부재 사유 설명" 이기 때문이다.
...
> **구현 상태: 구현 완료 (status: implemented).** 한국 이커머스 SaaS MakeShop 의 신형 Shop API ([공식 문서](https://developer.makeshop.co.kr/docs/api/shop/상점-설정-정보)) 통합. Cafe24(§5.8) 와 **동형 설계**이며, 하나의 Integration 이 (a) 워크플로의 [`makeshop` 노드](../4-nodes/4-integration/5-makeshop.md), (b) AI Agent 의 MCP 도구 양쪽에서 사용된다. 본 절은 메이크샵 고유 분기만 명시하고 공유 메커니즘(상태 전이·토큰 자동 갱신·설치 HMAC·rate limit wrapper·연결 테스트 401 자가회복)은 §5.8 정책을 재사용한다.
...
| 필드 | 타입 | 필수 | 비밀 | 설명 |
|------|------|------|------|------|
| `shop_uid` | string | ✓ | × | MakeShop 상점 식별자. base URL `https://connect.makeshop.co.kr/api/v1/{shop_uid}/...` + `mall_id` 컬럼 투영 ([data-model §2.10](../1-data-model.md)). ShopStore 설치 redirect 의 `shop_uid` 파라미터에서 획득 |
| `client_id` | string | ✓ | × | OAuth client_id (파트너센터 앱 등록 시 발급) |
| `client_secret` | string | ✓ | 🔒 | OAuth client_secret. 설치 HMAC 검증 키 겸용 |
| `access_token` | string | ✓ | 🔒 | OAuth access token (1시간 유효) |
| `refresh_token` | string | ✓ | 🔒 | OAuth refresh token (기본 30일·최대 90일, **1회 사용 후 회전**) |
| `scopes` | string[] | ✓ | × | 사용 권한 scope. `<x-scope>.read` / `<x-scope>.write` 형식 (예: `store.read`, `product.write`) |
| `expires_at` | ISO8601 | ✓ | × | `access_token` 만료 시각. §10.5 원자 갱신 정책으로 동기화 |

**OAuth 흐름 (cafe24 와 다른 점)**

- **인증 = Authorization-Code + PKCE(OAuth 2.1)**. authorize: `https://auth.makeshop.com/oauth/authorize` (response_type=code, PKCE S256), 토큰 교환·refresh: `POST https://auth.makeshop.com/oauth/token` (Basic auth `client_id:client_secret`). 자동 갱신은 §10.5 흐름 그대로 — **refresh token 회전(rotation) 으로 새 refresh_token 을 매번 저장**.
- **scope wire format = 공백 구분 (표준)** — cafe24 의 콤마 예외 미적용. begin 의 provider 분기에서 makeshop 은 표준 공백 구분.
- **설치(ShopStore)**: 마켓 설치 시 등록된 App URL 로 `?shop_uid=...&timestamp=...&action_type=install&hmac=...` redirect. HMAC-SHA256(client_secret) 검증 + ±5분 replay 방어는 cafe24 Private 설치(§5.8, [Cafe24 노드 §9.8](../4-nodes/4-integration/4-cafe24.md#98-private-앱-app-url-hmac-검증)) 와 동일 패턴 — `install_token` capability 모델·rate-limit/nonce 보호 재사용. ⚠ MakeShop HMAC 메시지 구성(정렬·인코딩)은 코드에 `VERIFY` 마킹 — production 전 공식 문서로 확정 ([MakeShop 노드 §9.7](../4-nodes/4-integration/5-makeshop.md#97-미확인-항목-production-전-검증-필요)).
- **pending_install 안내 화면의 에러 표시**: callback 실패로 기록된 `status_reason` 은 알려진 5종 (`oauth_token_exchange_failed` / `oauth_state_mismatch` / `oauth_state_expired` / `oauth_invalid_scope` / `hmac_verification_failed`) 만 명시적 i18n 키로 매핑해 표시하고, **미지의 사유는 generic 메시지로 fallback** — raw 백엔드 에러 텍스트(HMAC 상세·token exchange trace)를 사용자에게 노출하지 않는다.

**Scope 권장 프리셋** (catalog x-scope × R/W)
...
**AI Agent 노출**: `service_type='makeshop'` Integration 은 AI Agent `mcpServers` 셀렉트에서 선택 가능하며, `MakeshopMcpToolProvider` 가 in-process `AgentToolProvider` 로 7 섹션의 **161 REST operation**(수치 SoT: [MakeShop 노드 §지원 범위](../4-nodes/4-integration/5-makeshop.md)) 을 MCP tool 로 노출한다 ([MakeShop 노드 §8](../4-nodes/4-integration/5-makeshop.md#8-ai-agent-노출-internal-mcp-bridge)).

**Webhook (범위 밖)**: CPIK webhook 11개(이벤트 수신)는 본 통합 범위 밖 — cafe24 와 공통의 trigger 노드 후속 과제 ([MakeShop 노드 §9.6](../4-nodes/4-integration/5-makeshop.md#96-cpik-webhook-범위-분리--cafe24-와-공통-후속)).
...
- `GET /api/integrations/:id/usages`는 모든 워크플로우의 `Node.config`를 JSONB 조회하며, 두 참조 경로의 **합집합**을 반환한다:
  - (a) **직접 참조** — 최상위 `config->>'integrationId' = :id` (HTTP·Database·Send Email·Cafe24·MakeShop 등 Integration 노드)
  - (b) **MCP 참조** — AI Agent 노드의 `config->'mcpServers'` 배열 안 객체의 `integrationId`. JSONB containment 로 매칭한다: `config->'mcpServers' @> '[{"integrationId":"<id>"}]'::jsonb`
- 결과는 워크플로우 단위로 그룹화 `{ workflowId, workflowName, isActive, nodes: [{ id, label, type, usageKind }] }[]`
  - `usageKind: 'direct' | 'mcp'` — 노드가 통합을 참조하는 방식. 일반 노드는 `mcpServers` 가 없고 AI Agent 노드는 최상위 `integrationId` 를 쓰지 않으므로 두 경로는 실질 배타적이다. 한 노드가 양쪽 모두에 해당하면 `direct` 를 우선한다.
...
- 차단 대상에는 통합을 **MCP 도구로 참조하는 AI Agent 노드도 포함**된다 (§7.1 합집합). 사용 중인 MCP 통합이 실수로 삭제돼 AI Agent 가 런타임에 파손되는 것을 방지한다. 다이얼로그의 MCP 노드 행도 `MCP` 배지로 구분 표시한다.
...
> ※ **필드명 주의** — 동일한 "서비스 식별자" 개념이지만 두 엔드포인트가 서로 다른 필드명을 쓴다: `oauth/begin` 은 `OAuthBeginDto.service` (필드명 `service`), `preview-test` 는 `PreviewTestDto.serviceType` (필드명 `serviceType`). 두 DTO 가 독립적으로 발전한 결과이며 `OAuthBeginDto.service` rename 은 별도 breaking-change PR 범위다 (현재 정합 대상 아님).
...
| GET | `/api/integrations/services/:type/catalog` | 서비스 타입별 API 카탈로그 조회. 응답: `{ operations: Array<{ key, method, path, labelKey, descriptionKey }> }`. `key` 는 §4.6 의 `apiLabel` 과 join key (cafe24 의 경우 `cafe24.<resource>.<operation>` — [`spec/conventions/cafe24-api-metadata.md`](../conventions/cafe24-api-metadata.md); makeshop 은 `makeshop.<resource>.<operation>` — [`spec/conventions/makeshop-api-metadata.md`](../conventions/makeshop-api-metadata.md)). `method`/`path` 는 §4.6 표시용 endpoint subtext fallback. `labelKey`/`descriptionKey` 는 frontend i18n dict 가 사람 친화 라벨로 변환한다. **초기 응답 정책**: `:type='cafe24'` 및 `:type='makeshop'` 만 backend 메타데이터에서 추출한 `operations[]` 를 채워 반환하고, `:type ∈ {http, database, email, webhook, mcp, google, github}` 은 빈 배열 (`{ operations: [] }`) 을 반환한다 — 그 외 통합은 활동 로그의 `apiLabel` 이 NULL 이라 catalog 매핑이 의미 없기 때문 (§4.6 의 endpoint-only fallback 으로 충분). 미지원 `:type` 은 일반 404. 모든 인증된 요청에서 접근 가능, workspace 격리 없음 (메타데이터는 동일 응답). 백엔드 응답 DTO 이름: `OperationCatalogDto` (기존 `ServiceCatalogDto` — `GET /api/integrations/services` 의 "서비스 종류 목록" — 와 명확히 구분). NestJS 선언 순서 — `:id` 동적 경로보다 앞, `services/:type` 단독 경로 (장래 추가 시) 보다 뒤. 기존 `cafe24/precheck` 사례와 동일 패턴. |
...
| `at` | ISO8601 | 호출 시각 |
| **`apiLabel`** | string \| null | 통합별 channel 별 라벨 (catalog key 형식). 채우기 정책은 [`spec/4-nodes/4-integration/_product-overview.md §2.4`](../4-nodes/4-integration/_product-overview.md#24-사용처-추적-및-라이프사이클) 참조 — 요약: cafe24 는 `cafe24.<resource>.<operation>`, makeshop 은 `makeshop.<resource>.<operation>`, 그 외 통합은 NULL |
...
- 실패: `{ code, message, details? }`
  ...
  - `CAFE24_PRIVATE_APP_ALREADY_CONNECTED` (409) — 동일 `(workspaceId, mall_id)` 에 이미 cafe24 Integration (`app_type` 무관 — public/private 모두) 이 존재. 통일 store-identifier UNIQUE 가 `(workspace_id, service_type, mall_id)` 기준이므로 같은 service+mall 에 app_type 분리 보유 불가. **두 경로에서 동일 코드 반환**: ① Cafe24 Public/Private begin 의 사전 SELECT (connected row 만 차단), ② `POST /api/integrations` finalize 단계의 통일 partial UNIQUE 위반 (race backstop — `idx_integration_workspace_service_mall` (V072) 의 `23505` 를 `throwIfUniqueViolation` 이 serviceType='cafe24' 분기로 본 코드로 변환). 코드 이름의 `PRIVATE` 토큰은 historical artifact (신설 당시 Private 흐름 한정이었음) 이며 의미는 본 spec 정의에 따른다 — 클라이언트는 코드 이름이 아닌 본 의미(mall_id 기준 중복) 로 분기 (의미 기반 명명 선례 예외, Rationale "CAFE24_PRIVATE_APP_ALREADY_CONNECTED 코드명 유지 결정" 참조). swagger 규약(spec/conventions/swagger.md §2-4 — 중복/충돌은 409, `INTEGRATION_IN_USE(409)` 선례) 에 맞춤.
...
> 만료 스캐너는 **네 개의 독립 BullMQ job** (`connected-expiry` / `pending-install-ttl` / `usage-log-prune` / `cafe24-background-refresh`) 으로 운영된다 — 각 job 은 자체 retry (`attempts: 3`, 60s exponential backoff) 와 큐 메트릭을 가지므로 한 패스의 실패가 다른 패스의 실행을 막지 않는다. Cafe24 Private 및 MakeShop ShopStore install-first 통합의 `pending_install` 24h TTL 만료는 `pending-install-ttl` job 이 담당 (service-agnostic — `pending_install` 상태의 모든 service_type 을 sweep). Cafe24 의 `refresh_token` 14일 만료 전 자동 갱신은 `cafe24-background-refresh` job 이 enqueuer 역할로 담당 (실제 갱신은 `cafe24-token-refresh` 큐의 worker — §10.5 참조). 상세 흐름·격리 정책은 [data-flow §1.4](../data-flow/5-integration.md#14-oauth-만료-스캐너-bullmq-integration-expiry-scanner) 참조.
...
**채널**: `notification_preferences.integrationExpiryEmail` (`integration_action_required` 에도 같이 적용) 활성화 시 `channel='both'`, 기본 `'in_app'`.
...
- 사용자별 프로필 설정에 **`notification_preferences.integrationExpiryEmail`** 토글 — **opt-in(기본 off)**. SoT: [사용자/워크스페이스 §5.1](./9-user-profile.md), 저장 위치는 `User.notification_preferences` JSONB.
- 활성화 시 `Notification.channel = 'both'` 로 생성되어 **`NotificationsService.dispatchEmails`**(`MailService` 주입)가 이메일을 best-effort 발송한다.

> **필드명·클래스명 정정 (2026-07-17)**: 종전 이 절은 옛 필드명 `notifyIntegrationExpiryByEmail` 과 `NotificationDispatcher` 가 이메일을 발송한다고 서술했으나 **둘 다 코드와 어긋난다**.
> - **필드명**: 실제는 `integrationExpiryEmail` (`user.entity.ts` `notification_preferences`, `integration-expiry-scanner.service.ts`·`integration-action-required-notifier.service.ts` 의 `prefs.integrationExpiryEmail === true`). 옛 이름은 소스 주석에 이력으로만 남아 있다. [9-user-profile §5.1 각주](./9-user-profile.md)가 이 표면 차이를 이미 지적하고 있었다.
> - **클래스명**: `NotificationDispatcher` 는 **실존하지만 이메일 발송자가 아니다** — `modules/external-interaction/notification-dispatcher.service.ts` 의 EIA **webhook 큐**(`NOTIFICATION_WEBHOOK_QUEUE`) 전용 enqueuer 다 ([External Interaction API](../5-system/14-external-interaction-api.md)). 이메일은 `modules/notifications/notifications.service.ts` 의 `dispatchEmails` 가 담당한다. 이름이 비슷한 별개 컴포넌트를 잘못 지목한 것이라 **"존재하지 않는 클래스" 가 아니라 "다른 계층의 클래스"** 였다.
...
- 인덱스 추가: `(workspace_id, name) UNIQUE`, `(workspace_id, status)`, `(token_expires_at)`, `IntegrationUsageLog (integration_id, at DESC)`, `(install_token) WHERE install_token IS NOT NULL` (V043), 통일 store-identifier UNIQUE `(workspace_id, service_type, mall_id) WHERE mall_id IS NOT NULL` (V072 — 옛 cafe24 전용 V046 통일, service_type 무관)
...
- 실패 시 핸들러는 `IntegrationError(code, message)`를 **`port: 'error'` + `output.error`로 라우팅**(D4 결정, 2026-05-17 — 종전 throw 폐지)하며 `Integration.status`·`last_error`가 함께 갱신되고 `logUsage({ status: 'failed' })`가 기록된다. config 형식 자체 오류(`handler.validate` 실패)만 throw 로 남는다.
...
- 노드 설정 패널에서 Integration 선택은 `IntegrationSelector` 공용 드롭다운을 사용한다 — `serviceTypes` prop으로 목록을 필터(Send Email은 `email`, Database는 `database`, HTTP의 `authentication='integration'` 모드는 `http`, Cafe24 노드는 `cafe24`, MakeShop 노드는 `makeshop`, AI Agent 의 `mcpServers` 항목은 `['mcp', 'cafe24', 'makeshop']`).
- AI Agent 의 `mcpServers` 셀렉트는 `service_type='mcp'`·`service_type='cafe24'`·`service_type='makeshop'` 을 모두 받는다 — `cafe24`·`makeshop` 은 backend `Cafe24McpToolProvider`·`MakeshopMcpToolProvider` 가 in-process `AgentToolProvider` 구현체로 동작 ([Spec MCP Client §2.3 Internal Bridge](../5-system/11-mcp-client.md#23-internal-bridge-in-process)). UI 는 그룹을 시각적으로 분리 표시 (`🌐 Generic MCP (HTTP) servers` / `🛒 Cafe24 stores (Internal Bridge)` / `🛒 MakeShop stores (Internal Bridge)`).
- AI Agent 노드는 Integration 노드와 달리 `mcpServers` 가 다중 선택 (multi-select) 이며, 서버별로 도구 allowlist·resource/prompt 노출 토글 UI 가 추가된다 — Cafe24 의 경우 도구 수가 많아(Resource × Operation = **485**, 2026-07-17 실측 — [Spec Cafe24 노드 §지원 범위](../4-nodes/4-integration/4-cafe24.md) SoT) allowlist UI 가 카테고리 단위 grouping 으로 노출된다. 이 규모는 `AI_AGENT_TOOL_COUNT_MAX` 기본값(128)을 상시 초과하므로 allowlist 는 UX 편의가 아니라 **사실상 필수**다 ([Spec AI Agent §4.2](../4-nodes/3-ai/1-ai-agent.md#42-도구-정의-payload-예산-tool-definition-payload-budget)) (용어는 [Spec Cafe24 API 메타데이터 §8](../conventions/cafe24-api-metadata.md#8-allowlist-와의-관계) 기준; [Spec AI Agent](../4-nodes/3-ai/1-ai-agent.md), [Spec MCP Client §5.6](../5-system/11-mcp-client.md#56-도구-allowlist)).
...
Integration 생성·수정·삭제·회전·재인증·scope 전환 이벤트를 `resource_type='integration'`로 기록한다. `action`은 `integration.created`, `integration.updated`, `integration.deleted`, `integration.rotated`, `integration.reauthorized`, `integration.scope_changed` (단일 SoT 는 [인증 spec §4.1](../5-system/1-auth.md) · [data-flow §1.1](../data-flow/1-audit.md)).

**재인증(reauthorize) 분기 — `integration.reauthorized` 는 모든 재인증을 포괄하지 않는다.** 이 action 은 **OAuth provider 가 없는 통합의 reset 경로 전용**이다: `reauthorize()` 가 상태를 `connected` 로 되돌리는 즉시 `details.mode='reset'` 으로 기록한다. 반면 **OAuth 통합의 재인증**은 `reauthorize()` 가 `oauth/begin` 으로 위임만 하므로 begin 시점에 audit 를 남기지 않고, 이어지는 OAuth callback 완료 경로도 현재 `integration.reauthorized` 를 기록하지 않는다 — 즉 **OAuth 재인증 완료는 현 시점 audit 커버리지 갭**이다([data-flow §1.1](../data-flow/1-audit.md) 이 커버리지의 ground truth). 구현 확장 시 callback 성공 지점에서 기록하도록 보강한다.
...
### 사용처 추적 — AI Agent MCP 참조 포함

초기 사용처 추적(INT-US-01/§7.1)은 노드 `config.integrationId` 최상위 필드만 스캔했다. 그러나 AI Agent 노드는 통합을 MCP 서버로 `config.mcpServers[].integrationId` (중첩 배열) 에 저장하므로 이 경로가 추적에서 누락됐다. 세 가지 이유로 MCP 참조를 합집합에 포함한다: (1) **spec 내부 정합** — §14.2·cafe24/makeshop 섹션은 이미 통합이 `mcpServers` 에서 사용됨을 명시하고, INT-US-05 활동 로그는 MCP 호출을 이미 기록한다. 사용 탭만 제외하면 "활동 탭엔 호출이 있는데 사용 탭엔 사용처 없음" 모순이 생긴다. (2) **삭제 안전성** — §7.2 삭제 차단이 같은 쿼리를 쓰므로, MCP 참조 누락은 사용 중 통합의 무경고 삭제(AI Agent 파손)로 직결된다. (3) **사용자 신뢰** — MCP 도 정당한 사용 형태이므로 추적되는 것이 사용자 기대와 일치한다.

`usageKind` 필드로 `direct`/`mcp` 를 구분해 UI 배지로 노출한다 — 한 통합이 일반 노드와 MCP 양쪽에서 쓰일 때 사용자가 맥락을 구분하도록. 두 경로는 노드 타입상 실질 배타적이라 한 노드가 양쪽에 잡히는 경우는 없으나, 방어적으로 `direct` 우선 규칙을 둔다.

쿼리는 직접 참조는 기존 `->>` 동등 비교, MCP 참조는 JSONB containment(`@>`)로 배열 내부 객체를 매칭한다. `@>` 는 GIN 인덱스를 탈 수 있어 워크플로우 수 증가에도 확장 가능하다.
...
| makeshop | catalog key (`makeshop.<resource>.<operation>`) | operation 의 HTTP method | operation 의 path template (placeholder 그대로) |
...
cafe24·makeshop 이 catalog 라벨을 갖고 나머지(http/database/email 등)는 endpoint-only — 단일 `api` 컬럼으로 합치면 (a) cafe24·makeshop 의 `(label, endpoint)` 2줄 표시가 깨지고, (b) 향후 method/path 별 인덱스·필터 (예: "최근 7일 5xx 응답만") 확장 여지가 사라진다. denormalize 비용은 통합당 최대 392 byte (128+8+256) 인데 단건 row 평균 200~500 byte 의 `error_message`/`error_code` 와 비교해 무시할 수준.

**왜 catalog endpoint 를 신설했나** — cafe24·makeshop 라벨의 i18n 책임을 backend 가 아닌 frontend 에 두기 위해서다. backend 가 i18n 결과 (예: `"상품 목록 조회"`) 를 직접 `api_label` 에 적재하면 (a) UI 언어 변경 시 DB 의 옛 한국어 라벨이 영문 UI 에 그대로 노출되는 회귀가 영구화되고, (b) catalog 변경 시 DB 의 옛 라벨이 stale 해진다. 책임 분리는 다음과 같다:

- **DB** (`api_label`) — catalog key (`cafe24.<resource>.<operation>` / `makeshop.<resource>.<operation>`) 만 저장. 언어 정보 없음.
...
- **Frontend i18n dict** — `labelKey` → KO/EN 사람 친화 라벨로 매핑. 사용자 언어 설정에 따라 동적 변환 (`cafe24Catalog` / `makeshopCatalog` dict).

**왜 cafe24·makeshop 만 operations 를 채워 반환하나** — http/database/email 등은 활동 로그 `apiLabel` 이 NULL 이라 catalog lookup 자체가 발생하지 않는다 (frontend 의 §4.6 fallback 이 endpoint subtext 한 줄로 처리). 빈 배열을 응답해 endpoint 자체는 호출 가능하게 유지 — 향후 다른 서비스가 catalog 를 가질 때 추가 분기만 채우면 되도록. cafe24·makeshop 은 backend 가 operation 메타데이터를 보유해 `operations[]` 를 채운다.
...
**현행 (V045+)**: `mall_id` 가 plain 컬럼 (`integration.mall_id`) 으로 분리되어 — `credentials.mall_id` (encrypted JSONB) 와 동일 값을 plain 컬럼으로 복제 — SQL WHERE 절로 직접 필터링·UNIQUE 제약 강제가 가능. 통일 store-identifier 부분 UNIQUE 인덱스 `(workspace_id, service_type, mall_id) WHERE mall_id IS NOT NULL` (V072) 가 같은 workspace 내 같은 (service, mall) 중복 통합 생성을 SQL constraint violation 으로 거부 (TOCTOU race 차단). begin 핸들러는 in-memory 사전 체크 (connected → 409 / pending → reuse 분기 판단) 와 함께 SQL UNIQUE 를 backstop 으로 사용 — 두 검사를 모두 통과한 동시 INSERT 는 `23505 unique_violation` 으로 변환되어 같은 409 응답을 받는다.
...
1. 같은 mall_id 의 cafe24 row 들 조회 (통일 store-identifier UNIQUE (V072) 로 workspace 당 1건, 통일 lookup `(service_type, mall_id)` 로 O(1) 조회 — 보통 1~2건).
...
비용: O(N) HMAC verify (회복 분기에서만, 정상 흐름 zero impact). 옛 폐기된 "100건 mall_id 스캔 + trial HMAC" (Rationale "install_token 을 App URL path 식별 키로 승격" 항 참조) 과 형태는 비슷하나 (a) 호출 빈도가 낮고 (404 fallback only), (b) **같은 workspace 안에서는** 통일 store-identifier partial UNIQUE `(workspace_id, service_type, mall_id) WHERE mall_id IS NOT NULL` (V072) 가 같은 (cafe24, mall_id) row 를 최대 1개로 제한하며, 회복 분기 스캔이 workspace 횡단이라도 같은 mall_id 를 둘 이상 workspace 에서 동시 사용하는 케이스는 드물어 N=1~2 가 실무 값 ("구조적 상한 N≤2" 가 아니라 workspace-scoped 1개 보장 + 실무적으로 소수). 정상 식별은 여전히 install_token 단일 row 조회.

**TOCTOU 부재**: 회복 분기는 SELECT + HMAC verify 만 수행하는 read-only 조회로 INSERT/UPDATE 가 없어 race 자체가 발생하지 않는다. begin 핸들러의 통일 store-identifier partial UNIQUE backstop (`CAFE24_PRIVATE_APP_ALREADY_CONNECTED` Rationale 참조) 은 INSERT 단계의 동시 신청 차단을 담당하는 보완 보증이며, 본 분기와는 다른 시점의 보증.
...
**알림 정책 (§11.2)**: `integration_expired` (passive) 알림은 `expired` 전이 중에서도 `token_expired` 경로(refresh_token 없는 provider)에만 발사. `install_timeout` 도 `expired` 전이지만 별도 결정으로 미발사 — 아래 ["install_timeout 알림 미발사"](#install_timeout-알림-미발사) 항 참조. `error(auth_failed/insufficient_scope/network)` 전이는 active `integration_action_required` 알림으로 통지(§11.2 표) — passive/active 분리 원칙으로 **결정·구현 완료**.

### `isRefreshCapable` — makeshop 포함 결정

만료 스캐너의 refresh-capable 판별(`isRefreshCapable`)은 cafe24 뿐 아니라 **makeshop 도 포함**한다 (`service_type ∈ {cafe24, makeshop}` AND `credentials.refresh_token` 존재).

- **(a) 거짓양성 격하 방지**: cafe24·makeshop 모두 refresh_token 을 보유하므로 access_token(만료 짧음)만 만료된 정상 통합을 0d 분기가 `expired` 로 격하하면, 자동 갱신으로 멀쩡히 동작 중인 통합이 불필요한 재연결 요청을 받는 UX 회귀가 생긴다. 격하 제외로 방지.
- **(b) makeshop 은 배경 큐 불필요**: cafe24 는 0d 에 `cafe24-token-refresh` 큐로 safety-net refresh 를 걸지만, makeshop 은 in-call proactive (`ensureFreshToken`) + reactive_401 자가 회복으로 갱신되므로 배경 enqueue 가 필요 없다 (refresh_token TTL 30~90일).
- **(기각 대안)** makeshop 을 스캐너에서 cafe24 와 다르게 취급해 `expired` 격하 유지 — 정상 동작 통합의 거짓 만료 알림·재연결 요청을 유발하므로 기각.
...
Public 흐름은 begin 단계에서 Integration row 를 만들지 않으므로 통일 store-identifier partial UNIQUE 가 발사되는 시점이 `POST /api/integrations` finalize 단계로 미뤄진다. 사용자가 Cafe24 동의 페이지까지 마친 뒤에야 충돌이 드러나고, `IntegrationsService.throwIfUniqueViolation` 의 옛 분기는 `integration_workspace_name_unique` 만 처리해 store-identifier UNIQUE 위반은 raw `QueryFailedError` → 500 으로 빠지던 UX 결함이 있었다.
...
- **race backstop 확장** — `throwIfUniqueViolation` 이 통일 store-identifier UNIQUE `idx_integration_workspace_service_mall` (V072) 위반을 serviceType 별 코드로 분기 변환. begin pre-check 통과 후 동시 INSERT race / finalize 시점 충돌도 동일 409 코드로 변환.

**다른 status (`pending_install`/`expired`/`error`) 가 begin 단계에서 차단되지 않는 이유**:

- `pending_install` 은 Private 흐름의 idempotent begin 정책 (같은 row 를 reuse 해 install_token 보존) 과 호환되어야 한다 ([CAFE24_PRIVATE_APP_ALREADY_CONNECTED 의 mall_id 비교 경로](#cafe24_private_app_already_connected-의-mall_id-비교-경로) 항 참조). Public 흐름은 begin 단계에서 row 를 만들지 않으므로 pending_install 이 있더라도 begin 자체는 무영향 — 통일 store-identifier UNIQUE 가 finalize 단계에서 차단.
- `expired`/`error` 는 사용자의 재연동 의도를 반영해 begin 진입 자체는 허용하되, 한 workspace 안에서 같은 (service_type, mall_id) 의 통합이 최대 1행이라는 invariant 는 통일 store-identifier partial UNIQUE 가 finalize 단계에서 보장 (사용자는 기존 행을 먼저 삭제해야 새 통합 등록 가능).
- 결과적으로 모든 비-connected status 의 race / 충돌은 finalize 의 통일 store-identifier UNIQUE backstop 이 동일 409 코드로 변환 → 클라이언트는 단일 분기.
...
**O(N) 폐기와의 관계** — [install_token 을 App URL path 식별 키로 승격](#install_token-을-app-url-path-식별-키로-승격) 항에서 폐기된 "전방위 O(N) mall_id 스캔 + HMAC trial" 패턴과 본 endpoint 는 다르다. precheck 는 plain mall_id 컬럼의 통일 인덱스 lookup (통일 UNIQUE `(workspace_id, service_type, mall_id)` (V072) 또는 통일 lookup `(service_type, mall_id)`) 으로 O(1) row 만 가져온다. legacy `mall_id IS NULL` fallback 만 backfill 완료 전 임시로 추가 쿼리 발행 — 향후 backfill 종료 시 제거된다 (구현 코드 주석 `findAllCafe24RowsForMall` 참조).
...
### Cafe24/MakeShop pending_install 자동 폴링 — 수동 새로고침 서술의 대체

§3.2 Cafe24 Private step 7 은 본래 "사용자는 통합 목록에서 상태를 확인한다 (새로고침 또는 연결 상태 배지)" 로 수동 확인을 서술했다. 그러나 install-first 흐름에서는 OAuth 팝업 복귀 메커니즘(§3.5 `postMessage`)이 구조적으로 동작하지 않는다 — Cafe24 Developers "테스트 실행" 과 MakeShop ShopStore 설치 모두 외부 사이트가 자체 창에서 흐름을 시작하므로, 그 창의 `window.opener` 는 우리 부모창이 아니라 외부 탭이다. 사용자가 외부 작업을 마치고 돌아와도 우리 화면은 완료를 알 수 없어, 수동 새로고침 의존은 "성공했는데 화면은 여전히 대기 중" 인 dead-end UX 를 만든다.

이에 설정/설치 안내 화면이 해당 Integration row 를 **3초 간격 폴링**하는 상태기계로 대체했다: `connected` 전이 시 토스트 + 쿼리 invalidate + 상세 페이지 자동 라우팅, **10분 soft timeout** (백엔드 row 는 유지, 재진입/window focus 시 폴링 재개 — install TTL 24h 보다 훨씬 짧은 UI 측 안내일 뿐 상태를 바꾸지 않는다), callback 실패 기록 시 `last_error`/`status_reason` 진단 표시. 3초는 외부 콘솔 왕복(수십 초~수 분 단위 작업) 대비 충분히 즉각적이면서 단일 row GET 폴링이라 부하가 무시 가능한 값. MakeShop 훅은 추가로 `status_reason` 을 알려진 사유만 i18n 키로 매핑하고 미지의 사유는 generic 메시지로 fallback 해 raw 백엔드 에러 텍스트 노출을 차단한다 (§5.9).
