...
> `pending_install` 은 Cafe24 Private 앱 및 MakeShop ShopStore install-first 통합에서 사용하는 상태. 이 상태의 Integration 은 노드·AI Agent 에서 사용할 수 없다 (`INTEGRATION_INCOMPLETE` — §4.2). Cafe24 Private 는 사용자가 Cafe24 에서 "테스트 실행" 을 완료해야 `connected` 로 전이하고, MakeShop 은 상점에서 ShopStore 앱 설치가 완료돼야 전이한다. callback 시도가 실패해도 status 는 보존되어 재시도가 가능하며, 24시간 내 성공하지 못하면 `expired` 로 자동 전이된다 (install_timeout — `install_token` 도 NULL 로 소거).
