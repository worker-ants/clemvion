...
| `HTTP_BLOCKED` | HTTP target host 가 사설/loopback/클라우드 메타데이터라 SSRF 가드에 차단 (전 인증 방식 공통, refactor 04 C-3. 기본 ON, `ALLOW_PRIVATE_HOST_TARGETS` opt-out) | HTTP 노드 `error` 포트 출력 |
