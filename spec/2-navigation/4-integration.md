...
| `pending-install-ttl` | `status='pending_install' AND COALESCE(install_token_issued_at, created_at) < now-24h` (Cafe24 Private 한정) | `status='expired', status_reason='install_timeout', install_token=NULL` 으로 bulk UPDATE |
...
**`error(*)` 전이는 알림 미발사** (2026-05-16 명시): `integration_expired` 알림은 `expired` 전이 (token_expires_at 만료 또는 `pending_install` 24h TTL) 에만 발사한다. refresh 실패로 `error(auth_failed)`, transport 3회 실패로 `error(network)`, 403 + scope 시그널로 `error(insufficient_scope)` 전이는 별도 알림을 발사하지 않으며 UI 배지 (사이드바 카운트 + 목록 카드 뱃지) 와 노드 에디터 경고 (§7.3) 로만 통지한다. 향후 `error` 도메인 알림이 필요해지면 `Notification.type` 에 `integration_action_required` 같은 별도 타입을 신설한다.
...
**결정**: `error(auth_failed)` 채택. 옛 `expired (refresh_failed)` 분기 폐기. `expired` status 는 이제 `pending_install → expired (install_timeout)` 한 경로만 사용.
