...
**Rate Limit 정책**: MakeShop 은 data-call rate limit 을 공개 문서화하지 않았다 (토큰 발급만 client_credentials 한정 5회/분). `MakeshopApiClient` wrapper 가 429 응답의 `Retry-After` best-effort backoff 로 최대 2회 재시도 ([MakeShop 노드 §9.7](../4-nodes/4-integration/5-makeshop.md#97-미확인-항목-구현-전-검증-필요)).
