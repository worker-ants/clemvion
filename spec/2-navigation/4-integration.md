...
| `[Request scopes]` 버튼 | 체크된 추가 scope와 함께 `POST /api/integrations/:id/request-scopes` 호출. 응답 분기: ① 일반 OAuth provider(Google/GitHub/Cafe24 Public) — `authUrl` 반환 → 팝업 OAuth → 성공 시 credentials.scopes 병합. ② **Cafe24 Private** — popup 진입점 없음. 응답: `{ mode: 'cafe24_private_pending', integrationId, appUrl, callbackUrl, scopesAdded: [...] }` + 사용자 안내 "Cafe24 Developers 의 앱 권한 설정에서 추가 scope 를 활성화한 뒤 '테스트 실행' 을 다시 누르면 새 token 으로 갱신됩니다." (Private 은 우리 서버가 OAuth 를 시작할 수 없으므로 사용자가 Cafe24 측에서 권한 추가 후 재호출 — Rationale "Cafe24 Private request-scopes 흐름" 항 참조). |
...
- 실패: `{ code, message, details? }`
  ...
  - `CAFE24_INSTALL_INVALID_TOKEN` (404) — App URL 의 `install_token` 미존재 (통합 삭제 또는 24h TTL 만료로 소거). callback 성공만으로는 소거되지 않음 (post-install navigation 의 식별 키로 보존).
...
> 만료 스캐너는 **세 개의 독립 BullMQ job** (`connected-expiry` / `pending-install-ttl` / `usage-log-prune`) 으로 운영된다 — 각 job 은 자체 retry (`attempts: 3`, 60s exponential backoff) 와 큐 메트릭을 가지므로 한 패스의 실패가 다른 패스의 실행을 막지 않는다. Cafe24 Private 의 `pending_install` 24h TTL 만료는 `pending-install-ttl` job 이 담당. 상세 흐름·격리 정책은 [data-flow §1.4](../data-flow/integration.md#14-oauth-만료-스캐너-bullmq-integration-expiry) 참조.
