...
| 더보기(⋮) | 4 항목 드롭다운: ① 상세 보기 (드로어 오픈, 모든 역할 가시 — 메타·인증·Schedule·Recent Calls 등 모든 카드 노출), ② 활성/비활성 토글 (`editor`+), ③ 호출 이력 (**별도 Dialog** 로 Recent Calls **만** 표시 — 메타·인증·EIA·Schedule 카드는 노출되지 않음. 모든 역할 가시. Rationale R-6 참조), ④ 삭제 (`editor`+, [§4 확인 다이얼로그](#4-삭제-정책)). Schedule 타입은 추가로 "스케줄 관리에서 편집" 항목 표시 (→ `/schedules?triggerId=…` 딥링크). 트리거 "수정" 은 별도 항목이 아니라 ①의 드로어 안에서 [§2.3.1 필드 권한 매트릭스](#231-필드-권한-매트릭스) 의 카드별 edit 토글로 수행 |
...
- **상세 보기** — 메타 / Webhook 인증 / External Interaction / Schedule / Recent Calls 등 **모든 카드** 가 한 화면에 보이는 풀 detail drawer (`SlideDrawer`, 우측 슬라이드). 편집·cron 확인·EIA 설정 등이 목적.
