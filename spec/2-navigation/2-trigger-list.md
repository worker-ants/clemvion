...
| Webhook Configuration | `endpointPath` | edit | `PATCH /api/triggers/:id { endpointPath }`. 변경 시 옛 URL 은 즉시 404 — 다이얼로그 경고 후 진행. `(endpoint_path)` UNIQUE(전역 — 다른 워크스페이스의 트리거와도 겹칠 수 없다) 위반, **또는 다른 워크스페이스가 예약한 경로**(지웠거나 바꾼 경로 — [데이터 모델 §2.8.1](../1-data-model.md#281-webhookendpointreservation)) 시 409 `RESOURCE_CONFLICT` (세부 코드 `TRIGGER_ENDPOINT_PATH_CONFLICT`) |
...
> `(endpoint_path)` UNIQUE(전역 — 다른 워크스페이스의 트리거와도 겹칠 수 없다) 위반, **또는 다른 워크스페이스가 예약한 경로**(지웠거나 바꾼 경로 — [데이터 모델 §2.8.1](../1-data-model.md#281-webhookendpointreservation)) 시 409 `RESOURCE_CONFLICT` (세부 코드 `TRIGGER_ENDPOINT_PATH_CONFLICT`, `details.field='endpoint_path'`). 길이/이름 검증 실패는 400 `VALIDATION_ERROR` ([Spec 에러 처리](../5-system/3-error-handling.md)).
