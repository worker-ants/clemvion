...
내부 ref (`botTokenRef`, `secretTokenRef`) 는 사용자에게 노출하지 않음 — `hasBotToken: boolean` 만 응답에 포함. UI 매트릭스에서도 표기 X (보안 — CCH-SE-03 의 UI 차원 적용).
...
| POST | /api/triggers/:id/chat-channel/rotate-bot-token | Chat Channel bot token rotation (24h grace). webhook HMAC secret rotate (`/auth/rotate-secret`) 와 대상 secret 이 다름 (외부 provider bot token vs HMAC signing secret). 본 endpoint 는 [Spec Chat Channel §5.4](../5-system/15-chat-channel.md#54-bot-token-rotation-api-응답-계약) 가 single-path SoT — PATCH body 의 `config.chatChannel.botTokenRef` 직접 변경은 차단됨 ([R-CC-10](../5-system/15-chat-channel.md#r-cc-10-bot-token-변경-single-path-rotate-api-only-2026-05-23)) |

> `PATCH /api/triggers/:id` 본문은 다음 부분 갱신 키를 받는다 (모두 optional): `name`, `isActive`, `endpointPath`, `config` (Deep merge — `config.authType` / `config.hmacHeader` / `config.hmacSecret` / `config.bearerToken` / `config.notification` / `config.interaction` / `config.chatChannel.uiMapping` / `config.chatChannel.rateLimitPerMinute` / `config.chatChannel.languageHints` 등 서브 키 단위 부분 갱신). **단, `config.chatChannel.botTokenRef` 는 PATCH 로 변경 불가** — single-path 정책에 따라 `POST /api/triggers/:id/chat-channel/rotate-bot-token` 사용. 위반 시 400 `VALIDATION_ERROR` (`details.field='botTokenRef'`). 상세 [Spec Chat Channel §5.4.1](../5-system/15-chat-channel.md#541-bot-token-변경-single-path-정책).
...
내부 ref (`botTokenRef`, `secretTokenRef`) UI 미노출 정책은 [CCH-SE-03](../5-system/15-chat-channel.md#34-신뢰성--보안) 의 UI 차원 적용 — secret store ref 는 backend 내부 식별자로 사용자에게 노출 가치 없음. UI 는 `hasBotToken: boolean` 만 받아 "등록 됨 / 안 됨" 표시.
