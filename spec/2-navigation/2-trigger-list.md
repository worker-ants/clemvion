...
| POST | /api/triggers/:id/notification/rotate-secret | Outbound notification HMAC secret 회전. 응답 `{ secret, rotatedAt }`. SoT 는 [Spec EIA §7.1](../5-system/14-external-interaction-api.md#71-trigger-엔티티-확장) |
| POST | /api/triggers/:id/interaction/revoke-token | per_trigger inbound interaction 토큰 (itk_*) revoke. SoT 는 [Spec EIA §7.3](../5-system/14-external-interaction-api.md#73-interactiontoken-in-memory--redis) |
...
- EIA notification secret 은 외부 수신자가 보유한 키를 사전에 교체 동기화하기 위한 grace 기간이 필요 ([Spec EIA §7.1](../5-system/14-external-interaction-api.md#71-trigger-엔티티-확장)) — 단순 입력 교체는 grace 가 없어 즉시 단절.
