...
발견 경로: 사용자 요청 (2026-05-29) — "/triggers 목록 유형 다음에 인증 항목 추가 + 외부 노출 webhook 무인증 시 경고 아이콘". 사전 일관성 검토 `review/consistency/2026/05/29/00_03_34/SUMMARY.md` 의 C-1 (R-15 cross-file ID 충돌) 은 파일별 독립 Rationale 번호 컨벤션 (R-2 는 7개 파일, R-3/R-5 는 각 4개 파일에서 재사용) 에 비추어 검증된 false positive 로 판단, 본 파일 순번 `R-15` 를 유지한다.

### R-16. drawer 안 `isActive` 는 read-only 배지, 편집은 §2.1 ⋮ 행 액션 단일 경로 (2026-05-29)

§2.3.1 매트릭스의 Overview `isActive` 행을 `edit (토글 버튼)` → `read-only (배지)` 로 정정한다. 상세 drawer 는 활성 상태를 배지로만 표시하고, 활성/비활성 **전환 컨트롤**은 [§2.1](#21-트리거-목록-항목) 목록의 ⋮ 행 액션 "활성/비활성 토글" 한 곳에서만 제공한다.

근거:

- **구현 정렬**: 구현이 이미 이 형태로 출시됐다 — detail drawer 는 `isActive` 를 read-only Badge 로 렌더하고, 토글은 목록 ⋮ 행 액션 (→ `PATCH /api/triggers/:id { isActive }`) 으로 동작한다. 기존 매트릭스의 `edit (토글 버튼)` 문구만 shipping 상태와 불일치했으므로 spec 텍스트를 현실에 맞춘다 (추가 구현 없음).
...
- **R-4 와의 구분**: [R-4](#r-4-isactive-편집-경로를-patch-body-와-toggle-양쪽-모두-유지-2026-05-22) 는 **API 편집 경로** (PATCH body + `/toggle`) 이원화 결정이고, 본 R-16 은 **drawer UI 표현** 결정이다. 두 축은 독립 — API 계약상 PATCH body `{ isActive }` 경로는 그대로 살아 있다.

대안:

1. **(채택) drawer read-only 배지 + ⋮ 행 액션 단일 편집 경로**: shipping 동작과 일치, 추가 구현 0, 편집 진입점 단일화로 mental model 단순.
2. **(기각) drawer 안 inline 토글 신규 구현 (기존 `edit (토글 버튼)` 명세 유지)**: shipping 동작과 불일치를 코드 추가로 메우는 방향. ai-review (INFO-7) 가 버그가 아닌 "spec 회색지대" 로 분류한 사안에 UI 구현 비용을 투입할 근거가 약하다.

발견 경로: ai-review INFO-7 (`review/code/2026/05/22/15_08_07/requirement.md`) 가 spec ↔ 구현 회색지대로 지적 → `plan/in-progress/spec-fix-isactive-drawer-toggle.md` 에 Option A/B 로 정리 → 사용자 결정 (2026-05-29) 으로 Option B 채택. 사전 일관성 검토 `review/consistency/2026/05/29/08_45_29/SUMMARY.md` (BLOCK: NO). R-16 번호는 `spec/6-brand.md` 의 R-16 과 중복되나 파일별 독립 Rationale 번호 컨벤션상 허용 (cross-file 인용 시 파일 경로 동반 표기).
