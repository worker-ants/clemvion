...
| 호출 이력 테이블 | 대상 트리거명, 상태, **소스 IP**, **응답 코드**, 시각 (최근 20건) | ✅ `recentCalls` 가 `triggerName` / `status` / `sourceIp` / `responseCode` / `startedAt` 반환. 소스 IP(`Execution.source_ip`)·응답 코드(`Execution.response_code`)의 캡처·저장 규칙은 [데이터 모델 §2.13](../1-data-model.md#213-execution) 이 SoT — 화면 표시는 미캡처(비-HTTP 트리거) 소스 IP 를 `—`, 응답 코드가 없으면 워크플로 `status` enum 으로 폴백한다 (R-6, WH-MG-05) |
...
| GET | /api/model-configs/:id/models | 사용 가능한 모델 목록 조회 (chat/embedding). 쿼리 `?type=chat\|embedding`(선택) 으로 타입 필터 — 허용값 외 `type` 은 `400 Bad Request`(컨트롤러 `ParseEnumPipe` 강제) **(Viewer+ — 조회)** |
