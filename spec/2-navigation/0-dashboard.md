...
> **경로 표기 규약**: 본 문서의 `(main)` 경로(`/dashboard`·`/workflows`·`/workflows/:id/executions/:executionId` 등)는 활성 워크스페이스 기준 `/w/<slug>/...` 로 렌더된다(URL slug = FE 라우팅 SoT, [_layout §2.2](./_layout.md#22-메뉴-항목)). 에디터 canvas 도 슬러그 라우팅 phase 2 부터 `/w/<slug>/workflows/:id` 로 렌더된다.
