...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | /api/schedules | 목록 조회 (쿼리: page, limit, search, sort, order — sort/order 는 `findAll` 이 whitelist 기반으로 반영, 기본 `created_at DESC`). 페이지네이션 응답 형식은 [API 규약 §5.2](../5-system/2-api-convention.md#52-목록-응답) 준수 |
...
- **trigger → schedule** (`/schedules?triggerId=`): 해당 스케줄 행을 **강조·스크롤**만 한다. 스케줄은 triggerId 단건 조회 엔드포인트가 없고 목록은 페이지네이션되므로, 편집 다이얼로그를 열려면 그 스케줄이 현재 페이지에 로드돼 있어야 한다. 강조는 로드된 행에 대해서만 동작하고 없으면 조용히 no-op 한다. cross-page 포커스·자동 편집 오픈은 서버 목록에 `triggerId` 필터를 신설해야 가능한 후속이다.
