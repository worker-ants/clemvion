...
| `<ImplAnchor>` | reverse-evidence 가드용 — 가이드 본문이 약속한 UI/API surface 가 코드에 존재함을 build-time 으로 검증. props: `kind` ∈ `{ui-entry, component, api-endpoint, e2e-scenario}` · `file` (레포 루트 기준 상대경로) · `symbol` (grep 대상) · `describes`. 사용자 view 에서 hidden 렌더. `kind="api-endpoint"` 는 추가로 NestJS HTTP route 데코레이터(`@Post`/`@Get`/`@Put`/`@Patch`/`@Delete`)와 `describes` 의 `METHOD /path` 가 controller 파일에 존재함을 검증. SoT: [`spec/conventions/user-guide-evidence.md`](../conventions/user-guide-evidence.md). 가드: `impl-anchor-existence.test.ts` / `integrations-coverage.test.ts` / `triggers-coverage.test.ts` |
