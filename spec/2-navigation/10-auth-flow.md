...
> **이미 로그인한 사용자의 진입 분기** — 초대 메일 링크(`/auth/register?invitationToken=…`)는 미가입자를 기준으로 하지만, 이미 로그인한(다른 계정을 가진) 사용자가 같은 브라우저의 새 탭에서 클릭할 수도 있다. 이 경우 회원가입 폼을 보여주는 대신 초대 수락 확인 페이지 `/invitations/accept?token=…`([Spec 인증/인가 §1.5.3](../5-system/1-auth.md#153-흐름-이미-가입한-사용자가-다른-워크스페이스에-초대된-경우))로 즉시 리다이렉트한다. `(auth)` 라우트 그룹에는 세션 하이드레이션(AuthProvider)이 없어 클라이언트 스토어로는 기존 로그인을 감지할 수 없으므로, [§7 인증 상태 관리](#7-인증-상태-관리)의 `has_session` 힌트 쿠키(로그인 시 심고 로그아웃 시 지움)로 판정한다. 힌트가 stale(쿠키만 남고 refresh 만료)이면 accept 페이지의 라우트 가드가 정상적으로 로그인 화면으로 되돌린다. 아래 표(1~7)는 로그인하지 않은 미가입자 경로에만 적용된다.
