...
code:
  ...
  # §3 이 약속하는 "URL slug = FE 라우팅 SoT" 의 구현 — catch-all 흡수/terminal + 링크 생성
...
> 관련 문서: [PRD 내비게이션](./_product-overview.md#312-user-profile-사용자-프로필) · [Spec 레이아웃](./_layout.md) · [Spec 인증/인가](../5-system/1-auth.md) · [데이터 모델 - User](../1-data-model.md#21-user) · [데이터 모델 - Workspace](../1-data-model.md#22-workspace)
...
- **cold-load reconcile = URL 우선**: `[slug]` layout mount 시 resolved-id ≠ 활성 id 면 URL 에 맞춰 `switchWorkspace` 로 재조정한다(store 우선 아님). 무효/비멤버 slug 는 default 워크스페이스로 redirect — **UX 편의이며 인가 경계가 아니다**(강제 지점은 backend `RolesGuard` 403 — 라우트 `@Roles()` 유무와 무관하게 워크스페이스 멤버십을 검증한다. [data-flow §Rationale "멤버십 검증은 가드 1곳에서"](../data-flow/12-workspace.md)). 구 무-slug 경로·알림 딥링크·`/`는 `(main)/[...rest]` catch-all 이 활성 slug 로 흡수한다(query/hash 보존). 단 **이미 `/w/` 로 시작하는 경로는 흡수 대상이 아니다** — 재부착하면 매칭되지 않는 경로가 한 세그먼트씩 길어지는 무한 리다이렉트가 되므로 catch-all 이 terminal 로 처리한다(`/w/<slug>` 단독은 dashboard forward, 그 외는 `notFound()`). 상세·기각 대안은 [_layout.md §2.2 각주 · R-3](./_layout.md).
...
| GET | /api/workspaces/:id/settings | 워크스페이스 설정 조회 (**멤버 read** — viewer 포함, 설정 화면 표시용). 응답 `{ interactionAllowedOrigins: string[]; timezone?: string }`. 비-멤버 403 `NOT_A_MEMBER` |
