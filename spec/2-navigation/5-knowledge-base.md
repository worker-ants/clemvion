...
`embedding_dimension == null` 인 KB 는 RAG 검색에서 제외되는데(저장 청크가 현재 모델과 차원/공간이 불일치할 수 있어 stale 검색 방지 — [RAG 검색 §3.1·§5](../5-system/9-rag-search.md#5-임베딩-모델-일관성)), 이 상태가 되면 AI 에이전트의 KB 검색이 **경고 없이 0건**이 된다. 특히 임베딩 모델을 바꾸고 재임베딩을 잊은 KB(`reembed_status='idle'` + dimension NULL)는 영구히 조용한 검색불가 상태가 된다. §2.2 의 "임베딩 모델 변경 경고"는 **편집 폼**에서 모델을 바꾸는 *시점*에만 보이므로 사후 발견을 보장하지 못한다. 따라서 KB **목록 카드**에 검색불가 상태를 노출해 사용자가 회귀를 즉시 인지하고 재임베딩으로 조치하게 한다. 에이전트 측 대응(검색 결과로 "재임베딩 필요" 신호 전달)은 [RAG 검색 §2.2·§6](../5-system/9-rag-search.md#6-에러-처리)에서 정의한다. 근본 원인(모델 변경 시 재임베딩 미트리거)의 자동화는 비용·UX 정책 결정이 더 필요해 별도 후속([`kb-model-change-reembed-followup.md`](../../plan/in-progress/kb-model-change-reembed-followup.md))으로 분리한다.

### R-3. 상세 상단에 검색 불가 배너 + "지금 재임베딩" CTA 를 둔 이유

R-2 의 목록 카드 경고는 **발견**(어느 KB 가 검색불가인지)을 보장하지만, 사용자가 조치하려면 카드 → 상세 진입 → 진행 박스에서 재임베딩 경로를 스스로 찾아야 한다. 근본 원인 후속([`kb-model-change-reembed-followup.md`](../../plan/in-progress/kb-model-change-reembed-followup.md))의 정책 결정에서 **자동 트리거(비용 부담)·저장 차단(UX 마찰)** 대신 **③ 배너 강화**(발견 지점에서 1-클릭 조치)를 택했다. 검색불가는 모델 변경 후 *재임베딩을 잊은* 상태이므로, 조치 동선을 발견과 같은 화면에 붙이는 것이 비용·마찰을 늘리지 않으면서 구멍을 닫는 최소 개입이다.
