...
| rerank_mode | Enum | 검색 후처리(리랭킹) 모드: `off` (default) / `cross_encoder` / `cross_encoder_llm`. **검색 시점 적용 — 사후 변경 가능, 재임베딩 불요** ([Spec RAG 검색 §3.3](./5-system/9-rag-search.md#33-검색-후처리--리랭킹-선택적)). `off` 면 아래 rerank_* 컬럼 무시 (Planned) |
| rerank_config_id | UUID? | FK → RerankConfig. cross-encoder 리랭커 설정. NULL 이면 워크스페이스 default RerankConfig, 그것도 없으면 `off` 강등 (Planned) |
| rerank_candidate_k | Integer | 리랭크에 투입할 1차 회수 후보 수 (default 50). `rerank_mode = 'off'` 시 무시 (Planned) |
| rerank_score_threshold | Float? | 리랭크 점수 동적 컷 임계 (NULL 이면 컷 없이 점수순 정렬 후 top-k). `rerank_mode = 'off'` 시 무시 (Planned) |
| rerank_llm_config_id | UUID? | FK → LLMConfig. `rerank_mode = 'cross_encoder_llm'` 의 listwise grading LLM. NULL 이면 워크스페이스 default chat LLMConfig (Planned) |
| created_at | Timestamp | 생성 시각 |
| updated_at | Timestamp | 수정 시각 |
...
### 2.16.1 RerankConfig (Planned)

cross-encoder 리랭커 provider 설정. chat/embedding 과 API shape(전용 `/rerank` 엔드포인트)가 달라 §2.16 LLMConfig 와 분리한 sibling 리소스 ([Spec LLM Client §3.6](./5-system/7-llm-client.md), [Spec RAG 검색 §3.3](./5-system/9-rag-search.md#33-검색-후처리--리랭킹-선택적)). 셀프호스팅(`tei`/`local`) 또는 외부 API(`cohere`/`jina`/`voyage`) 를 지원하며, SSRF 가드·secret-store transformer 는 LLMConfig 와 동일 인프라를 재사용한다.

| 필드 | 타입 | 설명 |
|------|------|------|
| id | UUID | PK |
| workspace_id | UUID | FK → Workspace |
| provider | String | `cohere` / `jina` / `voyage` / `tei` (HF Text-Embeddings-Inference, 자가호스팅 bge-reranker) / `local` (OpenAI-compatible `/rerank`) |
| name | String | 사용자 지정 이름 |
...
| Integration | (workspace_id, mall_id) WHERE service_type='makeshop' AND mall_id IS NOT NULL UNIQUE | MakeShop 통합 중복 방지 — `mall_id` 컬럼에 투영된 `shop_uid` 기준. cafe24 partial UNIQUE 와 동일 패턴의 병렬 인덱스. `V071__integration_makeshop_workspace_mall_idx` 로 적용 완료 (`CREATE UNIQUE INDEX CONCURRENTLY`) |
