...
| name | String | 컬렉션 이름 |
| description | String? | 설명 |
| embedding_model_config_id | UUID? | FK → ModelConfig (kind=embedding). 임베딩 모델 설정. NULL 이면 워크스페이스 default embedding ModelConfig. 모델 식별자·provider·차원은 참조된 ModelConfig 가 소유 `(V091)` |
| embedding_llm_config_id | UUID? | **[LEGACY — PR4b 제거 예정]** PR2 점진적 전환의 폴백용 구 piggyback config. `embedding_model_config_id`·ws default kind=embedding 가 모두 없을 때만 `embedding_model` 문자열과 함께 사용 (FK → ModelConfig). 데이터 마이그레이션 선행이 필요해 PR4(V092) 의 `rerank_config` DROP 과 달리 존치 |
| embedding_model | text? | **[LEGACY — PR4b 제거 예정]** 구 임베딩 모델 문자열 (default `text-embedding-3-small`). 1급 경로에서는 ModelConfig.default_model 이 우선하며 본 컬럼은 legacy 폴백에서만 쓰인다 |
