...
User ──┬── Workspace (1:N)
       ...
       │       │       └── Execution (1:N)
       │       │               ├── NodeExecution (1:N)
       │       │               └── ExecutionToken (1:N, iext jti 추적 — V060)
       │       │
       │       ├── Integration (1:N)
       ...
       │       │
       │       ├── AgentMemory (1:N)
       │       ├── ModelConfig (1:N)   # kind: chat | embedding | rerank
       │       ├── LlmUsageLog (1:N)   # chat 계열 호출만 (§2.24)
       │       ├── AuthConfig (1:N)
       │       ├── AuditLog (1:N)
       │       ├── Notification (1:N)
       │       ├── SecretStore (1:N)
...
- 중첩 깊이 제한: 최대 5단계 (생성·부모 변경 모두에 적용)
- `parent_id` 는 **같은 워크스페이스**의 폴더만 가리킨다
- 계층은 **비순환(acyclic)** — 폴더는 자기 자신·자손을 부모로 가질 수 없다 (부모 변경 시 검증)
...
| Schedule is_active 변경 | 연결된 Trigger is_active도 동기화. 역방향도 동일 — `PATCH /api/triggers/:id { isActive }` 가 schedule.is_active 와 BullMQ job scheduler 를 함께 갱신 ([data-flow §1.4](./data-flow/10-triggers.md#14-schedule--trigger-동기화) 참조) |
...
| resume_call_stack | JSONB? | `NULL` 허용 (V087, **구현·사용 중** — exec-park D6, PR-B2b 2026-06-06). **중첩 sub-workflow(`executeInline`) 안의 blocking 노드**가 `waiting_for_input` 으로 park 할 때, 재개에 필요한 **executeInline 호출 체인**(outermost→waiting inner 직전, `{version, frames:[{workflowId, invokerNodeId, recursionDepth}]}`)을 commit 하는 durable resume 매체 — rehydration([§7.5](./5-system/4-execution-engine.md#75-resume-after-restart-rehydration))이 이 스택을 따라 `driveCallStackResume`/`driveResumeFrame` 으로 최내층 waiting 노드부터 frame-by-frame 재진입(bubble-up)해 top-level 까지 forward 한다. **`NULL` = top-level park(중첩 깊이 0) / park 한 적 없는 실행 / 배포 이전 row** → 단일 레벨 재개. 컨테이너(Loop/ForEach/Map/Parallel) body blocking 은 [§3.2](./5-system/4-execution-engine.md) 금지라 선형 스택만(iteration/branch 상태 없음). `version` 은 `CALL_STACK_SCHEMA_VERSION`(checkpoint 와 독립). 상세: [실행 엔진 §6.2/§7.5/§Rationale(exec-park D6)](./5-system/4-execution-engine.md#62-저장-전략) |
| source_ip | Varchar(45)? | `NULL` 허용 (V096). webhook/chat-channel 트리거 발화 시 `hooks.service` 의 `extractClientIpFromHeaders`(CF-Connecting-IP 신뢰 시 → X-Forwarded-For 첫 IP, 헤더 기반·`req.ip` 폴백 없음) 결과. 비-HTTP 트리거(schedule)·수동 실행·배포 이전 row 는 NULL. 인증 설정 사용 내역([config §A.3](./2-navigation/6-config.md))의 호출 이력 "소스 IP" 컬럼 소스. 길이 45 = IPv6 표기 최대(IPv4-mapped 포함) |
| response_code | Varchar(10)? | `NULL` 허용 (V096). webhook 호출이 받는 **실제 HTTP 응답 코드** 문자열 (execution 생성 성공 경로 = `'202'` Accepted; 인증/검증 실패는 execute 전 throw 라 row 미생성). 비-HTTP 트리거는 HTTP 코드가 없어 NULL → `getUsage` 가 워크플로 `status` enum 으로 폴백 표시. [config §A.3](./2-navigation/6-config.md) 호출 이력 "응답 코드" 컬럼 소스 ([WH-MG-05](./5-system/12-webhook.md) 이행) |
| single_node_id | UUID? | `NULL` 허용 (V098). 단일 노드 실행([3-execution §1.3](./3-workflow-editor/3-execution.md#13-단일-노드-테스트))의 대상 노드 id. 세팅되면 엔진(`runExecution`)이 그 노드만 실행하고 downstream 으로 진행하지 않는다(§1.2 부분실행과 구분). NULL = 일반/부분 실행. `_node_id` 접미사가 Node FK 도메인을 표기, `single_` 한정자는 §1.2 `fromNodeId`(컬럼 아님, `input_data` 경유)와 구분 — `dry_run`/`re_run_of` 와 동일한 mode-encoding 컬럼. 명시적 FK 제약·인덱스 없음(re_run_of 선례; 디버그 전용이라 조회 패턴 없음). `dry_run` 과 직교 — 단일 노드 실행은 항상 `dry_run = false`(두 모드 조합 미지원, v1) |
| previous_execution_id | UUID? | `NULL` 허용 (V098). 단일 노드 실행 시 입력 seed 출처가 되는 직전 실행 id — 그 실행의 직속 predecessor `NodeExecution.output_data` 를 현재 실행 context 에 복원해 상류 출력을 자동 주입한다. NULL = seed 없음(수동 입력만). `re_run_of`(re-run 파생 부모)와 의미가 다르다 — 입력 주입 참조일 뿐 chain 관계가 아니다. API 응답 DTO 미포함(whitelist 매핑 자동 배제) |

> **AuthConfig 호출 집계 경로 (SoT)**: 인증 설정 사용 내역(`GET /api/auth-configs/:id/usage`, [config §A.3](./2-navigation/6-config.md))의 `totalCalls`/`periodCounts`/`recentCalls` 는 **전용 로그 엔티티 없이** `Execution.trigger_id → Trigger.auth_config_id` 조인으로 산출한다 — 인증 설정에 연결된 트리거들의 `Execution` 행이 곧 "호출" 이다. `periodCounts` 는 `Execution.started_at` 의 롤링 윈도 조건부 집계(`COUNT(*) FILTER`), `recentCalls` 는 최근 20건의 `triggerName`/`status`/`source_ip`/`response_code`/`started_at`. (Integration 이 전용 `IntegrationUsageLog`(§2.10.1)를 두는 것과 달리, AuthConfig 의 "호출" 은 워크플로 실행과 1:1 이므로 `Execution` 을 재사용해 이중기록을 피한다.)
...
### 2.13.2 ExecutionToken (`execution_token`, V060)

External Interaction API 의 `iext_*`(per_execution JWT) 발급 jti 를 영속 추적한다. 토큰 인증 자체는 JWT stateless 검증이지만, **execution 종료 시 해당 execution 이 발급한 모든 토큰을 enumerate 해 일괄 revoke** 하려면 발급 jti 목록이 필요하기 때문이다 (terminal revoke at-least-once — [EIA §3.4 EIA-RL-06 / §7.3 / §9.3 R15](./5-system/14-external-interaction-api.md)). Revoke = jti 를 Redis blacklist 등재 + 본 row 삭제.

| 필드 | 타입 | 설명 |
|------|------|------|
| jti | TEXT | PK. JWT jti — Redis blacklist 키 |
| execution_id | UUID | FK → Execution (ON DELETE CASCADE) |
| issued_at | TimestampTZ | 발급 시각 (기본 `NOW()`) |
| exp_at | TimestampTZ | JWT exp(절대 시각). terminal blacklist 시 `(exp_at - now)` 를 Redis TTL 로 사용 — 만료된 jti 는 등재 불필요 |

**인덱스**: `idx_execution_token_execution_id (execution_id)` — execution 별 발급 토큰 회수(`revokeAllForExecution`) 및 reconciliation sweep(`execution_token ⋈ terminal execution`)용. 한 execution 은 refresh 흐름으로 여러 jti 보유 가능 (구 jti blacklist 후 신 jti 발급).

### 2.13.3 WorkflowTestDataset

워크플로우 에디터 Mock Input(테스트 입력)을 이름 붙여 저장·재사용하는 데이터셋 (V097). 권한·소유 모델 SoT: [워크플로우 실행 §2.2 / R-2.2](./3-workflow-editor/3-execution.md#r-22-테스트-데이터셋-저장--권한소유-모델-2026-06-14).

| 필드 | 타입 | 설명 |
|------|------|------|
| id | UUID | PK |
| workflow_id | UUID | FK → Workflow (ON DELETE CASCADE). 데이터셋이 귀속된 워크플로우 |
| owner_id | UUID | FK → User (ON DELETE CASCADE). 소유 유저 — 생성 시 항상 요청 유저, 수정/삭제 권한의 단일 기준 |
| workspace_id | UUID | FK → Workspace (ON DELETE CASCADE). 워크스페이스 격리·공유 목록 쿼리용 (workflow 의 workspace 비정규화) |
| visibility | Enum | `private`(기본, 소유자만) / `workspace`(워크스페이스 read-only 공유). 비소유자는 공유본을 clone 해 수정 |
| name | Varchar(255) | 데이터셋 이름 |
| data | JSONB | Mock Input JSON (`POST /workflows/:id/execute` body.input 으로 사용). **API surface 키는 `input`** — TransformInterceptor 의 top-level `data` 키 래핑 휴리스틱 충돌 회피 (DB 컬럼은 `data`, 엔티티/DTO 속성은 `input`) |
| created_at / updated_at | TimestampTZ | 생성/수정 시각 |

**제약**: `(workflow_id, owner_id, name)` UNIQUE — 한 유저가 같은 워크플로우에 같은 이름 중복 저장 금지 (위반 시 409 `DUPLICATE_NAME`).

**인덱스**: `(owner_id, workflow_id)` — 유저의 해당 워크플로우 데이터셋 조회. `(workspace_id, visibility)` — 워크스페이스 공유본 목록 필터.
...
워크스페이스가 사용하는 모든 AI 모델 설정(chat / embedding / rerank)을 `kind` 판별자로 단일 관리하는 테이블 `(V088)`. provider 자격증명·endpoint 를 가진 리소스이며, API Key 마스킹·SSRF 가드·secret-store transformer 를 kind 무관 공유한다. chat/embedding/rerank 의 API shape 차이(특히 rerank 의 `/rerank` 전용 호출)는 **실행 레이어 팩토리**(`LLMClientFactory` / `RerankClientFactory`)가 `kind` 로 분기해 흡수하고, 설정 테이블은 단일화한다 ([Spec LLM Client §4](./5-system/7-llm-client.md), [Spec 설정 §Part B](./2-navigation/6-config.md)).

> **용어 구분**: **ModelConfig** = 저장된 provider 설정 엔티티(DB row). **ModelInfo** = provider `listModels` 응답 항목(DTO, [LLM Client §3.5](./5-system/7-llm-client.md)). 접두어 `Model` 을 공유하나 별개다.

> **구현 상태**: chat(구 LLMConfig `V001`)·rerank(구 RerankConfig `V081`, `cross_encoder`·`cross_encoder_llm` 두 모드) 구현됨. embedding 1급화 + 단일 테이블 통합은 본 개정 `(V088~V092)`. 마이그레이션: `llm_config`→`model_config` rename + `kind`/`dimension` 추가 `(V088)`, is_default 유니크 `(workspace,kind)` 재정의 `(V089)`, rerank_config 흡수 `(V090)`, embedding 1급 파생 + KB repoint `(V091)`, **orphaned `rerank_config` 테이블 DROP `(V092)`** 으로 통합 정리됐다. (KB legacy embedding 컬럼 `embedding_llm_config_id`·`embedding_model` 은 **PR4b 에서 은퇴**됐다 — V093 repoint 로 모든 KB 를 1급 `kind=embedding` config 에 고정한 뒤 V094 가 두 컬럼을 비가역 DROP. [§2.11](#211-knowledgebase)·[임베딩 파이프라인 §5.5](./5-system/8-embedding-pipeline.md#55-임베딩-설정-해석-폴백-체인-resolveembedding).) chat row 는 UUID 보존이라 노드 `config.llmConfigId`·assistant·usage_log·extraction·rerank_llm 참조 무변경.
...
| base_url | String? | 커스텀 endpoint. local/tei/azure 는 필수 (SSRF 가드 — local/tei 사설망 예외, [LLM Client §5.5](./5-system/7-llm-client.md)) |
| default_model | String | 기본 모델 ID. rerank 예: `dragonkue/bge-reranker-v2-m3-ko` / `rerank-3.5` |
| default_params | JSONB | 기본 파라미터 (temperature, max_tokens 등). **chat 전용** — embedding/rerank 는 미사용 |
| dimension | Integer? | **embedding 전용** 벡터 차원 (예: 384/512/768/1024/1536/3072). pgvector 컬럼 차원과 결합된 **SoT** — `KnowledgeBase.embedding_dimension` 은 이 값의 파생 캐시. chat/rerank 는 NULL. embedding 연결 테스트 성공 시 probe embed 로 자동 감지·자동 저장된다 (상세 [Config §B.3](./2-navigation/6-config.md#b3-프로바이더-연결-테스트)) |
| is_default | Boolean | **`(workspace_id, kind)` 당 최대 1개** (partial unique). kind 별로 워크스페이스 기본 모델 |
| created_at | Timestamp | 생성 시각 |
| updated_at | Timestamp | 수정 시각 |

**참조 관계 (kind 별)**: chat → AI 노드 `config.llmConfigId`(JSONB, FK 없음)·`AssistantSession.llm_config_id`·[`llm_usage_log.llm_config_id`](#224-llmusagelog)·`KnowledgeBase.extraction_llm_config_id`·`KnowledgeBase.rerank_llm_config_id`. embedding → `KnowledgeBase.embedding_model_config_id`. rerank → `KnowledgeBase.rerank_config_id`.

#### Rationale (ModelConfig 통합)

- **단일 테이블 (kind 판별)**: chat/embedding/rerank 는 모두 "provider 자격증명 + 모델"이라는 동일 골격이고, 마스킹·SSRF·secret-store 인프라를 이미 공유한다. 구 LLMConfig/RerankConfig sibling 분리(API shape 차이 근거)는 **실행 레이어** 관심사일 뿐 설정 테이블을 쪼갤 이유가 아니었고, 그 결과는 CRUD·DTO·컨트롤러·프론트 페이지·i18n 의 통째 중복과 설정 화면 분산뿐이었다. `kind` 로 흡수해 관리 포인트(테이블 2+piggyback→1, 설정 화면 3→1)를 제거한다. ([Spec 설정 §Rationale R-3](./2-navigation/6-config.md) 의 sibling 분리 결정을 번복.)
- **임베딩 1급화**: 이전엔 embedding 이 chat 용 LLMConfig 를 빌려 썼다(같은 row 가 역할에 따라 chat/embedding). embedding 모델은 **차원이라는 고유 불변속성**(pgvector 컬럼 차원과 결합)을 가지므로 1급 row(`kind=embedding`, `dimension` 소유)가 정합적이다. provider 클라이언트는 기존 embed 경로(openai/azure/google/local) 재사용 — 신규 provider 추가 아님.
...
| ip_whitelist | String[]? | 허용 IP 목록 — 각 항목은 단일 IP 또는 CIDR 표기 (예: `10.0.0.0/8`, `2001:db8::/32`; 단일 IP 는 /32·/128 호스트로 취급). **저장(create/update) 시 각 항목을 형식 검증**하며 단일 IP/CIDR(IPv4·IPv6) 가 아니면 `400` 으로 거부한다 — 런타임 평가와 동일한 수용 기준. webhook 수신 시 `auth_config_id` 가 연결된 트리거에 한해 시행 ([Spec Webhook §3.2 WH-SC-09](./5-system/12-webhook.md#인증-및-보안)) |
| is_active | Boolean | 활성 상태. `false` 면 연결된 webhook 호출은 401 `AUTH_FAILED` |
| last_used_at | Timestamp? | 마지막 사용 시각. webhook 인증 성공 시 fire-and-forget UPDATE ([Spec Webhook §3.2 WH-SC-08](./5-system/12-webhook.md#인증-및-보안)) |
| created_at | Timestamp | 생성 시각 |
| updated_at | Timestamp | 수정 시각 |
...
- **ip_whitelist 저장 시점 형식 검증**: 무효한 IP/CIDR 가 DB 에 적재되면 런타임 평가에서 조용히 fail-closed 매칭 실패로만 드러나(=설정자가 의도와 다른 차단을 디버깅하기 어려움), 저장 시점에 형식을 검증해 `400` 으로 조기 반환한다. 검증 기준은 런타임 `AuthConfigsService.parseIp` 와 **동일한 `ip-address`(`Address4`/`Address6.isValid`)** 를 재사용 — DTO 가 통과시킨 값은 런타임이 항상 파싱 가능해 저장/평가 수용집합 drift 가 없다. class-validator 의 `@IsIP` 는 CIDR 를 거부하므로 단일 IP·CIDR 를 함께 허용하는 커스텀 `@IsIpOrCidr` 를 사용한다 (런타임 enforcement 요건은 [Spec Webhook §3.2 WH-SC-09](./5-system/12-webhook.md#인증-및-보안), 저장 검증의 SoT 는 본 §2.17).
...
| details | JSONB | 변경 상세 |
| ip_address | String? | 요청 IP (없을 수 있음 — IP 미파악·시스템 내부 액션) |
...
| ip_address | String? | 발급 시점 클라이언트 IP (`CF-Connecting-IP` 는 `TRUST_CF_CONNECTING_IP=true` 일 때만 1순위, 기본 off — [1-auth §2.3](./5-system/1-auth.md#23-세션-정책)) |
...
| resource_id | UUID? | 관련 리소스 ID (딥링크 대상 — 실패 알림은 workflow id) |
| background_run_id | UUID? | `background_failed` 알림의 per-run attribution 전용 키 (V107). 딥링크(resource_type/resource_id=workflow)와 분리 — Background 본문 모니터링 API 만 소비. REST 미노출(`select:false`). 상세는 [data-flow/8-notifications.md §2.1 / Rationale](./data-flow/8-notifications.md#21-postgres) |
...
> 관련 문서: [data-flow LLM Usage §1.3 Caller 카탈로그](./data-flow/7-llm-usage.md) — **적재 대상·attribution 채움 현황의 단일 진실** · [Spec LLM Client](./5-system/7-llm-client.md) · [통계 §2.5](./2-navigation/7-statistics.md)

`LlmService` 의 **chat 계열 호출(`chat` / `chatStream`)** 이 남기는 append-only 사용량 로그 (`LlmUsageLogService.record`, V014). `embed` 는 같은 facade 를 거치지만 적재하지 않고, `kind=rerank` 의 전용 `/rerank` 호출은 `LlmService` 를 거치지 않아 본 로그 밖이다. 통계 위젯·`llm_cost` 알림 룰의 소비 대상.

| 필드 | 타입 | 설명 |
|------|------|------|
| id | UUID | PK |
| workspace_id | UUID | FK → Workspace (CASCADE). 소유 워크스페이스 — 로그 라이프사이클의 기준 |
| workflow_id | UUID? | FK → Workflow (SET NULL). 비정규화 — `workflowId` 스코프 비용 집계용 |
| execution_id | UUID? | FK → Execution (SET NULL) |
| node_execution_id | UUID? | FK → NodeExecution (SET NULL) |
| llm_config_id | UUID? | FK → ModelConfig ([§2.16](#216-modelconfig), `kind=chat`) SET NULL. V014 생성 당시 대상 테이블명은 `llm_config` 였고 V088 rename 으로 참조 대상 이름만 바뀌었다 |
| provider | varchar(50) | `openai` / `anthropic` / `google` / `azure` / `local` |
| model | varchar(100) | 호출된 모델 ID |
| prompt_tokens | Integer | NOT NULL, 기본 0 |
| completion_tokens | Integer | NOT NULL, 기본 0 |
| total_tokens | Integer | NOT NULL, 기본 0 |
| thinking_tokens | Integer? | reasoning / thought 토큰 (OpenAI reasoning · Gemini 2.5). Anthropic 은 미보고 → NULL. **`cost_usd` 계산에 미포함** (V018) |
| cost_usd | Numeric(12,6)? | `pricing.ts` 단가표 기반 계산. 단가 미등재 모델은 NULL — 집계 `SUM(cost_usd)` 에서 자연 제외 |
| created_at | Timestamp | 적재 시각 |

**context 3열의 nullable 근거**: `workflow_id` / `execution_id` / `node_execution_id` 는 워크플로우 밖 caller(KB graph 추출 · AgentMemory 추출 processor 등)도 같은 facade 를 쓰기 때문에 nullable 이다. `record` 는 **ungated** (`?? null` 폴백) — 식별 필드가 없어도 로그 자체는 적재하고 attribution 만 비운다. 자매 로그 `IntegrationUsageLog`([§2.10.1](#2101-integrationusagelog)) 가 식별 필드 부재 시 로그를 **drop** 하는 gated 동작인 것과 대비된다. **어느 caller 가 채우고 어느 caller 가 NULL 인지는 본 문서에 복제하지 않는다** — [data-flow §1.3](./data-flow/7-llm-usage.md) 이 단일 진실이다.

**보존 기간**: 정리 배치 없음 (무기한 append-only). `IntegrationUsageLog` 의 90일 보존과 달리, 통계·비용 집계가 장기 시계열을 소비한다.

**인덱스**: [§3](#3-인덱스-전략) 참조 (V014 에서 3종 생성).

> **넘버링 주의**: 과거 `§2.16.1` 은 `RerankConfig` 엔티티였고 `unified-model-management` 개정에서 [§2.16 ModelConfig](#216-modelconfig) 의 `kind=rerank` 로 흡수·삭제됐다. 본 절이 그 번호를 재사용하지 않는 이유다 — `llm_usage_log` 는 `ModelConfig` 가 아니라 **Workspace** 가 CASCADE 소유한다(`llm_config_id` 는 SET NULL 이라 config 삭제 후에도 로그가 남는다).
...
| LlmUsageLog | (workspace_id, created_at DESC) | 워크스페이스별 사용량·비용 집계 (통계 요약 카드·시계열). V014 |
| LlmUsageLog | (provider, model, created_at DESC) | 프로바이더×모델 축 기간 합계 표 ([통계 §2.5](./2-navigation/7-statistics.md)). V014 |
| LlmUsageLog | (workflow_id, created_at DESC) WHERE workflow_id IS NOT NULL | `workflowId` 스코프 비용 집계(통계 필터·`llm_cost` 알림 룰). 워크플로우 밖 caller 의 NULL row 를 partial 로 배제해 인덱스 경량. V014 |
...
옛 `execution.execution_path UUID[]` 컬럼은 단일 인스턴스 환경에서는 동작했으나, 다중 backend 인스턴스가 동시에 `array_append()` 로 갱신할 때 인스턴스 간 절대 순서가 보장되지 않았다. 대체 모델로 append-only 테이블 `execution_node_log` 를 도입했고, BIGSERIAL `id` 가 PostgreSQL sequence (concurrency-safe) 로 부여되므로 `(execution_id, id)` 정렬이 곧 노드 실행 순서가 된다. 기록 시점은 노드 **처리 완료**(COMPLETED 또는 blocking output) 기준이며 throw 경로는 미기록 — [data-flow §1.2](./data-flow/3-execution.md) 참조.
