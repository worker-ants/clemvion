...
| embedding_llm_config_id | UUID? | 임베딩 호출에 사용할 LLMConfig (chat 이 아닌 embedding endpoint) `(V029)`. NULL 이면 워크스페이스 default LLMConfig. 생성·임베딩 파이프라인 흐름은 [Data-flow Knowledge Base](./data-flow/6-knowledge-base.md) 참고 |
...
| id | UUID | PK |
| workspace_id | UUID | FK → Workspace |
| provider | String | **`tei`** (HF Text-Embeddings-Inference, 자가호스팅 `bge-reranker-v2-m3-ko` 등) · **`cohere`** (외부 API) 2종으로 **종결**. provider 확장(`jina`/`voyage`/`local`/`builtin`)은 **Dropped (2026-06-05 결정)** — [LLM Client §2.1](./5-system/7-llm-client.md#21-리랭크-프로바이더) 참조 |
| name | String | 사용자 지정 이름 |
