...
cross-encoder 리랭커 provider 설정. chat/embedding 과 API shape(전용 `/rerank` 엔드포인트)가 달라 §2.16 LLMConfig 와 분리한 sibling 리소스 ([Spec LLM Client §3.6](./5-system/7-llm-client.md), [Spec RAG 검색 §3.3](./5-system/9-rag-search.md#33-검색-후처리--리랭킹-선택적)). 셀프호스팅(`tei`) 또는 외부 API(`cohere`) 를 지원하며, SSRF 가드·secret-store transformer 는 LLMConfig 와 동일 인프라를 재사용한다.
