...
| resume_call_stack | JSONB? | `NULL` 허용 (V087). **중첩 sub-workflow(`executeInline`) 안의 blocking 노드**가 `waiting_for_input` 으로 park 할 때, 재개에 필요한 **executeInline 호출 체인**(outermost→waiting inner 직전, `{version, frames:[{workflowId, invokerNodeId, recursionDepth}]}`)을 commit 하는 durable resume 매체 — rehydration([§7.5](./5-system/4-execution-engine.md#75-resume-after-restart-rehydration))이 이 스택으로 top-level→sub-workflow 프레임을 재귀 재진입(executeInline 재호출)해 최내층 waiting 노드까지 도달한다. **`NULL` = top-level park(중첩 깊이 0) / park 한 적 없는 실행 / 배포 이전 row** → 단일 레벨 재개. 컨테이너(Loop/ForEach/Map/Parallel) body blocking 은 [§3.2](./5-system/4-execution-engine.md) 금지라 선형 스택만(iteration/branch 상태 없음). `version` 은 `CALL_STACK_SCHEMA_VERSION`(checkpoint 와 독립). 상세: [실행 엔진 §6.2/§7.5/§Rationale(exec-park D6)](./5-system/4-execution-engine.md#62-저장-전략) |
