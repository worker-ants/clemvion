...
| source_ip | Varchar(45)? | `NULL` 허용 (V096). webhook/chat-channel 트리거 발화 시 `hooks.service` 의 `extractClientIp`(CF-Connecting-IP 신뢰 시 → X-Forwarded-For 첫 IP) 결과. 비-HTTP 트리거(schedule)·수동 실행·배포 이전 row 는 NULL. 인증 설정 사용 내역([config §A.3](./2-navigation/6-config.md))의 호출 이력 "소스 IP" 컬럼 소스. 길이 45 = IPv6 표기 최대(IPv4-mapped 포함) |
...
| provider | String | kind 별 허용: **chat** = openai/anthropic/google/azure/local · **embedding** = openai/azure/google/local (Anthropic 제외 — embedding 미지원) · **rerank** = `tei`(자가호스팅) / `cohere`(외부 API). **rerank Planned(후속)**: `jina` / `voyage` / `local` / `builtin` |
...
| ip_whitelist | String[]? | 허용 IP 목록 — 각 항목은 단일 IP 또는 CIDR 표기 (예: `10.0.0.0/8`, `2001:db8::/32`; 단일 IP 는 /32·/128 호스트로 취급). **저장(create/update) 시 각 항목을 형식 검증**하며 단일 IP/CIDR(IPv4·IPv6) 가 아니면 `400` 으로 거부한다 — 런타임 평가와 동일한 수용 기준. webhook 수신 시 `auth_config_id` 가 연결된 트리거에 한해 시행 ([Spec Webhook §3.2 WH-SC-09](./5-system/12-webhook.md#32-인증-및-보안)) |
