...
| 구조 | `{ nodeId: "uuid" \| null, code: "ERROR_CODE" \| null, message: "에러 설명", details?: {...} }` — `nodeId` 는 노드 없는 엔진 인프라 실패(worker 크래시 등)에서, `code` 는 분류 가능한 코드가 없는 일반 `catch` 경로에서 `null` 이다. 억지 fallback 코드를 넣지 않는 이유는 [EIA §6.4](./5-system/14-external-interaction-api.md) 참조 |
