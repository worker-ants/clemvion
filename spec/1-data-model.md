...
User ──┬── Workspace (1:N)
       ...
       │       ├── LlmUsageLog (1:N)    # chat 사용량 로그 (§2.24)
       │       ├── AuthConfig (1:N)
       │       ├── AuditLog (1:N)
       │       ├── Notification (1:N)
...
| input_data | JSONB | 노드 입력 데이터. **응답·emit 시 자격증명 값-패턴 마스킹**(DB 는 원문 보존) — 상위 `Execution.input_data` 와 **같은 규칙**이다(2026-08-20 이전에는 상위만 카브아웃이었다). 한쪽만 가리면 REST 폴링이 WS 마스킹 값을 덮어 flip-flop 이 난다. 범위: [EIA §R17](./5-system/14-external-interaction-api.md) |
...
| 직접 기록 | **복사가 유일한 채움 경로는 아니다.** admission 대기 한도 초과(§8 큐 대기)는 노드가 시작된 적이 없어 복사할 NodeExecution 이 없다 — 엔진이 `Execution.error = { code: EXECUTION_QUEUE_WAIT_TIMEOUT, message }` 를 직접 UPDATE 한다. 이 경로의 종결 상태는 `failed` 가 아니라 **`cancelled`** 라 [에러 처리 §1.4](./5-system/3-error-handling.md) 의 "엔진 수준 에러" 목록에는 나오지 않는다 ([4-execution-engine §8](./5-system/4-execution-engine.md#8-동시-실행-제한)) |
...
**참조 관계 (kind 별)**: chat → AI 노드 `config.llmConfigId`(JSONB, FK 없음)·`AssistantSession.llm_config_id`·`llm_usage_log.llm_config_id`·`KnowledgeBase.extraction_llm_config_id`·`KnowledgeBase.rerank_llm_config_id`. embedding → `KnowledgeBase.embedding_model_config_id`. rerank → `KnowledgeBase.rerank_config_id`.
...
> **`LlmUsageLog` 위치**: chat 사용량 로그 엔티티는 §2.16 의 자식이 아니라 **[§2.24 LlmUsageLog](#224-llmusagelog)** 다 — CASCADE 소유 부모가 `ModelConfig` 가 아닌 **Workspace** 이기 때문(`llm_config_id` 는 `SET NULL`). §2.16 은 chat/embedding/rerank **설정**만 다룬다.
...
> 관련 문서: [Spec LLM Usage](./data-flow/7-llm-usage.md) §1.3(Caller 카탈로그)·§2.1(스키마/인덱스)·§4(외부 의존) — 적재 정책·attribution 채움 현황 SoT

chat 계열 LLM 호출(`chat`/`chatStream`) 후 provider 응답 토큰 수를 append-only 로 1행 기록한다 (embed 계열은 provider 가 토큰 usage 를 반환하지 않아 미적재 — §1.3). 자매 로그 `IntegrationUsageLog`(§2.10.1) 와 대칭이다. CASCADE 소유 부모는 **Workspace**(`workspace_id`)이며, 나머지 FK(`workflow_id`/`execution_id`/`node_execution_id`/`llm_config_id`)는 모두 nullable `ON DELETE SET NULL` 이다.

> **넘버링 주의**: 이 엔티티는 **top-level `§2.24`** 다 — CASCADE 소유 부모가 Workspace 이므로(자매 `IntegrationUsageLog` §2.10.1 이 CASCADE 부모 Integration 아래 놓인 것과 같은 원칙). §2.16 ModelConfig 아래 `§2.16.1` 로 두지 않는 이유는 (a) `llm_config_id` 가 `SET NULL` 이라 ModelConfig 가 소유하지 않고, (b) 구 `§2.16.1` 은 `unified-model-management` 이전 **RerankConfig** 의 번호라 다수 `plan/complete/*`(예 `rag-rerank-impl.md`) 가 그 의미로 링크하고 있어 재사용 시 "조용히 틀린 링크" 가 되기 때문이다.

| 필드 | 타입 | 설명 |
|------|------|------|
| id | UUID | PK |
| workspace_id | UUID | FK → Workspace (CASCADE). `config.workspaceId` 로 **항상 채움** — 워크스페이스 단위 집계는 컨텍스트 누락과 무관하게 온전 |
| workflow_id | UUID? | FK → Workflow (SET NULL). AI 노드 호출은 채움(첫 턴/단발 `context.*`, resume `state.*`), 워크플로우 밖·미배선 caller 는 NULL (§1.3) |
| execution_id | UUID? | FK → Execution (SET NULL). `workflow_id` 와 동상 |
| node_execution_id | UUID? | FK → NodeExecution (SET NULL). 현재 turn 의 NodeExecution row PK. `workflow_id` 와 동상 |
| llm_config_id | UUID? | FK → ModelConfig (kind=chat), `ON DELETE SET NULL` (물리 테이블 `llm_config`). 호출에 쓰인 config (JSONB `config.llmConfigId` 유래도 가능) |
...
> `plan/in-progress/spec-draft-schedule-index.md`, 구현은 V110.
