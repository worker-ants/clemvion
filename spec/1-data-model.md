...
| embedding_llm_config_id | UUID? | **[LEGACY — V092 제거 예정]** PR2 점진적 전환의 폴백용 구 piggyback config. `embedding_model_config_id`·ws default kind=embedding 가 모두 없을 때만 `embedding_model` 문자열과 함께 사용 (FK → ModelConfig) |
| embedding_model | text? | **[LEGACY — V092 제거 예정]** 구 임베딩 모델 문자열 (default `text-embedding-3-small`). 1급 경로에서는 ModelConfig.default_model 이 우선하며 본 컬럼은 legacy 폴백에서만 쓰인다 |
