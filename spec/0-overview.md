...
| **임베드형 웹채팅 위젯 + SDK** | 외부 사이트에 삽입하는 iframe 격리형 웹채팅 위젯 SPA(`codebase/channel-web-chat`, Next.js CSR) + 개발자 SDK(스니펫 로더 / npm, `codebase/packages/web-chat-sdk`) + 샘플. [External Interaction API](./5-system/14-external-interaction-api.md) 의 client-side consumer. 제품 내 **운영 콘솔**(사이드바 "웹채팅" 메뉴 `/web-chat` — 인스턴스 생성·외형 빌더·설치 스니펫·라이브 미리보기 전부 구현 완료, navigation NAV-WC-01~06 ✅)도 제공한다([운영 콘솔](./7-channel-web-chat/5-admin-console.md)). 영역 spec 6문서 중 5문서가 `implemented`, [3-auth-session](./7-channel-web-chat/3-auth-session.md) 은 **`partial`** 이다 — 재로드 복원의 `404`·복구불가 `401` REST 분기와 `401` 낙관적 refresh 가 미구현이며 [`webchat-reload-rest-error-branches.md`](../plan/in-progress/webchat-reload-rest-error-branches.md) 가 소유한다(2026-08-10). 그 잔여를 뺀 세션 흐름 자체는 동작한다. **위젯 chrome 문자열 EN 다국어화(`locale` 활성)는 2026-07-12 구현 완료**([1-widget-app §4](./7-channel-web-chat/1-widget-app.md)) — 그 외 잔여 품질·하드닝 항목은 비차단 backlog 로 분리. spec: [Channel Web Chat](./7-channel-web-chat/_product-overview.md). |
