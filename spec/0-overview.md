...
| **임베드형 웹채팅 위젯 + SDK** | 외부 사이트에 삽입하는 iframe 격리형 웹채팅 위젯 SPA(`codebase/channel-web-chat`, Next.js CSR) + 개발자 SDK(스니펫 로더 / npm, `codebase/packages/web-chat-sdk`) + 샘플. [External Interaction API](./5-system/14-external-interaction-api.md) 의 client-side consumer. 제품 내 **운영 콘솔**(사이드바 "웹채팅" 메뉴 `/web-chat` — 인스턴스 생성·외형 빌더·설치 스니펫·라이브 미리보기 전부 구현 완료, navigation NAV-WC-01~06 ✅)도 제공한다([운영 콘솔](./7-channel-web-chat/5-admin-console.md)). 영역 spec 6문서가 모두 `implemented` 다 — [3-auth-session](./7-channel-web-chat/3-auth-session.md) 이 마지막까지 `partial` 이었던 재로드 복원의 `404`·복구불가 `401`/`410` REST 분기와 `401` 낙관적 refresh 는 2026-08-10 구현 완료([`webchat-reload-rest-error-branches.md`](../plan/complete/webchat-reload-rest-error-branches.md)). **위젯 chrome 문자열 EN 다국어화(`locale` 활성)는 2026-07-12 구현 완료**([1-widget-app §4](./7-channel-web-chat/1-widget-app.md)) — 그 외 잔여 품질·하드닝 항목은 비차단 backlog 로 분리. spec: [Channel Web Chat](./7-channel-web-chat/_product-overview.md). |
...
| **실행 상세 cross-node ConversationThread 뷰** | 여러 노드의 presentation/AI turn 을 seq·timestamp·source 로 interleave 한 통합 대화 뷰 재구성 (derived view). 단일 AI Agent 노드 Preview 는 §6.1 EH-DETAIL-06 ✅. 상세: [Execution History EH-DETAIL-12](./2-navigation/_product-overview.md#315-execution-history-실행-내역), [Conversation Thread §7](./conventions/conversation-thread.md#7-v2-로드맵). |
...
- **Redis**: 캐시, BullMQ 큐 백엔드 (`execution-run` intake / `execution-continuation` / `background-execution`), 운영 lock (`exec:recover:lock`), pub/sub 채널 (`integration:cache:invalidate` — integration 자격증명 회전(rotate)·삭제(remove) 시 멀티 인스턴스 캐시 무효화, [DB Query §4](./4-nodes/4-integration/2-database-query.md#4-실행-로직)), KB 채널 등, 세션 관리
...
### 3.3 상세/설정 패널 패턴
- 우측 슬라이드 패널 또는 모달
- 저장 방식: 설정 패널은 저장/취소(또는 `변경 저장`) 버튼, 워크플로우 에디터 캔버스는 수동 저장(`Ctrl+S`·`Save`) + 실행 직전 저장 (타이머 자동 저장 없음 — 상세: [캔버스 §8](3-workflow-editor/0-canvas.md#8-저장))
- 유효성 검증 즉시 피드백
...
- **배경**: Backend 의 ORM 은 **TypeORM** 이고, 스키마를 ORM 이 만들게 두지 않는다(`synchronize: false`) — ORM 이 엔티티에서 만드는 마이그레이션 대신 SQL 을 직접 쓴다.
- **채택**: SQL 기반 Flyway (`V<NNN>__<descriptor>.sql`) 를 별도 도구로 도입. ORM 이 엔티티에서 만드는 마이그레이션 대신 SQL 기반을 쓰는 이유: (a) 운영 DB 에 적용되는 SQL 을 PR 에서 그대로 리뷰 가능, (b) `ALTER` 의 락 거동·트랜잭션 모드 세밀 제어, (c) extension·partial index·CHECK 제약·`NOT VALID` 등 Postgres 고유 기능 표현.
- **trade-off**: TypeORM 엔티티 데코레이터와 Flyway SQL 의 schema 가 이중으로 존재 — drift 위험. `codebase/backend/src/migrations.spec.ts` 가 CI 마다 schema_history vs 파일 정합성을 검증해 silent skip 을 차단하지만, schema 정의 자체의 이중 source 는 받아들인 비용이다. 그 drift 는 e2e 가드 `entity-schema-declarations` 가 잡는다 — 인덱스 · 제약은 선언 → DB 한쪽, 컬럼 정의는 양방향([데이터 모델 Rationale «`code:` 에 전용 e2e 가드 셋»](./1-data-model.md#code-에-전용-e2e-가드-셋-2026-09-19)). 상세 운영 규약: [Flyway 마이그레이션 운영 규약](./conventions/migrations.md).
...
> **정정 (2026-09-19)**: 이 절은 ORM 을 Prisma 로 적었다(«NestJS + Prisma» · «`prisma migrate`» · «Prisma client 의 schema», #256 이 넣은
> 문장). 저장소 이력 어디에도 Prisma 는 없고 첫 커밋부터 TypeORM 이다. 결정(Flyway · SQL 기반)과 그 이유는 그대로다.
