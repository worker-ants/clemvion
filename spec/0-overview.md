...
| **임베드형 웹채팅 위젯 + SDK (채널)** | 외부 사이트 삽입용 iframe 격리형 웹채팅 위젯 SPA(`codebase/channel-web-chat`, Next.js CSR) + 개발자 SDK(스니펫 로더 / npm, `codebase/packages/web-chat-sdk`). [External Interaction API](./5-system/14-external-interaction-api.md) 의 client-side consumer. 제품 내 **운영 콘솔**(사이드바 "웹채팅" `/web-chat` — 인스턴스 생성·외형 빌더·설치 스니펫·라이브 미리보기, 위젯 co-deploy 자급 빌드) 제공. 영역 spec 6문서 전부 `implemented`(영역 종결); 잔여 품질·하드닝은 비차단 backlog. spec: [Channel Web Chat](./7-channel-web-chat/_product-overview.md) · [운영 콘솔](./7-channel-web-chat/5-admin-console.md). |
