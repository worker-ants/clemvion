...
자산은 단일 `<path>` + linear gradient 로 구성된다 (R-14 참조).
...
1. **자산 안에 박힌 컬러** — 정식 로고/파비콘 SVG (§8.4.1) 는 자체 fill 로 다음 **teal → lime gradient** 를 사용한다. 본 값들은 SVG 안에 박혀 있고, **앱 테마 토큰으로의 매핑은 본 spec 의 책임 밖**이다 (현 상태 = 매핑 없음).
   ...
   - **로고 자산 컨테이너 fill** — **transparent** (R-16). 라이트/다크 surface 위에 직접 얹힘.
...
라이트/다크 자산 선택은 노출 자리(surface) 의 배경 톤을 기준으로 한다. **R-17** 이후 사이드바·인증 화면 모두 dedicated 한 `#111e14` dark backdrop 을 두지 않고 surface(Shadcn `--card` / 인증 gradient) 위에 transparent 로고가 직접 얹힘 — `<Logo theme="auto">` 가 현재 테마 모드를 따라 light/dark 자산을 자동 토글한다.
...
#### 8.6.1 single-path 추상 마크 (R-14) + 워드마크 layout 개정 (R-15)

현 자산은 단일 path + linear gradient `#12acaa → #8be67e` 의 추상 마크 (R-14) + `Clemvion` 대문자 C, font 500/48px 단색, sub-copy 없음, viewBox `320×80` 워드마크 (R-15) 로 구성된다. 자산 파일명·`<Logo>` 컴포넌트 API 는 그대로 유지하므로 외부 import 경로 변경 없음.
...
| `codebase/frontend/public/apple-icon.svg` | 교체. R-16 예외 — 컨테이너 유지. PNG 변환 follow-up 은 8.6.2 그대로 |
| `codebase/frontend/public/opengraph-image.svg` | 교체 (embedded 풀로고 R-15/R-16 반영, 텍스트 흑색). R-16 예외 — 컨테이너 유지. metadata 미선언 follow-up 은 8.6.2 그대로 |

#### 8.6.2 임시 상태 (follow-up 대기)
...
위 3건은 별도 follow-up 의 대상이다. PNG 생성 소스는 현 R-14/R-15 의 새 SVG 다.

---

## Rationale

본 섹션은 §8 정식 개정의 배경·근거를 inline 으로 보관한다.

### R-1. 덩굴 곡선 → 노드 그래프 모티프 전환 — *Superseded by R-14*

> 본 결정은 자산 교체로 폐기. 노드 그래프 모티프 (spine + branches + 다수 nodes 분해) 자체가 R-14 로 대체됨. SVG 자산은 single-path 추상 마크로 전환. 본 R-1 의 *"덩굴 곡선이 아닌 노드 graph 표현"* 원칙은 부분 유지 — R-14 의 새 마크 역시 "노드 간 연결 관계" 의 추상화이므로 곡선 식물 모티프로의 회귀가 아니다.

### R-2. 4-step → 7-step Vine ramp 도입 — *Superseded by R-13*

> 본 결정은 §8 부분 롤백으로 폐기되었다. 7단계 Vine ramp 의 spec 정식화는 R-13 으로 무효화. SVG 자산 안의 라이트/다크 vine 컬러는 §8.2 (보류) 의 1항에 *디자인 참고* 로 남는다.

### R-3. 워드마크 2-tone 시그니처 채택 — *Superseded by R-13, R-15*

> 본 결정은 2-단계로 폐기:
> 1. **R-13** — §8.4.4 spec 정식 규정은 단색으로 복원. 단 *자산 안에 박힌 2-tone 디자인* 은 잠정 유지.
> 2. **R-15** — 자산 안 박힌 `vi` 2-tone 디자인까지 폐기. 모든 워드마크 SVG 가 단색 단일 fill 로 통일.
>
> 2-tone 정식화 재논의는 §8.2 갱신 시.

### R-4. 다크 모드 토큰 동시 도입 — *Superseded by R-13*
...
### R-5. `AGENTIC WORKFLOW` 서브카피 상시 부착 — *Superseded by R-15*

> 본 결정은 워드마크 개정으로 폐기. 풀로고는 mark + wordmark 2요소 (sub-copy 없음). *Agentic Workflow* tagline 은 alt/aria-label/메타 description 등 시각 영역 외부에 잔존 — R-15 본문 참조. 카테고리 디스크립터의 외부 노출 필요성 (옛 R-5 핵심 근거) 은 풀로고가 아닌 marketing-page hero copy / OG meta 로 옮겨졌다.
...
### R-8. 토큰 네이밍에서 일반 단어 회피 — *Superseded by R-13*
...
### R-10. 시각 토큰과 코드 토큰의 분리 (구현 위임) — *Superseded by R-13*
...
**근거**:
- §8.2 의 Vine ramp 를 `codebase/frontend/src/app/globals.css` 의 Shadcn 슬롯(`--primary` 등)에 매핑한 결과, 사용자가 *"전체적인 색상이 별로"* 로 거부. globals.css 는 neutral 팔레트로 복원.
- spec 이 정의한 컬러 토큰이 코드에 반영되지 않으면 단일 진실(single source of truth) 원칙 위반. 임시 불일치 윈도우(§8.2.6) 로 무한 연장하는 것은 spec 신뢰성 훼손.
- 사용자가 spec 도 함께 롤백하는 안을 선택. 가장 단순하고 spec/코드 정합성 우선. spec 만 유지하고 코드만 롤백하면 단일 진실 원칙 위반을 무한 연장하게 된다.
...
**향후 작업**:
- **§8.2 컬러 토큰 정식화** — 사용자/디자이너 협업으로 새 컬러 가이드 확정 후 spec 갱신 + 코드 매핑. 재정식화 시 **이번 폐기한 R-2/R-3/R-4 의 결정·근거를 동일 형태로 재채택하기 전에**, 이번 사용자 피드백(컬러 거부)의 원인을 다시 검토해야 한다 — 단순 재선언으로 같은 거부를 반복하지 않도록.
- **R-10 (시각/코드 토큰 분리 원칙)** — §8.2 재정식화 시 보존 가치가 있어 재검토 대상.
- **R-4 (다크 모드 동시 도입 vs 분리)** — §8.2 재정식화 시 다시 결정.
- `apple-icon.png` / `opengraph-image.png` 생성 — raster 도구 도입 후.
- `favicon.ico` multi-size 합성 — 동일 follow-up.

### R-14. node-graph 분해 모티프 → single-path 추상 마크
...
- 사용자가 `temp/logo.svg` 로 새 마크 디자인을 직접 제공 — *user-driven brand change*.
...
- 다크 컨테이너 hex 는 새 gradient 와의 색상 조화를 위해 `#0a1f1f` (dark teal) 사용. 단, sidebar 헤더 영역의 `#111e14` 하드코딩은 호환성 위해 유지 (옛 *vine-dark-bg-elevated* 명칭은 *dark brand surface* 로 호칭 변경, hex 동결).
...
- 새 gradient (teal → lime) 의 app 테마 토큰 매핑 — R-13 의 §8.2 보류 결정을 그대로 따른다.
- `apple-icon.png` / `opengraph-image.png` raster 생성 — 생성 소스는 R-14 의 새 SVG.
- sidebar 헤더 `#111e14` 와 dark mark 컨테이너 `#0a1f1f` 의 색상 조화 — 동일 dark surface 가 다른 hex 를 쓰는 일시 불일치. 사용자 시각 검토 후 통일.

**출처**:
- 새 마크 원본: `temp/logo.svg` (사용자 제공).

### R-15. 워드마크 layout 개정 — capital C, monochrome, no sub-copy
...
- `apple-icon.png` / `opengraph-image.png` raster 생성 — OG 의 생성 소스는 R-15 의 새 SVG.
...
- `apple-icon.png` / `opengraph-image.png` raster 생성 — raster 도구 도입 시점에서 R-16 의 새 SVG 가 소스.

### R-17. 사이드바·인증 로고 surface 의 dedicated dark backdrop 폐기
...
### R-18. Tailwind v4 `dark:` variant 와 `.dark` class 토큰 전략 정렬 — 인프라 픽스

**결정**: `codebase/frontend/src/app/globals.css` 에 `@custom-variant dark (&:where(.dark, .dark *));` 1줄 추가. Tailwind v4 의 `dark:` utility variant 가 OS `prefers-color-scheme: dark` 미디어 쿼리 대신 `.dark` ancestor class 를 추적하도록 override.

**근거**:

- 사용자 보고: *"라이트/다크 모드 테마 상관없이 다크모드의 로고가 표시되어 라이트모드에서 Clemvion 글자가 안 보임"*. R-17 의 `<Logo theme="auto">` 가 실제로는 *theme=auto* 가 아니라 *OS 가 다크면 무조건 dark* 로 동작.
- 원인 진단:
  - Tailwind v4 의 `dark:` variant 기본값은 `@media (prefers-color-scheme: dark)`.
  - `codebase/frontend/src/app/globals.css` 는 `.dark { --background: ...; ... }` 처럼 *class selector* 로 다크 토큰 정의 — Shadcn 컨벤션 (class-based dark mode).
  - 두 전략이 동시 작동하지 않음. `.dark` 클래스가 어디에도 안 붙어있어 토큰은 늘 light, 그러나 OS 가 dark 면 `dark:hidden`/`dark:block` 같은 Tailwind utility 는 활성화 → 두 시스템이 충돌.
  - `<Logo theme="auto">` 의 dual-img 패턴 (`block dark:hidden` + `hidden dark:block`) 이 OS 기준으로 토글되어 dark 자산만 표시.
  - 같은 증상이 다른 `dark:` 사용처 (statistics·triggers·integrations 페이지의 색조 토글) 에도 잠재. 본 픽스로 모두 정상화.
- `@custom-variant dark (&:where(.dark, .dark *));` 는 Tailwind v4 의 공식 syntax — Shadcn class-based dark mode 가이드와 동일.
- 본 픽스 후의 동작:
  - 앱이 light mode (`.dark` 클래스 부재): 토큰 light + Tailwind `dark:` 비활성 → 모든 자리가 일관되게 light. `<Logo>` 는 light 자산 (검정 텍스트) 표시.
  - 앱이 dark mode (`.dark` 클래스 부착, 미래 theme provider 도입 시): 토큰 dark + Tailwind `dark:` 활성 → 일관되게 dark. `<Logo>` 는 dark 자산 (흰 텍스트) 표시.
  - OS preference 와 무관 — 앱의 actual theme state 만 따름.

**보존되는 결정**:

- R-15·R-16·R-17 의 워드마크/자산/wrapper 결정 모두 그대로. R-18 은 인프라 픽스이지 디자인 결정 아님.
- `<Logo>` 컴포넌트 외부 API (variant·theme·size) 변경 없음.

**부분 폐기**:

- 없음 — 본 결정은 *기존 결정의 의도가 실제 동작과 일치하지 않던 버그* 를 정정. R-17 의 *theme="auto"* 가 비로소 의도대로 작동.

**향후 작업**:

- Dark mode 의 entry point — theme provider (예: `next-themes`, custom React context) 도입 시 `<html className="dark">` 토글로 자동 작동. 별도 spec/PR.
- 본 R-18 은 brand spec 의 §8 본문에는 표면화하지 않음 (인프라 픽스 — 디자인 결정 아님). 본 Rationale 에만 보존.
- 다른 `dark:` 사용처 (statistics·triggers·integrations) 의 dark mode 시각 검토 — 본 R-18 으로 동작이 *바뀌지는 않음* (light mode 가 일관되게 됨), 그러나 미래 dark mode 도입 시점에 한꺼번에 디자인 검토 필요.

**참고**:

- Tailwind v4 공식 가이드: [class-based dark mode via @custom-variant](https://tailwindcss.com/docs/dark-mode#using-a-data-attribute).
