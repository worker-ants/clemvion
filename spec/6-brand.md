...
| 2026-05-25 | Tailwind v4 dark variant 정렬 (인프라) | 사용자 보고 *"라이트/다크 모드 테마 상관없이 다크 로고가 표시되어 라이트모드에서 Clemvion 글자가 안 보임"*. 원인: Tailwind v4 의 `dark:` 기본 전략이 OS `prefers-color-scheme: dark` 미디어 쿼리인데, 앱 토큰은 `.dark` class 전략 (`globals.css` 의 `.dark { ... }` 선택자) 으로 정의되어 둘이 불일치. OS 가 다크면 `dark:` 활성화되어 `<Logo theme="auto">` 가 dark 자산 노출, 그러나 `.dark` 클래스 미부착이라 surface 토큰은 light 유지 → 흰 글자가 light surface 위에 비가시. 픽스: `globals.css` 에 `@custom-variant dark (&:where(.dark, .dark *));` 1줄 추가하여 Tailwind utilities 가 `.dark` class 를 추적하게 함. R-18 신설. 영향 범위: 전체 codebase 의 `dark:` 사용처 (statistics·triggers·integrations 등) 모두 같이 정상화 |
...
### R-18. Tailwind v4 `dark:` variant 와 `.dark` class 토큰 전략 정렬 (2026-05-25, 5차 개정 — 인프라 픽스)
...
- `<Logo>` 컴포넌트에서 `theme="auto"` 를 `theme="light"` 강제 — 미래 dark mode 도입 시 다시 수정해야 함. 인프라 픽스가 옳음.
- 워드마크 SVG 의 `fill` 을 `currentColor` 로 변경 — surface 의 text color 토큰을 따라가게 함. 1) SVG 자산 6개 모두 수정 필요, 2) `<img>` 태그는 외부 `color` 를 상속하지 않음 (inline SVG 만 currentColor 적용) → API 전환 비용. 기각.
- Tailwind v3 호환 `darkMode: 'class'` 설정 — v4 에서는 config 파일이 없고 CSS-first 설정만 지원. `@custom-variant` 가 v4 정석.
- 모든 `dark:` 사용처 manual audit + 개별 수정 — 본 픽스 1줄로 모두 일괄 해결.
...
- 본 R-18 은 brand spec 의 §8 본문에는 표면화하지 않음 (인프라 픽스 — 디자인 결정 아님). §9 변경 이력 + 본 Rationale 에만 보존.
...
- 사용자 보고 (2026-05-25 대화 5차): *"라이트모드와 다크모드에서 표시할 로고가 바뀌었어"* + 후속 확인 *"라이트/다크모드 테마 상관없이 다크모드의 로고가 표시되어서, 라이트모드에서는 로고의 Clemvion 글자가 안 보임"*.
