...
1. **자산 안에 박힌 컬러** — 정식 로고/파비콘 SVG (§8.4.1) 는 자체 fill 로 다음 **teal → lime gradient** 를 사용한다. 본 값들은 SVG 안에 박혀 있고, **앱 테마 토큰으로의 매핑은 본 spec 의 책임 밖**이다 (현 상태 = 매핑 없음).
   ...
   - **컨테이너 fill** — light: `#eef5ec` (pale green). dark: `#0a1f1f` (deep teal).
   - **워드마크 base fill** — light: `#0e1a12` (ink). dark: `#e8f5ec` (cream).
   - **워드마크 `vi` accent fill** — light: `#0e8c8a` (teal-deep, gradient start 의 어두운 변종). dark: `#8be67e` (lime, gradient end 와 동일).
   - **서브카피 (`AGENTIC WORKFLOW`) fill** — light: `#3aa686` (mid-teal-green). dark: `#7ed9a8` (light mint).
...
> 옛 vine-green ramp (7단계, light `#1a4f2c`–`#5ab872`, dark `#1e4a2a`–`#9efab2`) 는 2026-05-25 자산 교체 (R-14) 로 폐기. 별도 컬러 가이드 결정 전까지 옛 토큰명·hex 는 사용하지 않는다.
...
| 워드마크 accent (`vi`) | 동일 폰트 | weight **600**, fill = `#0e8c8a` (light SVG) / `#8be67e` (dark SVG). SVG 자산에 박힘 |
| 서브카피 (`AGENTIC WORKFLOW`) | monospace (`Courier New`) | font-size 8px, letter-spacing 3px, uppercase, fill = `#3aa686` (light) / `#7ed9a8` (dark). SVG 자산에 박힘 |
...
| 변종 | 정식 경로 | 사용처 |
| --- | --- | --- |
| Full logo (light) | `codebase/frontend/public/logo.svg` | 라이트 배경 풀로고 — mark + wordmark + sub-copy 동반 |
| Full logo (dark) | `codebase/frontend/public/logo-dark.svg` | 다크 배경 풀로고 |
| Icon mark (light, 96px master) | `codebase/frontend/public/logo-mark.svg` | 사이드바, 로딩, 카드 |
| Icon mark (dark) | `codebase/frontend/public/logo-mark-dark.svg` | 다크 배경 mark |
| Wordmark only | `codebase/frontend/public/logo-wordmark.svg` | 좁은 자리 — sub-copy 없이 텍스트만 |
...
> 단, 현재 정식 SVG 자산(`logo.svg`, `logo-dark.svg`, `logo-wordmark.svg`) 은 `clem`**`vi`**`on` 의 `vi` 두 글자를 별도 weight + 색(`#0e8c8a` 라이트 / `#8be67e` 다크) 으로 강조하는 2-tone 시그니처를 **자산 안에 박힌 디자인**으로 보유한다. 이는 정식 비주얼 가이드(§8.2 보류) 상태에서의 잠정 자산 디자인이며, 새 워드마크 자산을 그릴 때 본 §8.4.4 의 단색 규정을 따를지 / 2-tone 시그니처를 정식화할지는 §8.2 갱신과 함께 결정한다.
...
라이트/다크 자산 선택은 노출 자리(surface) 의 배경 톤을 기준으로 한다 — 어두운 surface (예: 사이드바의 dark brand 헤더 영역 `#111e14` — 옛 명칭 *vine-dark-bg-elevated*, R-14 모티프 교체 후에는 *dark brand surface* 로 호칭) 위에는 dark 변종을 둔다.
...
#### 8.6.2 2차 교체 — single-path 추상 마크 (2026-05-25, R-14)

2026-05-25 사용자 결정으로 node-graph 모티프(spine + branches + 다수 nodes 분해) 를 폐기하고, 단일 path + linear gradient `#12acaa → #8be67e` 의 추상 마크로 전환. 자산 파일명·`<Logo>` 컴포넌트 API 는 그대로 유지하므로 외부 import 경로 변경 없음.

| 자산 경로 | 변경 |
| --- | --- |
| `codebase/frontend/public/logo.svg` | 교체 (node-graph → single-path mark + 새 wordmark 색) |
| `codebase/frontend/public/logo-dark.svg` | 교체 |
| `codebase/frontend/public/logo-mark.svg` | 교체 (96px master). 컨테이너 `#eef5ec` 유지 |
| `codebase/frontend/public/logo-mark-dark.svg` | 교체. 컨테이너 `#111e14` → `#0a1f1f` (dark teal) |
| `codebase/frontend/public/logo-wordmark.svg` | 교체 (`vi` accent 색만 변경) |
| `codebase/frontend/public/favicon-16.svg` | 교체 (단일 path 그대로 축소) |
| `codebase/frontend/src/app/icon.svg` | 교체 (32px) |
| `codebase/frontend/public/apple-icon.svg` | 교체. PNG 변환 follow-up 은 8.6.3 그대로 |
| `codebase/frontend/public/opengraph-image.svg` | 교체. metadata 미선언 follow-up 은 8.6.3 그대로 |
