...
AI 카테고리 3 노드 공통 규약. v1 은 `ai_agent` 만 주입 동작을 구현하고, `text_classifier` / `information_extractor` 는 동일 인터페이스로 v2 에 추가된다. 모든 AI 노드의 turn 누적 (자동 push) 은 v1 부터 적용된다 ([Spec Conversation Thread §2.2](../../conventions/conversation-thread.md#22-ai-agent)).
...
| 일자 | 변경 |
|------|------|
| 2026-05-14 | §10 Conversation Context 공통 규약 신설. AI 카테고리 3 노드 공통 인터페이스 정의 (v1 ai_agent 만 주입 동작, push 는 3 노드 모두 적용). 기존 §10 CHANGELOG → §11 로 번호 변경 |
