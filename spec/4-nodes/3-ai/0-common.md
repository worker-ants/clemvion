...
| enabledTools | String[]? | 일반 도구 allowlist (bare operation id 배열). `['*']` 또는 미설정 = 전체 노출. 메타도구(resources/prompts)에는 영향 없음. Cafe24 의 경우 도구 수가 많아(**485**, 2026-07-17 실측 — [4-cafe24 §지원 범위](../4-integration/4-cafe24.md) SoT) UI 는 Resource 단위 grouping 으로 노출. **대형 카탈로그는 미설정(전체 노출) 시 `AI_AGENT_TOOL_COUNT_MAX`(128) 상시 초과 → allowlist 사실상 필수** ([1-ai-agent §4.2](./1-ai-agent.md#42-도구-정의-payload-예산-tool-definition-payload-budget)) |
...
LLM 3 노드는 `output.result.*` / `output.error.*` / `output.interaction.*` wrapper 를 공유한다 (CONVENTIONS Principle 1.1(config/output 직교)·3.2(error 컨트랙트)·4.4~4.5(interaction 존재+payload shape)·8.2(result/category 1차 네이밍) 분담 — Principle 11 은 "출력 예시 문서화 서식" 규칙이라 wrapper 계약 근거가 아님). 도메인 결과는 `output.result.*` 하위에 두고, 에러는 `output.error.{code, message, details?}`, 사용자 인터랙션은 `output.interaction` 에 둔다.
...
> AI 노드의 출력 구조는 [공통 §5 응답 형식 규약](#5-응답-형식-규약) 의 `output.result.*` / `output.error.*` / `output.interaction.*` wrapper 컨트랙트를 따른다.

## 10. Conversation Context (자동 컨텍스트 주입)

AI 카테고리 3 노드 공통 규약. **세 노드 (`ai_agent` / `text_classifier` / `information_extractor`) 모두 push (final assistant 누적) 와 `contextScope` 기반 자동 주입을 구현**한다 (handler 의 `pushClassifierTurn` / `pushExtractorTurn` + 공유 inject 유틸 `shared/conversation-context-injection.ts`). **`memoryStrategy` 자동메모리 주입은 `ai_agent` (summary_buffer / persistent) + `information_extractor` (persistent 만)** — IE 는 ai_agent 와 동일 scope key 로 cross-session 회수/추출을 수행하되 `summary_buffer`(working-memory 압축)는 제외한다 ([3-information-extractor §7](./3-information-extractor.md#7-persistent-메모리-recall--extraction)). `text_classifier` 는 single-turn·상태없음으로 자동메모리 제외 — 항상 `contextScope` (manual) 만. `contextScope` (stateless thread 주입) 와 `memoryStrategy` (상태누적 메모리) 의 적용 범위 차이를 구분한다 ([Spec Conversation Thread §2.3](../../conventions/conversation-thread.md#23-적용-범위-push-vs-inject-구분)). 노드별 final assistant text 변환 규칙은 §1.4 의 해당 행 참조.
...
| memoryStrategy | `ai_agent`: `manual` / `summary_buffer` / `persistent` · `information_extractor`: `manual` / `persistent` | | `manual` | 메모리 **관리 전략** 축. `manual`(기본) = 위 4 범위 필드 동작 그대로. 자동 전략(`summary_buffer`/`persistent`) = 위 4필드 무효 (자동 전략이 대체). **`ai_agent` 는 3값, `information_extractor` 는 `manual`/`persistent` 2값** (추출 노드는 `summary_buffer` 없음 — working-memory 압축 무의미). **`text_classifier` 는 본 필드를 갖지 않으며** 항상 `manual`(=`contextScope` 동작). 별도 필드 채택 근거 [AI Agent §12.9](./1-ai-agent.md#129-memorystrategy-를-contextscope-enum-확장이-아닌-별도-필드로-둔-근거). 상세: [Spec AI Agent §1·§6](./1-ai-agent.md#1-설정-config) + [3-information-extractor §7](./3-information-extractor.md#7-persistent-메모리-recall--extraction) + [Spec Agent Memory](../../5-system/17-agent-memory.md) |
