...
**도구 정의 payload 예산 경고 (저장 시점)** — ⚠ **구현 현황(Planned)**: 런타임 fail-fast(위 §10 행 `TOOL_DEFINITION_PAYLOAD_EXCEEDED`)만 구현됨. 아래 config-time graph warning(`ai_agent:tool-payload-budget` · `AI_AGENT_TOOL_BUDGET_STRICT_SAVE`)은 **미구현**이며 후속 [`ai-agent-tool-payload-budget-followups.md`](../../../plan/in-progress/ai-agent-tool-payload-budget-followups.md)에서 추적한다. 아래는 목표 설계다: `mcpServers`/`presentationTools` 로부터 추정한 도구 정의 payload (§4.2 estimator) 가 예산을 초과하면 [cross-node-warning-rules §4·§5·§8](../../conventions/cross-node-warning-rules.md) 의 `GraphWarningRuleResult`(rule id `ai_agent:tool-payload-budget`, 기본 severity `warning`)로 표면화된다. 이 backend-only 결과의 **surfacing 은 서버 권위 조회 endpoint `GET /workflows/:id/graph-warnings`(`getGraphWarnings`)** 가 결과 배열에 append 해 담당한다(**별도 응답 필드 신설 없음** — saveCanvas 응답은 graph warning 을 싣지 않는 기존 계약 유지). 저장(`POST /workflows/:id/save`, `saveCanvas`)은 기본(warning) 시 통과하되, `AI_AGENT_TOOL_BUDGET_STRICT_SAVE=true` 면 hard 초과를 severity `error` 로 승격해 `saveCanvas` 가 기존 `GRAPH_VALIDATION_FAILED` 로 400 차단한다(3중 가드 ①). 이 rule 은 통합 granted scope 의 **async 조회**가 필요해 pure/shared `@workflow/graph-warning-rules` 규칙(frontend/backend 동기 SSOT)으로 표현 불가 → **backend-only** 평가(`WorkflowsService`). 따라서 frontend canvas 가드 ②(로컬 pre-evaluate)는 이 rule 을 계산하지 않으며, 실질 안전망은 런타임 fail-fast(`TOOL_DEFINITION_PAYLOAD_EXCEEDED`) + saveCanvas 가드 ① 이다. 기본이 warn 인 이유: 저장 시 추정은 근사(런타임 granted scope·live MCP tool list 미확정)라 정상 설정 오차단을 피하기 위함 (§12.15).
