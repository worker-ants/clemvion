...
pending_plans:
  ...
  - plan/in-progress/node-output-redesign/ai-agent.md
---

# Spec: AI Agent

> 관련 문서: [AI 공통 규약](./0-common.md) · [Spec 노드 개요](../0-overview.md) · [Spec MCP Client](../../5-system/11-mcp-client.md) · [Spec RAG 검색](../../5-system/9-rag-search.md) · [Spec Graph RAG](../../5-system/10-graph-rag.md) · [Spec Config > Models](../../2-navigation/6-config.md) · [CONVENTIONS](../../conventions/node-output.md)
...
| temperature | Float | | ModelConfig 기본값 | 오버라이드 |
| maxTokens | Integer | | ModelConfig 기본값 | 오버라이드 |
...
| ragTopK | Integer | | — | KB tool 호출 시 주입 청크 수 **상한(override, optional)**. **미지정 시 동적 점수 컷**(token-budget + 내부 inject-cap 12)이 주입 수를 결정하고, 명시 시 그 값이 ceiling (LLM 이 호출 인자로 override 가능). KB `rerank_mode ≠ off` 면 리랭크 후 적용 ([Spec RAG 검색 §3.4](../../5-system/9-rag-search.md#34-동적-점수-컷-생성-주입-모든-모드-공통)) |
| ragThreshold | Float | | `0.7` | 최소 유사도 임계값 (0-1) 의 기본값 (LLM 이 호출 인자로 override 가능). KB `rerank_mode ≠ off` 면 cosine 임계가 아니라 **리랭크 점수 임계**로 해석 |
| mcpServers | McpServerRef[] | | `[]` | MCP-capable Integration 참조 목록. `service_type ∈ ('mcp', 'cafe24', 'makeshop')` 모두 수용 — `cafe24`·`makeshop` 은 backend `Cafe24McpToolProvider`·`MakeshopMcpToolProvider` 가 in-process `AgentToolProvider` 구현체로 동작 ([Spec MCP Client §2.3 Internal Bridge](../../5-system/11-mcp-client.md#23-internal-bridge-in-process)). [공통 §3](./0-common.md#3-mcp-서버-연결-ai-agent-전용). **⚠ 대형 카탈로그([Cafe24 485](../4-integration/4-cafe24.md) · [MakeShop 161](../4-integration/5-makeshop.md) operation — 수치 SoT 는 각 노드 spec §지원 범위)는 `enabledTools` allowlist 없이 연결하면 도구 개수·payload 예산을 상시 초과해 런타임 실패 — allowlist 가 사실상 필수 ([§4.2](#42-도구-정의-payload-예산-tool-definition-payload-budget))** |
...
| memoryTtlDays | Integer | | — | `memoryStrategy: 'persistent'` 시 추출 메모리 TTL (일). set 시 저장 메모리에 `expires_at = now() + ttlDays` 부여 → 만료 후 회수 제외·evict 삭제. 미설정 = 무만료 (디폴트, 기존 동작 보존). SoT: [Spec Agent Memory §4](../../5-system/17-agent-memory.md) |
| embeddingModelConfigId | ModelConfig select | | — | `memoryStrategy: 'persistent'` 시 메모리 회수·추출(저장)에 쓸 **embedding ModelConfig 의 id**. `/models` 에 등록된 embedding ModelConfig 를 select(`embedding-config-selector`)로 고른다 (config.id 저장). 회수와 저장이 **같은 config** 를 써야 차원이 일치한다. 미설정 시 워크스페이스 기본 embedding ModelConfig 로 폴백 (`ModelConfigService.resolveEmbedding`). 노드 `llmConfigId` 와 **독립** — 고른 config 자신의 provider/credential/모델로 임베딩한다. SoT: [Spec Agent Memory §3 임베딩 출처](../../5-system/17-agent-memory.md) |
| summaryModelConfigId | ModelConfig select | | — | `memoryStrategy ∈ {summary_buffer, persistent}` 시 **롤링 요약 압축 LLM 콜** (§6.1 단계 1.5) 에 쓸 **chat ModelConfig 의 id**. `/models` 에 등록된 chat ModelConfig 를 select(`chat-config-selector`)로 고른다 (config.id 저장). 미설정 시 노드 main `llmConfig` (=`llmConfigId`) 로 폴백 (기존 동작 유지). 고른 config 자신의 provider/credential/defaultModel 로 호출하므로 **노드 main LLM 과 다른 provider 의 저비용 모델**도 지정 가능 (§12.12) |
| extractionModelConfigId | ModelConfig select | | — | `memoryStrategy: 'persistent'` 시 **턴 경계 추출 LLM 콜** (§6.1 단계 2.7) 에 쓸 **chat ModelConfig 의 id**. `/models` 에 등록된 chat ModelConfig 를 select(`chat-config-selector`)로 고른다 (config.id 저장). 미설정 시 노드 `llmConfigId` → ModelConfig 기본 모델로 폴백 (기존 동작 유지). 고른 config 자신의 provider/credential/defaultModel 로 호출하므로 노드 main LLM 과 독립된 저비용 추출 모델 지정 가능 (§12.12) |
...
**Memory 섹션 `visibleWhen`**: `Strategy` 드롭다운은 항상 표시. `Token Budget` 과 `Summary Model` 은 `memoryStrategy ∈ {summary_buffer, persistent}` 일 때, `Memory Key`/`Top-K`/`Threshold`/`TTL (days)`/`Extraction Model` 은 `memoryStrategy == persistent` 일 때만 노출. `memoryStrategy == manual` (기본) 이면 Memory 섹션은 Strategy 만 보이고 Conversation Context 섹션의 5필드가 그대로 표시된다 (현행 유지). `memoryStrategy != manual` 이면 Conversation Context 섹션 전체가 숨겨진다 (자동 전략이 대체 — §1 비고).
...
LLM 요청에 실리는 **도구 정의(스키마) 전체의 직렬화 크기**를 예산으로 관리한다. 이는 §6.1 단계 3.g 의 도구 **호출 횟수** 한도(`maxToolCalls`) · §1 의 working-memory 토큰 예산(`memoryTokenBudget`) 과 **다른 축** — 도구 **정의 자체의 크기**다 (같은 문서에서 "budget" 이 세 자원에 쓰이므로 주의).

도구 **개수**가 아니라 **직렬화 bytes** 가 1차 지표다. 대형 카탈로그(예: Cafe24 Admin API) 는 오퍼레이션 개수가 그대로여도 필드 단위 스키마 상세로 도구당 payload 가 수배 팽창할 수 있고, 그 팽창이 프롬프트를 provider transport timeout 너머로 밀면 provider 무관하게 "응답 없음 / 무한 반복(SDK 재시도)" 으로 나타난다. 개수 cap 만으로는 이 팽창을 잡지 못한다 (§12.15).

**estimator (단일 진실)**: `estimateAgentToolPayload(tools) → { bytes, approxTokens, toolCount, perProvider?: [{ key, bytes, toolCount }] }`. `bytes = Buffer.byteLength(JSON.stringify(tools))`, `approxTokens` 은 language-aware 휴리스틱(§6.1 1.5 와 동일 `shared/agent-memory-injection.ts` 재사용, 근사). `perProvider` 는 "범인 provider 지목" 용. 런타임 판정 · 저장 시점 경고 · 관측 로깅이 **모두 이 함수를 공유**한다.
...
| `AI_AGENT_TOOL_BUDGET_STRICT_SAVE` | false | 저장 시점(§10 config 경고) hard 초과를 severity `warning`→`error` 로 승격해 `GRAPH_VALIDATION_FAILED` 로 차단하는 env-only opt-in |

> **⚠ 대형 카탈로그는 allowlist 가 사실상 필수 (2026-07-17)**: Internal MCP Bridge 로 노출되는 first-party 이커머스 카탈로그는 위 `AI_AGENT_TOOL_COUNT_MAX` 기본값(**128**)을 **상시 초과**한다 — Cafe24 **485** operation([4-cafe24 §지원 범위](../4-integration/4-cafe24.md) SoT, 2026-07-17 실측), MakeShop **161** REST operation([5-makeshop](../4-integration/5-makeshop.md)). 따라서 이들 서버를 `mcpServers[].enabledTools`([11-mcp-client §5.6](../../5-system/11-mcp-client.md#56-도구-allowlist)) 없이 연결하면 **정상 설정으로 보이는 상태에서도** 런타임 pre-flight 가 `TOOL_DEFINITION_PAYLOAD_EXCEEDED` 로 실패한다(개수 cap 과 bytes hard 예산 양쪽에서). **allowlist 로 실제 필요한 operation 만 노출하는 것이 정상 사용 방식**이며 회피책이 아니다 — 에이전트에게 485개 도구를 한 번에 쥐여주는 것은 payload 예산 이전에 **도구 선택 정확도** 관점에서도 바람직하지 않다.
>
> 본 기본값 128 은 §12.15 가 밝히듯 도구 **정의 payload bytes** 팽창 사고 대응의 **2차 sanity cap** 으로 정해진 값이며, 대형 카탈로그 전량 노출을 상정한 값이 **아니다**. 1차 지표는 어디까지나 bytes(`AI_AGENT_TOOL_PAYLOAD_HARD_BYTES`)다. (기본값 자체의 타당성 재검토는 별건 — 본 절은 현행 기본값 하에서의 사용 제약을 명문화한다.)

**런타임 판정 위치**: `AiTurnExecutor.buildTools` 직후 (single-turn·multi-turn resume 공통 헬퍼). hard(또는 count) 초과 시 **LLM 호출 전에** `TOOL_DEFINITION_PAYLOAD_EXCEEDED` throw → §7.3/§7.9 `error` 포트(`details` 구조화). provider SDK timeout 에만 의존하던 최대 수 분의 hang 대신 즉각·명확한 실패로 전환. soft 초과는 실패시키지 않고 로깅만. **error 포트 귀결 경로는 turn 종류에 따라 비대칭**이다 (최종 결과는 동일): multi-turn 은 이 throw 를 orchestrator 까지 전파해 `extractAiTurnErrorPayload` 가 `output.error`(code/details) 를 조립하고, single-turn 은 `executeSingleTurn`(`buildSingleTurnToolsOrError`)이 throw 를 try/catch 로 흡수해 §7.3 error output 을 직접 조립·반환한다. 이 return-vs-throw 계약은 `AiTurnExecutor` 클래스 JSDoc 에 명문화돼 있다(통일은 후속 plan 범위).

**저장 시점 경고**: §10 config 경고 계약 참조 (`ai_agent:tool-payload-budget` graph warning).
...
> **구현 레이어 (M-1 god-handler 분할 후)**: 핸들러 `AiAgentHandler`(`ai-agent/ai-agent.handler.ts`)는 **facade** 로, turn 실행 루프는 `AiTurnExecutor`(`ai-agent/ai-turn-executor.ts` — `executeSingleTurn`/`executeMultiTurn`/`processMultiTurnMessage`), 조건 분류는 `AiConditionEvaluator`(`ai-agent/ai-condition-evaluator.ts` — 상세 §6.1 단계 3.a), 자동 메모리 전략은 `AiMemoryManager`(`ai-agent/ai-memory-manager.ts` — 상세 §6.1 단계 1.3/1.5/2.7)에 단방향 위임한다. multi-turn 의 park/resume lifecycle 구동은 엔진측 `AiTurnOrchestrator`(`modules/execution-engine/ai-turn-orchestrator.service.ts`)가 맡는다 (핸들러는 무상태 collaborator 에 위임만).
...
1.3. **persistent 메모리 회수 (`memoryStrategy: 'persistent'` 시, LLM 호출 전 동기)** — 스코프 키 `(workspace_id, memoryKey ?? execution_id)` 로 `agent_memory` 에서 top-k 의미검색 (`memoryTopK` / `memoryThreshold`) 을 동기 수행해 회수된 사실/선호를 **systemPrompt 안정 프리픽스 영역**에 주입한다 (회수 블록은 휘발성 최근 turn 보다 앞 — [공통 §11.4](./0-common.md#114-주입-위치-및-ordering) ordering 의 안정 프리픽스). 회수는 hot path 동기이지만 추출(2.7)은 비동기다. 회수 건수는 `meta.memory.recalledCount` 로 노출. 구현: `AiMemoryManager.injectMemoryContext` (`ai-agent/ai-memory-manager.ts` — M-1 분할로 핸들러 facade 에서 추출; 공유 헬퍼는 `shared/agent-memory-injection.ts`). SoT: [Spec Agent Memory §회수](../../5-system/17-agent-memory.md).
1.5. **컨텍스트 메모리 주입** (CONVENTIONS Conversation Thread §5) — LLM 호출 **전**, `memoryStrategy` 에 따라 분기:
    ...
    - **`summary_buffer` / `persistent`** (자동): working-memory (자기 history + 주입 thread) 의 토큰 추정치가 `memoryTokenBudget` 을 초과하면 **오래된 turn 부터 롤링 요약으로 압축**한다. 토큰 추정은 균일 `char/3` 이 아니라 **language-aware 휴리스틱** (스크립트군별 chars-per-token 가중: Latin/ASCII ~4, CJK 한·중·일 ~1.7, 그 외 ~3 — 혼합 한국어+영어 텍스트의 편향을 줄임) 으로 산정한다. 여전히 **근사** 이며 provider tokenizer-exact (모델별 정확 카운트) 가 아니다 — 그것은 v3 로드맵 잔존 (§12.10). KB 청킹의 char/3 추정 (`text-chunker`) 과는 별개 경로다. — 요약 블록은 **system_text 안정 프리픽스** ([공통 §11.4](./0-common.md#114-주입-위치-및-ordering)) 에 배치하고 (1.3 의 회수 블록과 같은 안정 프리픽스 영역), 압축되지 않은 최근 원문 turn 만 휘발성 꼬리로 둔다. **요약 갱신은 예산 임계치 도달 시에만** 수행해 prompt cache 안정 프리픽스를 보호한다 (매 turn 재요약 금지). 요약 LLM 콜은 `summaryModelConfigId` 가 가리키는 ModelConfig (미설정 시 노드 main `llmConfig`) 를 사용한다 — set 시 **그 config 자신의 provider/credential/defaultModel** 로 직접 호출하고 (노드 `llmConfigId` 와 독립 — 다른 provider 의 저비용 모델 가능), 미설정 시 노드 main LLM 으로 폴백해 기존 동작 100% 유지 (§12.12). 요약 보관은 `ConversationThread.runningSummary` / `summarizedUpToSeq` ([conversation-thread §1.3·§5.3](../../conventions/conversation-thread.md#13-conversationthread)). 압축 발생 여부는 `meta.memory.summarized`, 예산 사용량은 `meta.memory.tokenBudgetUsed` 로 노출. 자동 전략(`summary_buffer`/`persistent`) 의 주입·롤링 요약 압축 구현은 `AiMemoryManager.injectMemoryContext` (`ai-agent/ai-memory-manager.ts`) 가 담당 (공유 휴리스틱·압축 헬퍼는 `shared/agent-memory-injection.ts`).
1.7. **`ai_user` turn push** (spec/conventions/conversation-thread.md §2.2) — LLM 호출 **전**, `userPrompt` resolved 직후 1회.
2. systemPrompt + userPrompt로 LLM 호출 (tools 파라미터에 위 도구들이 포함됨). single-turn 루프(LLM 호출 → 도구 실행 → 재평가 → turn 종결 출력 조립)의 구현 진입점은 `AiTurnExecutor.executeSingleTurn` (`ai-agent/ai-turn-executor.ts` — M-1 분할로 핸들러 facade 에서 추출). (도구 분류 자체는 단계 3.a 의 `AiConditionEvaluator.classifyToolCalls`.)
...
2.7. **persistent 메모리 추출 (`memoryStrategy: 'persistent'` 시, 턴 경계 비동기)** — 턴 경계에서 직전 turn(들)에서 추출할 사실/선호를 **비동기 background** 로 추출해 스코프 키 `(workspace_id, memoryKey ?? execution_id)` 로 `agent_memory` 에 저장한다 (LLM latency 에 추출 LLM 콜을 얹지 않는다 — hot path 비차단). 추출 enqueue 의 구현은 `AiMemoryManager.scheduleMemoryExtraction` (`ai-agent/ai-memory-manager.ts` — M-1 분할로 핸들러 facade 에서 추출; producer 측 enqueue, 공유 헬퍼는 `shared/agent-memory-injection.ts`). background 격리는 `scheduleBackgroundBody` 의 turns snapshot shallow-copy 격리 invariant 를 준수한다. **증분 추출**: 멀티턴은 `_resumeState.lastExtractionTurnSeq` watermark 초과 turn 만 snapshot 해 enqueue 후 watermark 전진 (신규 turn 0개면 skip). 추출 LLM 콜은 `extractionModelConfigId` 가 가리키는 ModelConfig (미설정 시 노드 `llmConfigId`) 를 사용한다 — set 시 **그 config 자신의 provider/credential/defaultModel** 로 직접 호출하고 (노드 main LLM 과 독립), 미설정 시 노드 `llmConfigId` → ModelConfig 기본 모델로 폴백해 기존 동작 100% 유지 (§12.12). 추출 항목은 `{content, kind}` (kind ∈ fact/preference/entity) 로 분류돼 `metadata.kind` 저장. `memoryTtlDays` set 시 `expires_at` 부여. 저장은 의미기반 dedup (유사 기존 fact UPDATE) ([Spec Agent Memory §3·§4](../../5-system/17-agent-memory.md)). `summary_buffer` 는 세션 간 추출이 없으므로 본 단계 미적용.
3. LLM이 도구 호출을 요청하면:
   a. `toolCalls`를 **조건 도구**(`cond_*`) / **KB 도구**(`kb_*`) / **MCP 도구**(`mcp_*`) / **표현 도구**(`render_*`) / **일반 도구**(`tool_*`) 로 분류한다. 구현은 먼저 등록된 `toolProviders` 중 `matches(tc.name)` 가 참인 첫 provider 를 찾고 (kb → mcp → render 순으로 등록), provider 매칭이 없으면 condition 이름 집합과 대조하며, 그래도 매칭이 없으면 일반 도구로 분류한다. 모든 provider prefix(`kb_`/`mcp_`/`render_`)·`cond_` 가 서로 disjoint 하므로 provider-우선 검사라도 분류 결과는 결정적이다 (구현: `AiConditionEvaluator.classifyToolCalls`, `ai-agent/ai-condition-evaluator.ts` — M-1 god-handler 분할로 추출; `ai-agent.handler.ts` 는 무상태 collaborator 로 위임). 이름 prefix 가 동일하더라도 provider 가 등록되지 않았으면 (예: `presentationTools` 빈 배열일 때 LLM 이 환각으로 `render_xxx` 를 호출한 경우) 매칭 안 됨 → 일반 도구 분류. `tool_*` 재작성 미완 상태에서 일반 도구로 분류된 호출은 현재 LLM 에 가짜 성공 stub `{result: "Tool <name> executed", arguments: {...}}` tool_result 를 회신한다 (도구 미연결을 알리는 `tool_call_not_implemented` 회신은 **미구현 (Planned)** — `tool_*` 도구 연결 입력 경로 재작성 시 도입 예정).
...
> **구현 진입점**: 첫 진입 turn 은 `AiTurnExecutor.executeMultiTurn`, 사용자 메시지 수신 후 후속 turn 처리는 `AiTurnExecutor.processMultiTurnMessage` (`ai-agent/ai-turn-executor.ts` — M-1 분할로 핸들러 facade 에서 추출). park/resume lifecycle 자체의 구동은 엔진 `AiTurnOrchestrator` (§6 서두 레이어 주석).
...
2. **사용자 메시지 수신 시:**
   ...
   c.fallback. **`pendingFormToolCall` 누락 시 (invariant 예외)**: dispatch ([Presentation 공통 §10.9](../6-presentation/0-common.md#109-form-submission-wire-format-internal-bus-sentinel)) 가 `action.type === 'form_submitted'` 로 form turn 진입했으나 `state.pendingFormToolCall` 가 falsy 인 경우 — 예: 사용자가 `render_form` 호출 없는 turn 에 `execution.submit_form` 명령을 직접 보냄, 또는 race condition 으로 `pendingFormToolCall` 가 이미 클리어된 상태에서 늦은 제출 수신. **silent drop 금지** — form JSON 데이터를 plain `ai_user` 메시지로 thread 에 push 하여 LLM 이 raw JSON 을 reasoning 입력으로 받아 자연어 응답을 생성하도록 fallback 한다. `logger.warn('[processMultiTurnMessage] form submission without pendingFormToolCall — fallback to plain user message', { executionId, nodeId, formData })` 로 진단 surface. 이는 §7.4 invariant ("`interactionType: 'ai_form_render'` 진입 ↔ `pendingFormToolCall` set 은 1:1") 의 예외 처리 경로 — invariant 위반 자체는 회귀 신호이나 사용자 surface 는 끊기지 않도록 graceful degradation (§12.4 의 KB/MCP 격리 패턴과 동형).
   d. Knowledge Base 가 설정된 경우 LLM 능동 호출 시 RAG 재검색
   d.5. **컨텍스트 메모리 재주입 (매 turn)**: `memoryStrategy` 에 따라 분기 (§6.1 단계 번호 참조) —
      - **`manual`**: `contextScope ≠ 'none'` 일 때 매 turn 마다 messages 배열을 `[system, ...injectedThread, ...selfHistory]` 로 재빌드 — `injectedThread` 는 자기 노드 turn 을 제외해 중복 방지 ([Spec Conversation Thread §5](../../conventions/conversation-thread.md#5-contextscope-자동-주입-세-ai-노드-공통)).
...
>
> AI Agent 의 출력은 6 케이스 + multi-turn 의 transient `resumed` 1 케이스 = 총 7 케이스로 분류된다. LLM 공통 wrapper ([공통 §5](./0-common.md#5-응답-형식-규약)) 를 따라 도메인 결과는 `output.result.*` 하위에, 에러는 `output.error.{code, message, details?}` 하위에, 사용자 인터랙션은 `output.interaction.{type, data, receivedAt}` 하위에 둔다.
>
...
>
> **범위 — multi-turn 종결 4값 한정**(`user_ended` / `max_turns` / `condition` / `error`). 아래 §7.1·§7.2 의 single-turn `endReason: "out"` 은 **의도적으로 패키지 도메인 밖**이다: backend 가 유니온으로 선언한 적 없이 인라인 추론할 뿐이고, single turn 은 `result.messages` 가 없어 대화 판정 대상이 아니다(대화 UI 게이트가 `messages` 부재로 먼저 거른다). **`'out'` 이 `AiAgentEndReason` 에 없는 것은 누락이 아니므로 추가하지 말 것** — 근거: [plan `is-conversation-output-restructure` §유니온 분기](../../../plan/complete/is-conversation-output-restructure.md).
>
...
| `meta.memory` | object? | handler return | `memoryStrategy ≠ 'manual'` 시에만 echo. `{ strategy, summarized, recalledCount, tokenBudgetUsed, compactedMessages? }` — `strategy` 는 적용 전략, `summarized` 는 이 turn 에 롤링 요약 압축이 발생했는지 (Boolean), `recalledCount` 는 persistent 회수 청크 수 (summary_buffer 는 `0`), `tokenBudgetUsed` 는 working-memory 토큰 추정 사용량, `compactedMessages?` 는 멀티턴 누적 `messages` 물리 압축(§6.2 d.6) 으로 제거된 메시지 수 (압축 미발생 시 생략). 적용 결과 (config echo 아님, Principle 2 정합). SoT: [Spec Agent Memory](../../5-system/17-agent-memory.md) |
...
> `meta.turnDebug[].toolCalls` 는 해당 턴의 provider tool(KB·MCP) 별 trace `{ toolCallId, name, providerKey, status: 'success' \| 'error', durationMs, startedAt?, finishedAt?, error? }` — `startedAt?`/`finishedAt?` 는 tool 실행 시작/종료 절대 시각(ISO8601, 둘 다 optional, [WS §4.4](../../5-system/6-websocket-protocol.md) 정합). 타입 단일 진실은 `interface ToolCallTrace` (`ai-agent/ai-turn-executor.ts`, M-1 분할로 핸들러 facade 에서 이동). provider 가 throw 한 경우에도 핸들러가 catch 해 `'error'` 로 마킹하고 LLM 에는 sanitize 된 에러 content 를 그대로 전달한다 (turn 자체는 계속 진행). condition tool / 일반 tool stub 은 즉시 결과를 만들므로 포함하지 않는다. WS `execution.tool_call_started` / `execution.tool_call_completed` 가 손실되어도 동일 데이터로 클라이언트가 복구 가능 (Conversation Inspector tool 항목 success / error 배지의 권위 출처).
...
> ⚠ **구현 현황 (single-turn 일반 LLM 실패는 현재 이 포트로 미라우팅 — Planned)**: 본 §7.3 `error` 포트 output shape 은 **목표 계약**이다. 현재 `executeSingleTurn` 의 일반 `llmService.chat` 호출(`ai-turn-executor.ts:1209` 첫 호출 / `:1439` tool-loop)은 try/catch 로 감싸이지 않아, single-turn 의 LLM 호출 실패(timeout/rate limit/API 오류)는 이 `error` 포트로 라우팅되지 않고 **엔진 레벨 무분류 `FAILED`** 로 귀결된다. 예외: §4.2 도구 정의 payload 예산 초과(`ToolDefinitionPayloadExceededError`)만은 `buildSingleTurnToolsOrError` 의 pre-flight try/catch 가 흡수해 이 shape 로 반환한다. 일반 chat 실패의 try/catch 래핑은 후속으로 남으며(`plan/in-progress/node-output-redesign/ai-agent.md` CRITICAL 추적), multi-turn 오류 종결(§7.9)은 엔진 `handleAiTurnError` 공통 경로로 이미 구현돼 있다 (§12.16 이 이 turn 종류별 비대칭을 명시).
...
| 필드 | 타입 | 출처 | 설명 |
|------|------|------|------|
| `config.*` | (raw echo) | Principle 7 | 첫 turn 은 `context.rawConfig`, 후속 turn 은 `state.rawConfig` 를 echo — frozen 범위는 한 turn (매 turn rehydration 이 현재 `node.config` 에서 fresh 재유도·재freeze, D3 fresh-per-turn) |
...
| `meta.durationMs` / 토큰 / `turnDebug` | — | (§7.1 과 동일 위치) | 진행 중 누적치를 노출해 References / LLM Usage 탭이 동작. **`turnDebug[].ragSources` 는 추가로 conversation Preview 의 보조 관찰성 레인(🔎 `rag` 행 · 📚 chip)도 소비**한다 ([Conversation Thread §8.6](../../conventions/conversation-thread.md#86-rag-source-신설--보조-관찰성-레인의-정식화) · [§9.3](../../conventions/conversation-thread.md#93-데이터-소스-선택)) — 형제 필드 `llmCalls` 는 debug 탭 전용 그대로다 |
| `status` | `"waiting_for_input"` | handler return | 엔진이 실행을 일시 정지 |
| `_resumeState` | object (top-level) | handler return | 다음 턴 처리에 필요한 internal state. expression resolver 에서는 비노출 (Principle 4.2). credential / 내부 상태 보호 — DB 저장 시 strip. **매 turn 재개**(모든 turn 재개가 §7.5 rehydration 단일 경로 — turn-단위 park)를 위해 엔진이 `_resumeState` 의 **credential-strip 부분집합**을 `_resumeCheckpoint` 로 `outputData` 에 영속한다 (아래 생명주기 비교표) |
...
> ⚠ **구현 현황 (structured `resumed` 스냅샷은 현재 미emit — Planned)**: 위 `status: 'resumed'` structured 스냅샷은 **목표 관측 계약**이다. 현재 엔진의 AI 대화 turn 경로(`ai-turn-orchestrator.service.ts` `processAiResumeTurn` → `handleAiMessageTurn`)는 AI 메시지 turn 에서 `message_received` interaction / structured `status:'resumed'` 스냅샷을 `setStructuredOutput` 으로 emit 하지 않는다 (`resumed` 스냅샷 emit 은 form/buttons 경로에만 존재). 다만 **라이브 조기 노출**(아래 `execution.user_message` WS 신호)은 구현돼 있어 진행 신호 자체는 유지된다. structured 스냅샷 구현(또는 본 절 폐기 결정)은 후속(`plan/in-progress/node-output-redesign/ai-agent.md` 추적). Information Extractor §5.5 도 동형 미구현이며 동시 처리 권고.
...
**`_retryState` top-level 필드** — `retryable === true` 일 때만 운반. shape 은 §7.4 `_resumeState` 의 부분집합 + `expiresAt: ISO 8601` (TTL — 기본 60분). credential 미포함 (~~`maskSensitiveFields` boundary strip~~ **allow-list 로 애초에 배제** — 그 boundary 는 2026-08-24 에 제거됐고, **이 배제는 그것과 무관**하다(`buildRetryState`/`buildResumeState` 가 옮길 키를 열거한다)). expression resolver / autocomplete 비노출 (Principle 4.2 / 4.2.1).

`retryable === false` 케이스에서는 `_retryState` 가 emit 되지 않는다 (재시도 진입 자체가 불가).

> Multi-turn 의 `error` 종결에서도 부분 수집 결과(`output.result.*` 의 `messages` / `turnCount` 등)와 `output.error` 가 **병존** 가능 — 부분 결과를 후속 노드가 활용할 수 있도록 둘 다 보존한다. `output.error` 존재 여부로 에러/정상을 판단한다 ([공통 §5](./0-common.md#5-응답-형식-규약)).
>
> **재시도 진입**: 사용자가 conversation thread 의 `system_error` item 우측 `[다시 시도]` 버튼을 클릭하면 WS 명령 `execution.retry_last_turn` 이 송신된다 ([Spec WebSocket §4.2](../../5-system/6-websocket-protocol.md#42-실행-제어-명령-client--server)). backend 가 `_retryState` 를 lookup → `expiresAt` 검증 → 새 NodeExecution row 를 spawn → 마지막 turn 단발 재진입(`processAiResumeTurn`, exec-park full B3 turn-park 모델 — 옛 장수 loop 폐기) → 마지막 user message 부터 LLM 재호출. 워크플로우 Re-run ([§13 replay-rerun](../../5-system/13-replay-rerun.md)) 과 다름 — 동일 Execution 안 노드 단위 재시도.
>
...
- `requestPayload` 에 시스템 프롬프트 및 전체 대화 이력이 포함될 수 있음에 유의 — credential 은 ~~`maskSensitiveFields` 가 boundary 에서~~ **egress(REST/WS)에서** 자동 마스킹 (2026-08-24 정정 — DB·표현식은 원문)
...
- **Canonical 타입 (C-1 후속 ③)**: 각 `llmCalls[]` 항목 = canonical `LlmCallRecord`(`codebase/backend/src/shared/llm-tracing/llm-call-record.ts`, AI Agent + Information Extractor 공유). 위 `requestPayload`/`responsePayload`/`durationMs` 외 `startedAt?`/`finishedAt?`(ISO 8601 wall-clock)도 보유 가능(전 필드 optional). turn 항목은 base `TurnDebugEntry`(`{ turnIndex, llmCalls?, totalDurationMs? }`)에 진단 delta(`toolCalls?`/`ragSources?`/`ragDiagnostics?`/`mcpDiagnostics?`)를 더한 superset 이다([AI 공통 §6 Canonical 타입 SoT](0-common.md#6-토큰-회계-meta)).
...
| `TOOL_DEFINITION_PAYLOAD_EXCEEDED` | `buildTools` 결과 도구 **정의(스키마)** 직렬화 크기가 hard 예산(`AI_AGENT_TOOL_PAYLOAD_HARD_BYTES`, 기본 256 KB) 또는 개수(`AI_AGENT_TOOL_COUNT_MAX`, 기본 128)를 초과 — **LLM 호출 전 pre-flight** (§4.2). 도구 **호출 횟수** 축인 `MAX_TOOL_CALLS_EXCEEDED`/`tool_call_budget_exceeded` 와 **다른 실패 지점**(정의 크기 vs 호출 횟수). `output.error` = `{ code, message, details: { retryable: false, totalBytes, budgetBytes, toolCount, culpritProvider? } }` (`retryable` 은 §7.3 형식대로 `details` 안 — LLM 계열 필수). message 에 해결법(`mcpServers[].enabledTools` allowlist / 서버 off). | false | runtime (pre-LLM) |
...
> **분류는 HTTP status 기반** (멀티턴 `chat()` 경로는 raw provider SDK 에러의 **top-level `.status` property** 를 받는다): 429→`LLM_RATE_LIMIT`, 401/403→`LLM_CALL_FAILED`(non-retryable), 5xx·network·timeout→`LLM_CALL_FAILED`(retryable). **명시 code passthrough**: 에러가 명시 `code` 를 포함하면 — 등록 code(`LLM_RESPONSE_INVALID` 등)뿐 아니라 **미등록 vendor 코드(예: `LLM_PROVIDER_QUOTA`·`LLM_API_ERROR`)도** — 그대로 보존(passthrough)하고 `retryable=false` 로 둔다(별도 `AI_*` 코드로 래핑하지 않음 — §10 단일 LLM taxonomy 의 연장). status·code·network 신호가 **모두 없는** 경우에만 `LLM_CALL_FAILED`(non-retryable) fallback 으로 매핑한다. 회귀가드 테스트: 후속 ② PR #630. 구현: `AiTurnOrchestrator.classifyLlmError`(공개 진입점 `extractAiTurnErrorPayload`, `ai-turn-orchestrator.service.ts` — C-1 분할로 엔진에서 이동).
...
**도구 정의 payload 예산 경고 (저장 시점)**: `mcpServers`/`presentationTools` 로부터 재현한 도구 정의 payload (§4.2 estimator) 가 예산을 초과하면 [cross-node-warning-rules §4·§5·§8](../../conventions/cross-node-warning-rules.md) 의 `GraphWarningRuleResult`(rule id `ai_agent:tool-payload-budget`, 기본 severity `warning`)로 표면화된다. 이 backend-only 결과의 **surfacing 은 서버 권위 조회 endpoint `GET /workflows/:id/graph-warnings`(`getGraphWarnings`)** 가 결과 배열에 append 해 담당한다(**별도 응답 필드 신설 없음** — saveCanvas 응답은 graph warning 을 싣지 않는 기존 계약 유지). 저장(`POST /workflows/:id/save`, `saveCanvas`)은 기본(warning) 시 통과하되, `AI_AGENT_TOOL_BUDGET_STRICT_SAVE=true` 면 hard 초과를 severity `error` 로 승격해 `saveCanvas` 가 기존 `GRAPH_VALIDATION_FAILED` 로 400 차단한다(3중 가드 ①). config-time 재현은 connected cafe24/makeshop 정적 카탈로그 + presentation 도구만 집계하며, generic MCP(`service_type='mcp'`)·비-connected 통합은 live connect/토큰 side-effect 없이 재현 불가라 best-effort skip 한다. 이 rule 은 통합 granted scope 의 **async 조회**가 필요해 pure/shared `@workflow/graph-warning-rules` 규칙(frontend/backend 동기 SSOT)으로 표현 불가 → **backend-only** 평가(`WorkflowsService`). 따라서 frontend canvas 가드 ②(로컬 pre-evaluate)는 이 rule 을 계산하지 않으며, 실질 안전망은 런타임 fail-fast(`TOOL_DEFINITION_PAYLOAD_EXCEEDED`) + saveCanvas 가드 ① 이다. 기본이 warn 인 이유: 저장 시 추정은 근사(런타임 granted scope·live MCP tool list 미확정)라 정상 설정 오차단을 피하기 위함 (§12.15).

## 11. 캔버스 요약

[공통 §8](./0-common.md#8-캔버스-요약) — `AI Agent` 행 인용. mode·model·KB 수·MCP 수·조건 수·표현 도구 수를 동적으로 조합 (`multi_turn` 시 `Multi Turn` 접두어).

> ⚠ **미구현(Planned)**: `summaryTemplate` 부재로 현재 AI Agent 캔버스 요약은 표시되지 않는다([공통 §8](./0-common.md#8-캔버스-요약) 참조). 아래는 목표 예시. 구 `· {N} tools` 세그먼트는 Tool Area 제거로 폐기.

목표 예: `gpt-4o · 1 KB · 1 MCP · 3 cond` (single) / `Multi Turn · gpt-4o · 1 KB · 2 MCP · 2 cond · 3 render` (multi, `presentationTools.length === 3`)
...
**v1 vs v2 경계**:
- v1: 단일 thread, char 기반 cap, in-memory + NodeExecution 분산 SoT (실행 이력) + `Execution.conversation_thread` durable park 스냅샷 (park→재시작/타 인스턴스 재개 무손실 — [conversation-thread §4·§8.4](../../conventions/conversation-thread.md#84-executionconversation_thread-컬럼-채택--durable-park-resume)), `ai_agent` 만 자동 주입 (push 는 3 AI 노드 모두).
- v2: multi-thread, token-aware cap, `text_classifier`/`information_extractor` 도 주입, 실행 이력 화면의 cross-node thread view 재구성 (NodeExecution 분산 저장 derived view, N+1 회피 — EH-DETAIL-12).
...
- **추정기는 정확 tokenizer 가 아니라 language-aware lite 휴리스틱 (A4 lite)**: 메모리 예산의 토큰 추정은 균일 `char/3` 대신 스크립트군별 chars-per-token 가중 (Latin ~4, CJK ~1.7, 그 외 ~3) 으로 산정한다. **정확 tokenizer (js-tiktoken 등 신규 의존성) 를 도입하지 않은 근거**: (1) 주력 Claude 모델용 로컬 tokenizer 가 부재해 정확 카운트는 네트워크 콜이 필요하나 추정은 매 turn 동기 hot-path 라 부적합, (2) 신규 의존성 0 원칙 — 청킹 경로(`text-chunker` char/3)와 동일하게 무의존 유지, (3) provider tokenizer-exact 는 §7 v3 로드맵 항목으로 의도적 보류. lite 휴리스틱은 혼합 스크립트(한국어+영어)에서 단일 비율 대비 편향만 줄이는 **근사 개선** 이며, 정확 카운트를 약속하지 않는다. 적용 범위는 **메모리 예산 경로 한정** (`agent-memory-injection`) — KB 청킹·csv 청킹·thread char-cap 은 무변경 (회귀 0).
...
- **agent_memory 테이블은 `conversation_thread` 컬럼과 별개 저장소**: 본 작업(`persistent` 세션 간 메모리)의 DB 영속은 **별도 테이블 `agent_memory`** ([Spec Agent Memory](../../5-system/17-agent-memory.md)) 다. 이는 ConversationThread 본문의 durable resume 영속용 `Execution.conversation_thread jsonb` 컬럼([conversation-thread §4·§8.4](../../conventions/conversation-thread.md#84-executionconversation_thread-컬럼-채택--durable-park-resume) — 별도 작업으로 채택)과 목적·저장소가 모두 분리된다 (agent_memory = 세션 간 추출 사실 검색 회수, conversation_thread = 한 실행 내 in-flight thread 무손실 재개). 실행 중 요약 보관 필드 (`runningSummary`/`summarizedUpToSeq`) 의 영속 경로·유실 fallback 은 §12.13.
...
### 12.12 요약·추출 전용 LLM 모델 옵션 (`summaryModelConfigId` / `extractionModelConfigId`)

**과거 결정 (v1, scope-freeze)**: 초기에는 `summary_buffer`/`persistent` 의 **요약 LLM 콜** (1.5) 과 `persistent` 의 **추출 LLM 콜** (2.7) 이 별도 모델 필드를 신설하지 않고 노드 `model`/`llmConfigId` 를 그대로 재사용했다. 당시 **`summaryModel` 전용 필드 도입을 기각**한 근거는 (a) config surface 최소화 (scope-freeze), (b) 노드가 이미 선택한 provider 의 토큰 회계·rate-limit·credential 경로 단일 재사용, (c) 요약·추출이 노드 main 모델과 동일 품질을 쓰도록 보장 — 이었고 전용 모델 옵션은 v2 로드맵으로 유보했다.

**현 결정 (번복 — 전용 필드 도입)** — ⚠️ *이 단락의 필드명(`summaryModel`/`extractionModel`)·저장 형태(모델명 문자열)·fallback 체인은 아래 "재번복 결정"(§1297 단락)에서 `summaryModelConfigId`/`extractionModelConfigId`(`config.id` 저장, provider 디커플)로 대체됐다. 본 단락은 "전용 필드 도입" 결정 자체의 의사결정 이력으로만 보존한다*: 이제 `summaryModel` / `extractionModel` **두 optional 필드를 도입**한다 (요약/추출 경로를 독립 지정). 이는 위 v1 기각을 의도적으로 **번복**하는 것으로, 번복 근거는 다음과 같다 — (1) **비용 절감**: 요약·추출은 메인 응답 추론보다 단순한 압축/추출 작업이라, 메인 추론에는 고성능 모델을 쓰면서 보조 콜에만 저비용 모델 (예: 소형/저가 tier) 을 지정하면 멀티턴 누적 보조 콜의 비용을 실질적으로 줄일 수 있다. (2) **품질 요구 분리의 정당성**: 메인 추론 품질 요구 (정확한 답변 생성) 와 요약/추출 품질 요구 (사실 보존·간결 압축) 는 성격이 달라, 둘을 같은 모델에 묶을 본질적 이유가 없다 — v1 의 (c) "동일 품질 보장" 은 *제약*이었지 *요구*가 아니었다. (3) **안전성 (하위호환)**: 두 필드는 모두 optional 이고 **미설정 시 fallback 체인** `[전용필드] → [노드 model] → [llmConfig 기본]` 으로 기존 동작을 100% 보존한다 — 즉 v1 기각 근거 (a) scope-freeze 의 보호 대상이던 "기본 동작 회귀 0" 불변식은 그대로 유지된다. credential/provider 경로는 여전히 노드 `llmConfigId` 를 단일 재사용하므로 (b) 의 우려 (provider/credential UI 추가) 도 발생하지 않는다 — 신규 필드는 **모델 식별자(모델명 문자열) 1개씩**일 뿐 provider 선택 UI 를 추가하지 않는다 (v2 는 expression 입력, 이후 등록 모델 select 로 전환 — 아래 "후속 결정" 참조. provider 무추가 불변식은 select 전환 후에도 유지). 따라서 v1 기각의 핵심 우려 (a)(b) 는 본 설계가 그대로 충족하고, (c) 는 *제약 완화*로서 의도적으로 푼다.

**후속 결정 (입력 위젯: expression 문자열 → 등록 모델 select)** — ⚠️ *이 단락의 "노드 provider 내 모델명 select" 설계(`chat-model-selector`/`embedding-model-selector`, 모델명 문자열 저장)는 바로 아래 "재번복 결정" 단락에서 다시 번복돼 **등록 ModelConfig 선택(`config.id` 저장)**으로 대체됐다. 본 단락은 의사결정 이력으로만 보존하며 현행 구현·위젯은 재번복 결정을 따른다*: v2 에서 세 모델 필드(`embeddingModel` / `summaryModel` / `extractionModel`)는 자유 입력(`embeddingModel` 은 `text`, summary/extraction 은 `expression`)이었다. 이를 **노드 `llmConfigId` provider 의 등록 모델만 고르는 순수 select**(`embedding-model-selector` / `chat-model-selector`)로 전환한다. 근거 — (1) **잘못된 모델 ID 저장의 구조적 차단**: 자유 입력은 오타·존재하지 않는 모델명을 그대로 저장해 런타임 콜 실패(특히 `embeddingModel` 은 차원 불일치로 회수가 *조용히* 실패)를 유발한다. 등록 모델 select 는 이를 구조적으로 막는다 ([KB §Rationale R-1](../../2-navigation/5-knowledge-base.md) 의 select-only 원칙과 동일 논리). (2) **§12.12 핵심 불변식 보존**: select 는 노드가 이미 고른 `llmConfigId` provider 내부의 모델명만 노출하므로, v2 가 지킨 "provider/credential UI 무추가" (위 (b)) 가 그대로 유지된다 — 신규 provider 선택 UI 가 아니라 기존 provider 의 모델명 picker 일 뿐이다. (3) **하위호환**: 저장 형태는 여전히 모델명 문자열이고 fallback 체인·런타임 resolve 경로는 불변이며, 기존에 저장된 임의 모델명은 select 의 saved-value fallback 으로 계속 노출된다. **trade-off**: v2 가 허용하던 `{{ }}` 동적 모델명(실행 시점 모델 주입)은 제거된다 — 보조 콜 모델을 실행마다 다르게 주입하는 사용 패턴은 드물고, 잘못된 ID 저장 차단의 안전성 이득이 그 유연성보다 크다고 판단한다.

**재번복 결정 (등록 모델명 → 등록 ModelConfig 선택 + provider 디커플)**: 위 "후속 결정"은 모델**명 문자열**을 노드 `llmConfigId` provider **안**에서 골랐다(저장 형태 = 모델명, 런타임 = 노드 provider 재사용). 이를 다시 **`/models` 에 등록된 ModelConfig 한 건을 `config.id` 로 고르는 방식**으로 전환한다 — 세 필드를 `embeddingModelConfigId` / `summaryModelConfigId` / `extractionModelConfigId` 로 개명하고, 위젯은 `embedding-config-selector`(embedding kind) / `chat-config-selector`(chat kind), 저장 형태는 `config.id`(LLM 제공자 셀렉터와 동일)다. 이는 위 "후속 결정"이 지키려던 핵심 제약 — **"노드가 고른 `llmConfigId` provider 내부 모델만"(위 (2)·v1 (b))** — 을 의도적으로 **번복**한다. 번복 근거 — (1) **provider 횡단 비용 절감의 정당성**: §12.12 본 결정의 동기는 "보조 콜에 저비용 모델"이었는데, 노드 main provider 안으로 모델을 가두면 *다른* provider 의 더 싼 모델(예: main 은 GPT-4o, 요약은 타 provider 소형 모델)을 못 쓴다 — 이는 본 결정의 비용-절감 동기를 스스로 제약한 것이었다. ModelConfig 는 provider+credential+defaultModel 을 1급으로 묶으므로, config.id 선택은 보조 콜을 **노드 main LLM 과 완전히 독립**시켜 비용 동기를 온전히 실현한다. (2) **검증된 KB 선례 재사용**: KB 의 `embeddingModelConfigId` + `ModelConfigService.resolveEmbedding` 폴백 체인은 이미 운영 중인 1급 패턴이다 — embedding 필드를 이 선례에 정렬해 "저장·회수 동일 config → 차원 일치" 불변식(agent-memory §3)을 KB 와 같은 코드 경로로 보장한다. (3) **잘못된 ID 저장 차단 유지**: 등록 ModelConfig select 이므로 위 "후속 결정"의 안전성 이득((1) 오타·미존재 차단)은 그대로 유지된다 — config 가 삭제되면 stale 경고를 노출한다. **trade-off**: ⓐ `config.id` 저장이라 종전 모델명 문자열과 비호환 — 기존 노드의 옛 `embeddingModel`/`summaryModel`/`extractionModel` 값은 폐기되고 미설정 폴백(노드 main LLM / 워크스페이스 기본 embedding)으로 graceful degrade 한다(보조 콜이라 대화 무결성 영향 없음). ⓑ provider 디커플로 "노드 토큰 회계·rate-limit 단일 경로"(v1 (b)) 가 보조 콜에 한해 깨지지만, 보조 콜은 그 config 자신의 회계 경로로 집계되므로 회계 누락이 아니라 **분리**일 뿐이다.

### 12.13 요약 보관 필드 (`runningSummary` / `summarizedUpToSeq`) 유실 시 fallback

**결정**: `runningSummary` / `summarizedUpToSeq` ([conversation-thread §1.3](../../conventions/conversation-thread.md#13-conversationthread)) 는 thread 의 일부이므로 **park 시 `Execution.conversation_thread` 컬럼에 thread 와 함께 durable 영속**된다 ([conversation-thread §4·§8.4](../../conventions/conversation-thread.md#84-executionconversation_thread-컬럼-채택--durable-park-resume)) — park→재시작/타 인스턴스 재개는 이 스냅샷에서 무손실 복원하므로 요약 보관 필드도 보존된다. 단 **park 사이 active 세그먼트 진행 중**에는 in-memory `ExecutionContext` 에만 존재하므로, 그 구간에서 TTL 만료 / Redis·프로세스 장애로 ExecutionContext 가 유실되고 직전 park 스냅샷도 없으면 (예: 첫 park 이전) 요약 보관 필드가 사라질 수 있다. 이때 **유실된 요약을 재요약으로 복구하지 않고, 원문 thread (messages history) 로 컨텍스트를 재구성하는 graceful degradation 으로 fallback** 한다 — 요약은 derived 압축물이라 손실돼도 대화 자체 (`output.result.messages` 누적, [conversation-thread §4](../../conventions/conversation-thread.md#4-영속화) NodeExecution SoT) 는 보존되므로, 다음 호출은 요약 없이 원문 turn 으로 진행하고 예산 초과 시 다시 임계치에서 롤링 요약을 시작한다. 요약 손실은 토큰 효율 저하일 뿐 대화 무결성 손상이 아니다.
...
### 12.15 도구 정의 payload 예산 (§4.2) 도입 근거

**배경 (회귀 사고)**: 2026-07-06 커밋(#828, "Cafe24 field-set docs 전량 미러")이 Cafe24 오퍼레이션 **개수는 그대로** 둔 채 각 오퍼레이션의 파라미터 스키마를 필드 단위로 팽창시켰다(`order.ts` 1,353→5,742줄 등, 메타데이터 +18,879줄). Cafe24 MCP 서버를 `enabledTools` allowlist 없이 연결한 AI Agent 는 scope 허용 383개 오퍼레이션 전량을 도구로 노출했고, 팽창한 스키마로 도구 정의 payload 가 ~118k 토큰까지 커졌다. 그 결과 Gemini·Ollama(로컬 gemma-4-26b) **양쪽 모두** LLM 호출이 프롬프트 처리 단계에서 transport timeout 을 넘겨 실패했다(local: 120초 SDK timeout×3 재시도=6분 "무한 반복", Gemini: 연결 종료). 사용자 설정·AI 턴 코드는 불변이었고 바뀐 것은 카탈로그 데이터의 스키마 상세도뿐이었다.

- **왜 bytes 우선 지표인가**: #828 은 도구 개수 불변(383), 도구당 크기만 팽창 → **개수 cap 만으로는 이 사고를 못 잡는다**. 실패의 물리량은 프롬프트 토큰이고 그 결정론적 근사가 도구 정의 직렬화 bytes 다. 개수 cap 은 값싼 2차 sanity 로만 둔다.
- **왜 런타임 fail-fast 인가**: 기존에는 앱 레벨 타임아웃 없이 provider SDK(120초×재시도)에만 의존해 최대 수 분 hang + 정체불명 종료(`Request timed out.`/`400`)로 나타났다. `buildTools` 직후 pre-flight throw 로 즉각·명확한 실패(범인 provider 지목)로 전환해 진단 비용을 없앤다.
...
- **`LLM_CALL_FAILED` vs `LLM_TIMEOUT` disambiguation**: 본 app-level timeout(multi-turn 분류 시)은 [§5-system/7-llm-client.md](../../5-system/7-llm-client.md) 의 기존 분류(network/timeout → `LLM_CALL_FAILED`, retryable)를 따른다 — **ai_agent 노드 실행 경로는 timeout 을 `LLM_CALL_FAILED` 로 분류하며 `LLM_TIMEOUT` 을 발행하지 않는다.** `codebase/backend/src/nodes/core/error-codes.ts` 의 `LLM_TIMEOUT` 은 별개 enum 이다(현재 어느 실행 경로도 미발행 — enum 자체의 사용처·스코프 정리는 error-handling taxonomy 소관, 본 §12.16 범위 밖). 이 문단은 "ai_agent timeout ⇒ `LLM_CALL_FAILED`" 만 규정한다.
...
- **왜 timeout 이 signal 과 독립인가**: `withTimeout` 은 자체 `AbortController` 로 동작하므로 노드 `abortSignal` 소스 유무와 무관하게 적용된다. **resume 경로(`processMultiTurnMessage`)에는 전파할 signal 이 애초에 없다** — 사용자 Stop 은 `AbortController` 를 만들지 않고 Execution 행만 UPDATE 하며, 저장소에서 `AbortController` 를 만드는 곳은 `parallel-executor.ts`(cancel-others-on-fail) 한 곳뿐이다. 그래서 그 경로의 취소는 signal 전파가 아니라 **turn 경계 DB 관측 가드**로 처리한다 (구현됨 2026-07-27 — [node-cancellation.md §2.4](../../conventions/node-cancellation.md)). `ResumableMessageOptions.signal` 은 **현재 항상 `undefined` 인 executor-side plumbing** 이다(향후 abort 소스가 생기면 전파되도록 열어둔 자리이지, 도입이 예정된 것은 아니다). timeout 백스톱은 이와 무관하게 지금도 동작한다.
...
- **provider SDK 타임아웃과의 관계 (실효 상한 = 더 작은 쪽)**: 앱 레벨 타임아웃은 provider SDK 자체 타임아웃과 **병렬 적용**되어 둘 중 먼저 발화하는 쪽이 실효 상한이 된다. OpenAI/Anthropic/Azure/Local(OpenAI 계열) 클라이언트는 SDK 생성 시 ~120초 request timeout 이 이미 박혀 있어, 기본 600000ms(10분)보다 짧다 — 즉 이 provider 들에선 대개 SDK 타임아웃이 먼저 발화하고 앱 레벨은 "그보다 긴 hang"(SDK 타임아웃이 없거나 비활성인 구성) 및 **호출당 단일 설정 knob** 역할을 한다. 앱 레벨을 1차 상한으로 쓰려면 env 를 provider SDK 타임아웃보다 낮게 설정한다.
- **타임아웃이 실제 요청을 취소하도록 signal 배선**: `LlmService.chat` 은 `withTimeout` 이 콜백에 넘기는 내부 timeout signal 을 호출자 `opts.signal` 과 `AbortSignal.any` 로 병합해 SDK 요청에 전달한다 — 타임아웃 발화 시 `AbortController.abort()` 가 실제 provider HTTP 요청을 취소해 소켓이 백그라운드에 leak 되지 않는다(`listModels` 와 동일 패턴). Google client 도 `chat(params, signal)` 로 signal 을 받아 `generateContent` 의 `abortSignal` 로 전달한다. (`embed` 경로는 본 배선 밖 — client.embed 가 signal 미지원, 별도 후속.)

### 12.17 Multi Turn `out` 포트 부재 — ND-AG-24 "조건 0개 하위호환 out" 문구 정정 (2026-07-18)

**배경**: 요구사항 ND-AG-24 (`spec/4-nodes/_product-overview.md` · `spec/4-nodes/3-ai/_product-overview.md` 2곳) 는 "Multi Turn 조건 0개 시 `out` + `error` 제공 (하위 호환)" 이라 서술했으나, 이는 기술 spec §3.2 ("Multi Turn 모드에는 `out` 포트가 존재하지 않는다 … 조건이 0개인 경우에도 동일") 및 코드 SoT 와 정면 모순이었다 (consistency plan_coherence 5회 연속 노출). 본 절은 §3.2 를 정본으로 확정하고 ND-AG-24 를 정정한 근거를 남긴다.

**코드 SoT (양축 확정)**:
- 포트 **정의** — frontend `resolve-dynamic-ports.ts` 의 `aiAgentConditionalPorts` (`condPorts.length === 0 && isMultiTurn` 분기) 는 `user_ended`/`max_turns`/`error` 만 발행하고 `out` 을 발행하지 않는다 (회귀 테스트 `resolve-dynamic-ports.test.ts` "multi_turn + no conditions → user_ended + max_turns + error").
- 포트 **라우팅** — backend `ai-turn-executor.ts` 의 `multiTurnPortForEndReason` 는 `user_ended`/`max_turns`/`error` 만 반환하고 `condition`·default 는 방어적으로 `error` 로 매핑한다. 어느 `endReason` 도 `out` 으로 귀결하지 않는다.

**결정**: ND-AG-24 의 "조건 0개 하위호환 out" 은 **구현된 적이 없는** 서술이었다 — §3.2 마이그레이션 절이 이미 "레거시 multi_turn + 조건 없음 노드의 `out` 엣지는 dangling 되어 `user_ended`/`max_turns` 로 수동 재연결" 이라고 규정해, 하위호환 out 을 **제공하지 않는** 것이 확정된 계약이다. 따라서 이는 설계 결정의 번복이 아니라 기술 spec·코드와 어긋난 요구사항 텍스트 잔재의 삭제다. ND-AG-24 양쪽을 "조건 0개에서도 `out` 없음" 으로 정정했다. (앞서 durable 추적하던 `spec-drift-ai-agent-outport-countmax.md` Critical 1 은 본 정정으로 종결 → `plan/complete/` 이관.)
