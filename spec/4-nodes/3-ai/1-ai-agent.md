...
> **라이브 조기 노출 (`execution.user_message`)**: 위 observability 기록과 **별개로**, 엔진은 같은 수신 시점(다음 턴 LLM 호출 전)에 WS 이벤트 `execution.user_message` 를 1회 emit 해 사용자 발화(q)를 라이브 대화 surface 에 즉시 노출한다 ([Spec WebSocket §4.4 `execution.user_message`](../../5-system/6-websocket-protocol.md#44-실행-진행-이벤트) / §4.4.6 / §588 Reconciliation). 이 이벤트의 `nodeExecutionId` 는 **이 시점에 `waiting_for_input` 상태였던 NodeExecution row 의 PK** ([실행 엔진 §1.3](../../5-system/4-execution-engine.md) 의 `execution_id + node_id + status='waiting_for_input'` 단일 매칭과 동일 row), `receivedAt` 은 `output.interaction.receivedAt` 과 동일 값이다. 권위 출처는 여전히 turn 종료 `execution.ai_message.messages` 스냅샷이며 (§7.4), `user_message` 는 라이브 전용 비권위 진행 신호다 — `output.result.messages` 의 turn-경계 영속 정책(§7.4)은 불변(라이브 신호는 영속 대상 아님).
