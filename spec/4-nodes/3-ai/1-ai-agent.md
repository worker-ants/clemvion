...
| embeddingModel | String | | — | `memoryStrategy: 'persistent'` 시 메모리 회수·추출(저장)에 쓸 임베딩 모델 식별자. 회수와 저장이 **같은 모델**을 써야 차원이 일치한다. 미설정 시 워크스페이스 기본 embedding ModelConfig 임베딩 모델 → 최후 하드코딩 기본(`text-embedding-3-small`) 으로 폴백. SoT: [Spec Agent Memory §3 임베딩 출처](../../5-system/17-agent-memory.md) |
| summaryModel | String (Expression 가능) | | — | `memoryStrategy ∈ {summary_buffer, persistent}` 시 **롤링 요약 압축 LLM 콜** (§6.1 단계 1.5) 에 쓸 전용 모델 ID. 미설정 시 노드 `model` → ModelConfig 기본 모델로 폴백 (기존 동작 유지). 메인 추론보다 단순한 요약 작업에 저비용 모델을 지정해 비용을 줄일 수 있다 (§12.12) |
| extractionModel | String (Expression 가능) | | — | `memoryStrategy: 'persistent'` 시 **턴 경계 추출 LLM 콜** (§6.1 단계 2.7) 에 쓸 전용 모델 ID. 미설정 시 노드 `model` → ModelConfig 기본 모델로 폴백 (기존 동작 유지). 저비용 추출 모델 지정으로 비용 절감 (§12.12) |
