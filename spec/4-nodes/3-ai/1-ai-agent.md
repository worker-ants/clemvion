...
**`_retryState` top-level 필드** — `retryable === true` 일 때만 운반. shape 은 §7.4 `_resumeState` 의 부분집합 + `expiresAt: ISO 8601` (TTL — 기본 60분). credential 미포함 (~~`maskSensitiveFields` boundary strip~~ → **egress 마스킹**, 2026-08-24 정정). expression resolver / autocomplete 비노출 (Principle 4.2 / 4.2.1).
