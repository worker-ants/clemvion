...
3. LLM이 도구 호출을 요청하면:
   ...
   d.i. **표현 도구 (`render_*`) display-only (`table`/`chart`/`carousel`/`template`)**: payload 를 해당 presentation 노드의 zod schema 로 validate → `defaults` overlay 적용 → 1MB cap 적용 → ConversationTurn (현재 turn 의 `ai_assistant`) 의 `data.presentations[]` 에 push → tool_result 로 `{ok: true}` 스텁 회신. LLM 이 동일 turn 안에서 이어서 텍스트를 생성하거나 다른 도구를 호출할 수 있다. validate 실패 시 §4.1 "Schema 위반 처리" 적용.
