...
### 12.6 `render_form` submit 후 LLM 의 동일 form 재호출 회귀 차단 (2026-05-24)

**문제** (사용자 보고 2026-05-24): `render_form` 으로 띄운 form 을 사용자가 submit 한 직후, 같은 turn 의 LLM 응답이 **동일한 `render_form`** 을 다시 emit 해 form 이 또 떠 버린다. 재현 시나리오 — 사용자가 "샘플상품 1 → 문의하기" 흐름의 form 에 subject=대량구매 / content=1000개 구매하려는데 납품 가능일… 입력 후 submit → 즉시 같은 "샘플상품 1 문의하기" form 이 재렌더링.

**원인 분석**:

회귀 시점은 `30e02117` (PR #299, 2026-05-23). 그 PR 이전 tool_result content 는 stub `{ok:true, pending:'form_submission'}` 였고, submit 시 동일 shape 유지로 `ok:true` 신호가 살아 있었다. PR #299 가 submit 후 content 를 `{type:'form_submitted', data:{…}}` 로 교체하면서 `ok:true` 가드 신호가 소실됐다.

다음 두 가지가 결합해 회귀를 일으켰다:
...
| 안 | 장점 | 단점 | 채택 |
|---|---|---|---|
| (A, **채택**) tool_result content shape 보강 — 기존 `{type, data}` 유지 + 가드 필드 `ok:true`, `message:'<재호출 금지 + 후속 행동 유도 안내문>'` 추가 + `PRESENTATION_TOOLS_GUIDANCE` 에 `form_submitted` 케이스 안내 라인 추가 | 4-layer SSOT 의 다른 layer (NodeOutput interaction.type / internal bus sentinel / WS wire) 영향 0. PR #278 (`e4b7aa12 render_* 무한 retry 차단 — rich tool_result`) 의 동형 라인. LLM reasoning 자율성 (§12.5) 유지 — 안내문은 상태 신호이지 행동 강제가 아님 | display-only 의 `rendered:true` 와 동형 키 `rendered:false` 추가는 의미 혼동 우려로 별도 기각 | ✅ |
| (B, 기각) render-tool-provider 에 same-execution form idempotency 가드 추가 (직전 tool_result 가 form_submitted 면 같은 toolCallId 의 재호출은 schema violation 으로 reject) | 코드 가드 강제 | LLM 의 reasoning 자율성 침해 (§12.5 와 충돌) + 실제 회귀 원인 (가드 신호 누락) 우회. tool_result 보강이 회귀를 닫는지 먼저 검증해야 함 | ❌ (추후 별도 plan 으로 분리, 본 PR scope 제외) |
| (C, 기각) form_submitted tool_result 와 함께 `messages.push({role:'user', content:'<제출된 form data 요약>'})` 도 추가 | 모델이 user role 메시지를 통해 명시적 신호를 받음 | 동일 시그널을 두 layer (tool_result + user message) 에 중복 운반. ConversationThread 의 `presentation_user` source 와의 의미 중복. spec 변경 면적 큼 | ❌ |
...
**근거**: 사용자 보고 (2026-05-24) — 스크린샷 + 회귀 재현. 사용자 결정 (2026-05-24): spec 동반 갱신을 본 PR 에 포함 (consistency-check 결과 BLOCK 해소 + shape 설계 권장 반영 후 spec write). plan: [plan/in-progress/form-resubmit-fix.md](../../../plan/in-progress/form-resubmit-fix.md). consistency-check 세션: `review/consistency/2026/05/24/16_37_48/`.

### 12.7 `render_form` submit 후 formData 크기 cap (2026-05-24)

**문제** (PR #301 ai-review security INFO #1): §12.6 에서 `form_submitted` tool_result content shape 을 `{ok, type, data, message}` 로 보강했으나, `data` 필드 (사용자가 form 에 입력한 값) 자체에는 크기 제한이 없다. 사용자가 form 의 textarea 에 대량 텍스트 (예: 100KB) 를 입력하면 그 전체가 LLM 컨텍스트로 직접 직렬화되어 token 비용 폭주 + LLM rate limit / context window 초과 위험.
...
- `formDataTruncation` (≠ `truncation`) — `PresentationPayload.truncation` (display-only render_* 의 carousel/table tail-truncate 메타, §7.10) 과 키 이름 분리. consistency-check `naming-collision` 권고.
- `bytesAfterCap` (≠ `cappedBytes`) — `render-tool-provider.ts` 의 로컬 변수 `cappedBytes` 와 의미 분리. 같은 권고.
...
| 안 | 장점 | 단점 | 채택 |
|---|---|---|---|
| (A, **채택**) per-field string 균등 truncate + `formDataTruncation` 메타 | 모든 필드 보존 — LLM 이 부분 정보로도 reasoning 가능 + 어떤 필드가 잘렸는지 식별 | 단일 거대 필드 (큰 textarea 1개) 케이스에서 다른 필드 truncate 의미 적음 | ✅ |
| (B, 기각) 전체 reject — cap 초과 시 tool_result 에 error 회신 + LLM 에 retry 요청 | 명확한 에러 신호 | form submit 은 사용자 행위라 retry 가 의미 없음 (사용자가 입력 줄여 재제출해야 함). 사용자 UX 단절 | ❌ |
| (C, 기각) tail-truncate (마지막 필드 통째로 drop) — render-tool-provider 의 1MB cap 패턴 직접 차용 | 알고리즘 단순 | 필드 순서가 의미를 가지지 않는 form 에서 무작위 drop 효과. LLM 이 어느 정보가 사라졌는지 알기 어려움 | ❌ |
...
**근거**: PR #301 ai-review security INFO #1 (`review/code/2026/05/24/17_12_34/security.md`). plan: [plan/in-progress/ai-agent-formdata-size-limit.md](../../../plan/in-progress/ai-agent-formdata-size-limit.md). consistency-check 세션: `review/consistency/2026/05/24/18_00_25/` (BLOCK: NO, WARNING 5건 모두 본 절 / plan rewrite 에 반영).
