...
**런타임 판정 위치**: `AiTurnExecutor.buildTools` 직후 (single-turn·multi-turn resume 공통 헬퍼). hard(또는 count) 초과 시 **LLM 호출 전에** `TOOL_DEFINITION_PAYLOAD_EXCEEDED` throw → §7.3/§7.9 `error` 포트(`details` 구조화). provider SDK timeout 에만 의존하던 최대 수 분의 hang 대신 즉각·명확한 실패로 전환. soft 초과는 실패시키지 않고 로깅만. **single-turn 경로**(`executeSingleTurn`)는 현재 이 throw 를 감싸는 try/catch 가 없으므로 (multi-turn `handleAiMessageTurn` 과 비대칭) 본 에러코드가 `output.error`+`error` 포트로 귀결되도록 라우팅을 보장한다.
