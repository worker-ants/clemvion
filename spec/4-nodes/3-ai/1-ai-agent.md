...
| mcpServers | McpServerRef[] | | `[]` | MCP-capable Integration 참조 목록. `service_type ∈ ('mcp', 'cafe24')` 모두 수용 — 후자는 backend `Cafe24McpToolProvider` 가 in-process `AgentToolProvider` 구현체로 동작 ([Spec MCP Client §2.3 Internal Bridge](../../5-system/11-mcp-client.md#23-internal-bridge-in-process)). [공통 §3](./0-common.md#3-mcp-서버-연결-ai-agent-전용) |
...
2. **사용자 메시지 수신 시:**
   ...
   b. 엔진이 `status: 'resumed'` 스냅샷을 1회 emit (§7.5) — 일반 채팅은 `output.interaction.{type:'message_received', data:{content, role:'user'}, receivedAt}` 운반, form 제출은 `output.interaction.{type:'form_submitted', data:{<field>:value}, receivedAt}` 운반. **일반 채팅(`message_received`) 시 엔진은 같은 시점에 WS `execution.user_message` 라이브 이벤트도 함께 emit** 해 사용자 발화를 AI 응답 전에 조기 노출한다 (§7.5 의 라이브 조기 노출 노트 / [WebSocket §4.4](../../5-system/6-websocket-protocol.md#44-실행-진행-이벤트))
