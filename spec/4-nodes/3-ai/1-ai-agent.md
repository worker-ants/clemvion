...
| ~~conversationHistory~~ | ~~`none` / `last_n` / `full`~~ | | ~~`none`~~ | **DEPRECATED** — `contextScope` 로 대체 ([공통 §10](./0-common.md#10-conversation-context-자동-컨텍스트-주입)). schema 는 호환성을 위해 1 cycle 더 유지하되 handler 가 읽지 않는다. v2 에서 schema 제거 예정 |
| ~~historyCount~~ | ~~Integer~~ | | | **DEPRECATED** — `contextScopeN` 으로 대체 |
...
2. **사용자 메시지 수신 시:**
   ...
   c. 사용자 메시지를 대화 이력에 추가 + ConversationThread 에 `ai_user` turn 자동 push
...
### 12.2 `conversationHistory` deprecated 사유

`conversationHistory: 'none' | 'last_n' | 'full'` + `historyCount` 는 schema·spec 표에는 정의되어 있었으나 **handler 코드가 한 번도 읽지 않는 deadweight** 였다. 의도된 동작 ("이전 N개 대화 이력 보관") 은 multi-turn 자체 messages 배열로 항상 누적되었으므로 사실상 noop. `contextScope`/`contextScopeN` 으로 대체하면서 1 cycle 호환성 유지 후 schema 제거.
