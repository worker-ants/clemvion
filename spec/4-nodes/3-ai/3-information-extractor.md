...
| embeddingModel | Model select? |  |  | 회수·추출 임베딩 모델 (scope 저장 메모리와 차원 일치 필요). 노드 `llmConfigId` provider 의 등록 embedding 모델 select(`embedding-model-selector`). 비우면 워크스페이스 기본. `persistent` 일 때 노출 |
| extractionModel | Model select? |  |  | 추출 LLM 콜 전용 모델. 노드 `llmConfigId` provider 의 등록 chat 모델 select(`chat-model-selector`). 비우면 `model → llmConfig 기본` 폴백. `persistent` 일 때 노출 |
...
2.6. **persistent recall** (§7) — `memoryStrategy === 'persistent'` 이면 LLM 호출 직전 `AgentMemoryService.recall(workspaceId, scopeKey, queryText, {topK, threshold, embeddingModel})` 로 세션 간 사실을 회수해 system 메시지 뒤 안정 프리픽스(recall 블록)로 주입한다. `scopeKey = resolveScopeKey(memoryKey, executionId)`, `queryText = inputField`(빈 값이면 systemPrompt 폴백). `manual`(default) 이면 호출 없이 무변경. 회수 실패는 graceful (빈 회수).
