...
pending_plans:
  - plan/in-progress/node-output-redesign/information-extractor.md
---
...
LLM 을 사용해 비정형 텍스트에서 `outputSchema` 에 정의된 구조화 필드를 추출한다. **Single Turn**(1 회 LLM 호출 + JSON 응답) 과 **Multi Turn**(블로킹 대화로 부족한 필드 보강 — `finalize_extraction` tool 호출 종결) 모드를 제공한다. AI 카테고리 3 노드는 `output.result.*` / `output.error.*` / `output.interaction.*` wrapper 컨트랙트를 공유한다 ([공통 §5](./0-common.md#5-응답-형식-규약)).
...
| model | String? |  | (ModelConfig 기본값) | 모델 ID (프로바이더별) |
...
| memoryStrategy | `manual` / `persistent` |  | `manual` | 메모리 관리 전략 (§7). `manual` = 위 contextScope 필드를 그대로 적용. `persistent` = 추출 LLM 콜 전 세션 간 메모리 recall 주입 + 턴 경계 비동기 extraction. **`summary_buffer` 값은 없다** (추출 노드에 working-memory 압축 무의미 — §Rationale). `manual` 은 기존 동작 100% 유지 |
| memoryKey | Expression? |  |  | persistent 메모리 scope key. 같은 키는 실행을 넘어 같은 기억을 회수. 비우면 `executionId` 로 격리. `persistent` 일 때만 노출. ai_agent 와 동일 scope key 규칙([§17 agent-memory §2](../../5-system/17-agent-memory.md)) |
| memoryTopK | Integer? |  | `5` | persistent 회수 top-k (KB RAG Top-K 와 독립). `persistent` 일 때 노출 |
| memoryThreshold | Float? |  | `0.7` | persistent 회수 최소 유사도 (0–1, KB RAG Threshold 와 독립). `persistent` 일 때 노출 |
| memoryTtlDays | Integer? |  |  | persistent 메모리 만료(일). 비우면 무만료. `persistent` 일 때 노출 |
| embeddingModelConfigId | ModelConfig select? |  |  | 회수·추출 임베딩 **config.id** (scope 저장 메모리와 차원 일치 필요). `/models` 등록 embedding ModelConfig select(`embedding-config-selector`, 노드 `llmConfigId` 와 독립). 비우면 워크스페이스 기본 embedding config (`resolveEmbedding`). `persistent` 일 때 노출 |
| extractionModelConfigId | ModelConfig select? |  |  | 추출 LLM 콜 전용 **chat config.id**. `/models` 등록 chat ModelConfig select(`chat-config-selector`, 노드 main LLM 과 독립 — 고른 config 의 provider/defaultModel 로 호출). 비우면 노드 `llmConfigId → 기본 모델` 폴백. `persistent` 일 때 노출 |
| contextScope | `none` / `thread` / `lastN` |  | `none` | ConversationThread 자동 주입 범위. [공통 §10](./0-common.md#10-conversation-context-자동-컨텍스트-주입) (세 노드 공통). `memoryStrategy = manual` 일 때만 적용 (persistent 면 숨김) |
...
2.6. **persistent recall** (§7) — `memoryStrategy === 'persistent'` 이면 LLM 호출 직전 `AgentMemoryService.recall(workspaceId, scopeKey, queryText, {topK, threshold, embeddingModelConfigId})` 로 세션 간 사실을 회수해 system 메시지 뒤 안정 프리픽스(recall 블록)로 주입한다. `scopeKey = resolveScopeKey(memoryKey, executionId)`, `queryText = inputField`(빈 값이면 systemPrompt 폴백). `manual`(default) 이면 호출 없이 무변경. 회수 실패는 graceful (빈 회수).
...
5. 응답 content 를 JSON.parse. 성공 → 추출 결과를 `output.result.extracted` 에 담고 `out` 포트로 라우팅 (§5.1). thread push (`ai_assistant`, `JSON.stringify(extracted)`) 직후 **persistent extraction** (§7) — `memoryStrategy === 'persistent'` 이면 `AgentMemoryService.scheduleExtraction(...)` 로 turn 경계 비동기 추출을 enqueue 한다 (hot-path 비차단). `manual` 이면 no-op.
...
1. **첫 턴 진입**:
   ...
   - **persistent recall** (§7) — `memoryStrategy === 'persistent'` 이면 위 주입 직후, 첫 LLM 호출 직전에 1회 recall 해 system 메시지에 회수 블록을 append 한다. system 메시지가 `_resumeState.messages` 로 운반되므로 후속 turn 은 재회수하지 않는다 (single-turn 과 동일 회수 패턴). persistent 운반을 위해 thread ref·executionId·평가된 메모리 config·증분 watermark(`memoryState.lastExtractionTurnSeq` — I12 sub-namespace; 읽기는 구 평면 키 `lastExtractionTurnSeq` 폴백으로 in-flight 하위호환)·`memoryStrategy` 를 multi-turn state 에 실어 turn 간 유실을 막는다.
...
3. **종결 판정** (각 턴 끝):
   ...
   - **종결 thread push + persistent extraction** (§7, [conversation-thread §2.3](../../conventions/conversation-thread.md#23-적용-범위-push-vs-inject-구분)) — `buildMultiTurnFinalOutput` 종결 경로(`completed` / `max_turns` / `user_ended`)에서 최종 추출 스냅샷을 `ai_assistant` turn (`JSON.stringify(extracted)`) 으로 **1회** push 한다 (single-turn 과 동형). `waiting_for_input` 상태에선 push 하지 않는다. `error` endReason(LLM 호출 실패)은 불완전 결과를 누적하지 않으려 push 하지 않는다. push 직후 `memoryStrategy === 'persistent'` 이면 그 thread 를 source 로 extraction 을 fire-and-forget enqueue 한다 (응답 비차단). 이 push 가 추출 source + 다운스트림 contextScope 가시성을 준다 (구 v2 limitation 해소).
...
>
> AI 3 노드 공통 wrapper: `output.result.*` / `output.error.{code, message, details?}` / `output.interaction.{type, data, receivedAt}` ([공통 §5](./0-common.md#5-응답-형식-규약)). 옛 `output.output.extracted.*` 이중 중첩 포맷은 폐기 (Principle 8).
>
> **Config echo 정책 (Principle 7)**: 모든 종결·waiting·resumed 시점에서 `config` 는 사용자가 입력한 raw 값 (template `{{ ... }}` 보존) 을 echo 한다. multi-turn resumed 턴에서는 engine 이 `state.rawConfig` 를 통해 동일 raw 값을 운반한다 — frozen 범위는 한 turn 처리로 한정되며, 매 turn 재개(rehydration) 시 현재 `node.config` 에서 fresh 재유도·재freeze 된다 (D3 fresh-per-turn: park 중 워크플로 편집은 다음 turn 부터 반영 — [실행 엔진 §6.1 rawConfig snapshot 정책](../../5-system/4-execution-engine.md#61-컨텍스트-구조)). 후속 노드는 `$node["X"].config.{mode, model, schema, instructions, examples, inputField, maxTurns, maxCollectionRetries}` 로 raw 를 읽고, 평가된 모델 식별자는 `meta.model` 에서 읽는다.
>
...
`error` 포트는 single-turn 의 LLM 호출 실패 / JSON 파싱 재시도 소진, multi-turn 의 LLM 호출 실패 (`max_retries` 는 §5.6) 가 모두 라우팅된다. multi-turn 의 mid-conversation 실패에서는 `output.error` 와 함께 `output.result` 가 부분 수집 결과로 병존한다 ([공통 §5](./0-common.md#5-응답-형식-규약)).
...
> ⚠ **구현 현황 (structured `resumed` 스냅샷은 현재 미emit — Planned)**: 본 `status: 'resumed'` structured 스냅샷은 **목표 관측 계약**이다. 현재 엔진의 AI 대화 turn 경로(`ai-turn-orchestrator.service.ts` `processAiResumeTurn` → `handleAiMessageTurn`)는 IE turn 에서 `message_received` interaction / structured `status:'resumed'` 스냅샷을 emit 하지 않는다 (`resumed` 스냅샷 emit 은 form/buttons 경로에만 존재) — [AI Agent §7.5](./1-ai-agent.md#7-출력-구조) 와 동일 미구현. 다만 같은 수신 시점의 **라이브 조기 노출**(WS `execution.user_message`)은 node-type 비의존 공통 경로라 IE multi-turn 메시지 수신에도 적용된다. structured 스냅샷 구현(또는 본 절 폐기 결정)은 후속(`plan/in-progress/node-output-redesign/information-extractor.md` 추적).
...
`collectionRetryCount > maxCollectionRetries` 초과. `error` 포트로 라우팅되며 **`output.error` 와 `output.result` 가 병존** — 부분 수집 결과를 후속 노드가 활용할 수 있도록 둘 다 보존한다 ([공통 §5](./0-common.md#5-응답-형식-규약)).
...
| `meta.contextInjection` | object? | handler return | `contextScope ≠ 'none'` + thread non-empty 시에만 echo (§5.1 정의와 동일). multi-turn 은 첫 진입 시 1회 주입한 결과를 state 로 운반해 종결 출력에 echo. 세 노드 공통 디버그 echo. 상세: [공통 §10](./0-common.md#10-conversation-context-자동-컨텍스트-주입) · [Spec Conversation Thread §5.3](../../conventions/conversation-thread.md#53-cap-v1) |
...
## 7. Persistent 메모리 (recall + extraction)
...
- 추출 LLM 콜 직전 1회: `AgentMemoryService.recall(workspaceId, scopeKey, queryText, { topK: memoryTopK, threshold: memoryThreshold, embeddingModelConfigId })`.
...
- turn 경계(thread push 직후)에 `AgentMemoryService.scheduleExtraction({ workspaceId, scopeKey, llmConfigId, model, extractionModelConfigId, embeddingModelConfigId, turns, ttlDays })` 로 **비동기** enqueue (큐 add 까지만 await — 추출 LLM 콜은 worker). hot-path 비차단.
- single-turn 은 최종 응답 push 후 1회. multi-turn 은 종결 push 후 1회 (각 turn 마다가 아님 — push 가 종결 1회뿐이므로).
- `turns` 는 thread 스냅샷의 shallow-copy (격리 invariant). 증분 watermark(`memoryState.lastExtractionTurnSeq` — I12; 구 평면 키 폴백) 는 multi-turn state 로 운반되나, multi-turn 추출이 종결 1회라 실무상 전체 thread snapshot 한다.
...
설계 결정의 SoT 는 다음 참조 (응답·토큰·Thread·Prefix 는 공통 규약을 그대로 따른다):
...
### 9.1 IE 에 persistent O / summary_buffer X

추출 노드의 가치는 **세션 간 사실 누적**(이전 추출 결과를 다음 실행이 활용)에 있다. 이는 회수(recall)로 과거 사실을 끌어오고 추출(extraction)로 새 사실을 저장하는 persistent 사이클과 정확히 맞는다. 반면 `summary_buffer` 는 **단일 실행 내** 긴 대화의 working-memory 토큰을 압축하는 장치인데, 추출은 outputSchema 로 범위가 한정된 짧은 수집 대화라 토큰 압축의 이득이 거의 없다. 추출 노드에 summary_buffer enum 값을 두지 않아 설정 표면을 단순하게 유지한다.

### 9.2 text_classifier 제외

text_classifier 는 single-turn·상태 없는 분류기로, 회수할 "이전 세션 사실"도, 저장할 "추출 사실"도 정의되지 않는다. 따라서 persistent 메모리 대상에서 제외하고 contextScope(manual 주입)만 유지한다 ([§0-common §10](./0-common.md#10-conversation-context-자동-컨텍스트-주입)).

### 9.3 multi-turn thread push 를 이번에 해소

종전 multi-turn 종결 경로는 thread push 를 하지 않는 v2 limitation 이 있었다 (single-turn 만 push). persistent 추출은 **push 된 thread turn 을 source** 로 삼으므로, multi-turn 종결 push 가 선행돼야 추출이 의미를 가진다. 동시에 이 push 는 다운스트림 노드의 `contextScope` 가시성도 준다 (추출 결과를 thread 에서 읽을 수 있음). 따라서 본 작업에서 종결 1회 push 를 함께 해소한다. `waiting` 중에는 push 하지 않아 (종결 시 1회) 미완성 결과가 thread/메모리에 누적되지 않는다.
...
### 9.4 메모리 모델 필드 = 등록 ModelConfig 선택 (config.id)

IE 의 메모리 모델 필드 `embeddingModelConfigId` / `extractionModelConfigId` 는 AI Agent 와 동일하게 **`/models` 에 등록된 ModelConfig 를 `config.id` 로 고르는 방식**을 채택한다 (위젯 `embedding-config-selector` / `chat-config-selector`, 노드 `llmConfigId` 와 독립 — 고른 config 의 provider/defaultModel 로 resolve). 채택 근거·재번복 이력의 SoT 는 [AI Agent §12.12 재번복 결정](./1-ai-agent.md#1212-요약추출-전용-llm-모델-옵션-summarymodelconfigid--extractionmodelconfigid) 이며, IE 는 그 결정을 그대로 따른다. 단 **IE 는 `summaryModelConfigId` 를 갖지 않는다** — IE 는 working-memory 롤링 요약(summary_buffer) 경로가 없어 요약 LLM 콜 자체가 없기 때문이다 (§9.1). 종전 모델명 문자열 필드(`embeddingModel`/`extractionModel`)로 저장된 기존 노드 값은 신 필드(`*ModelConfigId`)와 키가 달라 폐기되고 미설정 폴백(워크스페이스 기본 embedding / 노드 `llmConfigId`)으로 graceful degrade 한다 — 보조 콜이라 추출 무결성 영향이 없다.
