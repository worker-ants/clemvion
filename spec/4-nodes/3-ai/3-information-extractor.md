...
pending_plans:
  - plan/in-progress/spec-sync-information-extractor-gaps.md
---
...
| `output.error.details.retryable` | `boolean` | handler — runtime | **미구현 (Planned)** — [CONVENTIONS Principle 3.2.1](../../conventions/node-output.md#321-details-의-공통-표준-필드-llm-계열-노드-한정-필수) 은 LLM 계열 노드 한정 필수로 규정하나 현재 handler 의 모든 error 경로 (`LLM_CALL_FAILED` / `LLM_RESPONSE_INVALID` / `MAX_COLLECTION_RETRIES_EXCEEDED`) 가 `details` 에 `retryable` 을 채우지 않는다 ([handler](../../../codebase/backend/src/nodes/ai/information-extractor/information-extractor.handler.ts) `buildErrorOutput`). 향후 충전 시 invariant: `LLM_CALL_FAILED` / `LLM_RATE_LIMIT` → `true`, `LLM_RESPONSE_INVALID` / `MAX_COLLECTION_RETRIES_EXCEEDED` → `false` (§6 에러 코드 표) |
| `output.error.details.retryAfterSec?` | `number?` | handler — runtime | **미구현 (Planned)** — `retryable` 미충전과 함께 현재 handler 가 set 하지 않음. 향후 provider 가 `Retry-After` 등 신호 제공 시 권장 대기 시간 (초). invariant: `retryable === true` 일 때만 set (Principle 3.2.1) |
