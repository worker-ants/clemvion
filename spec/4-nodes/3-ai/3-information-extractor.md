...
| embeddingModel | String? |  |  | 회수·추출 임베딩 모델 (scope 저장 메모리와 차원 일치 필요). 비우면 워크스페이스 기본. `persistent` 일 때 노출 |
| extractionModel | Expression? |  |  | 추출 LLM 콜 전용 모델. 비우면 `model → llmConfig 기본` 폴백. `persistent` 일 때 노출 |
