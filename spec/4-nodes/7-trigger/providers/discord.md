...
|---|---|
| `setupChannel` | (a) [`GET /applications/@me`](https://discord.com/developers/docs/resources/application) — application identity 캐시 (id, name, `verify_key`→`botIdentity.publicKey`; 응답 `verify_key` 와 사용자 입력 public key(`inboundSigningRef`) cross-verify, 불일치 시 `BOT_TOKEN_INVALID` — §3.1). (b) [`PUT /applications/{app_id}/commands`](https://discord.com/developers/docs/interactions/application-commands#bulk-overwrite-global-application-commands) — slash command bulk overwrite (`/<prefix>` + sub-options) |
...
| `sendMessage` (image) | **현재 v1**: `POST /channels/{id}/messages` 에 `content` = caption (없으면 `fallbackText`) 만 전송 — 실제 이미지 미첨부. **미구현 (Planned)**: `attachments: [{filename, ...}]` + multipart/form-data file upload, 또는 `embeds: [{image: {url: ...}}]` |
...
> **현재 구현**: `setupChannel` 은 `GET /applications/@me` 응답의 `id` / `name` 을 `botIdentity` 에 저장하고(`botId` 는 snowflake 문자열을 `hashStringToInt` 로 변환한 정수), 응답 `verify_key` (≡ application public key)를 `botIdentity.publicKey` 로 캐시한다(비민감 public key). 또한 `verify_key` 와 사용자 입력 public key(`inboundSigningRef` 로 resolve)가 **둘 다 있고 불일치하면** `BOT_TOKEN_INVALID` 로 throw 해 잘못된 앱/키 등록을 setup 시점에 차단한다. inbound 서명 검증의 SoT 는 여전히 `inboundSigningRef` (SecretStore) + §6 의 트리거 생성 시점 정규식 검증 (`assertInboundSigningPlaintextByProvider`) 이며, setup 시점 cross-check 와 publicKey 캐시는 그 위의 추가 안전장치·identity 편의다.
...
Body: [
  {
    ...
    "options": [
      ...
      {
        "name": "reply", "type": 1, "description": "Reply to the active AI conversation",   // R-CC-13 / R-D-3 (v1 Gateway 미사용 결과)
        "options": [
...
HTTP 200 (Interactions Webhook 응답 body)
{
  ...
  "data": {
    ...
    "title": "<formConfig.title>",          // formConfig.title (extractFormTitle) → ≤45자 truncate, 없으면 languageHints.formModalTitle, 최종 '양식' fallback
    "components": [
      ...
      { "type": 1, "components": [
        { "type": 4, "custom_id": "<field.name>", "label": "<field.label>",
          ...
          "placeholder": field.description (있으면, ≤100자),
          // field.validation.{minLength,maxLength} → min_length / max_length 부여 (Discord 0–4000 cap). 부재 시 미부여
...
| `type === 3` (MESSAGE_COMPONENT) & `data.custom_id === "__reply__"` (BUTTON) | `{ kind: "open_form_modal", openContext: { interactionId, interactionToken, modal: "reply" } }` — AI reply modal (§5.1 b). `HooksService` 가 `openFormModal(modalKind='reply')` 로 `clemvion_reply` MODAL 을 연다 |
...
| `type === 5` (MODAL_SUBMIT) & `data.custom_id === "clemvion_form"` | **`{ kind: "form_submission", fields }`** — `data.components[].components[]` 의 `{ custom_id: field.name, value }` 평탄화. native form modal 채택 ([R-D-6](#r-d-6-form--text-계열--5-fields-native-modal-그-외-다단계)) |
| `type === 5` (MODAL_SUBMIT) & `data.custom_id !== "clemvion_form"` (예: `clemvion_reply`) | `{ kind: "text_message", text: <TEXT_INPUT 값> }` — `clemvion_form` 이외 MODAL_SUBMIT 의 TEXT_INPUT 을 일괄 normalize. AI Multi Turn reply modal (§5.1 b) 의 결과 경로. 진입점(Reply 버튼 `__reply__` → `clemvion_reply` modal)이 구현돼 있어 도달한다 |
...
자연 대화 (일반 DM 텍스트로 reply) 의 완전 지원은 v2 Gateway 진입 시 해소.
...
[Convention §3 매핑 표](../../../conventions/chat-channel-adapter.md#3-eia--internal-event--rendernode-매핑) 의 `uiMapping.visualNode` enum 분기 적용. **v1 = markdown 텍스트/monospace + Embed** (의존성 추가 없음). v2 SSR PNG 격상은 후속. v1 단계에서 `photo` 선택 시 fallback to text + warning 로그.
...
| `carousel` | `output.items[]` → 각 카드 **bold title + description** 을 하나로 join (`renderVisualFallback`). imageUrl 무시. 카드 cap 10장 | fallback to `auto` v1 | 5장 collage PNG attachment (1 송신) | **미구현 (Planned)** — 계획: 카드별 imageUrl 있으면 `embeds: [{image: {url: imageUrl}}]` (title + description embed fields), 없으면 plain content. 현재는 `text` 와 동일 fallback (embeds 호출처 없음) | 카드별 분기 + 5장 collage PNG |
...
| `message_reference` | 미부여 (DM only — [R-D-4](#r-d-4-dm-only-guild-채널-거부)) |
...
- **Application Public Key (ed25519)**: Discord application 의 [Interactions endpoint 검증](https://discord.com/developers/docs/interactions/receiving-and-responding#security-and-authorization) 은 `X-Signature-Ed25519` + `X-Signature-Timestamp` 헤더 + raw body 를 application public key 로 verify. public key 는 Discord Developer Portal 에서 사용자가 복사해 입력 — `SecretResolver` 가 관리, ref = `secret://triggers/{id}/inbound-signing` ([Convention Secret Store §1](../../../conventions/secret-store.md#1-uri-scheme) 의 provider 공통 슬롯).
  - **형식 (Discord 발급 표준)**: `^[a-f0-9]{64}$` (lowercase hex 64 chars, ed25519 public key 32 bytes). Backend 의 `assertInboundSigningPlaintextByProvider` (`chat-channel-input-rules.ts` — `TriggersService` 가 생성 경로에서 호출) 가 trigger 생성 시점에 정규식 검증 — 위반 시 400 `VALIDATION_ERROR` (`details.field='inboundSigningPlaintext'`, `details.code='INVALID_FIELD'`). uppercase hex 는 외부 Discord ed25519 verify 실패 회피를 위해 사전 차단.
...
두 예외는 본 spec §3.1 / §5.2 / §5.3 에 인라인 명시.
...
- **구현됨 (2026-08-13)**: `interaction.id` 기반 dedup — 같은 id 가 30초 안에 두 번 도착하면 두 번째는 무시 (Discord retry 정책: 3회까지). parser 가 채운 `idempotencyKey = interaction.id`(snowflake, 재전송 시 동일)를 `ChatChannelDedupService` 가 소비한다 (Redis `SET NX EX 30`, 키 `cc:dedup:<triggerId>:<interactionId>`, inbound 진입에서 rate-limit 앞). Redis 미가용 시 fail-open. 요구사항: [CCH-SE-02](../../../5-system/15-chat-channel.md) / 키·TTL·게이트 순서 상세 SoT: [data-flow/14 §2.2](../../../data-flow/14-chat-channel.md#22-redis).

---

## Rationale

### R-D-1. `inboundSigningRef` 단일 슬롯 공유 + public key 도 SecretResolver 로 관리

본 항은 두 결정이 결합된다:

**결정 1**: public key (비밀 아닌 공개 정보) 도 `SecretResolver` 로 관리한다. rotation (Discord Developer Portal 에서 reset 가능) / workspace 격리 / audit 일관성 + ref 형식·라이프사이클이 `botTokenRef` 와 정합. `Trigger.config.chatChannel.publicKey` 평문 저장은 비밀이 아니라 평문 OK 지만 config JSONB 가 SecretResolver 를 우회해 모듈 일관성이 깨지므로 채택하지 않는다.

**결정 2**: ref 슬롯은 `inboundSigningRef` 단일 (Telegram / Slack 공유) — Slack `R-S-1` 와 동일 role-based generic naming 통합. 자원 성격 (Telegram shared secret / Slack HMAC key / Discord ed25519 public key) 의 차이는 backend 의 provider 분기로 흡수. provider 별 `publicKeyRef` 필드를 두면 Convention §2.3 의 필드 수가 늘고 새 provider 추가 시 필드 신설 의무가 생기며 자원의 "공통 role" 이 명시되지 않는다.

asymmetric public key 라는 자원 차이는 `SecretResolver` API 측면에서는 평범한 string blob 으로 다루고 (`store` / `resolve`), 검증 단계의 `crypto.verify('ed25519', ...)` 알고리즘만 backend 의 `DiscordAdapter` 가 책임진다.

### R-D-2. slash command `setupChannel` 시 bulk overwrite

`PUT /applications/{app_id}/commands` bulk overwrite 로 멱등성을 보장한다 (Convention §1.1 setupChannel 의무) — 기존 command 와 신규 command 의 diff 계산이 불필요하다. 개별 `POST` + 기존 조회 후 diff 는 race condition 위험·코드 복잡도가 크고, 사용자 수동 등록은 UX 부담 + bot 별 명령 일관성 보장이 불가하다. Discord bulk overwrite API 자체가 멱등성을 위해 설계되어 setupChannel 의 자연스러운 매핑이다.

### R-D-3. v1 = Interactions Webhook only, Gateway 는 v2

v1 은 Interactions Webhook only (stateless HTTP) 를 채택한다 — slash command / button / modal 위주 UX 로 Discord 의 modern bot 패턴 (Discord 자체가 권장). Gateway WebSocket 은 long-lived connection 1개/bot + heartbeat 관리 + shard 라우팅 (대규모 bot) 부담 + 다중 인스턴스 환경의 connection 라우팅 추가 인프라가 필요하므로 v2 로 미룬다.
...
Gateway 도입은 v2 후보 (사용자 요청 시 진입).

### R-D-4. DM only, guild 채널 거부
...
### R-D-5. `MESSAGE_COMPONENT` `custom_id` 에 buttonId UUID 직접

custom_id 에 buttonId (UUID v4, 36 chars) 를 직접 저장한다 — Discord custom_id 100 chars 한계 안에서 여유가 있어 별 매핑 테이블이 불필요하다. 단축 ID + Redis 매핑은 over-engineering 이고, executionId 등 추가 메타는 ChannelConversation 에서 충분히 보강 가능하므로 custom_id 는 buttonId 만 담는다. Telegram R2 와 동일 결론.

### R-D-6. Form — text 계열 & ≤5 fields native modal, 그 외 다단계

Discord MODAL (TEXT_INPUT components) 을 native form 으로 채택한다. [Convention §4.1 / R-CCA-8](../../../conventions/chat-channel-adapter.md#r-cca-8-native-form-modal-예외-절--5-fields-이하-single-modal) 예외 절에 따라, `formMode ∈ {auto, native_modal}` && `fields ≤ 5` && **전 필드 text 계열** form 은 단일 MODAL (§3.3 / §5.3). 그 외 (fields > 5 / `multi_step` / select·radio·checkbox·file 포함) 는 §4.2 다단계.

native MODAL 은 TEXT_INPUT only 제약 때문에 text 계열 form 한정이며, Discord modal 은 interaction response (type 9) 라 버튼 게이팅 (`__open_form__` 클릭 → MODAL 응답) 으로 token 을 확보한다 (Convention §4.1 step 2). select/checkbox 등은 SELECT_MENU 를 modal 에 못 넣는 Discord 플랫폼 제약 때문에 modal 에 담지 못하므로, 해당 타입을 포함하거나 6+ fields 인 form 은 component 기반 §4.2 다단계 시퀀스로 처리한다.

### R-D-7. v1 file 필드 사실상 미지원 (R-D-3 의 결과)
...
Form spec 의 `file` 타입 자체는 미변경 — Discord provider 만 v1 한계로 fallback. 사용자가 Discord provider + form file 필드를 만들면 노드 정의 검증 단계에서 warning 을 표시한다.

### R-D-8. PING / Interactivity 의 `200 OK` 예외

Slack R-S-8 와 동일 정신. Discord 의 두 케이스만 예외 (PING + Interactivity ack) — Spec Chat Channel §5.5 의 `202 Accepted` 정책에 명시적 예외.

### R-D-9. file 필드 v1 한계 명시 — Form spec 미변경

Form spec 의 `file` 타입 추상화는 그대로 두고 Discord provider 만 v1 implementation 한계로 unsupported 처리한다. Form spec 에 `unsupportedProviders: ["discord"]` 같은 필드를 추가하면 spec 이 provider-specific 한계를 알아야 하는 의존성 역전이 생기고, file 필드를 자동 외부 storage URL 입력으로 변환하면 사용자 의도와 다른 동작이 되어 UX 혼동을 부른다. spec abstraction 을 우선하고 provider 한계는 provider spec 의 Rationale 에 명시하며, impl 단계에서 warning UI 로 가이드한다.
