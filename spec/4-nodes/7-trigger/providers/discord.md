...
- **Application Public Key (ed25519)**: Discord application 의 [Interactions endpoint 검증](https://discord.com/developers/docs/interactions/receiving-and-responding#security-and-authorization) 은 `X-Signature-Ed25519` + `X-Signature-Timestamp` 헤더 + raw body 를 application public key 로 verify. public key 는 Discord Developer Portal 에서 사용자가 복사해 입력 — `SecretResolver` 가 관리, ref = `secret://triggers/{id}/discord-public-key` ([Convention Secret Store §1](../../../conventions/secret-store.md#1-uri-scheme) 예시 표 갱신 대상).
  - public key 는 비밀이 아니라 **공개키** 지만, `SecretResolver` 로 일관성 있게 관리 (rotation / workspace 격리 / audit 동일 흐름). Rationale R-D-1.
  - HooksController 가 raw body + 두 헤더 → `SecretResolver.resolve(discordPublicKeyRef)` → ed25519 verify (Node.js `crypto.verify('ed25519', ...)`). 실패 시 `401 Unauthorized` ([Spec Chat Channel §5.5](../../../5-system/15-chat-channel.md#55-inbound-http-contract) 적용). Discord 는 401 응답 시 retry 하지 않음.
- **Config 필드**: `Trigger.config.chatChannel` 에 `publicKeyRef: "secret://triggers/{id}/discord-public-key"` 신설 — Discord 전용 필드. Slack 의 `signingSecretRef` 와 의미 유사하나 자원이 다름 (HMAC secret vs ed25519 public key) — Rationale R-D-1.
...
### R-D-1. `publicKeyRef` 별 필드 + `SecretResolver` 로 public key 관리 (2026-05-24)

대안:
1. **(채택) `publicKeyRef` 별 필드 + SecretResolver 사용**: public key 는 비밀이 아니라 공개키지만, rotation (Discord 에서 reset 가능) / workspace 격리 / audit 일관성 위해 동일 store 에서 관리. ref 형식·라이프사이클이 botTokenRef / slack signingSecretRef 와 정합.
2. **(기각) `Trigger.config.chatChannel.publicKey` 평문 저장**: 비밀이 아니라서 평문 OK 지만, config JSONB 가 SecretResolver 우회 → 모듈 일관성 깨짐. signingSecretRef / botTokenRef 와 다른 패턴 도입 부담.
3. **(기각) `secretTokenRef` 재사용**: Telegram secretToken (server-issued) 와 의미 충돌 — Slack signingSecret 와 같이 별 필드가 정합.

근거: Slack §6 (R-S-1) 와 동일 정신 — provider 별 인증 자원은 별 ref 필드. SecretResolver 의 일관성 우선 > public key 의 평문 가능성.
