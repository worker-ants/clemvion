...
- **미구현 (Planned)**: LLM 응답 처리 직전 `sendChatAction(action=typing)` 1회 (5초 자동 만료) — UX 개선. 어댑터 `sendMessage` 는 `kind:'typing'` 케이스를 지원하나, 현재 renderer (`telegram-message.renderer.ts`) 가 `execution.ai_message` 에서 `typing` ChannelMessage 를 생성하지 않아 실제로 발송되지 않는다.
...
- callback_query 도착 시 처리 순서:
  ...
  3. **미구현 (Planned, 옵션)**: 직전 메시지를 `editMessageReplyMarkup` 으로 키보드 제거 — 중복 클릭 차단. 현재 어댑터는 `answerCallbackQuery` 만 호출하고 `editMessageReplyMarkup` 메서드/호출이 없다 (`telegram.adapter.ts` `ackInteraction`).
...
| `/help` | **미구현 (Planned)** — 봇 기본 도움말 (v1 정적 텍스트 — `languageHints.help` 또는 default 문구) 목표. `HooksService` 에 `/help` 정적 안내 분기가 있으나 **도달 불가(dead)**: `parseTelegramUpdate` 의 `readCommand` 가 `/start`·`/cancel` 외 `/`-prefix 텍스트를 `null` 로 반환하므로 `/help` 는 `text_message` 가 되지 못해 `parseUpdate` 가 `null` → 호출자가 "지원하지 않는 메시지 형식입니다" 안내를 발송한다. 정적 도움말을 살리려면 parser 가 `/help` 를 별도 command kind 로 통과시켜야 한다 |
| 그 외 명령 (`/...`) | 무시 — v1 은 `/start`·`/cancel` 만 처리 (위 `/help` 도 현재 미도달) |
