...
| (특수) `phone` — Form spec 의 `type` Enum 에 `phone` 미존재. v1 은 `type: 'text'` + phone pattern (`/^\+?[\d\s\-()]+$/`) custom ValidationRule 로 처리하기로 stub. 단 Form spec [§1](../../6-presentation/4-form.md#1-설정-config) 에 `ValidationRule.phone` 또는 명시적 pattern 예시가 spec 화되지 않아 어댑터 측 임시 가정 — 별 plan `spec-fix-form-phone-validation` 으로 추적 (Form spec 에 phone validation 예시 추가 또는 `type: 'phone'` 신설 결정 필요) | `request_contact: true` 버튼 — share_contact. Form spec 의 `type` Enum 자체는 미변경 (W-4) |
