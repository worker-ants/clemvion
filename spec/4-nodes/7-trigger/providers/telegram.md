...
| 그 외 (`sticker`, `voice` 등 unsupported) | `null` + `"지원하지 않는 메시지 형식입니다."` 안내 |
...
**v1 분리**: chart 는 [§11 PR-D](../../../5-system/15-chat-channel.md) 의 first 단계, carousel/table 은 SSR 인프라 정비 후 별 PR 권장. CCH-MP-04 의 "필수(단계적)" 등급은 chart 가 v1 필수, carousel/table 은 후속 plan.
