...
| 본문 | `text` = `surfaceMismatch` 문구, **MarkdownV2 escape 미적용** (아래 주의) |

> **주의 — non-escape 예외**: §5.6/§5.7 등 다른 안내는 `renderNode` 경로(어댑터가 escape, [R4](#r4-markdownv2-escape-책임을-어댑터로))를 타지만, `surfaceMismatch` 는 `HooksService` 가 `renderNode` 를 우회해 `adapter.sendMessage` 로 **직접** 발송하는 control-plane 메시지라 escape 가 적용되지 않는다(`executionStillRunning`/`groupChatRefusal` 등 다른 hooks 직접 발송 안내와 동일 경로). 따라서 default 문구는 telegram MarkdownV2 특수문자(`_*[]()~``>#+-=|{}.!`)를 포함하면 raw 전송이 거부되므로, 세 provider 모두 안전하도록 **특수문자를 배제**했다(단위 테스트가 canonical `escapeMarkdownV2` 로 불변식 강제). 운영자 override 도 동일 제약 — [§4.1.1](../../../5-system/15-chat-channel.md#411-languagehints-default-문구--ko--en) 참조.
