...
>
> **`regex` 연산자 주의**: **If/Else 와 Switch (expression mode) 핸들러 모두 `compileRegexCache` 로 조건별 정규식을 컴파일해 평가기에 전달하므로 `regex` 가 정상 동작한다** (`if-else.handler.ts` / `switch.handler.ts` 의 `execute()` → `evaluateCondition(input, cond, { strict, regex })`, Filter / Transform.array_filter 와 동일 경로). 잘못된 패턴(컴파일 실패)·과길이(>200자)·**`safe-regex` 위험 패턴(지수 백트래킹, refactor 04 M-3)** 은 컴파일이 skip 되어 해당 조건이 `false` 로 평가된다 (`compileRegexCache` → 단일 헬퍼 `compileUserRegex`: 길이 + safe-regex + 문법. 길이 200 단독은 ReDoS 방지 불충분이라 safe-regex 가 1차 방어).
