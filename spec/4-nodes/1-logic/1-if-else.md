...
>
> **`regex` 연산자 주의**: **If/Else 핸들러는 `compileRegexCache` 로 조건별 정규식을 컴파일해 평가기에 전달하므로 `regex` 가 정상 동작한다** (`if-else.handler.ts` 의 `execute()` → `evaluateCondition(input, cond, { strict, regex })`, Filter / Transform.array_filter 와 동일 경로). 단 **Switch (expression mode) 핸들러는 아직 컴파일된 정규식을 전달하지 않아 `regex` 가 항상 `false` 를 반환하는 no-op 이다** (`evaluateResolvedCondition` 의 `case 'regex': if (!compiledRegex) return false`). 잘못된 패턴(컴파일 실패)이나 빈 패턴은 If/Else 에서도 `false` 로 평가된다. Switch 에서 정규식 매칭이 필요하면 표현식(`{{ }}`)으로 우회한다.
