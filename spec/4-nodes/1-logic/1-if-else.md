...
> ⚠ **미구현 (P1)**: schema enum 의 `is_type` / `regex` 연산자가 `evaluateCondition` 에서 silent `false` fall-through 된다. user_memo 개선안은 schema enum 에서 두 연산자 제거 또는 validate에서 reject 제안. 본 문서 §1 표는 [공통 §2 지원 연산자](./0-common.md#2-지원-연산자) 인용에 의존한다.
