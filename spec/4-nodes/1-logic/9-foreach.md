...
> ForEach 의 완료 envelope 는 `config` / `output` / `meta` 3키만 stamp 한다 (`execution-engine.service.ts` runContainer foreach 분기 — `port` 키를 envelope 에 쓰지 않는다). `done` 분기는 envelope 의 `port` 필드가 아니라 엔진 edge 활성화로 라우팅된다. (envelope 에 `port` 를 직접 stamp 하는 것은 현재 Parallel 노드뿐이다.) 또한 `meta.durationMs` 는 foreach structured meta 에 포함되지 않는다 — 실행 시간은 DB NodeExecution row / WS 이벤트 레벨에서만 관측되고 노드 envelope 의 `meta` 에는 노출되지 않는다.
...
> 인덱스 보존 정책 / 중첩 컨테이너 스코프: [공통 §4](./0-common.md#4-에러-정책-errorpolicy) · [실행 엔진 §3.2](../../5-system/4-execution-engine.md#32-foreach--map-실행) 참조.

> **D3 결정 — Map 과의 의도된 분기**: Map 은 실패 항목을 `output.mapped[i] = { _skipped: true, error }` 인라인 마커로 유지한다 ([7-map.md §5.2](./7-map.md#52-case-완료-시점--변환-배열-수집-port-done)). 두 노드의 차이는 **시멘틱 차이**에서 의도된 것:
...
- 핸들러가 시작 시점에 반환한 `output: items[]` 는 외부 expression 으로 노출되지 않는다 (엔진이 덮어쓰기 전 중간 단계). **D2 결정**으로 이 동작은 그대로 유지 — `null` 반환 + internal 채널(B안)은 invariant 변경 비용이 효익을 초과해 기각.
...
| 발생 조건 | 메시지 / 코드 | 시점 |
|-----------|--------------|------|
| `arrayField` 가 빈 문자열 또는 누락 | `Array field must be entered.` (SSOT: `foreach.schema.ts` warningRule `foreach:no-array-field`. 캔버스 배지는 i18n 으로 번역 렌더) | warningRule (캔버스 배지) + handler.validate |
...
### R-1. `$itemIsFirst` / `$itemIsLast` 를 top-level 변수로 노출 (2026-06-03)

초기 spec 은 `itemContext.isFirst`/`isLast` 를 "컨테이너 실행 제어용 내부 상태이며 expression 으로 노출하지 않는다" 고 기술했다. body 표현식에서 first/last 분기 수요(`$itemIndex === 0` workaround 의 가독성 한계)가 확인되어 노출로 전환한다.

- **노출 형태 = top-level 변수** (`$itemIsFirst` / `$itemIsLast`): `$item` 은 raw 입력 원소(string/number 등 primitive 가능)라 `$item.isFirst` 형태로 속성을 부착하면 `$item` 자체 의미를 깨뜨린다. 따라서 `$loop.isFirst` 와 유사하되 별개의 top-level 변수로 둔다.
- 엔진은 이미 `itemContext.isFirst`/`isLast` 를 계산하므로 `expression-resolver.service.ts` 가 이를 그대로 전달하는 최소 변경으로 구현했다 (신규 계산 없음).
- 기각된 대안: (a) `$item` 래핑 — primitive item 파괴로 기각. (b) `$loop` 재사용 — ForEach 는 `$loop` 컨텍스트가 아니므로 의미 혼동.
