...
```
GET /api/v1/executions/:executionId/background-runs/:backgroundRunId
```
...
`GET /api/v1/executions/:executionId/background-runs/:backgroundRunId` 로 중첩한 이유:

- `backgroundRunId` 는 UUID v4 로 전역 유일하므로 `GET /api/v1/background-runs/:backgroundRunId` 같은 flat URL 도 기술적으로 가능
