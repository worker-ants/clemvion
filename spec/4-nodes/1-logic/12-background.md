...
3. `BackgroundExecutionProcessor` 워커가 job 을 pop 하면 `executeBackgroundSubgraph(job)` 이 다음을 수행한다:
   ...
   - 본문 안의 blocking 노드(form / buttons / ai_conversation)에서 park 가 발생하면 `executeInline` 이 `ParkReleaseSignal` 을 throw 하고, `executeBackgroundSubgraph` 가 이를 catch 해 본문을 그 지점에서 **즉시 graceful 종료** 한다 — fire-and-forget 이라 외부 `continueExecution` 재개 경로가 없어 durable park 가 무의미하다. 실패로 집계되지 않으며 `notifyOnFailure` 알림도 발송되지 않는다 (§6 표 참조)
...
| 본문에 blocking 노드(form / buttons / ai_conversation) 포함 → park 발생 | `ParkReleaseSignal` 로 본문 **즉시 graceful 종료** (§4-3). 실패가 아니므로 `notifyOnFailure` 알림 미발송, 외부 `continueExecution` 재개 불가 — 이 시나리오에서는 위 `maxDurationMs` 타임아웃 행이 발동하지 않는다 |
...
| `nodeExecutions.data` | NodeExecution[] | 본문 노드들의 NodeExecution 레코드. `parentNodeExecutionId = <Background 노드의 NodeExecution.id>` 로 필터. shape 은 [실행 상세 조회 §5.1](../../3-workflow-editor/3-execution.md#51-노드별-입출력-데이터) 재사용. **`error`·`outputData`·`inputData` 는 응답 egress 에서 값-패턴 마스킹을 거친다** (자격증명 형태 부분문자열 → `***`; DB 는 원문 보존) — 실행 상세(`GET /api/executions/:id`)의 `nodeExecutions[]` 와 **같은 관문**이며 SoT 는 [EIA §R17](../../5-system/14-external-interaction-api.md) "내부 읽기 경로" 불릿이다 (`error` 2026-08-16, 나머지 두 컬럼 2026-08-17 확장). **여기 `inputData` 는 마스킹 대상이다** — 2026-08-20 이전에는 `Execution.inputData` 의 재제출 카브아웃이 **Execution 레벨 한정**이라 노드 레벨만 마스킹했고, 지금은 **두 레벨 모두** 마스킹한다(프런트 마커 가드가 서면서 카브아웃이 닫혔다) |
...
### 본문 blocking 노드 park 시 즉시 graceful 종료 결정

종전 모델은 본문 안의 blocking 노드(form / buttons / ai_conversation)가 park 하면 `pendingContinuations` 에 등록된 채 `maxDurationMs` 까지 hang 하다 타임아웃 실패로 종결됐다 (`notifyOnFailure` 알림 발생 가능). exec-park D6 (durable park) 이후 `executeInline` 이 park 시점에 `ParkReleaseSignal` 을 throw 하고 `executeBackgroundSubgraph` 가 이를 catch 해 본문을 즉시 graceful 종료한다 — background 는 fire-and-forget 이라 외부 `continueExecution` 재개 경로가 없어 durable park 가 무의미하기 때문. 사용자 가시 동작 전환: 이전에는 타임아웃 **실패**로 가시화됐지만, 이제 에러 없이 해당 지점에서 조용히 종료된다 (실패 미집계·알림 미발송). 본문에 blocking 노드를 두는 구성 자체가 비권장이며, 사용자 입력 대기가 필요하면 메인 흐름에 배치한다.
