...
| `nodeExecutions.data` | NodeExecution[] | 본문 노드들의 NodeExecution 레코드. `parentNodeExecutionId = <Background 노드의 NodeExecution.id>` 로 필터. shape 은 [실행 상세 조회 §5.1](../../3-workflow-editor/3-execution.md#51-노드별-입출력-데이터) 재사용. **`error` 는 응답 egress 에서 값-패턴 마스킹을 거친다** (자격증명 형태 부분문자열 → `***`; DB 는 원문 보존) — 실행 상세(`GET /api/executions/:id`)의 `nodeExecutions[].error` 와 **같은 관문**이며 SoT 는 [EIA §R17](../../5-system/14-external-interaction-api.md) "내부 읽기 경로" 불릿이다 (결정 2026-08-16) |
