...
> ⚠ **미구현 (P1)** — [개선안 logic/variable_declaration.md](../../../user_memo/node-specs-improvement/logic/variable_declaration.md) §3 의 후속 정비안. 현재 핸들러는 `meta.durationMs` 외 메타필드를 반환하지 않는다. 아래는 P1 추가 후보:
>
> - **`meta.declared: Array<{ name, type, initialValue }>`** — 이번 실행에서 실제로 신규 등록된 변수 목록 (skip 되지 않은 항목). 후속 노드가 "어떤 변수가 새로 만들어졌는가" 를 가시화.
> - **`meta.skipped: Array<{ name, reason: 'already_declared' }>`** — 이미 존재하여 등록되지 않은 변수 목록. 사용자 의도한 초기화가 silent 로 누락된 경우 가시화.
> - **`meta.coercionWarnings: Array<{ name, type, originalValue, coercedValue }>`** — `coerceToType` 이 `null` 로 떨어진 케이스 (예: `type='number'` + `defaultValue='abc'` → `null`). 사용자 의도와 다른 저장을 가시화.
>
> 위 P1 필드는 모두 **additive** — 도입 시점까지 다운스트림은 `$var.<name>` 로 등록 결과를 직접 조회한다.
...
> ⚠ **silent fallback** (런타임 throw 아님): `defaultValue` 가 `type` 으로 coerce 실패하면 `null` 이 저장된다 (예: `type='number'` + `defaultValue='abc'`). 현재 구현은 별도 경고를 노출하지 않으며, 가시화 제안은 §5.1 의 P1 (`meta.coercionWarnings`) 참조.
>
> ⚠ **silent skip**: 같은 이름 변수가 이미 등록되어 있으면 신규 `defaultValue` 가 무시된다 (덮어쓰기 금지). 재초기화가 필요하면 [Variable Modification](./5-variable-modification.md) 의 `set` 작업을 사용한다.
