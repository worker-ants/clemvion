...
| switchValue | Expression | mode=value 시 ✓ | `''` | 비교 기준 값. expression resolver 가 평가 후 핸들러로 전달 (`{{ }}` 표현식은 핸들러 진입 전 primitive 로 해석). UI asterisk 는 `ui.requiredWhen: { field: 'mode', equals: ['value'] }` 화이트리스트 — [§8 Rationale](#8-rationale) |
...
- **시스템 예약어 금지**: `default`, `out`, `error` (CONVENTIONS Principle 6) 은 case id 로 사용할 수 없다 — 백엔드 `validateSwitchConfig` 의 `RESERVED_CASE_IDS` 집합에서 거부 (`switch.schema.ts:148,163-166`). schema regex 는 문법(slug)만 검증하고 의미 충돌은 이 imperative 검증이 차단한다.
...
3. **mode=expression**: `regex` 연산자를 쓰는 case 의 정규식을 case 별로 `compileRegexCache`(단일 헬퍼 `compileUserRegex`) 로 미리 컴파일한 뒤(잘못된/과길이/**`safe-regex` 위험 패턴(지수 백트래킹, refactor 04 M-3)** 은 skip → 해당 case false; If/Else §6 각주와 동일 정책 — 길이 200 단독은 ReDoS 방지 불충분이라 safe-regex 가 1차 방어), `cases` 를 순회하며 `evaluateCondition(input, case.condition, { strict, regex })` (regex 는 그 case 의 컴파일된 `RegExp`) 를 평가, 첫 true case 채택 → §5.1 (`port: <case.id>`). If/Else 와 동일 평가 경로 — `regex` 연산자가 정상 동작한다.
4. 매칭 실패:
...
> `meta.value` deprecated alias 는 존재하지 않는다. 기존 워크플로 표현식은 `codebase/backend/src/scripts/migrate-node-output-refs.ts` 의 `RENAMED_META_FIELDS.switch` 마이그레이션으로 `meta.resolvedValue` 로 자동 rewrite 된다.
...
> **설계 메모**:
> - `meta.value` deprecated alias 는 제거됐다. 마이그레이션 스크립트(`codebase/backend/src/scripts/migrate-node-output-refs.ts` `RENAMED_META_FIELDS.switch`) 가 기존 워크플로 표현식을 `meta.resolvedValue` 로 자동 rewrite.
> - case id reserved word 검증: `['default', 'out', 'error']` (`RESERVED_CASE_IDS`) 를 백엔드 `validateSwitchConfig` 에서 reject. schema regex 는 문법(slug)만 검증하고 의미 충돌은 이 backend imperative 검증이 차단한다.
> - `meta.switchPath` 는 추가하지 않는다 — switchValue 가 raw 표현식으로 `config` 에 echo 되므로 별도 필드 가치 낮음.
> - `config.strictComparison` 은 현 구현이 실제로 사용 중이므로 유지.
> - 매칭 실패 + `hasDefault=false` 의 throw 동작은 "비즈니스 로직 실패가 아닌 설정 실패" 로 분류되어 throw 유지 (Principle 3.1 Pre-flight 카테고리).
...
| 발생 조건 | 메시지 | 시점 |
|-----------|--------|------|
| `mode='value'` + `switchValue` 미설정 | `In Value mode, Switch Value must be entered.` (warningRule source; i18n 렌더 시 한국어 표시) | warningRule (캔버스 배지) + handler.validate (`evaluateMetadataBlockingErrors`) |
| `cases` 빈 배열 | `At least one case must be added.` (warningRule source) | warningRule (캔버스 배지) + handler.validate |
...
| `cases[i].id` 가 예약 포트명 (`default` / `out` / `error`) | `cases[i].id '<id>' is a reserved port name (default / out / error)` | `validateSwitchConfig` (`RESERVED_CASE_IDS`) |
...
### 8.1 `switchValue.requiredWhen` 화이트리스트 정책

`switchValue` 의 UI asterisk (필수 표시) 는 `ui.requiredWhen: { field: 'mode', equals: ['value'] }` 화이트리스트로 구현된다.

블랙리스트 (`notEquals: 'expression'`) 대신 화이트리스트를 쓰는 이유: `mode` enum 에 신규 값 (예: `'range'`, `'literal'`) 이 추가될 때 블랙리스트는 신규 mode 에도 자동으로 적용되어 의도와 다르게 required 표시가 발화하는 hidden coupling 이 생긴다. 화이트리스트는 신규 mode 마다 명시적 opt-in 을 요구하므로 이 결합을 제거한다.

`requiredWhen` DSL 은 단일 shape `{ field, equals: T | readonly T[] }` 로 단순화돼 있다. `equals` 가 단일 값이면 동등 비교, array 면 화이트리스트 (`.includes()`) 평가.

> **범위 한정**: 본 정준화는 `requiredWhen` DSL 한정. `visibleWhen` DSL 은 `notEquals`/`oneOf` 형태를 한시 유지한다 — `ai-agent.schema.ts:151` 등 능동 사용처 존재. `visibleWhen` 통합 정리는 별 follow-up.

### 8.2 신규 mode 추가 가이드라인

`mode` enum 에 신규 값을 추가할 때 (예: `'range'`):

1. `mode: z.enum([...])` 에 추가
2. **신규 mode 에서 switchValue 가 필요하면** `switchValue.requiredWhen.equals` 배열에 명시 추가 (예: `['value', 'range']`)
3. 신규 mode 에서 switchValue 가 불필요하면 array 그대로 (자동 opt-out)
4. `warningRule.when` 표현식도 화이트리스트 의미와 동기화 검토 — 현재 `'mode != expression && !switchValue'` 형태는 블랙리스트라 신규 mode 추가 시 의도와 다른 발화 가능 (별 follow-up `visibleWhen` 정리 시 함께 처리)
5. `visibleWhen` 의 mode-dependent 메타도 동일 패턴 적용 검토 — **별 follow-up 으로 추적** (현재 `visibleWhen` DSL 은 `notEquals` 형태를 한시 유지 중)
