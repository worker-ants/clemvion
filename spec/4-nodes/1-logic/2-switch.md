...
### 8.1 `switchValue.requiredWhen` 화이트리스트 정책 (2026-05-19)

`switchValue` 의 UI asterisk (필수 표시) 는 `ui.requiredWhen: { field: 'mode', equals: ['value'] }` 화이트리스트로 구현된다.

**종전 (~2026-05-19)**: `{ field: 'mode', notEquals: 'expression' }` — "mode 가 `'expression'` 이 아닐 때 필수" 라는 블랙리스트 의미.

**문제**: `mode` enum 이 향후 추가될 때 (예: `'range'`, `'literal'`) 신규 mode 에도 자동으로 `notEquals: 'expression'` 이 적용되어 의도와 다르게 required 표시가 발화. mode 확장 시 매번 `notEquals` 조건을 재검토해야 하는 hidden coupling 발생.

**선택지 비교**:

| 안 | 효과 | 채택 여부 |
|---|---|---|
| ① `notEquals: 'expression'` 유지 + spec 에 "mode 확장 시 재검토 필요" 명시 | 메커니즘 변경 없음, 그러나 spec 텍스트만으로 강제 어려움 | 기각 |
| ② **`equals: ['value']` 화이트리스트** | 신규 mode 추가 시 명시적 opt-in 필요 (whitelist) | **채택** |
| ③ `oneOf: ['value']` (기존 다른 DSL 형태) | `equals` array 와 의미 중복 | 기각 (DSL 통일성 손실) |

**메커니즘 변경**: `requiredWhen` DSL 자체를 단일 shape `{ field, equals: T | readonly T[] }` 로 단순화 — `notEquals` / `oneOf` 형태는 폐기. `equals` 가 단일 값이면 동등 비교, array 면 화이트리스트 (`.includes()`) 평가.
