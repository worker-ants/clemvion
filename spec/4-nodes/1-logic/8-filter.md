...
| `meta.matchedCount` | number | handler return | `output.match.length`. O(1) 카운트 (Principle 2) |
| `meta.unmatchedCount` | number | handler return | `output.unmatched.length`. O(1) 카운트 |
| `meta.totalCount` | number | handler return | `matchedCount + unmatchedCount`. fallback 시 `0` |
| `meta.fellBackToEmpty` | boolean | handler return | `inputField` 가 `null` / `undefined` 로 해석되어 `[]` fallback 처리된 경우 `true` (Principle 10). 빈 실배열 입력은 `false` (구분) |
| `meta.invalidRegexPatterns` | string[] | handler return | regex 컴파일 실패 / 길이 초과로 silent `false` 처리된 패턴 목록. 클린이면 `[]`. 동일 패턴은 dedup |
...
- `$node["X"].meta.matchedCount` → 매칭 개수 (`output.match.length` 와 동치)
- `$node["X"].meta.fellBackToEmpty` → `inputField` null/undefined fallback 여부
...
| `inputField` resolve 결과가 배열이 아님 (string·number·object) | `Filter inputField does not resolve to an array` | execute (런타임 throw) |
| `inputField` resolve 결과가 `null` / `undefined` | (throw 없음) `output.match: [], output.unmatched: []` + `meta.fellBackToEmpty: true` | execute (Principle 10 fallback) |

> **regex 실패 가시화**: `regex` 컴파일 실패 / 길이 초과는 silent `false` 처리되지만, 해당 패턴은 `meta.invalidRegexPatterns: string[]` 로 누적되어 다운스트림이 패턴 오류를 감지할 수 있다 (동일 패턴은 dedup).
