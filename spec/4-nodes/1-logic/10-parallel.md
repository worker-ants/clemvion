...
> **옛 워크플로우 호환 — 데이터 마이그레이션 없음 (2026-07-16 확정)**: DB 에 `config.waitAll: false` 가 저장된 케이스는 실행 시점에 schema validate 가 reject 하고, 사용자가 워크플로우 편집기에서 수정한다. **이것이 완결된 처리 경로이며, 저장된 값을 일괄 정정하는 별도 데이터 마이그레이션은 수행하지 않는다.**
>
> *근거*: 결정 K(2026-05-30)는 `waitAll=false` 를 **schema 에는 남기되 validate 에서 reject** 하는 형태로 spec out 했다 — 즉 잘못된 값은 조용히 다른 의미로 실행되는 것이 아니라 **명확한 에러 메시지**(§6 표: "Use waitAll=true (default) or the Background node…")로 즉시 드러나고, 그 메시지가 대안(Background 노드)까지 안내한다. 데이터 마이그레이션이 추가로 얻는 것은 "편집기에서 한 번 고치는 수고" 제거뿐인데, 그 대가로 사용자 워크플로우 config 를 소급 수정하는 쓰기 마이그레이션(의도 추정 필요 — `false` 를 `true` 로 바꾸면 semantics 가 달라지고, Background 노드로 옮기는 것은 그래프 구조 변경이라 자동화 불가)을 치러야 한다. **의미가 바뀌는 자동 변환은 사용자 의도를 대신 결정하는 것**이라 reject-and-guide 가 더 안전하다.
>
> *이력*: 종전 이 자리는 "별도 마이그레이션 작업은 `parallel-p2-followups.md` §2-E 의 후속 항목" 이라고 적었으나, 그 §2-E 는 plan 분할 과정에서 **유실**돼 실제로는 어떤 plan 도 이 약속을 추적하지 않는 고아 상태였다(현행 `parallel-p2-followups.md` 에 §2-E 없음, `waitAll` 문자열 자체가 부재). 2026-07-16 grooming 에서 그 사실이 드러나 위와 같이 처분을 확정하고 근거를 본 절에 남긴다 (consistency `23_36_57` rationale_continuity WARNING#1).
...
**왜 silent clamp 인가** — cap 초과 시 reject 대신 `effectiveConcurrency = max(1, floor(32 / parentEffective))` 로 자동 축소. `max(1, …)` 하한은 `parentEffective > 32` 인 경계에서 `floor` 가 0 이 되어 슬롯이 0 → 전 분기 미실행(deadlock) 으로 빠지는 것을 막는다 (최소 1 분기는 항상 실행). 워크플로우 작성자가 외부 / 내부 maxConcurrency 의 곱을 항상 정확히 계산하길 기대하기 어렵다. 의도와 실제의 차이는 두 경로로 가시화:
