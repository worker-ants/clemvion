...
입력을 동일하게 받는 N개의 분기를 동시에(병렬로) 실행하는 **컨테이너 노드** (`executionMetadata.kind = 'parallel'`). 핸들러는 `branch_0` ~ `branch_{N-1}` 동적 출력 포트를 fan-out 활성화하고, 모든 분기가 종료된 후 엔진이 `done` 포트로 `{ branches: [...], count: N }` 결과를 내보낸다 (CONVENTIONS Principle 9, [공통 §9.1](./0-common.md#91-컨테이너-노드-핸들러--엔진-오버라이트-컨트랙트)).
...
| `done` | Done | data | false | 모든 분기 완료 후 `{ branches, count }` 전달 |
...
6. 전체 완료 후 엔진이 `output` 을 `{ branches: [...], count: N }` 으로 **오버라이트** (Principle 9.2) → `done` 포트 (`port: 'done'`, 단일 문자열) 로 전달.
...
```json
{
  ...
  "output": { "userId": "u-1", "task": "process" },
...
| `output` | (input 전체) | handler return | 현재 구현은 input 을 그대로 echo 한다 — 외부에서 직접 관찰되지 않는 중간 형태 (Principle 9.1). 다운스트림이 `$node["X"].output.*` 로 관찰하는 값은 §5.2 의 완료 형태 |
| `port` | `string[]` | handler return | `branch_0` ~ `branch_{N-1}` 모두 활성화 (CONVENTIONS Principle 5 의 fan-out). 각 분기 서브그래프의 진입점 트리거 |

> ⚠ **미구현 (P1)**: user_memo 개선안은 시작 시점 `output: null` 을 권고한다 (loop/foreach/map 과 동일한 `output: null` + 엔진 오버라이트 패턴 통일). 현재 핸들러는 input pass-through 를 반환하지만, 외부 expression 노출 형태는 §5.2 이므로 다운스트림 영향은 없다 ([logic/parallel.md §3 항목 2](../../../user_memo/node-specs-improvement/logic/parallel.md#2-식별된-불일치)).
...
```json
{
  ...
  "output": {
    "branches": [
      { "userId": "u-1", "step": "validate", "ok": true },
      { "userId": "u-1", "step": "enrich", "ok": true },
      { "userId": "u-1", "step": "notify", "ok": true }
    ],
    "count": 3
  },
  "meta": {
...
| `output.branches` | unknown[] | engine override (Principle 9.2) | 각 분기의 terminal 노드 출력 배열. `branches[i]` 는 `branch_i` 분기의 결과. 컬렉션 키 = `branches` ([공통 §5](./0-common.md#5-반복분기-출력-구조-conventions-92)) |
| `output.count` | number | engine override | 수집된 분기 수 (= `branches.length`). 정상 완료 시 `config.branchCount` 와 동일 |
| `meta.durationMs` | number | engine inject | Parallel 컨테이너 전체 실행 시간 (ms) |
| `meta.branches` | number | engine inject | **실제로 완료된 분기 수** (Container 메트릭, Principle 2). 정상 완료 시 `config.branchCount` 와 동일하지만, 향후 `waitAll:false`/취소/에러 시나리오에서 다른 값을 가질 수 있는 런타임 메트릭 |
| `port` | `'done'` | engine | 단일 문자열 (Principle 5). 모든 분기 완료 분기 |

**Expression 접근 예**:
- `$node["Parallel"].output.branches[0]` → `branch_0` 분기의 terminal 출력
- `$node["Parallel"].output.count` → 분기 수 (O(1))
- `$node["Parallel"].meta.branches` → 완료된 분기 수 (런타임 메트릭)
- `$node["Parallel"].port` → `"done"`

> ⚠ **미구현 (P1)**: user_memo 개선안은 `output.branches[i]` 를 `Promise.allSettled` 모델에 맞춰 `{ status: 'success' | 'failure', value?, error? }` 로 표준화 + `meta.successCount` / `meta.failureCount` / `meta.concurrency` / `meta.errorPolicy` 추가를 제안한다 ([logic/parallel.md §3](../../../user_memo/node-specs-improvement/logic/parallel.md#3-제안된-output-구조)). 또한 `output.count` 는 `config.branchCount` 의 echo 이므로 P1.1 직교성 위반 — 제거 후 `branches.length` / `meta.branches` 사용을 권고한다. 현재 구현은 [공통 §5](./0-common.md#5-반복분기-출력-구조-conventions-92) 에 따라 `count` 를 유지한다.
...
|------|-------------|------|------|
| 시작 (N분기 fan-out 직전) | `output: input` (현재) / `null` (개선안) | `string[]` (`branch_0`~`branch_{N-1}`) | handler return — 분기 포트 트리거용 |
| 완료 (모든 분기 종료 후) | `output: { branches: [...], count: N }` | `'done'` (단일 문자열) | **엔진 오버라이트** |

**컨트랙트 핵심**:
- 다운스트림 노드가 `$node["Parallel"].output.*` 로 관찰하는 값은 **항상 §5.2 의 완료 형태**.
- 핸들러가 시작 시점에 반환한 `output` 은 외부 expression 으로 노출되지 않는다 (엔진이 덮어쓰기 전 중간 단계).
...
> ⚠ **미구현 (P1)**: user_memo 개선안은 시작 시점 `output: null` 통일을 권고하지만, 현재 핸들러 구현은 input pass-through 를 반환한다. 동작상 다운스트림 영향은 없다 (외부 노출은 §5.2 형태) — [logic/parallel.md §3 마이그레이션 P0](../../../user_memo/node-specs-improvement/logic/parallel.md#4-마이그레이션-영향도) 참조.
