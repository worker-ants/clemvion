...
code:
  ...
  - codebase/backend/src/nodes/core/condition-evaluator.util.ts
...
조건 평가 노드가 공통으로 사용하는 **단일 조건(Condition)** 구조. 코드 SSOT 는 `conditionGroupSchema` (if-else.schema.ts) + 평가 로직 `condition-evaluator.util.ts`.

| 필드 | 타입 | 설명 |
|------|------|------|
| field | Expression | 비교할 값 (표현식). Filter 등 배열 노드에서는 비우거나 `$item` 으로 현재 항목 참조 |
| operator | Enum | 비교 연산자 (아래 표 참조) |
| value | Expression | 비교 대상 값 (단항 연산자 `is_empty` / `is_null` 등에서는 생략) |

**조건 결합:** 조건들은 **단일 평면 배열 `conditions: Condition[]`** 로 저장되고, 노드 config 의 top-level `combineMode` (`and` / `or`, 기본 `and`) 로 결합된다 — 별도의 `logicalOperator` 필드를 둔 2계층 그룹 구조는 **존재하지 않는다**.

- **If/Else·Filter**: `conditions: Condition[]` + `combineMode` + `strictComparison` 을 직접 사용 (`if-else.schema.ts`, `filter.schema.ts`).
- **Switch**: `conditions[]` 를 쓰지 않고 `cases[]` 구조를 사용한다. 단, expression 모드의 각 case 는 단일 `condition` 필드(위 Condition 구조)를 가진다 (`switch.schema.ts` `caseDefSchema`).
- **ForEach**: 조건 평가 노드가 아니다 — `arrayField` + `errorPolicy` 만 갖는다 (`foreach.schema.ts`).
...
| 값 | 동작 |
|------|------|
| `stop` (기본) | 첫 분기 실패 시 throw (Parallel 노드 FAILS) |
| `continue` | 모든 분기 완료를 기다리고, 실패한 분기는 `output.branches[i].error` 로 수집 |
| `cancel-others-on-fail` | 첫 실패 시 진행 중인 다른 분기를 `AbortSignal` 로 abort (signal-aware 노드만 조기 cleanup, best-effort) |
...
| `maxConcurrency` | Parallel | 동시 실행 제한. `0` = branchCount 와 동일 (모든 분기 동시 실행), `1`~`16` = 해당 수만큼 동시 실행. 기본 `0` (`parallel.schema.ts` ui hint: "0 = same as branchCount, unlimited") |
...
- 동적 포트: config 항목이 보유한 **slug-regex(`^[a-zA-Z0-9_-]{1,64}$`) 유효 stable id** 를 포트 ID 로 사용한다 (Switch case 등 Logic 노드는 사용자 입력 의미 slug). 포트 이름 변경, 재정렬, 다른 포트 삭제 등 편집 작업에도 **기존 포트 ID는 불변**. 생성 방식의 노드별 상세(예: 자동 UUID v4 발급)·검증 SoT 는 [노드 §1.3](../0-overview.md#13-포트-정의-portdef) / `port-id.util.ts`.
...
| `background` (main 포트) | `main` (즉시) / `background` (background 본문 진입 시) | `meta.backgroundRunId` |
...
| [background](./12-background.md#5-출력-구조) | §5.1 (`main`) / §5.2 (`background`) | §5.7 (변형) | 컨테이너 (fire-and-forget) |
