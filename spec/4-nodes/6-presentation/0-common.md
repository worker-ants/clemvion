...
| (4) LLM tool_result content | AI Agent `render_form` 의 tool_result content (`render_form` 만 해당) | `{ ok: true, type: 'form_submitted', data: { …formData }, message: '<재호출 금지 안내문>' }` JSON. 가드 필드 (`ok`, `message`) 는 LLM 이 같은 form 을 재호출하지 않도록 차단하는 신호 ([AI Agent §12.6](../3-ai/1-ai-agent.md#126-render_form-submit-후-llm-의-동일-form-재호출-회귀-차단-2026-05-24)) | [AI Agent §6.2 step 2.c](../3-ai/1-ai-agent.md#62-multi-turn-모드-mode--multi_turn) |
