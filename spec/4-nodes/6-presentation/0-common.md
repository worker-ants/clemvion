...
**활성 form 의 UI 표면 — assistant turn 의 timeline 인라인 단일 진실** (2026-05-23): `render_form` 활성 단계에서 form 입력 UI 는 별도 surface (예: MessageInput 아래 stack) 가 아니라 **assistant turn 의 `presentations[*]` 중 `type: 'form'` 페이로드 위치에 inline 렌더**된다. frontend `AssistantPresentationsBlock` 의 case `"form":` 는 다음 predicate 로 분기:
...
캐러셀·차트·테이블이 timeline 안에 inline 되는 시각 패턴과 일관 — `render_form` 만 별도 stack 으로 분리되어 있던 옛 표현은 폐기 (§Rationale "render_form 활성 form 의 timeline 인라인 통합 (2026-05-23)").
...
| (4) LLM tool_result content | AI Agent `render_form` 의 tool_result content (`render_form` 만 해당) | `{ ok: true, type: 'form_submitted', data: { …formData }, message: '<재호출 금지 안내문>' }` JSON. 가드 필드 (`ok`, `message`) 는 LLM 이 같은 form 을 재호출하지 않도록 차단하는 신호 ([AI Agent §12.6](../3-ai/1-ai-agent.md#126-render_form-submit-후-llm-의-동일-form-재호출-회귀-차단-2026-05-24)). `data` 크기가 10KB cap (`FORM_SUBMITTED_MAX_BYTES`) 초과 시 옵셔널 `formDataTruncation: {originalBytes, bytesAfterCap, truncatedFields[]}` 메타 동시 부착 — LLM 이 truncate 사실을 reasoning 에 반영하도록 명시 ([AI Agent §12.7](../3-ai/1-ai-agent.md#127-render_form-submit-후-formdata-크기-cap-2026-05-24)) | [AI Agent §6.2 step 2.c](../3-ai/1-ai-agent.md#62-multi-turn-모드-mode--multi_turn) |
...
- LLM tool_result content (`{ok:true, type:'form_submitted', data:{…}, message:'<...>'}`, ai-agent §6.2 step 2.c) 는 LLM-facing layer. 기존 `{type, data}` SoT 는 유지 + 재호출 가드 필드 `ok`, `message` 가 함께 직렬화된다 ([AI Agent §12.6](../3-ai/1-ai-agent.md#126-render_form-submit-후-llm-의-동일-form-재호출-회귀-차단-2026-05-24)). 본 가드 필드는 LLM-facing layer 한정 — 위 (1)(2)(3) layer 형식은 변경 없다.
...
### render_form 활성 form 의 timeline 인라인 통합 (2026-05-23)

§10.6 본문 보강 — `render_form` 활성 단계의 UI 표면을 별도 surface 가 아닌 **assistant turn 의 `presentations[*].form` payload 위치에 inline 렌더** 하도록 단일 진실 명문화. 옛 표현 (run-results-drawer 의 `[ConversationInspector] → [DynamicFormUI]` vertical stack, execution detail page 의 별도 DynamicFormUI 분기) 폐기.

**배경**: 사용자 보고 (2026-05-23) — `render_form` blocking flow 정식 출시 (PR #273-#288) 후 활성 단계 UX 3가지 회귀 (submit 후 timeline 깜빡임 / form 잔존 / MessageInput 아래 stack 위치). 셋의 공통 root cause 는 "활성 form 의 UI 표면이 timeline 메시지가 아니라 별도 surface" 였다.

**결정 근거**: [AI Agent §12.5](../3-ai/1-ai-agent.md#125-render_form-활성-form-의-timeline-인라인-표현-통합-2026-05-23) 가 SoT. 본 절은 presentation common 측 cross-ref 만 둔다 — 본질이 AI Agent multi-turn 의 surface 통합이고, presentation 노드 본체 동작은 영향 없음 (그래프 form 노드 standalone 동작 무변경).
...
**MessageInput 그대로 활성 (form 우회 허용)**: 사용자 결정 (2026-05-23) — form 활성 중에도 MessageInput 은 비활성/숨김이 아닌 항상 활성 유지. form bypass 의 backend 처리는 [AI Agent §6.2 단계 2.c.bypass](../3-ai/1-ai-agent.md#62-multi-turn-모드-mode--multi_turn) — cancelled tool_result fallback 으로 LLM 의 다음 reasoning 자유 보존.
