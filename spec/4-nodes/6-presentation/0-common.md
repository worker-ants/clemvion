...
| 그 외 (`!type` / 미매칭) | **warn log + 현 turn no-op park** | silent skip 금지 — `console.warn('[processAiResumeTurn] unknown action.type', { executionId, action })` 후 현 turn 을 처리 없이 다시 park(다음 resume 대기)한다. turn 누적 방어는 `maxTurns` cap 이 별도 layer 에서 담당 |
