...
`continueExecution(executionId, formData)` 는 raw `formData` 를 그대로 enqueue 하지 않고 `{ type: 'form_submitted', formData }` 로 wrap 한 뒤 [Continuation Bus](../../5-system/4-execution-engine.md#74-분산-실행-multi-instance) (BullMQ `execution-continuation` 큐 — `bus.add({ type: 'continue', executionId, nodeExecutionId, payload: { type: 'form_submitted', formData } }, { jobId, attempts })`) 에 enqueue 한다. `'continue'` listener 는 wrap 된 payload 를 그대로 `resolvePending(executionId, payload)` 로 forward 한다 (unwrap 하지 않음 — dispatcher 가 sentinel 로 분기).
...
> Phase B 단계 주: 본 invariant 가 참조하는 `pendingContinuations` in-memory resolver 는 **멀티턴 AI(`waitForAiConversation`) 한정**으로 유지된다. 단발 폼/버튼 park 는 PR-B1 에서 코루틴을 즉시 해제(park=세그먼트 종료)하고 재개를 §7.5 rehydration 으로 일원화했으므로 `pendingContinuations` 를 등록하지 않는다. 멀티턴 AI 도 PR-B2 의 turn-단위 park 전환 시 동일하게 in-memory resolver 를 제거하며, 그때 본 graceful degradation 서술은 rehydration 모델(`else` 분기 = warn + 재개 대기) 기준으로 재작성된다. ([실행 엔진 §Rationale "park 즉시 해제 + slow-path 일원화"](../../5-system/4-execution-engine.md#rationale))
