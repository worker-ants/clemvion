...
> Source of truth: `codebase/backend/src/nodes/presentation/chart/chart.schema.ts` (export `chartConfigSchema`)
...
1. 입력 데이터 배열 정규화 (handler `execute` 우선순위):
   - `config.dataSource` 가 정의되어 있으면 (배열은 그대로 / 단일 값은 `[v]`) 사용. ⚠ **`dataSource` 는 `chartConfigSchema` 에 정의되지 않은 키**다 (schema 는 `.passthrough()` 라 저장 시 tolerated). §1 config 표에도 없으므로 정식 노출 surface 가 아니며, handler 의 방어적 경로로만 사용된다 (`chart.handler.ts:48-51`). 정식 데이터 추출은 아래 `dataField` 경로를 사용한다.
...
5. SVG 차트 렌더링은 **프런트엔드가 client-side 라이브러리(recharts)로 `output.data` + `config.{chartType, title, xAxis, yAxis, colors}` 로부터 직접 그린다** — backend 는 SVG snapshot 을 채우지 않음 (Principle 1.1, Carousel `rendered` 폐기와 동일 패턴)
...
>
> 핵심 원칙 (Principle 1.1): `chartType` / `title` / `xAxis` / `yAxis` / `buttons` 등 **리터럴 config 값은 `output` 에 echo 금지**. `output.data` 는 input 을 xAxis 기준으로 버킷팅 + aggregation 적용한 **런타임 집계 결과** 이므로 `output` 유지. **SVG 스냅샷은 backend 가 채우지 않음** — 프런트엔드가 client-side recharts 로 직접 렌더링 (Principle 1, 메모리 절약 + Carousel `rendered` 폐기와 일관).
...
| 발생 조건 | 메시지 | 시점 |
|-----------|--------|------|
| `chartType` 누락 | `Chart type must be selected.` | warningRule (캔버스 배지) + handler.validate |
| `chartType` 이 enum (`bar`/`line`/`pie`) 미일치 | `chartType is required and must be one of: bar, line, pie` | handler.validate |
| `xAxis.field` 누락 | `X-axis field must be entered.` | warningRule (캔버스 배지) |
| `yAxis.field` 누락 | `Y-axis field must be entered.` | warningRule (캔버스 배지) |
| `buttons[i]` 의 `id`/`label`/`url` 검증 실패 | `validateButtons` 규칙 위반 메시지 | handler.validate ([공통 §1.1](./0-common.md#11-유효성-검증)) |

> Source of truth: `chartType` / `xAxis.field` / `yAxis.field` 누락 메시지는 schema `warningRules` 의 영문 문자열(`chart.schema.ts:176,181,186`)이 SoT. 캔버스 배지의 한국어 표기는 i18n 렌더 결과다.

> ⚠ **Caveat (P1) — `chartType` schema/handler 불일치**: schema enum 은 `bar | line | pie | donut | area` 5종을 모두 허용하지만 handler 의 `validate()` 는 `bar | line | pie` 3종만 통과시킨다 (`codebase/backend/src/nodes/presentation/chart/chart.handler.ts:22` 의 로컬 `validTypes` 배열). schema 정의로 `donut` / `area` 를 저장한 워크플로우는 실행 단계에서 reject 된다. 프런트 렌더러(`presentation-renderers.tsx:308-362`)는 이미 bar/line/area/pie/donut 5종을 모두 그리므로 미해결 갭은 **handler.validate 의 enum 확장뿐**이다. 해소 옵션 두 가지: (A) handler 의 validation 을 schema 와 동일한 5종으로 확장 (렌더러는 이미 준비됨), (B) schema enum 에서 `donut` / `area` 제거 (기존 draft breaking). 본 spec 의 출력 구조에는 영향 없음 — 별도 트랙으로 추적.
