...
> - **실행이력 / run-results 프리뷰 (`CarouselContent`)** — 이미 실행이 끝난 **과거 스냅샷이라 인터랙션이 불가**하므로, 픽셀 충실한 시각 재현 대신 **텍스트 포워드 데이터 뷰**로 렌더한다: 슬라이드별 제목·설명·버튼 라벨을 나열하고 `layout` 값을 배지로 표시하며, 이미지는 **lazy 로딩 썸네일**(+URL 확인 가능)로 노출해 데이터·이미지 매핑 검증에 최적화한다. 다수 이미지의 eager 로드를 피한다. (결정 근거: [Rationale](#rationale).)
