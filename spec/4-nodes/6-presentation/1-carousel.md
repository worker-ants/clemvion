...
| itemButtons | ButtonDef[] | ✗ | `[]` | 동적 아이템 공통 버튼 (dynamic 전용, 최대 4개). 런타임에 `<itemButton.id>__item_<idx>` ID 가 생성되며 라우팅 시 base ID 로 매핑 (Principle 6) |
| layout | `card` / `image` / `minimal` | ✗ | `card` | 카드 레이아웃 |
| buttons | ButtonDef[] | ✗ | `[]` | 글로벌 버튼 정의 (최대 10개). 비어있지 않으면 Blocking Mode 활성화. ButtonDef 구조는 [공통 §1](./0-common.md#1-buttondef-구조) |
...
| buttons | ButtonDef[] | ✗ | per-item 버튼 (최대 4개). port/link 모두 지원. port 클릭 시 `selectedItem` 이 interaction payload 에 포함 (§5.5) |

> Source of truth: `backend/src/nodes/presentation/carousel/carousel.schema.ts` (export `carouselNodeConfigSchema`)
...
4. 1MB cap 적용 — `truncateArrayForOutput` 으로 tail 부터 element 단위 잘라냄. 잘린 결과는 array 형태 유지 (공통 §4).
5. cap 적용된 items 로 HTML 렌더링 (`output.rendered`).
6. 글로벌 + per-item 버튼 합산해 `buttonConfig.buttons` 작성, per-item ID → 아이템 인덱스 매핑을 `buttonConfig.buttonItemMap` 에 기록.
...
>
> ⚠ **목표 스펙**: 본 §5 의 Waiting `output` 은 CONVENTIONS Principle 1.1 / 4.3 의 목표 형태(static `{}` / dynamic `{ items }`) 로 기술한다. 현 핸들러는 두 모드 모두 `output.items` + `output.rendered` 를 반환하고 resumed 시 `previousOutput` 호환 필드를 surface 한다 — 자세한 미구현 항목은 각 케이스 말미 ⚠ 박스 참조 ([개선안 P0](../../../user_memo/node-specs-improvement/presentation/carousel.md#44-마이그레이션-전략)).
...
| `output.items` | Array | runtime — `source` resolve + field 매핑 (dynamic) / cap 적용된 `config.items` (static) | 1MB cap 적용 (§4) |
| `output.rendered` | String (HTML) | runtime — cap 적용 후 렌더 | layout 별 HTML snapshot |
| `meta.durationMs` | number | engine inject | 실행 시간 (ms) |
| `port` | `undefined` | — | 단일 출력 (`out`) — `port` 미설정 (Principle 5) |

> ⚠ **부분 미구현 (Principle 1.1)**: 본 케이스의 `output` 은 dynamic 모드 기준이라 Principle 1.1 에 부합한다. **static 모드의 비-블로킹** 은 현 핸들러가 `output.items` 를 echo 하고 있어 Principle 1.1 위반 — 개선안은 static 비-블로킹도 `output: {}` 으로 단순화하되, 실무상 비-블로킹 단일 케이스는 dynamic 위주로 사용된다 ([개선안 §3.1](../../../user_memo/node-specs-improvement/presentation/carousel.md#31-waiting-static-모드)).
...
> ⚠ **미구현 (P0)**: 현 핸들러는 §5.4-A static / §5.4-B dynamic 양쪽 모두 `output: { items, rendered }` 를 반환한다 — Principle 1.1 (static items echo) / Principle 4.3 (static `{}` / dynamic `{ items }`) 위반. 또한 `output.rendered` HTML snapshot 은 Principle 1 (output 은 비즈니스 결과물만) 의 경계 — `meta.rendered` 로 이동하거나 제거하는 것이 [개선안](../../../user_memo/node-specs-improvement/presentation/carousel.md#2-식별된-불일치)의 P0 항목. 본 spec 은 목표 형태로 기술하되, 코드 반영 전까지 다운스트림은 두 모드 모두 `output.items` 를 임시로 사용해도 동작한다.
...
> ⚠ **미구현 / 정합 격차 (P0)**: 현 핸들러는 resumed snapshot 에 `previousOutput` 호환 필드를 surface 하고 `status` 로 `'button_click'` / `'button_continue'` 리터럴을 일부 경로에서 사용한다 — Principle 4.1 (status `'resumed'` 통일) / Principle 4.2 (`previousOutput` 폐기) 위반. [개선안 §4.4 마이그레이션 전략](../../../user_memo/node-specs-improvement/presentation/carousel.md#44-마이그레이션-전략) P0 항목으로 코드 반영 시까지 다운스트림은 `port` 와 `interaction.data.*` 를 우선 참조한다.
...
- 최대 10개 버튼 (글로벌). per-item 은 ItemDef 당 최대 4개 (static), `itemButtons` 도 최대 4개 (dynamic)
- 버튼 클릭 시까지 무제한 대기 (외부 cancel/종료 외에는 타임아웃 없음)
...
| per-item 버튼 ≥5개 | `items[i]: maximum 4 buttons per item` (static) / `itemButtons: maximum 4 buttons per item` (dynamic) | validateConfig |
