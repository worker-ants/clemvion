...
| layout | `card` / `image` / `minimal` | ✗ | `card` | 카드 레이아웃. **현재 렌더러(`presentation-renderers.tsx` `CarouselContent`)는 본 필드를 미참조** — 항상 가로 스크롤 카드로 렌더. `image` / `minimal` 변형 렌더는 미구현 (Planned). config 필드 자체는 검증·echo 됨 |
...
> 프론트엔드는 `config` (mode/layout/items/...) + dynamic 모드의 `output.items` 를 조합해 카드를 재구성한다. 핸들러는 HTML snapshot 을 생성하지 않는다 (Principle 1). **`layout` 별 (`image` / `minimal`) 레이아웃 변형 렌더는 미구현 (Planned)** — 현재 `CarouselContent` 는 `config.layout` 을 미참조하고 항상 가로 스크롤 카드로 렌더한다.
