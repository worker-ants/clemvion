...
code:
  ...
  - codebase/channel-web-chat/src/widget/components/presentations.tsx
---
...
| layout | `card` / `image` / `minimal` | ✗ | `card` | 카드 레이아웃. **인터랙티브 채널(웹챗 위젯)** 은 본 값에 따라 슬라이드를 재구성한다 — `card`=이미지+제목+설명, `image`=이미지 지배(설명 생략), `minimal`=텍스트 전용(이미지 생략). **실행이력/run-results 프리뷰**(`presentation-renderers.tsx` `CarouselContent`)는 시각 레이아웃을 재현하지 않고 텍스트 포워드 데이터 뷰로 렌더하며 본 값을 배지로 노출한다 (§4). config echo 됨 |
...
> 프론트엔드는 `config` (mode/layout/items/...) + dynamic 모드의 `output.items` 를 조합해 슬라이드를 재구성한다. 핸들러는 HTML snapshot 을 생성하지 않는다 (Principle 1). 렌더 표면은 **두 곳**이며 목적이 달라 표현이 갈린다:
>
> - **인터랙티브 채널 (웹챗 위젯 등)** — `layout` 에 따라 **시각 레이아웃**을 재구성한다: `image` 는 이미지 지배·설명 생략, `minimal` 은 이미지 생략·텍스트 전용, `card` 는 이미지+제목+설명. 한 번에 한 슬라이드 + prev/next 내비게이션으로 최종 사용자가 스크롤·버튼 클릭한다. (구현: `codebase/channel-web-chat/src/widget/components/presentations.tsx` `CarouselView`.)
> - **실행이력 / run-results 프리뷰 (`CarouselContent`)** — 이미 실행이 끝난 **과거 스냅샷이라 인터랙션이 불가**하므로, 픽셀 충실한 시각 재현 대신 **텍스트 포워드 데이터 뷰**로 렌더한다: 슬라이드별 제목·설명·버튼 라벨을 나열하고 `layout` 값을 배지로 표시하며, 이미지는 **lazy 로딩 썸네일**(+URL 확인 가능)로 노출해 데이터·이미지 매핑 검증에 최적화한다. 다수 이미지의 eager 로드를 피한다. (본 PR 구현 — 결정 근거: [Rationale](#rationale).)
...
> 비-블로킹 핸들러 반환은 `{ config, output }` 만이다 (`carousel.handler.ts` non-blocking return). `meta.durationMs` 는 엔진이 주입한다 (handler 가 직접 넣지 않음).
...
> 메시지는 warningRule(SoT: `carousel.schema.ts` `carouselNodeMetadata.warningRules`)의 영문 원문을 인용한다.

| 발생 조건 | 메시지 | 시점 |
|-----------|--------|------|
| dynamic 모드인데 `titleField` 누락 | `In Dynamic mode, a Title field must be entered.` | warningRule (캔버스 배지) + handler.validate |
| static 모드인데 `items` 빈 배열 | `In Static mode, at least one slide must be added.` | warningRule |
| `mode` 가 `static` / `dynamic` 외 | `Mode must be either static or dynamic.` | warningRule |
...
### R-1. `layout` 렌더 two-surface 분리 (2026-07-07, 옵션 D)

`layout`(card/image/minimal)의 렌더 표면은 두 곳이며 목적이 달라 표현을 분리한다.

- **인터랙티브 채널(웹챗 위젯)**: 최종 사용자가 슬라이드를 스크롤·버튼 클릭하는 실사용 표면. 여기서는 `layout` 에 따라 시각 레이아웃을 재구성한다(`image`=이미지 지배·설명 생략, `minimal`=텍스트 전용·이미지 생략, `card`=이미지+제목+설명, 한 번에 한 슬라이드+prev/next). **이미 구현돼 있다**(`channel-web-chat` `CarouselView`).
- **실행이력/run-results 프리뷰**(`CarouselContent`): 이미 실행이 끝난 과거 스냅샷이라 **인터랙션이 불가**하다. 세 CSS 레이아웃을 픽셀 충실히 재현할 값어치가 낮고, 다수 이미지의 eager 로드는 순비용이다. 따라서 시각 재현 대신 **텍스트 포워드 데이터 뷰**(제목·설명·버튼 라벨 + `layout` 배지 + lazy 썸네일/URL)로 렌더해 디버깅(데이터·이미지 매핑 확인)에 최적화한다.

**기각된 대안**:
- *옵션 A (프리뷰에 세 시각형 포팅)*: 인터랙션 없는 디버그 화면에 시각 충실도를 투자할 실익이 낮고, 시각적 진실은 이미 웹챗에 존재해 제품 약속 손실이 없다.
- *옵션 C (`image`/`minimal` 강등·제거)*: `layout` 은 웹챗에서 실사용 중이라 제거 시 인터랙티브 렌더가 깨진다 — 불가.

과거 spec(§6.1·§5.2 등)이 프리뷰에 대해 명시하던 "좌/우 화살표 슬라이드 탐색·인디케이터 + 시각 레이아웃 재구성"은 실제 코드(가로 스크롤)와도 어긋난 drift 였으며, 본 결정으로 spec 을 실제 동작(text-forward)에 정합시킨다. (추적: `plan/spec-sync-carousel-gaps` 옵션 D.)
