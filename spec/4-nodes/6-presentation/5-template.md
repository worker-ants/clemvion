...
> Source of truth: `backend/src/nodes/presentation/template/template.schema.ts` (export `templateNodeConfigSchema`)
...
[공통 §4.2](./0-common.md#42-resumed-버튼-클릭--폼-제출-후) / [CONVENTIONS §4.4–4.5](../../../user_memo/node-specs-improvement/CONVENTIONS.md) 에 따라, waiting 시점의 `output.rendered` 는 그대로 유지하고 `output.interaction` 이 추가된다.
