...
1. **(엔진)** Expression resolver 가 `config.template` 내 `{{ }}` 토큰을 현재 실행 컨텍스트(`$input`, `$node[*]`, `$var`, ...) 로 치환한 문자열을 핸들러에 전달. 원본 raw 문자열은 `context.rawConfig.template` 으로 유지.
   - **입력 root-spread (Template 전용)**: 입력 데이터(nodeInput)가 **비-배열 객체**이면, 엔진이 그 최상위 키들을 expression context 의 root-level 변수로 spread 한다 — `{{ name }}` 이 `{{ $input.name }}` 과 동등하게 동작 (§2 목업의 `{{title}}` / `{{date}}` 가 이 규칙에 의존). 단, `$`-builtin 등 **기존 context 키와 동명이면 기존 키 우선** (덮어쓰지 않음 — `Object.hasOwn` 가드). 입력이 배열·원시값이면 spread 하지 않는다. 본 동작은 `NODE_TYPES.TEMPLATE` 분기에 의한 template-only 특수 처리로, 다른 노드 타입에는 적용되지 않는다.
