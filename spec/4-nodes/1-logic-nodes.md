...
배열 데이터를 개별 항목으로 분리하여 **배열로 출력**한다. 각 항목에 대한 반복 실행이 필요하면 ForEach 노드와 조합한다.
...
### 포트
- 입력: `in` (1개)
- 출력: `out` (1개 — 분리된 항목을 배열로 일괄 출력)

### 실행 로직
1. `fieldPath`로 배열 추출
2. 배열이 아니면 에러
3. 분리된 항목을 **배열로** 출력 포트에 전달 (개별 순차 출력이 아님)
4. keepOtherFields=true 시 원본 객체의 다른 필드를 각 항목에 병합하여 객체 배열로 출력

**예시:**
```
입력: { "id": 1, "items": ["a", "b", "c"] }

출력 (keepOtherFields=false):
  ["a", "b", "c"]

출력 (keepOtherFields=true):
  [
    { "id": 1, "item": "a" },
    { "id": 1, "item": "b" },
    { "id": 1, "item": "c" }
...
```
[Split] → out: ["a","b","c"] → [ForEach (arrayField: $input)] → body: 각 항목 처리 → done: 결과 배열
```

**예시 — 주문 항목 개별 처리:**
```
1. Split: $input.order.items → ["item1", "item2", "item3"] 배열 출력
2. ForEach: 배열의 각 항목에 대해 body 내 노드 그룹 실행
...
4. collectResults=true 시 각 body 실행 결과를 배열로 수집 (원본 배열과 **동일 인덱스** 유지)
   - errorPolicy=`skip` 시 스킵된 인덱스에 에러 객체 삽입: `{ _skipped: true, error: { code: "...", message: "..." } }`
   - errorPolicy=`continue` 시 에러 발생 항목도 결과에 포함 (에러 정보는 NodeExecution에 기록)
