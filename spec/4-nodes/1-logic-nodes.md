...
7. 반복 완료 후 **`{ iterations: [...], count: N }`** 를 `done` 포트로 전달 (CONVENTIONS §9.2).
...
3. 모든 iter 완료 시 **`{ mapped: [...], count: N }`** 를 `done` 포트로 전달 (CONVENTIONS §9.2).
...
3. `errorPolicy`에 따른 에러 처리 (원본 배열과 **동일 인덱스** 유지):
   ...
   - `stop`: 즉시 실행 실패.
4. 전체 완료 후 **`{ items: [...], count: N }`** 를 `done` 포트로 전달 (CONVENTIONS §9.2).
...
| maxConcurrency | Integer | 동시 실행 제한. `0` = 무제한(전체 동시), `1`~`16` = 해당 수만큼 동시 실행. 기본 `0`. |
...
4. waitAll=true → 모든 분기 완료 후 **`{ branches: [...], count: N }`** 를 `done` 포트로 전달 (CONVENTIONS §9.2)
