...
| timeout | Integer | 입력 대기 타임아웃 (초 단위, 기본: 300). `0 = no timeout` (무제한 대기) |
| partialOnTimeout | Boolean | 타임아웃 시 부분 병합 수행 여부 (기본: false). true 시 도착한 입력만으로 병합 수행. false 시 에러 처리 정책에 따름 (`MERGE_TIMEOUT` 에러). `timeout = 0`인 경우 적용되지 않음 |
...
| notifyChannels | String[] | 알림 채널 목록 (기본: `["in_app"]`). 지원 값: `in_app`, `email` |
...
| Background | 알림 채널 나열. 없으면 `no notification` | `notify: in_app, email` |
