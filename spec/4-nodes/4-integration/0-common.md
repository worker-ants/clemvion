...
>
> 이 판정은 schema `warningRules` 로 표현할 수 없다 — `warningRule` 의 `when` DSL 평가기(`node-summary` `evaluateWhen(expr, config)`)는 노드 **자신의 config 객체만** 인자로 받아 cross-entity(integration 실재 여부) 검증에 닿지 못한다. 기존 통합 노드 규칙의 `!integrationId` 류는 "필드 미선택"만 검사할 뿐 삭제 여부는 못 본다. 따라서 배지는 warningRule 이 **아니라 캔버스 렌더러 전용 판정**이다: 렌더러(`custom-node.tsx`)가 워크스페이스 integration 목록을 React Query 로 조회해(캔버스 전용 `["integrations","list"]` 키 — 설정 패널 셀렉터의 `serviceType` 필터 포함 키 `["integrations","list",{serviceType}]` 와 달리 전체 목록을 한 번에 받아 category=integration 노드 전반에 재사용) `config.integrationId` 의 실재 여부를 대조한다 — 실재 여부 판정 로직 자체는 설정 패널의 `integration-selector`·`mcp-server-selector` 가 이미 쓰는 missing 판정과 **동일 패턴**이다. 목록 로딩 중에는 배지를 억제해 위양성을 피하며, `integrationId` 가 비어 있으면(미선택) 이 배지가 아니라 각 노드 schema 의 `warningRules`(필드 미선택 경고)가 담당한다. 백엔드·`warningRules` DSL 무변경 (cross-entity 경고가 향후 다수 등장하면 DSL lookup 일반화를 재검토 — plan `spec-sync-integration-common-gaps` 옵션 C).
>
