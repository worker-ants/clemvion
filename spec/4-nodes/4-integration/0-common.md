...
code:
  ...
  - codebase/frontend/src/components/editor/canvas/integration-list-context.ts
---
...
> **⚠ Missing integration 배지 (렌더러 전용 cross-entity 표식)**: Integration 노드(`category: integration`)의 `config.integrationId` 가 현재 워크스페이스 integration 목록에 **없으면**(= 참조하던 Integration 이 삭제됨) 캔버스 노드 헤더에 앰버 `⚠ Missing integration` 배지를 표시한다.
>
> 이 판정은 schema `warningRules` 로 표현할 수 없다 — `warningRule` 의 `when` DSL 평가기(`node-summary` `evaluateWhen(expr, config)`)는 노드 **자신의 config 객체만** 인자로 받아 cross-entity(integration 실재 여부) 검증에 닿지 못한다. 기존 통합 노드 규칙의 `!integrationId` 류는 "필드 미선택"만 검사할 뿐 삭제 여부는 못 본다. 따라서 배지는 warningRule 이 **아니라 캔버스 렌더러 전용 판정**이다: 캔버스(`workflow-canvas.tsx`)가 워크스페이스 integration 목록을 React Query 로 **한 번** 조회해(캔버스 전용 `["integrations","list"]` 키 — 설정 패널 셀렉터의 `serviceType` 필터 포함 키 `["integrations","list",{serviceTypes}]` 와 달리 전체 목록을 무필터로 받는다) 실재 id 집합을 Context 로 내려주고, 각 노드 렌더러(`custom-node.tsx`)가 자기 `config.integrationId` 의 실재 여부를 그 집합과 대조한다. 목록 조회를 canvas 단일 지점으로 승격한 것은 per-node `useQuery` 구독(N개 노드 = N개 구독·리렌더)을 피하기 위함으로, AI 노드의 `hasDefaultLlmConfig` Context 와 동일한 패턴이다. 실재 여부 판정 로직 자체(`Set.has(id)`)는 설정 패널의 `integration-selector`·`mcp-server-selector` 가 이미 쓰는 missing 판정과 **동일**하나, 그 셀렉터들은 패널이 열린 노드 1개에만 마운트되는 반면 본 배지는 캔버스의 모든 integration 노드에 동시 표시되므로 카디널리티가 달라 Context 승격이 정당하다. 목록이 아직 확정되지 않았으면(로딩 중이거나 페이지네이션 한도로 잘려 전체를 확보하지 못함) 배지를 억제해 위양성을 피하며, `integrationId` 가 비어 있으면(미선택) 이 배지가 아니라 각 노드 schema 의 `warningRules`(필드 미선택 경고)가 담당한다. `http_request` 는 `integrationId` 가 `authentication === "integration"` 일 때만 유효한 조건부 필드이므로 그 조건이 아닌 잔존 값에는 배지를 걸지 않는다(나머지 통합 노드는 무조건 필수). 백엔드·`warningRules` DSL 무변경 (cross-entity 경고가 향후 다수 등장하면 DSL lookup 일반화를 재검토 — plan `spec-sync-integration-common-gaps` 옵션 C).
>
> **발동 조건 (방어적 표식)**: 정상 UI 삭제 경로는 사용처가 있는 Integration 의 삭제를 차단하므로([2-navigation/4-integration](../../2-navigation/4-integration.md) §7.2 — 409 `INTEGRATION_IN_USE`), "라이브 캔버스 노드가 참조하는데 Integration 이 사라진" 상태는 일상적으로 발생하지 않는다. 삭제 차단의 사용처 조회(`queryUsageNodes`)는 라이브 `Node` 테이블만 스캔하고 과거 `WorkflowVersion` 스냅샷·워크스페이스 이동·레거시/직접 DB 조작은 범위 밖이므로, 본 배지는 그 가드를 우회해 남은 **잔존 참조(dangling reference)를 위한 방어적 표시**다. 시각적으로는 앰버를 쓰되(캔버스 `⚠ Missing workflow` 배지와 동일 계열) graph-warning 배지(`AlertTriangle`)와 구분되는 **연결 끊김 아이콘**(`Unplug`)·별도 슬롯으로 렌더해 헤더에서 두 신호가 겹쳐 보이지 않게 한다.
...
| [makeshop](./5-makeshop.md#5-출력-구조) | §5.1 (`success`) | §5.3 (`error`) — API 호출 실패 + Resource/Operation 검증 + shop_uid 누락·형식 위반 + integration resolve 실패 모두 통합 |
