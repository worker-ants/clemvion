...
> **D4 결정**: 위 코드들이 발생하는 모든 경우는 핸들러 내부에서 catch 되어 `port: 'error'` + `output.error.{code, message, details?}` envelope 로 라우팅된다. Integration 노드 (HTTP / Database Query / Send Email / Cafe24, + MakeShop Planned) 모두 send-email 의 catch 패턴으로 통일한다. **`IntegrationError` 가 throw → 노드 실행 실패** 경로는 존재하지 않으며, 모든 `IntegrationError.code` 는 `output.error.code` 로 surface 된다. Usage 로그 (`status: 'failed'` + `error: {code, message}`) 는 동일하게 기록한다.
...
| MakeShop | `{{resource}} · {{operation}}` (라인 40자 초과 시 잘림) | `product · get-product` | **미구현 (Planned)** — [§5-makeshop.md](./5-makeshop.md) |
| Database Query | `{queryType} · {쿼리 첫 줄}` (잘림) | `SELECT · SELECT * FROM us…` | **미구현 (Planned)** — `database-query.schema.ts` 에 `summaryTemplate` 없음 |
| Send Email | `to: {수신자}`. 수신자 2명 초과 시 `+N` 표시 | `to: user@exam…, +2` | **미구현 (Planned)** — `send-email.schema.ts` 에 `summaryTemplate` 없음 |
...
Integration 노드(구현 4종 + MakeShop Planned)는 단일 에러 경로 (`port: 'error'` + `output.error.*`) 만 사용한다.
...
| [makeshop](./5-makeshop.md#5-출력-구조) (Planned) | §5.1 (`success`) | §5.3 (`error`) — API 호출 실패 + Resource/Operation 검증 + shop_uid 누락 + integration resolve 실패 모두 통합 |
