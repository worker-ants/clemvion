...
| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| INT-US-01 | 연동을 사용 중인 워크플로우·노드를 추적. 두 참조 경로의 **합집합** — (a) 노드 `config.integrationId` 직접 참조, (b) AI Agent 노드 `config.mcpServers[].integrationId` MCP 참조. 활성/비활성 무관. 노드별로 참조 종류(`usageKind`: `direct` / `mcp`)를 구분해 제공 | 필수 |
...
| makeshop ([§5-makeshop.md](./5-makeshop.md)) | catalog key (`makeshop.<resource>.<operation>`) | operation 의 HTTP method (`GET`/`POST`) | operation 의 path template (placeholder 그대로 — `product/{product_id}`) |
...
1. **노드 핸들러 경로** — cafe24 / makeshop / http-request / database-query / send-email 노드가 `IntegrationHandlerBase.logUsage` (`api` 인자 전달) 를 경유. base class 가 `api` 를 `IntegrationsService.logUsage` 로 forward.
2. **AI Agent Internal Bridge 경로** — AI Agent 노드에 연결된 통합을 MCP tool 로 노출하는 provider (현재 cafe24: `Cafe24McpToolProvider`, makeshop: `MakeshopMcpToolProvider`) 가 노드 base class 를 거치지 않고 `IntegrationsService.logUsage` 를 **직접 호출**한다. cafe24·makeshop Internal Bridge 는 노드 핸들러와 **동일한 catalog key 형식** (`<service>.<resource>.<operation>`) 으로 `api.label` 을 채운다.
...
| INT-SV-09 | MakeShop — 한국 이커머스 SaaS 의 Shop API (상점설정·상품·주문·회원·혜택·게시판 7 섹션, 161 REST operation). Cafe24 와 동일하게 같은 Integration 이 워크플로 노드와 AI Agent MCP 도구 양쪽에서 활용. OAuth 2.1 Authorization-Code + refresh 인증 ([Spec MakeShop 노드](./5-makeshop.md)). **구현 상태: 구현 완료** | 필수 |
...
| KB-VE-02 | 임베딩 모델 선택 가능 (Config Models 설정 (Chat/Embedding/Rerank) 연동) | 필수 |
