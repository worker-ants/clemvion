...
- 메타데이터 **형식** 은 [`spec/conventions/cafe24-api-metadata.md`](../../conventions/cafe24-api-metadata.md) 에 정의되어 있고, 실제 row 는 backend metadata 모듈(`backend/src/nodes/integration/cafe24/metadata/*.ts`) 에 저장된다.
...
> **잔여 위험 (2026-05-16 명시):** 위 ±5분 윈도우는 **유효 HMAC 을 가진 동일 timestamp 의 재전송** 까지는 차단하지 못한다. 공격자가 valid (mall_id, timestamp, hmac, install_token) 튜플을 5분 내에 가로채면 동일 윈도우 안에서 OAuth state 행을 중복 생성할 수 있음. 영향은 한정적 — install_token 자체가 capability token 이므로 가로채려면 이미 통합 owner 수준의 권한이 필요하고, 결과는 추가 state 행 생성 (DoS 보다는 noise). 완전 차단을 위해서는 `timestamp:hmac` 의 Redis nonce cache (10분 TTL) 가 필요하나 본 보안 묶음 (2026-05-16) PR 범위 밖 — Redis 의존성 주입과 운영 비용 평가가 필요. backlog (`cafe24-followup-backlog.md` B-1-3) 에서 별 plan 으로 추적.
