...
**Internal MCP Bridge** — 같은 Integration 1개, in-process bridge. AI Agent spec/handler 변경 0. `IMcpClient` 인터페이스의 backend 구현 경로는 `codebase/backend/src/integrations/mcp/imcp-client.ts` (interface) + `Cafe24McpBridge` 구현체 (메타데이터 → tools/list). 별도 외부 MCP 서버 + Integration 2번 등록 방식은 credential 중복·운영 부담이 크다.
