...
code:
  ...
  - codebase/frontend/src/components/integrations/cafe24-allowlist-editor.tsx
  - codebase/frontend/src/components/integrations/approval-required-badge.tsx
---
...
3. **Integration 자격증명 해석**: `IntegrationsService.getForExecution(integrationId, workspaceId)` → `serviceType='cafe24'` 검증, `status='connected'` 검증. 실패 시 `INTEGRATION_TYPE_MISMATCH` / `INTEGRATION_NOT_CONNECTED` 코드로 §5.3 라우팅 ([공통 §4.2](./0-common.md#42-공통-에러-코드), D4). integrationId 미존재/타 워크스페이스 소속은 공통 §4.2 의 `NotFoundException` 흡수 동작을 그대로 따른다 (별도 `INTEGRATION_NOT_FOUND` 코드 없음 — `INTEGRATION_CALL_FAILED` 로 surface).
...
11. **Usage 로깅** ([공통 §4 의 6단계 Usage 로깅](./0-common.md#4-handler-실행-세멘틱)): 성공·실패 무관 1건. `error.code` 는 §6 의 vocabulary. **활동 로그 API 식별 정보** ([`_product-overview.md` INT-US-05](./_product-overview.md#24-사용처-추적-및-라이프사이클)):
    - `api_label` = catalog key `cafe24.<resource>.<operation>` — 예: `cafe24.product.product_list`. cafe24·makeshop 이 catalog key 를 가진다 (HTTP/DB/Email 통합은 NULL). frontend 가 `GET /api/integrations/services/cafe24/catalog` ([통합 §9.3](../../2-navigation/4-integration.md#93-사용처활동)) 응답의 `labelKey` 와 i18n dict 를 결합해 사람 친화 라벨로 렌더한다 — DB 는 언어 정보 없는 catalog key 만 저장 ([`cafe24-api-metadata.md`](../../conventions/cafe24-api-metadata.md))
...
| `output.error.message` | string | handler return | `Cafe24 API returned <status>` (statusText 미포함 — 서버 측 사유는 `details.cafe24ErrorCode`/`cafe24Message` 로 보존) |
...
- **`execute()` 안의 모든 IntegrationError**: §5.3 (`port: 'error'` + `output.error.*`) 으로 라우팅된다. 다음 코드들이 해당:
  ...
  - `INTEGRATION_TYPE_MISMATCH` / `INTEGRATION_NOT_CONNECTED` / `INTEGRATION_INCOMPLETE` ([공통 §4.2](./0-common.md#42-공통-에러-코드) — integrationId 미존재는 `NotFoundException` 흡수로 `INTEGRATION_CALL_FAILED` surface, 별도 `INTEGRATION_NOT_FOUND` 코드 없음)
...
[공통 §5](./0-common.md#5-캔버스-요약) — `Cafe24` 행 인용. 요약 포맷: `{resource} · {operation}` (예: `product · product_list`). 연결된 Integration 이 삭제된 경우 캔버스 렌더러가 `⚠ Missing integration` (앰버색) 배지를 표시한다 ([공통 §5](./0-common.md#5-캔버스-요약) — 렌더러 전용 cross-entity 표식).
...
| `resource='customer'`, `operation='customer_delete'` | `mcp_<int8자>__customer_delete` |
