...
code:
  ...
  - codebase/backend/src/common/redis/integration-cache-bus.service.ts
---
...
0. **Abort 사전 체크 + in-flight 취소**: execute 진입 시 `context.abortSignal?.aborted` 이면 즉시 `AbortError` (`err.name='AbortError'`) 를 throw 한다 — cancel-others-on-fail 등으로 이미 취소된 뒤 dispatch 된 케이스. **진행 중(in-flight) abort** 는 driver-level 취소로 처리한다: abort 시 **별도 pool 연결**로 PG `SELECT pg_cancel_backend(<pid>)` / MySQL `KILL QUERY <threadId>` 를 발행해 진행 중 쿼리만 끊는다(연결 유지). 취소로 인한 driver 에러(PG `57014`/MySQL `ER_QUERY_INTERRUPTED`)는 catch 에서 `AbortError` 로 재throw 해 `cancelled` 로 분류한다([node-cancellation §2.1·§5](../../conventions/node-cancellation.md)). best-effort — 취소 권한·타이밍에 의존하며 실패해도 무해, 정상 완료 시 리스너 해제.
...
2. **풀 캐시**: integrationId + credential SHA-256 해시를 키로 풀을 재사용한다 (`POOL_MAX_CONNECTIONS=5`, `POOL_IDLE_TIMEOUT_MS=30000`). credential 회전 시 stale 풀을 evict 후 새 풀 생성.
   - **멀티 인스턴스 무효화 (Redis pub/sub)**: 풀 캐시는 인스턴스-로컬 in-memory 다. 자격증명이 바뀌거나(`IntegrationsService.rotate`) integration 이 삭제(`remove`)되면 그 작업을 수행한 인스턴스는 자기 캐시만 갱신할 수 있어, **타 인스턴스의 풀에는 회전된(stale) 자격증명으로 맺은 idle 연결이 잔존**한다 — 침해 대응(회전된 자격증명 즉시 차단) 맥락에서 MTTR 갭. 이를 닫기 위해 `IntegrationsService` 는 해당 integrationId 를 Redis pub/sub 채널 `integration:cache:invalidate` 로 broadcast 하고, 전 인스턴스의 구독자가 그 풀을 즉시 evict 한다 (다음 쿼리는 새 자격증명으로 풀 재생성). **트리거는 `rotate`·`remove` 두 동기 경로뿐**이다 — `update` 는 name 만 바꾸고(자격증명 불변), OAuth 토큰 갱신·`reauthorize` 는 DB/email 같은 풀-캐시 소비자가 OAuth 자격증명을 쓰지 않아(구독자 부재) broadcast 대상이 아니다. **fail-safe**: pub/sub 가 미가용(Redis 순단)이어도 위 credsHash 비교 evict 가 다음 실행에서 stale 풀을 교체하므로 안전 degrade — broadcast 는 즉시성(MTTR)을 보강하는 best-effort 계층이다. 채널 payload 는 integrationId 평문 문자열이며, 채널은 integration-generic 이라 같은 메커니즘에 다른 인스턴스-로컬 자격증명 캐시(예: Send Email transport)도 구독 등록할 수 있다.
...
6. **MySQL 실행**: `pool.getConnection()` → `connection.query(sql, params)` → `connection.release()` — 결과가 배열이면 SELECT 계열 (`rows` / `fields`), `ResultSetHeader` 면 INSERT/UPDATE/DELETE 계열 (`rowCount = affectedRows`, `insertId`). (명시 connection 획득 사유: in-flight 취소용 `threadId` 확보 — step 0.)
7. **SSL 매핑**:
...
> **Dry-run (재실행)** — 노드는 dry-run 을 지원한다 (`metadata.supportsDryRun=true`, [재실행 §7.1](../../5-system/13-replay-rerun.md#71-외부-부수효과-노드-분류)). config 검증·echo 직후, DB 연결을 열기 전에 **WRITE 작업만** mock 으로 단락한다: `queryType` 이 `insert`/`update`/`delete` 이거나 (`raw`/누락 시) SQL 첫 토큰이 write 동사 화이트리스트(`INSERT`/`UPDATE`/`DELETE`/`UPSERT`/`MERGE`/`REPLACE`/`TRUNCATE`/`DROP`/`CREATE`/`ALTER`/`GRANT`/`REVOKE`) 면 write 로 간주. 모호하면 read 로 떨어뜨려 실제 실행한다(재현성 우선). **READ(SELECT) 는 dry-run 에서도 실제로 실행**한다 (부수효과 없음). mock 경로는 pool/connect 를 절대 호출하지 않고 `port: 'success'` + `buildDryRunMock('database_query', { operation, sqlPreview })` (`sqlPreview` = SQL 앞 ~200자, 바인딩 값 미포함) 로 흐른다.

> **SSRF 가드** — 자격증명 충족 검증 직후, 풀 연결 전에 `credentials.host` 를 검사한다 (`assertSafeOutboundHostResolved`). 사설(RFC1918)·loopback·link-local·CGNAT·IPv6 사설 대역으로 해석되면 차단하고 `port: 'error'` 로 라우팅한다. [HTTP Request §4](./1-http-request.md#4-실행-로직)·Send Email 과 **동일 메커니즘·플래그** — 기본 차단, self-host(예: VPC 안 RDS·내부 DB)는 `ALLOW_PRIVATE_HOST_TARGETS=true` 로 opt-out. 차단 시 전용 코드 `DB_HOST_BLOCKED` 로 `port: 'error'` 라우팅한다 (HTTP 의 `HTTP_BLOCKED`·Email 의 `EMAIL_HOST_BLOCKED` 와 대칭). 클라이언트 노출 메시지는 차단 host/IP 를 포함하지 않는 일반화 문구이며, 차단 상세는 활동 로그(`logUsage`)에만 남는다.
...
| `output.error.code` | enum | handler return | `DB_QUERY_FAILED` / `DB_CONNECTION_ERROR` / `DB_CONSTRAINT_VIOLATION` / `DB_PERMISSION_DENIED` / `DB_HOST_BLOCKED` (SSRF 차단) 중 하나, 또는 [공통 §4.2](./0-common.md#42-공통-에러-코드) 의 `IntegrationError` 코드 (e.g. `INTEGRATION_NOT_CONNECTED`). UPPER_SNAKE_CASE |
...
- **`execute()` 안의 모든 IntegrationError / 파라미터 파싱 실패**: §5.3 (`port: 'error'` + `output.error.*`) 으로 라우팅된다. 다음 코드들이 해당:
  ...
  - `INTEGRATION_TYPE_MISMATCH` / `INTEGRATION_NOT_CONNECTED` / `INTEGRATION_INCOMPLETE` ([공통 §4.2](./0-common.md#42-공통-에러-코드)). integrationId 부재/타 워크스페이스 소속은 `requireEntity` `RESOURCE_NOT_FOUND` fallback 으로 `INTEGRATION_CALL_FAILED` 로 surface (별도 `INTEGRATION_NOT_FOUND` 코드 없음 — 공통 §4.2)
...
| `DB_HOST_BLOCKED` | SSRF 가드가 `credentials.host` 를 사설/loopback/link-local/CGNAT/IPv6 사설 대역으로 해석해 차단 (풀 연결 전, §4 SSRF 가드). 기본 ON, `ALLOW_PRIVATE_HOST_TARGETS=true` 로 opt-out. 클라이언트 메시지는 host/IP 미포함 일반화 문구 | — (가드 차단 — 드라이버 미관여) |
| `INTEGRATION_*` ([공통 §4.2](./0-common.md#42-공통-에러-코드)) | D4 — Integration resolve / 자격증명 누락 실패. `INTEGRATION_TYPE_MISMATCH` / `INTEGRATION_NOT_CONNECTED` / `INTEGRATION_INCOMPLETE` / `INTEGRATION_CALL_FAILED`(integrationId 부재 — `requireEntity` `RESOURCE_NOT_FOUND` fallback) 모두 본 경로로 surface. **SSRF 가드가 차단 판정(`SsrfBlockedError`)이 아닌 오류를 던진 경우(가드 자체의 고장)도 `INTEGRATION_CALL_FAILED` 로 surface** — `DB_HOST_BLOCKED` 는 차단 **판정** 에만 쓴다([공통 §4.2](./0-common.md#42-공통-에러-코드)). DB 노드에는 리다이렉트가 없어 HTTP 와 달리 시점 구분이 없다 | — |
...
[공통 §5](./0-common.md#5-캔버스-요약) — `Database Query` 행 인용. `summaryTemplate: {{queryType|upper}} · {{query}}` (라인 40자 초과 시 잘림). 전체 query 를 truncate 표시하며("첫 줄" 은 DSL 줄분리 미지원), 연결된 Integration 이 삭제된 경우 캔버스 렌더러가 `⚠ Missing integration` (앰버) 배지를 표시한다 ([공통 §5](./0-common.md#5-캔버스-요약) — 렌더러 전용 cross-entity 표식).
...
credential 회전 시 다음 쿼리가 credsHash 불일치를 감지해 stale 풀을 evict·재생성하므로 충분하다.
그러나 멀티 인스턴스 배포에서는 회전을 수행한 인스턴스가 **자기 캐시만** 갱신할 수 있어, 타
인스턴스의 풀에는 회전된 자격증명으로 맺은 idle 연결이 `POOL_IDLE_TIMEOUT_MS`(30s) 또는 다음 쿼리
시점까지 잔존한다. 위협 맥락이 **침해 대응**(유출된 자격증명을 회전으로 즉시 무력화)이라면 이 잔존
창은 MTTR 갭이다.

**채택 — Redis pub/sub broadcast (옵션 A)**: `IntegrationsService` 가 자격증명이 바뀐 integration
의 id 를 채널 `integration:cache:invalidate` 로 publish 하고, 전 인스턴스 구독자가 해당 풀을 즉시
evict 한다. 회전 즉시 전 인스턴스에서 stale 연결이 제거돼 MTTR 을 최소화한다.

- **fail-safe**: pub/sub 는 즉시성 보강 계층일 뿐 — Redis 순단으로 메시지가 유실돼도 credsHash
  비교 evict 가 다음 실행에서 stale 풀을 교체하므로 정합성은 깨지지 않는다 (안전 degrade). 따라서
  broadcast 전송 실패는 경고 로그만 남기고 삼킨다.
- **기각 — 옵션 B (`POOL_IDLE_TIMEOUT_MS` 하향)**: 시간 기반 완화일 뿐 즉시성이 없고, 하향할수록
  연결 churn 이 늘어 풀 캐시의 존재 이유(연결 재사용)와 직접 상충한다. 활성 사용 중 연결은 idle
  타이머에 걸리지도 않는다.
- **채널 generic**: payload 는 integrationId 평문 문자열이며, 채널은 노드 비종속이라 다른
  인스턴스-로컬 자격증명 캐시(예: Send Email transport)도 같은 메커니즘에 구독 등록할 수 있다.
