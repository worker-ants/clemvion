...
> Database Query 는 동적 포트가 없다. D4 (2026-05-17) 이후 자격증명 누락·`INVALID_PARAMETERS`·integration resolve 실패 등 모든 실패 경로가 `error` 포트로 흐른다 (§5.3).
...
D4 결정 이전에 본 절은 `IntegrationError` throw → 노드 실행 실패 경로를 정의했었다. 현재는 다음 두 경로로 분리된다:
...
> D4 이전의 "`IntegrationError` 가 throw 되면 노드 실패" 동작은 폐기 — 모든 `IntegrationError.code` 가 `output.error.code` 로 surface 된다. Usage 로그 (`status: 'failed'` + `error: {code, message}`) 는 양쪽 모두에서 동일하게 기록.
...
| `INTEGRATION_*` ([공통 §4.2](./0-common.md#42-공통-에러-코드)) | D4 (2026-05-17) — Integration resolve / 자격증명 누락 실패. `INTEGRATION_NOT_FOUND` / `INTEGRATION_TYPE_MISMATCH` / `INTEGRATION_NOT_CONNECTED` / `INTEGRATION_INCOMPLETE` 모두 본 경로로 surface | — |
| `INTEGRATION_SERVICE_UNAVAILABLE` | D4 — IntegrationsService 가 핸들러에 주입되지 않음 (deployment 누락). 종전 throw → 노드 실패 였으나 D4 이후 본 경로 | — |
| `INVALID_PARAMETERS` | D4 — `config.parameters` JSON parse 실패 / 배열 아닌 형태. 종전 throw 였으나 D4 이후 본 경로 | — |
