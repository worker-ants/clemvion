...
| `DB_CONNECTION_ERROR` | 실행 중 커넥션 drop / 타임아웃 / 인증 거부 | `ECONNRESET`, `ECONNREFUSED`, `ETIMEDOUT`, `ENOTFOUND`, mysql `PROTOCOL_CONNECTION_LOST` / `ER_ACCESS_DENIED_ERROR` |
...
| `DB_PERMISSION_DENIED` | 권한 부족 | pg `42501`, mysql `ER_TABLEACCESS_DENIED_ERROR` / `ER_COLUMNACCESS_DENIED_ERROR` / `ER_DBACCESS_DENIED_ERROR` / `ER_SPECIFIC_ACCESS_DENIED_ERROR` |
