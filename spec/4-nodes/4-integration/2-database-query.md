...
2. **풀 캐시**: integrationId + credential SHA-256 해시를 키로 풀을 재사용한다 (`POOL_MAX_CONNECTIONS=5`, `POOL_IDLE_TIMEOUT_MS=30000`). credential 회전 시 stale 풀을 evict 후 새 풀 생성.
   - **멀티 인스턴스 무효화 (Redis pub/sub)**: 풀 캐시는 인스턴스-로컬 in-memory 다. credential 회전(`IntegrationsService` 의 rotate/update/remove/reauthorize/OAuth 토큰 갱신)이 일어난 인스턴스는 자기 캐시만 갱신할 수 있어, **타 인스턴스의 풀에는 회전된(stale) 자격증명으로 맺은 idle 연결이 잔존**한다 — 침해 대응(회전된 자격증명 즉시 차단) 맥락에서 MTTR 갭. 이를 닫기 위해 `IntegrationsService` 는 자격증명이 바뀐 integration 의 id 를 Redis pub/sub 채널 `integration:cache:invalidate` 로 broadcast 하고, 전 인스턴스의 구독자가 해당 integrationId 풀을 즉시 evict 한다 (다음 쿼리는 새 자격증명으로 풀 재생성). **fail-safe**: pub/sub 가 미가용(Redis 순단)이어도 위 credsHash 비교 evict 가 다음 실행에서 stale 풀을 교체하므로 안전 degrade — broadcast 는 즉시성(MTTR)을 보강하는 best-effort 계층이다. 채널 payload 는 integrationId 평문 문자열이며, 채널은 integration-generic 이라 같은 메커니즘에 다른 인스턴스-로컬 자격증명 캐시(예: Send Email transport)도 구독 등록할 수 있다.
