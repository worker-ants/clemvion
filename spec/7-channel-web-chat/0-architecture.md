...
- ⚠️ `srcdoc`/`about:blank` 자가 생성은 **기각** — 그 iframe 은 호스트 origin 을 상속해 cross-origin 격리가 깨진다.
  격리를 위해 iframe 은 **반드시 다른 origin 의 실제 `src`** 여야 한다(§R5). *예외*: admin 콘솔 **내부 미리보기**는 cross-origin
  격리가 목적이 아니므로 same-origin 동봉 위젯을 실제 `src` iframe 으로 로드한다(§4.1·§R5 carve-out) — `srcdoc` 자가 생성은 여기서도 금지.
...
- admin 콘솔 라이브 미리보기는 이 동봉 위젯을 same-origin iframe 으로 로드 — 버전 일치·외부 의존 0 (격리 carve-out 은 §R5).
...
`boot()` 가 iframe 하나만 host body 에 주입하고 위젯 SPA(정적 CDN 문서, §2.1)를 iframe 에 로드 — 런처(collapsed)와 패널은 모두 iframe 내부 SPA 가 렌더(§R4 단일 iframe 크기 토글). EIA webhook/SSE/REST 호출은
...
### R2. 클라이언트 consumer 로 한정 — EIA·신규 트리거 유형·facade 미신설
...
### R5. iframe 문서 — 정적 cross-origin CDN 자산 + soft 임베드 검증
