...
<script>
  (function(d,s){
   // 큐 스텁: loader.js(async) 로드 전 ClemvionChat 호출을 .q 에 버퍼링 → 로더가 replay(§1·R5).
...
> **`locale` 은 위젯 UI 렌더 언어 선택자**: `locale` 은 boot·query·운영 콘솔([5-admin-console §4·§6.1](./5-admin-console.md))을
> 거쳐 위젯에 전달되고, 위젯이 **boot 시 1회 해석**해 chrome 문자열(위젯 로컬 catalog)을 그 언어로 렌더한다. 해석 우선순위는
> **명시 `locale` → 브라우저 `navigator.language`(auto-detect) → `ko` fallback**(SoT [1-widget-app §4](./1-widget-app.md)). 미지정/미지원은
> `ko`(하위호환). **번역 범위 = 위젯 소유 chrome 한정** — 운영자 제공 콘텐츠(`headerTitle`·`welcome`·`disclaimer` 등)·AI 본문은
> 입력 언어 그대로 렌더한다(위젯 콘텐츠 현지화는 [_product-overview §2 비목표](./_product-overview.md#2-목표--비목표)). 이 `locale` 은
> 위젯 UI 언어일 뿐, Chat Channel 의 `languageLocale`(서버 발신 메시지 언어, [15-chat-channel §4.1](../5-system/15-chat-channel.md))과는 별개다.
...
### R5. 스니펫의 command-queue 스텁은 필수 전제 (생략 금지)
§1 스니펫의 첫 블록은 `loader.js` 를 **async** 로 붙이므로, 같은 페이지의 동기 `ClemvionChat('boot', …)` 호출이
로더보다 **먼저** 실행될 수 있다. 따라서 호출을 `.q` 에 버퍼링하는 **큐 스텁**
(`window.ClemvionChat=window.ClemvionChat||function(){(…q…).push(arguments)}`)을 boot 호출 **전에 동기 설치**해야
하며, 로더(`installGlobal`)가 로드되면 큐를 replay 한다(§1 명령 큐 패턴). 스텁을 생략하면
`Uncaught ReferenceError: ClemvionChat is not defined` 가 발생한다 — §1 예시·설치 스니펫 생성기·유저 가이드는
모두 이 스텁을 포함해야 한다(2026-06-25: 세 경로 모두 스텁 누락 drift 를 수정·복원).

### R6. `locale` 활성화 — 예약된 경로 실행 (구 reserved)
`BootConfig.locale` 은 v1 에서 **reserved/inert**(query·`wc:boot`·config state 로 실려 나르지만 렌더 경로 미소비, Korean-only)로
두되, 운영 콘솔 폼 필드이자 공개 SDK 계약이라 **삭제하지 않고** "EN 위젯 지원 착수 시 UI 언어 선택자로 활성화" 를 명시 예약했다.
그 활성화를 **2026-07-12 결정으로 실행**한다([webchat-i18n-scope](../../plan/complete/webchat-i18n-scope.md) 가 defer 한 옵션 (c)):
위젯이 `locale` 을 **boot 시 1회 해석**(명시 → 브라우저 auto-detect → `ko`)해 chrome 문자열을 위젯 로컬 catalog(ko/en)로 렌더한다
(§4 · [1-widget-app §4](./1-widget-app.md)). 이는 **예약된 경로의 실행이지 결정 번복이 아니다** — v1 "reserved" 는 애초에 영구
결정이 아닌 **스코프 경계**로 프레이밍됐다. 필드를 삭제하지 않은 판단(운영자 대면·공개 계약)은 이제 "이미 활성 계약" 으로 자연히
흡수된다. 번역 범위는 **위젯 소유 chrome 한정**(운영자 콘텐츠·AI 본문 제외 — [_product-overview §2](./_product-overview.md#2-목표--비목표)).
