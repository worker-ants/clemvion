...
- **서버측 execution 회수는 별도 backstop**: 위 client 토큰 만료는 클라이언트 잔존만 정리한다 — 이탈로 방치된
  **서버측 `waiting_for_input` execution row 자체**는 공개 위젯 idle-wait backstop([EIA §3.4 EIA-RL-07](../5-system/14-external-interaction-api.md#34-신뢰성일관성))이 발급 토큰 영구 만료(`execution_token.exp_at`) + grace 후 `cancelled` 로 회수한다(토큰만 죽고 execution 은 잔존하던 갭 해소, [1-widget-app §R9](./1-widget-app.md)).
