...
| `apiBase` 입력 검증 | **두 입력 경로 모두** `apiBase` 를 **http(s) 스킴만 허용**(`safeApiBase`)해 `javascript:`/`data:`/상대경로를 fetch base 로 쓰지 않도록 거른다(부적합 시 그 필드만 무시 + `console.warn` — 부팅 자체는 막지 않는다). 경로는 둘이다: **host 없는 직접 로드/샘플**의 `?apiBase=` 쿼리 폴백, 그리고 **정상 임베드**의 host postMessage(`wc:boot`). 코드 SoT: `use-widget.ts` 의 `safeApiBase`/`configFromQuery`/`mergeBootConfig` |
...
- **① 클라이언트 soft 검증(v1 기본)**: 부팅 시 위젯이 **`GET /api/hooks/:endpointPath/embed-config`** 로 워크스페이스
  allowlist 를 조회(`EmbedConfigDto { allowlist, enforce }`, `EmbedConfigService`. 성공 응답은 전역
  `TransformInterceptor` 가 `{ data }` 로 래핑하므로 wire body 는 `{ data: { allowlist, enforce } }` —
  [swagger §2-5](../conventions/swagger.md#2-5-응답-wrapping) · [api-convention §5.1](../5-system/2-api-convention.md#51-단일-리소스). 위젯은 `res.data` 언랩)한 뒤, 실제 host origin
  ...
  - **`/embed-config` 엔드포인트 동작(공개·무인증)**: 비존재 endpointPath·DB 오류·인증 webhook(`authConfigId` NOT NULL)
    모두 `{ data: { allowlist: [], enforce: false } }`(HTTP 200)로 동일 응답 → **존재 여부 누설(enumeration)·allowlist 노출 없음**.
...
**v1 기본 적용**
- IP 단위 대화 시작 rate-limit(예: 분당 10/IP). **[구현됨 v1]** `PublicWebhookThrottleGuard` + `PublicWebhookQuotaService`. **클라이언트 IP 식별 가능 시 per-IP 버킷**, 헤더(`X-Forwarded-For`/신뢰 시 `CF-Connecting-IP`) 부재로 **미식별 시 미식별 요청 전체를 단일 공유 버킷에 묶어 동일 fixed-window 한도를 적용**한다 — 과거 무제한 통과(`return true`) 우회를 유한 상한으로 강화한 **완화 한도**(근거 [R6](#r6-공개-webhook-ip-미식별--단일-공유-버킷-완화-한도)). `req.ip`/`req.socket.remoteAddress` 폴백은 채택하지 않는다([1-auth Rationale 2.3.B](../5-system/1-auth.md)).
...
- **EIA 자체 rate-limit 은 [EIA §8.4](../5-system/14-external-interaction-api.md) 가 단일 진실**이며 본 문서는 값·구현 상태를 복제하지 않는다(과거 복제본이 뒤처져 drift 를 만들었다 — §Rationale R3).
...
> **IP 식별성·인프라 권고(결정 1)**: 위 IP 단위 한도의 실효성은 신뢰 프록시(Cloudflare/LB)가 `X-Forwarded-For`(또는
> 신뢰 시 `CF-Connecting-IP`)를 채운다는 전제에 의존한다. 헤더가 없으면 앱은 미식별 요청을 단일 공유 버킷으로 완화
> 처리하지만(R6), **managed 배포는 CF/WAF/Ingress 에서 XFF 를 강제하고 헤더 없는 외부 직접 요청을 차단**해 per-IP
> 정확도를 확보하는 것을 권고한다. self-host 도 앱 레벨 공유 버킷으로 기본 보호되므로 인프라 통제가 없어도 무제한
> 우회는 발생하지 않는다.
...
### R0. `apiBase` 스킴 검증을 **두 경로 모두**에 거는 이유 (2026-08-11)
...
**거절 시 그 필드만 버린다** — 부팅을 막지 않는다. 쿼리 경로의 기존 동작과 대칭이고, `apiBase`
가 결국 없으면 `applyConfig` 가 자기 자리에서 실패해 진단이 그쪽에 모인다. 거절과 **부재**를
가르는 것도 의도다: 부재는 조용히 쿼리 값으로 폴백하고, 거절만 `console.warn` 을 낸다.
...
는 이들에 대해 `{ data: { allowlist: [], enforce: false } }` 로 응답한다(§3-①, WH-SC-01 공개 트리거 전제와 정합) — 존재
여부도 누설하지 않는다.
...
전환이 followup 후보(§4 blockquote). 본 항의 fail-open 은 **인프라 장애(Redis/DB)** 차원이며, **클라이언트 IP
미식별** 시의 처리는 별도 차원으로 [R6](#r6-공개-webhook-ip-미식별--단일-공유-버킷-완화-한도) 가 다룬다(무제한 통과 아님).

**EIA rate-limit 값·구현 상태는 복제하지 않는다(결정 2026-07-11)**: 본 문서 §4 는 한때 EIA §8.4 의 두 제한(SSE 동시
3/execution, interact 분당 60/execution)을 인용하며 후자를 "Planned(미구현)" 으로 적었으나, EIA §8.4 는 이미
"구현됨"(`InteractionRateLimiterService` + `InteractionRateLimitGuard`)이었고 코드도 그렇다. **자신이 SoT 로 인용한
문서와 직접 모순**하는 상태였다 — 복제본이 원본의 상태 flip 을 따라가지 못한 전형적 drift 다. 값을 갱신해 두면 다음
flip 에서 같은 일이 재발하므로, **숫자·구현 상태 서술을 지우고 §8.4 참조만 남긴다**. 본 문서가 SoT 인 것은 공개
webhook 남용 방어(§4 의 IP 단위·body 크기 가드)뿐이다.
...
### R6. 공개 webhook IP 미식별 — 단일 공유 버킷 완화 한도

공개 webhook rate-limit Guard(`PublicWebhookThrottleGuard`)는 클라이언트 IP 로 per-IP 버킷을 건다. IP 를 헤더에서
식별하지 못하는 경우(`X-Forwarded-For`·신뢰 시 `CF-Connecting-IP` 부재 — 공격자가 의도적으로 제거 가능)의 처리를
명문화한다. **무제한 통과(`return true`)에서 단일 공유 버킷 완화 한도로 강화** — 우회를 무제한에서 유한 상한으로
좁힌다 (D-12, 사용자 결정 2026-06-28).

- **채택 — 단일 공유 버킷(완화 한도)**: 미식별 요청 전체를 하나의 sentinel 버킷(`UNIDENTIFIED_IP_BUCKET`, 구현
  `public-webhook-quota.service.ts`)에 묶어 per-IP 와 동일한 fixed-window 한도(분당 10·시간당 20)를 적용한다. 단일
  공유 버킷이라 미식별 트래픽 총량이 한 IP 분량으로 상한되어 그 자체로 보수적이다. 정상 트래픽은 프록시가 XFF 를
  채워 per-IP 로 분류되므로, 공유 버킷에는 비정상(헤더 제거)·오설정 트래픽만 모인다.
- **기각 — fail-open(현행 유지)**: IP 미식별 시 그냥 통과시키면 헤더만 제거하면 rate-limit 이 무제한 우회된다.
  공개 진입점 brute-force 표면이므로 받아들일 수 없다.
- **기각 — fail-closed(거부)**: 미식별 시 거부(예: 429)는 가장 강하나, 프록시 없는 직결·self-host 정상 클라이언트를
  막을 수 있고, **비-인증 best-effort 레이어가 가용성을 희생**하는 것은 본 rate-limit 의 성격(인증 대체 아님, R3)·§4
  graceful degradation 원칙과 충돌한다. 인증·존재성은 `auth_config_id`·`endpointPath` UUID 가 책임진다.
- **기각 — `req.socket.remoteAddress`/`req.ip` 폴백**: 헤더 부재 시 socket 피어로 폴백하면 `trust proxy=1`
  (`main.ts`) 뒤에서 socket 이 cloudflared/LB 주소라 **전 트래픽이 단일 프록시 IP 버킷으로 붕괴** → 정상 사용자
  false 429. 이는 [1-auth Rationale 2.3.B m-3](../5-system/1-auth.md) 가 `ip_whitelist`/rate-limit 에서 `req.ip`
  폴백을 기각한 함정과 동일하므로 채택하지 않고 헤더 기반 추출(`extractClientIpFromHeaders`)을 유지한다.
- **인프라 권고(결정 1)**: 앱 레벨 공유 버킷이 portable 기준선이고, managed 배포는 CF/WAF/Ingress 에서 XFF 강제·
  헤더 없는 외부 요청 차단으로 per-IP 정확도를 보강할 것을 권고한다(§4 blockquote).

> **인증 webhook 의 `ip_whitelist` 와의 대비**: 같은 헤더 기반 추출을 쓰지만 IP 미식별 시 동작이 다르다 — 공개
