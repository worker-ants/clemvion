...
- **priority** — `manual(1) > webhook(2) > schedule(3)` (`EXECUTION_RUN_PRIORITY`). 호출부(webhook/chat-channel/schedule)가 `ExecuteOptions.triggerType`(`Trigger.type`)을 threading 하고, `execute()` 가 `executedBy` 우선 판정한다(`executedBy` 존재 시 `manual` — schedule "지금 실행" runNow 포함; 미전달 트리거는 `webhook` fallback). triggerType threading 구현 완료(2026-07-04).
...
| `execution` | 상태 전이 | UPDATE `status, finished_at, duration_ms, output_data, error, active_running_ms` — active_running_ms 는 RUNNING 이탈마다 적산 (PR2a §8). error 는 최초 failed NodeExecution.error + nodeId 복사. **`status IN (비-terminal)` guarded UPDATE 이며 트랜잭션 안에서 실행**된다 (2026-08-30) — 결과 shape 가드가 throw 하면 이 UPDATE 도 함께 롤백된다 ([실행 엔진 §1.1 원자성 보장](../5-system/4-execution-engine.md#11-execution-상태), 결정 배경은 같은 문서 [`## Rationale` 의 "else 분기 원자화"](../5-system/4-execution-engine.md#rationale)) | — |
...
| `node_execution` | 노드 실행 시작 | INSERT `execution_id, node_id, status='running', started_at, input_data, retry_count=0, parent_node_execution_id?` (V006/V012) — skipped 는 status='skipped' 로 직접 INSERT | `(execution_id)`, V034 `(execution_id, node_id, started_at DESC)` composite, V095 `(execution_id, status) WHERE status IN ('waiting_for_input','running')` partial (활성 노드 조회/전이) · V112 `(node_id)` (FK CASCADE — 캔버스 노드 삭제 · 워크플로 삭제) |
...
stateDiagram-v2
  ...
  pending --> cancelled: (a) 큐 대기 중 사용자 cancel(runExecutionFromQueue ack-discard) (b) 큐 대기 5분 초과 EXECUTION_QUEUE_WAIT_TIMEOUT — admission 시점 검사(§8) 또는 orphan pending backstop(recoverStuckExecutions, job 소실 시)
