...
코드 위치: `codebase/backend/src/modules/knowledge-base/knowledge-base.service.ts:723, 756`, `codebase/backend/src/modules/knowledge-base/embedding/embedding.service.ts:163`.
...
| KB 삭제 | 소속 document 전체 조회 후 각 key DELETE (best-effort, 실패 시 warn) | DELETE knowledge_base (CASCADE documents) |

> KB 삭제 시 S3 객체 cleanup 은 `remove(id, workspaceId)` 가 소속 document 를 조회해 `s3Service.delete(doc.fileUrl)` 를
> for 루프로 호출하여 수행한다 (`knowledge-base.service.ts:644-658`, 각 삭제는 best-effort — 실패 시 warn 만).
...
삭제는 best-effort 로 둔다 (문서 삭제 `removeDocument` 는 `knowledge-base.service.ts:755-759`, KB 삭제 `remove` 는
`644-658` 에서 동일하게 try/catch warn). warn 으로 스킵돼 누적된 orphan 은 정기 GC batch 로 정리할 계획.
