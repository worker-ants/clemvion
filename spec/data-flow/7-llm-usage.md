...
`embed` 는 동일 facade 를 거치지만 **usage 를 적재하지 않는다** (§1.3, §Rationale).

코드 진입점:

- `codebase/backend/src/modules/llm-config/llm-config.service.ts` — LLMConfig CRUD · default 스왑 · api_key encrypt/decrypt
...
- **리랭크 호출 계통은 별도 경로다**: `rerank_config` (V081) + `RerankClientFactory` (`tei` / `cohere`, `modules/llm/rerank/**`) 는 `LlmService` 를 거치지 않는 전용 `/rerank` 외부 호출이며 usage_log 미적재. 단, `cross_encoder_llm` 모드의 listwise LLM grading 은 `LlmService.chat` 을 쓰므로 usage 가 적재된다 (§1.3). 상세: [Spec LLM Client §3.6·§4.1](../5-system/7-llm-client.md), [데이터 모델 §2.16.1](../1-data-model.md).
...
sequenceDiagram
  ...
  alt is_default=true
    Svc->>PG: TX [ UPDATE 기존 default 해제 → INSERT/UPDATE 새 default ] (saveWithDefaultSwap)
  else
...
- 부속 엔드포인트 (`llm-config.controller.ts`):
  - `POST /api/llm-configs/preview-models` — `LlmPreviewService.previewModels` (미저장 자격증명, SSRF 가드, 30s 타임아웃, 캐시 없음)
  - `POST /api/llm-configs/:id/test` — `LlmService.testConnection`
  - `GET /api/llm-configs/:id/models` — `LlmService.listModels`. config 별 5분 캐시 (key `${workspaceId}|${configId}`), 30s 타임아웃, `?type=chat|embedding` 필터
...
| --- | --- | --- | --- |
| `llm_config` | 생성·갱신 | INSERT/UPDATE `workspace_id, provider, name, api_key (encrypted), base_url?, default_model, default_params, is_default` | V001. **`llm_config_workspace_default_unique` UNIQUE partial index 는 entity `@Index` 선언만 있고 SQL 마이그레이션이 없다** — `synchronize: false` 라 실제 DB 에 미생성, DB 단 강제는 현재 부재. 워크스페이스 당 default 1개 보장은 application 트랜잭션 (`saveWithDefaultSwap` / `setDefault`) 만으로 수행된다 (비교: 같은 패턴의 `rerank_config` 는 V081 이 SQL 로 인덱스를 생성) |
...
| `knowledge_base.embedding_llm_config_id` | KB 임베딩 config 선택 | V029. NULL → ws default 폴백 | FK `ON DELETE SET NULL` |
...
워크스페이스마다 default LLMConfig 가 정확히 1개여야 한다는 의도로 entity 에
`llm_config_workspace_default_unique` (`workspace_id WHERE is_default=true`) 가 선언되어 있으나,
이 인덱스를 생성하는 SQL 마이그레이션이 없다. `synchronize: false` 운용이라 entity 선언은 실제
DB 에 반영되지 않으므로 **DB 단 강제는 현재 부재**하고, application 단 트랜잭션
(`saveWithDefaultSwap` / `setDefault` 의 기존 default unset) 이 유일한 보호다. 같은 의도의
`rerank_config` 는 V081 에서 인덱스를 SQL 로 생성했으므로, llm_config 도 동일한 보완 마이그레이션
추가 여부가 결정 대상이다. (구판 문서의 "DB 단에서 강제" 서술은 코드 검증 결과 사실이 아니어서
폐기 — entity 선언과 실제 스키마의 괴리)
