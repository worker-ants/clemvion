...
> 관련 spec: [Spec 임베딩 파이프라인](../5-system/8-embedding-pipeline.md) · [Spec RAG 검색](../5-system/9-rag-search.md) · [Spec Graph RAG](../5-system/10-graph-rag.md) · [데이터 모델 §2.11~§2.12.4 · §2.16.1](../1-data-model.md) · [data-flow 개요](./0-overview.md)
...
sequenceDiagram
  ...
  C->>Svc: POST /api/knowledge-bases { name, embedding_model, chunk_size, chunk_overlap, rag_mode IN (vector/graph), extraction_llm_config_id?, embedding_llm_config_id?, rerank_mode?, rerank_config_id?, rerank_candidate_k?, rerank_score_threshold?, rerank_llm_config_id? }
...
sequenceDiagram
  ...
  loop batch of 20 chunks
    EP->>LLM: embed(texts[], inputType='document') via LlmService (kb.embedding_llm_config_id 또는 ws default)
...
- vector KB 는 `(embedding_model, embedding_dimension, embedding_llm_config_id)` 그룹 단위로
  묶어 그룹별 1회 query 임베딩 + 병렬 검색한다. 같은 모델명이라도 LLMConfig endpoint 가 다르면
...
sequenceDiagram
  ...
  par vector 그룹별
    R->>LLM: embed(query, inputType='query') — 그룹의 embedding_llm_config_id 또는 ws default
...
sequenceDiagram
  ...
  RS->>XE: rerank(query, contents[]) — RerankConfig(rerank_config_id ?? ws default) endpoint 로 전 후보 재점수화
...
| --- | --- | --- | --- |
| `knowledge_base` | 생성 | `workspace_id, name, embedding_model, embedding_dimension?, chunk_size, chunk_overlap, document_count, reembed_status, rag_mode, extraction_llm_config_id?, embedding_llm_config_id? (V029), max_hops, vector_seed_top_k, expanded_chunk_limit, entity_count, relation_count, reextract_status` + rerank 5컬럼 (V082): `rerank_mode IN (off/cross_encoder/cross_encoder_llm), rerank_config_id? (FK rerank_config SET NULL), rerank_candidate_k (1~200 CHECK), rerank_score_threshold?, rerank_llm_config_id? (FK llm_config SET NULL)` | FK CASCADE on `workspace_id` |
...
| `rerank_config` | 리랭커 설정 (V081) | `workspace_id, provider, name, api_key? (셀프호스팅 tei 는 불요), base_url?, default_model, is_default` | workspace 당 `is_default=TRUE` 최대 1개 (partial unique index). CRUD 는 `/api/rerank-configs` (`rerank-config` 모듈) |
...
| cross-encoder rerank provider | rerank 후보 재점수화 (§1.4) | `RerankClientFactory` — provider `tei`(셀프호스팅, api_key 불요) / `cohere`. endpoint 는 `rerank_config` 행이 정의 |
...
| cross-encoder rerank provider | 외부 | `rerank_config` 행이 정의하는 endpoint (tei / cohere) — §2.4 |
...
### `rerank_config` 를 `llm_config` 와 분리한 이유

cross-encoder 리랭커는 chat/embedding 과 API shape 가 달라 `llm_config` 의 sibling 리소스로
분리했다 (V081 주석). DDL 은 `llm_config` 를 미러링하되 ① `api_key` NULLABLE (tei/local
셀프호스팅은 키 불요) ② `default_params` 없음 (호출 파라미터 프리셋이 없다) 두 가지가 다르다.
workspace 당 default 1개는 partial unique index 로 강제한다.
