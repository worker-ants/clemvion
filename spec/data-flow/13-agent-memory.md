...
- `codebase/backend/src/nodes/ai/ai-agent/ai-memory-manager.ts` (`AiMemoryManager` — ai-agent recall + 턴 경계 enqueue; M-1 분할로 `ai-agent.handler.ts` facade 에서 추출) · `codebase/backend/src/nodes/ai/information-extractor/information-extractor.handler.ts` — recall + 턴 경계 enqueue 호출부
- `codebase/frontend/src/app/(main)/w/[slug]/agent-memory/page.tsx` + `codebase/frontend/src/lib/api/agent-memories.ts` — admin 화면
...
sequenceDiagram
  ...
  H->>Svc: scheduleExtraction({ workspaceId, scopeKey, llmConfigId, model, extractionModelConfigId, embeddingModelConfigId, turns, ttlDays })
  ...
  Note over P,Lc: 모델 폴백 체인: extractionModelConfigId 의 config → llmConfigId → model → llmConfig.defaultModel
  Lc-->>P: [{ content, kind }] — kind ∈ fact/preference/entity, 그 외 'fact' fallback (AGM-11)
  P->>Svc: saveMemories(workspaceId, scopeKey, items, { embeddingModelConfigId }, ttlDays)
...
`persistent` 전략의 노드가 LLM 호출 **직전 동기로** 수행한다 (`AiMemoryManager.injectMemoryContext`
[`ai-agent/ai-memory-manager.ts`, M-1 분할로 핸들러 facade 에서 추출] / information-extractor.handler.ts 동형 경로).
...
sequenceDiagram
  ...
  H->>Svc: recall(workspaceId, scopeKey, queryText, { embeddingModelConfigId }, { topK, threshold })
...
- **임베딩 출처 일치**: 회수(query)와 저장(document)이 같은 노드 config
  `embeddingModelConfigId` 를 쓰므로 차원·endpoint 가 일치한다. 미지정 시
  워크스페이스 기본 embedding ModelConfig 로 폴백(`ModelConfigService.resolveEmbedding`, 둘 다 없으면 `NotFoundException`).
...
| Sink | 흐름 | 비고 |
| --- | --- | --- |
| LLM provider (chat) | 턴 경계 추출 콜 (worker) | `LlmService.chat`, 모델 폴백 `extractionModelConfigId 의 config → llmConfigId → model → llmConfig 기본`, temperature 0 / JSON. 사용량 → [`llm-usage.md`](./7-llm-usage.md) |
| LLM provider (embed) | 저장 임베딩 (`document`) / recall query 임베딩 (`query`) | `LlmService.embed` — KB 와 동일 경로. 노드 config `embeddingModelConfigId` → ws 기본 embedding config 폴백 (`resolveEmbedding`) |
...
| LLM Usage | cross-ref | chat 계열 LLM 호출만 `llm_usage_log` 적재 (embed 계열 미적재) — [`llm-usage.md`](./7-llm-usage.md) §1.3 |
...
`extractionModelConfigId`/`embeddingModelConfigId`), 주입 ordering 상세, 요약 buffer 와의 상호작용은
