...
| `execution_failed` | 구현됨 | `ExecutionEngineService.dispatchExecutionFailedNotification` (runExecution FAILED 분기) | `execution.status='failed'` 인 **top-level 실행**(`!parentExecutionId` — background body/sub-workflow 하위 실행 제외, `background_failed` 중복 회피). 수신자: 워크플로우 owner + 실행자(`executedBy`, dedup). channel=`both`(§5.1). resource_type='execution'(§2.1 — `background_failed` 옛 fallback 과 키공간 공유). |
| `schedule_failed` | 구현됨 | `ScheduleRunnerService.dispatchScheduleFailedNotification` | 스케줄이 execution 을 **시작하지 못함**(파라미터 해석·enqueue 실패). 시작된 execution 의 이후 실패는 `execution_failed` 가 커버. 수신자: 워크플로우 owner(`createdBy`; 없으면 skip). channel=`both`(§5.1). |
