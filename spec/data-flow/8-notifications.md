...
### WebSocket emit 표기 정정 — `notification.new` + `notifications:<userId>` (2026-05-18)

본 문서는 §1 다이어그램·§2.2 표·코드 진입점 안내에서 emit 을 콜론 표기 (`notification:new`) + `user:<userId>` room 으로 기술해왔다. 그러나:
...
3. **본 문서 §4.6 follow-up 정합**: 같은 문서가 multi-device 동기화 follow-up 으로 `notification.read` / `notification.dismissed` (점 표기) 를 명시 — §1·§2.2 의 콜론 표기와 어긋나 있었다.

본 개정으로 §1·§2.2 의 표기를 프로토콜 권위에 맞춘다. 코드 측은 현재 emit 미구현 (`WebsocketService` 에 `emitNotificationEvent` 부재) — 실제 emit 구현은 follow-up phase 에서 수행하며, 그때 본 표기를 그대로 사용한다.

옛 콜론 표기 (`notification:new`) 와 `user:<userId>` 채널은 의도적 설계 결정이 아닌 본 문서 작성 당시의 오기였으며, 한때 채택된 적 없는 표기다 — 미래 검토자가 "한때 채택되었다가 번복" 으로 오독하지 않도록 명시.

> 관련 일관성 검토: `review/consistency/2026/05/17/16_22_22/SUMMARY.md` 의 Critical C-1·C-2 가 본 개정으로 해소.
