...
| `team_invite` | 구현됨 | `WorkspaceInvitationsService.dispatchTeamInviteNotification` | 새 멤버 초대 시 **초대 이메일이 이미 가입자(비멤버)** 인 경우 그 사용자에게 in-app + 이메일(channel=`both`, §5.1). ⚠ 초대 링크 이메일(`dispatchEmail`)과 별개라 기존 가입자는 이메일 2통 — UX 재검토 대기(`spec-update-notifications-firing.md`). |
