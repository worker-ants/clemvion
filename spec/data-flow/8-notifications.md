...
- `codebase/backend/src/modules/notifications/notifications.service.ts` — 적재(`createMany`) + 목록 / 카운트 / 읽음 / dismiss + `hasRecentByResource` 중복 방지 + `findByResource` (리소스 attribution 조회 — §2.1). (사용자 환경설정 확인·이메일 발송은 본 서비스에 미구현 — 위 주의 참조)
...
- `codebase/backend/src/modules/websocket/websocket.service.ts` — `notification.new` emit (**미구현 (Planned)**, follow-up phase; [`spec/5-system/6-websocket-protocol.md §4.4`](../5-system/6-websocket-protocol.md#44-알림-이벤트-server--client--계획미구현) 권위 표기)
...
| Sink | 흐름 | 비고 |
| --- | --- | --- |
| WebSocket 채널 `notifications:<userId>` | `notification.new` emit | 모든 알림에 대해 즉시. 이벤트 정의는 [`spec/5-system/6-websocket-protocol.md §4.4`](../5-system/6-websocket-protocol.md#44-알림-이벤트-server--client--계획미구현). 본 emit 은 follow-up phase 작업 — 현재 `WebsocketService` 에 해당 메서드 미구현이며, `WebsocketGateway.VALID_CHANNEL_PREFIXES` 에 `notifications:` prefix 만 등록되어 있다 |
...
즉시 사라짐) 는 [`spec/5-system/6-websocket-protocol.md §4.4`](../5-system/6-websocket-protocol.md#44-알림-이벤트-server--client--계획미구현)
...
1. **프로토콜 권위**: [`spec/5-system/6-websocket-protocol.md §4.4`](../5-system/6-websocket-protocol.md#44-알림-이벤트-server--client--계획미구현) 가 알림 이벤트의 정식 정의처이며, 거기서 채널은 `notifications:{userId}`, 이벤트 type 은 `notification.new` (점 표기) 로 명시. 다른 모든 WebSocket 이벤트 (`execution.started`, `execution.node.completed`, `auth.token_expired` 등) 도 점 표기를 따른다.
...
코드 측은 현재 emit 미구현 (`WebsocketService` 에 `emitNotificationEvent` 부재) — 실제 emit 구현은 follow-up phase 에서 수행하며, 그때 본 표기를 그대로 사용한다.
