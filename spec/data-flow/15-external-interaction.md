...
| BullMQ | 공개 위젯 idle-wait reaper (repeatable, 별도 서비스/큐명) | repeatable → `auth_config_id IS NULL` + per_execution 토큰 전 만료(`execution_token.exp_at`) `waiting_for_input` sweep → `cancelled`(`cancelledBy='timeout'` · `error.code='WEBCHAT_IDLE_TIMEOUT'`) + `revokeAllForExecution` | upsertJobScheduler 멱등 — 멀티 인스턴스 전역 1회. EIA-RL-06 형제 패턴(동일 `execution_token` 소스). abandoned 공개 위젯 세션 backstop 회수 ([EIA §3.4 EIA-RL-07 · §R19](../5-system/14-external-interaction-api.md)). 범위=공개 위젯 한정, `formConfig.timeout` 등 무관 |
