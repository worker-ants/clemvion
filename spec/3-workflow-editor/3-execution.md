...
| 히스토리 로드 | 계획·미구현 | 이전 실행의 입력 데이터 불러오기 (다이얼로그에 "Load from History" UI 없음) |
| 저장 | 계획·미구현 | 자주 사용하는 테스트 데이터 세트 저장/이름 지정 |
| 검증 | 계획·미구현 | 유효한 JSON인지 실시간 검증 (제출 시점 파싱만, 실시간 검증 UI 없음) |
...
### 10.12 단축키 _(계획·미구현)_

> **상태: 미구현 (계획).** 아래 두 단축키 핸들러는 구현돼 있지 않다. 에디터 전역 키 핸들러(`workflow-editor.tsx`)는 현재 Undo / Redo / Save / Assistant 토글만 처리한다.

| 단축키 | 동작 |
|--------|------|
| Ctrl + Shift + R _(계획)_ | Run Results 드로어 토글 (열기/닫기) |
| Escape (드로어 포커스 시) _(계획)_ | 캔버스로 포커스 복귀 |
