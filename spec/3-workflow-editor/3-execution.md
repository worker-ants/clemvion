...
## 7. 실행 히스토리 _(인-에디터 진입점·오버레이 미구현 — 계획)_

> **상태: 인-에디터 surface 미구현 (계획).** 에디터 더보기(⋮) 메뉴는 현재 Version History / Export / Delete 만 제공하며 "실행 히스토리" 항목이 없다 (`editor-toolbar.tsx`). 과거 실행을 캔버스에 오버레이하거나 "이 입력으로 다시 실행" 하는 인-에디터 흐름도 미구현이다. 실행 내역 자체는 전용 페이지([Spec 실행 내역](../2-navigation/14-execution-history.md))에서 조회하며, 에디터에서 직전 실행을 재실행하는 보조 진입점은 Run Results 드로어의 Re-run 버튼(§10.14)으로 제공된다. 아래 §7.1~§7.3 은 인-에디터 히스토리 패널의 설계 참고용이다.

### 7.1 접근 _(계획)_

(계획) 에디터 헤더의 더보기(⋮) → "실행 히스토리" 또는 전용 패널 탭
...
### partial 강등 — implemented 로 단정됐으나 부재한 surface (2026-06-03 audit)

2026-06-03 spec-vs-code audit 에서 본 문서가 `implemented` 로 단정한 surface 중 코드에 실재하지 않는 것이 확인돼 `partial` 로 강등했다 (추적: `plan/in-progress/spec-sync-execution-gaps.md`). 미구현 항목: §1.3 단일 노드 테스트, §2.2 Mock Input 의 History 로드/세트 저장/실시간 검증, §7 인-에디터 실행 히스토리(⋮ 진입점·캔버스 오버레이·재실행), §10.12 단축키(Ctrl+Shift+R / Escape). 함께 §1.2 부분실행 트리거(우클릭→툴바 드롭다운 "Run from Selected"), §8.1 노드 이벤트명(`execution.node.*`), §8.2 WS 명령 목록(start/stop 은 REST), §9 API 경로(부분실행은 execute body `input.fromNodeId`, 히스토리는 `GET /api/executions/workflow/:workflowId`), §10.3/§10.5 드로어·타임라인 치수는 기능이 존재하므로 본문 서술만 코드에 맞게 정정했다 (강등 사유 아님).
