...
| POST | /api/executions/:id/continue | 입력 대기(`waiting_for_input`) 실행 이어가기 (폼 데이터 전달). 대기 상태가 아니면 422 `INVALID_STATE` (publisher 측 사전 검증 — [실행 엔진 §7.5.1](../5-system/4-execution-engine.md#751-publisher-측-사전-검증--invalid_execution_state)) |
...
> **AI multi-turn retryable error 종결 시 예외**: AI Agent multi-turn 이 `port: 'error'` + `details.retryable === true` 로 종결된 경우에는 **Preview 우선** — conversation thread 안에 `system_error` item 이 인라인 표시되어 사용자가 대화 흐름 안에서 에러와 `[다시 시도]` 버튼을 직접 다룰 수 있기 때문. Error 탭은 `output.error` JSON 형식으로 여전히 접근 가능 (디버깅 용도).
...
| Multi Turn retryable error 종결 (`port: 'error'` + `details.retryable === true`) | 노드 status `failed`, conversation thread 마지막에 `system_error` item APPEND (자동), 인스펙터 입력 영역이 `[다시 시도]` 버튼 + `retryAfterSec` 카운트다운으로 전환 |
