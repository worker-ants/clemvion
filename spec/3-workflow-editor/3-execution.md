...
워크플로우 실행 시 Presentation 노드(Carousel, Table, Chart, Form, Template, PDF)의 결과를 **채팅형 히스토리** 방식으로 실시간 시각화하는 하단 드로어 패널. 노드 실행 순서대로 결과가 세로 스크롤 리스트에 누적 표시된다.

### 10.1 핵심 원칙

- Presentation 노드 중 **Form만 실행을 일시 정지**한다 (사용자 입력 대기). 나머지 5종(Carousel, Table, Chart, Template, PDF)은 실행을 중단하지 않고 결과를 드로어에 표시한다.
- 드로어는 **채팅/히스토리 UI** 형태로, 실행된 Presentation 노드의 결과가 시간순으로 세로 리스트에 누적된다. 최신 결과만이 아닌 **전체 이력**이 표시된다.
- 드로어는 워크플로우에 Presentation 노드가 포함된 경우에만 표시된다.

### 10.2 레이아웃

캔버스 하단에 슬라이드업되는 리사이즈 가능한 패널. 탭 대신 세로 스크롤 히스토리:

```
┌──────────────────────────────────────────────────────────────┐
│  Run Results  3/10 nodes · 2 results           [−] [✕]      │
│  ────────────────────────────────────────────────────────── │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ ▼ 📊 Chart "Revenue"                      ✅ Done     │  │
│  │   ┌──────────────────────────────────┐                 │  │
...
| 접힌 상태 | 36px 높이 바 — 타이틀 + 결과 수 배지 (예: "Run Results · 4 results") |
...
| 노드 카운터 | `N/M nodes` — 완료/전체 노드 수 |
| 결과 카운터 | `N results` — Presentation 노드 결과 수 |
| 최소화 버튼 [−] | 드로어를 접힌 상태(36px)로 축소 |
| 닫기 버튼 [✕] | 드로어 완전 숨김 (실행 상태 리셋) |

### 10.5 히스토리 항목

각 Presentation 노드의 실행 결과는 접기/펼치기 가능한 히스토리 항목으로 표시된다:

| 요소 | 설명 |
|------|------|
| 접기/펼치기 화살표 | ▶(접힘) / ▼(펼침). 기본: 펼침 |
| 노드 아이콘 | Presentation 카테고리 색상(`#EC4899`, 핑크) |
| 노드 라벨 | 사용자 지정 라벨 또는 노드 유형명 |
| 상태 배지 | ✅ Done (완료) / ⏸ Waiting (Form 대기 중) |
| 콘텐츠 | 각 노드 유형에 맞는 렌더링 결과 (§10.6 참조) |

히스토리 항목은 실행 순서(시간순)로 정렬되며, 새 항목 추가 시 자동으로 하단 스크롤된다.
...
| Chart | 렌더링된 HTML 차트 (인라인) |
| Template | outputFormat에 따라 HTML 렌더링 또는 Markdown/텍스트 코드 블록 |
| PDF | 파일명 + 다운로드/열기 링크 |
| Form (대기 중) | **인터랙티브 폼 UI** — 제목, 설명, 필드 목록 (동적 생성), Submit 버튼. 제출 시 `execution.submit_form` WebSocket 명령 전송 |
| Form (제출 후) | 제출된 데이터를 키-값 형태로 표시 |

### 10.7 상태 바 확장

기존 실행 상태 바에 Presentation 결과 카운트를 포함한다:

```
🔄 Running...   3/10 nodes · 2 results                [−] [✕]
✅ Completed    10/10 nodes · 4 results                [−] [✕]
⏸ Waiting for input...   5/10 nodes · 3 results       [−] [✕]
❌ Failed       5/10 nodes (1 failed) · 2 results      [−] [✕]
```
...
| 이벤트 | 드로어 동작 |
|--------|------------|
| 실행 시작 | 드로어 표시, 히스토리 비움. "Waiting for results..." 메시지 표시 |
| Presentation 노드 완료 | 히스토리에 새 항목 추가 + 자동 하단 스크롤 |
| Form 노드 `waiting_for_input` 진입 | 히스토리 하단에 폼 UI 항목 추가 + 자동 스크롤. 상태 바 "⏸ Waiting for input..." |
| Form 제출 | 폼 항목이 "제출 완료" 뷰(키-값)로 전환. 실행 재개, 상태 바 "🔄 Running..." |
| 실행 완료 | 드로어 유지. 사용자가 전체 히스토리를 스크롤하며 탐색 가능 |
| 실행 실패 | 드로어 유지. 실패 시점까지 수집된 결과 표시 |
| 새 실행 시작 | 이전 히스토리 클리어, 드로어 리셋 |
| 드로어 닫기 (`[✕]`) | 실행 상태 리셋 |
| Presentation 노드 없는 워크플로우 | "No presentation output to display" 메시지 표시 |
