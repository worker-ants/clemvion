...
| `execution.waiting_for_input` | executionId, nodeId, nodeType, interactionType, formConfig?, buttonConfig? | Form 노드에서 사용자 입력 대기 또는 버튼이 설정된 Presentation 노드에서 사용자 인터랙션 대기. `interactionType`: `form` / `buttons`. 상세: [WS 프로토콜 §4.4](../5-system/6-websocket-protocol.md#44-사용자-입력-대기-이벤트-상세) |
