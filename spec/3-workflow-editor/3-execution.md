...
워크플로우 실행 시 Presentation 노드(Carousel, Table, Chart, Form, Template, PDF)의 결과를 실시간으로 시각화하는 하단 드로어 패널.

### 10.1 핵심 원칙

- Presentation 노드 중 **Form만 실행을 일시 정지**한다 (사용자 입력 대기). 나머지 5종(Carousel, Table, Chart, Template, PDF)은 실행을 중단하지 않고 결과를 드로어에 표시한다.
- 드로어는 워크플로우에 Presentation 노드가 포함된 경우에만 표시된다.

### 10.2 레이아웃

캔버스 하단에 슬라이드업되는 리사이즈 가능한 패널:
...
│  Run Results                                    [−] [✕]     │
│  ┌──────────┬──────────┬──────────┬──────────┐              │
│  │ 📋 Table │ 📊 Chart │ 📄 PDF   │ 📝 Form  │  ← 탭       │
│  │ "Users"  │"Revenue" │"Report"  │"Approval"│              │
│  ├──────────┴──────────┴──────────┴──────────┘              │
│  │                                                          │
│  │  ┌──────┬────────┬──────┐                                │
│  │  │ Name │ Email  │Score │   (활성 탭의 렌더링된 콘텐츠)    │
│  │  ├──────┼────────┼──────┤                                │
│  │  │ Kim  │ k@...  │  95  │                                │
│  │  │ Lee  │ l@...  │  88  │                                │
│  │  └──────┴────────┴──────┘                                │
...
| 속성 | 값 |
|------|---|
| 기본 높이 | 280px (캔버스 영역의 약 35%) |
| 최소 높이 | 150px |
| 최대 높이 | 캔버스 영역의 60% |
| 위치 | 캔버스 영역 하단, 전체 너비 (노드 팔레트 ~ 설정 패널 너비) |
| 리사이즈 | 상단 드래그 핸들로 높이 조정 |
| 접힌 상태 | 36px 높이 바 — 타이틀 + 결과 수 배지 (예: "Run Results (4)") |
...
|------|------|
| 타이틀 | "Run Results" |
| 최소화 버튼 [−] | 드로어를 접힌 상태(36px)로 축소. 타이틀 + 결과 수 배지만 표시 |
| 닫기 버튼 [✕] | 드로어 완전 숨김. 상태 바의 `[Results ↑]`로 재열기 가능 |

### 10.5 탭 시스템

- Presentation 노드 실행 완료 순서대로 탭이 생성된다.
- 각 탭에는 노드 아이콘 + 사용자 레이블 (또는 레이블 미설정 시 유형명) 표시.
- 활성 탭은 Presentation 카테고리 색상(`#EC4899`, 핑크)으로 하이라이트.
- 실행 중인 노드의 탭은 아이콘 대신 스피너 표시.
- 새 탭 추가 시 기존 활성 탭은 유지하고, 새 탭에 펄스 애니메이션으로 주의를 끈다.
- 탭이 너무 많아 한 줄에 들어가지 않으면 좌/우 스크롤 또는 드롭다운 오버플로우 메뉴.

### 10.6 탭별 렌더링 콘텐츠

| 노드 유형 | 드로어 탭 콘텐츠 |
|-----------|-----------------|
| Carousel | 인터랙티브 캐러셀 — 좌/우 탐색 화살표, 카드/이미지/미니멀 레이아웃 렌더링 |
| Table | 전체 테이블 — 컬럼 헤더, 행 데이터, 정렬 가능 컬럼 클릭, 페이지네이션 컨트롤 |
| Chart | 인터랙티브 SVG 차트 — 호버 시 데이터 포인트 툴팁, 범례, 축 라벨 |
| Template | 렌더링된 결과 — HTML은 샌드박스 iframe, Markdown은 렌더링된 형태, Text는 코드 블록 |
| PDF | PDF 뷰어 임베드 (브라우저 PDF 렌더러) + `[Download]` 버튼 + `[새 탭에서 열기]` 링크 |
| Form (대기 중) | 실제 폼 UI — 제목, 설명, 필드 목록, 제출 버튼. 폼 제출 시 `execution.submit_form` WebSocket 명령 전송 |
| Form (제출 후) | 제출된 데이터를 키-값 형태로 표시 + 제출 시각 + 제출자 정보 |

### 10.7 상태 바 확장

기존 실행 상태 바에 Presentation 결과 카운트와 드로어 토글 버튼을 추가한다:

```
▶ Running...   0.5s elapsed   2 results    [Results ↑] [■ Stop]
✅ Completed   2.3s   10/10 nodes   4 results   [Results ↑]
⏸ Waiting for input   "Approval Form"     [Results ↑] [■ Stop]
❌ Failed at "Node C"   1.2s   2 results        [Results ↑]
```

| 요소 | 설명 |
|------|------|
| `N results` | Presentation 노드 완료 시마다 증가하는 카운터. Presentation 노드가 없으면 표시 안 함 |
| `[Results ↑]` | 드로어 토글 버튼. 드로어가 열려 있으면 `[Results ↓]`로 변경 |

### 10.8 라이프사이클

| 이벤트 | 드로어 동작 |
|--------|------------|
| 실행 시작 | 접힌 상태(36px)로 표시 |
| 첫 Presentation 노드 완료 | 자동 펼침 + 해당 노드 탭 생성 + 활성화 |
| Form 노드 `waiting_for_input` 진입 | 자동 펼침 + Form 탭 생성 + 활성화 (폼 UI 렌더링) |
| Form 제출 | Form 탭이 "제출 결과" 뷰로 전환. 실행 재개 |
| 추가 Presentation 노드 완료 | 새 탭 추가 (기존 활성 탭 유지, 새 탭에 펄스 애니메이션) |
| 실행 완료 | 드로어 유지. 사용자가 결과를 계속 탐색 가능 |
| 실행 실패 | 드로어 유지. 실패 시점까지 수집된 결과 표시. 실패한 Presentation 노드의 탭은 에러 상태 표시 |
| 새 실행 시작 | 이전 결과 클리어, 드로어 리셋 (접힌 상태) |
| 드로어 닫기 (`[✕]`) | 드로어 숨김. 상태 바의 `[Results ↑]`로 재열기 가능 |
| 캔버스에서 Presentation 노드 클릭 | 드로어 열기 + 해당 노드 탭 선택. 노드가 미실행이면 탭에 "Awaiting execution" 플레이스홀더 표시 |
| Presentation 노드 없는 워크플로우 | 드로어 미표시. 상태 바에 results 카운터 미표시 |

### 10.9 Loop/ForEach 내 Presentation 노드

컨테이너(Loop, ForEach) 내부의 Presentation 노드가 반복 실행되는 경우:

- 탭 제목에 이터레이션 표시: `"Table (Iteration 3/5)"`
- 기본적으로 최신(마지막) 이터레이션 결과를 표시
- 탭 내 상단에 이터레이션 드롭다운으로 이전 결과 탐색 가능:

```
┌──────────────────────────────────────────┐
│  📋 Table "Users"   Iteration: [3/5 ▼]  │
│  ────────────────────────────────────── │
│  ┌──────┬────────┬──────┐               │
│  │ Name │ Email  │Score │               │
│  ├──────┼────────┼──────┤               │
...
실행 히스토리(§7)에서 과거 실행을 클릭하면, 해당 실행의 Presentation 노드 결과로 드로어를 채운다. "이 입력으로 다시 실행" 시 드로어는 리셋된다.
...
### 10.13 Presentation 노드 감지

클라이언트는 기존 `node.completed` WebSocket 이벤트의 `output.type` 필드를 확인하여 Presentation 노드를 감지한다. `output.type`이 `carousel`, `table`, `chart`, `form`, `template`, `pdf` 중 하나이면 드로어에 탭을 생성하고 렌더링한다. 별도의 새로운 WebSocket 이벤트는 추가하지 않는다.
