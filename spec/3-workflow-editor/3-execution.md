...
### 1.3 단일 노드 테스트 _(계획·미구현)_

> **상태: 미구현 (계획).** 단일 노드만 실행하는 진입점(우클릭 메뉴 항목, 전용 엔드포인트)이 프론트·백엔드 어디에도 없다. 노드 우클릭 메뉴는 현재 Settings / Duplicate / Disable / Delete 만 제공한다 (`workflow-canvas.tsx`). 설계 참고용으로 남기되 v1 surface 가 아니다.
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| POST | /api/workflows/:id/execute | 워크플로우 실행. 부분 실행은 body `input.fromNodeId` 로 시작 노드 지정 (전용 부분실행/단일노드 엔드포인트 없음) |
