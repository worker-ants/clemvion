...
2026-06-03 spec-vs-code audit 에서 본 문서가 `implemented` 로 단정한 surface 중 코드에 실재하지 않는 것이 확인돼 `partial` 로 강등했다 (추적: `plan/in-progress/spec-sync-execution-gaps.md`). 추적 항목: §1.3 단일 노드 테스트, §2.2 Mock Input 의 History 로드/세트 저장/실시간 검증, §7 인-에디터 실행 히스토리(⋮ 진입점·캔버스 오버레이·재실행), §10.12 단축키(Ctrl+Shift+R / Escape).
