...
| `execution.ai_message` | executionId, nodeId, message, turnCount, messages | AI Agent Multi Turn에서 AI 응답 메시지 전달 (`messages` 는 system 제외 권위 스냅샷) |
...
- 진행 중인 본문 노드의 상태 변화(`NODE_STARTED` / `NODE_COMPLETED`) 가 실시간 타임라인 갱신.
- `BACKGROUND_RUN_COMPLETED` / `BACKGROUND_RUN_FAILED` 수신 시 카드 뱃지 갱신 + 채널 unsubscribe.
