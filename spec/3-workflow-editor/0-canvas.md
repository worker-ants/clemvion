...
명시 저장 + 실행 직전 저장은 `Save` 버튼·"저장되지 않은 변경 사항" 상태 텍스트·save-before-run 이 일관되게 맞물린 **의도된 설계**로 판단해(사용자 확인, 2026-07-08), 스펙을 현재 동작으로 정정한다(구현을 옛 스펙에 맞추지 않음). `spec/data-flow/11-workflow.md §1.4` 도 이미 "auto-save 없음, 500ms debounce 는 graph-warning 용" 으로 동일 사실을 서술한다. 유저 가이드(`03-workflow-editor/{overview,settings-panel,saving-and-sharing}` · `01-getting-started/{ui-tour,first-workflow}`)는 이미 현재 동작을 서술한다. 타이머 자동 저장·오프라인 초안 복원을 되살릴지는 별도 기획 판단으로 남긴다. 추적: [`plan/in-progress/spec-sync-canvas-gaps.md`](../../plan/in-progress/spec-sync-canvas-gaps.md).
