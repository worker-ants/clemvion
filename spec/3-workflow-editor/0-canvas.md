...
pending_plans:
  ...
  - plan/in-progress/spec-sync-canvas-gaps.md
---
...
| Ctrl + 휠 | 줌 인/아웃 (대안) |
| 핀치 (트랙패드) | 줌 인/아웃 |
| 줌 버튼 | 좌하단 오버레이의 줌인(+)·줌아웃(−) 버튼. _(미구현 (Planned): 줌 레벨 슬라이더·퍼센트 표시는 없음 — §6 참조)_ |
...
| Ctrl + A | 모든 노드 선택 _(미구현 (Planned): 키보드 단축키 미바인딩 — 현재는 빈 영역 우클릭 메뉴의 "전체 선택"으로만 가능. §10 참조)_ |
| Escape | 선택 해제 _(미구현 (Planned): 키보드 단축키 미바인딩 — 빈 영역 클릭으로 해제. §10 참조)_ |
...
| Ctrl + C | 선택된 노드(+연결된 엣지) 복사 _(미구현 (Planned): clipboard 로직 없음)_ |
| Ctrl + V | 복사된 노드 붙여넣기 (원본 옆에 오프셋 배치) _(미구현 (Planned): clipboard 로직 없음)_ |
| Ctrl + D | 선택된 노드 즉시 복제 _(미구현 (Planned): 단축키 미바인딩 — 현재는 우클릭 메뉴 "복제"로만 가능)_ |
...
| 붙여넣기 | 클립보드의 노드 붙여넣기 _(미구현 (Planned): clipboard 미지원 — 현재 메뉴에 항목 없음)_ |
| 전체 선택 | 모든 노드 선택 |
| 맞춤 보기 | Fit to View |
...
> **미구현 (Planned)**: 아래 다이어그램의 `⏱ Recent` 섹션과 `▼ Installed`(마켓플레이스) 섹션은 아직 렌더되지 않는다. 현재 팔레트는 백엔드 카테고리 목록(Trigger~Presentation)만 표시한다.
...
| 클릭 | 캔버스 중앙(또는 빈 영역)에 노드 추가 _(미구현 (Planned): 팔레트 아이템 클릭 핸들러 없음 — 추가는 드래그 앤 드롭으로만)_ |
| 패널 접기 | 토글 버튼으로 팔레트 숨기기/표시 _(미구현 (Planned): 접기 토글 없음)_ |
...
- _(미구현 (Planned): Enter 선택 / Escape 취소 키보드 핸들링 없음 — 현재는 클릭 선택, 바깥 클릭으로 닫힘)_
...
> **미구현 (Planned)**: `custom-node.tsx` 에 노드 우상단 ✕ 삭제 버튼이 아직 없다. 현재 노드 삭제는 우클릭 메뉴 "삭제" 또는 Delete/Backspace 키(ReactFlow `deleteKeyCode`)로만 가능하다. 아래는 계획 명세다.
...
> **구현 현황**: 줌 컨트롤(줌인 −/+, Fit)은 캔버스 **좌하단 오버레이**(`ZoomControls`, `workflow-canvas.tsx`)에 버튼 3개로 표시된다. Undo/Redo 는 별도 하단 바가 아니라 **헤더 툴바**(`editor-toolbar.tsx`)에 있다. 아래 다이어그램의 통합 하단 바 레이아웃과 줌 슬라이더·퍼센트는 미구현 (Planned)이다.

```
좌하단 오버레이:  [−] [+] [Fit]      ← 구현됨 (버튼 3개)
계획 레이아웃:    [−] ━━━━━●━━━━━ [+]  100%  │  [Fit]  │  [↩ Undo] [↪ Redo]   ← Planned
```
...
| Fit 버튼 | 전체 맞춤 보기. 구현됨 |
| 줌 슬라이더 | 25% ~ 200% _(미구현 (Planned))_ |
| 줌 퍼센트 | 현재 줌 레벨 표시 _(미구현 (Planned))_ |
...
> **미구현 (Planned)**: @xyflow `MiniMap` 컴포넌트가 `workflow-canvas.tsx` 에 렌더되지 않는다(현재 캔버스 오버레이는 `Background`·`ZoomControls`·Empty State Panel 만). 아래는 계획 명세다.
...
> **구현 현황**: 전역 keydown 핸들러(`workflow-editor.tsx`)는 현재 **Ctrl+S / Ctrl+Z / Ctrl+Y · Ctrl+Shift+Z / Ctrl+/** 4종만 바인딩한다. Delete/Backspace 는 ReactFlow `deleteKeyCode` 로 처리된다. 아래 표에서 (Planned) 로 표기된 단축키는 아직 미바인딩이다.
...
| Ctrl + V | 붙여넣기 | Planned |
| Ctrl + D | 복제 | Planned |
| Ctrl + A | 전체 선택 | Planned |
| Escape | 선택 해제 | Planned |
| Space + 드래그 | 캔버스 패닝 (대안) | Planned |
| Ctrl + + / - | 줌 인/아웃 | Planned |
| Ctrl + 0 | 줌 100% | Planned |
| Ctrl + 1 | Fit to View | Planned |
| Ctrl + Shift + R | Run Results 드로어 토글 | Planned |
...
> **미구현 (Planned)**: 컨테이너 삭제 시 Delete/Ungroup 확인 다이얼로그는 아직 없다. 현재 `editor-store.ts` 의 `removeNode` 는 해당 노드 + 연결 엣지를 제거하고 `containerId` 를 재계산할 뿐이며(자식 자동 승격은 엣지 제거에 따른 재계산의 부수효과), "전체 삭제 vs Ungroup" 선택 UI 와 ✕ 삭제 버튼은 없다. 아래는 계획 명세다.
...
> **미구현 (Planned) + §11.2 와 정합 주의**: §11.2 의 "시각 containment 미사용" 결정에 따라 컨테이너는 일반 노드와 동일 크기로 렌더되고 자식은 캔버스 어디에든 자유 배치되므로, 아래의 "배경 틴트" 같은 시각 중첩 표현은 현재 렌더 모델과 맞지 않는다. 또한 최대 깊이(3) 제한·초과 토스트 로직도 코드에 없다(`custom-node.tsx`/`editor-store.ts` 에 depth/tint 처리 부재). 멤버십은 `containerId` 데이터 모델로만 표현되며 중첩 자체는 `containerId` 체인으로 허용된다. 아래 시각·제한 항목은 시각 containment 도입 시 재정의 대상이다.

| 항목 | 설명 |
|------|------|
| 중첩 허용 | 컨테이너 안에 다른 컨테이너 배치 가능 (`containerId` 체인) |
| 최대 중첩 깊이 | 3단계 _(미구현 (Planned): depth 제한 enforcement 없음)_ |
| 레벨별 시각 구분 | 중첩 레벨마다 배경 틴트 변경 (L1: 5% 불투명도, L2: 10%, L3: 15%) _(미구현 (Planned): §11.2 시각 containment 미사용과 배치되는 시각 표현 — 재정의 대상)_ |
| 초과 시 | 3단계 초과 중첩 시도 → "최대 중첩 깊이(3)를 초과할 수 없습니다" 토스트 _(미구현 (Planned))_ |
