...
> **구현됨** (`custom-node.tsx`): 노드 우상단 ✕ 삭제 버튼. 우클릭 메뉴 "삭제"·Delete/Backspace 키(ReactFlow `deleteKeyCode`)에 더한 추가 어포던스로, 클릭 시 `removeNode` 로 연결 엣지까지 함께 제거한다. 아래 명세대로 동작한다.
...
> **부분 구현**: 노드 우상단 ✕ 삭제 버튼(§5.4)은 컨테이너 노드에도 렌더된다 — `custom-node.tsx` 는 일반 노드와 컨테이너를 같은 컴포넌트로 그리며 `isNodeDeletable`(manual_trigger 만 제외)로만 게이트하기 때문이다. 다만 컨테이너 삭제 시의 Delete/Ungroup **확인 다이얼로그는 아직 없다** — ✕ 버튼·Delete 키·우클릭 메뉴 모두 현재는 `editor-store.ts` 의 `removeNode` 로 즉시 삭제한다(해당 노드 + 연결 엣지 제거 + `containerId` 재계산; 자식은 `body` 엣지 소멸로 top-level 승격되는 부수효과 → 사실상 **Ungroup** 동작이라 자식 데이터 손실은 없다). 아래 확인 다이얼로그·"전체 삭제 vs Ungroup" 선택 UI 는 계획 사양으로, 구현 시 세 삭제 경로 전부를 감싼다.
...
컨테이너 노드의 삭제 버튼(✕)은 헤더 바 우상단 외곽에 표시된다 (버튼 자체는 §5.4 구현으로 이미 렌더됨 — 아래는 위치 참고. 클릭 시 확인 다이얼로그 연동만 Planned):
