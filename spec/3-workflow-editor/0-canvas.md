...
| 항목 | 설명 |
|------|------|
| 표시 | `⚠ Not configured` (앰버색, `#F59E0B`) |
| 조건 | 노드의 필수 config 필드가 하나 이상 비어 있을 때 |
| 예외 | Manual Trigger — config 없으므로 요약 표시 안 함 |
| 선택적 필드만 | 모든 필드가 선택적이면 기본값 기준 요약을 정상 표시 |

#### 5.3.3 컨테이너 노드 요약

컨테이너 노드(Loop, ForEach, Background)는 헤더 바의 사용자 레이블 우측에 요약을 표시한다:
...
| Settings 패널 Container 드롭다운 | 노드의 `containerId`를 명시적으로 지정/해제 |
...
### 11.2.1 자동 containerId 동기화

자식 노드의 `containerId`는 다음 두 가지 경로로 설정된다:

1. **Edge 자동 전파**:
   - `Container.body → X` 연결: X의 `containerId`가 비어 있으면 컨테이너로 set.
   - `Y → Container.emit` 연결: Y의 `containerId`가 비어 있으면 컨테이너로 set.
   - 컨테이너 child인 A에서 `A → B` 연결: B가 비어 있으면 동일 컨테이너로 chain 전파.
   - 양 끝이 다른 컨테이너에 속해 있으면 변경 없음 (충돌 회피).
2. **Settings 패널 Container 드롭다운**: 사용자가 명시적으로 지정/해제. 자동 전파가 잘못 잡힌 경우의 수동 보정.

### 11.2.2 제약

| 제약 | 동작 |
|------|------|
| 트리거 노드 child 금지 | trigger 카테고리 노드는 컨테이너 child가 될 수 없음 (`CONTAINER_INVALID_CHILD`). 드롭다운/drop-intersection 모두 거부 |
| 자기 자신 child 금지 | 컨테이너는 자기 자신을 `containerId`로 가질 수 없음. 드롭다운 옵션에서 제외 |
| 자손 컨테이너 child 금지 | A의 자손 컨테이너 B를 다시 A의 부모로 지정하면 cycle이 됨. 드롭다운 옵션에서 자손 트리 모두 제외. backend도 `CONTAINER_CYCLE` 에러로 거부 |
| Emit 필수 | 컨테이너 실행 시 `emit` 포트에 정확히 1개의 child 노드가 연결되어야 함 (`CONTAINER_MISSING_EMIT` / `CONTAINER_MULTIPLE_EMIT`) |
