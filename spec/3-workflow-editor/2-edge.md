...
**커플링 주의**: 위 원자성 보장은 `detectContainerConflict`(editor-store.ts)의 거부 분기가 source `body` / target `emit` 두 가지뿐이라는 데 의존한다. 그 함수에 새 거부 분기가 추가되면 `buildEdgeSplitPlan` 의 제외 규칙도 함께 갱신해야 반쪽 그래프 재발을 막는다(양쪽 JSDoc 에 상호 forward-pointer 기록).
