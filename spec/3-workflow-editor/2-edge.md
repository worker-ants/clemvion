...
> - **실행 완료(completed)** — source·target 둘 다 `completed` → `edge.className='edge-completed'` → globals.css `edge-complete-flash` 1회성 keyframe 이 초록(#22c55e)으로 잠시 표시 후 원래 포트색으로 복귀.
...
>
> - **포트 선택**: 새 노드의 **첫 입력 포트**(`firstInputHandleId`, 예약 `emit` 제외)를 `source→새 노드` 의 target 으로, 새 노드의 **첫 출력 포트**(`firstOutputHandleId`)를 `새 노드→target` 의 source 로 쓴다. 원본 엣지의 `sourceHandle`·`targetHandle` 은 두 신규 엣지에 **그대로 보존**된다(다중 출력 If/Else·Switch, 다중 입력 노드여도 원본 양끝 핸들 불변). **다중 출력 새 노드**(If/Else·Switch)를 삽입하면 첫 출력만 `target` 에 연결되고 나머지 분기 포트는 미연결로 남아 사용자가 수동 연결한다.
> - **적용 범위**: 새 노드가 입력·출력 포트를 **모두 가질 때만** 분할한다. 입력이 없는 트리거·출력이 없는 노드를 드롭하거나(→ `buildEdgeSplitPlan` 이 null), 엣지가 아닌 빈 영역에 드롭하면 분할 없이 그 위치에 노드만 추가한다(`0-canvas.md` 의 일반 팔레트 드롭과 동일한 fallback).
> - **컨테이너 새 노드 제외**: 드롭하는 새 노드 **자체가 컨테이너**(Loop/ForEach/Map, `isContainer`)면 그 첫 출력이 `body`(본문 진입)라 `새 노드→target` 이 target 을 새 컨테이너 본문 자식으로 재편입시켜 버린다 → 분할 대상에서 제외하고 노드만 추가한다(R-3).
> - **컨테이너 경계 엣지 제외**: `sourceHandle` 이 `body`(컨테이너 본문 진입)이거나 `targetHandle` 이 `emit`(loopback 입력)인 **컨테이너 경계 엣지**는 §6(컨테이너 내부 엣지 규칙)의 emit 단일성·경계 불가침과 `0-canvas.md` containerId 동기화 불변식과의 상호작용이 아직 정의되지 않아 분할 대상에서 제외한다. 컨테이너 본문 종료 출력 `done` 엣지는 제외하지 **않는다** — Parallel Branch 도 동명 `done` 을 일반 데이터 출력으로 쓰고, `done→새 노드` 분할은 body 재편입을 유발하지 않아 안전하다.
> - **연결·불변식(원자성)**: 분할 1회는 원본 엣지 제거 1 + `onConnect` **2회**(source→새 노드, 새 노드→target)를 발생시킨다 — `onConnect` 에 리스너를 추가하는 소비처는 "드롭 제스처당 2회" 를 전제해야 한다. 두 신규 엣지는 §1.2/§1.3 과 동일하게 표준 `onConnect`(→`evaluateConnection`) 경로를 재사용하므로 포트색 파생·유효성 검사가 그대로 적용된다. 위 제외 규칙(원본 body/emit 엣지 + 컨테이너 새 노드) 덕에 두 Connection 은 `detectContainerConflict` 의 유일한 거부 분기(source `body`/target `emit`)에 절대 걸리지 않고, 새 노드라 자기연결·중복도 불가능하다 → `onConnect` 두 번이 **항상 성공**한다. 따라서 `removeEdge` 후 `onConnect`×2 를 순차 실행해도 그래프가 반쪽만 갱신되는 원자성 문제가 구조적으로 발생하지 않는다(`buildEdgeSplitPlan` 이 non-null 을 반환하는 것 자체가 분할 안전성의 게이트). 거부 분기 핸들(`body`/`emit`)은 `edge-utils.ts` `CONTAINER_BODY_HANDLE`/`CONTAINER_EMIT_HANDLE` 공유 상수로 store 와 묶여 있다(hidden coupling 제거).
...
> **현재 구현**: `edge-data-preview.tsx` `EdgeDataPreviewTooltip`/`EdgeDataModal` + `use-edge-hover-preview.ts` 훅. 엣지 hover 시 `workflow-canvas.tsx` `onEdgeMouseEnter` 가 `useEdgeHoverPreview.show(edge.id, clientX, clientY)` 로 커서 위치에 툴팁을 예약한다. 툴팁은 엣지 **연결원(source) 노드의 최근 실행 출력**(`findLatestResultByNodeId` → `unwrapNodeOutput().output`)을 축약해 보여준다(실행 데이터 없으면 렌더 안 함). 축약·바이트 계산은 순수 함수 `edge-utils` 형제 `lib/utils/edge-data-preview.ts` `summarizeDataForPreview`/`formatBytes`. 엣지 진입 시 짧게 지연(90ms) 후 표시해 촘촘한 캔버스에서 커서가 여러 엣지를 스쳐 지날 때 정착하지 못한 엣지의 툴팁/직렬화를 건너뛰고(sweep 방어), 엣지에서 벗어나도 짧게 지연(200ms) 후 숨겨 커서를 툴팁으로 옮겨 "전체 데이터 보기" 를 클릭할 수 있다. 클릭 시 `EdgeDataModal`(Dialog)이 축약 없는 전체 JSON 을 보여준다(모달은 hover 생명주기와 독립적으로 canvas `dataModalEdgeId` 로 열림). 크기 표시는 직렬화 결과가 100KB 이하면 정확 `TextEncoder` 인코딩을, 초과하면 인코딩 할당을 생략하고 문자 수 하한 근사(`bytesApprox`)를 써 크기 앞에 `~` 를 붙인다(대용량 출력 hover 비용 상한).
...
- 툴팁의 **"전체 데이터 보기"** 버튼 클릭 시 축약 없는 전체 데이터 모달 표시 (§4 의 "엣지 클릭 = 엣지 선택" 과 별개 — 모달은 툴팁 버튼에서만 열림)
...
**후속 보강(ai-review 1회차 CRITICAL)**: 초기 구현은 원본 엣지의 경계 핸들만 걸렀고 "**드롭하는 새 노드 자체가 컨테이너**" 인 경우를 놓쳤다 — 컨테이너의 첫 출력이 `body` 라 `firstOutputHandleId` 가 `body` 를 골라 `새 노드→target` 이 `propagateContainerOnConnect` Rule 1 을 발동, target 을 새 컨테이너 본문 자식으로 조용히 재편입(또는 conflict 거부로 반쪽 그래프)시켰다. `buildEdgeSplitPlan` 에 `definition.isContainer` 가드를 추가해 컨테이너 새 노드는 분할에서 제외한다(노드만 추가). 이 제외로 두 신규 Connection 이 `detectContainerConflict` 의 body/emit 거부 분기에 걸릴 경로가 완전히 사라져 "removeEdge 후 onConnect 반쪽 실패" 원자성 우려도 **구성적으로 해소**된다(별도 store 원자 액션 불요). 또한 경계 판정을 `sourceHandle∈{body,done}` 에서 **`body` 만**으로 좁혔다 — Parallel Branch 가 컨테이너가 아님에도 동명 `done` 을 일반 데이터 출력으로 써 `done` 을 뭉뚱그리면 그 데이터 엣지 분할이 잘못 막히기 때문이고, 컨테이너 `done` 엣지 분할은 body 재편입을 유발하지 않아 안전하다.

**커플링 주의**: 위 원자성 보장은 `detectContainerConflict`(editor-store.ts)의 거부 분기가 source `body` / target `emit` 두 가지뿐이라는 데 의존한다. 두 핸들 값은 `edge-utils.ts` `CONTAINER_BODY_HANDLE`/`CONTAINER_EMIT_HANDLE` 공유 상수로 `buildEdgeSplitPlan`(분할 제외)과 store `detectContainerConflict`/`propagateContainerOnConnect`(거부·전파)가 함께 import 해 compile-time 으로 묶여 있다(hidden coupling 제거). 다만 그 함수에 **새 거부 분기(다른 핸들)**가 추가되면 `buildEdgeSplitPlan` 의 제외 규칙도 함께 갱신해야 반쪽 그래프 재발을 막는다(양쪽 JSDoc 에 상호 forward-pointer 기록).

**undo 단일 체크포인트 실측 보강(ai-review 3회차)**: 노드 생성 헬퍼 `buildAndAddNode` 가 자체 `pushUndo` 를 호출하고 그 안에서 `addNode`(store)가 또 `pushUndo` 해 삽입 1회가 동일 스냅샷 2개를 쌓던 잠재 결함(§1.2 도 공유)을 발견 — `buildAndAddNode` 의 중복 `pushUndo` 를 제거해 `addNode` 의 pushUndo 하나만 단일 체크포인트가 되게 정정했다. store 시퀀스(addNode→removeEdge(skipUndo)→onConnect(skipUndo)×2→undo)가 정확히 스냅샷 1개를 남기고 undo 1회로 완전 복원됨을 통합 테스트로 고정.
