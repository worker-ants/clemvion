...
| ED-SP-05 | 설정 변경을 `변경 저장`(Settings 탭)·`JSON 적용`(Code 탭) 으로 캔버스(in-memory store)에 반영. 저장 전 다른 노드로 전환하면 미저장 편집은 폐기 | 필수 |
...
| ED-SV-02 | 실행 직전 자동 저장 — 미저장 변경이 있으면 `Run`(전체·부분·단일 노드) 실행 직전에 먼저 저장. 주기적(타이머) 자동 저장은 두지 않음 | 필수 |
...
| ED-DB-05 | 브레이크포인트 설정 — 해당 노드에서 실행 일시 정지 _(미구현 — 로드맵, [§3-execution §6](./3-execution.md#6-브레이크포인트-향후-로드맵--미구현))_ | 권장 |
