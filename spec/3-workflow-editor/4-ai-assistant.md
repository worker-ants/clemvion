...
| **Stalled turn hint** | assistant 가 텍스트 출력 없이(= content 공백) 정상 종료(`done`) 되었고 **승인된 활성 plan** 에 실행 가능한 pending step 이 남아있으면, 프론트가 해당 메시지에 systemHint("이어서 진행해줘 라고 답해 주시면 남은 단계를 계속 실행할게요." 류) 를 자동 주입해 회색 info 박스로 렌더한다. 승인 대기 중 plan 에는 "계획대로 진행" 버튼이 이미 노출되므로 힌트를 띄우지 않는다 |
| Plan 카드 | §2.2 참조 |
