...
| **재시도 후 성공 축약** (✔ `retried`) | `add_edge` 실패가 `PORT_NOT_FOUND` 또는 `NODE_NOT_FOUND`, 또는 `add_node`/`update_node` 실패가 `INVALID_NODE_CONFIG` 에러코드를 가지고, 같은 편집 대상의 성공 배지가 **곧바로** 따라오면 두 배지를 하나의 "재시도 후 성공" 배지로 축약 렌더한다. 렌더 라벨은 성공한 호출 기준 + `retried` 꼬리표이며, 툴팁/hover 에 원본 실패 이유와 성공 시의 port 값을 함께 노출해 디버깅 정보를 보존한다. i18n `assistant.toolCallBadgeRetryRecovered`. **이 축약은 `PORT_NOT_FOUND` / `NODE_NOT_FOUND` / `INVALID_NODE_CONFIG` 코드에 한정**된다 (이 세 코드 모두 서버가 `knownPorts` / cascading 실패 FIFO / `invalidConfig` 세부 메시지로 "같은 턴 내 자연 복구" 를 설계한 경로). `LABEL_CONFLICT`·`CYCLE_DETECTED` 등 다른 shadow 에러는 기존처럼 빨간 배지 유지 — 사용자·디버거에게 명시적으로 보여야 유용하다 |
...
| `add_node` / `update_node` 의 최종 config 가 노드별 handler.validate 의 domain rule 을 통과해야 함 (버튼 수 상한, static/dynamic 필수 필드, 중복 id 등) | `{ok: false, error: 'INVALID_NODE_CONFIG', message: string, invalidConfig: string[]}` — `handler.validate(config).errors` 를 최대 5개까지 실어 LLM 이 같은 턴 내 해당 필드만 수정해 재시도할 수 있게 함. `update_node` 는 검증 실패 시 patch 전부 롤백 (부분 적용 없음). `add_node` 는 노드 저장 안 하고 `recordFailedAddNode` 에 label 을 기록해 이후 `add_edge` 가 존재하지 않는 UUID 를 참조하면 cascading NODE_NOT_FOUND 힌트가 실린다. tool-call-badge 의 `RECOVERABLE_ERROR_CODES` 에 포함되어 LLM 이 즉시 수정 성공 시 UI 가 retry-recovered 로 축약 렌더 |
