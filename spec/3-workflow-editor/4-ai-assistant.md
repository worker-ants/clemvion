...
code:
  ...
  - codebase/backend/src/nodes/core/button-slug.util.ts
  - codebase/backend/src/scripts/migrate-button-ids.ts
  - codebase/frontend/src/components/editor/assistant-panel/*.ts
  - codebase/frontend/src/components/editor/assistant-panel/*.tsx
  - codebase/frontend/src/lib/stores/assistant-store.ts
  - codebase/frontend/src/lib/stores/assistant-editor-bridge.ts
  - codebase/frontend/src/lib/api/assistant.ts
---
...
| Reversible 변경 | 에디터의 기존 Undo 스택과 저장 흐름(수동 `Ctrl+S`·실행 직전 저장)을 재사용한다. 사용자의 승인 없이는 DB에 영구 기록하지 않는다 |
...
| `verify_workflow` | `{verifiedNodeIds: string[], verifiedEdgeIds: string[], requestCoverage: string, concerns?: string[]}` | `{ok: true}` 또는 `{ok: false, error: 'VERIFY_INCOMPLETE', missingNodeIds, missingEdgeIds}` | 2-stage finish 의 **Phase 3** self-review 외부화 도구 (§10 "Workflow self-verify 가드"). LLM 이 자신이 명시적으로 검토한 node/edge id 를 보고하면 서버가 현재 캔버스의 모든 node/edge 가 포함됐는지 대조한다. 전부 커버되면 `ok:true` 로 self-review 완료 처리되어 다음 `finish` 가 통과하고, 누락이 있으면 `VERIFY_INCOMPLETE` + 누락 id 목록을 돌려줘 LLM 이 그 항목을 확인 후 재호출하게 한다. shadow 만 읽고 외부 자원 접근이 없는 read-only 도구라 `kind='explore'` 로 발행된다 |
...
| `running` / `waiting_for_input` | 현재까지 기록된 부분 timeline 을 그대로 반환. 아직 실행 안 된 노드는 생략, 진행 중인 노드는 `status: 'running'`/`waiting_for_input` 으로 표시. 응답의 `execution.finishedAt` · `durationMs` 는 `null`. §12.2 의 "실행 중 편집 도구 거부" (현재 미구현, 계획) 는 어느 경우에도 **read 도구에는 적용되지 않는다** — Assistant 는 실행 중인 워크플로에 대해서도 진단 목적의 조회를 수행할 수 있다 |
...
모든 편집 도구는 백엔드의 `ShadowWorkflow`에서 검증 후 성공 시 SSE 이벤트로 프론트에 전달된다.

> **인자 네이밍 관례 (혼재 — 실제 tool schema 기준).** `add_edge` 의 edge 끝점 인자는 **snake_case** (`source_id` / `source_port` / `target_id` / `target_port`) 이고, `add_node`/`update_node`/`remove_node`/`remove_edge` 의 식별·페이로드 인자(`id`, `type`, `patch`, `config`, `position` …) 와 모든 도구 공통의 plan-step 인자(`planStepId` / `planStepIds`) 는 **camelCase** 다. §4.4.1 의 hint 문구도 `source_id` / `target_id` (snake) 를 그대로 가리킨다. 본 spec 본문의 인자 표기는 `tool-definitions.ts` 의 실제 JSON Schema 키를 따른다.

모든 편집 도구는 `planStepId?: string` (단일, legacy) 와 `planStepIds?: string[]` (다중, 권장) 을 **선택 필드**로 공통 지원한다. 둘 다 지정된 경우 union 으로 집계되어 해당 step 들이 모두 ✓ 처리된다. 한 번의 tool-call 이 여러 step 을 cover 할 때는 배열을 사용하라 (예: `add_node` 의 config 에 버튼을 함께 넣어 `update_node` step 까지 해결한 경우 `planStepIds: ["s1","s3"]`).
...
**상한**: 단일 응답에서 한 쪽(`outputs` 또는 `inputs`) 최대 50개. dynamic 버튼을 극단적으로 많이 넣은 시나리오의 응답 폭주 방지. 상한에 걸려 절단된 경우 도구 응답 `result` 에 `portsTruncated?: true` 가 함께 실려 "서버가 일부 포트를 생략했다" 는 사실이 신호화된다 — 이 플래그가 켜지면 LLM 은 `ports` 에 없는 port id 를 추측하지 말고 `get_node_schema` 로 전체 목록을 조회해야 한다 (`shadow-workflow.ts`, frontend `lib/api/assistant.ts` 타입도 동일 전제. review W-5 — Rationale ED-AI-40 참조).
...
|-------------|-----------|-------------|
| `explore` | `get_node_schema`, `list_integrations`, `list_workflows`, `get_workflow`, `get_current_workflow`, `list_knowledge_bases`, `get_workflow_executions`, `get_execution_details`, `verify_workflow` | 메시지에 탐색 배지 추가 (§3.2) |
...
| `ASSISTANT_SESSION_NOT_YOURS` | 세션이 호출자 소유가 아님 (workspace/user 경계) | 접근 거부 |

> **(계획) 미구현 에러코드.** 다음 두 코드는 spec 이 정의했으나 현재 코드에 없다. ① `ASSISTANT_LLM_CONFIG_INVALID` — 지정 config 가 삭제·워크스페이스 밖인 경우. 현재는 별도 코드 없이 workspace default 로 폴백하고 폴백도 없으면 `ASSISTANT_NO_LLM_CONFIG` 로 떨어진다 (§12.3). ② `ASSISTANT_STREAMING_UNSUPPORTED` — 향후 스트리밍 미지원 provider 추가 시. v1 의 5개 provider (OpenAI / Anthropic / Google / Azure OpenAI / Local) 가 모두 ✅ 라 현재 발동 경로가 없다. (참고: LLM 모듈에는 별개의 `LLM_STREAMING_UNSUPPORTED` 가 존재하나 Assistant 전용 코드는 아니다.)
...
| 편집 결과 vs 저장 | Assistant가 수행한 편집은 editor-store에만 반영. **영구 저장은 사용자 `Ctrl+S`·`Save` 또는 실행 직전 저장을 통해서만** 일어남 → 실수로 닫아도 세션 시작 시점의 워크플로우 상태로 돌아갈 수 있음 |
...
- Assistant의 편집은 캔버스의 기존 저장 흐름(수동 `Ctrl+S`·실행 직전 저장)에 그대로 편입된다. 사용자는 Assistant 작업 중에 수동 저장(Ctrl+S)·실행(Run)을 언제든 할 수 있다.
...
| 항목 | 결정 | 근거 |
|------|------|------|
| 제품 명칭 | **Workflow AI Assistant** / 워크플로우 AI 어시스턴트 | PRD/Spec/i18n 전 영역에서 통일 사용 |
| 채팅 세션 영속화 | **서버 저장** (신규 엔티티 `AssistantSession`, `AssistantMessage`) | 페이지 새로고침·재접속 시 이어서 대화 지원. 관련: `spec/1-data-model.md` §2.20~2.21 |
| 변경 적용 방식 | 즉시 반영 + Undo (`editor-store` 재사용) | 기존 저장 흐름(`Ctrl+S`·실행 직전 저장)과 일관. DB 영구 기록은 사용자의 Save를 통해서만 |
| 스트리밍 | SSE + `LLMClient.stream()` 신규 메서드 | 관련: `spec/5-system/7-llm-client.md` §8 |
| 스트리밍 v1 지원 provider | OpenAI · Anthropic · Google · Azure OpenAI · Local 5종 모두 (§11) | 미지원 provider 선택 시 `ASSISTANT_STREAMING_UNSUPPORTED` 에러를 두기로 설계했으나 v1 의 5개 provider 가 모두 스트리밍을 지원해 발동 경로가 없어 **현재 미구현 (계획)** — §7 참조 |
...
채팅 히스토리는 서버에 영속화된다. 이를 위한 구성:

1. **DB 엔티티 2개**: `AssistantSession`, `AssistantMessage` (Flyway 마이그레이션 필요)
2. **REST API 5개**: `GET/POST/PATCH/DELETE /workflow-assistant/sessions`, `GET /workflow-assistant/sessions/:id`. SSE 엔드포인트는 `POST /workflow-assistant/sessions/:id/messages`.
...
### Workflow Assistant — 자체 점검 + 에러 풍부화

Assistant 가 복합 워크플로우 (예: 설문조사) 를 만들 때 발생하던 연쇄 tool call 실패를 줄이고, 완료 후 자체 점검을 수행하기 위한 결정·제약을 정리한다.
...
**verify_workflow 도구 도입 메모**: Phase 2 의 `WORKFLOW_VERIFY_REQUIRED` 는 prose-only 대조로 통과 가능하지만, LLM 이 실제로 모든 node/edge 를 훑었는지 서버가 검증할 수단이 없었다. `verify_workflow`(read-only, `kind='explore'`) 는 LLM 이 `verifiedNodeIds`/`verifiedEdgeIds` 를 명시 보고하게 해 누락(`VERIFY_INCOMPLETE` + `missingNodeIds`/`missingEdgeIds`) 을 구조적으로 catch 한다. snapshot 자체는 응답에 싣지 않는다 — Phase 2 의 `WORKFLOW_VERIFY_REQUIRED` payload 가 이미 `currentWorkflow` 를 동봉했기 때문.
...
### Workflow Assistant — 프로바이더 이상동작 대응 + review 항상 발동

다양한 LLM 프로바이더에서 관찰되는 이슈에 대한 대응을 정리.
...
> **본 가드는 "candidate 0 인 항목" 에만 발동한다.**
...
##### 증상
복합 설문조사 워크플로우 요청 → gemini-3-flash-preview 가
...
#### 11. NODE_NOT_FOUND label-lookalike hint
...
#### 10. Stall 자동 복구 UX — 메시지 박스 분리 + `auto_resume` SSE 이벤트
...
### Workflow Assistant — Candidate Picker 정책 결정

#### 배경

`integration-selector` 등 user-action widget 의 id 는 LLM 이 추측·주입할 수 없으므로(시스템 프롬프트가 금지), 워크스페이스에 사용 가능한 후보(예: 단일 SMTP integration)가 있어도 자동 연결되지 못하고 사용자에게 수동 설정을 미루는 문제가 있었다. 이를 해소하기 위해 in-message picker 정책을 도입한다.

#### 최종 정책 (ED-AI-39)

**"설정 가능한 항목이 존재하면 사용자에게 명시적 확인 후 주입, 없으면 기존 안내 유지"**:
...
어시스턴트가 자동 생성한 표현식이 분기 `null`로 터지는 이슈에서 출발해, 어시스턴트가 실행 결과를 읽고 원인을 진단·수정해 유사 실수의 셀프 복구를 가능하게 하려는 의도로 실행 조회 도구를 도입한다.
...
### Workflow Assistant — Runtime ports hint (ED-AI-40)

#### 배경

Assistant 가 `add_node` 직후 `add_edge` 를 시도할 때 두 가지 패턴으로 실패가 자주 일어난다.
...
5. 상한 50 (한 쪽당). 초과는 truncate 하고 `result.portsTruncated: true` 를 동봉해 절단 사실을 신호화한다 — 플래그가 켜지면 LLM 은 누락 port id 를 추측하지 말고 `get_node_schema` 로 전체 목록을 조회한다. *(최초 결정은 "플래그 없이 그냥 자른다 (현실 발생 시나리오 없음)" 였으나 review W-5 로 절단 신호화로 번복 — `shadow-workflow.ts` 구현·frontend `assistant.ts` 타입·`shadow-workflow.spec.ts` 회귀 테스트 모두 플래그 동봉 기준. §4.3.2 동기 갱신.)*
...
- `pendingUserConfig` 가 result 에 실리는 것과 동일 채널을 쓴다 (tool_result 에 함께 embed).
- DANGLING_OUTPUT_PORTS review guard 는 그대로 유지. 이번 변경은 "실패→복구" 라운드를 줄일 뿐 guard 자체와는 무관.
