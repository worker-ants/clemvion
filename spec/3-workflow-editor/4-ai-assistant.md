...
- `propose_plan.openQuestions`가 있으면 Plan 카드 안에 질문 목록과 함께 **인라인 답변 입력창**이 노출된다. 사용자가 답변을 입력·전송하면 일반 사용자 메시지로 서버에 전달되어 LLM이 계속 진행할 수 있다. 답변 전송 경로는 입력창 하단의 일반 메시지 입력과 동일한 턴 사이클을 탄다.
...
| `planStepId` | `string?` | `kind='edit'` 일 때 LLM 이 매칭한 plan step id (선택) |
...
> `finish` 도구는 대화 루프 종료 시그널로만 쓰여 SSE 로 발행되지 않는다. `propose_plan` 역시 `tool_call` 이 아닌 전용 `event: plan` 으로 발행된다.
