...
| 도구 | 인자 | 반환 |
|------|------|------|
| `add_node` | `{type, label, position: {x, y}, config, planStepId?, planStepIds?}` | `{ok, id?, error?, pendingUserConfig?}` — `pendingUserConfig` 는 §4.3.1 참고 |
| `update_node` | `{id, patch: {label?, config?, position?}, planStepId?, planStepIds?}` | `{ok, error?, pendingUserConfig?}` — `pendingUserConfig` 는 §4.3.1 참고 |
...
| `result` | `unknown` | 도구 실행 결과. 편집 도구는 `{ok, id?, error?, pendingUserConfig?, ...}` (pendingUserConfig 는 §4.3.1), 탐색 도구는 도구별 응답 shape |
