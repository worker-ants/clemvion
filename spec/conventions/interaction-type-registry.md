...
| `buttons` | engine waiting emit `'buttons'` (presentation 노드 + 버튼; via `ButtonInteractionService` — C-1 분할 후 엔진 위임) | (a)~(e) 동등 + `pauseForButtons` |
| `ai_conversation` | ai-agent.handler multi-turn waiting → `AiTurnOrchestrator.emitAiWaitingForInput` (interactionType meta; C-1 분할 후 엔진 위임) | (a)~(f) 동등 + `pauseForConversation` + conversation timeline hydration |
