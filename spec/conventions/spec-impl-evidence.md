...
| `code` | string[] (glob 허용) | status 별 다름 — §3 | 본 spec 이 약속한 surface 의 구현 경로. 레포 루트 기준 상대경로. **예외 — 시행 코드가 없는 순수 문서형 convention**: 강제하는 가드가 없어 "구현 경로" 가 성립하지 않으면 **그 규약을 실제로 지키는 예시 파일**을 적는다 (선례: [`review-citations.md`](./review-citations.md) — 주석 형태를 강제하는 가드가 없다). 넓은 트리 글롭으로 가드만 통과시키는 것은 아무것도 가리키지 않는 것과 같다 |
