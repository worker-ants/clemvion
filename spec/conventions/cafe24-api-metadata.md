...
Cafe24 노드 핸들러와 `Cafe24McpBridge` 가 [Spec 통합 §4.6 Recent activity 탭](../2-navigation/4-integration.md#46-recent-activity-탭) 의 `api_label` 컬럼에 적재하는 catalog key 형식은 다음과 같다:
...
| 2026-05-28 | **§7.5 신설** — 활동 로그 `api_label` 의 catalog key 형식 (`cafe24.<resource>.<operation>`) 명문화 + frontend 가 catalog endpoint 응답(`labelKey`/`descriptionKey`) 과 i18n dict 를 결합해 사람 친화 라벨을 렌더한다는 책임 분리 명시. [`spec/2-navigation/4-integration.md §4.6 / §9.3`](../2-navigation/4-integration.md#46-recent-activity-탭) 및 `_product-overview.md INT-US-05` 와 한 세트. 결정 근거는 [Spec 통합 ## Rationale "활동 로그 API 식별 — 3컬럼 (label/method/path) + catalog endpoint 신설 (2026-05-28)"](../2-navigation/4-integration.md#rationale). |
| 2026-05-28 (label 제거) | **§2 `Cafe24OperationMetadata.label` 필드 제거** — 직전 row 의 catalog key 분리 마무리. 한국어 hardcoded `label: string` 을 backend metadata 에서 완전 제거하고, 노드 에디터 operation 드롭다운까지 frontend i18n dict (`cafe24Catalog.<key>`) lookup 으로 일원화. `/nodes/definitions` 응답의 cafe24 extras (`operationsByResource[].label` → `labelKey`) 필드명 변경 — frontend ↔ backend 동시 머지 필요 (호환성 단절). §3 예시 3 row 의 `label:` 라인 제거. §7.5 책임 분리 표에 "노드 에디터 operation 드롭다운 노출" 행 추가 (catalog key 단일 형식). 결정 근거는 본 문서 ## Rationale "backend `label` 필드 제거 — frontend i18n dict 단일 SoT (2026-05-28)". |
| 2026-05-22 (G-1 store + supply + impliesValue) | **store 13건**: paymentgateway_*(2 + 3 sub) placeholder rename (paymentgateway_id→client_id, paymentmethod_id→payment_method_code), paymentgateway_paymentmethods_list (gateway-scoped path), paymentmethods_paymentproviders_list/update_display (per-method scope, {code}/{name} placeholders), redirects_update/delete (redirect_no→id), subscription_shipments_setting_update/delete (subscription_no 추가). socials_apple_settings_get docs 부재 JSDoc 경고. **supply 5건**: suppliers_users_regional_*(5건 모두) — regionalshippingfees→regionalsurcharges, {user_id}→{supplier_id} (cafe24 API placeholder inconsistency), settings→setting, regional_delete 에 {regional_surcharge_no} 추가. **§2 `Cafe24FieldConstraint.impliesValue` kind 신설** — value-aware implies (`{ kind: 'impliesValue', if, value, then }`). cafe24 docs 의 "X='T' 일 때 Y 필수" 패턴 (예: refund_method=T 시 bank fields 필수, material_composite=T 시 material+content_rate 필수) 의 spec 표현 가능. 두 채널 노출 — runtime validator (constraint-validator.ts 에 impliesValue branch, 6 unit test 추가), MCP description suffix (`constraintToSuffixLine` 에 impliesValue case 추가). JSON Schema 변환은 다른 kind 와 동일하게 안함 — LLM tool-call validator 들이 value 조건 implication 을 일관 처리 못 함. metadata.spec.ts 의 constraints invariant 도 impliesValue case 포함. **실제 metadata 적용은 follow-up PR** (해당 ops 의 field-set 확장 선행 필요). |
...
### backend `label` 필드 제거 — frontend i18n dict 단일 SoT (2026-05-28)

**문제**: 직전 PR (`integration-activity-api-label`, 2026-05-28) 에서 활동 로그 라벨의 i18n 책임 분리를 결정하고 (DB 는 catalog key, frontend dict 가 KO/EN 사람 친화 라벨 SoT), §7.5 에 책임 분리 표를 신설했다. 그러나 `Cafe24OperationMetadata.label: string` (한국어 hardcoded — 예: `'상품 목록 조회'`) 은 그대로 두고 본 PR 에서 점진 이주하기로 했었다. 잔여 노출 경로는 두 곳 — (1) `/nodes/definitions` 응답의 `extras.operationsByResource[].label` 이 frontend `integration-configs.tsx` 의 cafe24 operation 드롭다운 렌더에 그대로 사용되고, (2) `metadata/planned.ts` 의 type 정의가 `label` 을 유지. 영문 UI 사용자가 cafe24 노드를 편집하면 드롭다운 옵션이 한국어로 노출되는 회귀가 운영에서 가능.
...
**대안**:
- (A, 기각) **`label` 을 deprecated 로만 표기, 점진 이주** — 한국어 hardcoded 의 잔존 기간이 늘면 그 사이 사용자 영향 (영문 UI 에 한국어 라벨) 이 계속 발생. 옛 label 과 신규 labelKey 가 동시 존재하면 frontend 가 둘 중 무엇을 선호하는지 분기 추가 필요 → drift 가능.
- (B, 기각) **노드 에디터 드롭다운만 dict lookup 으로 옮기고 backend `label` 은 유지** — `/nodes/definitions` 응답 shape 은 그대로 보존, 단 frontend 가 무시. backend metadata 의 한국어 hardcoded 가 dict 와 drift 가능 (한 쪽만 갱신 시 운영자도 어느 쪽이 SoT 인지 헷갈림). spec §7.5 의 "frontend i18n dict 가 SoT" 약속이 backend metadata 와 어긋남.
- (C, 채택) **완전 제거 + 응답 필드명 변경** — backend metadata 가 영문 식별자 (id, catalog key) 만 보유. 한국어/영문 라벨은 dict 하나가 SoT. drift 가능성 구조적 소거. frontend ↔ backend 동시 머지 필요한 wire-format 단절은 작은 비용 (cafe24 단일 노드, 소비처 2곳).

**근거**: 사용자 결정 (2026-05-28) — 직전 PR 의 i18n 책임 분리 결정의 마무리. 영문 UI 사용자에게 한국어 hardcoded 가 노출되는 회귀를 영구 차단하고 backend metadata 의 SoT 책임을 식별·매핑(영문 ID) 에만 한정. plan: [`plan/in-progress/cafe24-mcp-label-i18n.md`](../../plan/in-progress/cafe24-mcp-label-i18n.md).
