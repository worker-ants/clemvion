...
**책임 분리** — DB 는 catalog key (single source of truth) 만 저장하고, frontend 가 i18n dict 와 결합해 사람 친화 라벨을 렌더한다:
...
| i18n 변환 | frontend i18n dict | `labelKey` → KO/EN 라벨 (예: `"상품 목록 조회"` / `"List products"`) |

**왜 이 분리가 필요한가**: backend 가 i18n 결과를 DB 에 직접 적재하면 (a) 사용자 UI 언어 변경 시 DB 의 옛 언어 라벨이 새 UI 에 그대로 노출되는 회귀가 영구화되고, (b) catalog 라벨 수정 시 DB 의 옛 라벨이 stale 해진다. catalog key 만 영속화하고 i18n 을 매 응답 시점에 수행하면 두 회귀 모두 발생하지 않는다. 결정 근거 상세는 [Spec 통합 ## Rationale "활동 로그 API 식별 — 3컬럼 (label/method/path) + catalog endpoint 신설 (2026-05-28)"](../2-navigation/4-integration.md#rationale) 참조.
