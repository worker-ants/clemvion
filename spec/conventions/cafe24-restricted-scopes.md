...
- **(A) 사용자가 체크 시 차단** — 이미 본사 승인을 받은 합법 사용자 케이스를 막아버린다. "안내만, 차단 없음" 정책 채택. 단 체크된 항목 중 1개 이상이 별도 승인 대상이면 inline alert (warning, amber — `spec/0-overview.md §3.4`) 로 인지를 강제한다.
