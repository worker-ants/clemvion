...
`mall.read_store` / `mall.write_store` 자체는 일반 사용 가능하지만, 안의 일부 operation 만 별도 승인 대상이다. 카탈로그 표에서 해당 row 만 `restricted: operation` 으로 표기한다.
...
4. **AI Agent allowlist UI** ([Spec Cafe24 노드 §8.3](../4-nodes/4-integration/4-cafe24.md#83-allowlist-mcpserversenabledtools))

배지 hover 시 tooltip 문구. SoT 는 i18n 사전 `integrations.approvalRequiredTooltip` 키 (`codebase/frontend/src/lib/i18n/dict/{ko,en}/integrations.ts`) 다 — 아래는 그 값의 인용이며 spec 본문이 아니라 i18n 사전이 진실원이다.

한국어 (`ko/integrations.ts` `approvalRequiredTooltip`):

> "카페24 본사 승인이 필요한 권한입니다. 미승인 상태로 동의를 시도하면 invalid_scope 로 실패하거나, 인증 후 호출 시 403 이 반환될 수 있어요."

영어 (`en/integrations.ts` `approvalRequiredTooltip`):
...
개발자센터 문의 링크("Cafe24 개발자센터 문의 →" / "Contact Cafe24 Developer Center →", i18n 키 `integrations.approvalInquiryLink`) 는 tooltip 본문에 포함되지 않고, §4.2 의 inline 알림(`RestrictedScopeNotice`) 안에 별도 앵커로 렌더된다.

### 4.2 차단 정책

체크/저장은 **차단하지 않는다**. 이미 본사 승인을 받은 사용자가 있을 수 있으므로 "알고 누른다" 만 보장. 단 체크된 권한 중 별도 승인 필요 항목이 1개 이상이면 위저드 Step 2 폼 하단에 **inline alert (warning, amber 톤 — [`spec/0-overview.md §3.4`](../0-overview.md#34-상태-표시-패턴))** 를 영구 표시한다 (사용자가 인지하지 못한 채 진행하는 사례 차단).
...
5. `pnpm --filter backend test -- catalog-sync` 로 양방향 동기 확인.
...
- **차단 대신 안내**: 이미 본사 승인을 받은 합법 사용자 케이스를 막지 않기 위해 "안내만, 차단 없음" 정책을 택한다. 단 체크된 항목 중 1개 이상이 별도 승인 대상이면 inline alert (warning, amber — `spec/0-overview.md §3.4`) 로 인지를 강제한다.
- **신규 에러 코드 없이 보강 필드만**: 새 에러 코드는 기존 `INSUFFICIENT_SCOPE (403)` / OAuth `invalid_scope` 처리 경로를 분기시켜 client 코드 호환성에 영향을 주므로, `details.requiresCafe24Approval` 보강 필드만으로 표현한다.
- **`status` enum 이 아닌 별도 `restricted` 컬럼**: `restricted` 는 `supported` / `planned` / `deprecated` 와 직교 차원이라 `status` enum 확장은 의미 오염이다. 별도 컬럼이 정답이며, catalog _overview §2 의 `restricted` 컬럼 설명에도 "이 컬럼은 `status` 와 직교하며 `status` 의 값이 아니다" 로 명시한다.
- **명단을 spec 본문에 직접 enumerate 하지 않음**: drift 위험을 피하기 위해 `cafe24-api-metadata.md` §3 의 "endpoint enumeration 을 spec 본문에 인라인하지 않는다" 정책과 동일 사유로 본 컨벤션 파일 하나에 집중한다.
