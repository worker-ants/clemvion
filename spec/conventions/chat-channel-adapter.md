...
type EiaEvent =
  ...
  | { type: "execution.ai_message";        /* EIA §6.5 (ai_message) + WS §4.4 */ executionId: string; triggerId: string; workflowId: string; message: string; turnCount: number; messages: unknown[]; metadata?: unknown; /* debug 전용 llmCalls 는 fanout strip 되어 어댑터 미수신 (WS §4.4 strip-only) */ /** AI Agent `render_*` 표현 도구 호출 turn 에서만 동봉. SoT: [Spec AI Agent §7.10](../4-nodes/3-ai/1-ai-agent.md#710-presentation-payload-render_-운반) / [EIA §6.5](../5-system/14-external-interaction-api.md#65-페이로드--executioncancelled--executionai_message). */ presentations?: PresentationPayload[]; timestamp: string; seq: number }
  // 종결 3종의 필드 집합·행동 계약 SoT: EIA §6 도입부 (아래 인용 링크). 여기 다시 열거하지
  // 않는다 — 같은 계약을 두 곳에 적어 두었더니 실제로 어긋났고, 그 drift 를 고치는 것이
  // 이 축약의 목적이다. 봉투는 flat(WS 계열) 이고 result 중첩은 유지된다.
  | { type: "execution.completed";        /* EIA §6.3 */ executionId: string; triggerId: string; workflowId: string; status: "completed"; result?: { outputs?: unknown }; durationMs?: number | null; timestamp: string; seq: number }
  | { type: "execution.failed";           /* EIA §6.4 */ executionId: string; triggerId: string; workflowId: string; status: "failed"; error: { code: string | null; message: string; nodeId: string | null; details?: unknown } | string; durationMs?: number | null; timestamp: string; seq: number }
  | { type: "execution.cancelled";        /* EIA §6.5 (cancelled) */ executionId: string; triggerId: string; workflowId: string; status: "cancelled"; result?: { cancelledBy?: "user" | "system" | "timeout" }; error?: { code: string; message?: string }; durationMs?: number | null; timestamp: string; seq: number };
```

내부 필드의 SoT 는 EIA §6 의 각 페이로드 형식. 본 컨벤션은 어댑터 입력으로 union 만 정의.
종결 3종은 [EIA 필드 집합](../5-system/14-external-interaction-api.md#종결-이벤트의-필드-집합-normative) ·
...
interface SetupResult {
   ...
   * 않는다 — 사용자 입력값이 직접 caller 의 `SecretResolver.rotate` 로 들어가고 ref 만 config 에 set.
   */
...
4. client-side 검증 (전 필드 type/pattern/minLength·maxLength·min/max(숫자 범위) — provider native 검증 우선 활용 후 어댑터 schema 검증)
   → 통과 시 EIA submit_form (data: fields) 단일 호출.
5. server-side 검증 실패 (400 `VALIDATION_ERROR` + `error.details[{field,message,code}]`) → modal 을 error 와 함께 재표시:
...
3. 필드 단위 클라이언트-side 검증 (type / pattern / minLength·maxLength / min/max(숫자 범위) 등 schema 차원)
...
5. EIA 가 server-side 검증 실패 (400 `VALIDATION_ERROR` + `error.details[{field,message,code}]`) 응답 시
   → `details[0].field` 를 currentFieldIdx 로 되돌리고 그 필드 재질문
...
- `parseUpdate` 는 raw body 의 trust 검증 후 호출 — 진입점 핸들러가 [WH-SC-02](../5-system/12-webhook.md#42-hmac-서명--authconfigtypehmac) 또는 provider 별 secret token (텔레그램 `X-Telegram-Bot-Api-Secret-Token`) 검증을 선행.
