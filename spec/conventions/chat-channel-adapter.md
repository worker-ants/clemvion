...
| `ackInteraction` | provider 가 요구하는 ack (텔레그램 `answerCallbackQuery`). provider 에 따라 noop 가능 — 함수 자체는 의무지만 구현체는 비어 있을 수 있음. **§4.1 native modal 게이팅**: `form_modal` 버튼 클릭 interaction 도착 시 modal open (Slack `views.open(trigger_id, view)` / Discord interaction response `type: 9` MODAL) 호출은 본 함수 안에서 수행한다 — trigger_id (3초) / interaction token 가 본 interaction 시점에만 가용하므로 ack 경로가 자연스러운 거처 (별 함수 신설 회피, R-CCA-8) | 외부 API 호출 (provider 의존) | yes |
...
**`form_modal` 의 의미**: `execution.waiting_for_input` (interactionType=form) 이 §4.1 native modal 경로로 분기될 때 어댑터 `renderNode` 가 합성하는 "양식 작성하기" 버튼 메시지 1건이다. server-initiated push 시점에는 trigger_id/interaction token 이 없어 modal 을 즉시 못 열기 때문에 (§R-CCA-8 대안 4), 버튼으로 사용자 클릭 interaction 을 유도한 뒤 그 시점에 `ackInteraction` 이 modal 을 연다 (§1.1). 필드 구성은 다음과 같다:
...
2. 사용자가 버튼 클릭 → provider interaction 도착
   (Slack block_actions: trigger_id 3초 / Discord MESSAGE_COMPONENT: interaction token 15분)
   → ackInteraction 이 즉시 modal open
...
- (b) **함수 개수 6 유지**: modal open 은 새 함수가 아니라 `ackInteraction` 내부 분기로 흡수 (§1.1) — [R-CCA-5 대안 2](#r-cca-5) / [R-CCA-7 대안 2](#r-cca-7) 의 "새 함수 추가 = 인터페이스 drift" 정신 보존. `supportsNativeForm` 은 함수가 아닌 capability 플래그.
