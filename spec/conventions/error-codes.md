...
| `INVALID_PASSWORD` | 401 | 이름은 *"입력한 비밀번호가 틀렸다"* 만 말하나, `users.service.changePassword` 는 **비밀번호가 아예 없는 계정**(OAuth-only, `passwordHash` 부재)에도 같은 코드를 던진다 — **이름이 실제 조건보다 좁다**. 형제 흐름 둘은 그 구분을 코드로 가른다: `AuthService.verifyPasswordForUser` = `PASSWORD_REQUIRED`(미설정·미입력) / `PASSWORD_INVALID`(불일치), `SessionsService.verifyReauth` = `REAUTH_REQUIRED`(400, 미입력) / `PASSWORD_INVALID`(401, 불일치). 정정 이득은 **이름 정확성뿐**이라 §2 가 이미 rename 을 금지한다. 게다가 `POST /users/me/change-password` 는 워크스페이스 JWT 로 호출 가능한 내부 REST 라 저장소 밖 호출자를 원리적으로 배제할 수 없는 **§5 등급 B 표면**이다 — 저장소 grep 에서 client 분기가 안 나온다는 것은 *"부재"* 가 아니라 *"미발견"* 이므로 rename 은 사용자 결정을 요구한다 | `POST /users/me/change-password` 의 현재 비밀번호 재확인 실패 — **미설정(OAuth-only) 또는 불일치**. **근접 명명 3중 주의**: (a) `PASSWORD_INVALID` — 재인증·민감 동작 재확인의 불일치 코드. 단어 순서만 다른 **별개 wire 코드**이며 그 이름은 정확하므로 본 레지스트리 대상이 아니다. (b) `PASSWORD_REQUIRED` — 그 형제의 미설정 케이스. (c) `login_history.failure_reason = 'INVALID_PASSWORD'` — 로그인 실패의 **감사 사유값**으로 wire 코드가 아닌 다른 레이어다. **미설정 조건을 별도 코드로 분리할지는 미결**이며 [`auth-change-password-oauth-only-code-split.md`](../../plan/in-progress/auth-change-password-oauth-only-code-split.md) 에서 결정한다 | [`1-auth.md §2.3`](../5-system/1-auth.md#23-세션-정책) · [`3-error-handling.md §1.2`](../5-system/3-error-handling.md#12-인증인가-에러) |
