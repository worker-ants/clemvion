...
| `INVALID_PASSWORD` | **조건별 2종** — `PASSWORD_REQUIRED`(비밀번호 미설정, OAuth-only) · `PASSWORD_INVALID`(현재 비밀번호 불일치) | 401 | [`auth-change-password-oauth-only-code-split.md`](../../plan/in-progress/auth-change-password-oauth-only-code-split.md) | **[등급 B — 잔여 위험 인수]** `POST /users/me/change-password` 가 **두 조건에 같은 코드**를 던지던 것을 형제 흐름(`AuthService.verifyPasswordForUser`)과 동일한 두 코드로 갈랐다 — **신규 코드는 만들지 않았다.** `PASSWORD_REQUIRED` 가 이미 그 조건을 정의하고 있었고, 신설(`PASSWORD_NOT_SET` 검토됨)은 근접 명명을 3종→4종으로 늘리는 데다 그 이름이 **이미 `login_history.failure_reason` 감사값으로 존재**해(`auth.service.ts`) wire/audit 동명 충돌을 재생산했을 것이다. **⚠️ 등급 B 인 이유**: 워크스페이스 JWT 로 호출 가능한 내부 REST 라 저장소 밖 호출자를 배제할 수 없다 — 프론트엔드는 이 코드로 분기하지 않으나(전수 grep 0건) 그것은 *부재*가 아니라 *미발견*이다. **사용자 결정 2026-09-02**. **⚠️ 문자열은 남는다**: `INVALID_PASSWORD` 는 `login_history.failure_reason` 감사값으로 계속 발행된다 — 다만 그것을 남기는 것은 **로그인 실패**(`AuthService.login`, `event: 'login_failed'`)이지 비밀번호 변경이 아니다(`users` 모듈은 `login_history` 를 쓰지 않는다). 감사값 정리는 `login_history` event/reason enum 변경을 동반하는 **별개 작업**이다 ([1-data-model §2.18.2](../1-data-model.md)) |
