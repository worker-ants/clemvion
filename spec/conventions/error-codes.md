...
| `invitation_not_found` · `invitation_expired` · `invitation_already_used` · `invitation_email_mismatch` · `rate_limited` | 404/410/400/403/429 | `lower_snake_case` — §1 `UPPER_SNAKE_CASE` 위반. 워크스페이스 초대 흐름 v1 출하 시 이 형태로 정착, 프론트(`invitations.ts` `INVITATION_ERROR_CODES`)·백엔드(`workspace-invitations.service.ts` / `auth.service.ts`)가 `code` 값으로 분기 → rename = breaking(§2) | 초대 토큰 부재/만료/사용됨/이메일 불일치/rate-limit. **초대 API 한정** — 본 `rate_limited` (lowercase) 는 초대 흐름 전용 historical artifact 로, 다른 영역의 `UPPER_SNAKE_CASE` 범용 코드와 별개다. (2026-09-25 `forbidden` 을 이 행에서 뺐다 — 발행처가 없었다(`code: 'forbidden'` 0건). 초대 권한 부족은 `RolesGuard` 의 `ADMIN_REQUIRED`) | [`1-auth.md §1.5.4`](../5-system/1-auth.md#154-에러-응답) |
...
| `AbortError` | — (HTTP 무관 — 노드 레벨 `error.code`, NodeExecution `cancelled`) | PascalCase — §1 `UPPER_SNAKE_CASE` 위반. 다만 이 값은 우리가 정한 코드가 아니라 **웹 표준 `AbortSignal`/`AbortController` 가 던지는 `DOMException.name`** 을 그대로 봉투 `code` 로 옮긴 것이다. fetch·각 SDK·DB driver 가 전부 이 이름으로 던지므로 정규화하면 엔진의 분류 키(`error.name === 'AbortError'`)와 봉투 `code` 가 갈라져 두 이름을 동시에 유지해야 한다. 다수 핸들러·테스트가 값으로 참조 → rename = breaking(§2) | 노드 외부 I/O 가 `abortSignal` 로 중단됨 → `NodeExecution.status = cancelled`. **엔진이 던지는 `ExecutionCancelledError`(§2.4 DB 관측 가드)는 별개 sentinel** 이며 같은 상태로 귀결되지만 봉투 `code` 를 싣지 않는다 | [`node-cancellation.md §2.4·§5.1`](./node-cancellation.md#5-aborterror-분류) |
...
**발행된 적 없는 코드는 이 표에 오지 않는다.** 코드베이스가 한 번도 내지 않았는데 spec 표에만 등재돼 있던 코드(spec-drift)는 rename 이 아니라 기록 오류다 — 등재 자체를 지우고, 지운 자리의 비고에 실측을 남긴다(예: 2026-09-25 `forbidden` — `code: 'forbidden'` 발행처 0건. 처리 기록은 §3 초대 모듈 코드 행의 비고에 있다).

**"제거" 는 wire 발행 중단을 뜻한다.** 같은 문자열이 **다른 레이어**(감사 사유값·엔진 내부 분류 등)에 계속 살아 있을 수 있고, 그런 잔존은 각 행의 **비고**에 적는다 — 머리말에 일반화해 실으면 그 사실이 없는 행까지 매번 재검증하게 만든다. 또한 교체는 1:1 이 아닐 수 있다 — 하나의 구 코드가 **조건별 복수 코드**로 갈릴 수 있으며 그때는 대체 코드 셀에 조건을 함께 적는다.
...
> **현재 B 등급 행은 2건이다** — `INVALID_INPUT`→`INVALID_TRIGGER_PARAMETERS`(2026-08-22, `#1193`) 와
> `INVALID_PASSWORD`→`PASSWORD_REQUIRED`/`PASSWORD_INVALID`(2026-09-02). 아래 문단이 요구하는 대로
> **개수를 세어 두는 자리**이며, 새 B 행을 추가하는 사람은 이 숫자도 함께 갱신한다.
>
...
| `INVALID_PASSWORD` | **조건별 2종** — `PASSWORD_REQUIRED`(비밀번호 미설정, OAuth-only) · `PASSWORD_INVALID`(현재 비밀번호 불일치) | 401 | [`auth-change-password-oauth-only-code-split.md`](../../plan/complete/auth-change-password-oauth-only-code-split.md) | **[등급 B — 잔여 위험 인수]** `POST /users/me/change-password` 가 **두 조건에 같은 코드**를 던지던 것을 형제 흐름(`AuthService.verifyPasswordForUser`)과 동일한 두 코드로 갈랐다 — **신규 코드는 만들지 않았다.** `PASSWORD_REQUIRED` 가 이미 그 조건을 정의하고 있었고, 신설(`PASSWORD_NOT_SET` 검토됨)은 근접 명명을 3종→4종으로 늘리는 데다 그 이름이 **이미 `login_history.failure_reason` 감사값으로 존재**해(`auth.service.ts`) wire/audit 동명 충돌을 재생산했을 것이다. **⚠️ 등급 B 인 이유**: 워크스페이스 JWT 로 호출 가능한 내부 REST 라 저장소 밖 호출자를 배제할 수 없다 — 프론트엔드는 이 코드로 분기하지 않으나(전수 grep 0건) 그것은 *부재*가 아니라 *미발견*이다. **사용자 결정 2026-09-02**. **⚠️ 문자열은 남는다**: `INVALID_PASSWORD` 는 `login_history.failure_reason` 감사값으로 계속 발행된다 — 다만 그것을 남기는 것은 **로그인 실패**(`AuthService.login`, `event: 'login_failed'`)이지 비밀번호 변경이 아니다(`users` 모듈은 `login_history` 를 쓰지 않는다). 감사값 정리는 `login_history` event/reason enum 변경을 동반하는 **별개 작업**이다 ([1-data-model §2.18.2](../1-data-model.md)) |
