...
### 3.1 내부 전용 legacy 분류 코드 (정규화 후 발행)

다음은 Code 노드 핸들러 내부의 **분류 단계 문자열**이다. `classifyCodeNodeError` 가 산출하지만
`output.error.code` 로 직접 발행되지 않으며, `LEGACY_TO_NORMALIZED` 표가 **발행 직전 public 코드로
정규화**한다 (`codebase/backend/src/nodes/data/code/code.handler.ts`). 따라서 클라이언트 계약(§2)에
영향을 주지 않는 **구현 내부 명칭**이며, 디버깅 용도로만 `output.error.details.legacyCode` 에 보존된다.
정식 public 코드는 우측 열이다 — 명명 정확성 향상을 위한 internal rename 은 안전(클라이언트 미노출).

| 내부 분류 코드 (legacy) | 정규화 → public 코드 | 의미 | 근거 |
|---|---|---|---|
| `EXECUTION_TIMEOUT` | `CODE_TIMEOUT` | Code 노드 스크립트 wall-clock timeout | [`3-error-handling.md §1.4`](../5-system/3-error-handling.md#14-워크플로우-실행-에러) · [`2-code.md §5.3`](../4-nodes/5-data/2-code.md) |
| `EXECUTION_MEMORY_EXCEEDED` | `CODE_MEMORY_LIMIT` | isolate 128MB 하드 리밋 초과 (isolated-vm hard-kill) | 위와 동일 |
| `CODE_RUNTIME_ERROR` | `CODE_EXECUTION_FAILED` | 그 외 사용자 코드 런타임 throw (fallback) | 위와 동일 |
