...
### Principle 6-B — 내부 SoT 노출 금지 (2026-05-26)
...
근거: PR #332 회수 패턴 — 사용자가 추후 갱신 예정 안내·내부 SoT 참조에 혼란을 보고. 가드: `no-internal-refs.test.ts` (결정성 있는 부분만). 로드맵성 문구는 자동 검출이 어려워 `user-guide-writer` agent 와 사람 검수에 의존.
...
- **개념 설명 절** (워크플로우 디자인, 데이터 모델, 표현식 의미 설명 등) 은 여전히 자동 검출 불가. 본 규약은 코드 리뷰 / consistency-check 의 점검 가이드로 사용된다.
