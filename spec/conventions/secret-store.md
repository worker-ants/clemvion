...
>    입력한 외부 자격증명이 아니다.
>
>    > **정정 (2026-09-08)**: **이 창은 닫혔다.** `#1291` 이 응답 경계 스트립
>    > (`TriggersService` 의 `TRIGGER_RESPONSE_STRIP_COLUMNS`)을 세웠고, 스케줄 조인 축은
...
>    >
>    > **이 등재는 그 상태를 승인하지 않았다.** 예외의 대상은 *"컬럼에 평문으로 보관"*
...
> **`User` 민감 컬럼은 본 절이 아니라 [데이터 모델 §2.1.1](../1-data-model.md#211-응답-노출-금지-민감-7컬럼)
> 이 소유한다.** 같은 금지(응답 DTO 선언·바디 노출 불가)와 같은 근거(`select: false` 는 값을
> 읽는 내부 경로를 fail-silent 로 만든다)가 걸리지만, 그 컬럼들은 **provider 자격증명이 아니고
> `secret://` 스킴에 살지 않는다.** 본 컨벤션의 관할을 그쪽으로 넓히지 않는다 — 넓히면 이
> 문서가 스스로 선언한 범위(Overview: *"외부 provider 자격증명의 보관 추상화"*)를 벗어난다.
