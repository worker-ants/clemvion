...
>
> **위 `AuthConfig.config` 예외와 같은 종류가 아니다.** 그쪽은 *"다른 메커니즘으로 **동등하게 암호화**된다"* 가 근거지만, 이 필드는 **암호화 자체가 없다.** 근거를 따로 세운다 — (a) 요청마다 timing-safe 비교하는 **hot-path bearer 토큰**이라 매 요청 복호화 또는 별도 캐시 계층을 요구한다, (b) revoke 가 **값 교체(rotation)** 로 즉시 무효화되어 `secret_store` 의 버전 관리 이점이 작다, (c) 값 공간이 서버 발급 랜덤 hex(`itk_` + 32 bytes)로 닫혀 있고 발급 응답에 **1회만** 노출되므로, 사용자가 입력한 외부 서비스 자격증명과 위험 프로파일이 다르다 (유출 시 영향 범위가 해당 트리거 하나로 한정된다).
>
