...
| I6 | Multi-turn 대기 시 `output.conversationConfig.message` 를 렌더링용 뷰로 사용 — 다른 blocking 노드는 `output.view.type` 으로 통일 예정 | **P4.3** | High |
...
| I8 | `output.interactionType: 'ai_conversation'` 문자열이 Case 2/3/4 모두에 중복 — `output.view.type` 으로 충분 | P4.3, P8 | Medium |
...
**원칙**: "LLM 계열 노드는 `output.result` 아래에 도메인 결과를 모은다" (Principle 8). 모든 case가 `output.result.response` 를 1차 응답 필드로, multi-turn 블로킹 시에는 `output.view` + `output.interaction` 을 1차 필드로 사용합니다.
...
핵심 변화:
- `output.type: 'ai_conversation'`, `output.interactionType`, `output.conversationConfig` 모두 제거. 단일 `output.view = { type: 'chat', messages, turnCount, maxTurns }` 로 통일 (P4.3, I6, I8).
- `_multiTurnState` → `_resumeState` (P4.2). `NodeHandlerOutput` 5필드 외의 internal 필드이며 expression에 노출되지 않음을 문서로 못박음.
- `status: 'waiting_for_input'` 은 top-level에만. `output.status` 중복 제거 (P0).

### Case 3b (After): Multi-Turn 재개 (`status: "resumed"`)

사용자 메시지를 받아 엔진이 `processMultiTurnMessage(userMessage, _resumeState)` 를 호출해 다음 턴을 시작하면, **종료 조건에 도달하지 않은 경우** 다시 대기로 돌아가기 전에 한 번 `resumed` 스냅샷을 방출합니다. 이 스냅샷은 `view` + `interaction` 이 함께 포함되어 "사용자 입력이 수신되었음" 을 후속 observability 단계(예: timeline)에서 관찰 가능하게 합니다.
...
```json
{
  ...
  "output": {
    ...
    "interaction": {
      ...
      "data": { "message": "환불 문의입니다", "role": "user" },
...
> `resumed` 는 **순간 상태**(transient)입니다. 엔진은 곧바로 다음 턴 처리를 이어가 `waiting_for_input` / `ended` 중 하나로 수렴합니다. 이 순간 스냅샷은 run history/timeline에만 기록되고, 후속 엣지 라우팅은 발생하지 않습니다 (I7 해결).
...
| `$node["Support Bot"].output.type` | `$node["Support Bot"].output.view.type` | `'chat'` 로 고정값 변경 |
| `$node["Support Bot"].output.interactionType` | — | **삭제** |
| `$node["Support Bot"].output.conversationConfig.message` | `$node["Support Bot"].output.view.messages[-1].content` | 마지막 assistant 메시지로 직접 접근 |
| `$node["Support Bot"].output.conversationConfig.messages` | `$node["Support Bot"].output.view.messages` | P4.3 |
| `$node["Support Bot"].output.conversationConfig.turnCount` | `$node["Support Bot"].output.view.turnCount` | P4.3 |
| `$node["Support Bot"].output.conversationConfig.maxTurns` | `$node["Support Bot"].output.view.maxTurns` | P4.3 |
...
| (없음) | `$node["Support Bot"].output.interaction.data.message` | 신설 |
...
- **영향받는 프런트엔드 코드**:
  ...
  - autocomplete schema (`backend/src/nodes/ai/ai-agent/ai-agent.schema.ts` 의 schema 주석) — output path 재설계 필요.
  - Run history timeline 렌더러 — `_multiTurnState` / `_resumeState` 필드명 변경 반영.
...
1. **Shim 배포 릴리스 (v1)** — 핸들러 반환값을 **개선 후 shape으로 재구성**하되, expression resolver 레벨에서 legacy path → new path alias를 주입.
   ...
   - 사용자 colsol에 **1회 deprecation warning** 표시 (brand color + path before/after 제시).
...
| **P4** (재개 contract) | `_multiTurnState` → `_resumeState`, `output.conversationConfig` → `output.view`, `resumed` 상태 신설. `form` / `carousel` / `chart` / `table` / `template` / `information_extractor` 와 동일 모양. |
...
| **P7** (config echo) | `config.systemPrompt` 는 크기 제한 없이 echo(디버깅 우선). `config.llmConfigId` / `config.model` 은 echo, 단 `config.llmConfigId` 가 가리키는 provider credential 은 절대 echo 금지(이미 상위 레이어에서 보장). |
| **P8** (중첩 제거) | `output.data.*`, `output.metadata.*`, `output.interactionType`, `output.conversationConfig` 모두 제거/이동. 단일 진입점 `output.result.*` 또는 `output.view` + `output.interaction`. |
...
- **`_resumeState` 을 internal 필드로 유지하는 이유**: blob 크기가 커질 수 있고(대화 기록 누적), credential echo 리스크 가능성이 있으므로 autocomplete/expression resolver 레벨에서 숨김 처리합니다. 디버깅은 run history 의 raw payload 에서 수행.
...
| 블로킹 view | `output.view.type = 'chat'` | — | `output.view.type = 'chat'` |
| 블로킹 resume | `_resumeState` | — | `_resumeState` |

3개 AI 노드가 문자열 수준으로 호환되는 shape 을 공유하게 되어, 신규 사용자는 노드 종류와 무관하게 `result.*` / `meta.*` / `output.view` 만 기억하면 됩니다.
