...
test("Execution 상세 페이지에서 Background 노드 선택 시 본문 실행 결과 섹션 표시", async ({
  ...
  // Background 노드 카드 선택 (페이지 로드 시 자동 선택되거나, 클릭으로 활성화).
  // Page 가 nodes 리스트 첫 노드를 default selected 하는지에 따라 다름.
  // 안정성 위해 라벨 클릭으로 명시 선택.
  await page.getByText("Background", { exact: false }).first().click();
