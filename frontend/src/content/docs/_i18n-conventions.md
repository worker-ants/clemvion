...
새 섹션 디렉터리를 추가했다면 `frontend/src/lib/docs/locale.ts`의 `SECTION_LABELS_BY_LOCALE` 테이블에 **한국어·영어 레이블**을 함께 등록해요. 등록하지 않으면 `humanize()` 폴백 로직이 동작해 품질이 떨어져요.

## 새 로케일 추가 절차 (향후)

1. `frontend/src/lib/i18n/types.ts`의 `LOCALES` 배열과 `Locale` 타입에 코드 추가
2. `frontend/src/lib/i18n/dict/` 에 새 사전 파일 추가
3. `frontend/src/lib/docs/registry.ts`의 `LOCALE_SUFFIX` 맵에 `<code>` → `.<code>` 항목 추가
