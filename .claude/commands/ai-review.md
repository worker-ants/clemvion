...
2. **manifest 로드 + Workflow 실행**: `<session_dir>/_retry_state.json` Read (경로뿐) → `Workflow(name="ai-review", args={invocations, router, routing_status, agents_forced, summary})`. Workflow 가 router 선별(structured 반환) → 선택 reviewer 병렬(각자 prompt_file Read·output_file Write) → `code-review-summary` 가 통합 SUMMARY 를 `summary.output_file` 에 **직접 Write** 하고 짧은 status(`RISK`/`CRITICAL`/`WARNING`) 만 반환. 완료 시 task-notification.
3. **위험도 확인 (SUMMARY 는 이미 디스크에 기록됨)**: `summary_written=true` 면 SUMMARY 는 summary 에이전트가 `summary.output_file` 에 이미 Write 했으므로 반환의 `risk`/`critical_count`/`warning_count` 로 위험도만 확인한다 — **파일 전문을 다시 Read 하지 말 것** (불필요한 컨텍스트 적재). `summary_written=false` 인 fallback(write 차단) 경우에만 반환의 `summary_markdown` 을 `summary.output_file` 에 Write. `unfinished[]` 있으면 해당 reviewer 재실행.
