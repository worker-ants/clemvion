...
| `test_push_detection.py` | `guard_review_before_push._is_git_push` — **which** commands the push gate inspects (not what it then decides). Pins both directions: real pushes (`git -C`, env prefix, after `&&`/`;`/`\|`, `--force`, indirect execution via `$(...)`/backtick command substitution or a shell's `-c`/`eval` argument) must block, and commands that merely *mention* push (a heredoc/`-m` commit message, a quoted grep pattern, a known-boolean global option like `--no-pager` ahead of an unrelated subcommand) must not — the old regex allowed unbounded distance between `git` and `push` and had no notion of quoting, so it blocked `git commit`. Also pins the fail-safe (an unparseable or control-character-bearing command falls back to blocking) and a **differential test** that replays a frozen copy of the old regex over the whole corpus to prove `old ⊆ new` except for a short, justified exception list. |
