...
| `test_bootstrap_mermaid_install.py` | `bootstrap-session.sh`'s mermaid-lint install guard against a real temp repo with a stubbed `npm`: installs once and writes the completion marker, a partial `node_modules` with no marker is reinstalled (not accepted forever), an `mkdir` lock serialises the cold-start race (5 concurrent sessions → at most one install), a lock is stolen only when its owner PID is **dead** (a live-but-slow holder's lock survives however aged — the reviewer-reproduced regression), and a failed install stamps a cooldown so retries throttle. Non-vacuity is checked against the prior commit **and** a targeted liveness-removal mutant (the prior commit alone passes the live-lock test vacuously). |
