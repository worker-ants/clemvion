...
class OldEnvPrefixSupersetTest(unittest.TestCase):
    ...
    # prefix into two branches, so the boundary gained the extra `)`. §M briefly
    # narrowed the closing whitespace to `[^\S\n]+` and §N put it back to `\s+`
    # (newlines never reach these patterns — both hooks split first), so the
    # marker follows. It has to track the SHAPE of that boundary, not the branch
