...
- **흡수 시 삭제 (좁은 예외)**: 아직 착수되지 않은(`worktree: (unstarted)`) in-progress plan 이 다른 plan 이나 코드에 **완전히 흡수돼 남은 항목이 0** 이 되면, `complete/` 이동 대신 삭제할 수 있다. 조건 넷을 **모두** 만족해야 한다 — (a) 미착수 (b) 항목이 전부 다른 곳에서 실제로 해소됨(코드/문서로 지목 가능해야 한다) (c) 살아있는 문서에 인입 참조 0건 (d) **삭제 커밋 메시지에 흡수처와 사유를 남긴다**.
