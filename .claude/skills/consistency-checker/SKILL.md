...
| `convention-compliance-checker` | `spec/conventions/**` 정식 규약 위반 (노드 Output, Swagger 패턴 등) |
...
- `review/consistency/<timestamp>/_prompts/<checker>.md` — checker 별 페이로드.
- `review/consistency/<timestamp>/_retry_state.json` — 재시도/상태 파일.
- `review/consistency/<timestamp>/meta.json` — mode / target_path / checker 명단.
...
1. orchestrator 출력에서 세션 디렉토리 경로(`./review/consistency/<timestamp>/`) 받기.
2. 위 절차 2-6 진행 후 SUMMARY.md 를 사용자에게 1-2문단으로 요약.
3. Critical 항목은 각 checker 의 `<checker>/review.md` 에서 상세 근거 참고.
