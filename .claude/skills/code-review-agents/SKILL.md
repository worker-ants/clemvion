...
`Agent(subagent_type="review-router", prompt="prompt_file=<router_prompt_file>\noutput_file=<router_output_file>")`. 상태 파일에 `router_model` 키가 있으면 `model=<router_model>` 도 함께 전달. prompt_file·output_file 경로는 step 1 의 orchestrator 출력에 포함됨.
...
- **모두 완료** (pending=0):
  1. `Agent(subagent_type="code-review-summary", prompt="session_dir=<session_dir>")`. 상태 파일에 `summary_model` 키가 있으면 `model=<summary_model>` 도 함께 전달. summary sub-agent 가 자기 ctx 로 reviewer 결과 통합 후 SUMMARY.md Write.
