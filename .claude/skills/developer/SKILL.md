...
| `codebase/frontend/` | 클라이언트 코드베이스 (Next.js) | **Read/Write 자유** — 구현 주 영역 |
| `codebase/backend/` | 서버 코드베이스 (Nest.js) | **Read/Write 자유** — 구현 주 영역 |
...
5. **테스트 선작성** — 스펙 기반으로 `codebase/frontend/`·`codebase/backend/` 에 테스트 코드를 먼저 작성한다 (TDD).
...
| 신규 UI 문자열 (TSX) | `codebase/frontend/src/lib/i18n/dict/{ko,en}/<section>.ts` **양쪽** — 한쪽만 추가 금지 (parity 가드가 fail) | `cd codebase/frontend && npm test -- i18n` |
...
> **유저 가이드 파일 컨벤션** (자세한 규약: [`codebase/frontend/src/content/docs/_i18n-conventions.md`](../../../codebase/frontend/src/content/docs/_i18n-conventions.md)) :
...
- `codebase/frontend/src/lib/i18n/__tests__/i18n.test.ts` — `ko.ts` ↔ `en.ts` leaf key parity 강제. 한쪽 누락이면 fail.
- `codebase/frontend/src/lib/docs/__tests__/locale.test.ts` — `content/docs/` 의 모든 (숨김 아닌) 섹션이 `SECTION_LABELS_BY_LOCALE` 양쪽 로케일에 등록됐는지 검증.
- `codebase/frontend/src/lib/docs/__tests__/registry.test.ts` — MDX 프론트매터의 `spec:`/`code:` 경로 실존 검증.
...
1. **lint** — `cd codebase/backend && npm run lint` · `cd codebase/frontend && npm run lint`
2. **unit test** (jest, in-process) — `cd codebase/backend && npm test` · `cd codebase/frontend && npm test`
3. **build** — `cd codebase/backend && npm run build` · `cd codebase/frontend && npm run build`
4. **e2e test** (실 인프라) — `make e2e-test` (backend supertest, ~30–60s) · `codebase/frontend/` 코드 변경이 함께면 `make e2e-test-full` (playwright 포함)
...
- 한 단계 안에 codebase/backend/frontend 등 영역이 섞여 있어도 단계당 1 커밋을 원칙으로 한다 (단계 단위 atomicity 우선). 영역 분리가 필요한 특수 케이스는 사용자에게 먼저 묻는다.
