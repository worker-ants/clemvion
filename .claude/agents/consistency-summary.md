...
3. **하향 금지** — checker 가 `[CRITICAL]` 로 판정한 항목을 통합 단계에서 WARNING/INFO 로
   **낮추지 않습니다.** 근거가 아무리 타당해 보여도(위임 완료·승격 조건 문서화·"곧 고쳐질 예정")
   ...
   > 타당했지만 규약에 그런 재량 조항이 없었고, 게이트는 판정을 `BLOCK:` 줄로 읽으므로 그 하향이
   > **차단을 실제로 통과시켰습니다.** (이제 게이트가 checker 의 `[CRITICAL]` 과 모순되면
   > 경고를 냅니다 — 차단은 아니므로 이 금지 조항이 여전히 1차 방어입니다. 그 경고는 현재
   > `--impl-done` 세션이 push·턴종료 게이트에 채택될 때만 발화하므로, 다른 모드나
   > spec-linked 변경이 없는 경우는 이 조항에만 의존합니다.) 게이트 결과가 에이전트의 그때그때
