...
│   └── ingress.yaml            # HAProxy Ingress (codebase/frontend/backend host 분리, 두 Ingress 리소스)
└── overlays/
...
client bundle 에 인라인되는 한계상 환경별 이미지 빌드. `codebase/frontend/Dockerfile` 의 `--build-arg` 참고. 단일 이미지로 staging/prod 를 모두 커버하려면 같은 도메인 정책을 채택하거나 runtime-injection 패턴 (별도 작업) 도입 필요.
...
| `S3_PUBLIC_BASE_URL` | `https://cdn.example.com` | **브라우저가 도달하는** 주소. 아바타 이미지 URL 의 base 로, `S3_ENDPOINT` 가 내부 주소면 반드시 따로 준다. 추가로 `avatars/` 접두에 **익명 GET 을 허용하는 버킷 정책**이 필요하다 (`ListBucket` 은 허용하지 않는다 — 키의 UUID 가 접근 통제다). 없으면 업로드는 성공하고 **이미지만 403** 이다 |
...
> **동봉 위젯(웹채팅)**: frontend Dockerfile 의 builder 스테이지가 `channel-web-chat`(위젯 SPA) + `@workflow/web-chat`
> (loader) 소스·deps 를 갖추고 `build:widget` 을 **이미지 내부에서 자급 실행**해 `codebase/frontend/public/_widget/web-chat/v1/`
> 를 생성·동봉한다(`COPY .../public` → standalone runner). 따라서 **`docker build` 만 하면 되고** 호스트/CI(Jenkins·GitHub
