...
4. **Ingress host 설정** — 동일 파일의 frontend / backend 두 host placeholder 를 실 도메인으로 교체. TLS Secret (`idea-workflow-frontend-tls`, `idea-workflow-backend-tls`) 은 cert-manager 등으로 별도 발급.
5. **적용**
...
frontend / backend 는 **host 단위로 분리**되어 두 개의 Ingress 리소스 (`idea-workflow-frontend`, `idea-workflow-backend`) 로 구성. 각 host 는 `/` (Prefix) 전체가 해당 서비스로 매핑되며, `/api`·`/socket.io`·`/docs` 는 backend host 에서 동일하게 노출됩니다. WebSocket(`/socket.io`) 의 `timeout-tunnel: 1h` annotation 은 backend Ingress 에만 적용.
...
| ---- | -- | ------------------------------ | ------- |
| `idea-workflow-frontend` | `/spec/rules/0/host`, `/spec/tls/0/hosts/0` | `app.staging.idea-workflow.example.com` | 실 frontend 도메인 |
| `idea-workflow-backend`  | `/spec/rules/0/host`, `/spec/tls/0/hosts/0` | `api.staging.idea-workflow.example.com` | 실 backend 도메인 |
| `idea-workflow-frontend` | `tls.secretName` (base) | `idea-workflow-frontend-tls` | cert-manager 가 발급한 frontend Secret 이름과 일치 |
| `idea-workflow-backend`  | `tls.secretName` (base) | `idea-workflow-backend-tls`  | cert-manager 가 발급한 backend Secret 이름과 일치 |
...
docker build -f frontend/Dockerfile \
  --build-arg NEXT_PUBLIC_API_URL=https://api.staging.idea-workflow.example.com/api \
  --build-arg NEXT_PUBLIC_WS_URL=https://api.staging.idea-workflow.example.com \
