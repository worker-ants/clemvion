...
| ND-AG-17 | 조건을 LLM 도구(tool)로 자동 등록 — 각 조건의 프롬프트를 도구 설명으로 사용하여 LLM이 상황 판단 시 해당 도구를 호출하도록 유도한다. 도구 이름은 UUID 기반으로 자동 지정하여 충돌 및 LLM 오작동을 방지한다 | 필수 |
...
| ND-AG-19 | 종료 사유별 기본 출력 포트 — timeout, 사용자 종료, LLM 오류는 각각 전용 기본 포트로 출력하며, Multi Turn 모드에서는 maxTurns 포트도 추가로 제공한다 | 필수 |
...
| ND-AG-23 | Single Turn 모드의 포트 구조 — 기본 `out` 포트 + 조건 포트 + `timeout` 포트 + `error` 포트를 제공한다 | 필수 |
| ND-AG-24 | Multi Turn 모드의 포트 구조 — 기본 `out` 포트 없이 조건 포트 + `timeout` 포트 + `user_ended` 포트 + `max_turns` 포트 + `error` 포트를 제공한다 | 필수 |
