...
| 영역 | 상태 | 기능 |
|------|------|------|
| KB 모드 선택 | ❌ 로드맵 | KB 생성 시 `vector` / `graph` 모드 선택 (불변) |
| 그래프 추출 파이프라인 | ❌ 로드맵 | 문서 임베딩 완료 직후 자동 chained 으로 entity/relation LLM 추출 |
| 추출 LLM 설정 | ❌ 로드맵 | KB 단위 `extractionLlmConfigId` 신설 |
| Hybrid 검색 흐름 | ❌ 로드맵 | vector top-K seed → 1~2 hop graph expansion → rerank → 컨텍스트 주입 |
| 추출 상태 / 통계 UI | ❌ 로드맵 (P0) | KB 상세에 추출 진행률 / entity·relation 수 표시 |
| Entity 목록 보정 UI | ❌ 로드맵 (P1) | entity / relation 테이블 + 검색 + 개별 삭제 |
| 그래프 시각화 | ❌ 로드맵 (P2) | react-flow 기반 노드/엣지 렌더링 |
...
| KB-GR-EX-05 | 추출 실패 시 문서 단위 재시도 가능 (KB 상세에서 "Re-extract" 액션) | 필수 |
| KB-GR-EX-06 | 임베딩 재실행(`reEmbed`) 또는 KB 전체 재임베딩 시 그래프도 함께 재추출 | 필수 |
| KB-GR-EX-07 | 추출 비용을 사용자에게 표시 — 이번 추출에 사용된 토큰 수와 KB 누적 토큰 수 | 권장 |

### 3.3 데이터 모델 (`KB-GR-DM-*`)

| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| KB-GR-DM-01 | KB 단위로 entity 를 저장. 동일 KB 안에서 `(name, type)` 으로 dedup | 필수 |
| KB-GR-DM-02 | KB 단위로 relation 을 저장. (`head_entity_id`, `predicate`, `tail_entity_id`) 복합 unique | 필수 |
| KB-GR-DM-03 | chunk → entity 매핑(`chunk_entity`)으로 검색 시 chunk 회수 가능 | 필수 |
...
| KB-GR-SR-03 | 검색 2단계: 회수된 chunk 가 언급한 entity 들에서 1~`maxHops` (기본 1) 까지 그래프 확장 | 필수 |
...
| KB-GR-SR-06 | 검색 결과 메타데이터에 `traversedEntities`, `traversalDepth`, `seedChunkIds` 포함 | 권장 |

### 3.5 KB 검색 파라미터 (`KB-GR-PA-*`)

| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| KB-GR-PA-01 | KB 설정에 `maxHops` (1 또는 2, 기본 1), `vectorSeedTopK` (기본 5), `expandedChunkLimit` (기본 15) 노출 | 필수 |
| KB-GR-PA-02 | 파라미터 변경 시 추출/임베딩 재실행은 불필요 (검색 시점 적용) | 필수 |
| KB-GR-PA-03 | AI Agent 노드의 KB 연동 UI 는 그대로 유지 (`ragTopK`, `ragThreshold`만 노출). 그래프 파라미터는 KB 단위에서만 제어 | 필수 |
...
| KB-GR-UI-04 (P1) | Entity 목록 화면 — 이름·타입·등장 횟수 컬럼, 검색·정렬, 개별 삭제 | 권장 |
| KB-GR-UI-05 (P1) | Relation 목록 화면 — head·predicate·tail, 등장 chunk 미리보기, 개별 삭제 | 권장 |
| KB-GR-UI-06 (P1) | 문서 상세에서 해당 문서 chunk 가 언급한 entity 목록 표시 | 권장 |
| KB-GR-UI-07 (P2) | 그래프 시각화 (react-flow 또는 동등) — 노드/엣지 렌더, 줌, 호버 시 chunk 미리보기 | 선택 |

### 3.7 비용·관측성 (`KB-GR-OB-*`)

| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| KB-GR-OB-01 | 추출에 사용된 LLM 토큰을 LLMUsageLog 에 기록 (기존 사용량 추적과 동일 채널) | 필수 |
| KB-GR-OB-02 | 추출 진행 / 완료 / 에러 이벤트를 WebSocket 으로 노출 (KB 상세 실시간 갱신) | 필수 |
| KB-GR-OB-03 | KB 단위 entity / relation 카운트는 캐시 컬럼으로 유지 (조회 시 매번 SELECT COUNT 회피) | 권장 |
...
| **P2** | 그래프 시각화 + community detection 검토 + 도메인별 entity 타입 사전 | 시각적 탐색 + 글로벌 요약 |
...
| 의존 항목 | 현재 상태 | 필요 작업 |
|----------|----------|----------|
| BullMQ `document-embedding` 큐 | ✅ 구축됨 | `graph-extraction` 큐 추가 (동일 패턴) |
| LLMConfig | ✅ 구축됨 | KB 에 `extractionLlmConfigId` 컬럼 추가 |
| pgvector | ✅ 구축됨 | 그대로 사용 (vector seed) |
| KB 모드 선택 UI | ❌ | 생성 폼에 셀렉트 추가 |
| AI Agent 의 KB 연동 | ✅ 구축됨 | 변경 없음 (`ragTopK`/`ragThreshold` 그대로) |
