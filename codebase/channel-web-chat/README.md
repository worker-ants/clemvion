...
(`src/lib/conversation` — `[user-input]` strip + turn `source`(백엔드 5값 `presentation_user`/`ai_user`/
`ai_assistant`/`ai_tool`/`system`)→말풍선 role 매핑), 세션 복원(`src/lib/session-store` + EIA `getStatus` 가
durable `conversationThread` 동봉 → 새로고침 시 대화 히스토리 전체 복원), 헤더 세션 컨트롤(새 대화·대화 종료 +
