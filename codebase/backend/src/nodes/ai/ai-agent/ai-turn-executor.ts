...
/**
 * AI Agent 노드의 turn 실행 엔진 — 핸들러에서 분리한 무상태 collaborator
 * (refactor 02-architecture §M-1 3단계, behavior-preserving). single-turn /
 * multi-turn 첫 진입(park) / multi-turn 재개 루프 전체와, 그 루프가 구동하는
 * tool 실행(`executeProviderToolBatch` / `runProviderTool`)·turn 종결 출력 조립
 * (`buildMultiTurnFinalOutput` / `buildConditionOutput` / `endMultiTurnConversation`
 * / `_retryState`)·ConversationThread push 헬퍼를 담당한다.
 *
 * 핸들러(`AiAgentHandler`)는 NodeHandler 인터페이스 표면(`execute` / `validate`)
 * 과 엔진/IE 공유 계약(`processMultiTurnMessage` polymorphic 시그니처
 * — spec/5-system/4-execution-engine.md §1.3·information_extractor 공유) 및
 * 엔진 호출 진입점(`endMultiTurnConversation` / `buildMultiTurnFinalOutput`)만
 * 잔류시키고 본 executor 로 **단방향 위임**한다 (executor → handler 역참조 없음).
 *
 * 외부 의존(`llmService` chat·`toolProviders` 도구 실행·`eventEmitter` 텔레메트리·
 * `conversationThreadService` thread push)과 선행 collaborator(`conditionEvaluator`
 * #665·`memoryManager` #668)는 전부 생성자로 주입받아 클래스 자체는 실행별 가변
 * 상태를 보유하지 않는다 (#665·#668 무상태 collaborator 선례와 동형). 미주입
 * fixture 는 thread/telemetry 없는 동작으로 graceful degrade 한다.
 *
 * 동작 보존 체크리스트: spec/4-nodes/3-ai/1-ai-agent.md §6.2(멀티턴 blocking
...
export class AiTurnExecutor {
  constructor(
    ...
    /** 조건 평가 collaborator (refactor §M-1 1단계, #665) — 핸들러가 소유·주입. */
    private readonly conditionEvaluator: AiConditionEvaluator,
    /** 자동 메모리 전략 collaborator (refactor §M-1 2단계, #668) — 핸들러가 소유·주입. */
    private readonly memoryManager: AiMemoryManager,
    private readonly toolProviders: AgentToolProvider[] = [],
    /**
     * Optional. 각 provider 도구 실행마다 `tool_call_started` / `tool_call_completed`
     * 이벤트를 엔진 `ExecutionEventEmitter` facade 로 emit (단일 emit sink, EIA §R10).
     * 인라인 `import()` 타입으로 `nodes/` → `modules/execution-engine/` top-level import
     * 그래프·잠재 순환을 만들지 않는다 (핸들러 동일 패턴). 미주입 시 텔레메트리 무음.
     */
    ...
    /**
     * Optional. workflow-scoped ConversationThread 에 user/assistant/tool turn 을
     * push 하고 thread 를 LLM chat 에 주입한다 (단일 mutation 진입점, spec/conventions/
     * conversation-thread.md §2.2). 미주입 시 thread 없는 원래 동작으로 degrade.
     */
  ...
  async executeSingleTurn(
    ...
    // Turn 시작 시각 — pre-flight `buildTools` 실패(§4.2 payload 예산 초과)의
    // error 포트 output 에서 `meta.durationMs` 산정에 쓴다. LLM turn 타이밍용
    // `singleTurnStartedAt`(아래)와 별개로 method 진입 시점을 잡는다.
    const singleTurnEnteredAt = Date.now();
    ...
    // 종결이 **동일 shape** 를 공유하도록 한다.
    const singleTurnConfigEcho: Record<string, unknown> = {
    ...
    // spec §4.2 / §7.3 — multi-turn `handleAiMessageTurn` 은 buildTools throw 를
    // try/catch 로 감싸 error 포트로 라우팅하지만 single-turn 경로엔 그 래핑이
    // 없어 비대칭이다. `TOOL_DEFINITION_PAYLOAD_EXCEEDED` 를 여기서 잡아 §7.3
    // error 포트 output 을 **return** 한다 (throw 전파 아님). 다른 에러는 rethrow.
    let tools: ToolDef[];
    try {
      ...
      if (err instanceof ToolDefinitionPayloadExceededError) {
        return {
          ...
          meta: {
            model: model || llmConfig?.defaultModel,
            durationMs: Date.now() - singleTurnEnteredAt,
          },
          port: 'error',
          status: 'ended',
        };
    ...
    return {
      ...
      meta: {
        ...
        ragSources: ragAcc.getSources(),
        ragDiagnostics: ragAcc.getDiagnostics(),
        ...(AiTurnExecutor.buildMcpDiagnosticsMeta(mcpDiagnosticsAcc) ?? {}),
  ...
  async executeMultiTurn(
    ...
    const multiTurnStrategy = this.memoryManager.resolveMemoryStrategy(config);
  ...
  private handleMultiTurnUserMessageEntry(
    ...
    if (pendingFormToolCall && messageSource === 'form_submitted') {
      ...
      if (
        ...
        AiTurnExecutor.logger.warn(
          `processMultiTurnMessage — pendingFormToolCall 없음, JSON 형태 userMessage 를 plain ai_user 메시지로 fallback. spec §10.9 §Rationale (pendingFormToolCall 누락 fallback).`,
        );
      }
      messages.push({ role: 'user', content: userMessage });
      this.pushAiThreadTurn(
   ...
   * delegates to the executor-local {@link buildMultiTurnFinalOutput}.
   *
  ...
  buildMultiTurnFinalOutput(
    ...
    //     in multi-turn mode).
    const port = AiTurnExecutor.multiTurnPortForEndReason(endReason);
    ...
    const retryState =
      isRetryable && retryStateSource
        ? AiTurnExecutor.buildRetryState(
            retryStateSource,
            messages,
            turnCount,
            {
