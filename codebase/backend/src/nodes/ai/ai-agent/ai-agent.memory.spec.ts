...
describe('AiAgentHandler — auto-memory strategy', () => {
  ...
  describe('persistent (single-turn)', () => {
    ...
    it('임베딩 config: 노드 config embeddingModelConfigId 를 recall embed source 로 전달', async () => {
      ...
      const recallArgs = agentMemoryService.recall.mock.calls[0];
      // embed source (4번째 인자) 에 노드 config embeddingModelConfigId 가 전달 —
      // 서버가 그 embedding ModelConfig 로 resolve(회수·저장 동일 config).
      expect(recallArgs[3]).toMatchObject({
        embeddingModelConfigId: 'emb-cfg-1',
      });
    });

    it('임베딩 config: 노드 config embeddingModelConfigId 를 scheduleExtraction(추출) 로도 전달', async () => {
      const context = makeContext();
      const handler = buildHandler();
      await handler.execute(
        undefined,
      ...
      // 회수와 추출이 동일 embedding config 를 써야 차원이 일치한다.
      const arg = agentMemoryService.scheduleExtraction.mock.calls[0][0];
      expect(arg.embeddingModelConfigId).toBe('emb-cfg-1');
    });
  ...
  describe('턴 경계 비동기 추출 enqueue (spec §3·§6.1 단계 2.7 — producer)', () => {
    ...
    it('AGM-08: multi-turn 증분 추출 — 두 번째 turn 은 신규 turn 만 snapshot 하고 watermark 영속', async () => {
      ...
      expect(typeof getWatermark(state)).toBe('number');
      const wm1 = getWatermark(state) as number;
      ...
      state = (r2 as { _resumeState: Record<string, unknown> })._resumeState;
      const wm2 = getWatermark(state) as number;
    ...
    it('I12: watermark 영속 시 memoryState 의 기존 타 키를 보존한다 (병합 spread)', async () => {
      const context = makeContext();
      const handler = buildHandler();
      const first = await handler.execute(
        undefined,
      ...
      const state = (first as { _resumeState: Record<string, unknown> })
        ._resumeState;
      // 향후 메모리 상태 키가 추가된 상황을 모사 — watermark 영속이 이를 덮으면 안 된다.
      state.memoryState = { someOtherKey: 'keep' };

      const r1 = await handler.processMultiTurnMessage('첫 질문', state);
      const next = (r1 as { _resumeState: Record<string, unknown> })
        ._resumeState;
      const ns = next.memoryState as {
        someOtherKey?: string;
        lastExtractionTurnSeq?: number;
      };
      expect(ns.someOtherKey).toBe('keep'); // 타 키 보존.
      expect(typeof ns.lastExtractionTurnSeq).toBe('number'); // watermark 도 기록.
    });
    ...
    it('M1: enqueue dedup-drop (scheduleExtraction=false) 시 watermark 를 전진시키지 않는다', async () => {
      ...
      const r2 = await handler.processMultiTurnMessage('둘째 질문', state);
      state = (r2 as { _resumeState: Record<string, unknown> })._resumeState;
      expect(getWatermark(state)).toBe(wm1); // 전진 안 함.
      ...
      // 이제는 전진 (정상 수락).
      expect(getWatermark(state) as number).toBeGreaterThan(wm1);
    });
  ...
  describe('요약/추출 전용 config (summaryModelConfigId / extractionModelConfigId — A3)', () => {
    ...
    it('multi-turn resume(turn2): 요약 콜=summaryModelConfigId, 추출 enqueue=extractionModelConfigId (state 영속)', async () => {
      ...
      // 전용 config 가 resume state 로 영속되었는지 직접 확인 (C1 핵심).
      expect(state.summaryModelConfigId).toBe('summary-cfg');
      expect(state.extractionModelConfigId).toBe('extract-cfg');
