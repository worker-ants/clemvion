...
// 않는다** — 각 사본이 자기 워크플로에서 같은 불변식을 계속 지킨다.
//
// > **다만 그 안전은 조건부다.** 이 PR 안에서 두 번 반증됐다 — 라운드3 에서 접두 경계를
// > backend 만 고치고 "양쪽 다 고쳤다" 고 적었고(매치 단언 없는 치환이 조용한 no-op),
// > 그때 캐너리는 **심볼** 접두만 봐서 그 비대칭을 못 봤다. 로직 결함은 두 사본에 **동시에**
// > 존재하므로 중복이 그것을 막아 주지 않는다.
// >
// > **규칙**: 판정 분기를 새로 넣거나 고칠 때는 **양쪽에 대칭 캐너리를 함께** 넣는다.
// > 기계가 대칭을 확인하지 않으면 한쪽만 고쳐진 채로 완료형 서술이 남는다.
