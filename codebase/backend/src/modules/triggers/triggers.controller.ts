...
const TRIGGER_ENDPOINT_PATH_CONFLICT_DESCRIPTION =
  '같은 `endpointPath` 를 쓰는 트리거가 이미 있거나(다른 워크스페이스의 트리거 포함 — `endpoint_path` 는 전역 유일, V132), 그 경로를 다른 워크스페이스가 예약했다(지웠거나 바꾼 경로 — V133). 둘을 구분하지 않는다. `code=RESOURCE_CONFLICT`, `details.field="endpoint_path"`, `details.code="TRIGGER_ENDPOINT_PATH_CONFLICT"`.';
