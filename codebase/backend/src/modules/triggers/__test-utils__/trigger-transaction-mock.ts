 ...
 * **60개 케이스가 RED** 다(2026-09-17 재측정, 321건 중). 즉 이 위임은 장식이 아니라 그 60건을
 * 살아 있게 하는 배선이다 — GREEN 만으로는 증거가 되지 않아 빼 보고 셌다.
 *
 * > **이 수는 시점 의존이다.** 처음 쟀을 땐 13, `#1334` 종결 시점엔 53(R-CC-21 9 · lost-update 8 ·
 * > `rotateBotToken` 8 · schedule 동기화 7 · 생성 경로 5 · 그 외 16)이었고, 창 1 을 부분 객체
 * > `save` 로 바꾸며 이 대역의 `save` 반환을 보정하고 회귀 테스트를 더해 60 이 됐다. 위 값은
 * > **마지막으로 이 파일을 고친 PR 이 닫히는 시점**의 실측이다 — 이 파일을 고칠 땐 다시 재라.
 *
