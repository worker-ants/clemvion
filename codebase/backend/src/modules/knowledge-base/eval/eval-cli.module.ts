 ...
 * `LlmModule`/`ModelConfigModule` 의 export 만 의존하므로 여기서 직접 provide 한다.
 * 엔티티 메타데이터는 app.module 과 공유하는 `ROOT_ENTITIES` 전체를 등록해 충족
 * (ModelConfig→Workspace 등 관계 타깃 누락 방지. 검색/생성의 document_chunk·
  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        ...
        // 명시 목록 재사용: 엔티티 간 관계(예: ModelConfig→Workspace) 메타데이터가
