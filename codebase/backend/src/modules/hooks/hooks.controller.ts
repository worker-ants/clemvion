...
const EMBED_CONFIG_CACHE_MAX_MIN = Math.ceil(EMBED_CONFIG_CACHE_SEC / 60);
...
export class HooksController {
  ...
  @ApiResponse({
    ...
    headers: {
      'Cache-Control': {
        description: `${EMBED_CONFIG_CACHE_CONTROL} — 워크스페이스 설정 변경 후 최대 ${EMBED_CONFIG_CACHE_MAX_MIN}분 반영 지연(I1).`,
