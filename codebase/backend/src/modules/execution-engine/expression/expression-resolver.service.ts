...
export class ExpressionResolverService {
  ...
  private resolveString(
    ...
    try {
      ...
      // 표현식 평가 예외라 message·name 밖에 민감 속성이 붙지 않는다. 실측(2026-08-29):
      // `ExpressionError` 의 부가 own property 는 `code`(ErrorCode enum) 와
      // `position`(정수 오프셋) 뿐이다 — `packages/expression-engine/src/errors.ts`.
      // pg 드라이버의 `detail`/`hint` 같은 민감 필드가 붙는 형태가 아니다.
