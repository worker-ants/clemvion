...
// forwardRef 의 `() =>` 가 클래스 참조를 지연 평가하므로 ES module 순환 import
// (orchestrator → 본 파일의 helper/RehydrationError) 도 안전하다 (값 import 유지).
    ...
    EngineDriver
{
  ...
  // C-1 step2 — EngineDriver member (LLM-default single-flight 캐시 노출).
  public async resolveHasDefaultLlmConfigCached(
    workspaceId: string,
    context: ExecutionContext,
  ): Promise<boolean> {
