  ...
  implements OnModuleInit, OnApplicationBootstrap, WorkflowExecutor
{
  ...
  /**
   * [spec form §4·§6.2 / EIA §5.1] form 제출 데이터를 노드 field 정의(필수/type/length/선택지)에
   * 대해 검증한다. 실패 시 FIRST 오류로 {@link FormValidationError} throw (chat-channel
   * `validateFormSubmission` 재사용). file MIME/size/count 검증은 Planned — 본 단계 미적용.
   *
