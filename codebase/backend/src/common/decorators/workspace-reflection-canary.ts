 ...
 * 0 이면 throw** 한다. 이 저장소에는 그런 라우트가 다수 있으므로 0 은 "reflection 이 통째로
 * 깨졌다" 는 뜻이다.
 *
 * **세는 대상은 `@WorkspaceId()` 를 소비하는 라우트 전체다** (2026-08-09 정적 실측 142건 —
 * `*.controller.ts` 의 param 데코레이터 사용처). 이 주석이 처음 인용했던 `#1103` 의 **73건**
 * 은 그중 **`@Roles()` 가 없는 서브셋**이라 이 캐너리의 대상이 아니다 — 서브셋 수치를 상위집합
 * 자리에 적으면 개수 급락을 눈으로 볼 때 기준이 어긋난다(아래 "부분 파손" 관측이 그 수치에
 * 기댄다). 정적 실측이라 런타임 집계와 정확히 같지는 않다: 캐너리는 `DiscoveryService` 로
 * **모듈에 실제 등록된** 컨트롤러만 훑는다.
 *
