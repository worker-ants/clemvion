 ...
 * **단언 헬퍼 자신의 회귀 가드.**
 *
 * 이 헬퍼는 `trigger-workflow-ref.e2e-spec.ts` 의 **모든 양성 자리**에서 `TriggerDto.workflow`
 * 가 그 경로가 채워야 하는 대로 실렸는지 확인하는 **유일한 양성 수단**이다. 헬퍼가 무르게
 * 바뀌면 그 자리들이 **동시에** 조용히 통과한다 — 자매 헬퍼가 같은 이유로 self-spec 을 갖게
 * 된 이력이 있다 (`review/code/2026/09/06/01_13_50` W6).
 *
 * > **자리 «수» 를 여기 적지 않는다.** 종전엔 *"다섯 자리"* 라 두 번 적었는데 실제로는 6이었고,
 * > 정작 그 e2e 파일 헤더는 *"여섯 형태"* 라 적어 **두 문서가 다른 수를 주장**하고 있었다
 * > (`origin/main` 에도 있던 기존 결함 · `/ai-review` `review/code/2026/09/14/13_04_49`
 * > documentation WARNING#1). 수를 맞추는 대신 **복제를 없앤다** — 자리 수의 SoT 는 그
 * > e2e 파일이고, 여기서 다시 세면 한쪽이 반드시 낡는다.
 *
