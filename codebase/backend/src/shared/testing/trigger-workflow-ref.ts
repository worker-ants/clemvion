 ...
 *
 * > **이 절의 전제는 세 설정 파일(`jest.config.ts` · `test/jest-e2e.json` · `tsconfig.build.json`)의
 * > 현재 상태다.** 그중 하나가 바뀌면 이 서술이 조용히 낡고, 최악의 경우 self-spec 이 CI 리포트에
 * > 안 잡히는 상태가 된다 — 이 파일이 막으려는 dead-test 위험을 이 절 자신도 안고 있다.
 *
 * **production 빌드 유출도 처음 단정적으로 적었는데 그것 역시 과장이었다.**
