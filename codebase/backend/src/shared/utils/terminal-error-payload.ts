 ...
 * 으로 보존된다.
 *
 * 이 함수가 자리로 옳은 이유는 **`toTerminalErrorPayload` 호출부 5곳이 전부 emit 쪽**이라는
 * 것이다 — `EXECUTION_FAILED` 4곳 + `chat-channel.dispatcher` 재정규화 1곳, DB write 0.
 * (위 §"현재 호출부" 의 취소 이벤트 5곳과는 **다른 집합**이다.) 새 종결 emit 경로가 생겨도
 * 형태를 얻으려면 여기를 거치므로 마스킹이 **구조적으로** 빠질 수 없다 — "한 곳만 빠뜨린다" 가
 * 이 저장소의 반복 실패 형태다.
 *
 ...
 * ## 검증 범위를 정확히 적는다
 *
 * 아래 네 반환 분기가 모두 이 함수를 거치지만, **뮤테이션으로 판별된 것은 문자열·객체 두
 * 분기**다(래핑을 벗기면 RED). 스칼라(`number`/`boolean`/`bigint`)와 non-object 분기는
 * **값 공간에 secret 이 존재할 수 없어** 마스킹이 정의상 no-op 이므로 어떤 테스트도 그 두
 * 래핑을 판별하지 못한다. 그 둘을 통과시키는 이유는 보안이 아니라 **균일성**이다 — 새 분기가
 * 생겼을 때 한 갈래만 빠지는 걸 막는다. "전부 검증됐다" 로 읽히지 않게 여기 적어 둔다
 * (`10_19_30` testing W3).
 *
 * ## 무엇을 **못** 잡는지 (실측)
 *
