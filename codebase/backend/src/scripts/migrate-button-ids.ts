 ...
 *   npx ts-node codebase/backend/src/scripts/migrate-button-ids.ts --dry-run
 *
 *   # apply — requires workspace/user ids for the audit_log row
 *   npx ts-node codebase/backend/src/scripts/migrate-button-ids.ts --apply \
...
async function main(): Promise<void> {
  ...
  // 사고 방지 (review W-3). 로컬 dev 는 codebase/backend/.env 에 명시.
  const password = process.env.DB_PASSWORD;
  if (!password) {
    throw new Error(
      'DB_PASSWORD is required — set it in codebase/backend/.env or as an env var before running this migration.',
    );
  }
