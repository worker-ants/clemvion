 ...
 *
 * Usage (run from repo root OR codebase/backend/ — `codebase/backend/.env` is auto-loaded):
 *
 *   # dry-run — prints diff, no DB write
 *   npx ts-node codebase/backend/src/scripts/migrate-node-output-refs.ts --dry-run
 *
 *   # apply — requires workspace/user ids for the audit_log row
 *   npx ts-node codebase/backend/src/scripts/migrate-node-output-refs.ts --apply \
 *     --workspace-id <uuid> --user-id <uuid>
 *
 * Inline env override (skips `.env`):
 *   DB_PASSWORD=… npx ts-node codebase/backend/src/scripts/migrate-node-output-refs.ts --apply …
 *
...
// Load `codebase/backend/.env` relative to this script so the CLI works from any
// CWD (repo root, codebase/backend/, CI runner). `dotenv.config` does NOT override
