 ...
 *
 *   # dry-run — 출력만, 삭제 없음
 *   npx ts-node codebase/backend/src/scripts/cleanup-invalid-queue-jobs.ts
 *
 *   # apply — 손상 job 만 remove()
 *   npx ts-node codebase/backend/src/scripts/cleanup-invalid-queue-jobs.ts --apply
 *
 *   # apply + sweep 동안 큐 자동 pause (TOCTOU 방지)
 *   npx ts-node codebase/backend/src/scripts/cleanup-invalid-queue-jobs.ts --apply --pause-during-sweep
 *
 ...
 *   node codebase/backend/dist/scripts/cleanup-invalid-queue-jobs.js --apply --pause-during-sweep
 *
 * 운영 절차:
