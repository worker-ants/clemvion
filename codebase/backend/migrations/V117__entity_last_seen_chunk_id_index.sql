-- V117: entity (last_seen_chunk_id) partial — FK entity_last_seen_chunk_id_fkey (ON DELETE SET NULL)
--
...
--
-- 청크가 지워질 때마다 한 번씩 찾는다 — 재임베딩(문서의 청크를 지우고 다시 만든다) · 문서 삭제 · KB 삭제.
-- 800k: KB 삭제에서 52,323 ms → 약 7 ms (2,000회) · 재임베딩에서 674.8 ms → 0.65 ms (40회). 크기 12 MB (테이블 48 MB).
-- partial 인 이유: 컬럼이 nullable 이고 FK 트리거의 등치 조회는 IS NOT NULL 을 함의한다(V115 · V116 과 같은 이유).
-- 쓰기 비용: 10만 행 INSERT 5회 median 1,438.8 → 1,467.9 ms (+2.0%, 잡음 수준).
--
...
DROP INDEX CONCURRENTLY IF EXISTS idx_entity_last_seen_chunk_id;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_entity_last_seen_chunk_id
  ON entity (last_seen_chunk_id)
  WHERE last_seen_chunk_id IS NOT NULL;

-- DOWN(수동 롤백 참고 — Flyway 자동 실행 아님): DROP INDEX CONCURRENTLY IF EXISTS idx_entity_last_seen_chunk_id;
