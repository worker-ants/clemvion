-- V128: knowledge_base (workspace_id) — KB 목록 + FK knowledge_base_workspace_id_fkey (ON DELETE CASCADE)
--
...
--
-- knowledge_base 에는 PK 밖에 없었다 — 워크스페이스별 KB 목록(GET /api/knowledge-bases · KB 선택 상자 · 어시스턴트 도구
-- list_knowledge_bases)이 요청마다 테이블을 훑었다. W=10,000(KB 5만): 목록 1.53 → 0.033 ms. 워크스페이스 삭제의 FK 조회(3.2 ms)도 이것을 쓴다.
-- 크기 0.66 MB. 쓰기 비용: 839.6 → 923.8~992.4 ms (행당 +0.8~1.5 µs, 사람이 만든다).
--
...
DROP INDEX CONCURRENTLY IF EXISTS idx_knowledge_base_workspace_id;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_knowledge_base_workspace_id
  ON knowledge_base (workspace_id);

-- DOWN(수동 롤백 참고 — Flyway 자동 실행 아님): DROP INDEX CONCURRENTLY IF EXISTS idx_knowledge_base_workspace_id;
