-- V125: workflow_assistant_session (llm_config_id) partial — FK workflow_assistant_session_llm_config_id_fkey (ON DELETE SET NULL)
--
...
--
-- 모델 설정이 지워질 때 찾는다. 세션은 대화마다 생기고 자동 정리가 없어(사용자가 직접 지울 때만) 사용량으로 자란다.
-- W=10,000(세션 10만): 모델 설정 삭제 3.5 / 3.0 ms → 0.15 ms.
-- partial 인 이유: 컬럼이 nullable 이고 FK 트리거의 등치 조회는 IS NOT NULL 을 함의한다(V115~V118 과 같은 이유).
-- 크기 1.0 MB. 쓰기 비용: 1,968.2 → 2,100.4~2,227.0 ms (행당 +1.3~2.6 µs).
--
...
DROP INDEX CONCURRENTLY IF EXISTS idx_workflow_assistant_session_llm_config_id;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_workflow_assistant_session_llm_config_id
  ON workflow_assistant_session (llm_config_id)
  WHERE llm_config_id IS NOT NULL;

-- DOWN(수동 롤백 참고 — Flyway 자동 실행 아님): DROP INDEX CONCURRENTLY IF EXISTS idx_workflow_assistant_session_llm_config_id;
