...
**감수하는 비대칭**: 0) 은 대상이 invalid 잔재인지 정상 인덱스인지 구분하지 않습니다. 그래서 **이미 성공한** 마이그레이션을 Flyway 흐름 밖에서 수동 재실행하면 살아 있는 인덱스를 재빌드합니다(그 구간 seq scan). Flyway 는 성공한 마이그레이션을 다시 돌리지 않으므로 정상 흐름에서는 발생하지 않고, 반대편(인덱스 0개)이 훨씬 나쁩니다.
...
`ALTER TABLE ... ALTER COLUMN ... TYPE ...` 는 PostgreSQL 에서 `ACCESS EXCLUSIVE LOCK` 을 잡으며, **새 타입이 기존 타입과 binary-coercible 하지 않으면 테이블 전체를 rewrite** 합니다. 대형 테이블에서 rewrite + 락이 결합되면 수 분 단위로 DML 이 멈출 수 있습니다 (vector·jsonb·text 등 가변폭 컬럼에서 특히 위험). 신규 타입 변경은 아래 절차로 분류해 작성합니다.

**(a) 먼저 binary-coercible 여부를 판별한다.** rewrite 가 발생하지 않는 변경(예: `varchar(n)` → `text`, typed `vector(N)` → untyped `vector` 처럼 storage layout 이 동일한 메타데이터성 변경)은 락만 짧게 잡으면 충분합니다. **`SET lock_timeout` 을 선행**해 운영 중 무한 대기를 막습니다 (V021 의 typed→untyped vector 변경이 이 부류 — 당시 ad-hoc 으로 처리했던 것을 정식 규칙으로 승격).

```sql
-- (a) rewrite 없음(메타데이터성) — lock_timeout 선행으로 운영 보호
SET lock_timeout = '3s';
ALTER TABLE document ALTER COLUMN embedding TYPE vector;  -- typed→untyped, binary-coercible
RESET lock_timeout;
```

**(b) 실제 rewrite 가 발생하는 변경은 shadow column 3-step 으로 분리한다.** 단일 `ALTER COLUMN TYPE` 로 전체 rewrite 를 한 트랜잭션에 거는 대신:

1. **ADD** — 새 타입의 shadow 컬럼을 추가 (`ALTER TABLE ... ADD COLUMN <new> <type>`, nullable, lock 짧음).
2. **backfill** — *별도 V 번호* 마이그레이션에서 배치로 채운다 (`UPDATE ... WHERE <new> IS NULL ... LIMIT` 반복 또는 애플리케이션 배치). 단일 대량 UPDATE 는 그 자체가 장기 락/WAL 폭증이므로 금지.
3. **switch & drop** — 코드가 새 컬럼을 읽도록 배포한 *뒤*, 별도 마이그레이션에서 `DROP COLUMN <old>` + `RENAME COLUMN <new> TO <old>`. 각 단계는 `SET lock_timeout` 선행.

이렇게 하면 rewrite 가 한 번의 긴 락이 아니라 무중단 배치로 분산됩니다.

**(c) rewrite 대상 컬럼의 인덱스(HNSW·partial 등)는 rewrite 후 `CONCURRENTLY` 로 별도 파일에서 만든다.** ALTER TYPE / shadow-switch 가 끝난 뒤, §4·§5 규약(`.conf executeInTransaction=false`, 파일당 CONCURRENTLY 한 개)을 따라 인덱스를 재생성합니다. ALTER 와 같은 트랜잭션에 인덱스 생성을 섞지 않습니다.

> **기배포 V021 소급 수정 금지** — 위 규칙은 신규 마이그레이션부터 적용합니다. 이미 적용된 V021 은 그대로 둡니다(checksum·history 보존).
