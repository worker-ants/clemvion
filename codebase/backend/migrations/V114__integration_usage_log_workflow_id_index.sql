-- V114: integration_usage_log (workflow_id) — FK integration_usage_log_workflow_id_fkey (ON DELETE CASCADE)
--
...
--
-- 워크플로 삭제 한 번에 한 번 찾는다. 800k(로그 80k 행)에서 2.56 ms → 0.028 ms. 크기 0.6 MB.
-- 쓰기 비용은 V113 과 합쳐 쟀다(V113 헤더).
--
...
DROP INDEX CONCURRENTLY IF EXISTS idx_integration_usage_log_workflow_id;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_integration_usage_log_workflow_id
  ON integration_usage_log (workflow_id);

-- DOWN(수동 롤백 참고 — Flyway 자동 실행 아님): DROP INDEX CONCURRENTLY IF EXISTS idx_integration_usage_log_workflow_id;
