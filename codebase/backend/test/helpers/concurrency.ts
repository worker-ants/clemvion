...
export async function raceUnderHeldLock<T>(
  ...
  try {
    ...
    //
    // 1.5초인 이유: 트리거 삭제 경로의 `lock_timeout` 이 5초라 그 아래여야 한다. 그렇지 않으면
    // 가드가 기다리는 동안 요청이 **락 타임아웃으로** 끝나 «settled» 가 되고, 겹침 실패와
    // 구분되지 않는다.
...
/**
 * 공허성 가드 대기 시간. 트리거 삭제 경로의 `lock_timeout`(5초)보다 **짧아야** 한다 —
 * 위 본문 주석 참고.
 */
