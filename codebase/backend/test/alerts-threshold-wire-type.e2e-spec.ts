...
describe('알림 규칙 threshold 의 wire 타입 (e2e)', () => {
  ...
  it('POST → GET → PATCH 세 응답 모두 threshold 가 문자열이다', async () => {
    ...
    const created = await request(BASE_URL)
      .post('/api/alerts')
      .set(headers())
      .send({ type: 'failure_rate', threshold: 10, channel: 'in_app' });
    ...
    expect(Number(createdRule.threshold)).toBe(10);
    ...
    expect(Number(mine?.threshold)).toBe(10);
    expect(mine?.threshold).toMatch(/^\d+\.\d{4}$/);
    ...
    expect(Number(patchedRule.threshold)).toBe(15);
  }, 60_000);
});
