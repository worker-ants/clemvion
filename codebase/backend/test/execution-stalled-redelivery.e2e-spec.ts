...
/**
 * e2e: spec/5-system/4-execution-engine.md §7.1 (PR4 — BullMQ stalled 자동 재배달).
 *
 * 시나리오: 워커가 노드 dispatch 중간에 죽어 execution-run job 이 stalled 로 재배달되는
 * 상황을 재현한다. 정상 실행으로 완료 prefix(trigger·codeA)를 만든 뒤 frontier(codeB)를
 * 삭제하고 Execution 을 `running` 으로 되감아 "codeA 완료 후 codeB 직전 크래시" 를 합성.
 * 실 BullMQ stall 타이밍은 in-network e2e 로 재현 불가하므로, `_test/simulate-execution-
 * run-redelivery` 훅(§NODE_ENV=test 게이팅)이 `runExecutionFromQueue`(BullMQ processor
 * 진입점)를 직접 호출해 그 **RUNNING 분기(§7.5 case B 재구동)** 를 구동한다.
 *
 * 검증: (1) 재구동이 frontier(codeB)를 실행해 무손실 completed 도달,
 *       (2) 완료 노드(trigger·codeA)는 **재실행되지 않음**(row 수 불변, §7.3 exactly-once),
 *       (3) WORKER_HEARTBEAT_TIMEOUT 로 마킹되지 않고 정상 재구동됨.
 */
...
describe('BullMQ stalled 재배달 → RUNNING 세그먼트 재구동 (e2e, PR4 §7.1)', () => {
  let db: Client;
  ...
  beforeAll(async () => {
    db = createDbClient();
    await db.connect();
    const owner = await registerAndLogin(BASE_URL, uniqueEmail('stalled'), db);
  ...
  it('stalled 재배달(RUNNING execution 재처리) → §7.5 case B 재구동으로 frontier(codeB) 실행·무손실 completed, 완료 노드는 재실행 안 함', async () => {
    ...
    //    frontier(codeB) 실행 흔적 삭제 + Execution 을 RUNNING 으로. (stalled 재배달
    //    분기는 stale 나이를 보지 않으므로 started_at 조작 불요.)
    await db.query(
    ...
    await db.query(
      `UPDATE execution
         SET status = 'running', finished_at = NULL, output_data = NULL
       WHERE id = $1`,
      [executionId],
    );
    ...
    // 4. stalled 재배달 시뮬(§NODE_ENV=test 게이팅) — runExecutionFromQueue 를 RUNNING
    //    execution 에 대해 직접 호출 → 3-way switch 의 RUNNING 분기가 §7.5 case B 재구동.
    const redeliverRes = await request(BASE_URL)
      .post(
