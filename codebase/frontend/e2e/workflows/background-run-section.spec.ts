...
test("Execution 상세 페이지에서 Background 노드 선택 시 본문 실행 결과 섹션 표시", async ({
  ...
  // BackgroundRunSection 의 핵심 표식 — `Background 본문 실행` 헤더 (ko default,
  // `executions.backgroundRun.sectionTitle`).
  await expect(
    page.getByRole("heading", { name: /Background 본문 실행/i }),
  ...
  await expect(page.getByText("본문 노드 (1)")).toBeVisible();
  await expect(page.getByText("code-node")).toBeVisible();

  // status 뱃지: "완료" (ko default).
