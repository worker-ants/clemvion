          ...
          <DangerZoneTab
            ...
            onAfterMutation={async (opts) => {
              ...
              if (opts.switchAway) {
                ...
                const first = list[0];
                if (first) {
                  // 현재 워크스페이스를 나가/삭제했으니 URL 의 구 slug 는 무효 — 남은 첫 워크스페이스로
                  // 전환하고 그 slug dashboard 로 이동한다(`[slug]` layout 의 무효-slug 리다이렉트 백업).
                  void switchWorkspace(first.id);
                  router.push(buildWorkspaceHref(first.slug, "/dashboard"));
                }
              }
            }}
          />
