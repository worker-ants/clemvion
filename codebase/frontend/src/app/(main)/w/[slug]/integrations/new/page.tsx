...
export default function NewIntegrationPage() {
  ...
  const goToStep = (next: Step) => {
    ...
    qp.set("step", next);
    router.replace(buildWorkspaceHref(slug, `/integrations/new?${qp.toString()}`));
  };
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (created) => {
      ...
      router.push(buildWorkspaceHref(slug, `/integrations/${created.id}`));
    },
    onError: (err: unknown) => {
      toast.error(
  ...
  if (!service) {
    return (
      <div className="space-y-4">
        <Link
          href={buildWorkspaceHref(slug, "/integrations")}
