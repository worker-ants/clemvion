...
export default function IntegrationsPage() {
  ...
  const updateParam = (key: string, value: string | null) => {
    ...
    router.replace(buildWorkspaceHref(slug, `/integrations?${next.toString()}`));
  };
          ...
          onActivate={() => {
            ...
            if (attention.total === 1 && attention.mostUrgentId) {
              router.push(
                buildWorkspaceHref(
                  slug,
                  `/integrations/${attention.mostUrgentId}`,
                ),
              );
            } else {
...
function IntegrationCard({ integration }: { integration: IntegrationDto }) {
  ...
  return (
    <Link
      href={buildWorkspaceHref(slug, `/integrations/${integration.id}`)}
