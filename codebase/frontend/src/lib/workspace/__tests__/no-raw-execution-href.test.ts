 ...
 *
 * 이 경로는 `(main)/w/[slug]` 라우트라 항상 slug 를 붙여야 하는데, 리터럴이 여러 소비처에 흩어져
 * 한 곳이 slug 를 빠뜨리는 broken-link 회귀가 반복됐다(PR #865 rerun-modal·executions row-click·
 * dashboard·prev/next). raw 템플릿 리터럴을 소스에서 금지해 그 클래스를 CI 로 차단한다. ESLint
