...
describe("masked-markers", () => {
  /**
   * **이 테스트가 지키는 것** — 갱신됨 (`14_19_12` testing W1).
   *
   * 종전 서술은 *"backend 가 바뀌는 것은 못 지킨다 — 같은 파일 안의 리터럴-대-리터럴
   * 비교라 진짜 크로스체크가 아니다. 공유 패키지 추출이 선행돼야 값싸다"* 였다.
   * **그 추출이 됐다** — `MASKED_MARKERS` 는 이제 `@workflow/masked-markers` 를 재export
   * 한 것이고 backend 도 같은 패키지를 본다. 미러가 없으니 대조할 두 벌도 없다.
   *
   * 그래서 이 단언은 **SoT 리터럴의 회귀 고정**이다 — 패키지 값이 조용히 바뀌면 여기가
   * RED 다. 패키지 자신의 spec 에도 같은 pin 이 있으므로 이쪽은 *"프런트 소비 경로가
   * 그 값을 그대로 본다"* 를 함께 확인한다.
   *
   * > 이름을 정확히 유지하는 이유는 종전과 같다 — 지키지 못하는 것을 지키는 것처럼
   * > 부르면 다음 사람이 이 자리에 없는 방어가 있다고 믿는다. 이제는 반대로, **지키는
   * > 것을 못 지킨다고 적어 두면** 이미 닫힌 항목을 다시 열게 만든다.
   */
  it("마커 집합이 SoT 리터럴에서 이탈하지 않는다", () => {
