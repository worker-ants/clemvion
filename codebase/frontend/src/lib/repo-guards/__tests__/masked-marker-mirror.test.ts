 ...
 *
 * 그래서 backend 에 사본을 뒀는데, 그 중복이 다시 **두 번 결함의 근원**이 됐다(접두 경계를
 * 한쪽만 고치고 "양쪽 다 고쳤다" 고 적음 · 규칙 문단을 한쪽에만 추가). 대칭 캐너리로 막긴
 * 했지만 알고리즘이 바뀔 때마다 사람이 대칭을 재보증해야 하는 구조가 남았다.
 *
 * **`.github/workflows/repo-guards.yml` 이 그 구조를 없앴다** — `codebase/**` 어디가 바뀌든
 * 이 spec 을 돌리는 잡이 하나 생겼으므로 사본이 필요 없다. 경로 게이팅을 우회하려고 코드를
 * 복제하는 대신, 게이팅이 없는 자리를 하나 만든 것이다.
 */
