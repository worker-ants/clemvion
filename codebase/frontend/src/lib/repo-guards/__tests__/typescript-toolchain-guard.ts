...
//
// 워크스페이스 루트 탐색(`ROOT`)과 YAML 서브셋 리스트 추출(`listAtPath`)은 중립 모듈
// `_shared.ts` 에서 **재사용**한다. 같은 저장소에서 같은 일을 하는 파서를 두 벌 두면 한쪽만
// 고쳐지는 드리프트가 생긴다 — 실제로 이 저장소의 가드들이 반복해 겪은 실패 클래스다.
//
