...
>
> **위치 이동 (2026-07-16 grooming, 사용자 결정)**: `plan/in-progress/` → **`plan/research/`**.
> 본 문서는 **작업 plan 이 아니라 전략 리서치 산출물**(스스로 위 "성격" 줄에 그렇게 규정)이라
> "처리할 항목이 남은 진행 중 작업" 이라는 `in-progress/` 의 정의에 맞지 않았다. §8 체크리스트도
> 실행 작업이 아니라 **다른 plan 으로의 위임 인덱스**다. 그렇다고 `complete/`(= 모든 항목 종료) 도
> 아니다 — 아직 착지하지 않은 전략 항목(P1-2·P2-2·P3)이 살아있고, 리서치는 "완료" 되는 종류의
> 문서가 아니라 **참조되는** 문서다. 라이프사이클 정의는
> [`.claude/docs/plan-lifecycle.md §1`](../../.claude/docs/plan-lifecycle.md).
>
> **§8 위임 인덱스는 stale 하다 (2026-07-16 실측)** — 이 문서를 근거로 우선순위를 판단하기 전에
> 아래 교정을 반영할 것:
> - **P0-1(실행엔진 신뢰성)**: 대부분 **구현 완료** — intake 큐 PR1·admission gate #801·priority
>   3-tier #803·stalled 재배달 #798·orphan pending backstop #806. 동 항목의 "`spec/0-overview.md`
>   분산 워커 풀 문구 정직화" 도 이미 반영됨(`0-overview.md` 가 intake 큐/세그먼트 모델을 정확히 기술).
> - **P2-3(JWT_SECRET fallback)**: **해소됨** — PR #539 production fail-closed 가드. 아래 §8 이
>   링크한 `plan/in-progress/security-jwt-secret-fallback.md` 는 **존재하지 않는 dead link**.
> - **P0-2(spec drift 31건)**: 현재 `spec-sync-*` plan 들로 축소·분산 추적 중.
> - **P0-2b·P0-3·P1-3**: `ai-agent-tool-connection-rewrite.md`·`marketplace-and-plugin-sdk.md` 가 소유.
>
> 따라서 **아직 다른 plan 에 집이 없는 미착지 전략 항목은 P1-2(알림톡 대행사 노드)·P2-2(멀티에이전트/
> A2A 조사)·P3(업마켓 전환 결정) 3건**이다. 이 3건이 본 문서를 살아있게 하는 실질 내용이다.
...
### (2-b) 실행엔진 신뢰성 — 사용 방식과 무관한 기반 요건 (① 🔴 무조건 P0) · v2.3 일반화

> **v2.3 일반화**: 성능·안정성·대량·분산 처리 신뢰성은 "비즈니스 로직 실행 플랫폼"이라는 특정 포지셔닝의 조건부 요건이 아니라, **제품을 어떤 방식으로 쓰든(AI 챗 에이전트·한국 커머스 버티컬·범용 프로세스 자동화) 당연히 해결해야 하는 기반 요건**이다. 따라서 ①은 "목적이 X이면 P0"가 아니라 **무조건 P0**. v2.1의 🟠 하향("멀티 인스턴스가 안 깨진다")은 사실로서는 참이나, **"안 깨짐 ≠ 운영 신뢰성"** 이라 어떤 사용 시나리오에서도 부족하다.

**기준선 (durable execution 카테고리)**: Temporal(모든 스텝 event history 영속 → 워커 크래시 시 정확한 지점부터 재개, consistent-hashing task queue + backpressure + 자동 스케일) / n8n queue mode 벤치마크(**queue 162 req/s @ 0% 실패 vs single 23 req/s @ 31% 실패** — 7배 + 실패 제거; 큐 없으면 버스트에 backpressure 부재로 생산자가 소비자 압도).

**Clemvion 신뢰성 차원 맵 — "대기 단계는 Temporal급, 실행 단계는 n8n single-mode급"**:

| 차원 | Clemvion | 비고 |
|---|---|---|
| 정확성/상태 일관성 | 🟢 강함 | 단일 트랜잭션 전이·jobId 멱등성·frozen snapshot·config/output 직교 |
| 장기대기/사람개입 내구성 | 🟢 강함 | durable continuation·rehydration·무기한 보존 |
| **실행 중 크래시 복구** | 🔴 취약 | running takeover/재큐 없음, 30분 후 FAILED (resume은 `ai_agent` 한정) |
| **처리량/수평확장** | 🔴 취약 | fire-and-forget in-process, 큐 work-stealing 없음, LB 분산만 |
| **backpressure/과부하 안전** | 🔴 부재 | intake↔execution 큐 없음 → 버스트 시 무제한 동시 실행 |
| **멀티테넌트 공정성/자원 가드** | 🔴 부재 | §8 미구현 — 폭주 워크플로가 인스턴스 점유 |
| retry/DLQ/에러 격리 | 🟡 부분 | continuation DLQ 있으나 비용 증폭, 일반 노드 retry 한정 |
| 운영 관찰성 | 🟡 부분 | OTel traces·DLQ monitor·system-status, Prometheus metrics 없음 |

**좋은 소식 — 재설계 아닌 "이미 설계된 것을 짓기"**: 목표 아키텍처는 spec에 이미 있음(§4 BullMQ worker·§7 heartbeat·§8 가드, Planned). 최난도 조각(rehydration·영속 continuation·분산 recovery lock)은 **이미 구현** = §4가 올라설 토대. 남은 건 증분: ① intake 큐 도입(fire-and-forget→큐, work-stealing/backpressure) / ② 크래시 시 running을 checkpoint에서 재개(rehydration을 `ai_agent` 너머 일반 노드로 확장) / ③ §8 동시성 cap·실행 타임아웃 enforce(**단 타임아웃은 active running 시간 기준 — wait 대기 시간 미포함**) / ④ heartbeat 재큐. **build vs adopt**: BullMQ continuation을 이미 가진 상황에서 Temporal/Inngest 밑깔기는 과한 마이그레이션 → **§4를 기존 토대 위에 완성** 권고.

> **두 종류의 "stuck"을 반드시 분리 (v2.3)**: (a) **RUNNING execution의 워커 크래시** → 복구/재개 대상(②·④ heartbeat 재큐가 여기에만 적용). (b) **`waiting_for_input`(사용자 인터랙션 대기)** → 재큐·TTL 만료 **금지**. 무기한 durable 보존하고 **사용자 인터랙션이 도착할 때만** continuation 큐로 재개한다. 현 엔진이 이미 이렇게 동작(waiting은 stale-fail 30분 임계에서 제외·무기한 보존). 과거 테스트에서 wait를 재큐/만료시켰더니 사용자가 한참 뒤 선택하려 할 때 세션이 이미 만료된 사례가 바로 이 설계의 근거 — **heartbeat/타임아웃 메커니즘은 §7.1 RUNNING 한정으로 좁혀 구현하고 waiting 상태를 절대 건드리지 않아야 한다.**

**wedge와 충돌 아님**: 엔진 신뢰성은 버티컬 wedge의 **전제조건**(Cafe24 상점주가 주문처리·CS를 비즈니스 로직으로 올렸는데 버스트에 execution 흘리거나 크래시에 in-flight 잃으면 핵심 약속 붕괴). wedge=go-to-market, 엔진 신뢰성=기반 — 둘 다 P0, 층위가 다름.

**가장 위험한 정합성 갭**: `spec/0-overview.md`가 "분산 워커 풀 수평확장"을 현재형으로 광고하나 §4는 Planned. 사용 방식과 무관하게 이 문구로 제품을 파는 건 위험 → **문구 스코프 정직화 + 역량 구축** 둘 다 필요(신뢰성은 무조건 기반 요건이므로 결국 후자가 본질). (v2.1의 "마케팅 표현만 정정" 권고는 철회.)
...
## 1. 포지셔닝 — Clemvion의 정체 (v2.4 정정)

> **기반 요건(foundational) = "AI와 워크플로우의 범용 통합"을 신뢰성 있게 실행하는 플랫폼.** 한국 커머스(Cafe24)·옴니채널 챗은 그 위에 얹는 **강력한 통합 옵션이자 go-to-market 진입점**이지, 제품의 정체나 기반 요건이 아니다. (v2.3까지 "한국 커머스 버티컬 수직통합"을 코어/wedge로 과대 배치했던 프레임을 교정.)

Clemvion의 기반 가치는 **범용 AI+워크플로우 통합** — AI 에이전트가 워크플로우의 1급 구성요소로서 노드·도구·실행 흐름과 양방향으로 엮이고, 그 실행이 대량·분산·장애에도 신뢰성을 유지하는 것. 이 위에 선 **두 개의 진짜 해자(moat)**:
...
- **해자 B — AI-native 워크플로우 깊이**: Graph RAG(+3D 시각화) + MCP 클라이언트/Internal Bridge + Planner-first Conversational Building + 워크플로에 박힌 영속 멀티턴 에이전트. *(Cafe24 222 필드 카탈로그·옴니채널 챗·Presentation 노드는 이 범용 역량을 입증·확장하는 **통합 옵션의 모범 사례**다 — 해자 자체는 "범용 AI+워크플로우 통합"이고 커머스는 그 적용.)*

> **기반 요건의 직접 귀결**: (1) **신뢰성(분산/대량/장애복구)** 은 사용 방식 무관 무조건 P0(§0.1 (2-b)). (2) **②(임의 워크플로 노드 ↔ AI 에이전트 도구 연결)** 은 "AI+워크플로우 범용 통합"의 문자 그대로의 구현체이므로, mcp_가 도메인 액션을 커버하더라도 **코어 thesis에 직결**되는 작업이다(단순 주변 기능 아님).
...
**전략적 함의**: 코어는 "범용 AI+워크플로우 통합 플랫폼"이고, 한국 커머스는 그 위의 **통합 옵션 + 첫 go-to-market**이다(코어 대체 아님). 그 옵션 시장도 공백은 아니다 — CS는 채널톡, 마케팅은 Cafe24 앱스토어 앱이 선점. Clemvion이 이 옵션에서 차별화되는 이유는 이들이 **단일목적(point) 앱**인 반면 Clemvion은 같은 범용 엔진(프로그래머블+에이전틱+깊은 API)을 커머스에 적용한 것이기 때문. 즉 **"범용 AI+워크플로우 플랫폼"이 본체, "한국 커머스 적용"이 첫 칼끝**. 다른 버티컬·범용 자동화로도 같은 엔진이 확장된다.
...
- **① 실행엔진 신뢰성 — 사용 방식 무관 기반 신뢰성 갭** (🔴 무조건 P0, §0.1 (2-b)) — 정확성·장기대기 내구성은 🟢이나 **실행 중 크래시 복구·처리량/수평확장·backpressure·멀티테넌트 자원 가드 4개 차원이 n8n single-mode 수준**(§4/§7/§8 Planned). 어떤 사용 시나리오에서도 운영 신뢰성 미충족. 아키텍처는 이미 설계됨(증분 구축 가능). 단 복구/재큐는 RUNNING 한정 — `waiting_for_input`은 무기한 보존(§0.1 (2-b) "두 종류의 stuck").

> 등급 트레일: ②(node-as-tool)는 도메인 액션이 mcp_로 살아있어 🟡(§0.1 (1)). ①은 v2.1에서 🟠로 내렸다가 v2.3에서 **🔴 무조건 P0로 확정**(§0.1 (2-b)) — 신뢰성은 특정 포지셔닝 조건부가 아니라 사용 방식과 무관한 기반 요건이기 때문.
...
1. **기반 = 범용 AI+워크플로우 통합을 신뢰성 있게 실행 (v2.4)** — 코어는 "AI 에이전트가 워크플로우의 1급 구성요소로 엮이고, 그 실행이 대량·분산·장애에도 신뢰성을 유지"하는 범용 플랫폼. 한국 커머스는 그 위의 **통합 옵션·첫 칼끝**이지 코어가 아니다.
2. **수평 "커넥터 폭" 경쟁은 구조적으로 패배** — 커넥터 4개 + 마켓플레이스 0 + 플러그인 SDK 0 + custom 노드 enum 부재. n8n식 커넥터 수 경쟁은 봉쇄 → MCP로 우회. (단 이건 "통합 폭"의 문제이고, "AI↔워크플로우 통합 깊이"라는 기반과는 다른 축.)
3. **진짜 해자는 둘** — (A) 사람개입/대화형 신뢰성, (B) AI-native 워크플로우 깊이(Graph RAG·MCP·Conversational Building·영속 멀티턴 에이전트). 커머스 도메인 깊이는 (B)의 적용 사례. 코드로 실재 확인.
4. **P0 재구성 (v2.4)** — ① 실행엔진 신뢰성(크래시 복구·처리량·backpressure·멀티테넌트 가드)은 사용 방식 무관 무조건 P0(§0.1 (2-b)). ②(임의 노드↔에이전트 도구 연결)는 "AI+워크플로우 범용 통합"의 직접 구현체라 코어 thesis 직결. → 진짜 P0 = **①(엔진 신뢰성)·②(범용 노드-도구 통합)·③(통합/확장 봉쇄)·④(spec drift)**. ⑨(알림톡 대행사 노드)는 **통합 옵션 P1**(코어 아님). 복구/재큐는 RUNNING 한정 — waiting_for_input 무기한 보존.
5. **멀티에이전트는 다가오는 table stakes** — A2A 표준화 + 경쟁사 전면 채택. 중기 필수.
6. **엔터프라이즈(SSO/메트릭/Org)** — 업마켓 전환 결정 전엔 후순위(범용 SMB·셀프호스팅엔 SAML 불필요).

---

## 6. 우선순위별 액션 (재정렬)

### P0 — 기반: 신뢰성 + 범용 통합 + 정합성 (v2.4)
- **① 실행엔진 신뢰성 구축** (사용 방식 무관 기반 요건) — §4 BullMQ worker 모델 완성(intake 큐·work-stealing), 크래시 시 running checkpoint 재개(rehydration 일반 노드 확장), §8 동시성 cap·실행 타임아웃 enforce, §7 heartbeat 재큐. **모든 복구/재큐/타임아웃은 RUNNING 한정 — `waiting_for_input`(사용자 대기)은 재큐·만료 제외, 무기한 보존; 타임아웃은 active running 시간 기준.** 기존 continuation/rehydration 토대 위 증분. + `spec/0-overview.md` "분산 워커 풀" 문구를 역량과 일치시킴(스코프 정직화). **← 본 라운드 구현 착수.**
- **② 범용 노드↔에이전트 도구 통합 재구현** — "AI+워크플로우 범용 통합"의 직접 구현체(ND-AG-06/10/21 재작성). 도메인 액션은 mcp_로 커버되나 임의 워크플로 노드를 에이전트 도구로 쓰는 범용 경로가 코어.
- **③ 통합/확장 봉쇄 완화**: 핵심 SaaS 커넥터 + MCP-first(아래 P1) / **④ spec↔code drift 31건 수렴 로드맵**.

### P1 — 통합 옵션 + MCP 포지셔닝 + 차별화
- MCP-first 포지셔닝("422개가 아니라 MCP 10,000+ 서버") / 템플릿 갤러리 MVP / Graph RAG·Conversational Building 마케팅 / **(통합 옵션) 알림톡 대행사 연동 노드**(솔라피/비즈고/알리고, Cafe24 주문→알림톡) + 한국 커머스 첫 go-to-market — 코어가 아니라 첫 칼끝으로 추진.
...
> **Clemvion의 기반은 "AI와 워크플로우의 범용 통합을 신뢰성 있게 실행하는 플랫폼"이다. 한국 커머스(Cafe24)·옴니채널 챗은 그 위의 통합 옵션이자 첫 go-to-market이지 코어가 아니다. 진짜 자산은 _사람개입/대화형 신뢰성_ 과 _AI-native 워크플로우 깊이_ 두 해자. 그러나 기반 자체(① 실행 신뢰성·분산, ② 범용 노드↔에이전트 통합)가 아직 덜 지어져 있어, 차별화 마케팅보다 "기반 구축(P0)"이 선행돼야 한다. 그 뒤 MCP로 커넥터 폭을 우회하고, 커머스 옵션으로 첫 칼끝을 내며, LLMOps·멀티에이전트로 격차를 메운다.**

---

## 8. 후속 액션 체크리스트

- [ ] **P0-1 실행엔진 신뢰성 구축** (사용 방식 무관 기반 요건) — §4 worker 모델(intake 큐·work-stealing)·크래시 running checkpoint 재개·§8 동시성/타임아웃 가드·§7 heartbeat 재큐. **복구/재큐/타임아웃은 RUNNING 한정 — `waiting_for_input`은 무기한 보존·재큐/만료 제외, 타임아웃은 active running 시간 기준.** 기존 continuation/rehydration 토대 위 증분. + `spec/0-overview.md` "분산 워커 풀" 문구 정직화
- [ ] **P0-2 spec↔code drift 31건 수렴 로드맵** — `plan/in-progress/spec-sync-*` 우선순위화
- [ ] **P0-2b 범용 노드↔에이전트 도구 통합 재구현(②)** — "AI+워크플로우 범용 통합" 코어 (ND-AG-06/10/21 재작성)
- [ ] **P0-3 통합/확장 봉쇄 완화 착수** — 핵심 SaaS 커넥터 + MCP-first 결합
- [ ] **P1-1 MCP-first 포지셔닝 확정** — 제품 메시징/랜딩 1급 축 + `spec/0-overview.md` 비전 반영 여부
- [ ] **P1-2 (통합 옵션) 한국 커머스 첫 go-to-market** — 알림톡 대행사 연동 노드(솔라피/비즈고/알리고, Cafe24 주문→알림톡) + "Cafe24 상점주 AI 운영·CS" 진입. **코어 아님, 첫 칼끝**으로 추진
...
- [x] **P2-3 보안 백로그 해소 우선순위** — JWT_SECRET fallback·ENCRYPTION_KEY 분리·AuthConfig audit. **해소 (PR #539)**: production 에서 JWT_SECRET fallback 을 fail-closed 가드로 차단. *(종전 이 줄이 링크하던 `plan/in-progress/security-jwt-secret-fallback.md` 는 실존한 적 없는 dead link — 2026-07-16 제거.)*
