...
### 작업
- [ ] **V093 repoint (가역, 비파괴)**: NULL KB 마다 creds 출처 우선순위
      ...
      말미 `RAISE EXCEPTION` fail-loud: UPDATE 후에도 NULL 남은 KB 1건↑ → 전체 롤백 + id 목록 출력.
- [ ] **V094 DROP (비가역)**: `fk_kb_embedding_llm_config` + `embedding_llm_config_id` + `embedding_model` DROP.
      헤더에 rationale + `-- DOWN:` irreversible 명시. V093 commit(검증 통과) 후에만 진행.
- [ ] `resolveEmbedding` step-3 제거 → 2-step + 미해결 시 `notFound()`. JSDoc 갱신.
- [ ] KB 엔티티 `embeddingModel`·`embeddingLlmConfigId` 필드 제거.
- [ ] DTO 정리: create/update-knowledge-base, embedding-probe, knowledge-base-response.
- [ ] embedding.service `resolveEmbedding` 호출 인자(`embeddingLlmConfigId`/`legacyModel`) 제거.
- [ ] spec 갱신 (→ project-planner 위임): 2-navigation/5-knowledge-base.md, 6-config.md,
      5-system/8-embedding-pipeline.md 의 legacy step-3 서술 제거.
...
### 작업
- [ ] 백엔드 emit 사이트 신코드 전환 (status 결정 반영).
- [ ] 프론트 `loader-error-messages.ts` legacy 키 정리 (이미 양쪽 매핑 — UI 무영향).
- [ ] spec `5-system/3-error-handling.md` 표 갱신 (→ project-planner 위임).
- [ ] 관련 test (llm.service.spec, llm-preview.service.spec, rerank.service.spec,
      sanitize-loader-error.test) assert 갱신.
- [ ] 외부 소비자 Sunset/deprecation 정책 검토 (api-contract-reviewer).
...
- [ ] /ai-review → resolution-applier
- [ ] project-planner 위임 (spec 본문: 8-embedding-pipeline §5.5, 1-data-model §2.11, 3-error-handling §1.3 신코드, error-codes §3 retired, 7-llm-client §5.5/§6, data-flow/6-knowledge-base §2, 2-navigation/5·6 legacy 서술 제거)
- [ ] --impl-done
