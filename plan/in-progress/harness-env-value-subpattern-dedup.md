...
- [ ] **(조건부, 그 plan 이 남긴 유일한 미종결 항목) `plan-scan.ts` 가 452줄이 됐다** —
      > **수치 출처**: `wc -l codebase/frontend/src/lib/docs/__tests__/plan-scan.ts` = **452**
      > (2026-08-29 재실측). 원래 "449줄" 이라 적혀 있었는데 **처음부터 틀렸다** — 항목을
      > 낳은 커밋 `a6d916192`(#1146) 시점에도 452 였다. 그 뒤 이 파일은 무변경이라
      > 조건부 트리거는 여전히 미발동이고, "다음 확장 때 재판정" 처분은 그대로다.
