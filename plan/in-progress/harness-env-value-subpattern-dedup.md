...
- [ ] **(조건부, 그 plan 이 남긴 유일한 미종결 항목) `plan-scan.ts` 가 449줄이 됐다** —
      ...
      > **무엇이 낡았나**: "449줄" → **452줄**. 항목 자체(조건부 분리 유예)는 유효하고 트리거도 미발동이다 — 파일이 #1146 이후 손대진 적이 없다. 수치만 고치면 되고, 다음 확장 때 재판정한다는 처분은 그대로 둔다.
      > **실측**: `wc -l codebase/frontend/src/lib/docs/__tests__/plan-scan.ts` = **452**. 항목을 낳은 커밋 a6d916192(#1146) 시점에도 452 였다(`git show a6d916192:… | wc -l`) — 처음부터 3줄 틀렸다. 그 뒤 이 파일은 무변경.
