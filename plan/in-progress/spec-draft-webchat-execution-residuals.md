...
- [x] commit + PR (#916 머지)
- [x] **developer 위임 — 두 PR 완료**:
  - [x] **PR-1 (위젯)** single-flight coalesce + 확립세션 cancel — #917 머지. impl-prep BLOCK:NO(`.../17_54_21/`). TDD 25/25·lint·unit·build·e2e(252) PASS. `/ai-review`(`.../18_18_31/`) Critical 0·Warning 7 조치(RESOLUTION.md).
  - [x] **PR-2 (백엔드)** idle-wait reaper(EIA-RL-07) — 브랜치 `claude/webchat-idle-wait-reaper`. `WebchatIdleReaperService`(BullMQ repeatable) + `findIdleWebchatExecutionIds` + engine `markWebchatIdleTimeout`(트랜잭션 원자). impl-prep BLOCK:NO(reaper 영역 Critical 0; graph-rag Critical 무관→별건 `task_29ab68ff`). TDD(reaper/token/engine +에러경로)·lint·unit(backend 8090)·build·e2e(253, reaper 실 Postgres) PASS. `/ai-review`(`.../19_51_59/`) Critical 0·Warning 7+2 → W1(트랜잭션 원자화)·W2 문서화·W5 JSDoc·W6/testing 테스트 반영, W3/W4 DRY defer(RESOLUTION.md). spec Planned→구현됨 flip.
