...
- [x] 2라운드 `/ai-review` `review/code/2026/09/12/23_40_57` — **Critical 0 · WARNING 5**.
      셋은 이미 등재/조치불요, 둘은 `codebase/**` 라 고쳤고, 하나(W1)는 **이 배치에서 가장
      값진 지적**이었다.
      | # | 지적 | 처분 |
      |---|---|---|
      | 1 | 핵심 전제(22P02)가 **실 DB e2e 로 한 번도 검증되지 않았다** — 두 신규 테스트 다 mock | **e2e 2건 추가 + 그것으로 전제를 실측** (아래) |
      | 2 | `uuid.spec.ts` 의 *"호출부는 한 곳뿐"* 을 **내 변경이 거짓으로 만들었다** | 개수를 다시 박지 않고 **세는 법**을 적었다 |
      | 3 | 계약 비대칭 — 리뷰도 *"되돌릴 필요 없음"* | 기등재 (§C 에 미해소 명시 완료) |
      | 4 | 근거 주석 복제 (5개 리뷰어 중복 지적) | 기등재 — **주석-only** |
      | 5 | 신규 테스트 1건만 `.then()` — vacuous 위험 | `async/await` 로 통일 |

## 전제를 **실 DB 로 실측했다** — 이 배치에서 유일하게 미검증이던 사슬

`#1328` 이래 *"비-UUID → 22P02 → 필터 분기 없음 → 500"* 을 **코드 읽기와 spec 문장**으로만
세워 왔다. 2라운드 testing WARNING 이 그 갭을 정확히 짚었다 — mock 은 *"검증이 값을 거부하는가"*
만 말하지 *"통과시켰을 때 Postgres 가 정말 22P02 를 내는가"* 는 원리적으로 말해 주지 못한다.
저장소에 같은 판단의 선례가 있다(`webhook-trigger.e2e-spec.ts` B4).

e2e 2건을 넣고(305 → **307**, 둘 다 PASS), **검증을 떼고 한 번 더 돌렸다**:

| 케이스 | 기대 | 검증 제거 시 **실측** |
|---|---|---|
| `GET /api/users/me/login-history?cursor=<유효 ISO>\|not-a-uuid` | 200 | **500** |
| `GET …/background-runs/…?cursor=<base64 {s:유효, i:not-a-uuid}>` | 400 `INVALID_CURSOR` | **500** |

**둘 다 실 Postgres 에서 500 이 관측됐다.** 이로써 (a) 전제가 처음으로 실측됐고 (b) 새 e2e 2건이
vacuous 하지 않음이 같은 실행으로 증명된다. 원복 후 재실행 PASS.

- [x] 3라운드 `/ai-review` `review/code/2026/09/13/00_13_51` — **Critical 0 · WARNING 4**, 넷 다 조치.
      | # | 지적 | 처분 |
      |---|---|---|
      | 1 | `decodeCursor` 가 소유권 검사보다 먼저 돌아 "타 WS + 잘못된 커서" 가 404→**400** | 되돌리지 않고 **테스트로 고정** (소유권 mock 을 일부러 안 세워 순서 자체를 증거로) + 죽은 mock 제거 |
      | 2 | 근거 주석 **3중** 복제 — **세 라운드 연속** 지적 | JSDoc 한 곳으로 집약, 호출부는 참조로 압축 |
      | 3 | `uuid.ts` JSDoc 의 **닫힌 캐너리 목록**이 이 diff 로 낡음 | 목록을 열고 세는 법을 `uuid.spec.ts` SoT 로 가리킴 |
      | 4 | 2라운드에 내가 쓴 **재현 grep 명령이 틀렸다**(정의부까지 잡아 4줄) | 필터 추가 + 사유 기록 |
      > **4번이 이 라운드의 요점이다.** stale 한 개수를 "세는 법" 으로 바꿔 고쳤는데 **그 명령이
      > 그 자리에서 또 틀렸다** — 옆에 적은 "3곳" 과 첫 실행부터 어긋났다. 그리고 2번·3번은
      > 내가 2라운드에 **사본(`uuid.spec.ts`)만 고치고 SoT(`uuid.ts` JSDoc)를 안 고친** 결과다.
- [x] 4라운드 `/ai-review` `review/code/2026/09/13/00_36_16` — **Critical 0 · WARNING 5**.
      **정지 규칙의 조건을 충족해 수렴 선언.** 둘(CHANGELOG 고지 누락·경로 표기)은 공개 계약
      고지라 고쳤고 — `CHANGELOG.md` 는 저장소 루트라 `codebase/**` 를 낡게 하지 않는다 —
      나머지 셋은 기등재·보일러플레이트·산문이라 등재했다. **이 라운드는 `codebase/**` 수정 0.**
      | # | 지적 | 세 기준 해당? | 처분 |
      |---|---|---|---|
      | 1 | 계약 비대칭이 §8.2 와 어긋남 | 기등재 planner — 리뷰도 *"막을 사유 아님"* | 조치 없음 |
      | 2 | CHANGELOG 에 404→400 조합 누락 | **공개 계약 고지** | 고침 |
      | 3 | CHANGELOG spec 경로 표기 불일치 | 문서 정확성 | 고침 |
      | 4 | 테스트 커서 인코딩 보일러플레이트 5곳 | 아니오 | 등재 |
      | 5 | `isUuidShaped` JSDoc 이 계약과 감사 이력 혼재 | 아니오 | 등재 |
      > **5번은 직전 라운드가 만든 압력이다** — 주석 3중 복제를 닫으려 JSDoc 한 곳에 모았는데
      > 그 선택이 JSDoc 비대를 키웠다. 두 압력이 반대 방향이라 자를 선을 한 번 정해야 한다.
- [x] RESOLUTION.md 4건 — **막히기 전에** 썼다. `#1328` 은 6라운드를 다 고쳐 놓고도 이 파일이
      없어 게이트가 하나도 "resolved" 로 세지 않았다(fix 커밋은 증거가 아니다).
- [ ] `--impl-done`
...
| 라운드 | C/W | 성격 |
|---|---|---|
| 1 | 0/3 | **내 대조군이 자매보다 약했다** |
| 2 | 0/5 | **전제가 실 DB 로 검증된 적 없다** · 내가 만든 stale 주석 |
| 3 | 0/4 | **stale 을 고치려 쓴 grep 이 틀렸다** · 사본만 고치고 SoT 누락 |
| 4 | 0/5 | 고지 누락(고침) + 보일러플레이트·산문(등재) → 수렴 |

지적이 대부분 **내 검증 도구 자체**를 겨눴다 — 대조군의 강도, 전제의 미실측, 재현 명령의 오류,
사본/SoT 혼동. 4라운드에서 처음으로 그 축이 비었다.
