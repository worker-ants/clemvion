...
### 13라운드 리뷰 처분 (`review/code/2026/09/15/01_09_53` — **C0** · W5)

네 건이 전부 **«대칭의 나머지 절반»** 이다 — 형제 경로에 이미 해 둔 것을 이쪽엔 안 한
자리이거나, 그 비대칭을 내 문서가 «둘 다 했다» 고 적은 자리다.

| # | 처분 |
|---|---|
| W1 schedules 삭제 테스트가 **상한을 단언하지 않는다** | **수용·수정** — 형제(`triggers.service.spec.ts`)는 이미 `timeout:`·`lock:`·`remove` 순서 배열을 쓰는데 이쪽만 «락 키가 들어 있다» 였다. 같은 배열 형태로 바꿨다. 뮤턴트 M1(`timeoutMs` 인자 제거) 46건 중 **1 RED**, M3(락·삭제 순서 뒤집기) **1 RED** — 종전엔 둘 다 전건 GREEN 이었다 |
| W2 `SchedulesService.remove()` 에 «반쯤 삭제» 로깅이 없다 | **수용·수정** — `removeJob` 이 **이미 끝난 뒤**라 행 삭제 실패는 되돌릴 수 없는 비대칭 상태다. 형제와 같은 `.catch(logger.error(...)) + throw` 를 달고, 실패 경로 테스트를 새로 넣었다(로그 내용 · schedule 행 미삭제 · 감사 미기록). 뮤턴트 M2(catch 본문 제거) **1 RED** |
| W3 `CHANGELOG.md` 가 «삭제 경로 하나만 상한» 이라 적혀 있다 | **수용·수정** — 12라운드에 코드는 두 경로로 넓혔는데 같은 문서의 **다른 문단**이 옛 주어를 유지했다. 한 문서 안에서 25줄과 48줄이 서로 다른 집합을 가리키고 있었다 |
| W4 mock JSDoc 의 «2 감쌈 / 4 안 감쌈» 이 낡았다 | **수용·수정** — 12라운드가 `schedules` 를 «안전» 에서 «이관» 으로 옮기면서 3/3 이 됐다. 숫자를 지우는 대신 **왜 낡았는지**를 함께 적었다 — 이 PR 에서 목록형 서술이 낡은 네 번째 사례다 |
| W5 `update()` 182줄 | **이미 등재됨**(6라운드 W4) — 아래 표 유지. 세 라운드째 같은 지적이나 동작 결함이 아니고, 지금 쪼개면 이 PR 의 diff 가 «락 도입» 과 «구조 변경» 으로 섞인다 |

> **이번 라운드로 `codebase/**` 편집을 닫는다.** 남은 지적은 전부 후속 표로 가고, 다음
