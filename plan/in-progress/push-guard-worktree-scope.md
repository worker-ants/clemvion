...
| WARNING | 조치 |
|---|---|
| 1 `main()` 의 `_push_targets` 폴백 무검증 | **내 2차 RESOLUTION 이 이 경로를 "커버됨" 이라 주장했으나 틀렸다.** 기존 테스트는 `_worktree_branches` 자체의 fail-open 을 탈 뿐 `main()` 의 `except` 를 타지 않는다 — `targets = []` mutation 이 **39/39 green 생존**. `_push_targets` 를 실제로 raise 시키는 `test_push_targets_crash_falls_back_to_cwd` 신설(M7). 2차 RESOLUTION 의 과대 표기도 정정 |
...
- [x] 테스트 **20건** 신설 + 카탈로그 등재 (리뷰 반영으로 9 → 18 → 19 → 20)
- [x] mutation 실측 **8건**
- [x] harness 전체 **487 passed**
- [x] `/ai-review` — 1차(C0/W7) · 2차(C0/W2) · 3차(C0/W2) 전량 반영. 4차 게이트 리뷰 예정

> **교훈 — "커버된다" 는 추론이 아니라 실측이어야 한다.** 1차 후 "테스트로 커버" 라고 적었다가
> 2차가 PLAN 게이트 미검증을 찾았고, 2차 후 "위 2건이 커버" 라고 적었다가 3차가 `main()` 폴백
> 미검증을 찾았다. 두 번 다 **내가 뮤턴트를 심은 지점만 커버로 세고, 이름·모양이 비슷한 인접
