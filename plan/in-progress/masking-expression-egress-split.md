...
| M2 | `DEFAULT_SENSITIVE_KEYS` 에서 `idToken` 제거 | 포함관계 캐너리가 **케이스를 잃어** 조용히 통과할 위험 | ⚠️ **1 failed / 40 passed** — 잡은 건 **기존 명시 테스트**, 내 캐너리는 예상대로 조용히 줄었다 |
| M3 | `CREDENTIAL_KEY_PATTERN` 에서 `[a-z0-9_-]*token` 제거 | 포함관계 캐너리 token 계열 다수 RED | ✅ **13 failed / 28 passed** |

### M2 가 드러낸 것 — 파생 fixture 는 **한 방향만** 지킨다
...
**M2 를 안 돌렸으면 "포함관계 캐너리가 목록 축소도 잡는다" 고 잘못 믿었을 것이다.**
예측이 어긋난 뮤턴트가 가장 많이 알려줬다.
