...
> 관련 spec: [`spec/7-channel-web-chat/2-sdk.md §3(재전송)`](../../spec/7-channel-web-chat/2-sdk.md) · [`1-widget-app.md §3.1`](../../spec/7-channel-web-chat/1-widget-app.md)
...
**두 항목 모두 spec 변경이 아니라 spec-impl 갭이다** — §3(재전송) 이 이미 동작을 확정해 뒀으므로 `project-planner` 위임 불요(developer 트랙).

## 실측 근거 (계획 수립 시 재현)

두 갭 모두 **코드 정독이 아니라 실행으로 확인**했다.

### A. spec §3(재전송) 위반 — 먼저 보낸 config 가 이긴다
...
`applyConfig` 호출 1건 = 1세대. 각 시도가 진입 시 `const bootGen = ++bootGenRef.current;` 하고 **모든 `await` 뒤** `if (bootGenRef.current !== bootGen) return;`. 나중 boot 이 앞선 boot 을 대체 → 마지막 boot 의 config 가 적용된다(§3(재전송) 충족).
...
- [x] **A-4. §3(재전송) 회귀 테스트 신설** — probe A 를 정식 테스트로: 마지막 boot 의 config 가 적용되는지 + resolve 순서 역전에도 성립하는지.
...
**§3(재전송) 위반 해소 확인**: resolve 순서를 역전시켜도 마지막 `wc:boot` 의 config 가 적용된다(도입 전엔 먼저 보낸 쪽이 이겼다).
...
- 그래서 저장 세션이 남고, host 가 `wc:boot` 을 재전송하면(§3(재전송) — 외형 갱신 등, 관리자 미리보기가 실제로 그렇게 한다) `applyConfig` 복원 분기가 그 세션을 `RESTORED` 로 되살려 **ended → streaming** 으로 부활시킨다.
...
`2-sdk.md §3(재전송)` 이 재전송 계약의 SoT 인데 그 `code:` 는 SDK 패키지만 가리켰다 — **§3(재전송) 의 위젯 측 구현에 증거 링크가 없었다**(`1-widget-app.md` 는 `channel-web-chat/**` 를 덮지만 재전송을 서술하지 않는다). 저장소 기존 패턴(주석 달린 `code:` 항목, 예: `2-navigation/10-auth-flow.md`)대로 `host-bridge.ts`·`use-widget.ts` 를 근거 주석과 함께 추가했다.
...
대체된 시도가 복원 seed 에서 "이미 종료된 세션" 을 발견하면 `finalizeEnded` → `teardownSession` → **world 세대 증가**가 일어난다. 그 무효화는 **정당한데**(세션이 실제로 종료됐다), `isAttemptStale` 이 두 축을 OR 로 보므로 **아직 살아있는 마지막 부팅까지 함께 stale 화**해 §3(재전송) 을 깨뜨린다 — 그 부팅은 아직 어떤 세션도 건드리지 않았는데 "내 world 가 사라졌다" 로 오독하고 물러난다.

- **재현**: `CRIT:: 최종 config.plan = A (spec §3(재전송) 기대 = B)` — 내가 고치려던 바로 그 위반이 다른 경로로 재발했다.
...
- **검증**: `CRIT2:: plan=B | phase=ended` — §3(재전송) 충족 + 종료 유지. 회귀 테스트 신설, mutation(supersede 검사 제거) → 그 테스트만 실패.
...
리뷰어 7명 전원 완료. CRITICAL 3건 전부 **실측 재현 후** 처리했다.

### 1. A-6 되돌림 — 일시적 명령 실패가 살아있는 대화를 영구 파괴 (requirement CRITICAL)

`sendCommand` 비-410 경로의 `teardownSession()` 이 5xx·409·form 4xx·네트워크 순단을 전부
"서버 확인된 종료" 로 취급해 storage 를 파괴했다. `3-auth-session.md` §3.1-3 의 정리 조건 열거에
"그 외 명령 실패" 는 없고, §3.1-2 는 "200 + running → 복원" 을 명시한다.

실측(격리 worktree, 그 한 줄만 mutate): 있으면 `저장세션=소거 / 새로고침후 phase=collapsed`,
없으면 `보존 / streaming / e1`. `getStatus` 는 내내 `200 {status:"running"}`.

리듀서의 `RESTORED`/`BOOTED` `ended` 가드도 같은 뿌리라 함께 되돌렸다 — 그 가드가 발화 가능한
유일한 상황이 곧 spec 이 복원하라고 명시한 상황이었다(storage 를 지우는 종료 경로는 전부 저장
세션을 함께 지우므로 `RESTORED` 자체가 오지 않는다).

**A-6 이 막으려던 부작용은 이미 같은 PR 의 `sessionEstablished()` 복원-스킵이 막고 있었다.**
teardown 은 불필요했을 뿐 아니라 유해했다. 즉 A-6 전체가 순변경 0 — CHANGELOG 에서도 뺐다.

**이 클래스의 8번째 거울상이다.** 앞선 7번은 "가드를 어느 축에 다느냐" 였고, 이번엔 **"내가 막으려는
게 진짜 막아야 할 것인가" 를 spec 에 묻지 않은 것**이 원인이다. 리뷰어가 내 테스트의 mock 이
`{status:"running"}`(서버 생존)을 고정해 두고도 storage 소거를 단언한다고 지적했다 — 테스트가
스스로 반증하고 있었는데 내가 못 봤다.

### 2. 대체된 부팅의 지연 `getStatus` 가 화면을 되감음 (concurrency CRITICAL)

리뷰어가 코드 추적만 하고 "실측하진 않았다" 고 명시 → 직접 재현 후 fix.
`seedWaitingFromStatus` 가 boot 토큰을 optional 로 받아 **WAITING dispatch 분기만** 게이팅한다.
종료 확정 분기는 boot 축을 일부러 보지 않는다 — 한 함수에 staleness 정책이 둘 공존하며, JSDoc 에
표로 명문화했다.

### 3. `bootGenRef` JSDoc 유실 재발 (documentation CRITICAL)

C3 와 같은 클래스가 **심볼만 옮겨** 재발했다(`unmountedRef` 선언이 인접성을 끊음). 블록에 재발
방지 경고를 명문화하고 `ts.getJSDocCommentsAndTags()` 로 10개 심볼 전수 재실측(전부 1개).

### 4. 블라인드 스팟 2건 (testing WARNING)

- ① `beginBootAttempt` 가 world 축까지 무효화해도 385건 통과 → **위 §2 의 짝 테스트가 닫았다**(재확인: 이제 정확히 1건 실패).
- ② `cannotApplyConfig` 의 `unmountedRef` 체크 제거해도 389건 통과 → **실재. 회귀 테스트 추가**(언마운트 후 도착한 embed 응답이 webhook POST 를 쏘지 않는다).

### 5. 문서 정합

- CHANGELOG 항목 1·3 이 **되돌려진 설계**를 서술(requirement·documentation 교차 지적) → 순변경만 남겨 재작성.
- 클로즈 표기 2단계 정정: (1) `§106` → `§110` 39건(내 frontmatter 4줄이 대상 문단을 밀어냄), (2) 후속 `--impl-done`(19_46_54) 이 행번호 표기 자체를 문제 삼아 `§110` → `§3(재전송)` 41건(섹션+조항명)으로 재정정. (배경·판정은 아래 "## 이월 (신규)" 의 `§NNN` 항목.)
- `sessionEstablished()` JSDoc 에 `pendingResetRef` 불변식 상호 참조 추가(concurrency WARNING).
...
- **`§NNN` 행-번호 clause-id 가 구조적으로 취약** — 이번에 실제로 깨졌다(내 frontmatter 4줄이 대상 문단을 밀어냄). `§106`→`§110`(39건)로 1차 정정했다가, `--impl-done` 이 행번호 표기 자체를 문제 삼아 `§3(재전송)`(섹션+조항명, 41건)으로 재정정했다.
  ...
  - 안정적 앵커 도입은 저장소 전역 규약 변경이라 planner 트랙 — 이번 PR 범위 밖.

## 후속 (23_58_23 · 00_51_53 처리 — 2026-07-18)

되감기 방어의 **9·10번째 거울상**. 이 클래스가 왜 반복 실패했는지와 최종 불변식을 남긴다.

### 9번째 (23_58_23) — `start()` 무방비 되감기 (CRITICAL, 3인 재현)

18_39_11 이 `seedWaitingFromStatus` 의 boot 게이트를 `applyConfig` 복원 분기에만 배선하고 `start()`
호출부는 "world 축만 필요" 로 남긴 것이 구멍이었다(그 라운드 concurrency INFO 가 "별도 수정 불필요"
로 잘못 종결). `start()` 가 세션을 쓴 직후·자기 getStatus 응답 전 창에 재전송이 끼면 같은 되감기(+
EventSource 이중 생성)가 재현된다. concurrency·requirement·side_effect 3인이 독립 재현.
→ (당시) `start()` 가 진입 시점 bootGen 을 읽기전용 스냅샷으로 캡처해 넘기도록 fix(`7cfbf2557`).

### 10번째 (00_51_53) — 그 fix 가 만든 고착 (CRITICAL, 3인 재현)

`7cfbf2557` 이 반대 구멍을 냈다. **아무것도 복원 못 하는 no-op 재전송**이 `start()` 의 webhook
in-flight(persist 전) 창에 끼면 `bootGenRef` 만 헛되이 올라, start() 자신의 정당한 seed 가 거짓
stale 처리돼 WAITING·openStream 을 둘 다 스킵 → **스피너에 영구 고착**. requirement·testing·
side_effect 3인이 독립 재현. 국소 수정("persist 후 캡처")도 또 다른 interleaving 구멍이 남았다
(재전송의 `beginBootAttempt` 는 persist 전, 복원은 persist 후).

### 최종 불변식 — boot 축을 버리고 `sessionEstablished()` 로

boot 세대 비교는 "더 최신 재전송이 이 세션을 **실제로 넘겨받았나**" 의 proxy 였고, 그 proxy 가 두 번
(함수 경계에서 안 닿음 / no-op 재전송) 깨졌다. 진짜 불변식은 **"스트림이 이미 열렸나"**
(`sessionEstablished()`) — 열린 순간부터 SSE 가 표면의 단일 진실이니 지연 seed 는 스킵, 안 열렸으면
(no-op 재전송 포함) 이 seed 가 그린다. `replay_unavailable` 폴백만 `allowWhileStreaming` opt-in 으로
통과(자기 스트림 재동기화). (`pendingResetRef` 때 "폐기 로직을 통째로 없앤" 것과 같은 "진짜 불변식으로
단순화" — `cffee0d28`.)

**⚠ 정정 (01_44_21)**: 위 재설계 커밋에는 "`openStream` 이 seed 반환 직후 동기 실행이라 이중 스트림도
원천 차단" 이라 적었으나 **오판이었다** — `await seedWaitingFromStatus` 와 호출부 `openStream` 사이엔
microtask 경계가 있어, 겹친 두 seed 가 같은 flush 에서 통과하면 둘 다 openStream 을 부른다(esCount=2,
3인 재현). 그래서 seed 게이트(표면)의 **짝**으로 `openStream` 직전에도 `sessionEstablished()` 게이트를
`start()`·`applyConfig` 양쪽에 뒀다(`77805bd32`). openStream=closeStream→set 이라 최종 상태는 어차피
단일 스트림으로 수렴하지만(낭비성 생성일 뿐 correctness 아님, MEDIUM) 그 짝 게이트로 생성 자체를 없앴다.
두 게이트는 resolve 순서별로 각각 필요해 대칭 회귀 테스트 2개로 개별 고정했다(`94b66b212`).

mutation 양방향 고정: 게이트 제거 → 되감기 2건 실패 / opt-in 제거 → replay 재동기화 실패.

### 그 외 정합 (00_51_53 WARNING)

- CHANGELOG 항목 3 을 최종 메커니즘(sessionEstablished, start() 경로 포함)으로 재작성.
- 되돌려진 C1 메커니즘을 서술하던 테스트 주석(`§3(재전송): 대체된 시도의 종료 확정...`) 정정
  (18_39_11 RESOLUTION 이 "fix" 표시했으나 CHANGELOG 만 고치고 테스트 주석은 놓쳤던 것).
- `useEiaSession` 분리 이월을 별도 plan `webchat-usewidget-extraction.md` 로 분리(산문 매몰 방지).
- 표기 정정 서술의 "일어난 적 없는 단계"(일괄 치환 부산물) → 실제 2단계로 정정. 깨진 `§후속-2`
  자기참조 제거.

## 후속 (01_44_21 · 02_25_54 처리 — 2026-07-18)

되감기 방어의 **11번째 거울상 + 테스트 층위 재발**. 등급이 CRITICAL→CRITICAL→MEDIUM→MEDIUM(코드버그
없음)으로 하강하며 종착했다.

### 11번째 (01_44_21) — 이중 EventSource (MEDIUM, 3인)

sessionEstablished() 재설계가 seed 게이트(표면)는 고쳤으나, `await seedWaitingFromStatus` 와 호출부
`openStream` 사이 **microtask 경계** 때문에 겹친 두 seed 가 같은 flush 에서 통과하면 둘 다 openStream 을
불렀다(esCount=2). openStream=closeStream→set 이라 **단일 스트림으로 수렴**(낭비성 생성, correctness
아님 → MEDIUM). seed 게이트의 **짝**으로 openStream 직전 게이트를 start()·applyConfig 양쪽에 추가(`77805bd32`).
내 "동기 실행이라 원천 차단" 오판 정정.

### 12번째 (02_25_54) — 테스트 층위 비대칭 (MEDIUM, 코드버그 없음)

8인 전원이 openStream 게이트 fix 를 정확하다고 확인. 단 double-stream 테스트가 한 resolve 순서만 재현해
**start() 게이트(:673)만 제거해도 전원 통과**하는 커버리지 갭(requirement·testing 독립 재현) — "비대칭
가드 누락" 이 테스트 층위에서 재발한 형태. resolve 순서 파라미터 헬퍼로 두 방향 대칭 고정(`94b66b212`),
start() 누락 dep 도 처리.

### 수용한 잔여 (비차단 — 재발방지 기록)

- **3-way+ 겹침의 순간적 표면 콘텐츠 race** (concurrency WARNING): boot 축으로 pruning 대상인 재전송의
  WAITING 콘텐츠가 순간 최종 표면을 차지할 수 있다. **스트림 개수엔 무영향, `cffee0d28`(재설계)부터의
  pre-existing, SSE 가 현재 waiting 이벤트로 즉시 교정하는 자가치유 transient.** 좁은 도달 조건 + 자가치유라
  수용. 근본 제거는 이 이미 까다로운 함수에 더 큰 복잡도를 요구하고 실익이 marginal.
- **짝 게이트의 구조적 강제 부재** (maintainability WARNING): openStream 게이트가 두 호출부(start·applyConfig)의
