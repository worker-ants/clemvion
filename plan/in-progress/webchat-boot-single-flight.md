...
- [ ] **A-0. (선결) `guardedAwait`/토큰 캡슐화 채택 여부 결정** — 보류 근거("소비자 하나뿐")가 A 로 소멸하므로 재평가하고 **판단 근거를 A-1 과 함께 기록**. 채택 안 해도 "왜 수동 다축이 이 경우엔 낫다고 봤는지" 를 남긴다.
- [ ] **A-1. 계약 문서 선행 갱신** — `pendingResetRef` JSDoc 에 "superseded 된 부팅은 '성공하는 부팅'이 아니므로 리셋을 이행하지 않는다"를 명시. `bootGenRef` JSDoc 신설(축 구분·조기 return 비-카운트 근거). **코드보다 먼저** 한다.
- [ ] **A-2. supersede 구현** — A-0 결정대로.
- [ ] **A-3. 기존 회귀 테스트 2건 기대값 정제** — A-1 의 계약을 근거로 `hookPosts` 기대를 조정하고, **"리셋이 소실된 게 아니라 다음 성공 부팅으로 이월됐다"**를 단언에 추가(그냥 `0` 으로 낮추지 말 것 — 소실과 구분되지 않는다). 예: 이어서 성공 boot 을 한 번 더 보내 그때 `hookPosts === 1` 이 되는지.
- [ ] **A-4. §106 회귀 테스트 신설** — probe A 를 정식 테스트로: 마지막 boot 의 config 가 적용되는지 + resolve 순서 역전에도 성립하는지.
- [ ] **A-5. mutation 검증** — 아래를 **전부** 돌린다:
...
- [ ] **B-1. (A 완료 후 재평가)** 동기 구간 비-async 추출. 값어치 없다고 판단되면 **하지 않고 근거를 기록**.
