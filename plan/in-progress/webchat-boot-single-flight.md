...
> 관련 spec: [`spec/7-channel-web-chat/2-sdk.md §110`](../../spec/7-channel-web-chat/2-sdk.md) · [`1-widget-app.md §3.1`](../../spec/7-channel-web-chat/1-widget-app.md)
...
**두 항목 모두 spec 변경이 아니라 spec-impl 갭이다** — §110 이 이미 동작을 확정해 뒀으므로 `project-planner` 위임 불요(developer 트랙).

## 실측 근거 (계획 수립 시 재현)

두 갭 모두 **코드 정독이 아니라 실행으로 확인**했다.

### A. spec §110 위반 — 먼저 보낸 config 가 이긴다
...
`applyConfig` 호출 1건 = 1세대. 각 시도가 진입 시 `const bootGen = ++bootGenRef.current;` 하고 **모든 `await` 뒤** `if (bootGenRef.current !== bootGen) return;`. 나중 boot 이 앞선 boot 을 대체 → 마지막 boot 의 config 가 적용된다(§110 충족).
...
- [x] **A-4. §110 회귀 테스트 신설** — probe A 를 정식 테스트로: 마지막 boot 의 config 가 적용되는지 + resolve 순서 역전에도 성립하는지.
...
**§110 위반 해소 확인**: resolve 순서를 역전시켜도 마지막 `wc:boot` 의 config 가 적용된다(도입 전엔 먼저 보낸 쪽이 이겼다).
...
- 그래서 저장 세션이 남고, host 가 `wc:boot` 을 재전송하면(§110 — 외형 갱신 등, 관리자 미리보기가 실제로 그렇게 한다) `applyConfig` 복원 분기가 그 세션을 `RESTORED` 로 되살려 **ended → streaming** 으로 부활시킨다.
...
`2-sdk.md §110` 이 재전송 계약의 SoT 인데 그 `code:` 는 SDK 패키지만 가리켰다 — **§110 의 위젯 측 구현에 증거 링크가 없었다**(`1-widget-app.md` 는 `channel-web-chat/**` 를 덮지만 재전송을 서술하지 않는다). 저장소 기존 패턴(주석 달린 `code:` 항목, 예: `2-navigation/10-auth-flow.md`)대로 `host-bridge.ts`·`use-widget.ts` 를 근거 주석과 함께 추가했다.
...
대체된 시도가 복원 seed 에서 "이미 종료된 세션" 을 발견하면 `finalizeEnded` → `teardownSession` → **world 세대 증가**가 일어난다. 그 무효화는 **정당한데**(세션이 실제로 종료됐다), `isAttemptStale` 이 두 축을 OR 로 보므로 **아직 살아있는 마지막 부팅까지 함께 stale 화**해 §110 을 깨뜨린다 — 그 부팅은 아직 어떤 세션도 건드리지 않았는데 "내 world 가 사라졌다" 로 오독하고 물러난다.

- **재현**: `CRIT:: 최종 config.plan = A (spec §110 기대 = B)` — 내가 고치려던 바로 그 위반이 다른 경로로 재발했다.
...
- **검증**: `CRIT2:: plan=B | phase=ended` — §110 충족 + 종료 유지. 회귀 테스트 신설, mutation(supersede 검사 제거) → 그 테스트만 실패.
...
- `§106` → `§110` 39건 정정 — 내가 frontmatter 에 넣은 4줄이 대상 문단을 밀어냈다.
- `sessionEstablished()` JSDoc 에 `pendingResetRef` 불변식 상호 참조 추가(concurrency WARNING).

## 이월 (신규)

- **`ERROR` 가 `phase: "ended"` 로 보내는 것 자체** — `1-widget-app.md` §2 Form 은 "실패 시 `error.details` 표시·재제출" 을 약속하는데 현 구현은 일시적 실패에도 대화를 끝낸다. **이 PR 이전부터의 gap**(A-6 이 이걸 "영구 소실" 로 격상시켰던 것을 되돌렸을 뿐). 제품 결정이 필요하다 — "비-410 실패는 종료인가" 를 정하면 spec §3.1-3·§2 를 명문화해야 하므로 **project-planner 트랙**.
- **`§NNN` 행-번호 clause-id 가 구조적으로 취약** — 이번에 실제로 깨졌다(4줄 밀림). spec 조항에 안정적 앵커를 주는 게 근본 해결이고 이는 spec 규약 변경이라 planner 트랙. 지금은 §110 으로 정정만 했다.
