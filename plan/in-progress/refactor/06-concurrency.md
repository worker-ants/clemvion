...
**spec 대조**: B — §4 의 "execute() 는 execution-run 큐 발행" 모델에서 executeAsync(sub-workflow 비동기)는 대상 명시 없음 — 드리프트는 아니나 §4 intake 모델과 비대칭인 잔여 fire-and-forget. 큐 경유 경로는 W7 fix 로 2차 실패까지 격리돼 있었고, **이 분기도 M-4 Option B(2026-07-03)로 동일하게 `failFirstSegmentSetupBestEffort` best-effort 마감으로 통일**됐다(과거엔 단순 로그 catch 라 RUNNING/PENDING 잔류 → §7.1 stale fail(30분, `started_at` 채워진 RUNNING 한정)이 최후 부분 커버였다). **잔여 비대칭(§4 큐 모델 밖 fire-and-forget 자체)은 Option A(큐 통일)로 후속** — PR2b admission "대상 한정" 결정 대기.
...
- [x] 완료 — ✅ 2026-06-10 (refactor-approved-batch): `freezeSharedCacheValues` dev/test deep freeze(NODE_ENV!==production), branch clone 직후 한정, production 무변경. 회귀 가드 2건. — `containers/parallel-executor.ts`
