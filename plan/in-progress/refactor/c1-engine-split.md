...
**후속(impl-done INFO)** — ✅ **완료 (후속 ①, 2026-06-19)**: `ExecutionCancelledError`(workflow-errors)·EngineDriver 인터페이스 신규 5멤버 `@internal` JSDoc 대칭 추가 + `ExecutionGraphState`/`NodeDispatchLoopParams` → 신규 leaf `types/graph-dispatch.types.ts` 이동(engine-driver.interface.ts↔service.ts 타입 레벨 순환 해소). 커밋 `29e38a38`(impl)·`8a9d8a06`(ai-review INFO 주석). TEST lint·unit(805)·build·e2e(34/202) ✓. ai-review LOW·C0·W4(전부 수용/검증)·impl-done **BLOCK:NO**(`review/consistency/2026/06/19/01_58_07`). RESOLUTION `review/code/2026/06/19/01_41_04`. PR: `claude/engine-jsdoc-leaf-1431bf` (#생성후 기재).
