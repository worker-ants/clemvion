...
## PR2 — AiTurnOrchestrator + EngineDriver — 설계 완료 (실행 대기)

> **외과적 추출 청사진: [c1-pr2-aiturn-blueprint.md](./c1-pr2-aiturn-blueprint.md)** — 메서드별 라인·콜백
> 표면, EngineDriver 인터페이스(~10멤버), registry 위임, 테스트 이전 맵, 6-phase 순서, 회귀 위험.
> 브랜치 `claude/engine-split-s2-aiturn` (base=PR1) 생성됨. **최고 회귀 민감 단계** (AI 멀티턴 park/resume).

AI 멀티턴 생명주기 ~900줄 추출: `waitForAiConversation`·`processAiResumeTurn`·`handleAiMessageTurn`·
