...
## Follow-up (별건, 본 PR 범위 밖)

- 복구 코드 KDF(argon2id/scrypt) 전환 — pre-existing, 보안 개선 task
- spec Rationale 등재(planner): 4-security.md sanitize deny-by-default + 빈입력 동작 1행, 5-system/1-auth.md otplib v13 업그레이드 근거, 1-data-model.md "(otplib base32)" 라이브러리 무관 표기
