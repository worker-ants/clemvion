...
- [x] **완료 (Option A, 단계별 PR — god-handler 분할 3단계 전부 추출)** — `nodes/ai/ai-agent/ai-agent.handler.ts`. 핸들러 3,402→219줄 facade. 1단계 `AiConditionEvaluator`(#665)·2단계 `AiMemoryManager`(#668)·3단계 `AiTurnExecutor`(본 단계) 무상태 collaborator 추출, 핸들러는 NodeHandler 표면+polymorphic 계약+엔진 진입점만 보유하는 composition-root facade.
  ...
  - **planner 후속(비차단 SPEC-DRIFT, M-1 god-handler 분할 전체 완료 — 일괄 처리)**: `1-ai-agent.md` frontmatter `code:` 에 `ai-condition-evaluator.ts`·`ai-memory-manager.ts`·`ai-turn-executor.ts` 등재 + §6.1 step 3a 구현 참조(`ai-agent.handler.ts classifyToolCalls` → `ai-condition-evaluator.ts AiConditionEvaluator.classifyToolCalls`; turn 루프 → `ai-turn-executor.ts AiTurnExecutor.executeSingleTurn/executeMultiTurn`) + §6.1 단계 1.3/1.5/2.7 구현 참조(→ `ai-memory-manager.ts`) 갱신 + §7 각주 `ToolCallTrace` shape 에 `startedAt?`/`finishedAt?` 추가(WS §4.4 정합) + §6 서두 레이어 주석(handler `AiAgentHandler`→`AiTurnExecutor` 위임 / 엔진 `AiTurnOrchestrator` multi-turn lifecycle 구동) + `interaction-type-registry.md` frontmatter·§12.6 SoT 참조 경로 갱신(`FORM_SUBMITTED_*` 이동 반영). 근거: `review/consistency/2026/06/21/23_30_04/SUMMARY.md`.
...
- [x] **완료** (권장안 A 의 Router→Guard→Persistence 3단계 전부 추출 — `streamMessage` 혼재 해소) — `workflow-assistant-stream.service.ts`. 1단계 `AssistantToolRouter`(#670)·2단계 `AssistantFinishGuard`(#680)·3단계 `AssistantTurnPersistenceService`(본 PR) 무상태 collaborator 추출, `streamMessage` 는 SSE 조립·tool-loop·turn-scoped 상태 소유만 잔류.
  ...
  - **planner 후속(비차단 SPEC-DRIFT, M-3 god-service 분할 전체 완료 — 일괄 처리)**: `4-ai-assistant.md` §10/§7 Rationale 의사코드 `this.persistAssistantTurn(...)` → `this.turnPersistence.persistAssistantTurn(...)` 위임 경로 갱신 + 최종 row 판정 `consecutiveStallRounds > 0` → `totalStallCount > 0` 정정(코드가 더 정확, revert 금지 — stall-then-progress 버그 재도입) + frontmatter `code:` glob 신규 파일 자동 커버 확인 + `data-flow/7-llm-usage.md` 행위자 `WorkflowAssistantStreamService` → `AssistantTurnPersistenceService` 위임 표기. 근거: `review/consistency/2026/06/24/10_12_59/`·`review/code/2026/06/24/10_03_52/`.
