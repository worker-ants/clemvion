...
- [~] **진행 중 — 재스코프 (2026-07-02)**: C-1 엔진분할(02 C-1 완료)로 대상 파일이 재편돼 원안 "50+ 1파일" 이 아니라 **~124건 / ~15파일** 로 분산(원 라인 refs `:370-371` 등 stale). 사이트별 성격이 달라 일괄 전환 불가 — **SAFE-TORECORD**(property-접근 전용)·**STORE-PRESERVE**(저장/보존, 배열 보존 필요)·**LOAD-BEARING**(throw 전제)·**RESUME-STATE**(§7.4 스키마 필요)로 분류해 검증된 클러스터 PR 로 진행. 사용자 결정(2026-07-02): 전체 ~124건 대상.
  ...
  - **RESUME-STATE 클러스터 (본 PR)**: `utils/resume-state.schema.ts` 신설 — `_resumeState`/`_resumeCheckpoint`/`_retryState` 3종의 형태 SoT 를 **zod 스키마**(I-5 credential allow-list executable 문서화 + I-8 라이프사이클 구분)로 정의하고 `z.infer` 로 `ResumeState`/`ResumeCheckpoint`/`RetryState` 타입 파생. **behavior-preserving (assertion-only)** — §7.5 graceful-reset 의 malformed/partial 허용 semantics 를 바꾸지 않도록 런타임 경계에서 parse 하지 **않고**, 스키마는 (a) allow-list 문서화 (b) 타입 파생 (c) 단위 테스트 oracle 로만 사용(사용자 결정 2026-07-02). 구조 단언 **6곳** 전환: `handler-output.adapter.ts`(가드된 `_resumeState`→`isRecord` 재사용)·`ai-turn-orchestrator.service.ts`(resumeState fallback·nextResumeState→`ResumeState`)·`retry-turn.service.ts`(`_retryState` 읽기 2곳→`RetryState`)·`execution-engine.service.ts`(`_resumeCheckpoint`→`ResumeCheckpoint`). 테스트: `resume-state.schema.spec.ts`(allow-list/라이프사이클 불변식 `.strict()` 검증) + `execution-engine.service.spec.ts` 실 checkpoint 경로 builder↔schema drift 가드 2곳(`.strict().safeParse` + credential 부재 단언). 검증: lint·build(tsc)·unit **7521** PASS. e2e: Docker 레지스트리 아웃티지(`flyway/flyway:10-alpine`·`node:24-alpine` DeadlineExceeded)로 미실행 — 회복 시 `run-test.sh e2e`(behavior-preserving assertion-only + unit 전건 PASS 라 회귀위험 최소). ai-review `review/code/2026/07/02/11_59_12`(Risk NONE→LOW, Critical 0·Warning 1→W-1 drift 가드 strict 화 fix·RESOLUTION) + fresh 재검 `review/code/2026/07/02/12_11_07`.
  - **후속 클러스터**: LOAD-BEARING(`isRecord` 가드 + throw→명시 방어 개별 검토)·STORE-PRESERVE(배열 보존 접근)·`ai-turn-orchestrator.service.ts`(잔여)·`ai-turn-executor.ts`(29, resume-state 재사용은 `ResumeState` 타입 활용)·relay 시그니처 통일(ai-review INFO — `processAiResumeTurn`/`handleAiMessageTurn` 등)·기타 파일. impl-prep `review/consistency/2026/07/01/16_32_55` **BLOCK:NO** + invariant 체크리스트(I-1 `status` 유니온·I-5 `_resumeCheckpoint`/`_retryState` credential allow-list·I-6 §4.4 WebsocketService sink 추상화 금지·I-7 raw/resolved config 분리·W-2/W-3 에러코드·버전상수 통합 금지).
