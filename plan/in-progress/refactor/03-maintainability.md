...
- [ ] **e2e 재실행 (레지스트리 회복 후)** — 환경 Docker 레지스트리 아웃티지(`flyway/flyway:10-alpine` FROM resolve DeadlineExceeded)로 미실행. 코드 무관(behavior-preserving + unit 7399 전건 PASS). `registry-1.docker.io` 도달 회복 시 `.claude/tools/run-test.sh e2e` 로 검증.
