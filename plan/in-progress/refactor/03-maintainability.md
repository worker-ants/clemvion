...
- [~] **진행 중 — 재스코프 (2026-07-02)**: C-1 엔진분할(02 C-1 완료)로 대상 파일이 재편돼 원안 "50+ 1파일" 이 아니라 **~124건 / ~15파일** 로 분산(원 라인 refs `:370-371` 등 stale). 사이트별 성격이 달라 일괄 전환 불가 — **SAFE-TORECORD**(property-접근 전용)·**STORE-PRESERVE**(저장/보존, 배열 보존 필요)·**LOAD-BEARING**(throw 전제)·**RESUME-STATE**(§7.4 스키마 필요)로 분류해 검증된 클러스터 PR 로 진행. 사용자 결정(2026-07-02): 전체 ~124건 대상.
  ...
  - **후속 클러스터**: STORE-PRESERVE `rawConfig`(스키마 `rawConfig` 타입화 필요)·unknown→domain 캐스트(messages/allPresentations/model — #783 스키마 필드타입 enrich 필요)·LOAD-BEARING(`isRecord` 가드 + throw→명시 방어 개별 검토)·`ai-turn-orchestrator.service.ts`(error-extraction 잔여)·`ai-turn-executor.ts` 나머지 state 소비 메서드(`buildAiNodeRefFromState`/`threadHolderFromState`/`processMultiTurnMessage` 내부) relay 시그니처 통일(ai-review INFO)·신규 util 을 spec `code:` frontmatter 추가(planner, convention INFO)·기타 파일. impl-prep `review/consistency/2026/07/01/16_32_55` **BLOCK:NO** + invariant 체크리스트(I-1 `status` 유니온·I-5 `_resumeCheckpoint`/`_retryState` credential allow-list·I-6 §4.4 WebsocketService sink 추상화 금지·I-7 raw/resolved config 분리·W-2/W-3 에러코드·버전상수 통합 금지).
