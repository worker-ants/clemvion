...
| [03-maintainability.md](./03-maintainability.md) | 15 | 7 (M-6·m-2 PR #522 + M-2 API_BASE_URL + C-2 ai-turn-executor 분해 #697·2차 + C-4 WS gateway helper + m-3 integrations/new 분할 + m-1 console→Logger) | 1 (철회 M-3) | 7 | 2 (C-3, M-4) | dead-code 제거 2건 + M-2(API_BASE_URL 3001→3011) + C-2(ai-agent god-method §6.1/§6.2 분해) + C-4(WS gateway 인증/소유권 helper) + m-3(integrations/new 1444→448줄 분할) + m-1(서비스 console.*→Logger + no-console 가드) 외 대부분 미착수. cafe24/makeshop 미러(DRY-deferral) |
...
> 완료(67) + 철회·종결(16) + 잔여(21) = 104. 처리 종료(완료+철회·종결) = 83/104.
