...
- 처방이 **설계 선택**이다: refresh in-flight 를 ref 로 단일화할지, 401 복구가 주기 타이머를
  먼저 취소할지, 아니면 `finalizeEnded` 를 "재차 401 이면서 in-flight 없음" 으로 좁힐지.
  셋이 실패 모드가 다르다.
...
실패하면 `"continue"` 를 돌려주는데, **토큰은 교체되지 않았다.** 호출부는 그 옛(만료된)
토큰으로 `openStream` 하고, `EventSource` 는 자동 재연결하므로 서버가 계속 401 로 거부하는
동안 재연결이 반복될 수 있다.
