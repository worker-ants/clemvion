...
- [ ] SUMMARY §1 위반 19건의 코드 수정 vs spec 하향 결정 (developer/project-planner) — 특히 severe 3: audit-logs Admin+ 가드 부재(V-03, 보안), makeshop expired 오격하(V-01), AI 노드 override UI 필드 누락(V-02)
  ...
  - [x] **V-06** (makeshop `services/makeshop/catalog` operations 미충전) + **V-08** (Activity 탭 라벨 namespace cafe24 고정) — `makeshop-catalog-labels` 브랜치(본 PR)에서 코드 수정 해소 (getServiceCatalog makeshop 분기 + tryTranslateLabel provider-prefix 일반화)
