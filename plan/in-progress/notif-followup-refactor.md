...
## 항목
- [ ] **finalizeFailedExecution 헬퍼 추출** (behavior-preserving) — `runExecution` catch(초기 세그먼트)와
...
- [ ] **spec §4.4 ModuleRef 문서화** — 현재 "엔진 순환=forwardRef" 만 명문. `ExecutionEngineService↔NotificationsService`
...
- [ ] **plan lifecycle**: [[spec-update-notifications-background-run-id]] 의 마지막 §4.4 항목 완료 → complete 이동.
