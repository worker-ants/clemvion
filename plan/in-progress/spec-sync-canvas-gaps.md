...
- [x] §4.1 팔레트 **Recent 섹션** — 최근 사용 노드 타입 상단 표시(`recent-nodes-store`, 세션 한정·최대 5·최신순·manual_trigger 제외·검색 중 숨김). `editor-store.addNode` 단일 choke point 기록.
- [ ] §4.1 팔레트 **Installed(마켓플레이스) 섹션** — 마켓플레이스 모듈(`marketplace-and-plugin-sdk.md`) 의존, backlog 유지. (Recent 와 분리)
- [x] §4.2 팔레트 아이템 클릭으로 노드 추가(뷰포트 중앙+지터, `palette-canvas-bridge`) + 패널 접기 토글(아이콘 레일)
- [x] §4.3 빠른 노드 추가 팝업 키보드 — ↑/↓ 하이라이트·Enter 선택·Escape 닫기(팝업 우선, `stopPropagation`). 순수 리듀서 `quick-add-nav`
