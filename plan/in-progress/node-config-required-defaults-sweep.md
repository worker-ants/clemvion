...
- **spec Rationale 공식화** — 노드 schema `.optional()` / `.default()` 의 설계 원칙 (저장 관용성·마이그레이션·LLM 도구 호출 부분 인자 허용) 과 `ui.required` 의 분리 관리 방침을 `spec/4-nodes/0-overview.md` 또는 카테고리별 `0-common.md` Rationale 에 공식화. consistency-check I-5 지적사항. **UiHint DSL canonical 정의** (`visibleWhen`/`requiredWhen` 허용 shape · 평가 규칙 · 정준화 정책) 도 함께 명시 권장 — 2026-05-19 `requiredwhen-dsl-whitelist` 의 consistency W-2 에서 식별.
- **visibleWhen DSL 통합 정리** — `requiredWhen` 은 `equals` 단일 shape 화이트리스트로 정준화 완료 (`requiredwhen-dsl-whitelist`). `visibleWhen` 도 동일 정책 적용 (notEquals/oneOf → equals array) — 현재 `ai-agent.schema.ts:151` 등 능동 사용처 마이그레이션 필요. 별 worktree 진행.
...
- ~~**switch mode 확장 가이드**~~ → 별 plan [`requiredwhen-dsl-whitelist`](./requiredwhen-dsl-whitelist.md) 로 분리 + 해소 (2026-05-19 결정: ② requiredWhen DSL 자체를 화이트리스트 `{ field, equals: T | T[] }` 로 단순화 + notEquals/oneOf 제거 + switch.switchValue 마이그레이션 + spec §8 mode 확장 가이드 추가).
