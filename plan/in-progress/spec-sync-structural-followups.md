...
### C-1. 상태 필터 파라미터 불일치 - isActive vs status  — 🔴 OPEN
...
### C-2. 비활성 chatChannel 트리거 응답이 410 Gone 대신 202 Accepted 여야 함  — 🔴 OPEN
...
### C-3. Variable Modification UI 에 무효한 set_field/delete_field 옵션 잔존  — 🔴 OPEN
...
### C-4. If/Else regex 연산자가 항상 false를 반환하는 no-op  — 🔴 OPEN
...
### C-5. Telegram /help 정적 안내가 도달 불가(dead code)  — 🔴 OPEN
...
### C-6. AlertsEvaluatorService가 발사하는 `alert_<rule.type>` 동적 값이 notification.type CHECK 제약 밖  — 🔴 OPEN
...
### C-7. 실행 목록 "Nodes" 열이 nodeExecutions 부재로 항상 —로 표시  — 🔴 OPEN
...
### C-8. Dashboard Success Rate 분모 및 최근 워크플로우 정렬 컬럼 불일치  — 🔴 OPEN
...
### C-9. 백엔드 QueryStatisticsDto enum 이 프론트 '1d' 프리셋을 거부 (validation 에러)  — 🔴 OPEN
...
### C-10. GET /api/schedules 의 sort/order 쿼리 파라미터 무시 (created_at DESC 고정)  — 🔴 OPEN
...
### C-11. Discord AI Multi Turn reply modal 진입점 및 setupChannel verify_key 검증 미구현  — 🔴 OPEN
...
### C-12. Slack file_shared → files.info → submit_form 및 response_url 비동기 갱신 미구현  — 🔴 OPEN
...
### C-13. Manual Trigger: meta.source 필드가 schema에 명시 선언 부재  — 🔴 OPEN
...
### C-14. DTO 의 dryRun 필드 description 이 stale — v1 완전 구현 됨  — 🔴 OPEN
...
### C-15. LlmUsageSummaryDto 누락 필드 3개 — spec "Input/Output 토큰 구분" 미충족  — 🔴 OPEN
...
### C-16. 코드가 spec 의도와 어긋나는 dead export/declaration 4건 중 1건 fixed, 3건 open  — 🟡 PARTIAL
...
### C-17. demo-host.tsx 의 show/hide/updateProfile 주석이 stale — 구현 완료되었으나 "미구현"이라 기술  — 🔴 OPEN
...
### C-18. ResultTimeline 분모 maxTurns 항상 0 fallback — 읽기 경로 불일치  — 🔴 OPEN
...
### C-19. KB 삭제 시 S3 정리 루프가 workspace 필터 없이 문서 조회  — 🔴 OPEN
