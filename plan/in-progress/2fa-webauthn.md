...
- [ ] `requiresTotp` deprecated 필드 제거 (두 마이너 버전 후 AND `methods` 만 보는 신규 프론트엔드 동일 배포 확인 후)
- [ ] 백엔드 e2e (`webauthn-2fa.e2e-spec.ts`) — SoftWebAuthnDevice (Ed25519 attestation/assertion 합성 helper) 구현 + Chrome Virtual Authenticator 연동. 본 PR 의 단위 테스트가 라이브러리 mock 으로 23개 회귀 잠금 중이며, e2e 는 시나리오 부피가 커서 별 PR 로 분리
- [ ] mobile Safari 실기기 수동 검증 + 가능 시 BrowserStack 통합
- [ ] WebAuthn-only 계정의 비밀번호 재설정 흐름 검토 (현재 TOTP 와 동일 — `passwordResetToken` 사용)
- [ ] `auth.controller.ts forgotPassword`·`checkEmail` 응답 wrap 누락 (impl-prep checker C3·C4) — 본 PR scope 외 기존 버그. 별 PR 로 정리
- [ ] `SessionListDto.data` / `LoginHistoryPageDto.data` 필드명 → `items` 로 개명 (이중 중첩 해소, impl-prep W3·W4)
