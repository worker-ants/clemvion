...
- [ ] interface.ts union 제거 (INFO #2)
- [ ] interface.ts 분류 B 3 필드 non-optional promote (`structuredOutputCache`, `engineResolvedConfigCache`, `recursionDepth`)
- [ ] interface.ts 분류 C/E JSDoc 보강 (INFO #1)
- [ ] `execution-context.service.ts` defensive 가드 제거 (L54-56, L82-84) + `MutableExecutionContext` 갱신
- [ ] `makeExecutionContext` factory 신설 (`backend/src/modules/execution-engine/__test__/make-execution-context.ts`)
- [ ] 24 production 핸들러 시그니처 갱신 (`Promise<unknown>` → `Promise<NodeHandlerOutput>`)
- [ ] 핸들러 spec mock signature 갱신
- [ ] `mockOutput` 헬퍼 신설 (`backend/src/modules/execution-engine/__test__/mock-output.ts`)
- [ ] mockHandler 기본값 canonical shape (`mockOutput(...)` 사용)
- [ ] 60+ fixture 마이그레이션 (`mockOutput` 헬퍼 적용)
- [ ] `adaptHandlerReturn` lenient 분기 제거 + JSDoc 단일화 (D-1)
- [ ] `handler-output.adapter.spec.ts` strict throw 회귀 케이스
