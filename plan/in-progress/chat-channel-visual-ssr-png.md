...
### 결정 항목 #2 — fallback 정책 (사용자 escalate)

| 옵션 | 시각 path | fallback path |
|---|---|---|
| (a) **PNG default + 실패 시 v1 텍스트** | SSR PNG → sendPhoto | SSR 실패 (font 누락 / timeout 등) → renderChartFallback / renderTableFallback / renderCarouselFallback. `chat_channel_health = degraded` 갱신 |
| (b) **`uiMapping.visualNode` 명시적 분기** | `visualNode: 'photo'` → PNG, `'text'` (신설) → 텍스트, `'text_only'` → skip | 명시 지정 안 하면 default = PNG |

권장: (a) — 자동 graceful degrade 가 사용자 친화적. `uiMapping.visualNode='text'` 신설은 사용자 명시 텍스트 선호 시 별도 옵션.
