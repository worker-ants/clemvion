...
- [ ] **셧다운 중 만료 콜백 미실행** (리뷰 2R W1) — 만료 타이머에 `.unref()` 를 걸었으므로
      (`#1270`) 그 타이머만 남은 상태에서는 프로세스가 **콜백 발화 전에 종료**될 수 있다.
      ...
      > **이 항목은 리뷰가 내 거짓 주장을 잡아 만들어졌다.** 1R RESOLUTION 에서 이 트레이드오프를
      > *"배포 런북에서 별도 추적 중"* 이라고 썼는데, 2R reviewer 가 이 plan 을 실측해 보니
