...
- [ ] 회귀 테스트: 앵커 A + cwd B 인 상태를 재현해 A 가 살아남는지 (`--dry-run` 으로 삭제 목록 검사)
- [ ] 기존 동작 유지: 머지된 무관 워크트리는 여전히 reap 되는지
- [ ] cwd == 앵커 인 평범한 세션에서 이중 skip 이 오작동 안 하는지
...
- [ ] 위 5개 케이스(A–E) 를 `.claude/tests/` 회귀 테스트로 고정 — 특히 **C(커밋 메시지)** 와 **B(grep)**
- [ ] 참 양성 목록(`git -C`, env prefix, `&&` 뒤, `--force`) 전수 통과 확인
- [ ] `shlex` 예외 시 차단으로 폴백하는지 (자가 sabotage 테스트)

---

## 체크리스트

- [ ] ① 안 B 구현 (bootstrap `--keep` 전달 + reaper skip 집합)
- [ ] ① 회귀 테스트 (앵커≠cwd 재현)
- [ ] ② 서브커맨드 판정으로 교체 + shlex 폴백
- [ ] ② 회귀 테스트 (오탐 B·C + 참 양성 전수)
- [ ] TEST WORKFLOW — `.claude/**` 전용이면 harness 스위트로 충분한지 확인.
