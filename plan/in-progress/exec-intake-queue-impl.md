...
- **PR2 — §8 동시 실행 제한 (2026-06-04 사용자 승인 분할: PR2a→PR2b)**:
  ...
  - [ ] **PR2b — 동시성 cap** (branch `claude/impl-concurrency-cap-pr2b`). **결정(2026-06-05)**: Q-source=**settings 필드 추가**(Workspace/Workflow.settings JSONB 동시성 cap 키, 기본 fallback 10/3), Q-scope=**전체 한 PR**(cap + queue-wait 5분 cancel + TOCTOU + priority 3-tier + INFO 4건). 잠정 설계: Admission gate(`runExecutionFromQueue` RUNNING 전이 직전) — 트랜잭션 + pg advisory lock → `COUNT(status='running')` workflow/workspace → cap 미만이면 conditional `UPDATE … WHERE status='pending'`(TOCTOU#1 동시) → 아니면 job delayed 재큐; `execution.queued_at` 컬럼(마이그레이션) + 5분 초과 `CANCELLED`; priority 3-tier `ExecuteOptions.triggerType` threading.
    - **⚠ exec-park(PR-B1/B2/B3) 대수술 후 재검토 필요 (2026-06-06)**: `#470/#483/#486/#494/#498/#499/#501/#502` 머지로 §4.x/§7.4/§Rationale 가 **full-durable 완료형으로 재전환 + in-memory 머신(`pendingContinuations`/firstSegmentBarriers/fast-path 이원화) 완전 제거(#501 B3)**, `resume_call_stack`(V087) 등 신규 컬럼·마이그레이션 추가(현 main max=V087 → PR2b 마이그레이션=V088). 본 브랜치를 **post-exec-park main 으로 rebase 완료(2026-06-06)**. **위 잠정 설계의 전제(admission gate 삽입 지점·세그먼트 직렬화 불변식·runExecutionFromQueue 흐름)를 새 코드 기준으로 다방면 재검증 후 확정**(사용자 지시). 마이그레이션 V088, 직렬화 불변식은 in-memory 제거로 근거 변동 가능 — 재확인 의무.
