...
worktree 가 `git worktree remove` 로 사라져도 docker 쪽 namespace 는 stale 로 남는 문제 대응:

```make
e2e-prune:
	@docker compose ls -a --filter "name=clemvion-e2e" --format json \
	  | jq -r '.[].Name' \
	  | xargs -I{} docker compose -p {} down -v --remove-orphans
```

help 메시지에도 한 줄 추가. jq 의존성은 Makefile 주석에 명시.
...
- [x] plan 파일 작성 (본 문서)
- [ ] consistency-check (판단: spec 변경 0, plan 갱신만 → skip 또는 plan_coherence 만)
- [ ] `docker-compose.e2e.yml` 수정 (name 제거 + image 명시 + 주석)
- [ ] `Makefile` 수정 (COMPOSE_PROJECT + -p flag + e2e-prune + help)
- [ ] `PROJECT.md` 갱신
- [ ] `README.md` 갱신
- [ ] `CLAUDE.md` 갱신 (선택)
- [ ] TEST WORKFLOW: `make e2e-test` 현 worktree 통과
- [ ] `make e2e-prune` syntax / jq 의존성 확인
- [ ] (best-effort) 동시 실행 검증: 타 worktree 에서 e2e-up 띄워두고 본 worktree 에서 e2e-test 충돌 없는지
- [ ] `/ai-review` 실행 → RESOLUTION.md 작성
- [ ] plan → `complete/` 이동 (같은 PR 안 별 commit)
- [ ] PR 생성
