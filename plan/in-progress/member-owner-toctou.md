...
> 세운 `affected === 0` 판별자가 **도달 불가**가 되고, 그것을 고정하는 기존 단위 테스트
> **3건**(«진 쪽은 404» + 드라이버 미보고 대조군 `undefined`·`null`)이 죽은 코드를 가리키게 된다.
> 그 판별자는 «감사가 두 번 남는다» 를 실측으로 닫은 장치다 — 8일 전에 세운 방어를
> 무르게 만들면서 얻는 것이 «형제와 같은 모양» 뿐이다.
>
> **이 3건은 구현 시 실측한다** — 예측이 틀리면 기각 사유가 무너지므로 그때 다시 판단한다.
