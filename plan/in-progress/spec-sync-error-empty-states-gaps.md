...
## 미구현 항목
- [ ] Triggers 목록 빈 상태 — 공유 `EmptyState` 적용 + **트리거 추가** CTA (§2.2). 현재 CTA 없는 인라인 `Inbox` 빈 상태(triggers/page.tsx:618-622).
- [ ] Schedule 목록 빈 상태 — 공유 `EmptyState` 적용 + **스케줄 추가** CTA (§2.2). 현재 CTA 없는 인라인 `Inbox` 빈 상태(schedules/page.tsx:978-983).
- [ ] 검색 결과 없음 전용 상태 + **필터 초기화** CTA (§2.3). 현재 Workflows 는 안내 문구만 교체(`adjustFiltersHint`)하고 전용 메시지·리셋 버튼 없음(workflows/page.tsx:378-402). 검색바/필터가 있는 모든 목록 화면 공통 적용 대상.
