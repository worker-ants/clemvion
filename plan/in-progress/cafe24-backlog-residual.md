...
- [ ] **C-6 — ⏩ TRIGGERED (2026-06-03, makeshop = 2nd provider)**: `buildIntegrationMeta` 레지스트리 패턴 — 현재 cafe24 하드코딩. 두 번째 provider 추가 직전 `Map<serviceType, (entity) => IntegrationMeta>` 전환. **메이크샵 통합이 그 second provider 이므로 deferred 조건 충족** → makeshop 작업으로 편입. spec 측(derived 필드 `appUrl`/`autoRefresh` 의 per-service registry 일반화)·구현 측(레지스트리 리팩토링) 분해는 [`makeshop-integration.md §C-6 편입`](./makeshop-integration.md#c-6-편입--buildintegrationmeta-레지스트리-전환-cafe24-백로그) 참조.
