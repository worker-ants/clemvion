...
- [x] **D-2** (defer — 결정 2026-06-02 → **완료 2026-07-17**): `process()` 에러 격리 정책 spec 명시 (`.catch(logger.error)` BullMQ 재시도 회피). ~~현재는 프레임워크(NestJS Logger) 에러 로그 출력으로 충분하다고 결정 — 별도 관측 도구(Sentry/Datadog 등) 선정 및 spec 명시는 관측 인프라를 **추후 일괄 도입**할 때 함께 진행한다.~~ (ai-review W7)
  **완료 근거**: defer 의 전제였던 **"관측 인프라 일괄 도입"이 충족됐다** — OTel 파이프라인이 `dc24f047d`(#594, 2026-06-14, 본 defer 결정 2026-06-02 **이후**)로 도입돼 `spec/5-system/_product-overview.md` 의 NF-OB-02(Prometheus 메트릭)·NF-OB-03(분산 트레이싱)·NF-OB-07(비즈니스 커스텀 메트릭)이 전부 ✅ 다(`codebase/backend/src/instrumentation.ts`, `business-metrics.service.ts`). 즉 "별도 에러 트래킹 도구(Sentry 등) 선정"을 더 기다릴 이유가 없다 — BullMQ job 실패는 큐 메트릭으로, 그 위 운영 관측은 OTel 이 받는다.
  또한 **원 W7 이 우려한 코드 상태는 이미 무효**다: `cafe24-token-refresh.processor.ts` 의 `process()` 는 `.catch(logger.error)` 로 삼키지 않고 **re-throw** 하며, `cafe24-token-refresh.processor.spec.ts` TEST-C2 가 그 invariant 를 회귀 고정하고 있다. 남은 것은 이 정책의 **spec 명시**뿐이었고 본 PR 에서 이행했다 → [`spec/2-navigation/4-integration.md §10.5`](../../spec/2-navigation/4-integration.md) 에 "worker 의 에러 격리 정책" 항목 신설 (re-throw + `attempts: 1` + 재시도는 호출자 위임 + 삭제된 통합 silent no-op + 회귀 고정 테스트 참조).
...
- [~] **전 resource 확장 (goal: G-1-remaining 전체 완료)** — resource 별 author-agent(scratchpad→apply→metadata invariant gate) 파이프라인.
  - [x] product(62, 파일럿) · personal(5) · salesreport(5) · design(9) · privacy(6) · mileage(8) · notification(12) · collection(15) · translation(9) · application(17) · shipping(15) · customer(22) · category(17) · community(21) · supply(20) · promotion(33) — 16 resource 커밋 완료, metadata invariant 105 pass 유지.
...
### G-4 — 응답 래퍼 ↔ 요청 파라미터 이름 충돌 설명 오염 (generator 재생성 대상)
