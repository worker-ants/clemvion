...
- [x] 회귀 **25 it-블록** 신규 (모달 12 · 툴바 4 · 마커 유틸 9 — `it.each` 가 있어 실제
      실행 케이스는 이보다 많다) + **뮤테이션 12종**. 전부 라운드 처분 과정에서 실행했다:
...
- [x] TEST WORKFLOW 4단계 PASS (라운드마다 재실행, 아래는 **최종 실측**) — lint /
      unit(backend **427 suites · 8,832** · frontend **6,070** · web-chat **23 · 451**) /
...
- [x] `/ai-review` **8라운드** — `14_08_45`(C2·W7) → `14_44_08`(C0·W8) → `15_10_25`(C0·W2)
      ...
      `16_51_19`(C0·W2, 1건 트래커 이월) → `17_13_19`(C0·W1, LOW) → `17_38_33`(C0·W4 —
      2건 수정, 2건 트래커). 발견의 성격이 **동작 → 구조 → 문서 자리 → 견고성 →
      캐너리 자매 → 단언 강도 → 엣지 케이스 교착**으로 내려와 수렴
      ...
      > 사실이 표 안에서 보이게 했다.
- [x] `--impl-done` **8라운드** 전부 **BLOCK: NO** (`14_44_42` · `15_10_56` · `15_33_05` ·
      `15_59_50` · `16_26_26` · `16_52_12` · `17_14_02` · `17_39_11`). `16_26_26`~`17_14_02`
      세 라운드는 5 checker 전원 위험도 **NONE**·WARNING **0**, 마지막 `17_39_11` 은
      swagger JSDoc 길이 규약 WARNING 1건(같은 턴 수정). 그 앞에 `--spec` 2회(`12_29_59` · `12_41_29`) ·
