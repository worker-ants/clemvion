...
- [ ] **동일 커밋의 형제 파일이 부분만 뽑히는 원인 확인** — code-review changeset 산정에서
      ...
      **2026-08-10 — 배치 분할과 무관한 두 번째 형태를 관측했다. 이쪽이 더 위험하다.**
      `plan-lifecycle-gates` 세션 `03_12_29` 에서 `--prepare --branch origin/main` 이
      changeset 을 27파일로 잡았는데 그중 **직전 두 커밋이 바꾼 코드 3파일이 하나도
      없었다**. 배치 분할이 아니다 — 27파일은 상한 50 아래고 배치 로그도 없었다.

      - 커밋 `bc10e215e`(03:07:38)·`e64b1218b`(03:10:27) 가
        `plan-scan.ts`/`plan-scan.test.ts`/`plan-frontmatter.test.ts` 를 수정
      - 세션 준비는 **03:12:29** — 두 커밋보다 **뒤**다(타이밍 레이스 아님)
      - `meta.json` files 27건에 그 3파일 **0건**. 목록은 전부 `review/**` 와 spec 문서
      - 그 결과 router 가 "소스 코드 변경 없음(문서 전용)" 으로 판정 →
        security·testing·scope·maintainability 등 **12명 skip**, 강제 2명만 실행

      ⇒ **거짓 PASS 다.** 게이트는 "리뷰됨" 으로 닫히는데 정작 그 라운드는 새 코드를 보지
      않았다. 앞선 배치 분할 형태는 *많은* 파일에서 일부가 빠지는 것이고, 이쪽은 *적은*
      파일에서 **직전에 바뀐 것만** 빠진다.

      가설: 파일 선별이 "이전 세션에서 이미 리뷰된 파일" 을 제외하는데, 그 판정이 **"그
      뒤에 또 바뀌었는가" 를 보지 않는다**. 3파일 모두 직전 세션(`02_47_31`)의 대상이었다.

      - [ ] 위 가설을 코드로 확정 — `code_review_orchestrator.py` 의 changeset 산정에서
            이전 세션 대상 제외 로직을 찾고, mtime/커밋 비교가 있는지 본다
      - [ ] 확정되면 **회귀 테스트 먼저**: "세션 A 가 파일 F 를 리뷰 → F 수정 → 세션 B 의
            changeset 에 F 가 있어야 한다". 지금은 이 단언이 어디에도 없다
      - [ ] router 쪽 방어도 함께 볼 것 — changeset 이 비어도 **직전 세션 이후 커밋이
            `codebase/**` 를 건드렸으면** "문서 전용" 판정을 못 내리게 하는 편이 안전하다
            (fail-closed). 지금은 산정 결함이 곧바로 reviewer 전원 skip 으로 증폭된다
