...
- **"병렬 fan-out 중 리뷰어가 실제 저장소를 뮤테이션한다"** (문서 끝, 2026-08-11) —
  한 리뷰어의 뮤테이션이 다른 리뷰어들의 판정을 오염시켰고 그중 하나는 `git restore` 로
  남의 워킹트리를 되돌렸다. **2026-08-28: 후속 3건 이관** — 관측 오염+원인 오귀속 ·
  untracked 파일 파괴 near-miss · `.bak` 재발. 셋 다 무해했지만 **무해했던 이유가 매번
  우연**이라 처분 후보 (d)(백업 목적지를 세션 scratch 로 강제)를 추가했다.
...
- [ ] **(신규, `15_01_34` W2) 링크 추출을 `fromMarkdown` AST 순회로 옮길지 판정** —
      ...
      > 인용문 안에서는 줄이 `>` 로 시작한다. 인용문 안에서는 이 "각각 코드로 떼기" 가
      > 유일하게 통하는 방법이다.
      >
