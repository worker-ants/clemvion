...
worktree: fix-harness-budget-arithmetic-858262
started: 2026-08-07
owner: developer
priority: P3
spec_impact: none
...
> 워크트리에서 처리했다.
>
> _(2026-09-10)_ `worktree:` 를 `eslint10-upgrade-5e3cf9` → `fix-harness-budget-arithmetic-858262`
> 로 갱신했다. 앞 값은 **reap 돼 사라진 워크트리**라 링크가 이미 끊겨 있었다 (`worktree list` 실측).
> 이번 편집(§"승격은 됐는데 굶는다" 의 산술 정정)을 한 워크트리로 맞춘다 — 위 2026-08-29 항목과
> 같은 이유다.
...
> **여기에 남은 항목의 개수를 적지 않는다.** 이 자리에 *"남는 이유는 이제 **둘**이다"* 라고 적었고
> **그 문장이 두 번 낡았다** — 2026-08-29 에 취소선 없이 남은 불릿과 어긋나 `15_30_59` W1 이
> 잡았고, **2026-09-10 에 새 항목 두 건이 추가됐는데 그 문장을 또 안 고쳤다**(tier 굶주림은 같은
> 날 오전에 등재됐는데 이 요약에 없었다). 개수는 **아래 `## 미해결 항목` + 각 `##` 절의 미체크
> 체크박스가 단일 진실**이고, 이 목록은 열린 것의 *성격*만 적는다. 형제 트래커
> `spec-draft-nullable-notation-followups.md` 가 같은 이유로 같은 처분을 이미 했다.
...
- **"승격은 됐는데 굶는다"** (문서 끝, 2026-09-10 실측) — `prioritize_bundle_files` 는 옳게
  승격하는데 그 tier 안의 거대 파일 하나가 corpus 몫을 다 먹어 뒤가 조용히 절단된다.
  랭킹 층 결함이라 세 모드 전부에 걸린다.
- **§M** (문서 끝, 2026-09-10 실측) — `--impl-done` 번들의 diff 는 **커밋 기준**인데 같은
  프롬프트의 preamble 은 **워킹트리**를 SoT 라 선언한다. 미커밋 fix 가 조용히 안 보인다.
- **§N** (문서 끝, 2026-09-10 실측) — 「리뷰어 뮤테이션」 종결 문구가 실제 계약보다 넓다
  (*"scratch 로 강제"* vs 실제는 기본 scratch + `cp` 폴백). 그리고 **백업 명령이 훅에
  차단되면 뮤테이션만 진행된다** — 안전망만 걷히는 방향.

> **이 목록이 §N 을 빠뜨린 채로 한 번 더 낡았다** (`review/code/2026/09/10/16_55_52`
> documentation W1). §N 은 §M 과 **같은 세션·같은 날** 등재됐고 바로 다음 절인데도 목록에
> 안 들어갔다 — 개수 서술을 성격 목록으로 바꾼 것이 *바로 이 재발을 막으려는* 편집이었으므로,
> **형태를 바꾼 것만으로는 부족하고 항목을 추가할 때 이 목록을 함께 보는 습관이 필요하다**는
> 것이 이번에 확인된 사실이다. 세 번째 재발이면 산문 규율 말고 코드로 바꿔야 한다 —
> 후보: 문서 끝의 `## §X` 절 개수와 이 목록의 불릿 수를 대조하는 docs 가드.
- ~~**"`spec-link-integrity` 가 멀티라인 마크다운 링크를 통째로 못 본다"** (2026-08-11 추가)~~
  → **해소 (2026-08-29, `#1235`).** 아래 §미해결 항목의 그 항목 참조 — 뮤테이션으로
  "옛 구현 GREEN(침묵 통과) / 새 구현 RED" 를 직접 쟀다. **따라서 남는 이유는 이제 둘이다.**
- ~~**"병렬 fan-out 중 리뷰어가 실제 저장소를 뮤테이션한다"** (문서 끝, 2026-08-11)~~
  → **해소 (2026-08-29, `#1229`).** 처분 후보 (d)(뮤테이션을 저장소 밖 scratch 로 강제)를
  리뷰어 프롬프트 조립부에 상시 계약으로 넣었고 회귀 가드를 붙였다 — 문서 끝 절의 그
  체크박스가 `[x]` 다.
  > 원 경위: 한 리뷰어의 뮤테이션이 다른 리뷰어들의 판정을 오염시켰고 그중 하나는
  > `git restore` 로 남의 워킹트리를 되돌렸다. 후속 3건(관측 오염+원인 오귀속 ·
  > untracked 파일 파괴 near-miss · `.bak` 재발)은 셋 다 무해했지만 **무해했던 이유가
  > 매번 우연**이었다.
  >
  > **이 불릿이 취소선 없이 남아 요약의 "둘" 과 어긋나 있었다** (`15_30_59` W1).
  > 저장소가 세 번째로 겪는 "plan 서술은 철회로 거짓이 된다" 형태다 — 항목을 닫을 때
  > **그 항목을 세는 요약도 같이** 봐야 한다.
- ~~**미해결 조사 1건** (문서 끝) — 형제 파일 부분 추출 원인.~~ → **종결 (2026-08-10).**
  원인 두 겹(같은 초 세션 충돌 · 배치 분할)이 모두 닫혔고, 그 결함이 거짓 PASS 로 증폭되던
  구조도 router fail-closed 교차검사로 막았다. 문서 끝 항목 참조.
...
- [x] **`spec-link-integrity` 가 멀티라인 마크다운 링크를 통째로 못 본다** (2026-08-11 실측,
      ...
      **`text.split(/\r?\n/)` 로 자른 뒤 줄마다** `LINK_RE` 를 돌린다. 그래서 `[` 와 `](` 가
      ...
      - ⚠️ **표면이 넓어졌다 (2026-08-27, `doclink-guard-scope` / `18_26_27` W2)**:
        `spec-link-integrity` 에 **scope 3(거버넌스 문서 — 루트 `*.md` + `.claude/**.md`)** 가
        추가됐고, 그 스코프도 **같은 사각지대를 그대로 상속**한다. 실측: 58파일 중 **2건** —
        `.claude/skills/code-review-agents/SKILL.md` · `.claude/skills/consistency-checker/SKILL.md`
        (둘 다 `[`report_paths.py`\n> `has_report()`](../../_shared/report_paths.py)` 형태).
        **지금은 무해**하다 — 타깃 `.claude/_shared/report_paths.py` 가 실재한다(확인).
        그러나 그 두 링크는 **아무도 안 보고 있다**.
        > 이 2건을 한 줄로 펴지 **않았다** — 위 "증상 처리" 판단이 그대로 적용되고, 오히려
        > 근본 수정 PR 이 감당해야 할 전수에 **거버넌스 스코프도 포함된다**는 증거로 남긴다.
        > 탐지 방법: per-line `LINK_RE` 매칭 수 < whole-text 매칭 수 인 파일을 세면 된다
        > (`]` 로 끝나고 다음 줄이 `(` 로 시작하는지만 보면 **놓친다** — 실제 형태는 링크
        > **텍스트**가 줄을 넘는 것이다. 이 오탐지로 한 번 0건이라 오판했다).

      **→ 해소 (2026-08-29, `#1235`).** `extractLinks()` 가 줄 단위 매칭을 버리고
      **마스킹된 전문(全文)** 을 한 번에 매칭한다.

      **뮤테이션이 이 PR 의 근거다** — 라이브 트리의 실제 멀티라인 링크
      (`.claude/skills/code-review-agents/SKILL.md` 의 `report_paths.py` 링크) 목적지를
      없는 경로로 바꾸고 가드를 돌렸다:

      | 뮤턴트 | 예측 | 실측 |
      |---|---|---|
      | 멀티라인 링크 목적지를 없는 경로로 — **옛 구현**(origin/main 로 되돌림) | GREEN(침묵 통과) | **GREEN** |
      | 같은 뮤턴트 — **새 구현** | RED | **RED** |
      | 같은 링크를 한 줄로 편 뒤 같은 뮤턴트 (대조군) | RED | **RED** |

      첫 줄이 사각지대의 존재를, 둘째 줄이 그것이 닫혔음을 보인다. 인용이 아니라 직접 쟀다.

      **"전수를 감당해야 한다" 는 경고는 비용이 0 이었다** — 위 §주의가 *"지금 침묵으로
      통과하던 기존 링크들이 새로 RED 가 될 수 있다"* 고 적었는데, 고친 뒤 라이브 트리
      전수(`spec/**` + 거버넌스 + plan + codebase 소스)가 **그대로 GREEN** 이다. 숨어 있던
      8건(spec 6 + 거버넌스 2)의 타깃이 전부 유효했다. 경고 자체는 옳았고 — 확인 없이는
      알 수 없었다 — 결과가 운이 좋았을 뿐이다.

      **구현이 지켜야 했던 세 가지** (하나라도 놓치면 다른 가드가 깨진다):
      1. 인라인 코드는 **지운다**(공백으로 채우지 않는다) — 아래 형태는 코드를 지워야
         비로소 링크가 된다(전용 회귀 테스트 존재):

         ```
         [a]`code`(b)
         ```

         > **이 예시를 펜스로 감싼 이유 — 이 PR 이 그 함정에 직접 빠졌다.** 처음엔 이
         > 문구를 인라인 코드(`` `` ``)로 적었는데, 마스킹이 백틱 쌍을 지우고 나면 남는
         > 것이 `[a](b)` 라 **진짜 링크가 된다**. 그리고 그 링크를 보이게 만든 것이 바로
         > 이 PR 의 수정이라, `findBrokenPlanLinks` 가 `b` 를 DEAD 로 잡아 빌드가 깨졌다
         > (`/ai-review` CRITICAL, `vitest run plan-frontmatter` 로 재현). 펜스 안은
         > 스캔 대상이 아니므로 안전하다.
      2. 그래서 오프셋이 밀리므로 **줄 번호를 따로 들고 간다**(마스킹 줄 → 원본 줄 지도).
      3. 펜스 줄 자리에 `]` 를 남긴다 — 그냥 건너뛰면 앞뒤가 붙어 **없던 링크가 생긴다**.

      넓히는 방향만 잠그면 "전부 링크로 본다" 는 반대 오류가 통과하므로, **목적지는 줄을
      넘지 못한다**(CommonMark 도 `<…>` 형태가 아니면 불허) · **펜스를 사이에 둔 `[` 와
      `](` 는 링크가 아니다** 두 역방향 단언도 함께 넣었다.

      **역방향을 한 축 빠뜨렸고 리뷰가 잡았다** (2라운드 `15_01_34` W1). 위 문장이
      "양방향으로 잠갔다" 고 선언했는데 **빈 줄(문단 경계)** 축이 안 잠겨 있었다 —
      `mdast-util-from-markdown`(이 파일이 헤딩 슬러그에 쓰는 그 파서)로 대조한 실측:
      ...
      빈 줄도 펜스와 같이 `]` 로 끊어 고쳤고(`isBlank`), 뮤테이션으로 그 단언이 실제로
      관측함을 확인했다(`isBlank` 제거 → **RED**). 교훈은 이 저장소가 이미 아는 것이다 —
      **"양방향" 이라고 쓰기 전에 안 되는 방향을 하나씩 세어라.**

- [ ] **(신규, `15_01_34` W2) 링크 추출을 `fromMarkdown` AST 순회로 옮길지 판정** —
      같은 파일 안에 **정본 파서**(`headingSlugs` 는 `fromMarkdown` 을 쓴다)와 **손으로 짠
      정규식 스캐너**(`extractLinks`)가 공존한다. 이 PR 한 번에 그 스캐너가 CommonMark 와
      갈리는 지점이 **세 번** 드러났다(멀티라인 미탐지 · 예시 문구가 링크가 됨 · 문단 경계
      오판). 저장소 교훈은 "대상에 진짜 문법 + 정본 파서가 있으면 AST 가 이긴다" 이고,
      마크다운은 그 조건을 만족하며 **파서가 이미 이 파일에 import 돼 있다**.
      > **그런데 자명한 (a) 가 아니다** — 이 가드는 CommonMark 와 **의도적으로 다른** 판정을
      > 하나 갖고 있다: `[a]` 바로 뒤에 인라인 코드가 오고 그 뒤에 `(b)` 가 오는 형태를
      > CommonMark 는 링크로 보지 않지만 이 가드는 **본다**(인라인 코드를 지운 뒤 판정,
      > 전용 회귀 테스트가 그 동작을 고정). 그 의도는 "문서에 그렇게 써 두면 사람 눈에
      > 링크로 보인다" 는 쪽에 가깝다.
      >
      > **이 문장을 쓰는 방식 자체가 함정이다 — 세 번째로 밟았다.** 그 형태를 한 줄에
      > 그대로 인용하면 마스킹이 백틱을 지운 뒤 **진짜 링크가 되어** 가드가 이 파일을
      > DEAD 로 잡는다. 위처럼 `[a]` 와 `(b)` 를 **각각 별도 인라인 코드**로 떼어 놓으면
      > 마스킹이 둘 다 통째로 지워서 대괄호가 남지 않는다.
      >
      > **펜스로 감싸는 것은 여기서 안 통한다** — `FENCE_RE` 는 줄 시작(공백만 허용)에
...
- [ ] **(작음, 한 묶음) 링크 가드 통합층 대칭성 2건** — 둘 다 `spec-links.test.ts` 의
      ...
      - `LinkViolation.line` 을 통합 테스트에서도 단언 (`15_55_00` W1). **계약 자체는 이미
        단위 층 5곳이 잠근다**(시작 줄 · 멀티 2개 · 혼재 3개 · 3줄 스팬) — 통합층이 더하는
        것은 "전달이 끊기지 않았는가" 뿐이다.
      - **ANCHOR** 위반의 멀티라인 케이스 (`15_01_34` INFO #17 · `15_55_00` INFO #8).
        지금은 DEAD 경로만 통합 검증돼 있다.
      > **왜 그 턴에 안 했나** — 이 PR 이 4라운드를 돌았고 발견의 성격이 **동작(빌드 차단)
      > → 동작(문단 경계 오판) → 테스트 위생 → 이미 잠긴 계약의 재단언** 으로 이동했다.
      > `spec-links.test.ts` 는 `codebase/**` 라 한 줄만 고쳐도 리뷰가 freshness 로 무효가
      > 되고 4단계 테스트 + 리뷰 한 바퀴가 더 돈다. 근거는 `15_55_00/RESOLUTION.md`.
      >
      > (이 문장을 처음 쓸 때 백틱 세 개를 인라인 코드 안에 그대로 넣었다가 **구분자와
      > 내용의 백틱 길이가 같아** 코드 스팬이 안 닫혔다 — 같은 함정의 **네 번째** 재발이고,
      > 하필 그 함정을 경고하는 문장에서였다. 백틱 런은 **말로 풀어 쓴다**.)
      >
      > 그러니 AST 전환은 **버그 수정이 아니라 그 판정을 바꾸는 설계 변경**이다. 착수 전
      > 물을 것: 그 케이스를 (i) 버린다 / (ii) AST 순회 뒤 별도 규칙으로 유지한다 —
      > 어느 쪽인지 정하고 시작해야 한다. 정규식 유지 근거는 `extractLinks` JSDoc 에 적어 뒀다.

- [x] ~~**`spec-impl-evidence.md §4.2` 가 링크 위생 책임을 없는 곳에 돌리고 있다**~~ →
      **정정 완료 (2026-08-27, `doclink-guard-scope` planner 턴 / `18_26_27` W3)**. (2026-08-10 실측.)
      > **전제가 바뀌어 있었다**: 이 항목은 *"그 책임을 아무도 지고 있지 않다"* 로 적혔는데,
      > 예고된 `#1123` 가드가 실제로 **존재한다** — 같은 표의 `plan-frontmatter.test.ts` (3)
      > *"top-level 살아있는 plan 의 상대링크 무결성"*. 즉 책임은 이제 **있고**, §4.2 만
      > 없는 곳(`plan-coherence-checker`)을 가리키고 있었다. 그 checker 정의에 링크 언급이
      > **0건**임을 재확인하고(grep), 문장을 취소선으로 남긴 채 실재 가드로 귀속시켰다.
      > **왜 이번에 잡혔나**: 이 PR 이 scope 3 을 넣느라 **같은 셀을 두 번 편집**했는데도
      > 인접한 이 오기를 못 봤고, `--impl-done` 이 그것을 짚었다. 편집하는 셀의 **나머지
      > 문장을 읽지 않는** 형태다.
...
## 후속 — `--impl-done` 세션은 spec-linked 편집 **이후에** 준비해야 한다 (2026-08-10 실측)

`review_guard` 의 freshness 규칙은 리뷰의 "완료 시각" 을 **세션 디렉터리 타임스탬프**
(`<Y>/<m>/<d>/<H>_<M>_<S>`)로 읽는다. 커밋되지 않은 SUMMARY/RESOLUTION 의 mtime 은
"folded in" 되지만 **커밋하는 순간 세션 디렉터리 시각만 남는다.**

실측 (`plan-lifecycle-gates` 브랜치):

| 항목 | 시각 |
|---|---|
| `--impl-done` 세션 준비 | `2026/08/10/01_09_04` |
| `spec/conventions/spec-impl-evidence.md` 커밋 author date | **01:11:56** |

세션이 spec 편집보다 **2분 52초 앞선다.** 그래서 그 세션에 완벽한 SUMMARY 를 써도 커밋하는
순간 "리포트가 코드보다 낡았다" 로 다시 막힌다 — checker 5개를 돌리고도 게이트가 안 열린다.

- [x] ~~**워크플로 규약으로 명문화**~~ → **완료 (2026-08-28,
      `impl-done-ordering-convention`)**. `developer/SKILL.md` §4 에 순서 규약 블록을 넣고
      같은 문서 체크리스트에도 한 줄 못박았다.
      > 함정의 성격: 게이트 메시지는 "재실행하라" 고만 말하고 **왜 방금 것이 무효인지**
      > 는 말하지 않는다. 세션 디렉터리 시각이 판정 기준이라는 사실이 메시지에 없다.
      > **집행 근거 (실측)**: 한 세션에서 이 순서를 **다섯 번** 밟았다 —
      > `masking-expression-egress-split`(2회) · `doclink-guard-scope`(2회) ·
      > `eia-misc-hygiene`(1회). 매번 리뷰/consistency 를 한 바퀴 더 태웠다. 2026-08-10 에
      > 등재된 뒤 **18일 동안 미집행**이었고 그 사이 비용이 계속 났다.
- [x] ~~(선택) 게이트 메시지에 그 이유를 한 줄 넣을지 판정~~ → **넣기로 판정하고 집행
      (2026-08-28, `impl-done-ordering-convention`)**.
      > **규약만으로는 부족하다**: 이 함정을 다섯 번 밟은 세션이 `developer/SKILL.md` 를
      > 읽고 있었다. 문제는 규약의 부재가 아니라 **메시지가 판정 기준을 안 보여 준다**는
      > 것이고, 그래서 매번 게이트 결함으로 오진했다. 메시지에 두 시각을 실으면 그 오진이
      > **구조적으로 불가능**해진다.
      > 집행: `review_guard.py` 의 SPEC-CONSISTENCY freshness 분기에
      > `(리포트 세션 시각 <ts> < 최신 spec-linked 편집 <ts> — 판정 기준은 세션 디렉터리
      > 시각이다. 규약: developer/SKILL.md §4 '순서'.)` 를 덧붙였다. `_fmt_ts` 헬퍼 신설
      > (0 이면 `(없음)`). 하네스 `test_review_gate_ci` · `test_block_integrity` OK.
## consistency 번들이 **편집 중인 target 파일 자신**을 예산에서 떨군다 (2026-08-10 실측)

`--plan` 모드 `rationale_continuity` 번들이 이번 라운드의 **검토 대상 3파일을 전부 누락**했다 —
`spec/7-channel-web-chat/{2-sdk,3-auth-session,4-security}.md` 가 하단 "컨텍스트 예산 초과로
생략된 파일 77개" 목록에 그대로 들어 있었다. checker 가 워킹트리를 직접 읽어 우회했으나,
**우회하지 않았으면 자기가 검토해야 할 문서를 못 본 채 판정**했을 것이다.

기등재된 `--spec` 예산 항목(`related_specs` 가 `5-system/` 에 도달 못 함)과 **다른 형태**다:
...
- [x] **동일 커밋의 형제 파일이 부분만 뽑히는 원인 확인 — 종결 (2026-08-10).**
      원인이 두 겹이었고 **둘 다 닫혔다**: (1) 같은 초 세션 충돌로 앞 배치가 디스크에서
      지워지던 것(`create_session_dir` 원자화, 아래 `[x]`), (2) 그래서 애초에 소비되지도
      않던 첫 배치 — **배치 분할 자체를 제거**해 닫았다(아래 `[x]`). router fail-closed
      교차검사까지 더해 증폭 구조도 막았다. 조사 항목으로서는 답이 나왔으므로 종결한다.
      _(원 서술은 아래에 그대로 둔다 — 처분의 근거이고, 두 겹을 갈라낸 과정이 그 안에 있다.)_

- [~] _(원문 — 종결의 근거로 보존)_ **동일 커밋의 형제 파일이 부분만 뽑히는 원인 확인** — code-review changeset 산정에서
      ...
      **2026-08-10 — 원인 확정: (b) 타임스탬프 충돌이다.** 코드와 실행 양쪽으로 갈랐다.

      - `session.create_session_dir()` 이 경로를 `<Y>/<m>/<d>/<hh>_<mm>_<ss>` **초 단위**로
        만들고 `os.makedirs(..., exist_ok=True)` 로 끝난다 — **충돌 처리가 없다**
        (`.claude/skills/code-review-agents/lib/session.py:23-44`).
      - `main()` 의 배치 루프가 배치마다 `prepare_session()` 을 부르는데
        (`code_review_orchestrator.py:1586-1597`), 배치들은 같은 초 안에 준비된다.
      - **실행 재현**: 같은 초에 `create_session_dir()` 을 두 번 부르면 **동일 경로**를
        반환하고, 두 번째 `save_metadata()` 가 첫 번째를 덮어써 배치 1의 파일 목록이
        흔적 없이 사라진다.

      ⇒ 처방은 **세션 이름에 배치 인덱스를 넣는 것**(또는 충돌 시 suffix). (a) 가정이었던
      "배치 = 순차 리뷰 계약 문서화" 는 해당 없음.

      - [x] **`create_session_dir()` 충돌 회피 — 구현 완료 (2026-08-10).**
            `exist_ok=False` 로 **원자적으로** 만들고, 이름이 선점됐으면
            `<hh>_<mm>_<ss>_2`·`_3` … 로 비켜난다. 원자성은 병렬 세션 때문에 필요하다
            (두 프로세스가 동시에 "내가 이겼다" 고 믿을 수 없다). 디렉터리명을 파싱하는
            곳은 없어서 — 게이트들은 트리를 걸어 `SUMMARY.md` 를 찾는다 — 접미사는 무해하다.
            소진 시엔 옛 동작(`exist_ok=True`)으로 떨어진다: 세션 디렉터리를 잃는 건
            나쁘지만 리뷰를 아예 못 도는 건 더 나쁘다.
            테스트 `test_review_session_dir_collision.py` 7건, 뮤테이션 5종 RED.
            **시계를 고정**해 같은 초를 재현한다 — 실제 시계로는 초가 넘어가 결함이 살아
            있어도 초록일 수 있다.
      - [x] **배치 2개 이상이면 첫 배치는 여전히 리뷰되지 않는다 — 처분 완료 (2026-08-10).**
            **막힌 전제가 풀렸다.** 아래는 "`REVIEW_BATCH_SIZE` 가 막던 실패 양태를 모른 채
            없앨 수 없어 근거 수집이 선행" 이라 적어 뒀는데, `git log -S` 로 갈렸다 —
            배치 루프 도입 커밋(`3446d0d57`)의 코드 주석이 `One session_dir per stdout line.
            Main parses these.` 이고 당시 SKILL.md 는 "마지막 줄**(들)** … **batch 별로 한 줄씩**"
            이었다. 문서 압축 커밋 `73dea0864` 이 `(들)` 과 괄호 설명만 흘렸고 **같은 커밋이
            `REVIEW_BATCH_SIZE` 행은 유지**했다. 코드 모듈 docstring 은 지금까지도
            `one line per batch` 였다. 즉 기전은 온전하고 **소비 계약만 부수적으로 유실**됐다 —
            의도적 폐기가 아니므로 "되살리기 전 폐기 이력 확인" 규칙에 걸리지 않는다.
            **증폭 경로를 실측했다** — `compute_forced_agents` 는 forced 집합을 *주어진
            changeset* 에서 도출하므로, 꼬리 배치가 문서뿐이면 집합 자체가 쪼그라든다.
            60파일(50 `.ts` + 10 `.md`) 프로브: **forced 7 → 2**, security·testing·scope·
            maintainability·side_effect 소실. `_verify_coverage` 는 그 2명만 검사해 자명하게
            통과한다 — 버려진 배치가 **깨끗한 리뷰로 읽힌다.**
            처분: **(a) 안 채택** — `--prepare` 는 세션을 하나만 만든다. 크기는 쪼갤 수 있는
            곳(reviewer 별 프롬프트 예산)에서만 쪼개고 그쪽은 버린 것을 숫자로 말한다.
            (b) 안은 coverage 게이트가 세션 단위라 N개를 각각 검사해야 해 택하지 않았다.
            **없애야 할 것은 배치가 아니라 "마지막 줄" 이라는 문구였다.**
            `REVIEW_BATCH_SIZE` 는 분할이 아니라 **안내 임계값**으로 남긴다.
            회귀 `test_review_prepare_single_session.py` **19건**(첫 조치 10 + fail-closed 7 +
            리뷰 반영 call-site 2), **그 파일에 대한** 뮤테이션 6/6 RED(분할 복원·경계
            `>`→`>=`·`compute_forced_agents` 무력화·교차검사 호출부 절단·소스 필터 제거·
            안내 호출부 절단).
            > 별건: `test_line_anchors.py` 의 삭제-전용 커밋 가드 뮤턴트 2건도 RED 지만
            > **다른 파일·다른 커밋**(`2b71fd9f7`)이다. 처음엔 위 목록에 "7/7" 로 뭉쳐
            > 적었다가 `14_46_37` requirement WARNING 으로 갈랐다 — 이 브랜치가 반복한
            > "다른 것을 하나로 뭉쳐 세는" 형태다.
            증폭 자체를 `ForcedSetShrinksWithTheChangesetTest` 로 **별도 고정**했다 — 분할이
            어떤 이유로든 되살아나도 거짓 PASS 가 조용히 재무장하지 못한다.
            > **내 테스트가 그 파일이 경고해 둔 함정에 걸렸다** — 증폭 테스트 첫 판이
            > `lib.router_safety` 를 in-process import 해 `discover` 에서만 에러가 났다.
            > 그 모듈 docstring 이 바로 그 `_lib` 충돌을 경고하며 "standalone 으로 돌려서
            > 고치지 말 것" 이라 적어 둔 자리다. fresh-interpreter 경로로 옮겼다.

      - [~] _(원문 — 위 처분의 근거로 보존)_ **배치 2개 이상이면 첫 배치는 여전히 리뷰되지 않는다 — 원인이 두 겹이었다.**
            위 수정은 **디스크 소실**만 닫는다. 남은 겹은 호출부다: `--prepare` 는 배치마다
            세션 경로를 한 줄씩 찍는데 `code-review-agents/SKILL.md §1` 이 "stdout
            **마지막 줄** = 세션 디렉토리" 로 계약한다. 즉 디렉터리를 갈라도 배치 1은
            소비되지 않는다 — 겹 1은 그 사실을 디스크에서 지워 **보이지 않게** 만들었을 뿐이다.
            선택지: (a) `--prepare` 를 changeset 당 세션 **하나**로 만들고 파일 수 shaping 은
            프롬프트 예산(`build_files_section`)에 맡긴다 — 실측상 그 기전은 작동한다
            (cap 을 올리자 프롬프트가 154KB→564KB 로 늘었다), (b) SKILL 계약을 "모든 줄" 로
            바꾸고 호출부가 세션마다 fan-out, (c) 분할이 생기면 `--prepare` 가 실패하고
            스코프를 좁히라고 지시. **(a) 가 유력**하나 `REVIEW_BATCH_SIZE` 가 막던 실패
            양태를 모른 채 없앨 수 없어 근거 수집이 선행이다. 그 전까지 우회는 위 "파일
            목록을 배치 상한 아래로 쪼개 각각 `--prepare`" 가 유효하다(검증됨).

      > **"우회가 통했다" 가 진단을 보증하지 않았다** — 우회(파일 명시 + `--route=all`)는
      > (a)/(b) 어느 쪽이어도 통했다. 코드를 읽고 **실행해서** 갈렸다.

      **2026-08-10 — 같은 결함의 두 번째 관측. 거짓 PASS 로 증폭된다.**
      세션 `03_12_29` 에서 `--prepare --branch origin/main` 이 잡은 changeset 27파일에
      **직전 두 커밋이 바꾼 코드 3파일이 하나도 없었다**(`bc10e215e` 03:07:38 ·
      `e64b1218b` 03:10:27, 세션 준비는 03:12:29 이라 타이밍 레이스도 아니다).

      그 결과 router 가 "소스 코드 변경 없음(문서 전용)" 으로 판정해
      security·testing·scope·maintainability 등 **12명을 skip** 했다 — 게이트는 "리뷰됨"
      으로 닫히는데 정작 그 라운드는 새 코드를 보지 않았다. **거짓 PASS 다.**

      > **처음에 "배치 분할과 무관한 별개 형태" 라고 적었다가 반증됐다.** 근거는
      > "27 < 상한 50 이니 배치가 아니다" 였는데, **27은 배치 크기가 아니라 나머지**였다.
      > 실측: 그 시점 브랜치 diff 는 **177파일**이고 50×3 + 27 로 갈린다. `meta.json` 의
      > 27개가 정렬된 전체 목록의 **인덱스 149부터 끝까지 연속된 꼬리와 완전히 일치**했다
      > (`tail == observed` → True). 무작위 누락이 아니라 **마지막 배치만 살아남은 것**이다.
      > 가설("이전 세션에서 리뷰된 파일을 제외한다")도 코드에 그런 로직이 없어 기각됐다.

      - [x] router 쪽 fail-closed 방어 — **처분 완료 (2026-08-10).**
            원문의 진단("산정 결함이 reviewer 전원 skip 으로 **증폭되는 구조**가 진짜 위험")이
            정확했다. router 는 소스/문서 구성을 **사실로** 통보받는데 그 사실이 changeset 에서
            도출되므로, 산정이 틀리면 오류가 "소스 코드 변경 없음" 이라는 **확신 있는 문장으로
            세탁**된다. 위 배치 수정은 원인 하나를 닫았을 뿐이고 changeset 이 틀릴 경로는 더
            있다(기본 working-tree 경로가 커밋 이후 비는 것이 문서화된 사례).
            처분: `_source_files_missing_from_changeset` — 브랜치 diff 에는 있는데 changeset 에는
            없는 **소스** 파일을 세어, 있으면 router 프롬프트가 그것들을 **이름으로** 나열하고
            "문서 전용을 사유로 제외 말 것 / 판단이 서지 않으면 넓게 선택할 것" 을 지시한다.
            비교 기준을 "직전 세션 이후 커밋" 이 아니라 **브랜치 diff** 로 잡았다 — 세션 간
            시각 비교보다 상태가 단순하고, `warn_if_committed_work_is_missing` 이 이미 쓰는
            프로브라 새 표면을 늘리지 않는다.
            changeset 자체는 안 건드린다(조용히 넓히면 호출자가 요청 안 한 파일을 리뷰한다).
            넓히는 것은 **router 에게 말해 주는 내용**뿐. git 실패는 삼킨다.
            뮤테이션 2/2 RED — **호출부 절단**(헬퍼는 그대로)과 소스 필터 제거. 전자를 따로
            돌린 이유는 이 저장소가 "헬퍼는 맞는데 아무도 안 부른다" 로 세 번 데였기 때문이다.

## 병렬 fan-out 중 리뷰어가 실제 저장소를 뮤테이션한다 (2026-08-11, `13_04_55` 실측)

- [x] **리뷰 프롬프트가 뮤테이션을 저장소 밖 scratch 사본으로 강제해야 한다.**
      → **집행 완료 (2026-08-29).** `code_review_orchestrator.py` 에
      `MUTATION_ISOLATION_CONTRACT` 를 신설하고 `build_agent_prompt_body` 에 상시로 붙였다.
      `LINE_ANCHOR_LEGEND` 와 같은 자리·같은 성격이다 — 역할 무관 상시 계약이고 한 곳에서
      조립돼야 드리프트가 없다.

      **네 축을 전부 담았다** — (a) 저장소 밖 scratch, (b) `git checkout`/`restore`/`stash`
      금지(=`cp` 원복), (c) **병렬 동시 실행 고지**, (d) 저장소 안 `.bak` 금지. (d) 가
      핵심이다: (a) 는 *뮤테이션 대상*만 말하고 *백업 목적지*를 말하지 않아 `18_16_05`
      파일 파괴를 못 막는다.

      **실측** — `build_agent_prompt_body` 를 직접 호출해 확인:
      등록된 **14개 역할 전원**에 부착, router 에는 **미부착**(router 는 코드를 건드리지
      않는다). 종전 갭 진단에 쓰였던 grep 프로브 `scratch`·`git restore`·`병렬` 이 이제
      **전부 1건 이상**이다(그때는 각 0건이었다).

      **회귀 가드**: `.claude/tests/test_reviewer_mutation_isolation.py`. 프롬프트 문구는
      리팩터 중 조용히 사라지는 종류라 — 이 항목이 2026-08-11 부터 미집행으로 남아 있던
      이유가 정확히 그것이다 — 조립 결과를 고정했다. 뮤테이션 3종 전부 RED 확인:

      | 뮤턴트 | 결과 |
      |---|---|
      | 계약 부착 제거 | RED (6 failures — 역할 전원 누락) |
      | 계약에서 `git restore`/`stash` 절 삭제 | RED (`git stash` 축만 정확히 지목) |
      | router 에도 부착 | RED (스코프 고정) |

      > **④ 네 번째 재발** (2026-08-29 등재, `15_47_21`): security·scope 리뷰어가
      > `cause: err` 가 일시 제거된 과도 상태와 `.bak` 을 관측했다. 또 testing 리뷰어의
      > 자체 뮤테이션이었다(그 리포트에 명시). 원복은 정상이었지만 security 는 그것을
      > 혼란스러워하며 별도 확인에 시간을 썼다. **네 번 연속 우연에 기댄 것이 (d) 집행의
      > 최종 근거**였다.

      _(아래 원 서술은 처방의 근거로 보존)_

      `13_04_55` 라운드에서 7명이 같은 워크트리에서 동시에 도는 동안, scope 리뷰어가
      **저장소 안의** `triggers.service.ts` 에 뮤턴트를 심었다(자기 리포트에 명시). 그 결과:

      - **side_effect** 는 자기 조작 없이 파일이 감사/저장 순서를 역전한 상태로 변했다
        사라지는 것을 관측하고 **CRITICAL 로 에스컬레이션**했다. 하필 그 PR 이 막으려던
        결함 형태라 그렇게 읽히는 것이 당연하다.
      - **testing** 은 같은 상태를 **3회** 관측하고 **`git restore` 로 되돌렸다**.
        그 시점에 미커밋 편집이 떠 있었다면 그대로 유실된다 — 이 저장소가 이미 두 번
        당한 형태다(`feedback_mutation_harness_restore_cp_not_checkout`).
      - **requirement** 도 같은 것을 관측해 "push 전 잔존 여부 확인 필요" 를 리포트에 적었다.

      즉 **한 리뷰어의 뮤테이션이 다른 리뷰어들의 판정을 오염시킨다**. 세 명이 같은 유령을
      쫓았고 그중 하나는 남의 워킹트리를 되돌렸다.

      직접 원인은 "직접 재현하라" 고 쓴 호출자 프롬프트지만, **프롬프트 문구에 기대는 것
      자체가 약한 방어다** — 같은 라운드에서 testing 은 시키지 않아도 scratch 를 썼고
      scope 는 시켰더니 저장소를 썼다. 리뷰어 프롬프트 템플릿(`_prompts/*.md` 조립부)에
      뮤테이션 규약을 **상시 문구로** 넣는 편이 맞다: (a) 저장소 밖 사본에서만,
      (b) 원복은 `cp`(`git checkout`/`git restore` 금지), (c) 다른 리뷰어가 동시에 돈다는
      사실을 알린다.

      부수 관찰 — 파일이 세션 밖에서 바뀌면 하니스가 "이 변경은 의도적이니 사용자에게
      알리지 마라" 는 취지의 정형 문구를 붙인다. side_effect 는 그것을 **은폐 지시로 의심해
      거부하고 전부 보고했고, 그 판단이 옳다**. 리뷰어에게 그 문구가 전달되는 것이 적절한지는
      별도로 볼 일이다 — 도구 출력의 문구가 리뷰어의 보고를 막을 수 있으면 리뷰가 무력해진다.

      ---

      ### 후속 사례 3건 (2026-08-27 ~ 08-28) — **정본 트래커에서 이관**

      > 이 셋은 처음에 `spec-sync-external-interaction-api-gaps.md` 에 별도 항목으로
      > 등재됐다가 여기로 합쳤다. **한 클래스가 두 집에 있으면 다음 사람이 한쪽만 본다** —
      > 리뷰어 프롬프트 템플릿은 하네스 소관이므로 주인은 이 문서다.

      **① 관측 오염 + 원인 오귀속** (2026-08-27, `14_10_42` INFO 9)

      requirement 리뷰어가 *"이 diff 와 무관한 미커밋 뮤테이션 잔여물
      (`DEFAULT_SENSITIVE_KEYS` 에 `oauthCred` 삽입) — **다른 병렬 세션**의 산물로 추정"*
      이라 보고했다. 실제로는 **같은 라운드의 testing 리뷰어**가 캐너리를 재현하려 넣은
      것이다. 라운드 종료 후 작업트리·HEAD 모두 clean 확인 — 실질 오염은 없었다.

      **원인이 잘못 귀속되면 조사 비용이 발생한다** — "다른 세션" 을 쫓게 된다.

      > **그때 실측한 갭**: `_prompts/testing.md` 에 `scratch`·`git restore`·`병렬` 언급
      > **각 0건**(grep). 즉 위 (a)(b)(c) 처방이 프롬프트 생성 경로에 **반영된 적이 없다**.
      > 이 문서의 2026-08-11 항목이 그대로 미집행 상태라는 뜻이다.

      **② 파일 파괴 near-miss — 등급이 다르다** (2026-08-27, `18_16_05`)

      testing 리뷰어가 뮤테이션 백업의 **`cp` 목적지를 잘못 지정**해 worktree 루트의
      untracked 파일 `scratchpad_idem_backup.ts` 를 덮어썼다가 삭제했다고 스스로 보고했다.
      **untracked 라 git 으로 복구할 수 없다.**

      실측: main checkout 의 그 파일은 **무손상**이었다(23,798 bytes, mtime 8/13). 그 worktree
      는 `origin/main` 에서 새로 만들었고 파일이 untracked 라 체크아웃되지 않으므로, 리뷰어가
      만든 파일을 리뷰어가 지운 것이다.

      **그래도 등급을 올린다.** ①은 *"관측이 오염될 수 있다"* 였지만 이건 *"복구 불가능한
      파일을 지울 수 있다"* 이고, **안 터진 이유가 설계가 아니라 우연**이다(그 경로 이름이
      그 worktree 에 없었다). 같은 실수가 main checkout 을 향했으면 8/13 부터 있던 남의
      파일이 사라졌다.

      > **처분 후보 (d) 추가**: 백업 목적지를 **세션 scratch 로 강제**한다
      > (`/private/tmp/claude-*/.../scratchpad`). 리뷰어가 저장소 트리 안에 **무엇도 쓰지
      > 않게** 하는 것이 (a)~(c) 보다 강한 불변식이다. 이 사례는 **(a) 만으로는 못 막는다**
      > — (a) 는 *뮤테이션 대상*을 말하지 *백업 목적지*를 말하지 않는다.

      **③ 세 번째 — 같은 형태가 또** (2026-08-28, `02_02_18` INFO 7)

      documentation 리뷰어가 워크트리에서 `use-execution-events.ts.bak`(52KB)을 관측했다.
      라운드 종료 후엔 없었다(`find` 0건). 그 라운드 testing 리뷰어가 *"뮤테이션 후 즉시
      원복"* 했다고 보고했으므로 `cp file file.bak` 로 저장소 안에 만들었다 지운 것이다.

      **①②③ 이 전부 무해했지만 무해했던 이유가 매번 우연이다.** 세 번 연속 우연에 기댔다는
      사실 자체가 (d) 를 집행할 근거다.
...
```
_CHECKBOX = re.compile(r"^\s*[-*]\s+\[(?P<mark>[ xX])\]")   # plan_guard.py:76

pg._CHECKBOX.match('        > - [ ] 항목')  → None     ← 인용문 안
pg._CHECKBOX.match('- [ ] 항목')            → Match
pg._all_checkboxes_done(ROOT, plan)         → True     ← 열린 항목 3건이 있는데도
```

앵커 `^\s*` 가 공백만 허용해 `>` 에서 끊긴다.

**왜 중요한가**: 이 저장소가 이미 아는 실패의 **새 입구**다 — 살아 있는 항목을 품은 채
`complete/` 로 봉인되면 유실된다(`spec-draft-error-cause-criterion.md` 선례). nudge 는
*"무시해도 된다"* 고 안내하지만, **판정 자체가 틀렸다는 신호는 어디에도 없다.**
그 자리에서는 plan 서식을 고쳐 회피했다(정본 체크박스를 최상위로 승격).

**판정이 필요한 이유 — 자명하게 (a) 가 아니다**:

- (a) **넓힌다**: 앵커를 `^[\s>]*[-*]\s+\[` 로. 한 문자 클래스 변경이고 blockquote 접두는
  유한한 문법이라 "정밀 파서로 미끄러지는" 종류가 아니다.
  다만 **인용문은 이 저장소에서 서술·인용용**이라, 남의 plan 을 인용한 블록 안의 체크박스나
  과거 상태를 인용한 블록까지 "열린 작업" 으로 세게 된다 — **거짓 양성 방향**으로 바뀐다.
- (b) **그대로 두고 규약으로**: "열린 체크박스는 인용문 밖에" 를 `plan-lifecycle.md` 에
  명문화. 게이트는 단순하게 두되 서식 규칙을 사람이 지킨다 — 지금 실제로 밟은 경로다.

방향이 다르다(거짓 양성 vs 거짓 음성). 거짓 음성이 봉인·유실로 이어지므로 (a) 가 유력한데,
**세지 않고 넓히면 이번엔 반대 방향 오탐을 만든다** — 그래서 등재하는 자리에서 바로 셌다.

**실측 (2026-08-29, `grep -rn '^\s*>.*\[ \]' plan/in-progress/`)** — 9건 / 6개 문서.
성격이 둘로 갈린다:

| 형태 | 건수 | 넓히면 매치되나 |
|---|---|---|
| 인용문 안의 **진짜 열린 체크박스** (`> - [ ] …`) | **3** (전부 `marketplace-and-plugin-sdk.md:86-88`) | **예 — 그리고 그게 옳다** (실제 열린 작업) |
| `[ ]` 를 **문자열로 인용**한 서술 (`> 종전엔 \`[ ]\` 로 남아 있어…`) | 6 | **아니오** — 불릿 구조가 없어 `[-*]\s+\[` 에 안 걸린다 |

즉 이 저장소의 실제 데이터에서 (a) 는 **거짓 양성을 만들지 않는다.** 새로 세어질 3건은
전부 세어져야 마땅한 열린 작업이고, 서술용 `[ ]` 언급은 불릿이 아니라 그대로 통과한다.
→ **(a) 로 기운다.** 착수 시 위 표를 회귀 픽스처로 쓸 것(양쪽 형태를 다 담고 있다).

**주의**: 넓히더라도 `marketplace-and-plugin-sdk.md` 의 판정은 오늘 안 바뀐다 — 그 문서는
최상위 미체크가 이미 여럿이라 `_all_checkboxes_done` 이 원래 `False` 다. 이 변경의 효과는
"지금 무언가를 고친다" 가 아니라 **"최상위 항목이 전부 닫힌 문서에서 인용문 안 잔여가
숨는 경로를 막는다"** 다 — 이번에 실제로 밟은 그 경로.
...
형태가 다르다. 그쪽은 *"관련 없는 것으로 분류돼 뒤로 밀렸다"* 였고, 이쪽은
**"최상단으로 승격됐는데 같은 tier 의 1번 파일이 몫 전체를 먹어 2~4번이 죽었다"** 다.
순서를 고치는 처방으로는 원리적으로 닫히지 않는 잔여다.

### 실측 (`--spec plan/in-progress/spec-draft-schedule-trigger-ref-nav.md`)

draft frontmatter `spec_impact` + 본문이 네 파일을 지목한다. 넷 다 tier 1 로 **정상 승격**됐다.
같은 tier 안은 자연순이라 `spec/1-data-model.md` 가 1번이 된다:

| 순위 | 파일 | 바이트 | **문자**(예산이 비교하는 단위) |
|---|---|---|---|
| 1 | `spec/1-data-model.md` | 112,225 | **82,740** |
| 2 | `spec/2-navigation/2-trigger-list.md` | 42,611 | 29,416 |
| 3 | `spec/2-navigation/3-schedule.md` | 13,035 | 8,239 |
| 4 | `spec/5-system/2-api-convention.md` | 44,094 | 29,903 |

기본 예산 `CONSISTENCY_MAX_CONTEXT_SIZE=262144` × `CHECKER_BUDGET_RATIO["corpus"]=0.40`
= **corpus 104,857자**.

결과 — `_prompts/cross_spec.md` 를 파싱해 청크별로 센 값: **적재 1개 / 생략 112개.**
적재된 1개는 `1-data-model.md` 이고, 생략 112개의 **앞 3개가 2·3·4번** 이다. 즉 이 checker 는
**draft 가 편집하는 세 문서를 한 글자도 못 본 채** 판정하려 했다.

`BLOCK: NO` 가 "봤는데 괜찮다" 가 아니라 "안 봤다" 가 되는, 이 저장소가 이미 세 번 밟은 형태다.

> ### ⚠️ 위 진단의 산술을 정정한다 (2026-09-10 재실측, 등재 당일)
>
> 처음 이 항목은 ~~*"1번 파일 하나가 그 몫보다 크다(112,225 > 104,857)"*~~ 라고 적었고
> 적재된 청크를 *"82,779자로 절단"* 이라고 적었다. **둘 다 틀렸다.** `112,225` 는 `wc -c` 로
> 잰 **바이트**이고, 예산이 비교하는 것은 **문자**다 (`len(text) <= budget`). 한국어 문서는
> 배율이 1.36~1.47 이라 이 혼동은 값을 40% 이상 부풀린다.
>
> | 값 | 바이트 | 문자 |
> |---|---|---|
> | `spec/1-data-model.md` | 112,225 | **82,740** |
>
> 즉 그 파일은 몫(104,857자)보다 **작고**, 번들에 실린 82,779자는 절단이 아니라 **전문 + 청크
> 헤더**다. *(이 저장소가 §9.1 작업에서 이미 한 번 밟은 바이트-문자 혼동을, 같은 세션에서
> 반복했다.)*
>
> **그러면 왜 2·3·4번이 죽었나 — 진짜 기제는 두 겹이다.**
>
> 1. **꼬리 드롭이지 greedy fill 이 아니다.** `truncate_file_bundle` 은 `kept.pop()` 으로
>    **맨 뒤부터** 하나씩 버리며 전체가 들어갈 때까지 반복한다. `1-data-model.md` 뒤 남은 몫은
>    22,078자였는데 다음 파일 `2-trigger-list.md`(청크 **29,470**자)가 안 들어갔고, **그 뒤의
>    `3-schedule.md`(청크 **8,289**자)는 산술적으로 들어가는데도 함께 죽었다.** 앞이 막히면
>    뒤는 크기와 무관하게 전멸한다.
>
>    *(예산이 세는 단위는 **청크** = 파일 문자 + `#### \`경로\`` 헤더 ~50자다. 위 표의
>    파일 문자와 여기 청크 값이 그만큼 다른 것은 오차가 아니다 — 29,416+54 · 8,239+50 ·
>    82,740+39 이 각각 실측 청크 29,470 · 8,289 · 82,779 와 맞는다.)*
> 2. **생략을 *알리는* 비용이 본문과 경쟁한다.** 드롭된 자리마다 stub 표식이 남고 말미에
>    경로 목록이 붙는다 — 112개면 stub ~13,400자 + 목록 ~5,700자 ≈ **19,000자(몫의 18%)**.
>    많이 버릴수록 알림 비용이 커져 **버릴 이유가 더 커지는** 방향으로 작동한다.
>
> **결함의 성격은 바뀌지 않는다** — 승격된 tier 의 꼬리가 굶고, 회귀 테스트는 순위만 단언한다.
> 바뀌는 것은 **처방의 후보**다: 아래 (a)/(b)/(c) 에 네 번째가 붙는다.
>
> - (d) **greedy fill 로 바꾼다** — 안 들어가는 청크는 건너뛰고 뒤의 작은 청크를 계속 담는다.
>   이번 케이스에서 `3-schedule.md`(8,289자)가 **살았을** 유일한 처방이다. 단 "kept 는 연속된
>   prefix, dropped 는 연속된 suffix" 라는 현재 불변식이 깨지므로(코드 주석이 그 전제로
>   stub 을 배치한다) stub 배치 로직을 함께 고쳐야 한다 — **비용이 (a)보다 크다.**
>
> 그리고 **어느 처방이든 측정 단위를 바이트가 아니라 문자로 적어라** — 이 항목이 그걸 한 번
> 틀렸다.

### 왜 기존 회귀 테스트가 못 잡나

`test_consistency_bundle_priority.py` 의 `TheDocumentBeingEditedIsNeverOmittedTest` 4건은
**순위**를 단언한다 — "미커밋 편집이 tier 0 에 도달", "`collect_context` 산출물의 0번 청크".
이번 케이스는 그 단언을 **전부 통과한다**. 승격은 실제로 일어났다. 단언되지 않은 것은
**생존**이다. 이름이 약속하는 것("never omitted")과 검사하는 것(never demoted)이 어긋나 있다.

### 이번 턴의 우회 (영구 처방 아님)

`CONSISTENCY_MAX_CONTEXT_SIZE=800000`(corpus 320,000)으로 재생성 → 네 파일 전부 본문 적재
(29,470 · 8,289 · 29,955 · 82,779). 폐기 세션 `11_08_19` 는 삭제했다 — 부분 세션이 게이트를
거짓 통과시킨다.

**단, `naming_collision` 은 이 예산으로도 안 닫혔다.** 그쪽은 corpus 를
spec/plan/conventions **3개 하위 코퍼스로 다시 쪼개**므로 spec 몫이 ~106,667자에 그쳐
1번 파일이 또 전부를 먹는다(적재 5개 중 spec 은 `1-data-model.md` 하나, 생략 445개).
전부 담으려면 예산 ~1,600,000 이 되고 프롬프트 5개가 각 ~800KB 다. 이번 draft 는 신규
식별자가 0건이라 그 비용을 쓰지 않고, **dispatch 프롬프트에 세 파일 직독을 명시**한 뒤
리포트에서 실제로 열었는지 확인하는 쪽을 택했다.

### 미해결

- [ ] **`prioritize_bundle_files` 가 승격한 파일의 *생존*을 보장하는 층이 없다.** 처방 후보 셋:
      (a) tier 0/1 에 **파일당 상한**을 걸어(예: 몫의 40%) 한 파일이 tier 를 굶기지 못하게 한다 —
      1번 파일은 절단되지만 2~4번이 산다. (b) tier 0/1 은 **몫 밖에서** 먼저 싣고 tier 2+ 만
      몫으로 경쟁시킨다 — 승격의 뜻에 가장 충실하지만 상한이 무의미해질 수 있다.
      (c) 생략 목록에 **"승격된 파일이 잘렸다"** 를 별도 경고로 승격시켜 최소한 조용하지 않게 한다 —
      가장 싸고, (a)/(b) 와 배타적이지 않다.
      **어느 쪽이든 `--spec`/`--impl-prep`/`--impl-done` 세 모드에 같이 걸어야 한다** — 이 결함은
      랭킹 층에 있어 모드와 무관하다.
- [ ] **회귀 테스트의 주어를 순위에서 생존으로 옮긴다.** 지금 이름이 `…IsNeverOmittedTest` 인데
      단언은 순위뿐이다. 캐너리: **tier 1 의 1번 파일이 corpus 몫보다 큰** 번들에서 2번 파일의
      본문이 살아 있는가. 이 캐너리는 오늘 RED 여야 한다(그게 이 항목의 전제 증명이다).
      > 2026-09-25 보탬(`harness-probe-isolation`): 이 클래스의 순위 테스트는 이제 실제 트리가 아니라
      > `spec/5-system` 의 **임시 사본**에서 프로브한다(`_harness.make_temp_repo_copy`). 캐너리가 미커밋
      > 편집을 필요로 하면 같은 헬퍼를 쓸 것 — 이 체크아웃에 쓰면 병렬 하네스 실행이 서로의 프로브를 되살린다.
- [ ] **`naming_collision` 의 3분할이 spec 몫을 구조적으로 굶긴다** — 위 처방과 같은 병이지만
      분모가 하나 더 있다. (a)/(b) 를 고를 때 이 checker 를 반드시 표본에 넣을 것.

## §M. `--impl-done` 번들의 diff 는 커밋 기준인데 preamble 은 워킹트리를 SoT 라 선언한다

**등재 2026-09-10** (`trigger-workflow-ref-canary` 세션, checker **두 명이 독립적으로 발견**).
`review/consistency/2026/09/10/15_23_41` — `naming_collision` 과 `rationale_continuity` 가 각각
*"프롬프트에 번들된 diff 는 세션 시작 시점의 것"* · *"프롬프트 번들의 diff 섹션은 stale"* 이라 적었다.

### 실측 — 미커밋 수정의 26%가 번들에서 보이지 않았다

| 잰 것 | 값 |
|---|---|
| 번들이 실은 diff (세-점, `<base>...HEAD -- codebase/`) | **3파일 / 402 insertions** |
| 같은 번들의 census 문장 | *"구현 diff: 3개 파일 / **420줄**"* |
| 실제 워킹트리 (두-점, `<base> -- codebase/`) | **3파일 / 541 insertions** |
| 번들에 안 보인 양 | **139 insertions (26%)** — `/ai-review` 수정 12건이 만든 그 줄들 |

### 왜 결함인가 — 세-점 자체는 옳다. 틀린 것은 **기준을 말하는 문장**이다

`_collect_code_diff()` 의 세-점(`A...B`)은 **의도적**이고 그 사유가 코드 docstring 에 적혀 있다 —
`diff_base` 가 fork point 를 지나 전진했을 때 두-점은 남의 커밋을 내 diff 로 주입한다
(`harness-consistency-bundler-budget` §H residual). **그 판단은 유지해야 한다.** 문제는 같은
프롬프트의 두 문장이 서로 다른 기준을 말하는 것이다:

- `_head_basis_notice()` — *"diff-base 대비 신규·변경 코드가 **모두 반영된** working tree"* 가 SoT 라고 선언
- census + diff 본문 — **커밋된 HEAD** 만 담는다

즉 미커밋 수정이 있으면 **번들이 델타를 조용히 축소해 말한다.** 이번엔 checker 둘이 preamble 의
*"상대경로를 신뢰하지 말고 절대경로로 재확인하라"* 지시를 따라 스스로 잡았지만, **복구가 checker 의
성실성에 의존했다** — 나머지 셋은 그 불일치를 언급하지 않았다. 그리고 이 형태는 `--impl-done` 의
정상 워크플로에서 **항상** 발생한다: developer SKILL §4 가 *"코드가 고정된 뒤 `--impl-done` 을
준비한다"* 고 규정하는데, 그 "고정" 은 커밋이 아니라 **fix 적용 완료**를 뜻하고 커밋은 보통 그 뒤다.

### 처방 — 두-점으로 바꾸지 말 것

- [ ] (a) **기준을 정직하게 말한다**(권장, 저렴): 코드 영역에 미커밋 변경이 있으면 census 에
      *"이 diff 는 **커밋 기준**이고 워킹트리에 미커밋 수정 N파일이 더 있다 — 절대경로로 읽어라"*
      한 줄을 넣는다. preamble 은 이미 절대경로 재확인을 지시하므로, **그 지시가 발동해야 하는
      상황임을 알려주는 것**이 빠진 부분이다.
- [ ] (b) 대안: `HEAD` 대신 미커밋을 포함한 트리를 diff 대상으로 삼는다 — fork-point 보호를
      유지하려면 merge-base 를 손으로 계산해야 하고, 세-점의 의미를 재현하는 것이라 (a) 보다 비싸다.
- [ ] 캐너리: **미커밋 코드 수정이 있는 워킹트리에서 `--impl-done` 을 준비**했을 때 census 가 그
      사실을 말하는가. 오늘은 RED 여야 한다(그게 이 항목의 전제 증명이다).

> **판정 근거로 쓰지 말 것 하나.** *"checker 가 잡았으니 실질 피해 0"* 은 이 항목의 근거가 아니다 —
> 잡은 것은 둘뿐이고, 잡는 방법(절대경로 재확인)은 preamble 이 **다른 목적**(checker CWD 가 변경 전
> 체크아웃일 수 있음)으로 넣은 지시다. 즉 이번 복구는 **다른 결함을 위한 방어가 우연히 덮은 것**이다.

## §N. 「리뷰어 뮤테이션」 종결 문구가 실제 계약보다 넓다 — 결함 재개가 아니라 문면 정정

**등재 2026-09-10** (`trigger-workflow-ref-canary` 2라운드 `/ai-review` 중 실시간 관측).

위 §현재 상태의 「병렬 fan-out 중 리뷰어가 실제 저장소를 뮤테이션한다」 항목은 2026-08-29 에
*"처분 후보 (d)(뮤테이션을 저장소 밖 scratch 로 **강제**)를 리뷰어 프롬프트 조립부에 상시 계약으로
넣었다"* 며 종결됐다. **그런데 조립된 계약은 강제가 아니라 2절 구조다** (`_prompts/*.md` §검증용
뮤테이션 규약, 실측):

1. 저장소 트리 안에 아무것도 쓰지 마라 — 대상도, 백업도, 임시 파일도.
2. **"그래도 저장소 파일을 고쳐야만 재현되는 경우에만"** 원본을 임시 디렉터리로 `cp` 해 두고
   고친 뒤 `cp` 로 되돌려라 (`git checkout`/`restore`/`stash` 금지).

2절은 **의도적으로 옳다** — jest 는 실제 경로를 해석하므로 헬퍼 뮤테이션은 저장소 파일을 고치지
않으면 재현되지 않는다. 문제는 종결 문구가 그 폴백을 지우고 *"강제"* 라고만 적은 것이다.

### 이번 세션이 그 비용을 실측했다

2라운드에서 `testing` 이 2절을 근거로 헬퍼를 직접 뮤테이션했고(계약 준수, `cp` 원복 완료),
그 사이 **다른 reviewer 둘이 그 중간 상태를 관측**했다 — `requirement` 는 *"관측된 저장소 이상
상태"* 로 리포트 상단 한 절을 썼고 오케스트레이터에게 원복 확인을 요청했으며, `maintainability` 는
인용 블록으로 남겼다. 즉 **판정이 오염되지는 않았지만 두 리포트의 지면과 주의가 소비됐다.**

### 처분

- [ ] 종결 문구를 *"scratch 로 강제"* → **"scratch 를 기본으로 하고, 저장소 파일을 고쳐야만
      재현되는 경우에 한해 `cp` 백업/복원을 허용"** 으로 정정한다. **결함 재개가 아니다** —
      구현이 옳고 문면이 넓었을 뿐이며, 이 저장소가 반복해 겪은 "문서한 보장이 구현보다 넓다"
      형태다.
- [ ] (선택) 2절을 쓰는 reviewer 가 **다른 reviewer 에게 보이는 창**을 줄일 방법 — 예: 뮤테이션
      실험을 리포트 작성 **전** 한 구간에 몰아 끝내라는 한 줄. 지금 계약은 원복만 요구하고
      **노출 시간**은 말하지 않는다. 비용이 리포트 지면이라 우선순위는 낮다.

### 3라운드가 더 나쁜 축을 하나 드러냈다 — **백업이 차단되고 뮤테이션만 진행된다**

`review/code/2026/09/10/16_26_57` 의 `testing` reviewer 가 스스로 밝혔다: 원본을 scratch 로 `cp`
하려던 스크립트에 `git … status` 가 **같이 들어 있어** 워크트리 격리 훅이 **스크립트 전체를
사전 차단**했고, 그것을 인지하지 못한 채 뮤테이션을 진행해 **백업 없이 저장소 파일이 뮤테이트된
창**이 생겼다. 복원은 직전 `Read` 로 확보해 둔 원문을 재삽입해서 했고, 라인 단위 일치 ·
12/12 GREEN · `git status` clean 세 가지로 확인됐다 — 결과는 무해했지만 **무해했던 이유가 우연**
이다(그 `Read` 가 없었으면 복원 근거가 사라진다).

이것은 이 저장소가 이미 아는 형태다 — *"차단되는 명령에 부수 작업을 붙이면 통째로 사라진다"*.
새로운 것은 **그 형태가 리뷰어의 안전망에 적용될 때의 결과**다: 사라지는 부수 작업이 `cp` 백업이면,
차단은 안전을 지키는 대신 **안전망만 걷어내고 위험한 동작은 통과시킨다.**

- [ ] 리뷰어 프롬프트의 뮤테이션 규약 2절에 **"백업 명령과 `git`/`cd` 를 같은 명령에 섞지 말 것 —
      훅이 스크립트 전체를 거부하면 백업만 사라지고 뮤테이션은 진행된다"** 한 줄을 넣는다.
      비용이 한 줄이고, 막는 것은 *복원 근거 없는 뮤테이션*이다.
