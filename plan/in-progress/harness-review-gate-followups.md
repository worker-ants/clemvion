...
- [ ] **워크플로 규약으로 명문화** — "`--impl-done` 은 spec-linked 편집이 **전부 끝난 뒤**
      준비한다." 지금 이 순서를 말하는 문서가 없어서, checker 를 돌린 뒤 후속 fix 가
      spec-linked 파일을 건드리면 그 라운드가 통째로 버려진다.
      > 함정의 성격: 게이트 메시지는 "재실행하라" 고만 말하고 **왜 방금 것이 무효인지**
      > 는 말하지 않는다. 세션 디렉터리 시각이 판정 기준이라는 사실이 메시지에 없다.
- [ ] (선택) 게이트 메시지에 그 이유를 한 줄 넣을지 판정 — "리포트 세션 시각(<ts>)이 최신
      spec-linked 편집(<ts>)보다 앞섭니다" 형태면 다음 사람이 두 번 돌리지 않는다.
