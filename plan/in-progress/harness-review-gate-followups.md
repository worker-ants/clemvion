...
- [ ] **동일 커밋의 형제 파일이 부분만 뽑히는 원인 확인** — code-review changeset 산정에서
      ...
      **2026-08-10 — 원인 확정: (b) 타임스탬프 충돌이다.** 코드와 실행 양쪽으로 갈랐다.
      ...
      - [x] **`create_session_dir()` 충돌 회피 — 구현 완료 (2026-08-10).**
            ...
            **시계를 고정**해 같은 초를 재현한다 — 실제 시계로는 초가 넘어가 결함이 살아
            있어도 초록일 수 있다.
      - [ ] **배치 2개 이상이면 첫 배치는 여전히 리뷰되지 않는다 — 원인이 두 겹이었다.**
      ...
      **직전 두 커밋이 바꾼 코드 3파일이 하나도 없었다**(`bc10e215e` 03:07:38 ·
      ...
      - [ ] router 쪽 fail-closed 방어 — changeset 이 "문서 전용" 으로 보여도 **직전 세션
            이후 커밋이 `codebase/**` 를 건드렸으면** 그 판정을 못 내리게 한다. 산정 결함이
            곧바로 reviewer 전원 skip 으로 **증폭되는 구조**가 진짜 위험이다
