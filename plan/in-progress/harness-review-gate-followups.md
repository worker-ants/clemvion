...
- [ ] **동일 커밋의 형제 파일이 부분만 뽑히는 원인 확인** — code-review changeset 산정에서
      ...
      **2026-08-10 — 원인 확정: (b) 타임스탬프 충돌이다.** 코드와 실행 양쪽으로 갈랐다.
      ...
      - [ ] `create_session_dir()` 충돌 회피 — 배치 인덱스 또는 `_2` 류 suffix.
            **회귀 테스트 먼저**: 같은 초 두 호출이 서로 다른 경로를 반환해야 한다
      - [ ] 배치가 여러 세션을 만들 때 stdout 의 세션 경로가 **배치 수만큼** 나오고 main 이
            그것을 전부 소비하는지 확인 — 지금은 같은 경로가 N번 출력된다
