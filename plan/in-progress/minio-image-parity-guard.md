...
| W3 | `self-hosting-deployment.md` 가 만들 새 매니페스트가 가드 목록에 반영될 길이 없다 | **반영** — 그 plan §3 · §4 에 «가드 자리 목록 · pathspec 에 추가» 체크박스, 가드 docstring · README 에 «새 매니페스트는 손으로 추가» 를 적었다. ~~같은 §3 의 «버킷 자동 생성» 줄이 아바타 공개 정책을 빠뜨린 것도 함께 적었다~~ — 그 체크박스는 리뷰가 두 라운드 연속 스코프 밖이라 짚어(`00_25_55` W3 · `00_56_30` W2) **트래커의 k8s 버킷 Job 항목으로 옮겼다** |
