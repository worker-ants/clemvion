...
## 3. 문서 3곳 중복 → hub canonical (#962 INFO#5)

"상태 기록은 자동 / SUMMARY 는 main 이 멱등 Write" 서술이 `subagent-call-contract.md`,
`code-review-agents/SKILL.md`, `consistency-checker/SKILL.md` 에 **각각 다른 문구로** 있다.
메커니즘이 또 바뀌면 3곳을 손으로 맞춰야 한다 — 실제로 "terminal" 오기가 7개 파일에 복제됐던
전례가 있다.

- [ ] `subagent-call-contract.md` 를 canonical hub 로 하고 두 SKILL 은 인용만 하도록 정리
