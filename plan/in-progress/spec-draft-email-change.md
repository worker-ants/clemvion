...
5. [ ] spec PR → merge 후 별도 PR 로 developer 구현 위임(`/consistency-check --impl-prep spec/2-navigation/` 부터). 구현 범위: data-model 마이그레이션 `V100`, users 모듈 email-change controller/service/DTO, MailService 2개 템플릿(KO/EN), frontend `/profile/change-email` + `profile-info-card` CTA + i18n, `AUDIT_ACTIONS` 에 `user.email_changed` 추가.
