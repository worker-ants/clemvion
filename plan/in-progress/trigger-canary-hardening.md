...
- [x] 1 — 비밀 컬럼 repo-guard — `trigger-secret-columns-{guard.ts,spec.ts}` 신설.
      **착수 시 9건 → 라운드 1~5 가 대조군 3건을 더해 최종 12건** GREEN
      (`/ai-review` `13_04_49` testing INFO#3 — 이 줄이 9 에 멈춰 있었다)
...
- [ ] `/ai-review` + `--impl-done` — **라운드 6 대기**
      ...
      | 5 | `13_04_49` · C0 W1 LOW | `13_04_59` · BLOCK:NO · C0 W1 LOW | **있음** → R6 필요 |
      | 6 | 대기 | 대기 | — |

      ### 라운드 5 — 수를 맞추지 말고 복제를 없앤다

      캐너리 self-spec 헤더가 *"다섯 자리"* 를 **두 번** 적는데 실측은 **6**이고, 정작 형제
      e2e 헤더는 *"여섯 형태"* 라 **두 문서가 다른 수를 주장**하고 있었다.
      `origin/main` 에도 있던 **기존 결함**이다(내가 만들지 않았다) — 그런데 **항목 3 에서 바로
      그 docstring 을 고쳐 쓰면서 두 줄 위의 틀린 수를 못 봤다.** 이 배치 여섯 번째 같은 형태다.

      처분은 «수를 6으로 고친다» 가 아니라 **«수를 여기서 빼고 e2e 파일을 SoT 로 가리킨다»** 다.
      맞추면 다음에 자리가 하나 늘 때 또 갈린다 — 복제를 없애야 그 클래스가 닫힌다.
      (항목 4 의 teardown 서술을 한 곳 정본으로 두고 자매가 가리키게 한 것과 같은 처분이다.)

      함께: 위 항목 1 체크리스트의 «9건» → 12건 갱신(이 배치의 stale 수치 **세 번째** —
      9자리 · 9건 · `delete()` 스코프).

      ### 리뷰어의 «한 줄» 이 한 줄이 아니었고, ratchet 이 그걸 잡았다

      INFO#4(`tmp` 를 `string | undefined` 로 + `afterAll` 가드)를 값싸다고 보고 집어 들었다가
      **타입체크 ratchet 이 `0 → 10` 진단**을 냈다 — 사용처 10곳이 `string` 을 요구한다.
      **jest 는 타입을 strip 해서 못 본다**; `build` 단계의 ratchet 만이 잡았다(기록된
      *"`run-test.sh` 4단계가 타입체크를 안 돈다"* 항목이 2026-09-08 에 `build` 안으로 들어온
      그 검사다).

      | 선택지 | 판정 |
      |---|---|
      | 사용처 10곳 수정 | 얻는 것(이미 실패 중인 환경의 이중 예외 회피)에 비해 과하다 |
      | `let tmp!: string` | «미할당일 수 있다» 를 «확실히 할당된다» 로 단언하는 **타입 거짓말** — 이 저장소의 `nullable-type-lie-cast` 가드가 겨누는 형태다 |

      **철회하고 그 판단을 코드 주석에 남겼다** — 다음 사람이 같은 «개선» 을 다시 집지 않도록.
      리뷰어는 *"신경 쓰인다면"* · *"차단 사유 아님"* 으로 정확히 낮춰 제안했고, **값싸다고
      본 것은 내 판단 착오**였다.

      ### `--impl-done` — carry-forward 하나뿐, 그리고 checker 처방이 하나 틀렸다

      유일한 W 는 **이 PR 이 만들지 않은** `delete()` 오기(등재분)이고, checker 가
      *"이번 PR 의 주석은 오히려 정확한 이름을 인용한다"* 와 *"Flyway 무조치 결정이 근거와 함께
      기록됨"* 을 확인했다.

      **INFO#5 의 처방은 채택하지 않는다.** checker 가 *"인접 plan 의 구값 7/8 을 신값 14/5/9 로
      교체하라"* 고 했는데 **두 값은 다른 양**이다:
      ...
      교체하면 다른 질문의 답으로 덮어쓴다. 대신 «결정 턴에 주어를 명시해 재측정» 하도록
      포인터만 남긴다.
