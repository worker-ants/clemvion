...
- [x] **`#1242` 의 planner draft 가 git 이력에 없다** (`19_26_58` requirement W1 의 자매 건).
      **완료 (2026-08-30)** — 등재해 둔 경로 그대로 번들(`16_50_38/_prompts/cross_spec.md`
...
- [x] **`updateExecutionStatus` self-deadlock 확인의 호출 스택 축** — **완료 (2026-08-30)**.
      `#1243` 은 **어휘적(lexical) 범위**까지만 보고 그 한계를 JSDoc 에 명시해 뒀다. 이번에
      나머지 축을 채웠다: backend 의 `.transaction(` 블록 **36개**(`*.spec.ts` 제외) 전수 —
      엔진 모듈 9개는 콜백 본문이 이 메서드에 도달하지 않고(블록 내 `this.*` 호출은
      `lockNonTerminalExecutionRow` 뿐), 모듈 밖 27개 중 엔진 서비스를 참조하는 파일은
      `executions.service.ts` 하나인데 그 트랜잭션 본문은 `manager` 만 쓴다.
      - **빈 스캔 결과를 그대로 믿지 않았다** — 대부분 블록이 "호출 없음" 으로 나와, 큰
        블록 하나를 직접 열어 `manager.createQueryBuilder()` 만 쓰는 것을 확인했다.
      - **남는 한계는 정적 분석**이다. DI 주입·이벤트 핸들러·큐 consumer 가 런타임에
        트랜잭션을 걸친 경로를 만들면 이 대조는 못 본다 — JSDoc 에 그렇게 적었다.
      - **세 판에 걸친 수치 정정 이력** (JSDoc 에서 이관, `20_46_48` maintainability W5):
        | 판 | 주장 | 무엇이 틀렸나 |
        |---|---|---|
        | `17_36_15` 후 | "호출부 **11곳** 전수 대조" | 그 11은 **이 파일 안만** 센 수. 리뷰어 INFO 를 재보지 않고 옮겼다 |
        | `18_10_28` 후 | "**20곳**, 단 어휘적 범위" | 수는 맞음. 호출 스택 축을 안 봤고 그 사실은 명시했다 |
        | `20_21_06` 후 | "36개 블록 전수, 호출 스택 포함" | 파일 목록을 만들 때 단순 패턴을 써 `webauthn.service.ts` 를 빠뜨려 **27 중 26** 만 봤다 |
        | `20_46_48` (현재) | 위 전부 정정 | 제네릭 포함·주석 제외 패턴으로 36=9+27 확정 |
        **매번 "전수" 라고 쓴 뒤 범위가 더 있었다.** 세는 패턴이 축의 일부다.

      > **이 항목은 `review/**` 의 RESOLUTION 에만 있었고 plan 에 없었다.** `review/` 는
      > SoT 가 아니므로 그대로 뒀으면 사라질 수 있었다. 종결하면서 여기 등재한다.

> **커밋 분리에 대한 판단 기록 (`20_21_06` scope W4)** — 위 두 항목(헤더 누출 발생원 ·
> self-deadlock 호출 스택 축)을 **한 커밋**에 넣었다. 리뷰어 지적대로 주제가 다르다.
> 사용자가 두 건을 함께 요청해 한 PR 로 묶었고, 두 번째는 순수 주석이라 기능 위험이
> 없다고 봤다. **되돌리지 않되 판단을 여기 남긴다** — 같은 상황이면 다음엔 커밋을 주제별로
> 가른다(PR 은 하나여도 커밋은 나눌 수 있다). 이 세션에서 같은 지적을 세 번째 받았다.

- [x] **새 계약이 실제 실행 경로에 붙는지 다음 세션에서 확인** — **확인 완료 (2026-08-31)**
      (`20_46_48` side_effect W4).
      이 PR 이 계약을 고쳤는데 **같은 세션의 두 리뷰 라운드는 구버전 문구로 돌았다.**
      - **실측**: persisted 워크플로 스크립트(`ai-review-wf_*.js`)를 저장소 파일과 diff 하니
        **내 편집만큼만** 다르다(325 vs 333줄, 블록 49→57). 즉 `Workflow({name:"ai-review"})`
        는 `.claude/workflows/ai-review.js` 를 **읽는 게 맞고**, 이 세션이 시작 시점
        스냅샷으로 해석해 편집이 반영되지 않았다.
      - 리뷰어는 "저장소 밖 caller wrapper 문제" 로 귀속했는데 **그건 아니다** — 고쳐야 할
        파일은 이미 고쳐졌다. 다만 **이 세션에서는 검증이 불가능**하다.
      - **확인 방법**: **새로운 top-level Claude Code 세션**(현재 세션 안의 새 리뷰 라운드가
        아니다)에서 `/ai-review` 를 한 번 돌린 뒤
        `grep -c "마크다운 본문만" <persisted script>` 가 1이면 반영된 것. 0이면 스냅샷이
        아니라 다른 해석 경로가 있다는 뜻이므로 그때 재조사.
      - ⚠️ **이 세션 안에서는 이 항목이 원리적으로 닫히지 않는다.** `21_12_21` side_effect 가
        실측했다: 이 세션의 persisted `ai-review-wf_*.js` **18개 전부가 17300 바이트로 불변**
        — 세션 시작 시점에 캐시된 뒤 이 브랜치의 커밋 편집을 한 번도 반영하지 않았다.
        라운드를 더 돌려도 같은 스냅샷을 볼 뿐이므로, 리뷰 라운드를 근거로 이 체크박스를
        닫지 말 것.
      - 그때까지는 **"발생원을 고쳤다" 가 아니라 "고친 자리가 실행 경로임을 확인했고 반영은
        미검증"** 이 정확한 서술이다.
      - **확인 (2026-08-31, 새 top-level 세션)** — `grep -c "마크다운 본문만"` = **1**. **통과**.
        | 항목 | 값 |
        |---|---|
        | 세션 | `b295a722-a64c-4bb2-a493-05c0d54b9ba0` (worktree `retry-turn-terminal-guard-review-7b7629`) |
        | 실행 | `/ai-review` 1회 — `REVIEW_AGENTS=security … --range HEAD~1..HEAD`, run `wf_656032a8-0dd` |
        | persisted 스크립트 | `ai-review-wf_656032a8-0dd.js` — **18016 B**, mtime `2026-08-31 12:41:45` |
        | `grep -c "마크다운 본문만"` | **1** |
        | 저장소 파일과 대조 | `diff` **0줄**, md5 `bcff0e5291f403fb4d6890f421dcf862` 양쪽 동일 |
      - **대조군이 결론을 고정한다** — 직전 세션(`b8e8a7f1…`)의 persisted 스크립트는
        **20개 전부 17300 B, 전부 grep 0**(위 ⚠️ 의 "18개" 이후 2회가 더 돌아 최종 20개 —
        수치가 갱신된 것이지 어긋난 게 아니다). 같은 명령이 새 세션에서 **18016 B / grep 1**
        을 낸다. 위 스냅샷 가설대로다.
      - **"이번 실행이 만든 파일" 이라는 근거는 mtime 이 아니다** — 파일명이 `Workflow` 반환의
        **Run ID(`wf_656032a8-0dd`)를 담고 있다.** 옛 세션 파일을 셀 여지가 구조적으로 없다.
      - **리뷰어 수·리뷰 대상은 이 확인과 무관하다** — persisted 되는 것은 `args` 와 무관하게
        `ai-review.js` **전문**이고, md5 가 저장소 파일과 같다는 것이 그 증거다. 그래서
        reviewer 1명·소규모 diff 로 돌려도 판정력이 줄지 않는다.
      - **grep 보다 강한 증거가 같은 실행에서 나왔다 — 산출물의 실제 형태.** 이 실행이 남긴
        `review/code/2026/08/31/12_41_13/` 의 `security.md`·`SUMMARY.md` 는 **둘 다 `#` 제목으로
        시작**하고, `^STATUS=` **0건** / `===REPORT_MARKDOWN_BELOW===` **0건**이다. 종전 계약이
        만들던 "STATUS= 로 시작하고 제목이 2~3행부터" 형태(기존 산출물 536개)가 재현되지
        않는다. 계약 문구가 프롬프트에 실렸다는 것을 넘어 **의도한 동작까지** 확인됐다.
      - 따라서 **위** 서술을 갱신한다: ~~"고친 자리가 실행 경로임을 확인했고 반영은 미검증"~~
        → **반영까지 확인됨**.
      - ⚠️ **확인된 범위는 여기까지다.** `Workflow({name:…})` 가 **새 세션에서** 저장소의
        현재 파일을 싣는다는 것만 실측했다. **세션 중 편집이 그 세션에 반영되는지는 여전히
        미검증이며, 직전 세션의 실측(20/20 불변)은 "반영되지 않는다" 쪽이다.** 하네스
        워크플로 스크립트를 고치는 편집은 그 세션의 리뷰 라운드로 검증하지 말 것.
- [x] **planner 턴 draft 를 커밋하는 것을 절차로 못박는다** (위 두 건의 발생원). **완료 (2026-08-30)**.
      ...
      - **후보 (a)를 골랐다.** `consistency_orchestrator.py` 가 `--spec` 세션에 target draft
        원본을 `_target/` 로 복사하고 `meta.json` 의 `target_snapshot` 이 그걸 가리킨다.
      - **(b)를 고르지 않은 이유**: 새 push 차단 조건은 표면이 무한하다. 이 저장소는 push
        가드를 정밀화했다가 **3라운드 회귀 끝에 철회**한 이력이 있다(`#970` — "유한한 문제를
        무한한 문제와 바꾸지 말 것"). (a)는 차단 표면이 0이면서 **잃었던 것(증거)을 정확히**
        되살린다. draft 를 `plan/complete/` 로 옮기는 관례는 사람이 지킨다.
      - **왜 프롬프트 번들로 충분하지 않은가**: 복원 두 번 다 `_prompts/` 코드펜스에서 떴는데
        그건 **부수 효과**다 — target 은 `budget_substitutions` → `truncate_file_bundle` 로
        예산에 따라 **잘리고**, 포맷이 바뀌면 사라진다. 우연을 계약으로 바꿨다.
      - 회귀 테스트 3건(`test_consistency_spec_draft_snapshot.py`) — 실제 CLI 를 subprocess 로
        구동해 (i) 바이트 동일 보존 (ii) `meta.json` 이 실재 파일을 가리킴 (iii) **`--plan` 은
        사본을 만들지 않음**(음성)을 고정. 뮤테이션: 보존 제거 → **RED 2**, 모드 검사 제거 →
        **RED 1**(음성만). 두 축이 각각 물린다.
      - `consistency-checker/SKILL.md` 의 `--spec` 절에도 반영했다.
...
- [x] **harness: sub-agent 프로토콜 헤더가 리뷰 산출물 본문에 새어 든다** (`18_10_28`
      documentation W1). checker/reviewer 가 `output_file` 에 쓸 때 `STATUS=…` 줄과
      `===REPORT_MARKDOWN_BELOW===` 구분자가 그대로 남아, 보고서가 `#` 제목이 아니라
      프로토콜 잡음으로 시작한다.
      - **완료 (2026-08-30)** — 발생원을 고쳤다.
      - **⚠️ 종전에 적어 둔 "825개" 는 틀린 수였다.** `grep -rl "^STATUS="` 는 파일
        **어디든** 그 줄이 있으면 세는데, 리포트 본문이 `STATUS=success` 를 인용하는
        경우가 많다. 1행이 `STATUS=` 인 파일로 다시 세면 **536개**이고, 그중 2행이
        구분자인 것은 **271개**(나머지 265는 구분자 도입 이전 형태)다. 느슨한 grep 결과를
        개수로 보고했고 그 값이 `#1243`·`#1244` 에 그대로 실렸다.
      - **원인**: `_lib/agent-return.mjs` 의 `REPORT_RETURN_CONTRACT` 가 1) 에 "결과를
        output_file 에 Write" 라고만 적고, 2·3) 의 STATUS 헤더·구분자가 **파일용인지 반환
        메시지용인지 말하지 않았다.** 그래서 agent 가 반환할 것을 그대로 파일에도 썼다.
      - **처방**: 계약에서 두 sink 를 갈랐다 — 파일은 마크다운 본문만(헤더·구분자 금지,
        `#` 제목으로 시작), 2·3항은 **반환 메시지**에만 적용. 정본을 고치고 3개 워크플로에
        verbatim 미러링했다(드리프트 가드 `test_workflow_scripts.py` 5 passed / 9 subtests).
      - **뮤테이션이 회귀 테스트 부재를 드러냈다**: 계약을 초판 문구로 되돌려도 기존
        11개가 전부 GREEN 이었다. 두 축(파일=본문만 / 2·3항=반환 메시지)을 고정하는
        테스트를 추가하니 같은 뮤턴트가 **RED 2**(신규 둘만, 기존 11 GREEN).
      - **기존 536개는 고치지 않았다** — 발생원을 막았으므로 새로 늘지 않는다. 과거
        산출물의 일괄 수정은 리뷰 이력을 사후 변조하는 셈이라 하지 않는다.
