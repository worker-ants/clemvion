...
- [x] **`removeMember()` 의 owner 보호 가드가 TOCTOU 로 뚫린다 — 실측 확인됨**
      ...
      **왜 이번 PR 에서 함께 닫지 않았나**: 두 결함의 계약이 다르다 — 이번 PR 은 «감사를 두 번
      ...
      > **✅ 2026-09-24 해소** — developer 턴 `plan/complete/member-owner-toctou.md`
      > (`--impl-prep` `review/consistency/2026/09/24/07_29_15` BLOCK: NO ·
      > `/ai-review` `09_10_41` 3라운드 수렴 · `--impl-done` `09_24_11` **BLOCK: NO · Critical 0**).
      >
      > 처방은 **위 후보 그대로가 아니다.** 후보는 0-행 재조회에서 «지금 role 이 owner 인가» 를
      > 물었는데, 리뷰(`08_09_57` W3)가 **제3 상태**를 짚었다 — 이양 연쇄로 그 사이 강등되면
      > 실재하는 멤버를 404 로 보고한다. DELETE 의 술어는 `role` 하나뿐이라 **행이 남아 있는데
      > 0행이었다면 그 시점에 owner 였다는 뜻** 말고는 없으므로, 재조회는 **존재 여부만** 본다.
      > 후보를 그대로 적용했다면 그 결함이 들어갔을 자리다.
      >
      > 재현은 레이스가 아니라 **재진입**으로 했다(둘 다 같은 행 락을 기다려 큐 순서에 달린다).
      > 고치기 전 **200**(멤버 행 삭제됨) → 고친 뒤 **403 `CANNOT_REMOVE_OWNER`**(행 유지).
      > 락을 새로 들이지 않은 근거(Postgres EvalPlanQual 재평가)는 e2e 가 직접 행사한다.
      >
      > **부수로 이 항목의 이웃 하나를 바로잡았다** — 아래 «권한 검사 순서 오라클» 항목은
      > 블라스트 반경을 **정확히** 적고 있었는데(«멤버가 아니어도» · 13/17 라우트), 내가 PR 의
...
- [x] **CHANGELOG 에 무엇이 들어가는지 성문 기준이 없다** (developer, 낮음, 2026-09-24 등재 ·
      ...
      **판정 기준을 적기 전에 기존 항목을 전수로 분류할 것** — 이번처럼 헤딩만 훑으면 또 틀린다.
      > **2026-09-25 종결** — plan `changelog-criteria`. `CHANGELOG.md` 상단에 «무엇이 항목을 만드는가» 블록(낸다: 제품 동작 ·
      > 배포 · 개발 흐름 / 안 낸다 / 이전 이력은 불완전). **위 «실제 관행» 문단도 관찰로서는 틀렸다** — 전수로 쟀더니 가드 강화
      > `#1245` · `#1261` · `#1262` · `#1263` 이 항목 없이 머지됐고, 제품 코드 PR 의 동반율은 월별 0% · 5% · 37% · 30% · 49% 였다.
      > 그래서 기준은 관행의 요약이 아니라 **처방**이다. `perf` 인덱스 마이그레이션 V110~V130(21개)은 기준 2 에 정확히 해당해
      > 한 항목으로 백필했다. documentation 리뷰어 관점 6(SSOT `role_instructions.py` + `.md` 두 곳)이 이 블록을 가리킨다.

- [x] **CHANGELOG 미동반 PR 12건을 새 기준으로 재판정해 백필한다** (developer, 낮음, 2026-09-25 등재 · plan
      `changelog-criteria` §B). 2026-08-15 이후 `feat` · `fix` · `perf` · `refactor` 로 제품 코드를 바꾸고 CHANGELOG 를 안 고친
      PR 중, 나중에 백필되지도 않은 것(`#N` 부재 실측)에서 동작 · 배포 · 가드 변경일 **수 있는** 후보. 하나씩 기준으로 판정할 것 —
      예: `#1354` · `#1358` 은 엔티티 선언만 고쳤는지 DB 가 바뀌었는지부터.
      - 동작: `#1364`(SSRF 가드 소비자가 «판정» 과 «고장» 을 가른다) · `#1206`(`inputOverride` 를 열린 map 으로 광고 — API 문서
        표면) · `#1270`(ws 이월 INFO)
      - 엔티티 선언: `#1354`(인덱스 · 제약 여덟 곳) · `#1358`(컬럼 아홉 곳)
      - 가드: `#1261`(lint 글롭이 18개 파일을 건너뛰던 것) · `#1262` · `#1263` · `#1245`(하네스) · `#1238`(default-export 캐너리) ·
        `#1275`(`refactor` 지만 제목이 «낡은 spec 캐스트 가드» 신설을 말한다)
      - `refactor` 인데 동작일 수 있는 것: `#1326`(«라벨 스왑을 잡는다»)
      나머지 `refactor` 11건은 제목이 추출 · 통합 · 주석 정리만 말해 넣지 않았다(동작 불변이 기본값). 그러니 후보는 12건이다.
      > **2026-09-25 종결** — plan `changelog-backfill-12`. **10 항목 · 2 비항목**, 주제별 6개 항목으로 백필했다(OpenAPI 계약
      > `#1206` · `#1326` / WS 타이머 셧다운 `#1270` / 엔티티 선언↔DB 가드 `#1354` · `#1358` / 가드 사각 넷 `#1261` · `#1262` ·
      > `#1263` / 낡은 spec 캐스트 가드 `#1275` / 리뷰 산출물 헤더 `#1245`). 비항목: `#1238`(한 모듈 캐너리 · 내부 개명),
      > **`#1364`** — 처음엔 항목으로 판정했다가 diff 대조로 뒤집었다: «연결 테스트 타임아웃 예산» 변화는 그 PR 안에서 생겼다
      > 사라진 회귀였고(main 에서도 가드가 신호 생성보다 먼저 돌았다), 나머지는 오늘 도달 불가 경로다. 그 교훈을 기준 ① 에
      > «판정은 main 대비» 로 넣었고, «OpenAPI 로 광고하는 계약» 도 보탰다(`#1206` · `#1326` 이 문구 밖이었다).
...
- [x] **`ModelConfigService.remove()` 도 동시 삭제에서 감사 행을 두 번 남긴다 — 여덟 번째**
      ...
      **등재 시 적은 «캐시 무효화 통지 중복까지» 는 과장이었고** 그 PR 이 정정했다(아래 정정 블록).
      ...
      > 리스너는 `llm.service.ts:81-82` 의 `clearClientCache(configId)` 하나뿐이고 캐시 축출은
      > **멱등**이라, 두 번 불려도 해로운 결과가 없다. 중복 통지는 **고쳐야 할 별개 결함이
      > 아니라** 이 수정이 진 쪽에서 함께 건너뛰게 되는 부수 효과다. 등재 시점에 리스너를
      > 따라가 보지 않고 «중복이면 나쁘다» 로 적었다.

- [x] **WebAuthn credential 삭제도 동시 요청에서 `user.2fa_disabled` 감사를 두 번 남긴다 — 아홉 번째**
      **2026-09-21 해소** (`plan/complete/webauthn-dup-delete.md`). 이미 `.delete()` 를 쓰고
      있었으므로 바꾼 것은 호출 형태가 아니라 **버려지던 `affected` 를 판정에 쓴 것**이다.
      e2e 로 먼저 재현했다 — 고치기 전 `[204, 204]` · 한 `credentialId` 에 감사 **2건**
      (둘 다 `remainingCredentials: 1`), 고친 뒤 `[204, 404]` · **1건**.
      부수로 DELETE 조건절에 `userId` 를 더했다(종전엔 소유권이 무락 조회 뒤 JS 비교로만 남았다).

      > ## 🏁 이 결함 클래스가 아홉 자리로 **종료**됐다
      >
      > «리소스 행을 지우고 감사를 남기는 요청» 아홉 자리가 전부 닫혔다:
      > #1369 워크플로·워크스페이스 · #1370 트리거 · #1371 스케줄 · #1372 통합 ·
      > #1373 멤버 · #1374 인증 설정 · #1375 모델 설정 · 이 PR WebAuthn.
      >
      > **열거 축을 세 번 틀렸다.** 처음엔 `AUTH_CONFIG_*_DELETED` 접미사로 셌고(→ `_REMOVED`
      > 누락), 다음엔 `remove(entity)` 호출로 셌고(→ 이미 `.delete()` 를 쓰던 자리 누락),
      > 그 다음엔 서비스 안의 감사 호출로 셌다(→ 감사가 **컨트롤러**에 있는 자리 누락).
      > **마지막 자리가 그 셋 모두에 안 걸리는 자리였다** — 축을
      > «지우고 감사한다» 는 **요청 단위 서술**로 바꾼 뒤에야 전수가 나왔다.
      >
      > 처방은 자리마다 달랐다: 부모 행 락 + 재조회 · advisory lock + 재조회 ·
      > CASCADE 때문에 **트리거** `affected` 가 판별자 · 락 없는 원자적 `DELETE` · 이미 있던
      > `affected` 를 쓰기. «같은 클래스 = 같은 처방» 이 아니었다.
...
- [x] **동시성 e2e 아홉 파일의 공용 헬퍼를 추출한다 — 테스트 전용 PR**
      **2026-09-21 해소** (`plan/complete/e2e-race-helper.md`). `codebase/backend/test/helpers/concurrency.ts`
      의 `raceUnderHeldLock()` 으로 **아홉 파일 11 블록** 전환. 프로덕션 코드 변경 0,
      단언 변경 0(사라진 것은 헬퍼로 옮겨간 `expect(raced)` 11건뿐).
      **등재 시 설계에서 둘을 실측으로 고쳤다**: `fire()` 단일 콜백 → **배열**(webauthn 둘째
      블록이 서로 다른 thunk 를 발사한다), 그리고 `integration-rotate` 는 **제외**(요청 하나 +
      락 안 UPDATE 인 **갱신** 경합이라 축이 다르다 — grep 공통 토큰만 보고 «포함» 이라
      결론졌다가 본문을 읽고 뒤집었다).
      **음성 대조군으로 가드가 살아 있음을 실측**했다 — 헬퍼의 락 쿼리를 제거하면
...
- [x] **`raceUnderHeldLock` 의 순수 동기 분기 둘이 어떤 테스트도 지나가지 않는다**
      ...
      **둘 다 실제 Postgres 타이밍과 무관한 순수 동기 분기**라 DB 없는 Jest unit
      (`codebase/backend/test/helpers/concurrency.spec.ts` 류)으로 행사할 수 있다.
      **특히 후자는 「주석 대신 코드로 고정」하려고 막 승격시킨 방어인데 그 방어 자체가
      미검증**이라는 점이 이 항목의 요지다.
