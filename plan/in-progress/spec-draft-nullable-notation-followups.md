...
- [ ] **`V110` 헤더의 "정상 흐름에서는 발생하지 않는다" 서술** (developer/문서, 2026-09-05 등재).
      `review/code/2026/09/05/10_20_57` W1 이 그 주장의 폭을 지적했다 — CREATE 성공 후
      DROP(old) 실패면 이 저장소가 지시하는 `repair` + 재실행 절차에서 재빌드가 난다.
      `migrations/README.md` §5 는 이번에 정정했지만 **V110 파일 자신은 append-only** 라
      건드리지 않았다. 처리 방법은 둘 중 하나다 —
      - (a) 그대로 둔다. 마이그레이션 헤더는 **적용 시점의 기록**이고 규약은 README 가 SoT 다
      - (b) `migrations.md` 나 README 에 "V110 헤더의 그 문장은 이후 정정됐다" 한 줄을 남긴다

      (a) 가 기본이지만, 이 파일이 규약의 **선례로 지목**되고 있어 읽는 사람이 헤더를
      규약으로 오인할 여지가 있다. 그래서 등재만 해 둔다.
...
- [ ] **해소 불가 bare 인용 8건 채우기** (developer, 2026-09-05 등재).
      ...
      그래서 §3 의 "소급 정리 안 함" 과 별개로 이 8건만 따로 둔다.
