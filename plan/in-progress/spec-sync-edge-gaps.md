...
- [x] §3.2 실행 상태 스타일 — 구현 완료. 신규 `use-edge-execution-state.ts` `useEdgeExecutionState` 훅이 실행 스토어(`status`/`nodeStatuses`)+노드 `isDisabled` 로 각 엣지에 상태 스타일을 입힌다(순수 판정 `edge-utils.ts` `resolveEdgeExecutionState`, 우선순위 inactive>flowing/completed). **데이터 흐름**(실행 중 source completed+target running)=`className edge-flowing`→globals.css 마칭 점선(`edge-flow` keyframe 재사용), **실행 완료**(둘 다 completed)=`className edge-completed`→`edge-complete-flash` 1회성 초록 flash 후 포트색 복귀, **비활성**(source/target `isDisabled`)=`data.edgeInactive`→`custom-edge.tsx` 반투명 점선. `useEdgeHighlighting`(§3.3) 앞단에서 합성(className Set 병합). **성능**: sibling 훅과 동일 per-edge bail-out(상태 불변 엣지=원본 참조 유지) + 안정 disabledKey(정렬 join)로 실행 tick·드래그 시 전체 엣지 재생성 방지. 스타일 조립은 순수 함수 `edge-utils.ts` `buildEdgeStyle`(selected/highlighted/inactive + baseStyle 우선)로 분리. 테스트: `resolveEdgeExecutionState` 9 + `buildEdgeStyle` 5 + `useEdgeExecutionState` renderHook 9(rerender 참조 안정 포함). ai-review 3라운드(`14_20_12` MEDIUM→`14_42_20` LOW→`15_01_46` 수렴): 성능(전 엣지 재생성 → per-edge bail-out+안정 disabledKey)·훅/스타일 테스트(드래그 참조안정·재활성화 토글 포함)·CSS 접두 `wc-`→`edge-`·주석 정정·run-and-debug 가이드 동반 갱신 반영.
- [x] §4 / §5 엣지 호버 데이터 미리보기 툴팁 (Data Flow Preview) + 축약 표시 + 전체 데이터 모달 — 구현 완료. 신규 `edge-data-preview.tsx`(`EdgeDataPreviewTooltip`/`EdgeDataModal`) + `use-edge-hover-preview.ts` 훅(표시/숨김 200ms 지연으로 툴팁 클릭 가능) + 순수 util `lib/utils/edge-data-preview.ts`(`summarizeDataForPreview`=중첩 배열 `[N items]`·객체 `{N fields}`·긴 문자열·최상위 배열 앞 5개 축약 + 원본 바이트, `formatBytes`). `workflow-canvas.tsx` `onEdgeMouseEnter` 가 커서 위치에 툴팁 예약 → 툴팁이 엣지 source 노드 최근 실행 출력(`findNodeResult`→`unwrapNodeOutput().output`)을 축약 표시(데이터 없으면 렌더 안 함), "전체 데이터 보기" 클릭 시 `EdgeDataModal`(Dialog, hover 독립 `dataModalEdgeId`)이 전체 JSON. 테스트: `summarizeDataForPreview`/`formatBytes` vitest 10케이스.
