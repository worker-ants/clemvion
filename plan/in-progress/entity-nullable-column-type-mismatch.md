...
- [x] **후속 — 넓혀진 필드를 겨눈 낡은 `.spec.ts` 캐스트 가드** — **완료.**
      ...
      > non-null 쪽의 **정당한** 캐스트를 오탐하기 때문이다(최소 픽스처로 재현). 판정 대상
      > 판정 대상이 그만큼 줄어든다(**2026-09-04 실측 135 → 115**). 재현율을 잃는 대신
      > 오탐을 0으로 유지한다 — 처방이 "지워라" 라서
...
넷 다 같은 형태다 — **내가 알아챈 자리만 고치고 그 클래스를 훑지 않았다.** 3번 뒤에
