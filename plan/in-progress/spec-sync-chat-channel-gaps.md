...
- [ ] CCH-NF-03 — `rateLimitPerMinute` 분당 enforcement + 초과분 skip + `chat_channel_health=degraded` 갱신 (현재 config 저장 슬롯만 존재). **메커니즘 확정** (2026-06-12, spec CCH-NF-03/R-CC-19): per-chat Redis fixed-window(기본 60, 초과분 버퍼링 없이 skip → 202 ignored + degraded, Redis 미가용 fail-open). "chat 단위 큐 적재→재발사" 는 WH-NF-01·R9 정합상 미채택. **구현 대기 — 별 PR** (spec 확정 ↔ 구현 분리; 본 spec PR 후속).
