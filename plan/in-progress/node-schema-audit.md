...
| 항목 | 상태 | 위치 |
|------|------|------|
| F-1.1 categoryDefSchema.id 부재 | STILL_VALID | text-classifier.schema.ts:8-17 |
| F-1.2 name/description default 부재 | STILL_VALID | text-classifier.schema.ts:9-12 |
| F-1.3 handler `class_${i}` 하드코딩 2곳 | STILL_VALID | text-classifier.handler.ts:314, 392 |
| F-1.4 resolver fallback 미구현 | STILL_VALID | resolve-dynamic-ports.ts:87 |
| F-1.5 테스트 하드코딩 7곳 | STILL_VALID | text-classifier.handler.spec.ts (L118,135,195,476,507,560,634) |
| F-1.6 system-prompt 잘못된 그룹핑 | STILL_VALID + 버그 확인 | system-prompt.ts:232 |
| F-1.7 information_extractor 의 conditions 부재는 의도된 설계 | 신규 (의도 확인) | information-extractor.schema.ts + resolve-dynamic-ports.ts:109-143 |
| F-2.1 4 노드 buttons[*].id optional | STILL_VALID | carousel/chart/table/template 동일 buttonDefSchema |
| F-2.2 fallback 패턴 3종 | STILL_VALID | resolve-dynamic-ports.ts:230,245,257 |
| F-2.3 shadow auto-generate 미구현 | STILL_VALID | shadow-workflow.ts (addNode/updateNode) |
| F-2.5 spec §8 buttons[*].id 행 부재 | STILL_VALID | spec/3-workflow-editor/4-ai-assistant.md §8 |
| F-3 form.optionSchema.value default 부재 | STILL_VALID | form.schema.ts:7-12 |
| F-4 keyValueSchema.passthrough 부재 | STILL_VALID | http-request.schema.ts:7-10 |
...
2. resolver fallback: `c.id?.trim() || \`class_${i}\``
3. system-prompt: text_classifier 를 `config.categories[*].id` 별도 라인으로 분리, information_extractor 는 그룹에서 제거
4. 핸들러 라우팅: category → port-id 매핑 테이블 도입

### F-2
1. (a) shadow auto-generate slug 채택
2. slug 생성 규칙: **UUID** (단 우려 §1 — 재확인 필요)
3. spec §8 개정 필요 — `project-planner` 경유 (단순 개정이 아닌 buttons[*].id 행 신설)
...
| Step | 항목 | 우선 | spec/외부 의존 |
|------|------|------|----------------|
| 1 | F-1 — text-classifier stable id (system-prompt 수정 포함) | HIGH | 없음 |
| 2 | F-3 + F-4 — 1줄 보강 묶음 | LOW | 없음 |
| 3 | F-2 — buttons 정책 (마이그레이션 + shadow + spec 신설) | MEDIUM | spec §8 신설 선행 (project-planner) |
