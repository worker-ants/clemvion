...
`엔드포인트도 403 을 낼 수 있다.` 뒤의 문장(«`@Roles()` 가 있으면 … 통일한다 — 코드는 data-flow 링크.»)을 아래로 바꾼다. 앞 문장과
...
- **기각한 대안 — `@Roles()` 를 읽어 설명을 자동으로 만드는 데코레이터**(이 draft 를 쓰며 검토했다): 데코레이터는 아래에서 위로 적용돼 `@ApiForbiddenResponse`
  ...
  택했다.
- **`--spec` · `--impl-prep` 경고 처리**: `--spec` `review/consistency/2026/09/26/11_12_13` BLOCK: NO(Warning 3) — W1(모집단 하한) ·
  W2(대조군 fixture)는 가드 spec 에 이미 있는데 이 draft 가 적지 않았다(위 Rationale 두 불릿), W3(트래커 repo-guard 등재 census)는
