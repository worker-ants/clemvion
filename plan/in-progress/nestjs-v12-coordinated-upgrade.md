...
> **2026-09-25 기준값 갱신 — 위 표의 수치는 이제 비교 기준이 아니다.** `workspace-path-guard`
> PR 이 `@WorkspaceParam` 을 더해 가드가 두 번째 reflection 판별(`workspaceParamNamesOf`)을
> 갖게 됐고, 캐너리 로그 · 3스위트 테스트 수가 바뀌었다. 같은 방법으로 다시 쟀다(base = 그 PR
> 브랜치, 머지 전):
>
> | 기준값 | 값(실측) | 어떻게 쟀나 |
> | --- | --- | --- |
> | 부트 캐너리 로그 | **`@WorkspaceId()` 142건 · `@WorkspaceParam()` 15건** | e2e backend 컨테이너 로그의 `[WorkspaceIdReflection]` 줄 — 앞 수는 위 142 와 같다 |
> | reflection 3스위트 | **89 통과 / 89** | 같은 세 스위트(가드에 경로 분기 · 거부 코드 테스트가 늘었다) |
> | MB `handlerConsumesWorkspaceId` → 항상 false | **RED 11** | 위 9 + 그 PR 이 더한 헤더 위조 · 병용 핸들러 케이스 |
> | MB2 `workspaceParamNamesOf` → 항상 `[]` | **RED 25** | 새 판별자 — 경로 라우트의 fail-open 시뮬레이션 |
>
> 업그레이드 뒤에는 **이 표**와 같은 값이어야 한다. 그 PR 이 머지된 뒤 main 에서 다시 재면
> 다른 PR 의 테스트 증감이 섞일 수 있으니, 착수 시점에 한 번 더 재서 «전» 칸을 채울 것.
...
- [ ] §3 재개 조건 셋이 모두 풀린다
- [ ] **`spec/5-system/1-auth.md` §Rationale «부트 캐너리» 의 `^11.0.1` 캐럿 인용 정정** —
      `@nestjs/common` 을 12 로 올리는 순간 그 문장이 거짓이 된다. `spec/` 쓰기라
      **planner 턴**이다(`--spec` 게이트). 숫자를 새 값으로 바꾸기보다 **숫자를 빼고
      «caret 범위라» 로 일반화**하는 편이 낫다 — 같은 Rationale 인접 문단이 이미 «캐너리 수치를
      spec 에 미러링하지 말 것, stale 해진다» 고 경고했고 버전 숫자에도 같은 원칙이 적용된다.
      (`--impl-prep` `review/consistency/2026/09/24/17_31_27` W3 · `/ai-review`
      `review/code/2026/09/24/18_48_20` W1 — 이 항목이 **여기 없어서** 두 번 지적됐다)
...
- [ ] §C 검증 셋 기록. **기준값은 이미 재 뒀다**(§C 하위 «업그레이드 전 기준값») — ~~캐너리
      ...
      (2026-09-25 정정: `workspace-path-guard` PR 이 판별자를 하나 더해 기준값이 바뀌었다 —
      캐너리 142 · 15, 3스위트 89통과, MB RED 11 · MB2 RED 25. 표는 §C 하위 «2026-09-25 기준값 갱신»)
