...
> manager 를 탄다*는 것까지다(원자성 자체가 아니라 그 전제). 실 DB 부분 커밋 검증은 자매
> plan 의 실 DB e2e 트랙과 같은 성격이라 그쪽에 묶인다.
...
이 함수의 다른 열린 항목(관용구 헬퍼 추출 · 단일 emit 관문 · 실 DB e2e)은 정본 트래커에
등재돼 있고 이번 PR 에서 건드리지 않는다 — **넓은 일괄 편집이 대상 밖 8곳을 조용히 바꾼
전례**가 이 계열에 있다.
