...
대상 파일: `backend/src/nodes/logic/switch/{switch.handler.ts, switch.schema.ts, switch.handler.spec.ts, switch.schema.spec.ts, switch.component.ts}`.
...
- [ ] (impl) `switch.handler.ts:86-90` 의 `configEcho` 에 `hasDefault: rawConfig.hasDefault` 와 `strictComparison: rawConfig.strictComparison` 추가 — spec §5.2 JSON 예시 (`hasDefault: true`) 와 정합 회복. 근거: spec `spec/4-nodes/1-logic/2-switch.md:165` + Principle 7 "사용자 UI 설정 비민감 값 항상 echo".
