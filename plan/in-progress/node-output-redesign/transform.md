...
> **6차 갱신 (2026-06-25 코드 재검증)**: 핸들러 분할 없음 — `transform.handler.ts` 단일 파일 유지. 미해결 §8 2건 모두 **잔여(STILL PENDING)**: ① `rawConfig.operations` echo 단위 테스트는 여전히 부재(`transform.handler.spec.ts` 에 `rawConfig` 참조 0건), ② spec §5.1 footnote 재구조화 시멘틱 보강도 미반영(`spec/4-nodes/5-data/1-transform.md:174` 은 여전히 "(Principle 8 예외 — 변형 결과를 root 에 둠)" 한 줄). **새 갭 없음**. **stale ref 정정**: ReDoS 방어가 PR #570(M-3)로 변경 — `MAX_REGEX_LENGTH = 200` 상수 제거, `safeCompileRegex`(`:38-40`)가 단일 chokepoint `compileUserRegex`(길이+safe-regex 위험패턴+문법)로 위임. 그 외 핸들러 return(`:154-161`)·validate(`:113-125`)·rawConfig fallback(`:151-155`)·structuredClone(`:133`) 등 라인 시프트 정정. schema.ts 라인 인용은 변동 없음.
...
`spec/4-nodes/5-data/1-transform.md:149-169` — §5.1 정상 (단일 출력):
...
1. **spec §5 ↔ handler return 정합성**:
   - `transform.handler.ts:154-161` 의 return 객체 키 `{ config, output, meta }` 가 spec §5.1 과 일치. `port` / `status` 미지정 (단일 출력 비-블로킹 — Principle 5 부합).
   - `config: { operations: rawConfig.operations ?? operations }` (`:155`) — Principle 7 raw echo. `set_field.value` 의 `{{ }}` 보존 의도가 명확.
   - `output: data` (`:156`) — 변형된 input 객체 자체. Principle 8 예외 (root 직접 노출) 와 일치.
   - `meta: { operationsApplied, operationsSkipped }` (`:157-160`) — spec §5.1 표 두 필드 일치. `meta.durationMs` 는 엔진이 inject 하므로 handler return 에 없음 (Principle 2 공통).
...
3. **validate 일관성**:
   - `transform.handler.ts:113-125` 의 `handler.validate()` 는 `evaluateMetadataBlockingErrors(metadata, config)` (warningRules + `validateConfig` SSOT) + "operations 비-배열" guard 만 추가. SSOT 침범 없음.
...
4. **에러 컨트랙트 (Principle 3)**:
   ...
   - Runtime 무결성 실패 (필드 부재, 타입 mismatch, JSON parse 실패, dayjs invalid, divide-by-zero, safe-regex 거부) 는 모두 silent no-op 으로 처리되고 `meta.operationsSkipped` 카운트. spec §4 의 "no-op" 명시와 핸들러 분기 (`transform.handler.ts:200, 211, 231, 245-257, 292, 362, 382-385, 409-413, 468, 483, 515-516, 543`) 모두 일치. → (2026-06-25) PR #570(M-3) 이후 "regex 길이 초과"는 더 넓은 "safe-regex 거부(길이+위험패턴+문법)"로 일반화 — `safeCompileRegex` 가 `null` 반환 시 해당 op skip.
   - Prototype pollution 방어: `BLOCKED_OBJECT_KEYS` (`transform.handler.ts:43`) 가 `__proto__` / `constructor` / `prototype` 을 `object_omit` 에서 차단. `set_field` 경로 차단은 `setNestedValue` (nested-value.util) 위임 — handler 가 `applied: true` 로 카운트 (`:221-224` 의 주석 명시: "사용자가 의도한 변형 시도이고, no-op 분류는 필드/타입 부재 케이스에 한정").
...
5. **conventions Principle 0–11 위반 패턴**:
   ...
   - Principle 7: `context.rawConfig?.operations ?? operations` fallback 패턴 (`:151-155`) 으로 raw echo. evaluated `operations` 가 다르더라도 raw 가 echo 됨. 부합.
...
6. **handler 테스트 (`transform.handler.spec.ts`)**:
   - validate 4건 (`:22-50`), 11개 op 각각 정상/엣지 케이스 (`:52-846`, 기본 + extensions 블록 포함), 보안·prototype pollution (`:772-846`), `meta.operationsApplied/operationsSkipped` 5건 (`:848-977`), 통합 chain (`:979-1013`). 커버리지 매우 높음.
   - **미세 누락 (잔여)**: `context.rawConfig` 가 설정된 케이스의 `config` echo 검증 부재 — handler 가 항상 `config.operations` 를 echo 하므로 `rawConfig.operations` 와 `config.operations` 가 다를 때 (예: `{{ }}` 평가 전후) raw 가 우선되는지 직접 확인하는 테스트가 없음. → (2026-06-25 재검증) 여전히 미보강 — `transform.handler.spec.ts` 전체에 `rawConfig` 참조 0건. 다른 노드(`if-else.handler.spec.ts` 등) 패턴과 비교하면 보강 여지 있음. 단 spec / Principle 7 위반은 아님 (코드는 `rawConfig.operations ?? operations` fallback 으로 올바르게 동작).
...
8. **구현 품질**:
   - `structuredClone(input)` (`:133`) 로 원본 불변 보장 — 테스트 (`transform.handler.spec.ts:330-340`) 가 검증.
   - `safeCompileRegex` (`:38-40`) 가 단일 chokepoint `compileUserRegex` (condition-eval.util) 로 위임해 ReDoS 방지 — 길이 상한 + safe-regex 위험패턴(`(a+)+$` 등) + 문법 검사. `string_op.replace` 와 `array_filter` (via `compileRegexCache`) 양쪽 적용. → (2026-06-25) PR #570(M-3) 로 변경: 기존 `MAX_REGEX_LENGTH = 200` 상수는 제거되고 검증이 `compileUserRegex` 로 일원화됨 (`transform.handler.ts:35-40` 주석 명시).
   - dead code 없음. `DATE_UNITS` 만 runtime 사용 (executor `dateOp` 분기), 다른 화이트리스트는 schema 로 이전 — 주석 (`:97-100`) 이 history 명시.
   - `setNestedValue` 의 prototype pollution 차단 (`__proto__` / `constructor` / `prototype`) 은 `transform.handler.spec.ts:773-799` (set_field + object_omit root 차단) 가 검증.

## 종합 개선안 (2026-05-16)

- [ ] (impl) `transform.handler.spec.ts` 에 `context.rawConfig.operations` 와 `config.operations` 가 다른 경우 (expression 평가 전후) `result.config.operations` 가 raw 를 echo 하는지 확인하는 테스트 추가. 근거: `transform.handler.ts:151-155` 의 fallback 분기는 다른 노드 (`if-else.handler.spec.ts:rawConfig echo`) 처럼 단위 테스트로 명시 검증되어야 conventions Principle 7 의 회귀 방지가 된다. — ⏳ (2026-06-25) 잔여: `transform.handler.spec.ts` 전체에 `rawConfig` 참조 여전히 0건.
- [ ] (spec) §5.1 footnote 또는 §4 본문에 "Transform 은 root 직접 노출 (Principle 8.2 예외) — 이유: 재구조화 시멘틱 보존" 을 명시. 현재 spec §5.1 표 footnote 에 "(Principle 8 예외 — 변형 결과를 root 에 둠)" 한 줄만 있어 의도 보강. 근거: code.md 의 같은 예외도 spec footnote 에 "shape 은 사용자 코드가 결정. `output.result` 래핑은 적용하지 않음" 으로 상세 — Transform 에도 동일 깊이 권고. — ⏳ (2026-06-25) 잔여: `spec/4-nodes/5-data/1-transform.md:174` 표 셀이 여전히 "(Principle 8 예외 — 변형 결과를 root 에 둠)" 한 줄, 재구조화 시멘틱 보존 근거 미반영.
