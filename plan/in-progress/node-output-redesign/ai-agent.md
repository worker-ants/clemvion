...
2. **에러 컨트랙트 (Principle 3) — CRITICAL gap**:
   ...
   - spec §7.3 / §7.9 는 `LLM_CALL_FAILED` / `LLM_RATE_LIMIT` / `LLM_RESPONSE_INVALID` / `TOOL_EXECUTION_FAILED` 4종 코드를 명시하지만 handler 는 어느 것도 만들지 않는다. spec §10 표가 "예약/현재 발생 안 함" 라벨로 일부 완화되어 있으나 `LLM_CALL_FAILED` 만은 "runtime" 으로 명시되어 있어 spec ↔ impl 비일관.
...
- [ ] (impl) **CRITICAL** `executeSingleTurn` / `executeMultiTurn` / `processMultiTurnMessageInner` 의 `llmService.chat` 호출을 `try/catch` 로 감싸 `output.error.{code:'LLM_CALL_FAILED', message, details?}` + `port:'error'` + `status:'ended'` 환경 반환. text-classifier `text-classifier.handler.ts:163-209` / info-extractor `information-extractor.handler.ts:241-313, :410-422, :490-511` 의 패턴을 따른다. 근거: `ai-agent.handler.ts:900, :1089, :1429, :1629` (현 throw 누락), spec §7.3 / §7.9.
  - **(2026-05-21 부분 완료)** multi-turn 경로 (`executeMultiTurn` waiting 후 `processMultiTurnMessageInner` 의 LLM throw) 는 engine 측 `handleAiTurnError` + handler `endMultiTurnConversation(state, 'error', errorPayload)` + `buildMultiTurnFinalOutput` 의 `errorPayload` 인자로 spec §7.9 shape (`port='error'`, `status='ended'`, `output.error` + 부분 `output.result.*` 병존) 으로 finalize 됨 — PR #209 (`50b3f83b`) + ai-review 후속 (`82383739`). 단, handler 의 빌더는 engine catch 가 주입한 `errorPayload` 에 의존 — handler 내부에서 자체 try/catch 로 throw → builder 호출하는 패턴은 아님. 잔여: **single-turn (spec §7.3) 경로** — `executeSingleTurn` 의 `llmService.chat` throw 가 여전히 그대로 engine 으로 빠져 `output.error` 포트 라우팅이 발생하지 않음. 코드 코드명: `LLM_CALL_FAILED` (spec §10).
- [ ] (impl) HTTP 429 (rate limit) provider 분류 시 `LLM_RATE_LIMIT` 코드 부여 — provider error 의 `status === 429` 분기. spec §10 의 코드 표와 정합.
