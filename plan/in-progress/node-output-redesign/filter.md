...
> **6차 갱신 (2026-06-25 코드 재검증)**: handler/schema/spec 5필드 envelope·양쪽 포트 동시 활성화 정합 유지. PR #570 (refactor 04 보안 M-3, 2026-06-12) 으로 regex 가 단일 chokepoint `compileUserRegex` 로 통일(길이 ≤ 200 + `safe-regex` ReDoS 거부 + 문법) — §7 item 8 의 "Transform 이 `MAX_REGEX_LENGTH` 를 inline 정의" 횡단 중복 갭은 **해소** (transform.handler.ts 가 공유 `compileUserRegex` 를 import). spec §4 step 4 (`8-filter.md:103`) 가 `compileUserRegex`·safe-regex 를 명시하도록 갱신됨. core util 이 `nodes/logic/core/` → `nodes/core/condition-evaluator.util.ts` 로 이동(`MAX_REGEX_LENGTH=200` `:45`), `_shared/condition-eval.util.ts` 는 re-export(`:23`). handler 라인 시프트(return `:168-189`, throw `:84`, fallback `:78-87`) stale ref 정정. §8 잔여 2건(MAX_REGEX_LENGTH 상수 SoT 명시·빈배열 다운스트림 분기 동작 문서화) 은 spec 본문에 여전히 미반영 — **잔여**.
...
대상 파일: `codebase/backend/src/nodes/logic/filter/{filter.handler.ts, filter.schema.ts, filter.handler.spec.ts, filter.schema.spec.ts, filter.component.ts}` 와 `codebase/backend/src/nodes/logic/_shared/condition-eval.util.ts` (공유 condition 평가; `MAX_REGEX_LENGTH`·`compileUserRegex` 는 `codebase/backend/src/nodes/core/condition-evaluator.util.ts` 의 `MAX_REGEX_LENGTH=200` `:45` / `compileUserRegex` `:71` 를 re-export — `_shared/condition-eval.util.ts:23,29`). → (2026-06-25) core util 경로가 `nodes/logic/core/` → `nodes/core/` 로 이동(PR #570 이전 리팩토링).

1. **spec §5 ↔ handler return 정합성**:
   - `filter.handler.ts:168-189` return `{ config: { inputField, conditions, combineMode, strictComparison }, output: { match, unmatched }, meta: { matchedCount, unmatchedCount, totalCount, fellBackToEmpty, invalidRegexPatterns } }` 가 spec §5.1 와 5필드/sub-key 모두 일치.
...
2. **schema ↔ spec config 정합성**: `filterNodeConfigSchema` (`filter.schema.ts:35-78`) 의 `inputField` / `conditions` / `combineMode` / `strictComparison` 모두 spec §1 표와 동일. default `''`/`[]`/`'and'`/`false` 정합. `conditionGroupSchema` 는 if-else.schema 에서 import — 공유. `filterNodeOutputSchema` 의 `output: { match, unmatched }` shape (`:22-29`) 가 spec §5.1 와 일치.

3. **validate 일관성**:
   - `handler.validate()` (`filter.handler.ts:46-58`) 는 SSOT (`evaluateMetadataBlockingErrors`) + `combineMode` enum guard 추가. `inputField` 빈/누락, `conditions` 빈/누락, per-condition `field` type/operator 화이트리스트 검증은 `warningRules` + `validateFilterConfig` 가 담당 (`schema.ts:99-142` validateFilterConfig, `:158-169` warningRules).
   - SSOT 분리 명확. 중복 검출 없음.
...
5. **conventions Principle 0–11 위반 패턴**:
   ...
   - Principle 7: `rawConfig` 4필드 모두 echo (`:169-174`) — `?? 'and'` / `?? false` fallback 도 schema default 와 일치하여 raw echo 완전. 부합.
   - Principle 8: 이중 중첩 없음.
   - Principle 10: `null`/`undefined` → `[]` + `meta.fellBackToEmpty: true` 분기 (`:78-87`). 부합. Map 의 silent `[]` fallback 과 달리 진단 메트릭 명시 — 일관성 권고 (Map 측 후속).
...
7. **횡단 일관성 (Logic 분기/변형 4종)**:
   ...
   - Filter 의 `MAX_REGEX_LENGTH=200` 은 `_shared/condition-eval.util.ts:23` 가 `nodes/core/condition-evaluator.util.ts:45` 를 re-export. → (2026-06-25) 해소: PR #570 (refactor 04 M-3) 가 regex 컴파일을 단일 chokepoint `compileUserRegex` (`nodes/core/condition-evaluator.util.ts:71`) 로 통일 — Transform 도 더 이상 상수를 inline 정의하지 않고 공유 `compileUserRegex` 를 import (`transform.handler.ts:19,21,39`). 횡단 상수 중복 제거됨.

8. **구현 품질**:
   - `regexCache` (`:95`) + `invalidRegexPatterns` insertion-ordered Set (`:100`) 으로 dedup + 재컴파일 방지. 매 iter cache hit 시 O(1). regex 컴파일은 `getRegex` (`:101-116`) → 공유 `compileUserRegex` 단일 경로 (PR #570).
   - `resolveIfExpression` 의 catch 분기가 `undefined` 반환 (`:207-220`) — 주석에 "Numeric comparators coerce it to NaN (always false)" 명시. silent 실패 의도가 명확.
   - per-item `itemCtx` (`:126-130`) 가 base context 를 spread + `$item`/`$itemIndex` overlay — workflow context 상속 정합.
   - dead code 없음.

## 종합 개선안 (2026-05-16)

- [ ] (spec) §4 의 regex 길이 cap (`≤ 200`) 출처를 `MAX_REGEX_LENGTH` 상수로 명시 (footnote / 인용). 근거: spec §4 step 4 (`8-filter.md:103`) 가 PR #570 이후 헬퍼 `compileUserRegex`·safe-regex·"길이 ≤ 200" 을 명시하도록 갱신됐으나, SoT 상수 `MAX_REGEX_LENGTH` (`nodes/core/condition-evaluator.util.ts:45`, `_shared/condition-eval.util.ts:23` re-export) 자체는 여전히 인용 미연결 — 헬퍼명만 연결됨. **잔여 (부분 진전)**.
- [ ] (impl/spec) `output.match.length` / `output.unmatched.length` 가 비어있을 때 후속 노드의 입력 분기 동작 (downstream 노드가 빈 배열을 받았을 때 skip 인지 실행인지) 을 spec 본문에서 한 줄 명시. 근거: 양쪽 포트 동시 활성화 + 빈 배열 케이스가 spec §5.1 "특수 값 동작" 표 (`8-filter.md:170-176`) 에 명시되어 있으나 다운스트림 분기(빈배열 수신 시 skip/실행) 동작은 여전히 미서술. **잔여**.
