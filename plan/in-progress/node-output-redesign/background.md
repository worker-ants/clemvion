...
- [x] (frontend) `background:run:<id>` WebSocket 채널 구독 / 상태 표시 UI — spec §8.5 명시. — ✅ (2026-06-25) WS 채널 `background:run:` (`codebase/backend/src/modules/websocket/websocket.gateway.ts:31,37`) + BACKGROUND_RUN_STARTED/COMPLETED 이벤트(`websocket.service.ts:175-176`, 발행 `background-execution.processor.ts:109-152`) + frontend `BackgroundRunSection`(`codebase/frontend/src/components/editor/run-results/`) + 모니터링 API(`codebase/backend/src/modules/executions/background-runs/`) 전부 구현. (2026-05-16 시점 이미 존재했으나 plan 이 "외부 추적"만 표기한 미검증 항목 — 회귀 확인하여 해소 처리.)
