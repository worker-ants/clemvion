...
> **6차 갱신 (2026-06-25 코드 재검증)**: handler/schema/test 코드는 2026-05-16 이후 **무변경** — §7 의 handler·schema·test `파일:라인` 인용 전부 현재도 정확(`background.handler.ts:54-79` return, `:56` randomUUID, `:40-46` validate; `background.schema.ts:31-64` config / `:75-84` metadata; spec `4-nodes/1-logic/12-background.md` §5.1 정합). 엔진은 `codebase/backend/src/execution-engine/` → `codebase/backend/src/modules/execution-engine/` 로 **재배치**됨 — `scheduleBackgroundBody` 는 이제 `modules/execution-engine/execution-engine.service.ts:5571`, `executeBackgroundSubgraph` 는 `:5659` (stale 경로 정정). §8 항목 처분: **①(spec durationMs 의미 명시) 해소** — spec §5.1 표가 `meta.durationMs` 를 "runtime — handler 측정 … 백그라운드 본문 실행 시간 아님 (CONVENTIONS Principle 2)" 로 명문화(`spec/4-nodes/1-logic/12-background.md:131`); **③(frontend `background:run:<id>` WS 채널 + 상태 UI) 해소** — `websocket.gateway.ts:31,37` 채널 + `BackgroundRunSection`(frontend) + BACKGROUND_RUN_STARTED/COMPLETED(`websocket-events.types.ts` `BackgroundRunEventType`) + 모니터링 API(`modules/executions/background-runs/`) 전부 구현됨(2026-05-16 시점 이미 존재했으나 plan 이 미검증으로 "외부 추적"만 표기); **②(`meta.jobId` 엔진 stamp) 잔여** — handler 미발행(설계 의도, `background.handler.spec.ts:146-156`)은 유지되나 엔진(`scheduleBackgroundBody`)도 NodeExecution.outputData 에 jobId 를 stamp 하지 않음(DTO `background-run-response.dto.ts` 에도 jobId 부재) — 의도된 미발행 잔여. 새 갭 없음.
...
1. **spec §5 ↔ handler return 정합성 (fire-and-forget 컨테이너)**:
   ...
   - `port: 'main'` 만 반환 — `background` 포트는 엔진(`ExecutionEngineService.scheduleBackgroundBody`, 현재 `codebase/backend/src/modules/execution-engine/execution-engine.service.ts:5571`; 본문 실행은 `executeBackgroundSubgraph` `:5659`) 별도 활성화 (spec §5.2 + §4 의 fire-and-forget 격리 컨트랙트). 부합. → (2026-06-25) 엔진 디렉터리가 `src/execution-engine/` → `src/modules/execution-engine/` 로 재배치됐으나 심볼/책임 동일.
...
5. **conventions Principle 0–11 위반 패턴**:
   ...
   - Principle 11: `meta.jobId?` 선택 필드 (spec §5.1 + schema `:21`) — handler 는 미발행 (`background.handler.spec.ts:146-156` 명시 검증). 향후 엔진이 stamp. → (2026-06-25) 엔진(`scheduleBackgroundBody` `modules/execution-engine/execution-engine.service.ts:5571`)도 여전히 NodeExecution.outputData / 모니터링 DTO(`background-run-response.dto.ts`)에 jobId 를 stamp 하지 않음 — 의도된 미발행 잔여(§8 ②).
...
- [x] (spec) §5.1 출처 표의 `meta.durationMs` 가 "handler 측정" 임을 다른 컨테이너의 "engine inject" 와 비교해 footnote 추가 — fire-and-forget 의도 명시. 근거: `background.handler.ts:74` vs Loop/ForEach 의 엔진 inject 패턴. — ✅ (2026-06-25) spec §5.1 표(`spec/4-nodes/1-logic/12-background.md:131`)가 `meta.durationMs` 를 "runtime — handler 측정 … 백그라운드 본문 실행 시간 아님 (CONVENTIONS Principle 2)" 로 명문화 — fire-and-forget 의 handler-측정 의미를 인-테이블로 명시(별도 비교 footnote 대신 동일 의미 표기).
- [ ] (impl) `meta.jobId` 엔진 stamp 후속 작업 — `ExecutionEngineService.scheduleBackgroundBody()` (`codebase/backend/src/modules/execution-engine/execution-engine.service.ts:5571`) 가 BullMQ job add 후 NodeExecution.outputData 에 stamp. 현 handler 미발행(`background.handler.spec.ts:146-156`) 은 의도된 분리 유지. (2026-06-25 재검증) 엔진도 여전히 jobId 를 outputData / 모니터링 DTO 에 stamp 하지 않음 — 의도된 미발행 잔여, 별도 plan 으로 추적.
- [x] (frontend) `background:run:<id>` WebSocket 채널 구독 / 상태 표시 UI — spec §8.5 명시. — ✅ (2026-06-25) WS 채널 `background:run:` (`codebase/backend/src/modules/websocket/websocket.gateway.ts:31,37`) + BACKGROUND_RUN_STARTED/COMPLETED 이벤트(`websocket-events.types.ts` `BackgroundRunEventType`, 발행 `background-execution.processor.ts:109-152`) + frontend `BackgroundRunSection`(`codebase/frontend/src/components/editor/run-results/`) + 모니터링 API(`codebase/backend/src/modules/executions/background-runs/`) 전부 구현. (2026-05-16 시점 이미 존재했으나 plan 이 "외부 추적"만 표기한 미검증 항목 — 회귀 확인하여 해소 처리.)
