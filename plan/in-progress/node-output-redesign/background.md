...
대상 파일: `backend/src/nodes/logic/background/{background.handler.ts, background.schema.ts, background.handler.spec.ts, background.component.ts}`. schema.spec.ts 부재 (schema 가 매우 단순해 별도 분리 불필요).
