...
- 옛 `output.type: 'chart'` / `output.chartType` / `output.title` / `output.rendered` (SVG snapshot) 모두 폐기 (§5.4 footnote)
> ⚠️ `previousOutput` 은 예외 — 폐기 예정이나 resume 경로가 재개 출력에 **지금도 주입한다**
> ([node-output §4.2](../../../spec/conventions/node-output.md#42-폐기할-필드--구조) 과도기 예외).
> **신규 소비 금지**, Phase 3 정리 시 backend·spec·frontend renderer 3자 동시 제거.
