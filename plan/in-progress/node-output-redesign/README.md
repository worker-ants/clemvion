...
- **P0** — ai-agent error 컨트랙트. **multi-turn 경로 ✅ 해소** (engine `handleAiTurnError`+`classifyLlmError`, `output.error`+`port:'error'`, error 포트 단위테스트). **잔여: single-turn (`executeSingleTurn`) `llmService.chat` try/catch 미적용** — single throw 가 여전히 engine FAILED. 추가 잔여: `LLM_RESPONSE_INVALID` spec "예약"→"runtime" 격상 vs single-turn raw-string fallback drift, config echo memory 필드 누락.
  - (EIA cross-ref) 본 P0 가 ai-agent `output.error` shape 을 신설할 경우 [Spec External Interaction API §6.4](../../../spec/5-system/14-external-interaction-api.md#64-페이로드--executionfailed) 의 `execution.failed` payload `error` 필드 매핑에 영향 (절 번호를 §6.3 으로 잘못 적고 있었다 — `execution.failed` 는 §6.4 다). 착수 전 호환성 확인 후 SSE payload spec 동기화 필요. **2026-08-13 이후 `error` 계약의 SoT 는 [§6 필드 집합 표](../../../spec/5-system/14-external-interaction-api.md#종결-이벤트의-필드-집합-normative)** 이며, 같은 표는 **2026-08-14 부터 `failed` 를 "전 경로 object" 로 기록한다**(종전의 "현행 일부 경로는 string" 은 해소됨).
