...
- [ ] `_overview.md` §5 coverage matrix 갱신:
  - store: `8` → `100` supported, `50+` → `6` planned (privacy_* 6건 잔존)
  - product: `14` → `63` supported, `25+` → `0` planned
  - order: `17` → `106` supported, `30+` → `0` planned
  - 합계: `264` → `499` supported, `~109` → `6` planned
- [ ] **W-1 fix**: `_overview.md` §4 규칙 8 에 "`planned` 행은 backend
  메타데이터 row 가 없으므로 본 검증 대상에서 제외 — `planned` 행의 `restricted`
  컬럼은 구현 예정 메모용" 문구 추가.
- [ ] **W-9 fix**: §5 Coverage Matrix 헤더의 `2026-05-17 기준` → `2026-05-21`.
- [ ] `_overview.md` §7 CHANGELOG 에 본 plan 의 phase 별 1줄 entry 추가
- [ ] `planned.ts` 의 product / order 배열이 `[]`, store 배열이 privacy_* 6건만
  잔존 확인.
- [ ] `/ai-review` SUMMARY 0 Critical / 0 Warning 확보
- [ ] `make e2e-test` 통과 (전체 작업 종료 시 1회 — D-3 결정)
