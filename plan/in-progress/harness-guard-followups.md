...
## G. bootstrap 책임#2(mermaid 설치)를 별도 스크립트로 추출 — 리팩터, 별건

> 출처: 00_59_56 W6 (architecture, LOW). "결함 아닌 개선 여지"로 명시됨.

A 로 인해 책임#2 가 ~7줄 → ~70줄의 동시성 상태머신(마커+락+liveness+throttle)으로 커졌다.
같은 파일 책임#4(reap)는 이미 `reap-merged-worktrees.sh` + 격리 테스트로 분리한 선례가 있는데
책임#2 는 인라인으로 남았다. 그 여파가 테스트에도 드러난다 — `test_bootstrap_mermaid_install.py`
가 책임#2 만 검증하려고 매번 전체 bootstrap 을 서브프로세스로 돌리며 무관한 reap 섹션을
`REAP_MIN_INTERVAL`/`REAP_GH_BIN` 로 무력화한다.

**왜 A 에서 안 했나**: behavior-preserving 리팩터를 버그 fix PR 에 섞으면 scope 오염이다(W4/W5 를
미룬 것과 같은 이유). 로직이 이미 `_install_throttled`/`_lock_is_dead` 등 named predicate 로
분해돼 있어 파일 경계만 옮기는 기계적 작업이다. 별 PR 로 하면 추출된 스크립트가 제대로 리뷰받는다.

- [ ] `ensure-mermaid-lint-deps.sh` 로 추출, `bootstrap-session.sh` 는 호출만. 테스트도
      `test_reap_merged_worktrees.py` 처럼 그 스크립트만 대상으로 단순화.
...
- [ ] G — bootstrap 책임#2 를 `ensure-mermaid-lint-deps.sh` 로 추출 (리팩터, 별건)
