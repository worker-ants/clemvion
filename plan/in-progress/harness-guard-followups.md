...
- [ ] **I3 — `.github/workflows/e2e.yml` 의 `paths-ignore` 에 `.github/**` 누락.** PROJECT.md e2e
      면제 화이트리스트(`.github/**`)와 실제 CI 트리거가 drift. 선재 결함, 이 PR diff 밖. e2e.yml 만 별도 정정.
- [ ] **W5(12_31_29) — 커버리지 매트릭스 무결성 가드 부재.** "pnpm 워크스페이스 밖 npm 트리는 반드시
      dependabot.yml 에 등록" 이라는 불변식을 강제하는 빌드 가드가 없다(현재 대상 1개라 실 drift 없음).
      미래 두 번째 트리가 생겨 등록을 잊으면 이번과 같은 "영구 무신호" 재발. `find package.json` vs
      dependabot `directory:` 대조 가드(test_doc_sync_matrix 패턴). 저우선(대상 1개).
