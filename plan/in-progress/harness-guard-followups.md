...
> | [`harness-consistency-bundler-budget.md`](harness-consistency-bundler-budget.md) | §H |
...
>
> **진행 (2026-07-24)**: §L·§I 종결(`complete/` 이동) · §H 해소(잔여 1건) ·
> §A잔여/§F W6 종결. **열린 것은 §K · §G(조건부) 둘**. §I 는 첫 실행에서 티켓이 예측한
> **6번째 누락**(`.claude/config/**`)을 실제로 잡아 등재까지 마쳤다.
...
**→ 이관됨**: [`harness-consistency-bundler-budget.md`](harness-consistency-bundler-budget.md) — 잔여 작업은 그 티켓이 정본이다.
본 섹션은 배경·측정 기록으로 보존한다.
...
- (이관) **H** → [`harness-consistency-bundler-budget.md`](harness-consistency-bundler-budget.md)
  — **원 전제 반증 + 다른 진짜 결함 해소**. 카탈로그가 target 을 치환한 게 아니라(키별 예산이
  2026-05-16부터 있었다) target 이 **자기 몫 안에서** 잘려 18개 중 9개 파일이 조용히 사라지고
  있었다. 티켓에 실측 기록. 잔여 1건(diff-base fork-point)만 열려 있다
