...
**F~J 가 추가**됐다. 섹션 목록이 실제 범위이며, 종결 상태는 하단 `## 체크리스트` 가 정본이다.
J 만 **차단성**(게이트 우회)이고 나머지는 비차단 개선이다.
...
- `_SEGMENT_IS_GIT`(release 경로)에도 같은 `\S+` 가 있으나 그쪽은 미매치 = **해제 안 함** =
  안전 방향이라 별개다.
- [ ] `_GIT_PUSH` 의 env 값에 따옴표 형태 허용. 단 이 패턴은 `test_push_guard_allowlist.py` 가
      **byte-for-byte 고정**하고 있고 차등 코퍼스(legacy ⇒ new)가 걸려 있으므로, 핀 갱신 +
      코퍼스 확장 + 뮤테이션까지 **별 PR** 로 처리한다. 넛지 훅 쪽 수정은 C PR 에서 완료
      (`guard_default_branch_bash.py` — 양쪽 정규식에 상호 참조 주석 있음).
- [ ] 같은 클래스 전수 점검: 다른 가드에 `=\S+` 형태의 env 접두 스킵이 더 있는지 grep.
...
- [ ] **J — push 가드 `_GIT_PUSH` 가 따옴표 env 접두에서 push 를 놓쳐 리뷰 게이트가 우회된다
      (차단성 — 최우선, 별건 PR)**
