...
**사용자 결정 (2026-07-23): "관측 가능하게"** — fail-open 은 유지하되 발동 시 로그/알림을 남기고,
연속 N회 발동하면 경고. 가드 오류가 작업을 막지 않으면서도 "조용히 무력화된 상태"를 알 수 있게 한다.
fail-closed 전환·현행 방치는 모두 기각.

- [ ] 구현 — 3개 fail-open 경로(① `_lib` import 실패 ② `evaluate_*` 예외 ③ `main()` 미처리 예외)에
      관측 지점 추가 + 연속 발동 카운터
