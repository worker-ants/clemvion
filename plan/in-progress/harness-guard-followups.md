...
- **실측**: `_MUTATING` 은 `^\s*` 로 **앵커**돼 명령의 첫 토큰만 본다. 프로브 8건
  (`echo "rm -rf /tmp/x"`, `grep -n "mkdir" f`, `git log --grep="commit"`, `echo "git commit"` …)
  전부 정분류, 불일치 0건. push 가드가 `_redact_inert_text` 로 막아야 했던 **오탐 클래스가 여기엔
  사실상 없다** — 공유해도 얻는 게 없다.
...
- [x] `_is_mutating` 은 그동안 **테스트 0건**이었다 → 신규 `test_guard_default_branch_bash_mutating.py`
      9건으로 현행 동작(FP 클래스 없음·read-only 침묵·간접실행 미분류)과 갭 회귀를 함께 고정.
      비-vacuity: 뮤턴트 4종(분할 원복/split 무력화/env 접두 제거/앵커 제거) 전부 RED.
- 분할은 인용을 모른다 → `echo "a && rm -rf x"` 는 이제 넛지한다. **의도된 수용**이며
  `AcknowledgedFalsePositiveTest` 로 고정했다: 인용을 아는 분할은 ② 가 이미 포기한 "정밀 셸 파서"
  경로(무한 표면)이고, 대가는 *이미 default branch 에 있을 때만 뜨는 세션 1회 소프트 넛지* 뿐이다.
