...
> **이 문서의 역할이 바뀌었다 (2026-07-24).** 열린 잔여 작업은 전부 개별 `harness-*` 티켓으로
> 이관됐고, 각 티켓이 그 항목의 **정본(SoR)** 이다. 본 문서는 이제 **인덱스 + 배경/측정 기록**
> 이다 — 섹션 본문(재현 실측·기각된 대안·라운드별 버그 표)은 티켓에 다 옮기지 않고 여기 남긴다.
>
> | 티켓 | 원 섹션 |
> | --- | --- |
> | [`harness-push-detection-env-value-concat.md`](../complete/harness-push-detection-env-value-concat.md) ✅ | §L (차단성) |
> | [`harness-checks-paths-coverage-guard.md`](../complete/harness-checks-paths-coverage-guard.md) ✅ | §I |
> | [`harness-consistency-bundler-budget.md`](../complete/harness-consistency-bundler-budget.md) ✅ | §H |
> | [`harness-gate-control-flow-extract.md`](../complete/harness-gate-control-flow-extract.md) ✅ (won't-do) | §K |
> | [`harness-mermaid-install-concurrency.md`](harness-mermaid-install-concurrency.md) | §G (조건부) |
> | [`harness-guard-residual-hygiene.md`](harness-guard-residual-hygiene.md) | §A 잔여 · §F W6 |
>
> **종결 조건**: 위 6개 티켓이 모두 닫히면 본 문서를 `plan/complete/` 로 이동한다.
> (지금 옮기지 않는 이유는 하단 Rationale 참고.)
>
> **진행 (2026-07-24)**: §L·§I·§H 종결(`complete/` 이동) · §K won't-do 종결 · §A잔여/§F W6 종결.
> **열린 것은 §G(조건부) 하나** — 그런데 §G 는 착수 조건(동시 첫-설치 오염 실제 관측)
> 미충족 시 won't-do 가 기본값인 **의도적 무조치** 항목이다(§G 티켓 참고). 즉 실질 작업 잔여는
> 없다. §K 는 재도출 결과 `1×4` 전제가 #1005 로 붕괴(실제 `2×2`)했고 추출이 net-negative 라
> won't-do (사용자 confirm). §I 는 예측된 **6번째 누락**(`.claude/config/**`)을 실제로 잡았다.
...
> **잔여 항목 이관됨**: [`harness-guard-residual-hygiene.md`](harness-guard-residual-hygiene.md). 아래 `(이관)` 표시 항목이 그 티켓의 체크리스트다.

락을 빼며 대부분의 잔여 WARNING(락 correctness 관련)이 moot 됐다. 남은 것:

- (이관) **W1 — "main 체크아웃 루트" 해석이 3곳(bootstrap·pre-commit·PostToolUse) 중복 + bootstrap 실패
...
- (이관) **W3 — 테스트 헬퍼 `_node_calls`/`_run` 도입부가 `test_mermaid_lint_ready.py` 내 중복.** 순수
      위생, 동작 무관.
- (이관) **W8 — `harness-checks.yml` 만 node 22 / setup-python 사용(다른 워크플로는 node 24).** 선재 CI
      설정, diff 밖.
...
- (이관) **W3(10_55_35) — bash mtime/cooldown 헬퍼가 `reap-merged-worktrees.sh` 와 중복**(`_file_mtime` vs
      `file_mtime`). python 은 `_lib/mermaid_lint_ready.py` 로 SoT 통합했는데 bash 는 안 됨. `.claude/tools/_lib/*.sh`
      공유 또는 최소 네이밍 통일. 저우선.
- (이관) **I1(10_55_35) — hung `npm install`(타임아웃 없음)의 blast radius 가 락 제거로 세션 1개 → 동시
...
**→ 이관됨**: [`harness-mermaid-install-concurrency.md`](harness-mermaid-install-concurrency.md) — 잔여 작업은 그 티켓이 정본이다.
본 섹션은 배경·측정 기록으로 보존한다.
...
- (이관) **W6(12_31_29) — `mermaid-lint/package.json` 의 `jsdom`·`mermaid` `"*"` range.** Dependabot 활성화로
...
**→ 이관·종결**: [`harness-consistency-bundler-budget.md`](../complete/harness-consistency-bundler-budget.md) — 해소 근거는 그 티켓이 정본이다.
본 섹션은 배경·측정 기록으로 보존한다.
...
**→ 이관·종결**: [`harness-checks-paths-coverage-guard.md`](../complete/harness-checks-paths-coverage-guard.md) — 해소 근거·경계 정의는 그 티켓이 정본이다.
가드 신설(`test_harness_checks_paths_coverage.py`)이 첫 실행에서 예측된 **6번째 누락**
(`.claude/config/**`)을 잡아 등재까지 마쳤다. 본 섹션은 배경·측정 기록으로 보존한다.
...
**→ 이관·종결**: [`harness-push-detection-env-value-concat.md`](../complete/harness-push-detection-env-value-concat.md) — 해소 근거는 그 티켓이 정본이다.
본 섹션은 배경·측정 기록으로 보존한다.
...
**→ 이관·won't-do 종결**: [`harness-gate-control-flow-extract.md`](../complete/harness-gate-control-flow-extract.md) — 재도출 결과
`1×4 동일 블록` 전제가 #1005 로 붕괴(실제 `2×2`, push 쌍은 `_evaluate_over_targets` 로 이미
추출·stop 쌍은 nudge 라 액션 상이). 공통 잔여(bypass/None 프리픽스)는 `failopen_state` 복원력을
깨지 않고는 추출 불가하고 net-negative. 행동은 블록별 테스트로 이미 전수 고정. 근거는 그
티켓이 정본. 본 섹션은 배경 기록으로 보존한다.
...
- (이관·**종결**) **L** → [`harness-push-detection-env-value-concat.md`](../complete/harness-push-detection-env-value-concat.md)
  — 해소. 부수로 **같은 alternation 의 살아있는 ReDoS** 도 제거됐다(286바이트 명령이 push 훅을
  15초 이상 정지시켰다 — 동기 게이트라 세션 정지 또는 타임아웃 fail-open)
- (이관·**won't-do 종결**) **K** → [`harness-gate-control-flow-extract.md`](../complete/harness-gate-control-flow-extract.md)
  — 재도출 결과 `1×4` 전제가 #1005 로 붕괴(실제 `2×2`, push 쌍은 `_evaluate_over_targets` 로
  이미 추출). 공통 잔여(bypass/None 프리픽스)는 `failopen_state` 복원력을 깨지 않고는 추출
  불가·net-negative, 행동은 블록별 테스트로 이미 전수 고정 → won't-do (사용자 confirm)
- (이관) **G** → [`harness-mermaid-install-concurrency.md`](harness-mermaid-install-concurrency.md)
  — 조건부, 착수 조건(동시 첫-설치 오염 실제 관측) 미충족이라 **의도적 무조치**
- (이관·**종결**) **I** → [`harness-checks-paths-coverage-guard.md`](../complete/harness-checks-paths-coverage-guard.md)
  — 가드 신설. 경계 정의(모듈레벨 상수·tracked 파일·product 제외)가 작업의 절반이었고,
  첫 실행에서 예측된 **6번째 누락**(`.claude/config/**`)을 잡아 등재까지 마쳤다
- (이관·**종결**) **H** → [`harness-consistency-bundler-budget.md`](../complete/harness-consistency-bundler-budget.md)
  — **원 전제 반증 + 다른 진짜 결함 해소**. 카탈로그가 target 을 치환한 게 아니라(키별 예산이
  2026-05-16부터 있었다) target 이 **자기 몫 안에서** 잘려 18개 중 9개 파일이 조용히 사라지고
  있었다. 잔여(diff-base)도 three-dot 이라 이미 발생 불가로 확인·종결
- (별건·종결) **§A 잔여 · §F W6** → [`harness-guard-residual-hygiene.md`](harness-guard-residual-hygiene.md)
  — 실착수 항목 전부 완료. I1(hung `npm install` 도달성)만 추적 상태로 유지
...
### 왜 지금 `complete/` 로 옮기지 않나 (2026-07-24)

체크박스는 전부 `[x]`(완료) 또는 `(이관)` 이라 형식상으로는 이동 가능하다. 그러나:

1. **자식 티켓들이 이 문서를 형제 경로로 링크한다.** 지금 옮기면 6개 티켓의 "부모" 링크가
   전부 dangling 된다 — 옮기려면 같은 커밋에서 6곳을 함께 고쳐야 하고, 그 6곳은 아직
   리뷰도 안 된 신규 파일이다.
2. **이 파일은 병렬 세션 충돌이 잦다.** 2026-07-23 하루에만 세 번(§E·§C·§J) 다른 세션과
   같은 항목을 동시에 건드렸다. 이동은 conflict 표면이 가장 큰 변경이다.

자식 티켓이 하나둘 닫히면서 링크 정리와 함께 옮기는 편이 안전하다.
