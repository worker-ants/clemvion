...
> | [`harness-gate-control-flow-extract.md`](harness-gate-control-flow-extract.md) | §K |
...
>
> **진행 (2026-07-24)**: §L·§I·§H 종결(`complete/` 이동) · §A잔여/§F W6 종결.
> **열린 것은 §K · §G(조건부) 둘**. §I 는 첫 실행에서 티켓이 예측한 **6번째 누락**
> (`.claude/config/**`)을 실제로 잡아 등재까지 마쳤다. §H 잔여(diff-base)는 three-dot 이라
> 이미 발생 불가로 확인·종결. (별건 #1005 후속 `harness-push-guard-scoping-residuals` 도
> 같은 PR 에서 종결 — 그 인덱스는 이 문서가 아니다.)
...
**→ 이관됨**: [`harness-gate-control-flow-extract.md`](harness-gate-control-flow-extract.md) — 잔여 작업은 그 티켓이 정본이다.
본 섹션은 배경·측정 기록으로 보존한다.
...
- (이관) **K** → [`harness-gate-control-flow-extract.md`](harness-gate-control-flow-extract.md)
  — 미착수. #1005 이후 push 쪽 2블록이 stop 쪽과 구조가 갈려 **`1×4` 추출은 더 이상 맞지 않는다**
  (`2×2` 가 현실). 착수 전 티켓 본문 재작성 필요
