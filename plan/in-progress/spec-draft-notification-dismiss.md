...
> **번호 (V0NN)** 는 developer 단계 착수 직전 `ls backend/migrations/ | sort | tail -1` 로 현행 최신
...
- `backend/src/modules/notifications/dto/responses/dismiss-result.dto.ts` (`DismissResultDto`).
- `backend/src/modules/notifications/dto/responses/dismiss-all-result.dto.ts` (`DismissAllResultDto`)
...
| 영역 | 영향 | 조치 |
| --- | --- | --- |
| `backend/src/modules/notifications/notifications.controller.ts` | endpoint 2 개 신규 (`POST /:id/dismiss`, `POST /dismiss-all`) | developer phase |
| `backend/src/modules/notifications/notifications.service.ts:findAll` | `dismissed_at IS NULL` 필터 추가 | developer |
| `backend/src/modules/notifications/notifications.service.ts:getUnreadCount` | 동일 필터 | developer |
| `backend/src/modules/notifications/notifications.service.ts:hasRecentByResource` | **변경 없음** — dismissed row 카운트 유지 (§4.4) | developer |
| `backend/migrations/V0NN__notification_dismissed_at_add.sql` | `ALTER TABLE notification ADD COLUMN dismissed_at TIMESTAMPTZ NULL` | developer (착수 직전 번호 재확인) |
| `backend/migrations/V0NN+1__notification_active_partial_index.{sql,conf}` | partial index 전환, `executeInTransaction=false` + CONCURRENTLY | developer |
| `frontend/src/components/layout/sidebar.tsx` | popover UX 전반 개편 (mutation·deep link·hover 액션) | developer |
