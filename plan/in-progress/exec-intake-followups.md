...
- [ ] **priority 3-tier (webhook/schedule 세분화)** — 현재 `EXECUTION_RUN_PRIORITY` 상수(manual1/webhook2/schedule3)는 있으나 `execute()` 가 `manual` vs 트리거 2-tier 만 부여(schedule 미구분). `ExecuteOptions.triggerType` 필드 신설 + execute() 호출부에서 `trigger.type`/`executedBy` 로 결정해 전달 → `resolveExecutionRunPriority(triggerType)`. spec §4.3/§8 3-tier. `execution-engine.service.ts` `TODO(PR2)` 지점.
