...
- [ ] `§NNN` 표기 규약 명문화 (`convention_compliance` INFO) — spec 내 절 참조 표기를 규약으로 고정
      > **표를 갈고, 세는 식을 함께 박았다 (2026-08-29 재실측).** 종전 표는 "2026-08-10
      > 실측" 이라고만 적고 **명령을 안 남겨서**, 값이 낡았을 때 재현조차 못 했다 —
      > 실제로 `§N` 행은 세 번 재서 세 값이 나왔다(원문 1,931 / `plan-audit` 재현 1,605 /
      > 이번 1,664). 숫자만 갱신하면 같은 자리에서 또 갈리므로 **식을 같이 싣는다.**
      >
      > | 표기 | 2026-08-10 | **2026-08-29** | 세는 법 (`spec/` 전수) |
      > |---|---|---|---|
      > | `§N.N` (`§3.1`) | 4,518 | **4,679** | `grep -rEo '§[0-9]+\.[0-9]+'` |
      > | `§N` (`§3`) | 1,931 | **1,664** | `grep -rEo '§[0-9]+([^0-9.-]\|$)'` |
      > | `§텍스트` (`§Rationale`) | 224 | **507** | `grep -rEo '§[A-Za-z가-힣]+'` |
      > | `§RN` (`§R6`) | 127 | **199** | `grep -rEo '§R[0-9]+'` |
      > | `§N-N` (`§5-4`) | 37 | **37** | `grep -rEo '§[0-9]+-[0-9]+'` |
      > | `섹션 N` (한글) | 1 | **1** | `grep -rEo '섹션 [0-9]+'` |
      >
      > **`§N` 행은 식에 민감하다** — 위 식은 `§3.1` 의 앞부분을 세지 않으려고 뒤에
      > `.`·`-`·숫자가 오지 않는 경우만 잡는다. 다른 식은 다른 값을 낸다. 그래서 값 하나가
      > 아니라 **식과 값을 한 쌍으로** 적는다.
      >
      > **결론은 재실측으로도 그대로다** — 압도적 다수가 `§N.N`/`§N` 이고 한글 "섹션 N" 은
      > 1건이다. 즉 명문화는 "새 규칙 제정" 이 아니라 **관측된 관행의 기록**이다.
      > 항목 자체도 유효하다(2026-08-29: `spec/conventions/` 에 spec **작성** 규약 문서
      > 없음, `§NNN`·"절 참조 표기" grep 0건).
      >
      > | 표기 | 건수 (2026-08-10, 원문 보존) |
      > |---|---|
      > | `§N.N` (`§3.1`) | 4,518 |
      > | `§N` (`§3`) | 1,931 |
      > | `§텍스트` (`§Rationale`) | 224 |
