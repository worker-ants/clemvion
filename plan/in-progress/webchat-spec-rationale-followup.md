...
- [ ] `§NNN` 표기 규약 명문화 (`convention_compliance` INFO) — spec 내 절 참조 표기를 규약으로 고정
      ...
      > **무엇이 낡았나**: 표 6행 중 4행을 현재 값으로 갱신하거나(§N.N 4,518→4,674 · §RN 127→199 · §텍스트 224→307), 수치를 빼고 "관행 수렴" 결론만 남기고 실측 시점을 명시. §N-N=37·섹션 N=1 은 불변. §N 행(1,931)은 표의 자기 커밋 기준으로도 내 재현값(1,605)과 어긋나 산출식이 불명 — 재측정 시 식을 함께 기록할 것. 결론(명문화=관행 기록)은 재실측으로도 유지된다.
      > **실측**: 항목은 유효(spec/conventions/ 24개에 spec 작성 규약 문서 없음, `§NNN`/"절 참조 표기" grep 0건). 그러나 "2026-08-10 실측" 표가 낡았다: 표 작성 커밋 c9e0e2ff7 에선 §RN=127 재현되나 현재 199, §N.N 4,520→4,674.
