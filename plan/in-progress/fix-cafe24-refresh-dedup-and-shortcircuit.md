...
- [ ] `refreshViaQueue` 에서 `source === 'reactive_401'` 일 때 jobId 를
...
- [ ] 기존 `resolveTokenExpiry` JWT exp 우선 회귀 테스트는 그대로 유지.
...
- [ ] 1) cafe24-api.client.ts — reactive_401 unique jobId
- [ ] 2) cafe24-token-refresh.processor.ts — JWT exp 한정 short-circuit
- [ ] 3) cafe24.module.ts — QueueEvents AUTH/TLS
- [ ] 4) 테스트 갱신/추가
...
- 운영 DB 의 `mall_id=gehrig0301` 즉시 회복 SQL — 사용자가 직접 실행.
- spec `## Rationale` 갱신 — project-planner 위임 (`plan/in-progress/spec-fix-resolve-token-expiry-rationale.md`
  draft 와 본 PR 의 reactive_401 unique jobId / worker JWT-only short-circuit
  결정을 함께 반영).
