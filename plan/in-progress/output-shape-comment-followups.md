...
|---|---|---|---|
| A | 최상위 게이트 `conversationConfig` disjunct | `detects … bare top-level conversationConfig` | 1 failed / 39 passed |
| B | 첫 OR-항 `hasLegacyMessages` guard | `rejects output.interactionType when output.messages is absent` | 1 failed / 39 passed |
| C | `looksLikeConversationEnd` 의 `hasResultMessages` | `rejects a whitelisted endReason without result.messages` | 1 failed / 39 passed |
| D | `isCanonicalWaiting` 의 `hasLegacyMessages` | `rejects waiting_for_input status alone without output.messages` | 1 failed / 39 passed |
| E | `outputInteraction` 분기 | `detects … via output.interactionType alone` | 1 failed / 39 passed |
| F | `hasConvConfig` 분기 | `detects … via nested output.conversationConfig alone` | 1 failed / 39 passed |
| G | `metaInteraction` 분기 | `detects … via output.messages + meta.interactionType without status` | 1 failed / 39 passed |
