...
- [x] **`6-websocket-protocol.md` 에 `### 4.4` 헤딩이 둘이고 절 번호 순서가 어긋난다**
      ...
      **완료 (2026-08-31).** *"문서 전체의 링크를 훑어야 한다"* 는 유예 근거는 맞았다 —
      ...
      - **⚠ 그 절차도 한 칸 좁았다 — 2라운드 리뷰가 또 잡았다 (같은 날).** 위 sweep 을
        `grep` 한 줄로 돌리며 **주어를 같은 줄에서만** 찾았다. 주어가 인접 줄에 있는 인용은
        전부 빠져나갔고, **내가 방금 편집한 파일 안에서** 자기모순이 남았다:

        | 지점 | 상태 |
        |---|---|
        | `websocket.service.ts:567` | 1라운드에 §4.5 로 고침 |
        | `websocket.service.ts:583·585` | **§4.4 그대로** — 주어(`notification.new`)가 581행에 있었다 |
        | `websocket.service.spec.ts:1268` | 1라운드에 §4.5 로 고침 |
        | `websocket.service.spec.ts:1283` | **§4.4 그대로** — 같은 테스트 블록 |

        **처방 확정**: 주어 판정은 **블록 문맥(±5줄)** 으로 한다. 그 방식으로 재실행하니
        후보 8건이 나왔고, 손으로 4건(실제 결함) / 4건(오탐)으로 갈렸다. 오탐 넷은
        `8-notifications.md:347`(그 줄의 `auth.token_expired` 는 점 표기 **예시**)과 이 문서
        자신의 이력 서술 3곳이다 — **자동 분류만으로는 끝나지 않는다**.

      - **트랙 판정 (3R WARNING #1) — 소정정 예외가 아니라 planner 트랙이다.** 3라운드
        연속 INFO 로 있던 항목을 리뷰어가 `git log -S` 근거와 함께 WARNING 으로 격상했고,
        **그 근거가 맞다**(직접 재확인):

        | 조건 | 판정 |
        |---|---|
        | (1) 대상 문장을 developer 자신이 썼는가 | **아니오** — §8.2 화이트리스트 문장의 출처는 `9ed6e6305`(2026-05-21, `spec:` prefix, PR #228) |
        | (2) 예고·트리거인가 | **아니오** — HMAC 알고리즘 화이트리스트는 CLAUDE.md 가 예외에서 **명시 배제**하는 *"API 계약"* 이다 |
        | (4) 원문을 취소선으로 남겼는가 | **아니오** — 전면 치환했다 |

        따라서 이 두 편집(`d743251b0` §8.2 · `50caf1a85` WS §4 재배치)은 **자기-반증형
        소정정이 아니다.** 정확한 성격은 **planner 트랙 작업**이다 — 두 항목 모두
        `owner: project-planner` plan 의 미체크 항목이었고, 사용자가 그 목록을 순서대로
        처리하라고 지시한 턴에서 수행됐다.
        - 그래서 빠진 것은 "예외 조건" 이 아니라 **planner 게이트**다: 쓰기 직전
          `--spec` 을 돌리지 못했다(그 사유는 세션 시작 시 사용자에게 고지하고 승인을
          받았다). 사후 그물인 **`--impl-done` 을 이 spec 파일들이 포함되는 scope 로**
          돌리는 것이 남은 의무이고, 이 커밋 다음 단계가 그것이다.
        - **취소선 미보존은 되돌리지 않는다** — §8.2 원문은 *폐기된 결정*이 아니라 **한 번도
          참이 아니었던 상태 서술**(구현은 처음부터 두 알고리즘을 지원했다)이다. 남기면
          "한때 그랬다" 로 읽혀 이력이 오히려 왜곡된다. 같은 판단의 선례가 이 문서
          §R8 `statusCode` 항목에 있다.

      - **⚠ 네 번째 재발 — 3R 리뷰. 키워드 sweep 자체가 틀린 도구였다.** 블록 문맥으로
        넓혀도 `L979` 를 놓쳤다: *"SSE 어댑터가 `Last-Event-Id` 헤더로 제공한다 (§4.6, …)"* —
        주어가 **SSE/EIA** 라 §4.7(외부 표면 매핑) 소관인데, 내 키워드 목록
        (`알림`/`시스템`/`외부 표면 매핑`)에 `Last-Event-Id`·`SSE` 가 없었다. 키워드를 넓히는
        것은 같은 실패를 반복한다 — **다음 키워드를 미리 알 수 없다**.

        **정본 절차 (키워드 없음)**: 그 파일의 `§4.x` 인용을 **전수 열거**하고 각각을
        헤딩 시퀀스와 1:1 대조한다. 단 **존재 검사만으로는 부족하다** — `L979` 의 §4.6 은
        *존재하는* 헤딩이었고 **주제만** 틀렸다. 그래서 각 건에 *"이 인용의 주어가 그 절의
        제목과 맞는가"* 를 물어야 한다. 이건 자동화가 아니라 사람이 읽는 단계다.

        | 라운드 | 훑은 정의 | 놓친 것 |
        |---|---|---|
        | 1R | 마크다운 **앵커 링크**만 | bare 프로즈 인용 12곳 |
        | 2R | + 프로즈, 주어를 **같은 줄**에서 | 주어가 인접 줄인 4곳 |
        | 3R | + **블록 문맥(±5줄)** 키워드 | 키워드 목록 밖 1곳(`L979`) |

      - **미조치 2건 등재 (3R WARNING #3).** 직전 커밋이 이 둘을 미조치로 판단하고 **사유를
        커밋 메시지에만** 남겼다 — 이 저장소 관례상 `review/**`·커밋 메시지는 SoT 가 아니다.
        - [ ] `spec/data-flow/8-notifications.md:349` 의 **죽은 §4.6 follow-up 참조** —
              `notification.read`/`notification.dismissed` 를 가리키는데 그 내용은 신·구 어느
              번호 체계에도 실재하지 않는다(2026-05 기원 선재 결함). *"번호 이동 오인용"* 과
              **다른 클래스**라 이번 스윕의 사각지대였다. 처방: follow-up 서술 삭제 또는
              §4.5 본문에 실제 내용 추가 + 앵커 연결.
        - [ ] `.claude/skills/consistency-checker/SKILL.md` 의 `--impl-done` 설명이 신설
              `_scope_delta_census` HEAD 블록을 언급하지 않는다. 한 문장 추가면 된다.

      - **덤으로 낡은 주장 하나**: `notifications-channel-authorizer.ts:12` 가
        *"emit 은 미구현(spec §4.4 Planned)이라 현재 실피해 0"* 이라 적고 있었다. 절 번호도
        틀렸고 **`emit` 은 이미 구현·배선돼 있다**(`websocket.service.ts:576` ←
        `notifications.service.ts:511`). 즉 그 IDOR 가드는 가정이 아니라 **실트래픽을 막는
        중**인데 주석은 "실피해 0" 이라 말하고 있었다 — 다음 사람이 가드를 낮게 평가할 문장이라
        절 번호와 함께 고쳤다.
