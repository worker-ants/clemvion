...
- [ ] **host `resetSession` booting 중 중복 webhook 가드** (widget-app §3.1 "새 대화" 행 Planned) — 헤더 UI 컨트롤은 `isActiveConversationPhase` 로 booting 을 제외하지만 host SDK `resetSession` 은 이 게이트 밖이라 in-flight `start()` 와 겹쳐 중복 `POST /api/hooks/:path` 를 발사할 수 있다(pre-existing). host-API 측 가드/드레인 필요. channel-web-chat 범위. (2026-07-10 consistency `plan-coherence` W3 로 등재.)
