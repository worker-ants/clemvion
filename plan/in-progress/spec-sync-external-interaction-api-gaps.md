...
- [~] **host `resetSession` booting 중 중복 webhook 가드** — **결정 완료 (2026-07-11, planner), 구현 developer 위임**: **single-flight coalesce**(서버 멱등 아님) — booting 중 `resetSession` 은 in-flight `start()` 에 흡수되어 2번째 POST·2번째 execution 미생성. 결정 SoT [`spec-draft-webchat-execution-residuals.md`](./spec-draft-webchat-execution-residuals.md), spec lock = [widget-app §R9·§3.1](../../spec/7-channel-web-chat/1-widget-app.md). 구현(위젯 single-flight 게이트)은 별도 channel-web-chat developer 세션.
- [~] **공개 위젯 idle-wait execution GC (EIA-RL-07)** — **결정 완료 (2026-07-11, planner), 구현 developer 위임**: eager-start 후 이탈로 `waiting_for_input` 잔존하는 익명(`auth_config_id IS NULL`) per_execution execution 을, ①"새 대화" best-effort `cancel`(source, widget-app §R9) + ②서버측 **idle-wait timeout backstop**(토큰 영구 만료+grace → `cancelled`/`WEBCHAT_IDLE_TIMEOUT`, EIA-RL-06 형 repeatable sweep)으로 회수. §7.4 무기한 보존 불변식과 정합(§1.1 예약 "타임아웃" 사유 구현). 결정 SoT [`spec-draft-webchat-execution-residuals.md`](./spec-draft-webchat-execution-residuals.md), spec lock = EIA §3.4 EIA-RL-07·§R19. 구현(서버 reaper + 위젯 cancel)은 별도 developer 세션.
