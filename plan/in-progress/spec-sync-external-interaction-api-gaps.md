...
- [x] **SSE 버퍼 만료 시 `execution.replay_unavailable` emit** (§5.2 / §11 / EIA-IN-07 / EIA-NF-03) — **완료 (2026-07-08)**: `sse-adapter.service.ts` `replayOrSignalUnavailable` 이 재연결 replay 시 재생 가능한 가장 이른 seq 가 `Last-Event-Id+1` 이 아니면(만료 또는 cap 폐기 gap) 부분 replay 대신 `execution.replay_unavailable`(seq=0 control frame) 1회 push → 클라 REST 재조회. `writeSseFrame` 이 seq≤0 시 `id:` 라인 생략(Last-Event-Id 오염 방지). spec-sync: EIA §5.2/§11(EIA-IN-07·EIA-NF-03·vocab)·Rationale R-replay-unavailable + WS §6.2(3곳)·widget-app §3.1·data-flow/15 §1.3 동기화. unit(sse-adapter 6건·writeSseFrame 3건)·lint·build 통과.
  - [ ] **(후속) web-chat 위젯 클라이언트 소비** — 위젯은 이벤트 리스너를 이미 등록(`eia-client.ts`)했으나 `use-widget.ts handleEiaEvent` 에 `execution.replay_unavailable` case 가 없어 no-op. 로컬 시간(>5분) 폴백을 이벤트 기반 감지로 교체하는 클라이언트 작업 필요(spec widget-app §3.1 TODO). channel-web-chat 범위 — 별도.
    > **진행 상태 (2026-07-17)**: **구현은 완료됐으나 미머지** — 별 branch `claude/webchat-replay-unavailable-consume`(5커밋 `436ee334e`~`7a9b4ce88`, TEST WORKFLOW·`/ai-review` 4라운드 통과). **머지 전까지 본 항목은 미완**이라 `[ ]` 유지.
    >
    > **왜 grooming PR 에서 분리했나 (사용자 결정)**: 이 항목은 2026-07-16 분류에서 "사용자 결정 없이 바로 처리 가능" 이었다. spec §3.1 이 동작을 확정해 둬 **제품 결정이 불요하다는 판단은 옳았으나, 구현 리스크 판정이 틀렸다** — `/ai-review` 4라운드에 걸쳐 **CRITICAL 2건 + WARNING 17건**이 나왔다. 문서·spec 정정(같은 grooming 의 ⑥⑦⑧ 등)이 이 위젯 리스크에 묶여 대기하지 않도록 분리한다.
    >
    > **⚠ 머지 전 판단 필요 — 구조 문제**: 같은 실패 유형이 **3연속 반복**됐다. 매 fix 가 비동기 경로 **하나만 보고 대칭 경로를 놓쳤다**:
    > 1. terminal 분기를 `seedWaitingFromStatus` 안에 넣음 → `applyConfig` 가 그 teardown 을 모른 채 `openStream`(무효 토큰 SSE + 종료 세션 storage 부활) — **CRITICAL**
    > 2. 3-state 계약 + 410 을 `finalizeEnded` 로 편입 → stale 410 이 살아있는 새 세션 오종료 — **CRITICAL**
    > 3. `sendCommand` 에 staleness 가드 보강
    >
    > 원인은 `useWidget` 이 ref 3개(`sessionRef`/`startGenRef`/`endedRef`)와 비동기 진입점 4개(SSE terminal · REST 폴백 · 명령 410 · 사용자 종료)를 한 훅에 안고 있어 **종료·staleness 판정이 단일 진입점에 강제되지 않는다**는 것. `useEiaStream`(가칭) 분리로 그 판정을 한 곳에 모으는 **선행 리팩터를 검토**할 것 — 없이 계속 패치하면 같은 패턴이 반복될 가능성이 높다. (매 라운드 나머지 경로를 reviewer 가 찾아줬다.)
    >
    > 리뷰 이력·mutation 검증·처분 상세: 해당 branch 의 `review/code/2026/07/17/{01_42_44,02_04_13,02_31_18,06_53_03}/RESOLUTION.md`. 함께 담긴 곁가지(⑨-2 webauthn 주석·⑨-3 EventSource stub 헬퍼)도 같은 branch 에 있다.
