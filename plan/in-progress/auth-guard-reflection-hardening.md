...
- [ ] `__test-utils__` 디렉터리가 **devDependency 를 import 하기 시작하면**
      ...
      > 채택했다 — `#1214`(b541484c2, 2026-08-27)가 test-only 디렉터리 두 곳을
      > `tsconfig.build.json` 에 **명시 경로로** 뺐고(`src/repo-guards/**`,
      > `src/shared/testing/**`), 그 사유가 정확히 이 항목이 말하는 위험이다: 주석이
      > "devDependency 인 typescript/@nestjs/testing 을 import 하면서 dist 안의 require 가
      > 지뢰" 라고 적는다.
      >
      > 즉 채택된 판별 기준은 **개수가 아니라 devDependency import 여부**이고, 처방도
      > glob 이 아니라 per-directory 명시 경로다. 개수 트리거를 그대로 두면 위험한 1곳은
      > 놓치고 안전한 3곳째에는 헛발을 짚는다.
      >
      > **재실측 (2026-08-29)**: `**/__test-utils__/**` exclude 는 여전히 없고,
      > `__test-utils__` 디렉터리도 여전히 2곳(`common/`, `modules/integrations/`)이며
      > 둘 다 순수 함수라 안전 — 유예는 유지, 트리거만 위 축으로 바꾼다.
