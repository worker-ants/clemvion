...
| 에러코드 | HTTP | 설명 |
|---|---|---|
| MODEL_CONFIG_NOT_FOUND | 404 | model config id 부재(id 지정 경로 전용). cross-kind access 도 동일 코드로 처리(노출 방지). |
| MODEL_CONFIG_DEFAULT_MISSING | 400 | id 미지정 시 워크스페이스 default config 없음(setup 안내용). resolveConfig / resolveEmbedding ws default 경로. |
