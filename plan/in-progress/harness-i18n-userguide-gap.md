...
### CLAUDE.md
- i18n 정책 언급 0건. README.md 만 "제품 최종 상태" 로 언급될 뿐, 유저 가이드(`frontend/src/content/docs/`) 와의 동기화 규약은 없음.

### `.claude/skills/developer/SKILL.md`
- §DOCUMENTATION (~line 50) 항목이 3줄로만 구성:
  - `frontend/docs 에서 제공되는 사용자 설명서` ← **경로 오기** (실제는 `frontend/src/content/docs/`)
  - backend swagger doc — "API 변동 시"
  - README.md — "제품 최종 상태 바뀐 경우"
- i18n dict (`frontend/src/lib/i18n/dict/{ko,en}.ts`), backend-labels, locale.ts SECTION_LABELS 는 **전혀 언급 없음**.
...
- `documentation-reviewer` 체크리스트는 docstring/JSDoc/README/API 문서/주석/CHANGELOG 등 **개발자 대상 문서**만 다룸. 유저 가이드(`frontend/src/content/docs/**`) 와 i18n dict 는 명시 항목 아님.
- 다른 12개 reviewer 도 i18n 키 parity·user-guide sync 를 직접 점검하지 않음.

### 자동 가드 (테스트·lint)
- `frontend/src/lib/i18n/__tests__/i18n.test.ts` (80줄): `translate()` 동작 / locale 상수 / fallback 만 검증. **ko.ts ↔ en.ts 키 parity 미검증**.
- `frontend/src/lib/docs/__tests__/locale.test.ts` (81줄): 알려진 섹션 라벨만 spot check. **현재 docs 디렉토리의 모든 섹션이 KO/EN 양쪽에 등록됐는지 미검증**.
- `frontend/src/lib/docs/__tests__/registry.test.ts` (213줄): frontmatter `spec`/`code` 경로 실존 검증은 있음 (✓). 하지만 KO mdx 의 sibling `<slug>.en.mdx` 존재·sync 는 미검증.
...
#### 1-A. 경로 정정
`frontend/docs` → `frontend/src/content/docs/` 로 수정.
...
| --- | --- | --- |
| 새 노드 추가 (`backend/src/nodes/<cat>/<name>/`) | (a) `frontend/src/content/docs/02-nodes/<cat>.mdx` + `.en.mdx`<br>(b) `frontend/src/lib/i18n/dict/{ko,en}.ts` 의 노드명·필드명·placeholder<br>(c) `frontend/src/lib/i18n/backend-labels.ts` (errorCode·label) | `cd frontend && npm test -- i18n docs` |
| 노드 schema 변경 (필드 추가·라벨 변경) | (a) `frontend/src/content/docs/02-nodes/<cat>.mdx` 의 FieldTable<br>(b) `frontend/src/lib/i18n/dict/{ko,en}.ts` 의 해당 키<br>(c) `frontend/src/lib/i18n/backend-labels.ts` 의 label | 동일 |
| UI 문자열 추가/변경 (TSX) | `dict/{ko,en}.ts` 양쪽 — 한쪽만 추가 금지 (parity 가드 fail) | `cd frontend && npm test -- i18n` |
| Integration 신규/변경 | `frontend/src/content/docs/06-integrations-and-config/<provider>.{mdx,en.mdx}` | docs registry test |
| 유저 가이드 신규 섹션 추가 (`content/docs/<NN>-<name>/`) | `frontend/src/lib/docs/locale.ts` 의 `SECTION_LABELS_BY_LOCALE` 양쪽 로케일 등록 | `cd frontend && npm test -- locale` |
| 백엔드 API 변경 | (a) swagger jsdoc<br>(b) 영향받는 user-guide 페이지 | swagger test |
| 인증·권한 흐름 변경 | `frontend/src/content/docs/07-workspace-and-team/` + e2e | `make e2e-test` |
| 표현식 언어 변경 | `frontend/src/content/docs/04-expression-language/` (basics / cheatsheet / variables) | 수동 |
...
- 추가 점검 항목 (`.claude/agents/convention-compliance-checker.md` 의 체크리스트에):
  ...
  2. 신규 spec 영역이 유저 가이드 섹션과 매핑되는지 — `frontend/src/content/docs/<섹션>/<slug>.mdx` 의 신규/갱신 plan 이 있는지
  3. `frontend/src/content/docs/_i18n-conventions.md` 의 파일 명명·frontmatter 규약 위반 여부 (planner 가 docs 갱신 plan 을 짤 때)
  4. `frontend/src/content/docs/_glossary.md` 의 문체·금지어 위반 여부
...
`frontend/src/lib/i18n/__tests__/i18n.test.ts` 에 추가:
...
`frontend/src/lib/docs/__tests__/locale.test.ts` 에 추가:
...
- `frontend/src/lib/i18n/__tests__/i18n.test.ts` — `flattenLeafKeys` 헬퍼 + `dict parity (ko ↔ en)` describe 신설 (2 테스트 추가).
- `frontend/src/lib/docs/locale.ts` — `hasExplicitSectionLabel(key, locale)` 신규 export.
- `frontend/src/lib/docs/__tests__/locale.test.ts` — `SECTION_LABELS_BY_LOCALE coverage` describe 신설 (3 테스트 추가) + `hasExplicitSectionLabel` 단위 테스트 2건.
