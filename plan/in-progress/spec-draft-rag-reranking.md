---
worktree: rag-rerank-decisions-dd1d68
started: 2026-06-03
owner: planner
---
...
- 정책·지시 기반 판단이 필요한 KB 는 `cross_encoder_llm` 모드로 LLM grading 까지 수행(D2).
...
| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `provider` | Enum | **1차: `tei`**(HF TEI, 자가호스팅 bge-reranker-ko) + **`cohere`**(API). **Planned: `jina`/`voyage`/`local`/`builtin`**(Transformers.js 인프로세스) |
| `baseUrl` | String? | 자가호스팅 endpoint. `tei`/`local` 은 필수. **SSRF 가드 재사용** — `local`/`tei` 는 사설망 예외([LLM Client §5.5 SSRF](../../spec/5-system/7-llm-client.md)) |
...
[회수] vector(또는 graph) 검색
   ...
   └─ rerank_mode ≠ off:
        ...
        3) [cross_encoder_llm 한정] survivors 에 listwise LLM grading 항상 수행 (§4.2)
...
- **v1 확정**: `cross_encoder` 수행 후 survivors(~15개)에 listwise LLM grading 을 **항상** 1콜 수행 (조건부 escalate 없음).
...
- **"정책 판단 KB" 표시 = `rerank_mode = cross_encoder_llm` 선택 자체** — 별도 플래그/컬럼/휴리스틱 없음.
- 비용·지연(+0.5~1s/콜)을 감수할 가치가 있는 KB 에만 이 모드를 선택. grader 가 "관련 없음" 판정 시 빈 결과 → tool_result `results: []`(LLM 이 근거 부재 인지, 환각 억제).
- (후속 최적화) 점수 평탄/모호 기반 conditional escalate — 정량 임계를 P0 평가셋으로 보정한 뒤 도입.
...
## 6. Rerank capability — 별도 `RerankClient` ([Spec LLM Client §3.6·§4.1](../../spec/5-system/7-llm-client.md))

> 반영 확정(I1): `LLMClient.rerank?()` 가 아니라 **별도 `RerankClient` 인터페이스 + `RerankClientFactory`** 로 분리 — chat/embedding 팩토리 switch 오염 방지.
...
```
- provider 별 매핑(1차 tei·cohere / 나머지 Planned): [Spec LLM Client §5.6](../../spec/5-system/7-llm-client.md).
- 미지원 provider 구성은 구성 시점 검증 실패라 `LLM_CONFIG_INVALID` 계열 재사용(전용 enum 불필요).
- `RerankClientFactory`·SSRF 가드(`tei`/`local` 사설망 예외)·secret-store 등 기존 인프라 재사용.
...
4. `spec/4-nodes/3-ai/1-ai-agent.md` — §1 `ragTopK`/`ragThreshold` 의미 보강 노트(리랭크 후 top-k / `rerank_mode≠off` KB 에서는 rerank 점수 임계로 해석, I4). ✅ **#455 에서 반영 완료** (본 결정 PR 은 이 파일 미편집). ⚠️ 동일 파일을 `kb-quality-fba2f2`(PR #457 OPEN)·`ai-context-memory-9c7e6e`(PR OPEN) 이 편집 중 — 후속 ai-agent.md 편집 시 직렬화 필요(W5).
   - ⚠️ **merge 경합(CRITICAL·절차적)**: 본 PR 이 편집하는 `spec/1-data-model.md`(§1 ER + §2.16.1 RerankConfig) 를 `ai-context-memory-9c7e6e`(PR OPEN)이 §1 ER + §2.23 AgentMemory 로 동시편집 중 → **본 PR merge 전 해당 PR merge 여부 확인 후 rebase**(미merge 면 선행 PR 후행화).
   - **status frontmatter 유지 근거**(WARNING I3): 대상 spec(9-rag-search·7-llm-client·1-data-model)은 `status: implemented` 유지. `implemented → partial` 은 lifecycle 미정의 역전이이고 #455 가 이미 implemented+`(Planned)` 표기로 머지된 선례. 리랭킹 구현 PR 이 `code:` evidence 를 추가하는 시점에 status 관리(컨벤션 `spec-only→partial→implemented` 정방향).
