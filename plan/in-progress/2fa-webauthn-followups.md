...
### 8. AuthModule 분리 (WebAuthnModule 서브모듈) — **완료**
...
- [x] (Phase B) WebAuthn HTTP 엔드포인트 (`/auth/2fa/webauthn/...`) 를 `WebAuthnController` 로 분리. controller host 는 AuthModule (단방향 의존 유지), 파일은 `auth/webauthn/webauthn.controller.ts`. `setRefreshTokenCookie` / `clearRefreshTokenCookie` 는 `auth/utils/refresh-cookie.ts` 공유 helper 로 추출. URL 경로는 그대로 — frontend·API 호환성 유지. spec §1.4.H 갱신.
