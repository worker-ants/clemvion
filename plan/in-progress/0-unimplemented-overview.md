...
> 작성일: 2026-05-11
> 최근 갱신: 2026-05-30 (`/goal` 감사 — 완료 plan 14건 git rm, 신규 갭 plan 2건 추가)
> 출처: `spec/0-overview.md` §6.2~§6.3, 각 spec 문서의 ❌·🚧 표기, 코드베이스 spot-check
> 검증 일자 기준: 2026-05-18 (본문 트리). 본 문서의 "현재 상태"는 본 시점의 코드/스펙 비교 결과이며, 진행 시점에 다시 확인할 것
>
> ⚠️ **부분 stale 경고 (2026-05-30)**: 아래 본문 트리는 2026-05-18 기준이라 이미 존재하지 않는 plan 을 일부 참조한다(예: `parallel-p2.md`, `2fa-webauthn-followups.md`, `spec-overview-followups-2026-05-18.md` 등 완료·삭제됨). 권위 있는 현재 목록은 `ls plan/in-progress/` 를 직접 확인. 본 인덱스 본문 트리의 전면 재동기화는 별도 grooming 필요.
> 2026-05-30 감사 산출 신규 plan: `spec-consistency-findings-2026-05-30.md` (spec 일관성 결정), `spec-impl-coverage-gaps-2026-05-30.md` (spec↔구현 갭).
...
| **Spec 5-system/4-execution-engine §7.5 / §11 Durable Continuation & Graceful Shutdown** | ✅ Phase 0~3 + 변경 2.3 완료 2026-05-29 (§7.5 rehydration / Phase 3.1 DLQ 모니터링 / 변경 2.3 publisher 동기 INVALID_EXECUTION_STATE). **§11 잔여**: WS `execution.start` gate(G1) / errorPolicy `continue` on SIGTERM(G2) / `exec:cont:seq` TTL(G3) 는 `execution-engine-residual-gaps.md` 추적. spec frontmatter `partial` 전이 완료 | `complete/workflow-resumable-execution.md` + `execution-engine-residual-gaps.md` (운영 회귀 대응 — k8s 재배포 시 WAITING_FOR_INPUT 일괄 FAIL 해소) |
