...
3. **P0 재구성 (v2.3)** — 실행엔진 신뢰성(크래시 복구·처리량·backpressure·멀티테넌트 가드)은 **사용 방식과 무관하게 당연히 해결해야 하는 기반 요건**이므로 ①을 무조건 P0로 둔다(§0.1 (2-b)). 따라서 진짜 P0 = **①(엔진 신뢰성)·③(통합/확장 봉쇄)·④(spec drift)**. ②(node-as-tool, mcp_로 도메인 커버)와 ⑨(알림톡 대행사 노드)는 P1. "한국 커머스 채널=알림톡 대행사 노드"는 wedge 직결로 P1 상단. (복구/재큐는 RUNNING 한정 — waiting_for_input 무기한 보존.)
