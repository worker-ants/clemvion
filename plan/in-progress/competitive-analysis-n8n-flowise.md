...
### (2-b) v2.2 재평가 — "비즈니스 로직 실행 플랫폼" 목적 축에서 ① 재격상 🟠→🔴 (foundational)

> 본 제품의 주요 목적 중 하나는 **고객사가 코드 대신 이 제품으로 비즈니스 로직을 구성·실행**하는 것. 이 목적이면 성능·안정성·대량·분산 처리 신뢰성이 기반 요건이 된다. v2.1의 🟠 하향은 "저부하 셀프호스팅" 기준이었고, 이 목적 축에서는 **기반(foundational) P0**다. v2.1 명제("멀티 인스턴스가 안 깨진다")는 여전히 참이나, **"안 깨짐 ≠ 비즈니스 로직 실행 플랫폼급 신뢰성"** — 후자의 바가 적용된다.
...
**좋은 소식 — 재설계 아닌 "이미 설계된 것을 짓기"**: 목표 아키텍처는 spec에 이미 있음(§4 BullMQ worker·§7 heartbeat·§8 가드, Planned). 최난도 조각(rehydration·영속 continuation·분산 recovery lock)은 **이미 구현** = §4가 올라설 토대. 남은 건 증분: ① intake 큐 도입(fire-and-forget→큐, work-stealing/backpressure) / ② 크래시 시 running을 checkpoint에서 재개(rehydration을 `ai_agent` 너머 일반 노드로 확장) / ③ §8 동시성 cap·실행 타임아웃 enforce / ④ heartbeat 재큐. **build vs adopt**: BullMQ continuation을 이미 가진 상황에서 Temporal/Inngest 밑깔기는 과한 마이그레이션 → **§4를 기존 토대 위에 완성** 권고.
...
- **① 실행엔진 신뢰성 — 비즈니스 로직 실행 플랫폼의 기반 갭** (v2.2 재격상 🔴, §0.1 (2-b)) — 정확성·장기대기 내구성은 🟢이나 **실행 중 크래시 복구·처리량/수평확장·backpressure·멀티테넌트 자원 가드 4개 차원이 n8n single-mode 수준**(§4/§7/§8 Planned). 고객사가 비즈니스 로직을 올려 대량·분산 실행하는 핵심 목적을 떠받치지 못함. 아키텍처는 이미 설계됨(증분 구축 가능).

> v2.1→v2.2 트레일: ②(node-as-tool)는 도메인 액션이 mcp_로 살아있어 🟡 유지(§0.1 (1)). ①은 v2.1에서 🟠로 내렸으나, "비즈니스 로직 실행 플랫폼" 목적 축에서 v2.2에서 **🔴 foundational로 재격상**(§0.1 (2-b)) — "멀티 인스턴스가 안 깨짐"은 참이나 그것이 "플랫폼급 신뢰성"은 아니기 때문.
...
3. **P0 재구성 (v2.2)** — "고객사가 비즈니스 로직을 실행하는 플랫폼"이 주요 목적이므로 **① 실행엔진 신뢰성(크래시 복구·처리량·backpressure·멀티테넌트 가드)이 기반 P0로 재격상**(§0.1 (2-b)). 따라서 진짜 P0 = **①(엔진 신뢰성)·③(통합/확장 봉쇄)·④(spec drift)**. ②(node-as-tool, mcp_로 도메인 커버)와 ⑨(알림톡 대행사 노드)는 P1. "한국 커머스 채널=알림톡 대행사 노드"는 wedge 직결로 P1 상단.
...
### P0 — 기반 신뢰성 + 정합성 (v2.2)
- **① 실행엔진 신뢰성 구축** (비즈니스 로직 실행 플랫폼의 전제) — §4 BullMQ worker 모델 완성(intake 큐·work-stealing), 크래시 시 running checkpoint 재개(rehydration 일반 노드 확장), §8 동시성 cap·실행 타임아웃 enforce, §7 heartbeat 재큐. 기존 continuation/rehydration 토대 위 증분. + `spec/0-overview.md` "분산 워커 풀" 문구를 역량과 일치시킴(스코프 정직화).
...
- [ ] **P0-1 실행엔진 신뢰성 구축** (비즈니스 로직 실행 플랫폼 전제) — §4 worker 모델(intake 큐·work-stealing)·크래시 running checkpoint 재개·§8 동시성/타임아웃 가드·§7 heartbeat 재큐. 기존 continuation/rehydration 토대 위 증분. + `spec/0-overview.md` "분산 워커 풀" 문구 정직화
