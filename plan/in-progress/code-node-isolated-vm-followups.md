...
- [ ] **INFO — `$helpers.base64` 비문자열 일관성**: `__host_b64encode/decode` 에 `typeof !== 'string'` TypeError(hostHash 와 일관). **(spec 완료, spec PR code-followups-spec)**: ⚠ 정정 — "현 silent-string 은 spec §2.2 NOTE 로 문서화된 의도" 는 사실이 아니었음(consistency I7 — 해당 NOTE 부재). spec PR 에서 §2.2 에 "base64 비문자열 → TypeError, *유효하지 않은 base64 문자열* 은 best-effort 유지" 입력 타입 계약을 **신규 등재** + Rationale 절. **code 후속 남음**: `__host_b64encode/decode` 에 hostHash 와 동형 `typeof data !== 'string'` TypeError 가드 + 단위 테스트(비문자열→error 포트, 유효하지 않은 base64 문자열→silent 유지).
- [ ] **INFO — 메모리 한도 env**: `CODE_NODE_MEMORY_LIMIT_MB` env(기본 128, clamp ≤512MB). **(spec 완료, spec PR code-followups-spec)**: §7.2/§5.3.3 + 외부 3문서(`4-nodes/0-overview.md §5`·`conventions/error-codes.md §4`·`5-system/3-error-handling.md`) 동기화 갱신 + Rationale(128MB 상한 의도의 확장, 번복 아님). **code 후속 남음**: 모듈 상수 `ISOLATE_MEMORY_LIMIT_MB` 를 `CODE_NODE_MEMORY_LIMIT_MB` env 파싱(기본 128, clamp ≤512)으로 **교체** + backend-labels `128MB` 메시지 sync.
