...
- **[WARNING]** `APP_URL` 폴백 리터럴 `'http://localhost:3011'` 이 두 파일에 걸쳐 6회 반복
  - 위치: `backend/src/modules/integrations/integrations.service.ts:830`, `1076` / `backend/src/modules/integrations/integration-oauth.service.ts:490`, `968`, `1079`, `1359`
...
- **[WARNING]** `LAST_ERROR_MESSAGE_MAX_LEN = 200` (OAuth 서비스) vs `MCP_ERROR_MESSAGE_MAX_LEN = 2048` (MCP 에러 코드) — 두 개의 서로 다른 메시지 길이 상한 상수가 공존하며 `clampMessage` 와 `sanitizeLastErrorMessage` 두 개의 유사한 클램프 함수가 별도로 구현되어 있다
  - 위치: `backend/src/modules/integrations/integration-oauth.service.ts:193,220` / `backend/src/modules/mcp/mcp-error-codes.ts:35` / `backend/src/modules/integrations/integrations.service.ts:99`
...
- **[WARNING]** `extractSid` / `extractOperationId` 내부 파싱 로직이 `Cafe24McpToolProvider` 와 `McpToolProvider`(`parseMcpToolName`) 에 별도로 구현되어 있다
  - 위치: `backend/src/nodes/ai/ai-agent/tool-providers/cafe24-mcp-tool-provider.ts:454-468` / `backend/src/nodes/ai/ai-agent/tool-providers/mcp-tool-provider.ts:150-161`
...
- **[INFO]** `sanitizeId` (ai-agent.handler.ts)와 `sanitizeToolName` (mcp-tool-provider.ts)가 동일한 정규식 `[^a-zA-Z0-9_]`으로 동일한 변환을 수행
  - 위치: `backend/src/nodes/ai/ai-agent/ai-agent.handler.ts:138` / `backend/src/nodes/ai/ai-agent/tool-providers/mcp-tool-provider.ts:93`
...
- **[WARNING]** `console.warn` 이 NestJS `Logger` 대신 사용된 위치가 4곳 존재하며 구조화 로깅 파이프라인에서 누락된다
  - 위치: `backend/src/modules/integrations/integrations.service.ts:702` / `backend/src/modules/integrations/integration-oauth.service.ts:307` / `backend/src/modules/integrations/services/credentials-transformer.ts:45,58`
...
- **[WARNING]** `table.handler.ts`의 `safeEvaluate`에 `console.error` 3연속 호출이 남아 있다
  - 위치: `backend/src/nodes/presentation/table/table.handler.ts:264-269`
...
- **[INFO]** `node-handler.registry.ts:89`에 `console.warn`이 비-프로덕션 전용 분기에 존재
  - 위치: `backend/src/nodes/core/node-handler.registry.ts:89`
...
- **[WARNING]** `EXPIRING_SOON_INTERVAL = "INTERVAL '7 days'"` 가 SQL 내장 문자열 리터럴로 하드코딩되어 있으며 프론트엔드의 `EXPIRING_SOON_DAYS = 7` 과 주석으로만 동기화를 안내한다
  - 위치: `backend/src/modules/integrations/integrations.service.ts:250`
...
- **[INFO]** `TOOL_RESULT_PREVIEW_CHARS = 200`, `MAX_DESCRIPTION_LEN = 500`, `MAX_INTEGRATION_NAME_LEN = 80` 등 LLM 관련 길이 상수들이 여러 파일에 분산
  - 위치: `backend/src/nodes/ai/ai-agent/ai-agent.handler.ts:66` / `backend/src/nodes/ai/ai-agent/tool-providers/mcp-tool-provider.ts:35-37`
...
- **[WARNING]** `integration-oauth.service.ts` 가 1818줄이며 Cafe24 Private/Public 분기, HMAC 검증, 토큰 교환, 콜백 처리, nonce 등 다수의 책임을 단일 클래스에 담고 있다
  - 위치: `backend/src/modules/integrations/integration-oauth.service.ts` (전체)
...
- **[WARNING]** `ai-agent.handler.ts` 가 2099줄이며 단일 턴, 멀티 턴, 컨디션 평가, RAG 누적, 도구 실행, WebSocket 이벤트 등 AI 에이전트의 거의 모든 책임을 포함한다
  - 위치: `backend/src/nodes/ai/ai-agent/ai-agent.handler.ts` (전체)
...
- **[INFO]** `frontend/src/components/editor/run-results/result-detail.tsx` 1111줄
  - 위치: `frontend/src/components/editor/run-results/result-detail.tsx` (전체)
...
- **[INFO]** `errMsg` 헬퍼가 `Cafe24McpToolProvider`(private 메서드)와 동일한 패턴의 인라인 표현 `err instanceof Error ? err.message : String(err)`이 여러 파일에 반복
  - 위치: `backend/src/nodes/ai/ai-agent/tool-providers/cafe24-mcp-tool-provider.ts:544` / 다수 catch 블록
...
- **[INFO]** `requestScopes` 메서드 내 로컬 변수 `creds` 와 같은 라인의 `entity.credentials` 가 함께 사용되어 혼란 야기
  - 위치: `backend/src/modules/integrations/integrations.service.ts:817`
...
- **[WARNING]** `workflow.handler.ts:216`의 TODO가 정리되지 않은 채 잔존
  - 위치: `backend/src/nodes/flow/workflow/workflow.handler.ts:216`
...
- **[INFO]** `review-workflow.ts:716`의 TODO(`ED-AI-39 legacy fallback`)가 잔존
  - 위치: `backend/src/modules/workflow-assistant/tools/review-workflow.ts:716`
...
- **[WARNING]** `IntegrationOAuthService.begin()` 함수가 Cafe24 private/public 분기를 if-else로 깊이 중첩하여 순환 복잡도가 높다
  - 위치: `backend/src/modules/integrations/integration-oauth.service.ts:364` (함수 전체)
...
- **[INFO]** `run()` 메서드 내부의 `resolveRecipients` N+1 패턴이 수동으로 수정(`recipientsByIntegration` + `allRecipientIds`)되었으나 루프 구조가 다소 복잡하다
  - 위치: `backend/src/modules/integrations/integration-expiry-scanner.service.ts:321-376`
...
- **[WARNING]** `credentials-transformer.ts`의 `warnedMissingKey`, `warnedUnreadable` 가 모듈-수준 변수(전역 상태)로 관리된다
  - 위치: `backend/src/modules/integrations/services/credentials-transformer.ts:38-39`
...
- **[INFO]** `Cafe24McpToolProvider.__resetForTesting()` 메서드가 public API로 노출되어 있다
  - 위치: `backend/src/nodes/ai/ai-agent/tool-providers/cafe24-mcp-tool-provider.ts:388`
