...
- **[CRITICAL]** `protobufjs` 다중 CVE — 코드 인젝션·DoS·Prototype pollution
  - 위치: `backend/package.json` (간접 dep: `@google/genai`, `@opentelemetry/sdk-node`)
...
- **[WARNING]** `@opentelemetry/sdk-node` / `@opentelemetry/auto-instrumentations-node` / `@opentelemetry/exporter-prometheus` — Prometheus DoS (CVSS 7.5 HIGH)
  - 위치: `backend/package.json` — `@opentelemetry/auto-instrumentations-node: "^0.55.0"`, `@opentelemetry/sdk-node: "^0.205.0"`
...
- **[WARNING]** `fast-uri` — path traversal · host confusion (CVSS 7.5 HIGH, x2 CVE)
  - 위치: `backend/package.json` 간접 dep (`@modelcontextprotocol/sdk` → `ajv` → `fast-uri@3.1.0`)
  ...
  - 제안: `backend/package.json` `overrides`에 `"fast-uri": ">=3.2.0"` 추가. 현재 overrides에 `lodash`, `picomatch` 등은 있으나 `fast-uri`가 누락되어 있다.

- **[WARNING]** `hono` — JWT 검증 오류 · CSS 인젝션 · cross-user 캐시 누수 (MODERATE)
  - 위치: `backend/package.json` 간접 dep (`@modelcontextprotocol/sdk@1.29.0` → `hono@4.12.16`)
...
- **[WARNING]** Playwright docker 이미지(`v1.47.0-jammy`)와 `devDependencies` 버전(`^1.59.1`) 불일치
  - 위치: `docker-compose.e2e.yml` line 169 vs `frontend/package.json` devDependencies
...
- **[INFO]** `packages/expression-engine/package.json`의 `dayjs` 버전이 상위 패키지보다 낮음
  ...
  - 상세: `dayjs: "^1.11.13"` vs backend/frontend의 `"^1.11.20"`. npm 설치 시 semver range 충족이 가능한 경우 hoisting으로 같은 버전이 공유되지만, file: 링크 방식의 내부 패키지라 분리 설치될 경우 다른 인스턴스가 적재될 수 있다.
...
- **[INFO]** `p-limit@7` (pure ESM)을 NestJS CJS 환경에서 사용
  - 위치: `backend/package.json`, 사용처: `execution-engine`, `mcp-client`, `graph-extraction` 등 5개 파일
