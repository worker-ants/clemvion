...
- `backend/src/nodes/` 와 `backend/src/modules/execution-engine/` 의 경계
- `packages/` 가 backend/frontend 어디에 의존하는지

## 검토 범위

- `spec/0-overview.md`, `spec/data-flow/` — 시스템 아키텍처
- `backend/src/modules/` 의 모듈 분할
- `backend/src/common/` vs `backend/src/shared/` 의 역할 차이
- `packages/` — 패키지 경계
- `frontend/src/app/` (route group) vs `components/` vs `lib/`
...
5. **모듈 경계**: backend/modules 간 직접 import vs event/queue
6. **패키지 경계**: `packages/*` 가 backend/frontend 내부를 import 하지 않는지
