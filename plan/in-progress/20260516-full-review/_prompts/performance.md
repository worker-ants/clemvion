...
- 모든 워크플로 노드 (`backend/src/nodes/`) 가 성능 hot path
- WebSocket, execution-engine 영역의 동시성·메모리 사용

## 검토 범위

- `backend/src/modules/` — execution-engine, workflows, statistics, websocket, llm 우선
- `backend/src/nodes/` — 노드 실행 hot path
- `packages/expression-engine/` — 사용자 표현식 평가 hot path
- `frontend/src/components/editor/` — workflow editor 렌더링 성능
- `frontend/src/lib/stores/` — Zustand 류 상태 관리
