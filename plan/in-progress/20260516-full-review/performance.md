...
- **[CRITICAL]** `sanitizePayloadForWs` 가 모든 WS 이벤트 emit 경로에서 재귀 순회 실행
  - 위치: `codebase/backend/src/modules/websocket/websocket.service.ts:92-107`, `emitExecutionEvent`, `emitNodeEvent`, `emitKbEvent`, `emitBackgroundRunEvent` 호출마다
...
- **[CRITICAL]** ForEach 내부에서 `allNodes.find()` O(N) 선형 탐색이 매 iteration 반복
  - 위치: `codebase/backend/src/modules/execution-engine/execution-engine.service.ts:3679`
...
- **[WARNING]** `getSummary` 에서 `workflowId` 필터 시 동일 쿼리를 두 번 실행
  - 위치: `codebase/backend/src/modules/statistics/statistics.service.ts:80-123`
...
- **[WARNING]** `executionPath` 조회 — `ExecutionNodeLog` findAll 후 전체 nodeId 배열을 메모리에 적재
  - 위치: `codebase/backend/src/modules/executions/executions.service.ts:123-127`
...
- **[WARNING]** `deriveContainerAssignments` 가 에디터에서 모든 엣지 변경마다 최대 16 패스 순회
  - 위치: `codebase/frontend/src/lib/stores/editor-store.ts:281-304`
...
- **[WARNING]** `appendExecutionPath` 가 노드 실행 시마다 개별 `INSERT` DB 쓰기 수행
  - 위치: `codebase/backend/src/modules/execution-engine/execution-engine.service.ts:1554-1567`
...
- **[WARNING]** `sanitizePayloadForWs` 의 재귀 순회에서 빈 `result` 객체를 매번 새로 생성
  - 위치: `codebase/backend/src/modules/websocket/websocket.service.ts:98`
...
- **[WARNING]** `resolveString` 에서 `EXPRESSION_PATTERN` / `FULL_EXPRESSION_PATTERN` 정규식이 캐시 없이 매번 실행
  - 위치: `codebase/backend/src/modules/execution-engine/expression/expression-resolver.service.ts:12-13, 239-245`
...
- **[WARNING]** `execution.snapshot` 을 조회하는 `emitExecutionSnapshot` 에서 `REPEATABLE READ` 트랜잭션 + `findById` 전체 조회 실행
  - 위치: `codebase/backend/src/modules/websocket/websocket.gateway.ts:258-284`
...
- **[INFO]** `statistics.service.ts` — `TO_CHAR(…, 'YYYY-MM-DD')` 를 GROUP BY 기준으로 사용
  - 위치: `codebase/backend/src/modules/statistics/statistics.service.ts:135-154`
...
- **[INFO]** `sortByStartedAt` 가 매 WS 이벤트마다 `nodeResults` 전체를 정렬
  - 위치: `codebase/frontend/src/lib/stores/execution-store.ts:208-215`
...
- **[INFO]** `deriveContainerAssignments` 의 fixed-point 루프 상한이 16으로 고정
  - 위치: `codebase/frontend/src/lib/stores/editor-store.ts:287`
...
- **[INFO]** Cafe24 rate-limit 재시도 시 `sleepImpl` 이 Node.js 이벤트 루프를 블로킹
  - 위치: `codebase/backend/src/nodes/integration/cafe24/cafe24-api.client.ts:1016-1019`
