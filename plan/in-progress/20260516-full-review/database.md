...
- **[CRITICAL]** `integration_action_required` 알림 타입이 DB CHECK constraint에 없음
  - 위치: `backend/migrations/V001__initial_schema.sql:338`, `backend/src/modules/integrations/integration-action-required-notifier.service.ts:76`
...
- **[CRITICAL]** `AuthConfig.config` JSONB 컬럼이 암호화되지 않은 채 저장됨
  - 위치: `backend/src/modules/auth-configs/entities/auth-config.entity.ts:31`, `backend/src/modules/auth-configs/auth-configs.service.ts`
...
- **[WARNING]** V049 마이그레이션이 파일과 디렉토리가 동일 이름으로 공존
  - 위치: `backend/migrations/V049__integration_consecutive_network_failures.sql` (파일), `backend/migrations/V049__integration_consecutive_network_failures.sql/` (디렉토리)
...
- **[WARNING]** `NotificationsService.findByResource`가 workspaceId 격리 없이 `resourceId`만으로 조회
  - 위치: `backend/src/modules/notifications/notifications.service.ts:22-30`
...
- **[WARNING]** `install_token` 컬럼 길이가 스펙과 일치하지 않음
  - 위치: `backend/src/modules/integrations/entities/integration.entity.ts:62`, `backend/migrations/V042__cafe24_private_app_pending_install.sql:13`
...
- **[WARNING]** 인덱스 누락: `hasRecentByResource`의 복합 조건 쿼리
  - 위치: `backend/src/modules/notifications/notifications.service.ts:125-134`
...
- **[WARNING]** `duplicate` (Workflow 복사) 시 Nodes/Edges가 트랜잭션 없이 누락됨
  - 위치: `backend/src/modules/workflows/workflows.service.ts:171-188`
...
- **[INFO]** `AuthConfig.config` CHECK constraint 누락 — `basic_auth` 타입 추가됨에도 V001의 CHECK는 `basic_auth`를 포함
  - 위치: `backend/migrations/V001__initial_schema.sql:199`
...
- **[INFO]** `LlmConfig.apiKey` 컬럼 길이(500)가 암호화 후 실제 저장 길이를 초과할 수 있음
  - 위치: `backend/src/modules/llm-config/entities/llm-config.entity.ts:38`, `backend/migrations/V001__initial_schema.sql:306`
...
- **[INFO]** `findByResource` N+1 잠재 위험: background_run 알림 조회 후 매핑
  - 위치: `backend/src/modules/executions/background-runs/background-runs.service.ts:402`
