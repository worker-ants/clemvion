...
- **[CRITICAL]** `HooksService.verifyAuth` — HMAC 양성 케이스 전 계층 미보호
  - 위치: `backend/src/modules/hooks/hooks.service.spec.ts` (전체), `backend/test/webhook-trigger.e2e-spec.ts:133-167`
...
- **[CRITICAL]** Cafe24 OAuth callback / BullMQ refresh e2e 미존재 — 장기 회귀 위험
  - 위치: `backend/test/` (cafe24 callback/refresh e2e 파일 없음), `plan/complete/cafe24-followup-backlog.md:97-98`
...
- **[WARNING]** `HooksService` 단위 테스트에서 `constantTimeEquals` 분기 미커버
  - 위치: `backend/src/modules/hooks/hooks.service.ts:176-181`, `backend/src/modules/hooks/hooks.service.spec.ts`
...
- **[WARNING]** Cafe24 install e2e — `mall_id 불일치(recovery fall-through 후에도) → 403` 케이스 미포함
  - 위치: `backend/test/integration-cafe24-install.e2e-spec.ts:20` (목록에 명시됐으나 테스트 없음)
...
- **[WARNING]** Nonce cache Redis 키 — HMAC 앞 8자 prefix 충돌 위험 미테스트
  - 위치: `backend/src/modules/integrations/cafe24-install-nonce-cache.service.ts:108`, `backend/src/modules/integrations/cafe24-install-nonce-cache.service.spec.ts`
...
- **[WARNING]** `cafe24-token-refresh.processor.spec.ts` — `Date.now()` 를 fake timer 없이 사용
  - 위치: `backend/src/nodes/integration/cafe24/cafe24-token-refresh.processor.spec.ts:32, 48`
...
- **[WARNING]** 웹훅 e2e — `Date.now()` 기반 `endpointPath` 생성으로 경합 가능성
  - 위치: `backend/test/webhook-trigger.e2e-spec.ts:74, 95, 112, 134`
...
- **[WARNING]** `integration-cafe24-install.e2e-spec.ts` — credentials 암호화 transformer 우회
  - 위치: `backend/test/integration-cafe24-install.e2e-spec.ts:84-111`
...
- **[WARNING]** 웹훅 HMAC 긍정 케이스가 `hooks.service.spec.ts` 가 담당한다고 명시했으나 해당 spec 에 HMAC 테스트 없음
  - 위치: `backend/test/webhook-trigger.e2e-spec.ts:155` ("양성 케이스는 hooks.service.spec.ts 가 담당")
...
- **[INFO]** 프론트엔드 e2e — Cafe24 Private App 설치 흐름 미커버
  - 위치: `frontend/e2e/integrations/list.spec.ts` (목록 렌더 2개 케이스만 존재)
  ...
  - 제안: `frontend/e2e/integrations/cafe24-install.spec.ts` 를 추가해 precheck API 응답 mocking 후 UI 상태(pending badge, app URL 표시, 설치 완료 전환)를 검증한다.

- **[INFO]** `integration-cafe24-install.e2e-spec.ts` — happy path 가 `beforeEach` 로 공유된 상태에 의존
  - 위치: `backend/test/integration-cafe24-install.e2e-spec.ts:115-138`
...
- **[INFO]** 프론트엔드 스토어 테스트 — Zustand 전역 상태 초기화 패턴이 일부 파일에서 누락
  - 위치: `frontend/src/lib/stores/__tests__/auth-store.test.ts:17-19`
