...
- **[WARNING]** WebSocket 게이트웨이의 CORS 와일드카드 허용
  - 위치: `codebase/backend/src/modules/websocket/websocket.gateway.ts:52`
...
- **[WARNING]** 웹훅 HMAC `hmacAlgorithm` 필드에 알고리즘 허용 목록 없음
  - 위치: `codebase/backend/src/modules/hooks/hooks.service.ts:144`, `codebase/backend/src/modules/triggers/dto/create-trigger.dto.ts:61`
...
- **[WARNING]** DOMPurify 허용 속성 `style` 이 CSS 인젝션 벡터를 열어 둠
  - 위치: `codebase/frontend/src/components/editor/run-results/renderers/presentation-renderers.tsx:45`
...
- **[WARNING]** HTTP Request 노드 SSRF 방어: DNS rebinding 2차 공격 미차단 (문서화된 잔여 위험)
  - 위치: `codebase/backend/src/nodes/integration/http-request/http-safety.ts:8-12`
...
- **[WARNING]** Database Query 노드 — 사용자 제공 DB 호스트에 SSRF 검증 없음
  - 위치: `codebase/backend/src/nodes/integration/database-query/database-query.handler.ts:333`
...
- **[WARNING]** `protobufjs <=7.5.5` 의존성에 알려진 CVE 5건 (high severity)
  - 위치: `codebase/backend/package.json` 의존성 체인 (`@google/genai@1.50.1`, `@opentelemetry/*`)
...
- **[INFO]** bcrypt 라운드 12 사용 — 현재 수준은 적절하나 향후 검토 필요
  - 위치: `codebase/backend/src/modules/auth/auth.service.ts:25`, `codebase/backend/src/modules/users/users.controller.ts`
  ...
  - 제안: `BCRYPT_ROUNDS` 상수를 `codebase/backend/src/common/config/auth.constants.ts` 단일 파일로 통합하고 주기적 검토 일정을 문서화.

---

- **[INFO]** 쿠키 `sameSite: 'none'` 은 `secure: true` 와 함께 사용됨 — 크로스사이트 시나리오 인지 필요
  - 위치: `codebase/backend/src/modules/auth/auth.controller.ts:523`
...
- **[INFO]** `.env` 파일이 git 추적에서 제외됨을 확인 — 시크릿 코드베이스 노출 없음
  ...
  - 상세: `.env`, `codebase/backend/.env`, `codebase/frontend/.env` 모두 git 추적 대상에서 제외되어 있으며, `git ls-files` 로 확인한 결과 추적되지 않는다. `.env.example` 은 플레이스홀더(`change-me-*`) 만 포함하고 있어 실제 시크릿 노출 없음.
...
- **[INFO]** `RolesGuard` 가 `@Roles()` 없는 라우트를 자동 통과(default-allow)함
  - 위치: `codebase/backend/src/common/guards/roles.guard.ts:51-53`
...
- **[INFO]** 에러 응답에서 민감 정보 노출 방지 구현 확인됨
  - 위치: `codebase/backend/src/common/filters/http-exception.filter.ts`, `codebase/backend/src/common/utils/mask-sensitive-fields.util.ts`
...
- **[INFO]** Cafe24 OAuth HMAC 검증 및 replay 방지 구현이 적절함
  - 위치: `codebase/backend/src/modules/integrations/integration-oauth.service.ts:1210-1335`, `codebase/backend/src/modules/integrations/cafe24-install-nonce-cache.service.ts`
