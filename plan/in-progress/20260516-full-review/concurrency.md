...
- **[WARNING]** `pendingContinuations` Map: 구독 전 메시지 도착 시 누락 가능
  - 위치: `backend/src/modules/execution-engine/execution-engine.service.ts:459-526`
...
- **[WARNING]** `ScheduleRunnerService.onModuleInit`: 다중 인스턴스 중복 cron 등록
  - 위치: `backend/src/modules/schedules/schedule-runner.service.ts:107-126`
...
- **[WARNING]** `ForEachExecutor`: `context.itemContext` 를 공유 객체로 직접 mutate
  - 위치: `backend/src/modules/execution-engine/containers/foreach-executor.ts:78-83`
...
- **[WARNING]** `llmDefaultConfigCache` Map: 완료 후 정리 timing
  - 위치: `backend/src/modules/execution-engine/execution-engine.service.ts:396-411`
...
- **[INFO]** `WebsocketGateway.subscriptions` Map: 단일 인스턴스 내 스레드 안전성
  - 위치: `backend/src/modules/websocket/websocket.gateway.ts:64`
...
- **[INFO]** `Cafe24InstallNonceCache`: Redis SETNX 원자성 — 올바른 구현
  - 위치: `backend/src/modules/integrations/cafe24-install-nonce-cache.service.ts:84-91`
...
- **[INFO]** `ContinuationBusService.acquireLock` / `releaseLock`: 분산 락 구현
  - 위치: `backend/src/modules/execution-engine/continuation/continuation-bus.service.ts:195-259`
...
- **[INFO]** `ParallelExecutor`: `nodeOutputCache` shallow copy — 잔류 위험 명시
  - 위치: `backend/src/modules/execution-engine/containers/parallel-executor.ts:83-85`
...
- **[INFO]** 프론트엔드 `useExecutionEvents`: 동일 이벤트 핸들러 이중 등록
  - 위치: `frontend/src/lib/websocket/use-execution-events.ts:659,728`
