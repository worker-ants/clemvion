...
- [ ] `ai-agent.handler` 의 `processMultiTurnMessage` form bypass 분기
...
- [ ] WS payload builder — `interactionType: 'ai_form_render'` 의 `formConfig`
...
- [ ] `app/(main)/workflows/[id]/executions/[executionId]/page.tsx`
  - `isWaitingForm` 분기를 `interactionType === 'form'` 한정으로 축소.
    `ai_form_render` 는 ConversationInspector 만 렌더.
  - DynamicFormUI 의 별도 분기 제거.
- [ ] `components/editor/run-results/renderers/assistant-presentations-block.tsx`
...
- [ ] `components/editor/run-results/conversation-inspector.tsx`
