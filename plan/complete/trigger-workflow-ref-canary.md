...
> Warning 2→4 · 라벨 133→132 · INFO 3→2 · 고아 JSDoc 3→4 — **항목 7개, 발생은 8회**
> ("길이 표" 가 2회다). 일곱 번 다 원인이 같다:
...
- [x] **`/ai-review` 4라운드 (마지막)** — `review/code/2026/09/10/16_55_52` (8 reviewer =
      강제 7 + `api_contract` 재선정). Critical 0 · **6명 NONE** · maintainability/documentation LOW.
      **`codebase/` 를 안 건드린 첫 라운드**라 누적은 21건 그대로다. Warning 3 중 `plan/**` 1건은
      수정하고 `codebase/**` 주석 2건은 **정지 규칙대로 후속 등재**했다

      > **4라운드 지적 하나가 아프게 정확했다** — 내가 *"자꾸 낡는다"* 는 이유로 다시 쓴 harness
      > 트래커 요약이 **같은 세션에 §M 바로 뒤에 등재한 §N 을 빠뜨렸다.** 형태를 바꾼 편집이 그
      > 편집의 목적을 그 자리에서 배신했다. 세 번째 재발이면 산문 말고 가드로 바꾼다(후보 등재).
      >
      > **처분을 파일 위치로 갈랐다** — 게이트 freshness 는 `codebase/**` 기준이라 `plan/**` 수정은
      > 라운드를 요구하지 않는다. 정지 규칙이 막으려는 것은 *코드를 고쳐 리뷰를 무효화하는 것*이지
      > 기록을 정확히 하는 것이 아니다. `--amend` 로 author date 를 보존해 라운드 없이 코드를
      > 고치는 길도 있었지만 **쓰지 않았다** — reviewer 가 안 본 코드를 타임스탬프로 통과시키는
      > 것이라 `BYPASS_REVIEW_GUARD` 와 같은 종류다.

- [x] **수렴 판정** — 1R **동작**(vacuous 단언·identity·`null`) → 2R **구조**(고아 JSDoc·중복·순서)
      → 3R **주석의 사실성**(목록 누락·횟수 오기) → 4R **주석의 표기·성격**. 발견의 성격이 단조롭게
      얕아졌고 마지막 라운드는 6/8 NONE 이다. *"발견 0"* 이 아니라 성격이 기준이다
