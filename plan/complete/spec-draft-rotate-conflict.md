---
title: (대체됨) rotate 동시 실행의 계약 — 409 신설안. `--spec` 이 반증해 코드로 닫는 쪽으로 돌렸다
status: superseded
...
> **대체됨 (2026-09-20).** 이 draft 는 `INTEGRATION_ROTATE_CONFLICT` (409) 신설을 제안했다.
> `/consistency-check --spec` (`review/consistency/2026/09/20/16_43_05`, **BLOCK: YES**) 이 그 판단을 반증했다 —
> (1) 대상 spec 이 `status: implemented` 라 미구현 계약을 얹으면 `partial` 로 낮추고 `pending_plans` 를 달아야 하고,
> (2) 이 저장소는 **같은 형태를 이미 락으로 닫은 선례**가 둘 있는데(`plan/complete/trigger-config-lost-update.md` ·
> 같은 모듈의 재인증 `CONC H-3`) 검토 없이 건너뛰었으며, (3) 근거로 삼은 «범용 conflict 코드가 없다» 는 실측이
> 과잉 일반화였다(`RESOURCE_CONFLICT` 기본값 · `WORKFLOW_VERSION_CONFLICT` 선례를 놓쳤다 — `code:` 리터럴만 grep 해
> **필터가 만드는 층**을 못 봤다).
>
> 결론: **spec 계약을 새로 만들지 않고 코드로 닫는다.** 실제 작업은
> [`plan/in-progress/rotate-lost-update.md`](../in-progress/rotate-lost-update.md) (developer, `spec_impact: none`).
> 이 문서는 그 방향 전환의 근거로 남긴다 — 아래 본문은 **철회된 안**이며 현행 계약이 아니다.

# rotate 의 동시성 계약을 정한다 (철회된 안)
