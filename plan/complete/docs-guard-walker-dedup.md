...
- [x] 통합 여부 결정 + (하기로 하면) 구현. **각 가드의 대상 파일 집합이 통합 전후로
      ...
      > **표현 정정 (ai-review requirement WARNING ×2)**: 처음엔 "원리상 테스트로 고정할 수
      > 없다" 고 적었는데 **그건 과장이었다**. 방법은 있다 — 옛 구현을 fixture 로 박제하거나
      > `git show <pre>:<path>` 로 추출해 CI 에서 새 구현과 대조하면 된다. 실제로 리뷰어가
      > 바로 그 방법으로 **7/7 byte-identical** 을 독립 재현했다.
      >
      > **안 한 이유는 "불가능" 이 아니라 트레이드오프다**: 옛 구현을 테스트용으로 영구
      > 보존하면 이 PR 이 없애려던 중복이 **일곱 번째로 되살아나고**, `git show` 추출은
      > rebase/squash 로 이력이 바뀌면 깨지는 fragile SoT 가 된다.
      >
      > 그래서 실제로 한 것은 (a) 통합 직전/직후 7개 집합의 **일회성 dump 대조**(원소·순서
      > 까지)와 (b) 새 구현의 필터 배선을 합성 fixture 로 **forward 고정**이다. 방어는
      > 뮤테이션 8건 RED + 합성 fixture + 리뷰어 독립 byte-diff 로 충분하다고 판단했다.
...
가드에서 **영영 빠진다**. 필요조건을 둘로 잡아 거짓 음성을 없앴고(통과 11.9%, 여전히 88%
