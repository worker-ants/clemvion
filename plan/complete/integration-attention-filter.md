...
- [x] (developer) i18n dict 갱신 (ko/en parity) — 신규 키 `statusAttention`, `attentionTitlePlural`, `attentionTitleSingle`, `attentionBreakdownExpired/Expiring/Error`, `attentionClickToFilter`, `attentionClickToOpen`; 옛 키 `attentionPrefix`/`attentionSuffix`/`attentionSingle` 제거. parity 가드 (`npm test -- i18n`) 통과 확인
- [x] (developer) backend status='attention' 분기 + 단위 테스트 — `integrations.service.spec.ts` 3건 추가, 51/51 통과
- [x] (developer) frontend banner/filter/jump 구현 + 단위 테스트
  - `computeAttentionBreakdown(integrations)` 헬퍼 — `needsAttention()` 재사용해 단일 진실 유지 (impl-prep W7 해소)
  - i18n 키 `attentionBreakdownExpired/Expiring/Error` 와 함수 `computeAttentionBreakdown` 의 네임스페이스 분리 (impl-prep W6 해소)
- [x] (developer) TEST WORKFLOW (lint·unit·build·e2e) — commit 9f08fdc2 시점 통과 (backend 3682 / frontend 1402 / e2e 12 suites 66 tests)
- [x] (developer) /ai-review — `review/code/2026/05/16/13_56_39/`. BLOCK: NO, WARNING 14건 자동 fix:
  - W1 배너 카운트 페이지 범위 의미 → spec §2.4 에 "현재 페이지 한정" 명시
  - W2 e2e 부재 → `integration-attention-filter.e2e-spec.ts` 신설 (합집합·7d 경계·invalid 400)
  - W3/INFO I8·I10 mostUrgentId 우선순위·빈 배열·7d 경계 → `status-badge.test.tsx` 5건 추가
  - W5 connected + 30d hides banner → `integrations-page.test.tsx` 보강
  - W6 7d 임계값 양쪽 하드코딩 → backend `EXPIRING_SOON_INTERVAL`, frontend `EXPIRING_SOON_DAYS` 상수
  - W8 Tailwind 톤 색상 중복 → `ATTENTION_BANNER_TONE` 객체 + `AttentionBannerProps` 인터페이스
  - W10 옛 i18n 키 잔존 참조 → grep 검증, 외부 참조 0건 확인
  - W12 plan 체크리스트 미갱신 → 본 갱신
  - W13 JSDoc 부재 → `AttentionBannerProps.onActivate` JSDoc
  - W4/W7/W9 (독립 banner test / module 추출 / 날짜 helper 공유) → RESOLUTION 에 추후 처리로 기록 (단일 사용처·LOW 영향)
  - W11 (DB 타임존 UTC 가정) / W14 (복합 인덱스) → 분석·추적만 (W14 마이그레이션은 안전 가드)
- [x] (developer) RESOLUTION.md 작성 (`review/code/2026/05/16/13_56_39/RESOLUTION.md`) + TEST WORKFLOW 재수행 (backend 3682 / frontend 1409 / e2e 13 suites · 71 tests, 신규 `integration-attention-filter.e2e-spec.ts` 포함)
