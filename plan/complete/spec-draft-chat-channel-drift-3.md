...
| **신규 두 차단을 §5.4.1 표에 행으로 추가** | 그 표 제목이 *"Bot token 의 신규 등록·변경"* 이라 **주제가 다르다.** #1313 에서 `cross_spec` 이 *"bot-token 표에 signing carve-out 을 끼우면 단일 진실이 쪼개진다"* 로 같은 형태를 이미 지적했다 — 같은 실수를 반복하지 않고 §5.4.1.2 로 분리한다 |
| **`data-flow` 각주를 후속으로 미룸** | checker 가 *"필수 아님"* 이라 했지만 **그 미러는 이 체인에서 이미 두 번 drift 했다**(#1311 심음 → #1313 정정). 미루면 세 번째가 된다 |
...
| **A3** | `15-chat-channel.md` **신설 §5.4.1.2** | **표에 행을 끼우지 않고 소절을 신설한다** — §5.4.1 표 제목은 *"Bot token 의 신규 등록·변경은 single-path"* 인데 두 신규 차단은 **bot token 정책이 아니다**(`chatChannel` 필드 존재성 · `provider` 불변성). §5.4.1.2 `chatChannel` 필드·`provider` 불변성 으로 분리하고 §5.4.1 에서 forward-link. 각 항목에 **R-CC-21 · R-12 cross-link**. 레이블에 *"최초"* 를 쓰지 않는다 — 기존 373·390 행이 *"최초 트리거 생성(POST)"* 이라 **POST 성공과 PATCH 차단을 같은 시점으로 오독**시킨다 |
...
| **C5** | `data-flow/14-chat-channel.md:151` PATCH 행 | 신규 두 400 분기 각주. **이 미러가 이 체인에서 두 번 drift 했다**(#1311 이 여기 blanket 문장을 심고 #1313 이 정정) — checker 는 *"data-flow scope 상 필수는 아님"* 이라 했지만 **미루면 세 번째 drift 가 된다** |
...
- [x] `/consistency-check --spec` 2R `07_22_40` (`convention_compliance` + `cross_spec` 타겟) —
      **BLOCK: NO / Critical 0 / Warning 0.** 3축 실측이 *"전수 재현 일치"* 로 확인됐다.
      INFO 3건 중 둘을 이 턴에 반영(A3 를 소절 분리로 · C5 신설), 하나는 후속 유지
- [x] 변경안 A1~A4 · B1~B3 · C1~C5 적용 (C5 는 2R INFO 로 신설)
- [x] docs 가드 — 21파일 / **3,231 PASS**. 이 값은 `complete/` 이동 **후**, 즉 PR 이 닫히는
      시점의 값이다(이동 전 3,232 — 같은 −1: per-file 케이스가 `in-progress/` 4건 →
      `complete/` Gate C 3건). 신설 `§5.4.1.2` 앵커를 네 자리가 인용한다
- [x] **트래커 종결 — 파일·라인을 지목한다**(`plan_coherence` W2, stale 체크박스 방지):
...
- [x] **후속 신규 등재**: 서비스 가드의 `details[].code` 부재(코드 사안 — 파이프는 `INVALID_FIELD`
