...
- [x] `/ai-review` + `--impl-done spec/5-system/` — **라운드 5 에서 종료 조건 성립**
      ...
      | 4 | `11_33_23` · C0 W3 LOW | `11_33_51` · **BLOCK:YES · C1** | §J (5건) |
      | 5 | `12_00_32` · C0 W3 LOW | `12_01_01` · BLOCK:NO · C0 W3 | §K (6건) — **종착** |
...
## J. 리뷰 라운드 4 — **가드가 막으라고 만든 결함을 가드가 통과시켰다**

`review/code/2026/09/13/11_33_23` (Critical 0 · WARNING 3 · LOW) +
`review/consistency/2026/09/13/11_33_51` (**BLOCK: YES · CRITICAL 1** · HIGH).

**두 게이트가 갈렸고, 갈린 쪽이 옳았다.** code reviewer 14명 전원이 LOW 를 냈는데
consistency 의 `naming_collision` 하나가 CRITICAL 을 잡았다 — 내가 라운드 3 에 추가한 문장의
`MAKESHOP_UNRESOLVED_PATH_PARAM` 이 **방출되는 코드가 아니다**.

실측: `makeshop.handler.ts:435` 가 `throw new Error('MAKESHOP_UNRESOLVED_PATH_PARAM: …')` 로
**일반 `Error`** 를 던지고, catch(`:359`)가 `err instanceof IntegrationError ? err.code :
'INTEGRATION_CALL_FAILED'` 이므로 사용자는 공용 fallback 을 본다. 같은 파일 `:355` 주석의
D4 pre-flight 목록에도 이 이름만 없다. 형제 셋은 `IntegrationError` 첫 인자라 맞다 —
**넷 중 넷째만 틀렸다.**

### 내 가드가 왜 못 잡았나

술어가 *"존재"* 이지 *"방출"* 이 아니다. 토큰이 **에러 메시지 문자열 안에** 실재하므로 통과한다.
`#1328` 이 닫은 **오귀속 축**이 이 PR 안에서 재발한 것이고, 이 PR 의 가드는 그 축을 안 본다.

체커 권고(술어를 `.code:`/`IntegrationError(` 위치로 좁혀라)는 **프로브 후 기각**했다 —
인용 101종 중 8종이 새로 RED 인데 **6이 오탐**(env 변수 5 · 외부 어휘 1)이라 허용목록이
필요해진다. 그건 이 가드가 처음부터 피한 설계다. 다만 그 프로브가 **같은 클래스 2건**
(`CONTAINER_MISSING_EMIT`·`CONTAINER_MULTIPLE_EMIT`, `#908` 선재)을 덤으로 찾아 등재했다.

### 처분

| 지적 | 처분 |
|---|---|
| **CRITICAL** (naming_collision) | **고침** — 가이드를 실제 동작으로. 전용 코드가 없고 `message` 접두로 자리를 알 수 있다는 것을 `<Callout>` 으로. handler 를 고치는 안(B)은 동작 변경 + spec 이라 등재 |
| consistency W#1 | ~~**등재**~~ → **라운드 5 에서 실제로 등재** — 이 칸이 거짓이었다(아래) |
| ai-review W#3 | **고침** — `PROJECT.md` 가드 카탈로그에 신규 2건 등재. developer 소유 문서다. `guide-error-code-existence` 에는 **한계**(존재≠방출)도 병기 |
| ai-review W#2 | **등재** — 형제 엔드포인트의 와이어-레벨 계약 검증 부재(기존 항목은 서비스 레벨 축이라 미커버) |
| ai-review W#1 · INFO 다수 | 조치 불요 / 이미 등재분 |

가드 파일 상단에도 *"이 주석을 지우지 말 것 — 가드가 무엇을 보장하지 **않는지** 가 없으면
다음 사람이 '통과했으니 실재한다' 로 읽는다"* 를 못박았다. **이 PR 에서 실제로 벌어진 일이다.**

### 처분 표에 "등재" 라고 쓰고 등재하지 않았다 — 이 PR 에서 **세 번째**

라운드 5 의 `plan_coherence` 가 `git show`+grep 으로 실측해 잡았다. **같은 라운드의
`cross_spec` 은 "이미 등재됨" 으로 잘못 판단**했고, summary 가 실측한 쪽을 채택했다.
내가 직접 grep 하니 **0건** — 체커가 맞다.

| # | 라운드 | 어떻게 좁았나 |
|---|---|---|
| 1 | 2 | 등재 문구가 가드 **하나만** 적었다 (둘째 가드는 같은 라운드가 낳았다) |
| 2 | 3 | §2.1 **산문 표만** 겨냥하고 같은 파일 frontmatter 를 빠뜨렸다 |
| 3 | 4 | **아예 안 썼는데 "등재" 라고 적었다** |

세 번 다 형태가 같다: **처분 표를 「의도」로 쓰고 「결과」로 확인하지 않았다.** 이것은
`plan 체크박스 = 실제 상태` 규칙의 한 층 위 버전이다 — 체크박스는 내 작업 상태를,
처분 표는 **다른 파일의 상태**를 주장한다. 후자가 더 쉽게 거짓이 된다(그 파일을 안 열어도
문장은 써지니까).

**구조적 처방**: 이제 등재 스크립트가 쓴 **뒤에 그 파일을 다시 읽어 grep 으로 검증**하고,
실패하면 `assert` 로 죽는다. 산문 규율이 세 번 실패했으면 코드로 옮겨야 한다.

### 정지 규칙 재적용

라운드 4 는 **CRITICAL** 을 냈다 — "문서 성격이면 등재하고 멈춘다" 가 적용될 국면이 아니다.
고치고 라운드 5 를 돈다. 라운드 5 의 종료 조건은 라운드 2 에 선언한 것과 같다:
**`codebase/**` 수정 0 으로 끝나는 라운드**.

## K. 리뷰 라운드 5 — 종료 조건 성립

`review/code/2026/09/13/12_00_32` (Critical 0 · WARNING 3 · LOW) +
`review/consistency/2026/09/13/12_01_01` (**BLOCK: NO** · Critical 0 · WARNING 3).

**세 WARNING 이 전부 `codebase/**` 를 안 건드리고 닫힌다** — 라운드 2 에 선언한 종료 조건
(*"`codebase/**` 수정 0 으로 끝나는 라운드"*)이 여기서 성립한다.

| 지적 | 처분 |
|---|---|
| consistency W#1 — **내 처분 표의 "등재" 가 거짓** | **고침 + 전수 감사** (아래) |
| consistency W#2·W#3 | 등재 확인 — 전수 감사에서 실재 확인 |
| ai-review W#1 — `PreviewTestResultDto` 도 `code` 미선언 | **등재**. 같은 클래스 **세 번째** DTO 지만 **다른 엔드포인트**(preview-test)라 내 diff 가 닿은 자리가 아니다 |
| ai-review W#2 — MakeShop `<Callout>` 문구 패리티 가드 없음 | **등재**. 선실측 조건 병기(`${...}` 보간이라 완전 일치 불가 — **접두까지만** 대조하는 축 필요) |
| ai-review W#3 — CHANGELOG 가 라운드 0 에 멈춤 | **고침**. 루트 파일이라 리뷰 시계 안 올림 |

### "등재했다" 가 세 번 거짓이었다 — 그리고 전수 감사

`plan_coherence` 가 `git show`+grep 으로 §J 의 "등재" 를 반증했다. **같은 라운드의
`cross_spec` 은 "이미 등재됨" 으로 잘못 판단**했고 summary 가 실측한 쪽을 채택했다 —
내가 직접 grep 하니 **0건**. 체커가 맞다.
...
셋 다 **처분 표를 「의도」로 쓰고 「결과」로 확인하지 않은** 것이다. 처방 둘:

1. 등재 스크립트가 쓴 **뒤에 파일을 다시 읽어 `assert`** 로 검증한다(라운드 5 등재가 그 형태).
2. **이 PR 의 등재 주장 11건을 전수 감사** — 각 항목을 트래커 본문의 고유 문구로 대조.
   **실재 11 · 부재 0.**

### 왜 W#1·W#2 를 고치지 않았나

둘 다 고치면 라운드 6 이 필요하고, 그 라운드는 또 인접 사각지대를 찾을 것이다 — 이 PR 이
5라운드 동안 실제로 보인 패턴이다(`ModelTestConnectionResultDto` → `TestConnectionResultDto`
→ `PreviewTestResultDto` 로 클래스가 계속 번졌다). **"한 칸 더" 를 멈추는 지점이 규칙의 존재
이유다.** 두 항목 모두 *"왜 이번에 안 했나"* 와 *"착수 전 무엇을 선실측할 것인가"* 를 등재
문구에 적었다.

## 라운드 총괄

| R | `/ai-review` | `--impl-done` | 발견의 성격 |
|---|---|---|---|
| 1 | C0 W5 LOW | BLOCK:NO C0 W5 | **동작** — UI 실패 경로 무테스트 |
| 2 | C0 W3 LOW | BLOCK:NO C0 W4 | **구조** — 형제 계약 미배선 · JSDoc 자기모순 |
| 3 | C0 W3 **MEDIUM** | BLOCK:NO C0 W3 | **완전성** — spec 대비 5종 누락 |
| 4 | C0 W3 LOW | **BLOCK:YES C1** | **오귀속** — 방출되지 않는 코드 |
| 5 | C0 W3 LOW | BLOCK:NO C0 W3 | **기록** — 처분 표가 거짓 |

**두 게이트는 대체재가 아니다.** 라운드 4 의 CRITICAL 은 code reviewer 14명이 전원 LOW 를
낸 changeset 에서 consistency checker 하나가 잡았다. 라운드 5 의 거짓 등재도 code review 가
아니라 `plan_coherence` 가 잡았다.
