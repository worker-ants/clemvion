...
| 765 | `assistant.opAdded` | 노드 추가: {label} / Added node: {label} | 노드 추가: {{label}} / Added node: {{label}} | `core.ts` `interpolate()` 의 `INTERPOLATION_RE` 가 `{{name}}` 만 치환한다 — 단일 중괄호는 그대로 보인다. i18n-userguide Principle 3-C 가 이 이중 중괄호를 «기존 컨벤션» 으로 인용한다(3-C 자체는 backend 동적 메시지 조항이라 여기서는 컨벤션의 존재 근거로만 쓴다) |
...
| 773 | `assistant.executionNotInScope` | 이 실행은 현재 워크플로의 것이 아니에요. / (en 그대로) | 이 실행은 현재 워크플로우의 것이 아니에요. / (en 그대로) | 글로서리 §2 표기 «Workflow → 워크플로우»(«워크플로» 는 §5 금지어 표에 따로 없다 — 표기 규칙 위반이다). 사전은 이미 «워크플로우» |
...
### D. 같은 divider 사실을 적는 곳 둘 더 (`--spec` `08_23_02` WARNING 1)

실시간 경로의 `autoResume` 메타에는 `max` 가 있고(`assistant-store.ts` `applyAutoResumeEvent` — `{reason, attempt, max}`),
재수화 경로에는 없다(같은 파일 `hydrateMessage` — «서버 상수 변경 시 오표시를 피하려 "N번째" 포맷으로 폴백», review W-10).

- **§5.3 이벤트 표 `auto_resume` 행 (536행)**: 처리 칸의 «`autoResume={reason, attempt}` 메타 포함» → «`autoResume={reason, attempt, max}`
  메타 포함». (divider 서술 «새 버블 앞에는 `autoResumedHint` divider» 는 실시간 경로라 맞다 — 그대로)
- **§5.3.2 표 아래 문단 (570행)**: «새 row 에는 `autoResume = {reason, attempt}` 메타가 붙어 divider 렌더링(§3.2 `autoResumedHint`)의
  트리거가 된다. 메시지 목록 rehydrate 시에도 `autoResumed=true` row 앞에 동일 divider 가 자동으로 렌더된다» → «새 row 에는
  `autoResume = {reason, attempt, max}` 메타가 붙어 divider 렌더링(§3.2 `autoResumedHint`)의 트리거가 된다. 메시지 목록 rehydrate
  시에도 `autoResumed=true` row 앞에 divider 가 렌더되지만, §6 응답에 `max` 가 없어 순번만 보인다(§3.2
  `autoResumedHintShort`)». (괄호 안 §6 필드 참고는 그대로)
- 605행(«rehydrate 시 … §3.2 의 divider 를 렌더») 은 C 로 §3.2 가 두 문구를 가르게 되므로 그대로 맞다 — 고치지 않는다.
...
- **`--spec` 검토 `review/consistency/2026/09/19/08_23_02`** — **BLOCK: NO** (Critical 0 · WARNING 2 · INFO 2). 반영:
  - WARNING 1 (divider 사실을 적는 §5.3 · §5.3.2 가 남는다) → D 를 더했다. 위 원칙(한 사실을 적는 곳은 전부)을 표 밖에서도
    지키지 못했다 — 🔄 만 찾고 «`max` 유무로 문구가 갈린다» 는 사실은 찾지 않았다. 605행은 §3.2 정정으로 참이 되어 두었다.
  - WARNING 2 (비대상 두 건이 트래커에 없다) → 이 턴의 커밋에서 `plan/in-progress/spec-draft-nullable-notation-followups.md`
    에 두 항목으로 등재한다.
  - INFO 1 · 2 (근거 인용 정밀도 — «워크플로» 는 §5 표에 없음, Principle 3-C 는 backend 조항) → 표 A 근거 칸을 고쳐 적었다.
