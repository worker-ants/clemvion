...
- [x] `--spec` 1R `21_53_42` — **BLOCK: YES** (미러 3/4)
- [x] `--spec` 2R `22_04_23` — **BLOCK: YES** (파일 내부 자리 누락)
- [x] `--spec` 3R `22_14_27` — **BLOCK: YES** (`:750` + frontmatter `started`)
- [x] `--spec` 4R `22_24_30` — **BLOCK: NO** (Critical 0 / Warning 2 / INFO 8)
- [x] 변경안 A1~A14 · B1~B2 · C1~C2 적용 + 4R WARNING 2건·INFO 3건 동반 반영
- [x] docs 가드 **21파일 / 3,225 PASS** — `complete/` 이동 **후**, 즉 PR 이 닫히는 시점의 값이다
      (이동 전 3,226). 델타 −1 은 per-file 케이스가 `in-progress/` 4건 → `complete/` Gate C 3건.
      1회 RED 로 stale 앵커 1건을 잡았다(`:755`).
      §5.4.1.1 제목을 바꿔 앵커가 이동했고 인용 2곳 중 하나를 놓쳤다 — **가드가 다섯 번째로
      내 좁은 열거를 잡은 자리**다
- [x] `plan/complete/` 이동
- [ ] **구현 턴에서**: `impl-chat-channel-patch-token.md` frontmatter `created:` → `started:`
      (같은 결함) + D-2 문장에 telegram 예외 캐비아트 (4R `cross_spec` WARNING)
