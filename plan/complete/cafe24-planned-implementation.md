...
- [x] `make e2e-test` 통과 (전체 작업 종료 시 1회 — D-3 결정). 93 tests, 61s
  (Phase 4 진입 직후) + 65s (WARNING fix 후 재수행).
- [x] `/ai-review` — Critical 0건 확보. Warning 9건 중 5건 (W-1/2/5/8/9) 본
  PR 안 fix, 나머지 4건 (W-3/4/6/7) 후속 plan 분리. SUMMARY +
  RESOLUTION: `review/code/2026/05/21/08_39_39/`.
- [x] `git mv plan/in-progress/cafe24-planned-implementation.md plan/complete/`
...
- `cafe24-restricted-scopes-followups.md §3` — privacy_* 6 row prefix 결정 + rename.
  완료 시 store catalog md 의 6 row 와 `planned.ts` 의 store 배열을 비울 수 있음.
- **신규 plan** `spec-update-cafe24-catalog-drift.md` (project-planner 위임 영역):
  - consistency W-4/W-5 (14 resource Rationale 보강)
  - consistency W-6 (id 규약 갱신: `<resource>_<verb>` vs cafe24 path prefix 허용)
  - consistency W-7/W-8 (boards_setting/orders_status 유사 id rename)
  - ai-review W-3 (`order.ts` `fields` 포맷 통일 — 멀티라인 vs 인라인)
  - ai-review W-4 (`bundleproducts_create` 등 빈 `fields: {}` 의도 주석, 37+ row)
  - ai-review Info: 모듈 레벨 JSDoc 추가, batch 주석 cleanup, body schema 명시 등
- **신규 plan** `cafe24-metadata-spec-coverage.md` (testing 보강):
  - ai-review W-6 (`metadata.spec.ts` 에 "write operation with empty fields" assertion)
  - ai-review W-7 (`metadata.spec.ts` 에 "list-but-non-paginated 의도 명시" 검증)
- **신규 plan** `lint-preexisting-fixes.md` (developer 영역, 본 PR 영역 밖):
  - sessions.controller.ts (SessionDto unused)
  - executions.service.ts (no-base-to-string)
  - llm.service.ts (redundant-type-constituents)
  - node-component.interface.ts / migrate-node-output-refs.ts
  - 누적 lint: 3 errors / 19 warnings (모두 본 PR 무관, git diff origin/main 동일 확인)
