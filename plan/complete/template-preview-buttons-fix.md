...
- [x] (TDD) 실패하는 테스트 추가: Template + buttonConfig 케이스에서 글로벌 버튼이 표시되어야 함 (5건 → 24건으로 확장)
- [x] `presentation-renderers.tsx` 의 Template early-return 제거, switch 분기에 합류
- [x] `TemplateContent` 시그니처/책임 조정 (자체 Output Data 섹션 → 공통 경로 사용)
- [x] 기존 Template 테스트(`renders text/html/markdown preview`, `shows Output Data section`, `falls back to JsonContent`) 가 여전히 통과하는지 확인
- [x] frontend lint + unit test (1358 pass) + build
- [x] /ai-review 13 reviewer 병렬 실행 + summary
- [x] Warning 해소 (W1 isHttpUrl, W3 legacy previewHeader, W7 previewOnly+버튼 테스트, W8 buttonItemMap 테스트, W9 plan 갱신)
- [x] RESOLUTION.md 작성
