...
## 같은 실패가 다섯 번 났다 — 라운드 1~5 (2026-08-11)

> 절 제목을 처음엔 "라운드 2~5" 라고 썼다. 표는 1~5 (다섯 행)이고 본문은 "다섯 번" 이라
> 세는데 제목만 넷이었다 — **개수를 정확히 세라는 절이 개수를 틀렸다**(documentation
> WARNING, `16_31_02`). 라운드 1 은 위에 자기 절이 따로 있고, 여기서는 표에 함께 넣어
> 다섯 사례를 한눈에 본다.

라운드 1 의 CRITICAL 이후 이 PR 이 잡힌 것은 전부 **한 형태**다:
...
라운드 5 는 특히 기록할 값이 있다. 4라운드 처분 커밋에 나는 **"grep 으로 복제본이 정확히
2곳임을 확인했다"** 고 썼는데 **그 실측이 틀렸다** — `샘플` 이라는 **문자열**을 셌기 때문이다.
세 번째 복제본은 같은 뜻을 다른 말(`direct-load 외부 입력 방어`)로 적고 있어 grep 을 통과했다.
문자열은 의미의 프록시다. 이 저장소가 이미 등재한 교훈("실측했다가 프록시 때문에 틀린다")의
재발이다.

**개념의 뿌리는 최초 위젯 PR #384 다.** rationale_continuity 가 `git log -S` 로 확정했다 —
`configFromQuery` JSDoc·폴백 호출부 주석 **2건**과 그것을 반증하는 SDK 의 `resolveIframeTarget`
이 **같은 커밋(`a652f8733`)에서 함께 태어났고**, 라운드 4 처분 직전까지 **글자 하나 안 바뀐 채**
유지됐다. 당시 폴백 코드도 host 유무를 검사하지 않았다. 즉 **낡은 주석이 아니라 처음부터 틀린
주석**이었고, 그것이 한 달 뒤 spec 오서술로 번졌다.

> **"출처" 라는 말을 두 뜻으로 썼다.** 세 번째 복제본(`use-widget.test.ts` 의
> `direct-load 외부 입력 방어`)의 **문자열**은 #384 가 아니라 `aba46cc90`(#761,
> `safeApiBaseFromQuery` 신설)에서 처음 나온다 — copy-paste 파생이 아니라 **같은 오해를
> 독립적으로 다시 쓴 것**이다. 계보를 잇는 것은 문자열이 아니라 **오해**다. 이걸 뭉뚱그려
> 적었다가 rationale_continuity 에게 지적받았다(`16_31_08`).

파생본만 고치면 되돌아온다 — 그래서 원본 2건까지 고쳤다. 그리고 **독립 재저술은 원본을
고쳐도 막히지 않는다** — 그게 다섯 번째 사례가 존재하는 이유다.

> **다음에 같은 상황이면**: 정정을 쓰는 그 순간에 **자매 자리를 전수로 센다.** 그리고 셀 때
> 문자열이 아니라 **뜻**으로 센다 — 같은 주장을 다른 말로 적어 둔 곳이 항상 하나 더 있다.

### 유예한 항목 하나 — `api-base.ts:5`

`codebase/channel-web-chat/src/lib/api-base.ts:5` 의 주석이 이 기능을 옛 별칭
`direct-load 쿼리 하드닝` 으로 부른다(rationale_continuity `16_31_08` INFO).

**고치지 않았다.** 근거는 추측이 아니라 리뷰어의 실측이다:

- 그 문장은 **배타성을 주장하지 않는다** — "`apiBase` 는 경로를 포함하는 게 정상" 이라는
  근거로 이 기능을 **참조만** 한다. "이 경로는 direct-load 전용" 이라고 읽히지 않는다.
- 파일도 관심사도 다르다(trailing-slash 정규화). 이 PR 이 닫으려는 오독 경로가 아니다.
- 반대로 고치면 `codebase/**` 가 바뀌어 리뷰·consistency 게이트가 재무장된다 — 위험 0인
  별칭 하나에 라운드 하나를 더 쓰는 값이 없다. maintainability 도 이미 주석 총량을 더
  늘리지 말 것을 권고했다(`16_31_02`).

**다음에 `api-base.ts` 를 손댈 때 같이 고친다.** 이 문단이 그 기록이다 — `review/` 는
SoT 가 아니므로 여기 적는다.
