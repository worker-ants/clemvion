...
## 라운드 2~5 — 같은 실패가 다섯 번 났다 (2026-08-11)
...
**출처는 최초 위젯 PR #384 다.** rationale_continuity 가 `git log -S` 로 확정했다 —
문제의 주석과 그것을 반증하는 SDK 의 `resolveIframeTarget` 이 **같은 커밋(`a652f8733`)에서
함께 태어났고**, 그 사이 어떤 커밋도 서술을 바꾸지 않았다. 당시 폴백 코드도 host 유무를
검사하지 않았다. 즉 **낡은 주석이 아니라 처음부터 틀린 주석**이었고, 그것이 한 달 뒤 spec
오서술로 번졌다. 파생본만 고치면 되돌아온다 — 그래서 원본까지 고쳤다.
