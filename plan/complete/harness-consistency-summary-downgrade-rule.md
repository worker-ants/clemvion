...
- [x] **드롭된 청크에 자리 표식 — 구현 완료 (2026-08-10)**. `truncate_file_bundle` 이 이제
      드롭한 청크의 **있던 자리**에 `#### \`경로\`` + "본문 생략됨 — 컨텍스트 예산 초과
      (원래 N 자). 조립 실패가 아니라 **의도된 절단**이다" 를 남긴다. 아래 오진(이름표를
      "미치환 placeholder" 로 읽고 CRITICAL) 이 구조적으로 불가능해진다.

      구현하며 **새 결함을 하나 만들 뻔했다가 뮤테이션으로 잡았다**: 표식이 자기가 대체하는
      청크보다 **크면** 드롭이 총량을 늘려 루프가 역행한다(작은 파일이 많은 번들). 실측
      (파일 30개 × 본문 1자, 예산 900): 비용 가드가 있으면 본문 11개가 실려 883자를 쓰고,
      없으면 본문 0개에 487자 — 예산 절반을 남긴 채 실을 수 있던 11개를 버린다. 그래서
      표식은 **자기 본문보다 쌀 때만** 남긴다. 손상이 계정(30/30)이 아니라 **내용 보존**에서
      나므로, 계정만 세던 첫 테스트는 이 결함을 통과시켰다.

      테스트: `test_consistency_context_budget.py` +4건, 뮤테이션 4/5 RED
      (미살해 1건은 비용 가드의 `<`→`<=` 경계 — 길이가 정확히 같을 때만 갈려 출력이 동일하다.
      테스트 파일 docstring 에 명시해 뒀다).

- [x] **잔여 — scope 사전 확인은 미착수.** orchestrator 의 `--impl-done`/`--impl-prep` scope
      산정이 **그 경로에 실제 diff 가 있는지** 사전 확인하도록 보강. diff 0건이면 (a) 관련
      spec 을 diff 에서 역산하거나 (b) 번들을 비우고 그 사실을 프롬프트에 명시.
      위 자리 표식이 원래 증상(오진)을 닫았고 아래 diff 재배치가 "diff 가 안 보인다" 를 크게
      줄여 **우선순위가 내려갔다**. 재관측되면 그때 착수한다.

      **완료 (2026-08-31).** *"재관측되면 착수"* 라 적고 우선순위를 내렸는데, **재관측
      조건은 이 문서 안에서 이미 충족돼 있었다** — 바로 아래 2026-08-06 기록(3세션 15개
      프롬프트 전부 diff 0바이트)이 그것이다. 유예 조건을 적어 둔 문서가 스스로 그 조건의
      충족을 기록하고 있었고, 아무도 둘을 잇지 않았다.

      **처방을 (a)/(b) 어느 쪽도 아닌 곳에 놓았다.** 원안은 *"diff 0건이면 (a) spec 을 역산
      하거나 (b) 번들을 비우고 명시"* 였는데, 실측된 실패는 **diff 가 0건인 것이 아니라 있는
      diff 가 예산에 잘린 것**이다(레이블은 살아남았다). (a)(b) 둘 다 **본문**에 쓰는
      처방이고 본문이 바로 잘리는 쪽이라, 어느 것도 이 실패를 막지 못한다.

      대신 `_scope_delta_census()` 를 만들어 **head 구역**(`truncate_file_bundle` 이 drop
      후보로 삼지 않는 자리)에 실측을 싣는다. checker 는 이제 *"diff 가 없다"* 와 *"diff 가
      잘렸다"* 를 가를 수 있다 — 잘린 프롬프트 안에서 이 둘은 구분 불가였고 **반대 결론**으로
      이어진다.

      함께 실은 것: scope 델타 0일 때 *"검토 전제가 무효라는 뜻이 아니다 · 델타 0 자체로
      CRITICAL 을 내지 말 것"*. 자매 plan(`update-returning-tuple-shape.md`)이 등재한
      **같은 입력에 4라운드 중 2번 CRITICAL** 이 났던 그 추론을 겨냥한다.

      **테스트 12건 · 뮤테이션 5축 (예측 / 실측)**:

      | 뮤턴트 | 예측 | 실측 |
      |---|---|---|
      | `_count_diff_files` 선행 헤더 `+1` 제거 | RED | **RED** (5건) |
      | scope 필터 `prefix` → `scope_rel` (형제 디렉토리 누출) | RED | **RED** (1건) |
      | scope 계수를 `changed_rels` 전체로 | RED | **RED** (3건) |
      | `"예산에 잘렸다"` 문구 제거 | RED | **RED** (2건) |
      | `collect_context` 의 census 호출 제거 (배선 축) | RED | **RED** (1건) |

      - **2번 뮤턴트는 처음에 생존했다 — 테스트가 틀렸던 것이다.** 단언이 `"0개 파일"`
        이었는데 그 문자열이 scope 줄이 아니라 **diff 줄**에도 있어서, scope 필터가 깨져도
        통과했다. **숫자만 떼고 주어를 안 붙인** 탓이다. `scope(...) 델타: N개 파일` 로
        주어를 붙이니 물렸다. 테스트 주석에 남겼다.
      - **배선 축을 따로 뒀다** — 나머지 11건은 헬퍼를 직접 부르므로 `collect_context` 의
        호출부를 지워도 전부 GREEN 이다("헬퍼 테스트 ≠ 호출부 테스트").
      - 회귀: `.claude/tests` 의 `test_consistency*` **100 tests 통과**.

      **2026-08-06 재발 — 이번엔 diff 가 잘려서 사라졌다(3세션 전수 실측).**
      `--impl-done` 을 spec/conventions · spec/4-nodes · spec/7-channel-web-chat 세 영역으로
      돌렸는데, **15개 프롬프트(5 checker × 3세션) 전부에서 diff 본문이 0바이트**였다.
      실측 방법과 결과:

      - 변경 파일 28개가 프롬프트에 등장하는 횟수: **0/5** (세 세션 모두)
      - ` ```diff ` 코드펜스: **0개**, `package.json` 언급 **0회**
      - 남아 있는 것은 파일 목록 마지막 줄의 이름표
        `` `<git diff origin/main...HEAD -- code_areas>` `` 하나뿐

      **원인은 치환 실패가 아니다.** 그 문자열은 `consistency_orchestrator.py:515` 의
      `diff_label` — diff 청크에 **일부러 붙인 이름**이고, 바로 위 주석이 왜 이름을 붙였는지
      설명한다("이름이 없으면 마지막 spec 파일 청크에 얹혀 가서 예산 절단이 diff 까지 통째로
      가져가는데 누락 고지에는 spec 파일 이름만 남는다"). 즉 **이름은 설계대로 살아남았고
      본문만 예산에 잘렸다** — 이 항목이 막으려던 바로 그 증상이 관측된 것이다.

      결과적으로 checker 5명은 **구현을 한 줄도 못 본 채** "spec vs 구현" 을 판정했고, 그중
      한 명은 원인을 "orchestrator 가 payload 조립에 실패한 미치환 placeholder" 로 **오진**해
      CRITICAL 을 냈다. 이름표만으로는 "잘렸다" 와 "만들다 실패했다" 가 구분되지 않는다 —
      **드롭된 청크에는 잘린 사실과 원래 크기를 본문 자리에 남길 것**(그래야 읽는 쪽이
      원인을 오진하지 않는다).

      같은 라운드에서 나온 두 번째 오진도 함께 적어 둔다: 워크트리 디렉터리 slug 가
      브랜치명과 다르다는 이유로 "워크트리 재사용/병렬 세션 충돌" 을 CRITICAL 로 올렸는데,
      **worktree 디렉터리명은 최초 task 의 slug 일 뿐 브랜치와 일치할 의무가 없다.**
      한 워크트리에서 후속 브랜치를 파는 것은 정상 절차다.
- [x] **구현 완료 (2026-08-10)** — 두 갈래로 처리했다.

      **(1) 티어 신설 — 이 브랜치가 건드린 plan.** `spec_impact` 만 보는 대신 그 상위 신호를
      썼다. 착수 전 실측이 전제를 뒤집었기 때문이다: **in-progress plan 63개 중 `spec_impact`
      에 실제 경로를 적은 것은 6개(10%)뿐**이라 그 필드 단독 티어는 대부분의 세션에서 비어
      있다. 반면 **"이 브랜치가 변경한 plan"** 은 항상 있고, 그 plan **전문**을 보면
      `spec_impact` frontmatter 는 자동으로 포함된다(별도 파서 불요).

      기존 tier 1("어느 in-progress plan 이든 언급")이 신호이길 그만둔 것도 실측으로 확인:
      63개를 이어 붙인 755,385자가 `spec/5-system/` **18개 중 14개(77%)** 를 태그한다 —
      이 함수가 애초에 구하려던 그 스코프다. 브랜치 plan 으로 좁히면 **5개**, 그리고 그
      5개가 실제 대상이다. 디렉터리의 77% 에서 켜지면 신호가 아니다.
      티어는 0 변경 / **1 브랜치-plan 언급** / 2 그 외 plan 언급 / 3 나머지 / 4 카탈로그.

      **(2) diff 를 folder dump 앞으로.** 위 "곁들여" 항목이다. `_splice_chunk` 로 on-topic
      prefix 바로 뒤, 나머지 dump 앞에 끼운다. 실측(`spec/5-system/`, 42KB diff 주입):
      **18/19 → 6/19**. `spec/data-flow/` 는 **기본 예산 262,144 에서 diff 가 생존**한다
      (종전 실측으로는 760KB 가 필요했다).

      **남는 한계 — 정직하게 적는다.** `spec/5-system/` 은 기본 예산에서 여전히 diff 가
      잘린다. 버그가 아니라 크기다: `14-external-interaction-api.md` 한 파일이 91,651B 라
      on-topic 6개 누적 297,820B 로 예산 몫 157,286B 를 **세 번째 파일에서 이미** 넘는다.
      플랜이 가정한 "대상 2개"(1-auth 62,959 + 2-api-convention 20,095 = 83KB, +diff 42KB)
      조건에서는 예측대로 생존한다. 이번 브랜치가 plan 6개를 건드려 on-topic 이 넓어진
      경우다. **on-topic prefix 자체를 누적 크기로 상한할지**는 별도 근거가 필요해 두지 않았다.

      테스트: `test_consistency_bundle_priority.py` +6건(티어 3 + diff 배치 3),
      뮤테이션 6종 전부 RED(diff 원위치·splice 반환 무시·빈 번들 유실·티어 제거·
      브랜치 텍스트를 전체로 대체·on-topic 항상 0).
