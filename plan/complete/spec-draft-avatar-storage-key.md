...
- **trade-off**: `kbId` 는 KB 메타데이터의 FK 로 workspace 에 종속되므로 KB 쪽 워크스페이스
  ...
  **격리 대상이 아니다** — 공개 읽기가 제품 결정이다(2026-08-31 사용자 결정,
  `spec/2-navigation/9-user-profile.md §6.1`). 향후 bucket policy 만으로
...
표 아래에 **근접 명명 각주**를 단다(이 저장소의 `PASSWORD_INVALID`/`INVALID_PASSWORD` 각주
관례와 같다):

> ⚠️ `s3.publicBaseUrl` 은 **S3 전용**이며, webhook `callbackUrl` 조립에 쓰면 안 되는
> `app.publicBaseUrl`/bare `publicBaseUrl` 과 무관하다. 후자는 실제 장애(Telegram webhook
> 거부)를 낸 적이 있어 회귀 테스트가 사용을 금지한다
> (`triggers.service.spec.ts`) — canonical 키는 `app.url` 이다. leaf 이름이 같으므로
> grep 조사 시 네임스페이스를 함께 본다.
...
**C-6b. `## Rationale` § "S3 key 패턴: workspace prefix 를 두지 않는 이유"** — 제목과
둘째 문단이 **KB 를 주어로 쓴 일반화**라, Avatar 가 같은 패턴에 들어오면 "prefix 없는 키는
DB 권한 검증으로 격리된다" 로 읽힌다. **Avatar 는 애초에 격리 대상이 아니므로**(§B) 그대로
두면 자매 문서끼리 모순된다. 두 축으로 가른다:

> 워크스페이스 prefix 가 없으므로 S3 정책(`s3:GetObject` IAM condition)의 키 prefix 만으로는
> workspace 단위 격리를 강제하지 않는다. **두 영역의 사정이 다르다**:
>
> - **KB 원본 문서** — 격리가 **필요하되** 키 prefix 로 하지 않는다. workspace 격리는
>   **DB 권한 검증**으로 보장한다(키 prefix 격리는 비채택).
> - **Avatar** — **격리 대상이 아니다.** 공개 읽기가 제품 결정이라(§1.3,
>   `spec/2-navigation/9-user-profile.md §6.1`) 워크스페이스 단위로 감출 것이 없다.
>   접근 통제는 키의 UUID 추측 불가능성이 담당한다.
>
> 두 근거 모두 `spec/0-overview.md` Rationale § S3 객체 키 prefix 설계 참조.
...
> 필요하다(`spec/data-flow/4-file-storage.md §2.3`).
...
- **`plan-lifecycle.md §5` 이동 자가점검 3항목** (첫 판이 빠뜨렸다 — 이 클래스의 실패가
  저장소에 실재한다고 checker 가 지적):
  1. frontmatter `spec_impact:` 에 갱신한 **6개 spec 경로** 나열 (현재 필드 자체가 없다)
  2. `status:` 를 `in-progress` → 종결 값으로
  3. **인입 상대링크 정정** — `spec-sync-user-profile-gaps.md:79` 가
     `./spec-update-avatar-upload-implemented.md` 를 가리키므로
     `../complete/spec-update-avatar-upload-implemented.md` 로 바꾼다
- `plan/complete/` 로 이동 (위 3항목을 **먼저** 반영한 뒤)
...
> (`POST /api/users/me/avatar`, `spec/2-navigation/9-user-profile.md §6.1`).
...
**주어만 넓히면 안 된다** — 괄호 안의 근거("워크스페이스 격리는 DB 권한 검증으로 보장 —
prefix scan 비용·키 길이 절감")는 **KB 전용**이다. 그대로 두고 주어만 넓히면 Avatar 에도
그 근거가 붙어 §B 가 갈라놓은 축(KB=비용 / Avatar=소유 모델)이 도로 뭉개진다.
...
+`spec/0-overview.md` Rationale § S3 객체 키 prefix 설계
