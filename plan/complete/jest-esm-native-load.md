...
#### 같은 스펙의 두 번째 불변식 — script 의 node 인자 구간

리뷰가 별도로 짚었다(2라운드 W1): 플래그는 `jest.config.ts` 가 아니라 **npm script 문자열**에
사는데 그 텍스트를 지키는 것이 없었다. 게다가 `test:cov`·`test:watch`·`test:debug` 는
**CI·Makefile·`test-stages.sh`·docker-compose 어디서도 실행되지 않는다**(실측 grep 0건) —
`test:debug` 가 scaffold 이후 깨진 채 방치된 이유다.

**이 단언은 세 라운드에 걸쳐 같은 형태에 세 번 뚫렸다.** 그 이력이 설계의 근거다:

| 라운드 | 뚫린 자리 | 그때의 처방 |
| --- | --- | --- |
| 1 | 진입점이 `node_modules/.bin/jest`(셸 shim) | 진입점 문자열을 고침 |
| 3 | 플래그 **존재**만 보고 **순서**를 안 봄 | `flagIdx < entryIdx` 추가 |
| 4 | 플래그 순서는 보는데 `-r` 둘의 순서는 안 봄 | — **여기서 접근을 바꿨다** |

매번 「그 자리」를 하나 더 열거했고 매번 다음 자리가 남았다. 그래서 자리가 아니라 **형태**를
고정한다 — script 별로 **node 가 해석해야 하는 인자 구간 전체 + 진입점**을 선언해 두고
접두어가 글자 그대로 같은지 본다. 어떤 node 옵션이 뒤로 밀리든·빠지든·진입점이 바뀌든
한 단언이 잡는다.

**현재 가드를 고정하는 뮤턴트 — 넷 다 예측=실측 RED:**

| 뮤턴트 | 재현하는 결함 | 실측 |
| --- | --- | --- |
| M10 `-r` 를 진입점 뒤로 | 4라운드 (`Unrecognized option "r"`) | **RED** |
| M11 플래그를 진입점 뒤로 | 3라운드 (`Unrecognized option "experimental-vm-modules"`) | **RED** |
| M12 진입점을 셸 shim 으로 | 1라운드 (`SyntaxError: missing ) after argument list`) | **RED** |
| M13 접두어 없는 새 script 추가 | 인구조사 축 | **RED** |

전부 원복 후 GREEN 재확인. (중간 형태를 고정하던 M5~M9 는 그 형태가 사라져 함께 폐기됐다 —
`review/code/2026/09/24/{15_26_17,16_02_28}` RESOLUTION 에 기록이 남아 있다.)

**대가**: node 옵션을 **정당하게** 추가·재배열해도 이 단언은 실패한다. 의도한 것이다 —
테스트 실행 명령의 변경은 조용히 지나갈 일이 아니고, 실패하면 선언을 함께 고치면 된다.
