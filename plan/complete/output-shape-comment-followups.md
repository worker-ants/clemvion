...
- [x] `/ai-review` + Critical/Warning 반영 — 3라운드, 최종 Critical 0 / Warning 0 수렴 (§리뷰 라운드)
- [x] TEST WORKFLOW 4단계 (wrapper 경유) — lint/unit/build/**e2e** 전부 PASS (§TEST WORKFLOW)

## TEST WORKFLOW (2026-07-23)

| 단계 | 결과 |
|---|---|
| lint | PASS (52s) |
| unit | PASS (65s) — backend jest 8227 / frontend vitest **281 files·5667** / web-chat 48 / channel-web-chat 394 / 내부 패키지 6종 |
| build | PASS (98s) |
| **e2e** | **PASS** (246s) — backend supertest 256 + playwright **51 passed (57.9s)** |

> **e2e 면제 자가판단은 오류였다 (교훈)**: 작업 중 세 차례 "테스트-only 라 재-e2e 불요" 로 적었으나
> `PROJECT.md §e2e 면제 화이트리스트` 는 *"회색 지대(예: `*.test.ts` 만 변경 …) 도 화이트리스트가
> 아니므로 e2e 수행"*, §53 은 *"자가 판단·'변경이 작아서' 는 모두 면제 사유가 아니다"* 로 명시한다.
> `output-shape.ts` non-comment diff 0줄은 사실이지만 **면제 사유가 아니다** — 면제는 변경 set 이
> 화이트리스트의 **부분집합** 일 때만 성립하고 `*.test.ts` 는 거기 없다. 실제 실행 결과 PASS.
>
> **비-vacuity**: wrapper 요약줄 `tests=256` 은 backend jest 만 센다(PROJECT.md §38). playwright 는
> 로그의 `51 passed (57.9s)` 로 확인. 같은 이유로 unit 단계의 `tests=14` 도 frontend 미실행 신호가
> **아니다** — 로그 전수 확인 시 frontend vitest 281 files·5667 passed 로 정상 실행됐다(vitest 기본
> 리포터가 통과 파일명을 출력하지 않아 `grep output-shape` 가 0건일 뿐).

## 리뷰 라운드 (3회, 수렴)

| 라운드 | 세션 | 판정 | 반영 |
|---|---|---|---|
| 1차 | `review/code/2026/07/23/14_19_49` | C0 / **W0** / I7 | INFO 4건 — **INFO 3(안전망 구멍 H)** + 링크 오류·주석 정리 |
| 2차 | `review/code/2026/07/23/14_34_01` | C0 / **W1** / I6 | WARNING 1(JSDoc fallback 서술) + **INFO 1(안전망 구멍 I)** + 2건 |
| 3차 | `review/code/2026/07/23/14_48_38` | C0 / **W0** / I15 | comment·plan-only 2건(INFO 11 불릿 포맷·INFO 10 legend) → **수렴** |

각 라운드가 실측 재현되는 **안전망 구멍을 하나씩** 짚었고(H·I) 둘 다 닫았다 — doc-루프가 아니라
실질 수렴이었다. 3차는 Critical 0 + **코드** Warning 0 이고 남은 INFO 는 전부 양성 확인·사전
존재 갭·이미 합의된 결정이므로, 프로젝트 관례(다회 리뷰는 Critical·Warning 0 에서 INFO 비차단
수렴)에 따라 여기서 종결한다. 3차 반영분은 주석·plan 문서뿐이라 4차 리뷰를 걸면 문안 nit 만
재표면화하는 비수렴 doc-루프가 된다.

## 후속 이월 (다음에 이 함수를 편집할 때)

3차 리뷰가 남긴 **비차단** 항목. 전부 diff 범위 밖(사전 존재)이거나 실무 위험이 낮다.

- **`result.endReason: null`(명시적 null) 미고립** (3차 INFO 5) — 현재 코드에서는 `??` 가 null 과
  undefined 를 동일 취급하므로(`node -e` 실측 확인) null fixture 는 키-부재 fixture 와 **동작이
  구별되지 않는다**. `!== undefined` 류로 바꾸는 리팩터에서만 갈라지고, backend producer 도 null 을
  내지 않는다. 그 리팩터가 실제로 제안될 때 함께 넣는 게 맞다.
- **최상위 타입가드 직접 테스트 부재** (3차 INFO 6) — `isConversationOutput(null)`·배열·원시값 →
  `false` 를 고정하는 테스트가 없다. 사전 존재 갭.
- **`Array.isArray` 의 truthy-but-not-array 변형 미고립** (3차 INFO 7) — 사전 존재 갭.
- **파일 내 JSDoc 언어·포맷 혼재** (3차 INFO 9·12) — `isConversationOutput` 만 한국어·헤딩 사용.
  본 작업이 스코프를 그 함수로 명시 한정했으므로 의도된 잔여. 나머지 함수를 손댈 때 통일 검토.
...
| 라벨 | 변형 | 잡는 테스트 |
|---|---|---|
| R1 | `has(endReason ?? "completed")` — 기본값 주입 | `rejects result.messages when the endReason key is absent entirely` |
| R2 | `typeof endReason !== "string" \|\| has(...)` — 조건 반전 | 〃 |
| R3 | `typeof endReason === "string" &&` conjunct 단순 제거 | (없음 — **타입 시스템**이 차단, TS2345) |
| H | `?? (output.endReason …)` fallback 단 제거 | `detects a terminal whose endReason sits at output.endReason …` |
| I | `??` 좌우 교환 — 우선순위 역전 | `prefers result.endReason over output.endReason when both are present` |
| A | 최상위 게이트의 `conversationConfig` disjunct 제거 | `detects … bare top-level conversationConfig` |
| B | 첫 OR-항의 `hasLegacyMessages` guard 제거 | `rejects output.interactionType when output.messages is absent` |
| C | `looksLikeConversationEnd` 의 `hasResultMessages` 제거 | `rejects a whitelisted endReason without result.messages` |
| D | `isCanonicalWaiting` 의 `hasLegacyMessages` 제거 | `rejects waiting_for_input status alone without output.messages` |
| E | `outputInteraction` 분기 제거 | `detects … via output.interactionType alone` |
| F | `hasConvConfig` 분기 제거 | `detects … via nested output.conversationConfig alone` |
| G | `metaInteraction` 분기 제거 | `detects … via output.messages + meta.interactionType without status` |
