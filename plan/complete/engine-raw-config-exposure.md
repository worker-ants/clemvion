...
| Phase 7 — 정리 / 클로저 | 완료 | spec/4-nodes raw/evaluated 패턴 sample 검증 → 추가 보강 불필요 (5-system + integration spec 이 SSOT). follow-up 2건은 `plan/in-progress/engine-raw-config-followups.md` 로 분리. memory 갱신. plan complete 이동 |
...
**작업 결과**:
1. **spec/4-nodes raw/evaluated 패턴 검증** — sample grep 으로 정합 확인. `4-integration` 카테고리 (HTTP / Send Email / DB Query) 와 `0-overview.md` §4.3 가 raw/evaluated 직교성 명시. 다른 카테고리 (logic / flow / presentation) 는 expression 미사용 필드 위주라 raw 와 evaluated 동일 — 별도 명시 불필요. SSOT 는 `spec/5-system/4-execution-engine.md` §5.5 / §6.3 + CONVENTIONS Principle 7 + CHANGELOG #14~16 으로 충분.

2. **Follow-up 분리** (`plan/in-progress/engine-raw-config-followups.md` 신규):
   - AI Agent `buildMultiTurnFinalOutput` / `buildConditionOutput` 의 raw plumbing — multi-turn cache lifecycle 영향 분석이 필요하므로 본 phase 에서 분리.
   - Information Extractor `multiTurnConfigEcho` 의 raw plumbing.
   - Carousel / Table 256KB cap 정책 결정.

3. **`memory/engine-raw-config-decision.md` 갱신** — "결정" → "완료" 상태 + 모든 phase 의 PR sha 기록 + follow-up cross-link.

4. **본 문서를 `plan/complete/` 로 `git mv`** (본 commit).

**검증**: backend·frontend 모두 lint·unit·build 영향 0 (docs/메모 변경).
...
- HTTP responseHeaders sanitize 정책 — Phase 2 진입 전 hybrid blacklist + 패턴 마스킹으로 결정.
- 256KB cap 측정 단위 — UTF-8 byte length, multi-byte boundary 안전 절단으로 결정.
- Replay 시 raw 재평가 vs 저장된 evaluated 재사용 — Phase 6 에서 옵션 C Hybrid (View + Re-run 분리) 로 결정. Re-run 실제 구현은 future PRD 로 분리.

모든 phase 종료. 본 문서를 `plan/complete/` 로 이동.
