...
- [x] **선행(2차)**: 사용자 결정(2026-05-17)으로 작은 spec 위생 PR — PR #142 (merge commit 96bf35cb) — `14-execution-history.md` 자기 참조 링크 제거 + `1-data-model.md §2.10` 에 `autoRefresh` derived 가상 필드 주석 추가 (W-1 동시 처리)
- [x] spec 위생 PR merge 후 본 worktree 로 복귀 → main rebase → consistency-check 재실행 (세션 `review/consistency/2026/05/17/13_19_29/`) — **BLOCK: NO** ✓ Critical 0, WARNING 11(모두 본 PR 범위 밖 / 후속 PR), INFO 14 → 구현 진입
