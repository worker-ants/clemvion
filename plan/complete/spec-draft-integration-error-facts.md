...
- ④ `5-makeshop.md` 가 403 을 `MAKESHOP_AUTH_FAILED` 로 묶는 것을 **의도**로 적는다(§9.5 «별도 승인 티어가 없어 403 을
  scope 부족으로 세분할 근거가 없다»). Cafe24 는 403 을 `CAFE24_INSUFFICIENT_SCOPE` 로 가른다. 즉 «동일» 한 것은 401
  자가회복(재시도)과 실패 카운터 정책이지 결과 코드 분류가 아니다.
  - **연결 테스트 경로를 따로 실측했다** (④ 가 고치는 자리는 노드가 아니라 §5.9 «테스트 방법» 이므로):
    `makeshop-api.client.ts` `pingConnection` 의 403 분기는 `{ success:false, code:'MAKESHOP_AUTH_FAILED' }` 를
    **status 격하 없이** 돌려주고(401 만 재시도 뒤 `markAuthFailed`), Cafe24 의 같은 자리는
    `cafe24-api.client.ts` 가 `CAFE24_INSUFFICIENT_SCOPE` 를 돌려준다. **«격하하지 않는다» 는 정말 같고, 코드만 다르다**
    — 그래서 정정은 «403 처리 전부» 가 아니라 **결과 코드**에 한정한다.

## 변경안

### ① `1-http-request.md` frontmatter `code:` 에 두 줄
...
code:
  ...
  - codebase/backend/src/nodes/integration/http-request/http-credentials.ts   # 추가 — §4.1 · §6 (2차 W1)
  - codebase/backend/src/nodes/integration/_base/sanitize-response-headers.util.ts
```

**(`--spec` 2차 W1)** `http-credentials.ts` 도 함께 넣는다. ①을 고치려고 디렉터리를 실측하다 드러난 같은 형태의
누락이다 — `resolveHttpCredentials` 가 §4.1 `auth_type` 별 주입 표와 §6 의 `INTEGRATION_AUTH_UNSUPPORTED` ·
`INTEGRATION_INCOMPLETE` 를 그대로 구현한다(변경안 ③이 `2-navigation` 쪽에서 인용하는 바로 그 함수다).
한 줄만 고치고 나머지를 남기면 다음 사람이 «이 목록은 훑어진 적 있다» 고 읽는다.
...
- `1-http-request.md` §4 step 8 끝에 한 문장: «가드가 던진 것이 차단 판정이 아니면(가드 자체의 고장) `HTTP_BLOCKED` 가
  ...
  리다이렉트 홉에서 같은 고장이 나면 전송 catch 로 떨어져 `HTTP_TRANSPORT_FAILED` 가 된다** — 두 시점의 코드를 통일할지는
  아직 정하지 않았다.»
  **(`--spec` 2차 W2)** 그 문장에서 **트래커 파일 경로를 쓰지 않는다**. backtick 코드로 적은 `plan/…` 경로는
  `spec-link-integrity.test.ts` 의 감시망(마크다운 링크) 밖이라, 트래커가 `plan/complete/` 로 옮겨가도 아무도 모르는 채
  spec 본문에 죽은 경로가 남는다. 영구 문서에는 **«아직 정하지 않았다» 라는 사실만** 적고, 어디서 정할지는 트래커가 스스로
  안다(그 항목이 이 문장을 인용한다).
...
**(`--spec` 2차 W3)** 새 문장을 **덧붙이지 않고 기존 문장을 고친다**. 지금 §5.9 «테스트 방법» 문단 끝은

> 401 자동 회복·403 처리·transport 카운터 제외는 §5.8 정책 동일.

인데, 여기에 «403 은 다르다» 를 한 줄 더 붙이면 **한 문단 안에서 서로를 부정하는 두 문장**이 남는다. 그래서 그 문장
자체를 다음으로 교체한다:

> 401 자동 회복·transport 카운터 제외는 §5.8 정책 동일. **403 은 결과 코드가 다르다** — 상태를 격하하지 않는 것은 같지만,
> Cafe24 가 `CAFE24_INSUFFICIENT_SCOPE` 로 가르는 자리에서 MakeShop 은 `MAKESHOP_AUTH_FAILED` 로 묶는다
> ([MakeShop 노드 §9.5](../4-nodes/4-integration/5-makeshop.md#95-별도-승인restricted-scope-미도입) — 별도 승인 scope
> 티어가 없어 403 을 scope 부족으로 세분할 근거가 없다).

§5.9 첫 문단의 «공유 메커니즘(… 연결 테스트 401 자가회복)» 은 401 만 말하므로 그대로 둔다.
...
- [x] `/consistency-check --spec …` 2차 — `review/consistency/2026/09/20/15_55_38` **BLOCK: NO** (Critical 0 · Warning 3).
  셋 다 변경안에 접었다: W1 `http-credentials.ts` 동반 누락 · W2 spec 본문의 트래커 경로 인용(링크 감시망 밖) ·
  W3 §5.9 는 덧붙이지 말고 기존 문장을 교체. 요약의 권고대로 반영 전 3차 재검증을 돌린다
- [x] `/consistency-check --spec …` 3차 — `review/consistency/2026/09/20/16_12_45` **BLOCK: NO** (Critical 0 · Warning 1 ·
  INFO 8). WARNING 1(cross_spec)은 **실측으로 검증한 뒤 반영**했다: `chat-channel-adapter.md` §3.1 이
  `HTTP_TRANSPORT_FAILED` → `executionFailedThirdParty` 로 매핑하고 `15-chat-channel.md` 가 그 문구를 «외부 서비스 응답을
  받지 못했습니다» 로 정의한다 — 즉 홉의 가드 고장이 그 코드로 합류하는 동안 내부 오류가 외부 탓으로 안내된다.
  §4 step 8 새 문장에 그 파급을 한 구로 달고, 트래커의 열린 항목(코드 통일)에도 명시했다.
  INFO 처리: #4(§14.1 삽입 위치)는 `INTEGRATION_INCOMPLETE` 행 바로 아래로 반영 · #6(`HTTP_TRANSPORT_FAILED` 행 문구)은
  미루지 않고 그 행에 트리거를 적어 닫음 · #1·#2 는 트래커에 후속 항목으로 신설 · #3·#5·#7·#8 은 조치 불요
- [x] spec 네 파일 반영 — `1-http-request.md`(frontmatter 2줄 · §4 step 8 · §4.2 행 · §6 두 행) · `0-common.md` §4.2 ·
  `2-database-query.md` §6.2 · `2-navigation/4-integration.md`(§5.3 두 줄 · §14.1 행 신설+보강 · Rationale · §5.9 교체).
  새로 넣은 링크 다섯의 대상 파일·앵커 제목을 직접 대조했다(`pnpm install` 없이 — 이 워크트리는 fresh 라 vitest 미설치)
- [x] 트래커 두 항목 해소 · 후속 둘 등재 · 이 draft `plan/complete/` 로
