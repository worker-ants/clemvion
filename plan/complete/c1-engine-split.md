...
- **엔진→서비스 주입 방향 제거(caller-side 전환) + EngineDriver ISP** (PR2·PR3 architecture WARNING + 후속 ⑤ 이연) — ✅ **완료 (후속 ④, 2026-06-19, branch `engine-di-isp-2288fe`)**: **Part B(ISP)** — 단일 12-멤버 `EngineDriver` 를 소비자별 부분 인터페이스로 분해(`CoreEngineDriver`{updateExecutionStatus·contextKeyOf} → `InteractionEngineDriver`{+stageDurableResumeSnapshot, Form/Button} / `ReentryStateDriver`{buildRetryReentryState, AiTurn·Retry 공유} / `AiTurnEngineDriver`(7) / `RetryEngineDriver`(8, @internal 5 보존) / `EngineDriver extends AiTurn&Retry`(12, 엔진만 implements)); 각 서비스 driver 필드·spec mockDriver 를 slice 로 narrow(ISP 컴파일 계약 실현, tsc mismatch 0). **Part A(방향 제거)** — 착수 전 호출경로 trace 로 4방향 중 **engine→Retry 만 제거 가능** 확정(AiTurn/Form/Button 은 dispatch-loop·resume-registry 가 그래프 순회 중 위임=구조적 필수). 모듈 export RetryTurnService → 외부 진입점(websocket.gateway·continuation processor)을 RetryTurnService 직접 호출 재배선 → 엔진 thin delegator(retryLastTurn·applyRetryLastTurn) + forwardRef(()=>RetryTurnService) 주입 삭제 → 엔진 spec 18곳 redirect. 단방향 Retry→engine 수렴. **순환 견고화** — import 재배치로 노출된 `ws.service↔gateway↔retry↔event-emitter` ES-module 순환을 `ExecutionEventEmitter→WebsocketService` forwardRef 로 지연 해석. 커밋 `9027479c`(impl)+`498da767`(ai-review fix). TEST lint·build(nest)·unit(355 suites/7120)·**dockerized e2e(35 suites/205, app.module 부팅 순환 에러 0)** ✓. impl-prep **BLOCK:NO**. **전수 ai-review(--route=all, architecture·concurrency 포함)** 1차 LOW·C0·W2(spec mock narrowing·event-emitter DI)→fix, 2차 fresh **LOW·C0·W0 수렴**(전 INFO dispositioning). RESOLUTION `review/code/2026/06/19/20_36_43`+`20_50_10`. **PR #638**: https://github.com/worker-ants/clemvion/pull/638.
  - **SPEC-DRIFT 후속(planner)**: `spec/5-system/4-execution-engine.md §Rationale C-1` (a) thin delegator 잔류 기술(L1463-1464) 제거 → "외부 진입점이 RetryTurnService 직접 호출" 로 정정, (b) 단일 EngineDriver 계약(L1465·L193)을 ISP 5-부분인터페이스 계층 + 소비자별 바인딩으로 갱신. (아래 "spec §Rationale C-1 EngineDriver 멤버목록 갱신" 항과 통합 처리.)
