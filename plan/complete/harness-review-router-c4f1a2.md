...
- [x] **4a** `.claude/hooks/_lib/branch_guard.py` — 공통 판정 함수.
- [x] **4b** `.claude/hooks/guard_default_branch_edit.py` — PreToolUse hook.
...
- [x] **4c** `.claude/hooks/guard_default_branch_prompt.py` — UserPromptSubmit hook.
...
- [x] **4g** 검증 — 5 시나리오. (운영 중 3-layer 차단 동작 확인 — 본 plan-housekeeping 작업도 main branch 에서 즉시 차단되어 worktree 분리 후 진행됨, BYPASS env var 동작 확인.)
...
- [x] **5d** 검증: `grep -r 'skills/[a-z-]\+/hooks'` 결과 0 건 확인. (live 문서 기준 0 건. `plan/complete/**`·`review/**` 의 옛 경로는 시점 기록 성격으로 보존 — CLAUDE.md "인입 참조" 규약 준수.)
