...
> **v2**: Principle 1.1 적용으로 `meta.subWorkflowId` 제거. `config.workflowId` 와 중복되므로 echo 금지. `meta.recursionDepth` 는 런타임 메트릭이므로 유지. async 모드의 `output.execution.id` 는 **sub-execution ID (런타임 생성)** 로, `config.workflowId` (= **workflow 정의 ID**) 와는 의미가 다름을 명확히 합니다.
...
| 5 | 실행 메트릭이 output 안에 섞임 | **P2 (meta 는 실행 메트릭)** | `output.executionId` (async 추적용 ID가 `output`) | 추적 ID 는 도메인 데이터이지만, 실행 시작 시각 같은 메트릭은 `meta.*` 로 |
| 6 | `durationMs` 등 공통 메트릭 부재 | **P2** | `meta` 가 async 에만 존재, sync 에는 `meta` 없음 | `meta.durationMs`, `meta.recursionDepth` 를 **두 모드 모두** 에 포함 |
...
| 8 | 초안 제안의 `meta.subWorkflowId` 가 `config.workflowId` 와 중복 | **Principle 1.1 — config ↔ output(meta) 직교성** | 초안: `meta.subWorkflowId` 를 두 모드 모두에 포함 | 제거. `config.workflowId` 로만 참조. `meta.recursionDepth` 는 런타임 메트릭이므로 유지 |
...
4. **공통 `meta` 필드**: `meta.durationMs`, `meta.recursionDepth` 를 두 모드 공통으로 제공. async 는 `meta.startedAt` 추가. **`meta.subWorkflowId` 는 제거** — `config.workflowId` 로 참조 (P1.1).
5. **스키마/핸들러 키 통일**: 본 문서 범위는 output 이지만, config echo 가 신뢰성 있으려면 `paramName` / `expression` 으로 스키마를 맞추는 것을 권장 (별도 변경셋).

### 3.2. `config.workflowId` vs `output.execution.id` — 의미 구분

두 값은 **서로 다른 식별자**이며 혼동하면 안 됩니다.

| 필드 | 의미 | 타입 | 런타임? | 예시 |
| --- | --- | --- | --- | --- |
| `config.workflowId` | **호출 대상 워크플로우 정의 ID** (사용자가 UI 에서 선택한 sub-workflow). 모든 실행에서 동일. | string | No (리터럴 config) | `"wf_uuid_1234"` |
| `output.execution.id` | **이번 호출로 생성된 sub-execution ID** (async 큐 등록 시 새로 발급). 매 실행마다 다름. | string | **Yes** (런타임 생성) | `"sub-exec-async-1"` |

> async 에서 `output.execution.id` 는 `config.workflowId` 를 echo 한 것이 **아닙니다**. 큐 시스템이 새로 발급한 실행 추적 ID 이므로 Principle 1.1 에 부합합니다. 반면 `meta.subWorkflowId` 는 `config.workflowId` 를 **그대로 복사**한 값이므로 직교성 위반 → 제거합니다.

### 3.3. Before / After — `sync` 모드 성공
...
> `output.execution.id` 는 큐 시스템이 이번 호출에 새로 발급한 **sub-execution 추적 ID** 입니다. `config.workflowId` (= workflow 정의 ID) 와 혼동하지 마십시오.
...
> `output.error.details.workflowId` 는 "어떤 workflow 를 찾지 못했는가" 라는 **에러 맥락**을 제공하는 debugging payload 입니다. `config.workflowId` 와 값이 같지만 `output.error.details` 는 에러별 자유 스키마이므로 P1.1 의 "config echo" 에 해당하지 않습니다 (에러 메시지 안에 식별자를 포함하는 것이 자연스럽기 때문).
...
| `output.execution.id` | string (async 성공 시) | 비동기 실행 추적 ID (**sub-execution ID, 런타임 생성**). `config.workflowId` 와 혼동 금지. |
...
| `meta.recursionDepth` | number | 이 실행이 몇 번째 중첩인지 (0 = 최상위, 1 = 최상위의 자식). 런타임 런처가 계산하는 값이므로 P1.1 합격. |
...
> **제거된 필드**: `meta.subWorkflowId` — `config.workflowId` 와 동일 값이므로 P1.1 위반. 사용자는 `$node["X"].config.workflowId` 로 접근.
...
| async 실행 ID (sub-execution) | `$node["X"].output.executionId` | `$node["X"].output.execution.id` | **YES** |
...
| 중첩 깊이 (신규) | — | `$node["X"].meta.recursionDepth` | N/A (신설) |
| sub-workflow 정의 ID | `$node["X"].config.workflowId` | `$node["X"].config.workflowId` | NO (P1.1 적용 — `meta.subWorkflowId` 는 도입하지 않음) |
...
CONVENTIONS **Principle 1** 은 "output 은 비즈니스 데이터만" 을 명시하며, **Principle 9** 는 container 노드의 최종 output 모양을 고정된 스키마(`{ iterations }`, `{ items }`, `{ branches }`) 로 통일합니다. `workflow` 는 container 카테고리(`isContainer: true`) 이면서도 현재 유일하게 **스키마 없이 raw 결과를 루트에 노출**합니다.
...
### 5.5. 왜 공통 `meta` 필드가 필요한가 (그리고 `meta.subWorkflowId` 를 왜 제거하는가)

현재 sync 는 `meta` 자체가 없고 async 만 `{ status }` 를 가집니다. 실행 성능 대시보드나 디버거 관점에서 sub-workflow 호출은 **지연의 가장 큰 후보** 이므로, `meta.durationMs` 는 필수적입니다. `meta.recursionDepth` 는 타임라인 UI 에서 중첩 실행을 시각화할 때 이미 엔진 내부 정보로 존재하는 값(런타임 런처가 스택에서 계산)이므로 echo 비용이 거의 없고, 실행 시점이 되어야 알 수 있는 값이므로 P1.1 합격입니다.

반면 초안에 포함되어 있던 `meta.subWorkflowId` 는 **`config.workflowId` 의 값을 그대로 복사**한 것에 불과합니다 — "이 값을 알기 위해 노드를 실제 실행해야 하는가?" 라는 P1.1 식별 기준에 "아니오" 로 답하게 되어 **직교성 위반**입니다. 따라서 **제거**합니다. 사용자가 실행 ID 와 함께 대상 workflow 정의 ID 를 보고 싶을 때는 `$node["X"].config.workflowId` 로 직접 참조합니다. 대시보드/로그 파이프라인이 `meta` 만 보고 sub-workflow 식별자를 모으고 싶다면, 수집 레이어에서 `config.workflowId` 를 함께 읽도록 하면 충분합니다.
...
반면 async 쪽 변경(`output.executionId` → `output.execution.id`, `meta.status` → `status`) 은 async 사용 패턴이 적고, 주로 `executionId` 하나만 꺼내 쓰므로 실제 영향은 제한적입니다. `meta.subWorkflowId` 는 애초에 초안에만 존재했을 뿐 현재 출시된 핸들러에 없으므로, 이 필드를 제거해도 기존 워크플로우에 영향은 없습니다.
...
- **sync**: `output.result` 로 1단 래핑, `status: 'ended'`, `meta.durationMs`/`recursionDepth` 포함.
- **async**: `output.execution: { id, startedAt }` (여기서 `id` 는 **sub-execution ID**, `config.workflowId` 와 다름), `status: 'started'` 최상위로, `meta` 는 동일 공통 필드 + `startedAt`.
- **에러**: `port: 'error'` + `output.error: { code, message, details? }` 도입. pre-flight 는 throw 유지.
- **제거**: `meta.subWorkflowId` — `config.workflowId` 와 중복이므로 P1.1 위반. `config.workflowId` 로 참조.
