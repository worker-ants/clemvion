...
> 인덱스: [README.md](../../in-progress/refactor/README.md). Critical 4 / Major 7 / Minor 4 — **spec 대조(2026-06-10) 후 유효 12건 / 철회 3건(M-3 진단오류 · C-3·M-4 2026-07-01 사용자 철회)**. M-2 는 진단 방향 정정됨.
...
- [x] **실질 종료 (2026-07-02) — 고가치 클러스터 완료, 잔여 defer (사용자 결정)**: C-1 엔진분할(02 C-1 완료)로 대상 파일이 재편돼 원안 "50+ 1파일" 이 아니라 **~124건 / ~15파일** 로 분산(원 라인 refs `:370-371` 등 stale). 사이트별 성격이 달라 일괄 전환 불가 — **SAFE-TORECORD**(property-접근 전용)·**STORE-PRESERVE**(저장/보존, 배열 보존 필요)·**LOAD-BEARING**(throw 전제)·**RESUME-STATE**(§7.4 스키마 필요)로 분류. **고가치 구조 단언(RESUME-STATE 3종 named type + retry/resume 경로 + 스키마 enrich + relay 통일)은 PR #782/#783/#784/#785 + 본 PR 로 종료**. 잔여(STORE rawConfig·domain enrich·LOAD-BEARING·미스코핑 파일)는 저가치·게이트·미지 스코프라 **defer** (아래 "잔여 defer" — 재기 조건부). 사용자 결정(2026-07-02): 여기서 M-7 마무리.
  - **첫 클러스터 (PR #782, 머지)**: `utils/to-record.ts`(`isRecord`/`toRecord` behavior-preserving 가드 — 배열·원시값→`{}`, `(x as Record) ?? {}` 안전 대체) + 단위테스트(7). `execution-engine.service.ts:1478`(cachedMeta, property-접근 전용) 1건 전환 — 이 파일 26건 중 **유일한 SAFE-TORECORD**(나머지 25 = STORE 10·LOAD-BEARING 11·RESUME 3, 제외).
  - **RESUME-STATE 클러스터 (본 PR)**: `utils/resume-state.schema.ts` 신설 — `_resumeState`/`_resumeCheckpoint`/`_retryState` 3종의 형태 SoT 를 **zod 스키마**(I-5 credential allow-list executable 문서화 + I-8 라이프사이클 구분)로 정의하고 `z.infer` 로 `ResumeState`/`ResumeCheckpoint`/`RetryState` 타입 파생. **behavior-preserving (assertion-only)** — §7.5 graceful-reset 의 malformed/partial 허용 semantics 를 바꾸지 않도록 런타임 경계에서 parse 하지 **않고**, 스키마는 (a) allow-list 문서화 (b) 타입 파생 (c) 단위 테스트 oracle 로만 사용(사용자 결정 2026-07-02). 구조 단언 **6곳** 전환: `handler-output.adapter.ts`(가드된 `_resumeState`→`isRecord` 재사용)·`ai-turn-orchestrator.service.ts`(resumeState fallback·nextResumeState→`ResumeState`)·`retry-turn.service.ts`(`_retryState` 읽기 2곳→`RetryState`)·`execution-engine.service.ts`(`_resumeCheckpoint`→`ResumeCheckpoint`). 테스트: `resume-state.schema.spec.ts`(allow-list/라이프사이클 불변식 `.strict()` 검증) + `execution-engine.service.spec.ts` 실 checkpoint 경로 builder↔schema drift 가드 2곳(`.strict().safeParse` + credential 부재 단언). 검증: lint·build(tsc)·unit **7521** PASS · **e2e 225 PASS (37 suites, 96s — 2026-07-02 레지스트리 회복 후, behavior-preserving 무회귀 확인)**. ai-review `review/code/2026/07/02/11_59_12`(Risk NONE→LOW, Critical 0·Warning 1→W-1 drift 가드 strict 화 fix·RESOLUTION) + fresh 재검 `review/code/2026/07/02/12_11_07`.
  - **ai-turn-executor 클러스터 (본 PR)**: #783 의 `ResumeState`/`RetryState` 명명 타입을 `ai-turn-executor.ts` 에 처음 도입. retry/resume-state 경로 3메서드의 `as Record`/`as number`/`as unknown[]` 단언 제거(behavior-preserving — `??` semantics 불변): `buildRetryState(source: ResumeState): RetryState`(pendingFormToolCall·totalThinkingTokens·knowledgeBases·ragSources·mcpServers 5곳)·`buildMultiTurnFinalOutput(retryStateSource?: ResumeState)`·`endMultiTurnConversation`(공개 인터페이스=IE 공유라 param 은 Record 유지, 본문 `const s = state as ResumeState` 로 좁힘 → turnCount·tokens·toolCalls·ragSources). `model`·`rawConfig`·`ragLastDiagnostics`·`allPresentations` 는 스키마상 `unknown` 이라 domain 캐스트 유지(스키마 필드타입 enrich 는 별건 defer). 동반: `to-record.ts` JSDoc caveat(#782 ai-review INFO — class 인스턴스·`Object.create(null)` 도 true) + 문서화 테스트 4건. 검증: lint·build(tsc)·unit **7523**·**e2e 225(37 suites)** PASS. ai-review `review/code/2026/07/02/13_08_49`(Risk NONE, Critical 0·Warning 2→W-1/W-2 `_retryState` 필드 운반 회귀 테스트 fix·RESOLUTION) + fresh 재검 `review/code/2026/07/02/13_21_27`(전 8 NONE) · impl-done `review/consistency/2026/07/02/13_09_40` **BLOCK:NO**.
  - **스키마 enrich 클러스터 (본 PR)**: #783 `resume-state.schema.ts` 의 z.unknown()/z.array(z.unknown()) 일부 필드를 **`z.custom<T>()` 로 enrich** — 런타임 validator **미추가**(zod v4: predicate 없으면 identity `()=>true`, 모든 값 통과), `z.infer` 타입만 concrete domain 으로 sharpen. §7.5 graceful-reset "런타임 미검증" 계약(#783) 불변(rationale-continuity 소스 레벨 검증). enrich 3필드: `messages`→`z.array(z.custom<ChatMessage>())`(배열검사 동일 강도)·`allPresentations`→`z.custom<PresentationPayload[]>()`·`turnDebugHistory`→`z.custom<unknown[]>()`. `model`·`rawConfig`·`conversationThreadRef`·`temperature` 등은 진짜 dynamic 이라 unknown 유지(사용자 결정 2026-07-02 "측정 enrich"). enrich 로 풀린 `ai-turn-executor.ts` domain 캐스트 **9곳** 제거(state 재할당 없음 → `const resumeState = state as ResumeState` 지역 narrow): endMultiTurnConversation(3, #784 `s`→`resumeState` 리네임)·handleMultiTurnConditionRoute(2)·processMultiTurnMessage(4: ragSources·turnDebugHistory·allPresentations×2). 잔존: `model`·`ragLastDiagnostics`(local 타입)·`rawConfig`·2440 messages spread(undefined-assert)·number 필드 존재전제. 검증: lint·build(tsc)·unit **7524**·**e2e 225(37 suites)** PASS. ai-review `review/code/2026/07/02/15_09_45`(Risk LOW, Critical 0·Warning 2→W-1/W-2 non-default 값·재개 누적 회귀 테스트 fix·RESOLUTION) · impl-done `review/consistency/2026/07/02/15_09_45` **BLOCK:NO**(rationale-continuity: z.custom=런타임 무검증 확인).
  - **relay 통일 클러스터 (본 PR, M-7 종료)**: `ai-turn-executor.ts` 잔여 state 소비 private 헬퍼 `buildAiNodeRefFromState`/`threadHolderFromState` param 을 `ResumeState` 로 통일(ai-review 반복 INFO "과도기적 비일관성" 해소) — `nodeId` domain 캐스트 1곳 제거, `rawConfig`/`conversationThreadRef` 는 스키마상 unknown 이라 캐스트 유지. `Record<string,unknown>` 가 `ResumeState`(`.catchall(z.unknown())`)에 assignable 이라 **call site(~19곳) 무변경**(compiler 확인). `narrowResumeState(state): ResumeState` 헬퍼 신설로 흩어진 `state as ResumeState` 3곳 통합(ai-review INFO). behavior-preserving(assertion/no-op 캐스트). 검증: lint·build(tsc)·unit **7526**·**e2e 225(37 suites)** PASS. ai-review `review/code/2026/07/02/<hh_mm_ss>` · impl-done `review/consistency/2026/07/02/<hh_mm_ss>`.
  - **잔여 defer (재기 조건부 — 사용자 결정 2026-07-02, M-7 종료)**: 아래는 저가치·게이트·미지 스코프라 M-7 에서 제외하고 필요 시 새 티켓으로 재기.
    - **STORE-PRESERVE `rawConfig`**: `state.rawConfig as Record` 다수. 스키마 `rawConfig` 타입화로 제거 가능하나 **진짜 dynamic node config** 라 unknown 유지가 옳음 → 사실상 영구 보류.
    - **domain enrich 잔여**: `model`(→string, 단 `metadata.model:string` undefined-friction) · `ragLastDiagnostics`(local `RagDiagnostics` 를 shared 로 추출해야 스키마 enrich 가능 — 별도 리팩터).
    - **LOAD-BEARING**: orchestrator error-extraction(`err as {code?}` 등)·`payload.type` guard·engine finalizeAiNode error 읽기 — 이미 `typeof` 가드된 방어 경로라 `isRecord` 전환 이득 작고 에러 분류 회귀 위험.
    - **2440 legacy** `state.messages as ChatMessage[]` spread — undefined-assert 이중 역할이라 enrich 로 자동 제거 불가.
    - **미스코핑 파일**: 원안 ~124건/~15파일 중 execution-engine + ai-agent 외 나머지 파일은 전수 인벤토리 미실시 — 재개 시 Explore 재스코핑 선행.
    - **planner/별건**: 신규 util(`resume-state.schema.ts`/`to-record.ts`)을 관련 spec `code:` frontmatter 추가(convention INFO)·`to-record.ts` → `src/common/utils` 승격 검토.
  - _(invariant SoT: impl-prep `review/consistency/2026/07/01/16_32_55` **BLOCK:NO** + I-1 `status` 유니온·I-5 credential allow-list·I-6 §4.4 sink 추상화 금지·I-7 raw/resolved 분리·W-2/W-3 에러코드·버전상수 통합 금지.)_
