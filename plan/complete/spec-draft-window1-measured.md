...
- `spec-link-integrity.test.ts` **19/19 통과**. 새 cross-doc 앵커(`15-chat-channel.md` → `2-trigger-list.md#3-api`)를
  일부러 깨서 **RED(`[ANCHOR] … 15-chat-channel.md:371`) 확인 → 원복**.
- 트래커 planner 항목 `[x]` + 3행 완료 표시 + (a)(b) 기록 — 이 draft 의 `complete/` 이동과 **같은 커밋**.
...
> **승격 시 주의**(`--spec` INFO#3): 이 500 을 전용 코드로 승격하기로 하면 `spec/5-system/3-error-handling.md`
> 카탈로그 등재가 함께 필요하다.

### `--spec` 처분 (`review/consistency/2026/09/17/16_06_23` — **BLOCK: NO** · WARNING 0)

| # | 처분 |
|---|---|
| INFO#4 «404» 와 «500» 이 서로 다른 하위 창인데 한 문단에 섞이면 오독 여지 | **수용** — A3 에 «PATCH 에서 트리거 부재는 두 하위 창으로 갈린다» 를 넣었다 |
| INFO#3 500 승격 시 카탈로그 등재 필요 | **수용** — 위 Rationale 에 한 줄 |
| INFO#5 자매 e2e `trigger-config-lost-update.e2e-spec.ts` 의 `code:` 등재 여부 | 이번 범위 밖 — §3 이 그 파일을 근거로 인용하게 되면 그때 등재 |
| INFO#1·#2·#6·#7 | 조치 불요 |

> **번들 누락 보정을 했다** — 프롬프트가 두 대상 문서 본문을 거의 싣지 않아(`15-chat-channel.md`
> 고유 헤딩 5 checker 전부 0회) «대상 spec 과 근거 코드를 절대경로로 직접 Read» 절을 붙였다.
> cross_spec·convention_compliance 가 코드 사실(부분 `save` · 404 호출부 2곳 · 23502/23503)을 직접
> 대조해 일치로 보고했다.
