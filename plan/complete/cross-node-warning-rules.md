...
`parallel-p2-followups.md` 의 결정 D (concurrency clamp 의 frontend canvas 사전 경고) + E (중첩 깊이 검증의 3중 가드 = 저장 시점 reject + canvas 사전 경고 + runtime reject) 는 부모-자식 노드 cross 평가 인프라를 요구한다.
