...
owner: developer
status: complete
# 인덱스 + 배경 기록 문서 — 어떤 spec 의 `code:` glob 에도 매칭되지 않는다.
spec_impact: none
---
...
> | 티켓 | 원 섹션 | 처분 |
> | --- | --- | --- |
> | [`harness-push-detection-env-value-concat.md`](harness-push-detection-env-value-concat.md) | §L (차단성) | ✅ 해소 (#1009) |
> | [`harness-checks-paths-coverage-guard.md`](harness-checks-paths-coverage-guard.md) | §I | ✅ 해소 (#1010, 6번째 누락 포착) |
> | [`harness-consistency-bundler-budget.md`](harness-consistency-bundler-budget.md) | §H | ✅ 해소 (#1009/#1010) |
> | [`harness-gate-control-flow-extract.md`](harness-gate-control-flow-extract.md) | §K | ✅ won't-do (#1011, 전제 붕괴) |
> | [`harness-guard-residual-hygiene.md`](harness-guard-residual-hygiene.md) | §A 잔여 · §F W6 | ✅ 해소 (#1009) |
> | [`harness-mermaid-install-concurrency.md`](harness-mermaid-install-concurrency.md) | §G (조건부) | ✅ won't-do (조건 미충족) |
>
> **종결 (2026-07-24)**: 6개 티켓 전부 닫혔다 → 본 문서를 `plan/complete/` 로 이동한다
> (자신의 종결 조건 충족). §K·§G 는 won't-do(전제 붕괴 / 조건 미충족), 나머지는 해소.
> §I 는 신설 가드가 첫 실행에서 예측된 **6번째 누락**(`.claude/config/**`)을 실제로 잡았다.
> 실질 작업 잔여 없음.
...
**→ 이관·종결**: [`harness-consistency-bundler-budget.md`](harness-consistency-bundler-budget.md) — 해소 근거는 그 티켓이 정본이다.
본 섹션은 배경·측정 기록으로 보존한다.
...
**→ 이관·종결**: [`harness-checks-paths-coverage-guard.md`](harness-checks-paths-coverage-guard.md) — 해소 근거·경계 정의는 그 티켓이 정본이다.
...
**→ 이관·종결**: [`harness-push-detection-env-value-concat.md`](harness-push-detection-env-value-concat.md) — 해소 근거는 그 티켓이 정본이다.
본 섹션은 배경·측정 기록으로 보존한다.
...
**→ 이관·won't-do 종결**: [`harness-gate-control-flow-extract.md`](harness-gate-control-flow-extract.md) — 재도출 결과
...
- (이관·**종결**) **L** → [`harness-push-detection-env-value-concat.md`](harness-push-detection-env-value-concat.md)
  — 해소. 부수로 **같은 alternation 의 살아있는 ReDoS** 도 제거됐다(286바이트 명령이 push 훅을
  15초 이상 정지시켰다 — 동기 게이트라 세션 정지 또는 타임아웃 fail-open)
- (이관·**won't-do 종결**) **K** → [`harness-gate-control-flow-extract.md`](harness-gate-control-flow-extract.md)
...
- (이관·**종결**) **I** → [`harness-checks-paths-coverage-guard.md`](harness-checks-paths-coverage-guard.md)
  — 가드 신설. 경계 정의(모듈레벨 상수·tracked 파일·product 제외)가 작업의 절반이었고,
  첫 실행에서 예측된 **6번째 누락**(`.claude/config/**`)을 잡아 등재까지 마쳤다
- (이관·**종결**) **H** → [`harness-consistency-bundler-budget.md`](harness-consistency-bundler-budget.md)
...
### `complete/` 이동 (2026-07-24) — 이제 옮긴다

한동안 in-progress 에 남겨둔 이유는 두 가지였다: (1) 자식 티켓 6곳이 이 문서를 부모로
링크해 이동 시 dangling 위험, (2) 이 파일이 병렬 세션 충돌이 잦아 이동이 conflict 표면이
컸다. **이제 둘 다 해소됐다** — 6개 자식이 전부 닫혀 같은 커밋에서 링크를 함께 정리할 수
있고(자식들은 이미 complete/ 에 있으니 same-dir 로 재배선), 백로그가 드레인돼 병렬 작업이
없다. 자신의 종결 조건("6개 티켓이 모두 닫히면 이동")이 충족됐으므로 본 문서를
`plan/complete/` 로 옮긴다.
