...
| 기존 `inboundSigning` 입력 가드 처리 | 같은 DTO line 144-155 | **(b) 채택 — legacy `@IsEmpty` 유지** + 신규 `inboundSigningPlaintext` 필드 추가. 두 필드명 공존이지만 (a) 완전 제거 시 외부 클라이언트가 잘못 보낸 `inboundSigning` 입력이 silent strip 되어 사용자가 secret 미저장 사실을 인지 못할 위험. `@IsEmpty` 차단 + hint 메시지 "provider-issued 입력은 inboundSigningPlaintext 사용" 으로 명시. ai-review documentation-reviewer W-13 정정 |
