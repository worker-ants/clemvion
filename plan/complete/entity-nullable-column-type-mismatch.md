...
- [x] **후속 — 관계 데코레이터 동명 충돌 캐너리** (리뷰 10R INFO#12) — **완료.**
      `[대조군]` 2건 추가: 관계끼리의 충돌 · `@Column` 과 관계가 섞인 충돌.

      > **착수 전 프로브로 전제를 먼저 확인했다 — 코드는 이미 옳았다.** 저장소의 관계 충돌
      > 3건(`integration`·`trigger`·`user`)이 전부 판정에서 제외돼 있었다. 없던 것은
      > 캐너리뿐이라, 이 테스트가 하는 일은 "고친 것을 지키는" 게 아니라 **이미 옳은 동작이
      > 조용히 갈라지지 않게 고정하는** 것이다.
      >
      > **뮤테이션 — 신규 대조군을 죽이는 것은 하나뿐이다.** 충돌 배제를 빼면 **3건 RED**
      > 이고 **이름으로 확인하니 신규 관계 대조군 2건 + 기존 `@Column` 대조군**이다.
      >
      > > **초판은 여기에 "관계 데코레이터만 빼면 2건 RED(관계 대조군만)" 이라고도 적었다 —
      > > 틀렸다** (리뷰 2R W1). 그 뮤턴트가 죽이는 2건은 신규 대조군이 아니라 **기존 테스트**
      > > (`관계(@ManyToOne + @JoinColumn)도 포함` · `관계 필드를 겨눈 캐스트도 잡는다`)이고,
      > > 신규 대조군은 그 뮤턴트에서 **공허하게 GREEN** 이다(필드가 어느 쪽이든 `widened` 에
      > > 안 들어간다). **"2 failed" 를 보고 어느 둘인지 확인하지 않고 단정했다** — 리뷰어가
      > > 독립 재현으로 잡았고, 내가 다시 재현해 이름으로 확인했다.
      > >
      > > 검증 자체는 유효하다 — 신규 대조군은 뮤턴트 1이 이름으로 죽인다. **틀린 것은
      > > 귀속 서술뿐**이다.
      >
      > **유예: `@OneToOne` 관계끼리의 동명 충돌은 캐너리를 만들지 않았다.** 저장소에 실사례가
      > **0건**이라 만들면 실재하지 않는 형태를 고정하게 된다(3라운드 연속 같은 판단).
      > 생기면 이 절의 `[대조군]` 에 `it.each` 로 더한다.
      >
...
- [x] **후속 — 넓혀진 필드를 겨눈 낡은 `.spec.ts` 캐스트 가드** — **완료.**
      ...
      엔티티에서 `| null` 필드명을 전수(관계 포함)로 뽑아 `.spec.ts` 의
      ...
      > **판정은 필드 *이름* 단위다 — 그래서 동명 충돌을 뺀다.** 한 엔티티는
      > nullable 이고 다른 엔티티는 non-null 인 이름(`userId`·`workflowId`·`triggerId` 등)에서
      > non-null 쪽의 **정당한** 캐스트를 오탐하기 때문이다(최소 픽스처로 재현). 판정
      > 대상이 그만큼 줄어든다(**2026-09-04 실측 135 → 115**). 재현율을 잃는 대신 오탐을
      > 0으로 유지한다 — 처방이 "지워라" 라서
      > 틀리면 사람이 코드를 깨뜨리는 방향이다. 실제로 제거했던 4건은 전부 충돌 밖이다.
      >
      > **이건 바로 앞 PR 에서 내가 반증한 실패 모드를 그대로 재도입한 것이다** — DTO 축의
      > "48건" 중 44건이 이름 매칭 오탐임을 확인해 놓고 같은 판정을 새 가드에 썼다(리뷰 2R W1).
...
## 한 자리만 고치는 버릇 — 이 plan 에서 일곱 번 반복했다

| # | 무엇 | 어떻게 드러났나 |
|---|---|---|
| 1 | 배치 1 RESOLUTION 이 "plan 이 추적한다" 고 썼는데 그 자리가 없었다 | 리뷰 |
| 2 | 배치 말 캐스트 훑기를 *그 배치가 넓힌 필드*로만 돌렸다 | plan 정리 중 실측 |
| 3 | `ipWhitelist` 를 고치고 그 위 "고치지 않았다" 문단을 그대로 뒀다 | 리뷰 |
| 4 | `"20건"` 을 한 자리만 지워 **깨진 상호 참조**를 만들었다 | 리뷰 |
| 5 | 절차를 세운 그 턴에 **재훑기를 편집한 파일 하나로만** 돌려 자매 파일을 놓쳤다 | 리뷰 |
| 6 | **이 표에 5행을 넣으면서** 위 헤딩("네 번")과 표 구조를 안 봤다 | 리뷰 |
| 7 | **가드를 만들면서** 그 가드가 닫은 체크박스를 `[ ]` 로 뒀다 | 리뷰 |

일곱 다 같은 형태다 — **내가 알아챈 자리만 고치고 그 클래스를 훑지 않았다.** 5번은 *고치기*
가 아니라 **확인하기**에서 났고, **6번은 이 절 자신에서 났다** — "한 자리만 고치는 버릇" 을
적는 표에 행을 더하면서 그 절의 헤딩을 안 고쳤고, 삽입한 빈 줄이 표를 끊어 5행이 파이프
문자가 드러난 문단으로 렌더링됐다. 3번 뒤에
*"조치로 앞 서술이 거짓이 되면 그것도 고치는 것까지가 조치다"* 라고 규칙을 적었는데
4번이 났다. **규칙을 쓰는 것으로는 안 고쳐진다** — 4번은 `grep -rn '20건'` 한 번이면
세 자리가 다 나왔다.

**그래서 절차로 바꾼다: 서술을 고칠 때는 그 문구를 grep 해서 나온 전부를 고친다.**
그리고 5번이 보탠 것 — **재훑기의 범위는 편집한 파일이 아니라 그 문구가 있을 수 있는 파일
전부다.** 좁히면 절차가 있어도 같은 결과가 난다.

### 검증은 부재가 아니라 결과로 한다

5R 이 잡은 두 건(`그 근거와 / 근거는`·`판정 대상 / 판정 대상이`)은 여러 줄 문장의 **중간부터**
치환하면서 앞줄 꼬리를 안 본 것이다. 그런데 더 나쁜 건 검증이었다 — **지운 문자열이 grep 에
없다**는 것으로 확인했다. 그것과 **남은 문장이 말이 된다**는 것은 다른 명제다.

- 고친 자리는 **문단을 출력해 읽는다**.
- 줄바꿈을 사이에 둔 **어절 중복을 기계적으로 검출**한다(앞줄 마지막 어절 == 뒷줄 첫 어절).
  이번에 돌리니 히트 13건이 전부 `});` 였다 — 산문 중복 0.

### 숫자를 어디에 쓸 수 있나 — 코드와 plan 의 선이 다르다

이 plan 은 `collectScanTargets` docstring 이 세운 "검증되지 않는 숫자는 적지 않는다" 를
코드 docstring 에 적용했다. 그런데 **plan 은 시점 기록**이고 코드 docstring 은 **지금 참인
서술**이라 같은 규칙을 그대로 쓸 수 없다.

- **코드 docstring**: 개수를 쓰지 않는다. 낡지 않는 예시와 세는 방법만.
- **plan·커밋 메시지**: 개수를 써도 되되 **날짜를 박는다**(`2026-09-04 실측 135 → 115`).
  `collectTsFiles` docstring 이 이미 그 형태다("2026-09-04 실측" 헤딩).
...
- [x] **가드 사각지대 — `.spec.ts` 의 낡은 캐스트** (배치 2 리뷰 W2·W3) — **가드로 닫음.**
      `findCastOffenders` 는 spec 을 **의도적으로 제외**한다(fixture 가 부분 객체를 캐스트하는
      것은 정당하다). 그런데 필드가 `| null` 로 넓혀지면 그 fixture 의 캐스트는 **불필요해지는데**
      그 가드가 구조적으로 못 본다 — 배치 1~3 에서 손으로 찾았다.

      > **이 항목의 근거를 내가 반증했다.** 원문은 *"캐스트가 겨누는 엔티티·필드를 역추적해야
      > 해서 텍스트 스캔으로는 부족하다. 배치마다 `grep` 으로 훑는 것이 현실적이다"* 였다.
      > 그런데 `widenedEntityFields` + `findStaleSpecCasts` 가 **정확히 그 역추적을 한다** —
      > 엔티티 AST 에서 `| null` 필드명을 뽑아 `.spec.ts` 의 캐스트와 대조한다.
      > "부족하다" 는 판단이 그 방법을 시도하기 전에 내려져 있었다.
      >
      > 체크박스가 `[ ]` 로 남아 있던 것을 리뷰 9R 이 잡았다 — **plan 에서 일곱 번째**
      > "한 자리만 고치기" 다. 가드를 만들면서 그 가드가 닫은 항목을 안 봤다.
