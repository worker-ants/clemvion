...
- [x] **`plan/**` 내부 상대링크 무결성 게이트 부재** — **해소 (2026-08-09)**.
      ...
      > 그 스코프에서 **9건을 정정**했는데, **누가 측정했느냐로 숫자가 갈린다**:
      > **가드가 보는 것은 8건**(이동 7 + spec 재편으로 stale 해진 `#anchor` 1)이고,
      > 나머지 1건은 ```markdown **펜스 안**의 잘못된 `../` 깊이라 `extractLinks` 가
      > 설계상 건너뛴다(plan 의 예시 스니펫은 없는 경로를 적을 자유가 있어야 한다).
      > 손으로 고쳤고 가드의 측정 대상이 아니다. 처음엔 "8건 전부 이동형" 이라고 적었는데
      > **둘 다 틀렸다** — 개수도 범주도(ai-review plan-coherence WARNING).
      > 남은 3건은 하위 그룹 폴더(`node-output-redesign/`)라 기존 면제 규칙에 걸린다.
