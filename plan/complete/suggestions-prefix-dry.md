...
- [x] 모듈 레벨 `NESTED_DRILL_SOURCES` 테이블: `{prefix, getSample, getSchema?, available?}`.
      ...
      - `$dataSource.`: 동일.
- [x] 4개 if-block → 단일 loop(prefix 매칭 + available 게이트 + buildNestedSuggestions).
      `$var.`(별도 패턴)·`$node[...]`(별도)는 유지. loop 위치는 기존 `$input.` 자리.
- [x] `$sourceItem.`/`$dataSource.` 미가용 시 root block fall-through 정확 보존.

## 테스트
- [x] 기존 use-expression-suggestions 테스트 전수 통과 = behavior 보증(신규 동작 없음).
