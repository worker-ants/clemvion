...
# 본 PR 은 spec 을 수정하지 않았다 — 구현이 기존 spec(_layout.md §2.2 "User Guide 는 slug 밖
# 유지")을 **위반하던 것을 spec 대로 되돌린** 정합화이기 때문. catch-all terminal 계약의 spec
# 문서 보강은 developer 권한 밖이라 durable 분리된 위임 plan 이 책임진다:
# plan/in-progress/spec-update-catch-all-terminal-contract.md (owner: project-planner)
spec_impact: none
---
