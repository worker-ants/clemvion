...
정정(취소선·근거는 그대로). 미체크 **29 → 27**.

> **수치를 한 번 틀렸다** (`20_36_01` requirement INFO 2): 처음에 "30 → 27" 이라 적었는데
> **다른 브랜치(#1205)의 수치**를 가져온 것이었다. 이 브랜치의 base 에서 실측하면 29 다
> (`git show origin/main:… | grep -c`). 브랜치가 병렬로 트래커를 고치는 동안 수치를 옮겨
> 적으면 이렇게 어긋난다 — **쓰는 그 브랜치에서 다시 세야 한다**.
