---
title: T1·T2 이동을 SoT 에 반영한다 — code 를 glob 으로 · §7 5파일 · 귀속 7곳
status: complete
owner: planner
...
표 위에 각주로 **세 갈래**를 적는다 — *"최초 setup/teardown 은 `chat-channel-binder.service.ts`,
PATCH 검증(비밀 차단 · 사후 부착 차단 · provider 불변)은 `chat-channel-input-rules.ts`,
회전·cleanup 은 `triggers.service.ts`"*.

> 두 갈래로만 적으려다 `--spec` INFO 2 가 **표의 세 번째 행(PATCH 거부 세 경우)의 소유자가
> 빠졌다**고 짚었다. 표가 세 행이면 각주도 세 갈래여야 한다 — **열거가 불완전하면 다음 사람이
> 빠진 것을 남은 둘 중 하나로 읽는다.** **헤더에 파일명을 박아 두면 행이 늘 때마다 낡는다** — 이번이
그 증거다.
...
- **`chat-channel-binder.service.ts` 의 `TriggersService:` 로그 리터럴** — 코드 사안(developer).
  (처음엔 소재를 `triggers.service.ts` 로 적었는데 **틀렸다** — 리터럴은 메서드와 함께
  옮겨갔고 지금은 전부 binder 안에 있다. `--spec` `20_47_56` INFO 1.)
...
`R-CC-21` 이고 **`R-CC-14` 는 결번**이다(재사용하지 않는다).

### 상위 원칙은 이미 있다 — 새로 만들지 않고 인용한다. **단 출처를 갈라 적는다**

`spec-impl-evidence.md` 의 **두 절**이 각각 다른 것을 말한다:

| 절 | 무엇을 말하나 |
|---|---|
| **§2.1 필드 정의** (`code` 행) | *"넓은 트리 글롭으로 가드만 통과시키는 것은 아무것도 가리키지 않는 것과 같다"* |
| **R-1** (`code:` 글로브 허용 vs 명시 파일만) | *"글로브 허용을 채택"* + **stale glob** 은 이 가드만으로 검출 불가(보완은 `/spec-coverage`) |

> **처음엔 이 둘을 한 절로 뭉쳐 적었다** — 앞 문구를 R-1 것이라고 썼는데 **실측상 §2.1(L81)이고
> R-1 은 L201 이다**(`--spec` `review/consistency/2026/09/11/20_47_56` W1).
> 한 grep 출력에서 인접한 두 줄을 보고 **주어를 갈라 적지 않은** 결과다.

그래서 `R-CC-22` 는 **둘을 각각 인용**한다: **R-1** 이 glob 자체를 허용하고, **§2.1** 이 그 허용의
한계를 그으며, 이번 결정(좁은 glob 3개 채택 · 통짜 `triggers/**` 기각)은 **그 한계선의 집행**이다.
즉 예외가 아니라 **적용례**다.

**문서 내부 선례도 함께 적는다** — 같은 파일의 `code:` 는 **최초부터** `modules/chat-channel/**`
를 glob 으로 갖고 있었다. `triggers/` 쪽만 명시 경로로 남아 있던 **비대칭을 없애는 것**이고,
새 관례를 들이는 것이 아니다.

R-1 이 경고하는 **stale glob** 위험도 적는다 — 좁은 glob 3개는 접두가 구체적이라 표면이 작지만
0은 아니다.

### "왜 여기만 glob 인가" 에 답하는 수치 — **정본 파서로 다시 쟀다**

| 측정 | 값 |
|---|---|
| `spec/**/*.md` 중 `code:` 보유 파일 | **137개** |
| `code:` entry 총합 | **748개** |
| 그중 `*` 없는(명시 경로) entry | **607개 = 81%** |

측정 방법: `review_guard._parse_frontmatter_code` 를 전 spec 에 돌려 entry 를 세었다
(기준 `77f4a88e5`).

> **처음엔 "633개 중 528개" 라고 썼다 — 낡은 값이다.** 그 수치는 내가 지어낸 게 아니라
> `review_guard.py` 의 `_MAX_GLOB_WILDCARDS` **주석에 박힌 과거 측정**을 그대로 옮긴 것이다.
> 정본 파서를 **실행**하니 748/607 이었다(`--spec` W2). *"정본 구현이 있으면 재현 말고 실행"* 이
> 이것이다 — **주석은 정본이 아니다.**
...
- [x] `/consistency-check --spec` **재실행** (`20_47_56`) **BLOCK: NO** · CRITICAL 0 · WARNING 2 ·
      RISK LOW. 1회차 지적 3건 전부 흡수 확인. 2회차 W1·W2 는 **둘 다 내 인용·수치 오류**라
      실측으로 정정했다(아래)
- [x] ① `code:` — glob 3줄 추가 + 중복 명시 2줄 제거. **정본 매처로 재측정: 미등재 0 · 무관
      파일 유입 0**
- [x] ② §7 — 5파일 추가 + `triggers.service.ts` 주석 정정 + **두 목록의 역할 분담 머리말**
      (`code:`=기계 술어 glob / §7=사람이 읽는 열거, 새 파일은 전자가 자동으로 덮고 후자는 손으로)
- [x] ③ 귀속 **7곳** (a)~(g)
- [x] `## Rationale` **`R-CC-22`** — `spec-impl-evidence.md` **R-1**(glob 허용)과
      **§2.1**(그 허용의 한계) 을 **각각** 인용 + 문서 내부 선례(`chat-channel/**`) + stale glob 위험
- [x] **반영 후 3축 스캔 재실행 — 축 1 대상 0곳** 확인. 축 2 잔존은 전부 검증된 비대상
      (`update()`·`remove`·`rotateBotToken` 귀속 + 새 §7 줄)
- [x] 트래커 항목 **2건 종결** + `code:` 축 해소 주석 + co-located 완료 주석 (`--spec` INFO 4)
- [x] `plan/complete/` 이동

## 2회차 `--spec` 이 잡은 **내 오류 둘** — 실측으로 정정했다

| # | 내가 쓴 것 | 실측 | 원인 |
|---|---|---|---|
| W1 | *"넓은 트리 글롭…" 문구를 **R-1** 이 못박는다* | 그 문구는 **§2.1(L81)**, R-1 은 **L201** | 한 grep 출력에서 **인접한 두 줄**을 보고 주어를 갈라 적지 않았다 |
| W2 | `code:` entry *"633개 중 528개가 `*` 없음"* | **748개 중 607개(81%)** | `review_guard.py` **주석에 박힌 과거 측정**을 그대로 옮겼다. **주석은 정본이 아니다** — 파서를 실행해야 한다 |

둘 다 *"인접한 두 기준이 있는 문서는 어느 줄이 내 대상인가가 실측의 일부"* 와 *"정본 구현이
있으면 재현 말고 실행"* 에 정확히 걸렸다. `R-CC-22` 에는 **고친 값과 측정 방법**이 들어갔다.
