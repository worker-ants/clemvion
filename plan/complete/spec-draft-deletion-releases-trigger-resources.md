...
묶으면 그 전제가 깨진다. 대가는 **커밋과 정리 사이에 프로세스가 죽으면 비밀이 남는 것**이다(D7-3).
...
보상(쓰기 쪽이 부재를 보면 되돌림)만으로 닫힌다. **provider 등록 쪽 절반은 여전히 미룬다**(D7-2).
...
번호는 `D7-N` 으로 적는다 — 같은 도메인의 `trigger-config-lost-update.md` 가 이미 «창 1~4» 를 쓴다.

| # | 창 | 남는 것 | 처분 |
|---|---|---|---|
| D7-1 | 외부 해제를 위한 열거(트랜잭션 **전**) 뒤, 부모 잠금 전에 생긴 트리거 | 그 트리거의 schedule job · provider 등록 · listener registry. **비밀은 D6 이 덮는다** | 발생 조건(부모 삭제와 트리거 생성이 동시)이 좁다. 계약 범위 밖 |
| D7-2 | 삭제 쪽 외부 해제 **뒤**, 행 삭제 **전에** 커밋된 동시 요청(provider 재등록 · schedule 재활성화) | 그 요청이 만든 provider 등록 · schedule job — `R` 이 `true` 라 D5 의 보상이 돌지 않는다 | **미룬다** — 위 5라운드 W1 의 provider 절반. 닫으려면 외부 해제도 행 삭제 **뒤로** 옮기고, 삭제 트랜잭션이 트리거 행을 `FOR UPDATE` 로 다시 읽어 최신 설정으로 teardown 해야 한다. 그러면 teardown 실패 시 «행은 없는데 provider 등록이 남는» 쪽으로 실패 방식이 바뀌고, §4.4 의 «정리 먼저, 락 나중» 순서도 다시 정해야 한다 — 별도 설계 검토 |
| D7-3 | 행 삭제 커밋과 비밀 정리 사이의 프로세스 종료 | 그 트리거들의 비밀 | D4 의 대가. 사후 정리(sweeper)의 대상 — 아래 «안 하는 것» |
...
frontmatter 의 **기존** `pending_plans:` 배열에 `plan/in-progress/spec-draft-nullable-notation-followups.md`
추가(이 문서는 이미 `partial`).
...
### S5. `spec/data-flow/10-triggers.md §1.4` — schedule 동기화 표: 기존 삭제 두 행에 포인터 + 행 추가

«Schedule 삭제» 행의 Trigger 칸 `—` 를 *"행 삭제 커밋 **뒤** 그 트리거의 `secret_store` 비밀 정리 — **미구현 (Planned)**
([트리거 목록 §4.3](../2-navigation/2-trigger-list.md#43-cascade-동작))"* 로, «Trigger(type='schedule') 직접 삭제» 행 끝에
*" · 비밀 정리는 행 삭제 커밋 뒤 ([트리거 목록 §4.3](../2-navigation/2-trigger-list.md#43-cascade-동작))"* 를 덧붙인다 —
새 행만 더하면 같은 표의 기존 삭제 행이 새 계약 밖에 남는다. 그리고 행 추가:
...
### S6. `spec/data-flow/11-workflow.md` — §2.1 매핑 표 + §3.1 FK 파급 표

**§2.1 Postgres 편집 흐름 표** — `workflow` 의 «버전 커밋» 행 다음에 추가(지금 이 표에는 워크플로 **삭제** 행 자체가 없다):
...
워크스페이스(S7)와 같은 깊이로 적는다 — 워크플로 삭제는 트랜잭션·잠금을 **새로** 두는 쪽이라 오히려 더 적을 것이 많다.

**§3.1 FK 파급 표** — `trigger` 행 끝에 덧붙임:
...
**§2 인터페이스 블록** — `exists` 다음에 선언을 더한다. §2.1·§5.3·§6 이 부르는데 선언에만 없었다(이 draft 이전
결함). 반환형은 구현 실측(`SecretResolverService.deleteByPrefix(prefix: string): Promise<number>`, 삭제 행 수)을 따른다:
...
**§5.3** — 제목 *"Trigger 삭제 시 — prefix 일괄 삭제"* 를 *"트리거 행이 없어질 때 — prefix 일괄 삭제"* 로
바꾸고(앵커 인용 0건 — 실측), 예시의 순서를 뒤집는다(예시는 트리거 화면 삭제 하나를 보인다):
...
**frontmatter** — `status: implemented` → `partial`, `pending_plans:` 필드를 **신설**해
`plan/in-progress/spec-draft-nullable-notation-followups.md` 하나를 둔다.
...
| `spec/2-navigation/3-schedule.md` 삭제 서술 | 트리거 cascade·`removeJob` 만 말하고 비밀은 말하지 않아 **틀린 문장이 없다**. 정리 계약은 §4.3 이 SoT 라 화면 문서에 순서를 복제하지 않는다(`1-workflow-list.md` 는 «비활성화» 오기를 고쳐야 해서 편집 대상이 됐다) |
| `spec/5-system/15-chat-channel.md` R8 «`teardownChannel()` (또는 `TriggersService.remove`) 시 unregister» | 괄호가 경로 하나를 예로 든다. listener registry 해제 호출부가 **실제로 늘어난 뒤** 그 실측과 함께 넓힌다 — DRT-2 에 후속으로 적었다 |
| `spec/conventions/spec-impl-evidence.md §3.1` — `implemented → partial` 하향 전이 규칙 미명문 | 가드가 방향을 강제하지 않고 선례가 있다. 규약 문면 보강은 이 draft 의 관심사가 아니다 |
| `1-workflow-list.md` `pending_plans` 의 `plan/complete/workflow-duplicate-nodes-edges.md` | dangling 이 아니다 — 규약이 `complete/` 경로도 실존이면 허용한다(`spec-impl-evidence.md` 필드 표). 남은 in-progress plan 이 있어 승격 의무도 없다 |

## 트래커 반영 (같은 PR)

라벨 `DRT-*` 는 이 draft 전용이다 — 트래커에 이미 `T1`/`T2`(chat-channel-binder PR 분할)가 있어
겹치지 않는 접두를 골랐다(`DRT-` 는 `spec/`·`plan/`·`.claude/docs`·`.claude/skills`·`CLAUDE.md`·`codebase/`
전체에 0건 — 2차 `--spec` 뒤 실측). 트래커 쪽 문장은 D·S 라벨도 **이 draft 파일명과 함께** 인용한다.

| # | 자리 | 반영 |
|---|---|---|
| DRT-1 | `spec-draft-nullable-notation-followups.md` developer 항목(트리거 락 후속) 표의 **8·9** | «→ `spec-draft-deletion-releases-trigger-resources.md` D5 로 흡수 — 구현은 DRT-2» 로 전환 표기 |
| DRT-2 | 같은 트래커에 **developer 항목 신설** | D1·D3~D6 구현: 네 삭제 경로 · `TriggersService.remove()` 순서 반전(+ 실패 로그 문구) · 쓰기 경로 보상(호출부는 `rewriteTriggerConfigLocked` 호출부와 락 밖 비밀 쓰기로 **전수** 열거) · e2e(워크플로 삭제 후 비밀 0행 · 재진입으로 끊은 동시 회전의 보상). **착수 첫 판단은 협력자의 모듈 위치다** — `WorkflowsModule → TriggersModule` 간선은 `TriggersModule → SchedulesModule → ExecutionEngineModule → WebsocketModule → WorkflowsModule` 과 순환을 만든다. 같은 트래커의 `#676`(`e827ed2a7`)은 `chat-channel→triggers` 역방향 의존을 **일부러 끊어** `forwardRef` 순환을 없앤 선례라, 새 간선을 `forwardRef` 로 때우지 말고 위치부터 정한다. 구현 뒤 planner 후속: `15-chat-channel.md` R8 의 «(또는 `TriggersService.remove`)» 괄호를 실제 해제 호출부로 넓힌다. 부수 주의 둘: 커밋 뒤 정리 단계는 이미 지워진 `workspace_id` 를 참조하는 감사 행을 남기지 않는다(FK 위반) · 새 `deleteByPrefix` 호출부도 prefix 를 UUID 로 조립한다(LIKE 메타문자 거부 불변식의 전제) |
| DRT-3 | 같은 트래커 planner 항목 «없는 메서드 `TriggersService.delete()` 가 세 곳» | 1·2 를 S8·S9 로 닫고, 3 은 기존 무조치 처분 → 항목 종결 |

## 이 draft 가 **안** 하는 것

- **구현** — developer 턴이 별 PR 로 한다(DRT-2). 모듈 배치는 구현 설계 문제라 spec 에 적지 않는다.
- **사후 정리(sweeper)** — 이미 남은 고아 `secret_store` row · provider 등록 · schedule job, 그리고 D7-3
  이 남기는 비밀을 치우는 일. 구현 뒤 별도 판단.
- **D7-2(provider 등록 절반)** — 외부 해제를 행 삭제 뒤로 옮기는 설계. 별도 설계 검토.
...
주어로 삼은 자리를 전수(§2.1 표 · §5.3 제목·예시 · §6 제목·두 문단 · §R4)로 고친다. §R4 제목은
주어가 경로가 아니라 «FK 를 두지 않았다» 는 사실이라 그대로 둔다(비대상 표).
...
| # | 지적 | 처분 |
|---|---|---|
| W1 | developer 구현 트래커 미등재 · `status: implemented` 문서에 `pending_plans` 없음 | DRT-2 신설. 미구현 약속을 담게 되는 `secret-store.md` 는 `partial` + `pending_plans`(S10). `1-data-model.md` 는 현재 사실만 적고 계약을 §4.3 에 위임해 `implemented` 유지(S8). data-flow 문서는 frontmatter 가 없어 문중 **미구현 (Planned)** 표기(해당 디렉토리 관례) |
| W2 | 착수 계기였던 트래커 항목 8 과 교차 참조 없음 | DRT-1 |
| W3 | `12-workspace.md §2.1` 매핑 표에 `secret_store` 행 없음 | S7 |
| W4 | S2 제목이 «표 셀에 덧붙임» 인데 내용은 blockquote | S2 제목을 «표 다음에 문단 추가» 로 |

### 2차 `--spec` 처분 (`review/consistency/2026/09/17/17_05_02`)

| # | 지적 | 처분 |
|---|---|---|
| CRITICAL | 트래커 반영 라벨 `T1`~`T3` 가 대상 트래커의 기존 `T1`/`T2`(`#676` 계열)와 충돌 | `DRT-1`~`3` 으로 교체 — **후보 접두를 저장소 전체에서 0건으로 먼저 잰 뒤** 골랐다(«트래커 반영» 절 머리). 같은 클래스(`D-*`→`CV-*`)를 두 번 겪고도 또 되풀이했다 |
| W1 | `secret-store.md §5.3` 제목이 여전히 «Trigger 삭제 시» | S10 에 제목 변경 추가. Rationale 의 «전수» 목록에 제목을 명시 |
| W2 | 워크플로 삭제의 새 트랜잭션·잠금이 `11-workflow.md` 에 한 줄 포인터뿐 — 워크스페이스(S7)와 비대칭 | S6 에 §2.1 매핑 행 추가 |
| W3 | DRT-2 의 모듈 순환 서술이 같은 트래커의 `#676` 선례를 인용하지 않음 | DRT-2 에 인용하고 «착수 첫 판단 = 협력자 위치» 로 적었다 |
| INFO 1·6 | 커밋 뒤 정리 단계의 감사 FK 함정 · 새 `deleteByPrefix` 호출부의 UUID prefix | DRT-2 의 부수 주의로 |
| INFO 5 | `pending_plans` 신설/추가 구분 모호 | S1(기존 배열)·S10(신설)로 구분 |
| INFO 7 | 멀티라인 링크 앵커 사각지대 | 반영 시 새 링크를 한 줄로 두고 `spec-link-integrity` 로 확인 |
| INFO 2~4 | rationale 연속성 확인(조치 불요) | — |

### 3차 `--spec` 처분 (`review/consistency/2026/09/17/17_20_54`, **BLOCK: NO**)

**정지 규칙(결과를 보기 전에 선언)**: Critical 0 이면 WARNING 을 반영·기록하고 재실행하지 않는다 — 반영이 D1~D7
결정을 바꾸면 재실행한다. 아래 반영은 전부 문면·포인터·선언 보강이라 결정을 바꾸지 않는다.

| # | 지적 | 처분 |
|---|---|---|
| W1 | `10-triggers.md §1.4` 기존 «Schedule 삭제» 행이 새 계약 밖 | S5 에 기존 삭제 두 행 포인터 추가 |
| W2 | `secret-store.md §2` 인터페이스 선언에 `deleteByPrefix` 없음(선행 결함) | S10 에 선언 추가 — 반환형은 checker 제안(`Promise<void>`)이 아니라 구현 실측 `Promise<number>` |
| W3 | D7 «창 1/2/3» 이 기존 «창 1~4» 와 형태 충돌 | `D7-1`~`3` (`spec/`·`plan/` 0건 실측), 접두 없던 Rationale 문장 포함 |
| INFO 1 | `3-schedule.md` | 비대상 표 — 틀린 문장이 없다 |
| INFO 2 | `15-chat-channel.md` R8 괄호가 좁다 | 비대상 표 + DRT-2 에 후속 |
| INFO 3 | 하향 전이 규칙 미명문 | 비대상 표 |
| INFO 4 | `1-workflow-list.md` `pending_plans` 의 `complete/` 항목 | 비대상 표 — 규약상 허용 |

### 반영 뒤 자체 대조

`spec/` 반영 diff 를 draft 실측표와 줄 단위로 맞대 보니 §4.3 문단의 *"그 전까지는 트리거 화면 삭제만 정리하고"* 가
실측보다 넓었다 — 스케줄 화면 삭제도 지금 schedule job 은 해제한다(«삭제 경로 넷» 표 둘째 행). *"트리거 화면 삭제만
네 자원을 모두 정리하고(스케줄 화면 삭제는 schedule job 만)"* 로 좁혔다. 결정은 바뀌지 않는다.

검증: `spec-link-integrity` + `spec-status-lifecycle` GREEN 171. 새 앵커가 실제로 검사되는지 뮤턴트로 확인했다 —
`secret-store.md §5.3` 과 `12-workspace.md §1.10` 의 새 링크 앵커를 하나씩 깨면 각각 **1 failed**(예측 RED, 실측 RED).
