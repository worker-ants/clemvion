...
> **기획자 행도 함께 고친다** (`11_28_04` cross_spec WARNING). 실측: 지금 `CLAUDE.md` Skill 표의
> 네 축은 각 역할 SKILL.md 「경로별 권한」 표와 **1:1 로 대응**한다. 개발자 행만 넓히고 기획자
> 행을 두면 `project-planner/SKILL.md` 는 "거버넌스 문서 Read/Write" 라 적는데 최상위 요약 표는
> `spec/**`·`plan/**` 만 보여 준다 — **표만 읽는 사람에게는 여전히 빈칸**이고, 그것이 이 항목이
> 고치려는 결함 그 자체다. 세 문서를 같은 커밋에서 맞춘다.
...
**그리고 `project-planner/SKILL.md` 「경로별 권한」 표에도 대응 행** — 권한을 *갖는* 쪽 문서에
그 권한이 없으면 이 항목이 고치려는 결함(*"역할이 자기 권한을 자기 문서에서 확인할 수 없다"*)을
planner 축에 그대로 재생산한다. 선례 `051c7e7c1` 도 권한을 갖는 쪽(`developer/SKILL.md`)을
직접 갱신했다 (`review/consistency/2026/09/08/11_14_39` — cross_spec·convention_compliance 두
checker 가 독립 지적):

```markdown
| `.claude/docs/**`, `.claude/skills/**/SKILL.md`, `CLAUDE.md` | Read/Write — 거버넌스 문서(역할 정의·워크플로 규약). harness **실행물**(`hooks/`·`tools/`·`tests/`)은 `developer` 소유라 대상 아님 |
```
...
> 실측 (2026-09-08, `grep -rn "r-2-webhook-hmac-secret" spec`): 인입은
> `15-chat-channel.md:610` **1건뿐**. 이 수치를 커밋 diff/PR 본문에 남겨 두어, 나중에 1건을
>넘으면 즉시 드러나게 한다 (`11_14_39` rationale_continuity INFO).
...
> **게이트는 파일 존재만 본다 — 항목 매칭은 안 본다.** `spec-pending-plan-existence.test.ts`
> 를 열어 확인했다: `fs.existsSync(in-progress) || fs.existsSync(complete)` 뿐이다
> (`11_14_39` INFO#2). 그러니 *"그 plan 이 sort/order 를 실제로 책임진다"* 는 **기계가 보증하지
> 않는다** — 자매 트래커에 developer 항목을 신설하는 위 단계는 사람이 지켜야 하는 의무이고,
> 빠뜨리면 `pending_plans` 가 아무것도 가리키지 않은 채 초록이 된다.
...
[`1-auth.md §3.2`](../../spec/5-system/1-auth.md#32-리소스별-권한-매트릭스) 권한 매트릭스를 근거로 인용한다.
...
[`15-chat-channel.md §5.4.2`](../../spec/5-system/15-chat-channel.md#542-응답-dto-derived-필드--hasbottoken)
...
```markdown
> **표기 주의 — 아래는 API 응답/엔티티 프로퍼티의 camelCase 다.** 위 §2.1 표의 DB 컬럼
> (snake_case)과 1:1 대응한다(`password_hash` ↔ `passwordHash` 등). 시행 코드
> (`USER_SECRET_KEYS`)가 camelCase 를 쓰므로 그 표기를 그대로 옮겨 대조 가능하게 둔다.
> (병기 선례: [`2-trigger-list.md §2.1`](./2-navigation/2-trigger-list.md).)
>
...
| 안 | 기각/채택 사유 |
|---|---|
| 컬럼 `select: false` | **기각 — fail-silent.** 그 7컬럼을 읽는 내부 경로(로그인 검증·토큰 소비·복구 코드 대조)가 값을 **직접 소비**하므로 조용히 `undefined` 를 받는다. 실측: `user.entity.ts` 에 `select: false`·`@Exclude()` **0건**(2026-09-06) |
...
> `select: false` 로 선언돼 있고(V107, `notification.entity.ts`), 그것이 옳다 — 그 컬럼의
> 유일한 내부 소비자(`findByBackgroundRun`)가 **WHERE 절에만** 쓰고 값을 읽지 않기 때문이다.
>
> | 소비 패턴 | 처분 | 사례 |
> |---|---|---|
> | 내부 경로가 **값을 읽는다** | 응답 경계에서 지운다 (`select: false` 금지) | `User` 민감 7컬럼 · `Trigger.notification_secret_v2`(로테이션 스윕이 승격에 쓴다 — `triggers.service.ts`) |
> | 내부 경로가 **WHERE 절에만 쓴다** | `select: false` 유효 | `Notification.background_run_id` |
>
...
## A-6. "노출 창이 아직 닫혀 있지 않다" 두 자리가 이미 거짓이다 (스코프 추가)

**등재된 planner 항목이고**(자매 트래커 — *"노출 창이 아직 닫혀 있지 않다 서술이 낡는다 —
`secret-store.md §1` 과 `14-external-interaction-api.md §7.1` 두 곳"*, 2026-09-05 등재),
**A-5 가 여는 바로 그 파일**에 있다. 넣지 않으면 편집 직후 같은 파일 안에 *새 규범 문장*과
*이미 거짓인 유출 경고*가 나란히 남는다 (`11_14_39` plan_coherence WARNING).

두 자리가 현재형으로 이렇게 적는다:

| 파일 | 문장 |
|---|---|
| `spec/conventions/secret-store.md` §1 (69~78행) | *"**노출 창은 아직 설계대로 닫혀 있지 않다.** … 현행 구현은 `GET/POST/PATCH /api/triggers` 와 `GET /api/schedules` … 응답에도 이 컬럼을 그대로 싣는다"* |
| `spec/5-system/14-external-interaction-api.md` §7.1 (934~936행) | *"**현재 이 컬럼은 응답에도 나간다** … 이는 **미해결 결함**"* |

**둘 다 지금 거짓이다.** `#1291`(`bfa124920`)이 그 창을 닫았다 — 실측:
`triggers.service.ts:99` 의 `TRIGGER_RESPONSE_STRIP_COLUMNS` 가 `notificationSecretV2` 를 담고
186행 루프가 응답 경계에서 지운다. 스케줄 조인 축은 `shared/testing/schedule-trigger-ref.ts` 가
같은 목록으로 단언한다.

→ 두 곳 모두 §7.1 이 이미 쓰는 "정정 이력" 패턴을 준용한다 — 원문을 취소선으로 남기고
`> **정정 (2026-09-08)**: 이 창은 #1291 (`bfa124920`) 로 닫혔다 — 응답 경계 스트립
(`TRIGGER_RESPONSE_STRIP_COLUMNS`). 저장 형태 예외(평문 보관)는 그대로 유효하다.` 를 붙인다.
**규범(§1.1)은 건드리지 않는다** — 닫혔다고 금지가 사라지는 것이 아니다.

> **전수 확인** (자매 항목이 2026-09-06 에 이미 수행): `grep -rn "노출 창\|응답에도 나간다\|미해결 결함" spec/`
> → 이 창을 서술하는 자리는 위 **둘뿐**이다. 나머지 매치는 다른 맥락(`6-config.md` 평문 hide
> 정책 · `1-auth.md` 초대 만료). 적용 시점에 재확인한다.
...
- [x] 자매 트래커(`spec-draft-nullable-notation-followups.md`) 체크박스 **9개 전부** 플립 —
      **숫자로 종료 판단 금지, 아래 열거로 확인** (`11_28_04` plan_coherence WARNING).
      ...
      | draft 항목 | 자매 트래커 bullet (실측 2026-09-08) |
      |---|---|
      | A-1 | `CLAUDE.md` 에 harness 수정 권한 조항이 없다 |
      | A-2-1 | `2-trigger-list.md` R-2 가 폐기된 설계를… |
      | A-2-2 | `2-trigger-list.md` frontmatter `status` 가 본문의 자백과 모순 |
      | A-2-3 | 트리거 drawer 의 "새 인증 설정 만들기" 링크가 editor 에게 dead-end |
      | A-2-4 | `2-trigger-list.md:106` botToken 행의 자기모순 |
      | A-3 | 도메인 세부 에러 코드의 표현 방식을 정식화한다 |
      | A-4 | 신규 검출 3축을 §5.4 「검증 층」과 `code:` 에 등재 |
      | A-5 | `User` 민감 7컬럼의 응답 노출 금지를 규약 문장으로 |
      | A-6 | "노출 창이 아직 닫혀 있지 않다" 서술이 낡는다 |

      > **줄 번호를 적지 않는다** — 이 표의 원 지적(`11_28_04` plan_coherence)이 인용한 라인
