...
- [x] spec 갱신 (Rationale 추가) — 2026-05-16 `plan/complete/spec-draft-cafe24-spec-sync.md` 에서 `spec/2-navigation/4-integration.md ## Rationale "Cafe24 install_token mismatch 회복 흐름 — 보안 전제"` 항목으로 처리
- [x] backend 구현 + 테스트 — commit `30f68b39` (PR #20-something) + PR #67 SEC H-2 (RECOVERY_CANDIDATE_LIMIT)
- [x] frontend 구현 — `meta.publicAppAvailable` 관련은 별도 commit 으로 머지 완료
- [x] PR 머지 후 본 plan 을 `plan/complete/` 로 이동 — 본 시점에 다음 plan PR (`cafe24-spec-sync-e2a8b9`) 머지 후 이동 예정
