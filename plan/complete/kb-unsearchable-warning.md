...
- 범위: **경고 노출만** (A 백엔드 신호 + B 목록 카드 경고). `update()` 자동 재임베딩/차단은 **follow-up 분리** → `plan/complete/kb-model-change-reembed-followup.md` (PR 머지 전 신설 예정).
...
- [x] 완료 시 5-knowledge-base.md status partial→implemented 복귀 + 양 spec pending_plans 에서 본 plan 제거 — 5-knowledge-base 졸업 #513, 9-rag-search/8-embedding-pipeline pending_plans 제거 + 8-embedding-pipeline 졸업(pending_plans 공백→implemented) 본 정리에서 완료. 근본원인(모델 변경 시 자동 재임베딩 미트리거)은 별도 `kb-model-change-reembed-followup.md` 로 분리 추적.
