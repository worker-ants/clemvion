...
> ⚠ **DEPRECATED (2026-05-08)** — 본 plan 의 "재작성 작업 항목 / 미해결 설계 질문 / ai-review 백로그" 단락은 옛 `toolNodeIds` / `toolOverrides` 디자인의 부분 복원을 전제로 작성됐다. 사용자 결정으로 **요구사항부터 전면 재설계** 가 확정되어 본 문서의 가이드는 더 이상 유효하지 않다. 새 도구 연결 디자인은 별도 PRD/Spec/plan 트랙에서 처음부터 시작한다 — 본 문서는 (a) 제거 범위 / 데이터 호환성 / 영향 받지 않는 영역의 history 기록, (b) ai-review 백로그 (`review/2026-05-06_13-01-52`) 위치 포인터로만 참고. 재설계 시 이 문서의 옵션·체크리스트를 그대로 따르지 말 것.
...
이 plan 문서는 제거된 영역을 추적하고 재작성 시 누락이 없도록 하기 위한 추적 문서다. 새 도구 연결 디자인이 결정되어 모든 작업이 끝난 순간 본 문서를 `plan/complete/` 로 옮긴다.

## 현재 상태 (2026-05-08)

신규 디자인 결정·구현은 미진행. 하단 "남은 작업" 의 모든 항목이 trigger 대기 상태다. 제거 자체는 이미 적용됐으며 legacy DB 데이터 호환만 가드 중.
...
새 도구 연결 디자인이 결정되어 트리거되는 순간 처리한다. 모든 항목이 미진행 상태다.

### 재작성 작업 항목

- 새 스키마 필드 추가 (`backend/src/nodes/ai/ai-agent/ai-agent.schema.ts`) — 기존 두 필드를 그대로 복원할지, 새 디자인에 맞춰 새 이름·구조로 정의할지 결정.
...
### 재작성 시 함께 검토할 ai-review 백로그

도구 연결 재작성과 직접 맞물리는 ai-review 결과들이다. 본 plan 으로 이관해 누락 방지. 자세한 위치는 `review/2026-05-06_13-01-52/SUMMARY.md` + `review/2026-05-06_13-01-52/RESOLUTION.md` 참조.
