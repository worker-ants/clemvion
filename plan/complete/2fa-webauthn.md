...
- [x] `spec/1-data-model.md` §1 ERD 다이어그램 — `WebAuthnCredential` (User 1:N) 관계 반영
...
- [x] `V057__webauthn_credentials_and_recovery.sql` — `webauthn_credential` 테이블 신설 + `user.webauthn_recovery_codes` 컬럼 추가 + 인덱스 2개. 단일 트랜잭션 (CONCURRENTLY 없음) → `.conf` 불필요.
...
- [x] `V058__login_history_webauthn_failed_event.sql` — `chk_login_history_event` CHECK 제약을 DROP + ADD (단일 statement, NOT VALID/VALIDATE 패턴 불요 — 신규 enum 값은 기존 row 에 위배 없음). 별도 마이그레이션으로 분리해 V057 의 롤백 단위와 격리.

### 4. 백엔드 구현 (TDD)

- [x] `@simplewebauthn/server` 의존성 추가 (`codebase/backend/package.json`, lock 갱신)
- [x] `codebase/backend/src/common/config/webauthn.config.ts` 신설 — `registerAs('webauthn', () => ({ rpID, rpName, origins: string[] }))`. 환경변수 `WEBAUTHN_RP_ID` / `WEBAUTHN_RP_NAME` / `WEBAUTHN_ORIGIN` (콤마 구분). 누락 시 `FRONTEND_URL` 의 hostname/origin 으로 폴백 + warn 로그
- [x] `codebase/backend/src/modules/auth/entities/webauthn-credential.entity.ts` — TypeORM entity, `app.module.ts` 의 entities 배열에 추가
- [x] `codebase/backend/src/modules/auth/webauthn.service.ts` — `WebAuthnService`
...
- [x] `codebase/backend/src/modules/auth/dto/webauthn.dto.ts` — request DTO + class-validator
- [x] `codebase/backend/src/modules/auth/dto/responses/webauthn-response.dto.ts` — response DTO (`WebAuthnRegisterOptionsDto`, `WebAuthnRegisterVerifyDto`, `WebAuthnAuthOptionsDto`, `WebAuthnCredentialDto`, `WebAuthnRecoveryCodesDto`). `swagger.md §5-1·§5-4` 준수
- [x] `auth.controller.ts` 에 endpoints 추가 (§spec 5-system/1-auth.md §5 와 1:1 대응)
...
- [x] `auth.service.ts` `/auth/login` 응답 진화
...
- [x] LoginHistory `event` enum 에 `webauthn_failed` 추가 + `failure_reason` 으로 세분화 (`WEBAUTHN_INVALID`, `WEBAUTHN_COUNTER_REGRESSION`)
- [x] 단위 테스트
...
- [x] 통합/e2e 테스트 `webauthn-2fa.e2e-spec.ts`
...
- [x] `@simplewebauthn/browser` 의존성 추가 (`codebase/frontend/package.json`)
- [x] `lib/api/auth.ts` 에 webauthn wrapper 추가 (`registerOptions/verify`, `authenticateOptions/verify`, `recovery`, credential 관리)
- [x] login 응답 타입 진화: `LoginResponseData` 에 `{ requires2fa, methods, challengeToken, requiresTotp? }` 추가, 기존 `requiresTotp` 분기는 호환 처리
- [x] `app/(main)/profile/security/page.tsx` 에 "Passkey · 보안 키" 카드 추가
...
- [x] `components/auth/login-form.tsx` 에서 `requires2fa` + methods 분기
...
- [x] i18n (ko/en) — `profile.security.webauthn.*`, `auth.twoFactor.webauthn.*`, `auth.login.webauthn.*`. i18n key 추가 시 ko↔en parity 테스트 통과 확인 (`npm test -- i18n`)
- [x] 단위 테스트 (RTL + jest) — security page 카드 동작, login-form WebAuthn 분기. `@simplewebauthn/browser` 는 mock
- [x] e2e (Playwright Virtual Authenticator) — Chrome 기준. mobile Safari 는 manual follow-up

### 6. 매뉴얼

- [x] `codebase/frontend/src/content/docs/07-workspace-and-team/` 하위 보안 가이드 (없으면 신규 `security-2fa.md`) 에 Passkey 등록/사용/복구 코드 추가 + WebAuthn 우선 정책·복구 코드 별도 발급 명시. 기존 섹션 안에 파일을 추가하므로 `SECTION_LABELS_BY_LOCALE` 재등록은 불요. 신규 섹션 디렉토리를 만들 경우 `locale.test.ts` coverage 통과 필요

### 7. REVIEW

- [x] `consistency-check --spec` 실행 (spec 수정 직전 의무) — 1차 BLOCK 해소 후 재실행하여 PASS 확보 (Critical 4 → 0)
- [x] `consistency-check --impl-prep` 실행 (구현 직전 의무) — V057 번호 점유 확인 포함
- [x] `ai-review` 실행 → Security 중심 (counter 검증, replay 방어, rpID/origin 정합성, 복구 코드 fallback, challenge JWT 만료, anonymous 흐름 차단)

### 8. Follow-up (본 PR 범위 밖 / 별 PR)

본 PR 의 scope 를 벗어나는 후속 작업은 별 plan 으로 분리해 추적한다 — 본 plan 은 그
별 plan 으로 위임한 시점에 `complete/` 로 이동한다.

- [x] 별 plan 으로 분리: [`plan/in-progress/2fa-webauthn-followups.md`](./2fa-webauthn-followups.md)
  - `requiresTotp` deprecated 제거 (두 마이너 버전 + 신규 프론트엔드 배포 확인 후)
  - 백엔드 e2e (`webauthn-2fa.e2e-spec.ts`) — SoftWebAuthnDevice 합성 helper
  - mobile Safari 실기기 수동 검증 + BrowserStack 통합
  - WebAuthn-only 계정의 비밀번호 재설정 흐름 검토
  - `forgotPassword`·`checkEmail` 응답 wrap 누락 정리 (본 PR scope 외 기존 버그)
  - `SessionListDto.data` / `LoginHistoryPageDto.data` 필드명 → `items` 개명
  - WebAuthn verify 트랜잭션 + pessimistic lock 강화 (ai-review W-8)
  - `AuthModule` 분리 (WebAuthnModule 서브모듈)
  - `LoginChallengeDto` union 분리 (ai-review W-9)
  - V058 NOT VALID 2-step 마이그레이션 (ai-review W-23)
