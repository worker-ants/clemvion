...
# Background context-key race — 완료 기록 (PR #406)

> 완료된 본체 작업 + ai-review INFO 결정 기록. 잔여 선택적 backlog 는
> `plan/in-progress/background-context-key-followups.md` 로 분리 (split, 2026-06-01).
...
ai-review(`review/code/2026/05/31/22_50_09/`)·자체 분석 결과.
...
behavior+spec 결정이 필요해 본 PR 범위 밖으로 둔다(followups plan 참조).
...
| #3 | `createContext` 5번째 위치 파라미터(중간 기본값 명시 불편) | **deferred** — options-bag 리팩터는 광범위, 즉시 가치 낮음. followups backlog. |
...
| #7 | `setStructuredOutput`/`setEngineResolvedConfig` context 미존재 시 silent no-op | **deferred** — 진단 warn 추가는 기존 동작 변경. followups backlog. |
