...
## 미구현 항목
- [x] §2.5.1 "Use Default Output" 기본값 설정 UI — 정책 select 에서 `Use Default Output` 선택 시 조건부 JSON 에디터 + 구문 강조/유효성 검증 + "Reset to Type Default" 버튼. 현재 `node-settings-panel.tsx` 의 Error Handling 은 단일 select 만 렌더링.
- [x] §2.4 Retry 설정 입력 UI — `Retry` 정책 선택 시 `maxRetries`/`retryInterval` 입력 필드. 현재 select 옵션만 존재하고 입력 UI 없음.
